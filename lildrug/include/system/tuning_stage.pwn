...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == 4646)
    {
        if(response)
        {
            ...
            switch(listitem+1)
            {
                case 1:if(!GetOwnableCarData(id, OC_COMFORT_PL)) return SendClientMessage(playerid, -1, "     {FFFF00 break; }Stage 1");
                case 2:if(!GetOwnableCarData(id, OC_SPORT)) return SendClientMessage(playerid, -1, "     {FFFF00 break; }Stage 2");
                case 3:if(!GetOwnableCarData(id, OC_SPORT_PL)) return SendClientMessage(playerid, -1, "     {FFFF00 break; }Stage 3");
                case 4:if(!GetOwnableCarData(id, OC_DRIFT)) return SendClientMessage(playerid, -1, "     {FFFF00 break; }Drift");
            }
