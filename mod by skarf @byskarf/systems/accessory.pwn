...
{
    button_PTD[playerid][0] = CreatePlayerTextDraw(playerid, 309.4163, 406.8438, "ЦЕНА:10000000"); // пусто
...
stock CreateTextDrawButtonBR()
{
    ...
    button_TD[11] = TextDrawCreate(321.9993, 391.0805, "Далее"); // пусто
    ...
    button_TD[12] = TextDrawCreate(260.1662, 391.0807, "Купить"); // пусто
    ...
    if(clickedid == button_TD[11]) //выход
{
    ...
    if(clickedid == button_TD[12]) // купить
    {
        ...
        else ShowNotificationNew(playerid,  2, 6, 0, 0, "У вас недостаточно денег", " ");

    }
	if(clickedid == acss_TD[1]) // Выход
{
...
if(clickedid == acss_TD[2]) // Сохранить
{
    for(new i; i < sizeof acss_TD; i++)
    {
        TextDrawHideForPlayer(playerid, acss_TD[i]);
    ...
    CancelSelectTextDraw(playerid); 
    TogglePlayerControllable(playerid, true);
    ShowNotification(playerid, 3, 5, 0, 0, "Аксессуар успешно сохранен", " ");
}
...
stock EditAccessory(playerid, database)
{
	...
	if(rows)
	{
		...
		// Показываем все нужные TextDraw'ы
for(new i; i < 11; i ++) TextDrawShowForPlayer(playerid, acss_TD[i]);
TextDrawShowForPlayer(playerid, acss_TD[17]);
TextDrawShowForPlayer(playerid, acss_TD[24]); // <-- Добавлено
TextDrawShowForPlayer(playerid, acss_TD[25]); // <-- Добавлено
...
stock CreateEditAccessoryTD()//автор редактора не welsi
{
	...
	acss_TD[0] = TextDrawCreate(35.0000, 18.0000, "gui:bracstext");
	...
	//КНОПКА "ВЫХОД"
	acss_TD[1] = TextDrawCreate(600.0000, 0.1, "gui:bracsbtnexit");
	...
	//КНОПКА "СОХРАНИТЬ"
	acss_TD[2] = TextDrawCreate(453.0000, 355.0000, "gui:bracssave");
...
//-НЕ НАЖАТЫЕ КНОПКИ
   	//КНОПКА "ВЛЕВО/ВПРАВО"
	acss_TD[3] = TextDrawCreate(35.0000, 65.0000, "gui:bracsn1");
	...
	//КНОПКА "ВВЕРХ/ВНИЗ"
	acss_TD[4] = TextDrawCreate(35.0000, 110.0000, "gui:bracsn2");
	...
	acss_TD[5] = TextDrawCreate(35.0000, 155.0000, "gui:bracsn3");
	...
	//КНОПКА "МАСШТАБ"
	acss_TD[6] = TextDrawCreate(35.0000, 200.0000, "gui:bracsn4");
	...
	acss_TD[7] = TextDrawCreate(35.0000, 245.0000, "gui:bracsn5");
	...
	acss_TD[8] = TextDrawCreate(35.0000, 290.0000, "gui:bracsn6");
	...
	acss_TD[9] = TextDrawCreate(35.0000, 335.0000, "gui:bracsn7");
...
//-НАЖАТЫЕ КНОПКИ
	//КНОПКА "ВЛЕВО/ВПРАВО"
	acss_TD[10] = TextDrawCreate(35.0000, 65.0000, "gui:bracsa1");
	...
	//КНОПКА "ВВЕРХ/ВНИЗ"
	acss_TD[11] = TextDrawCreate(35.0000, 110.0000, "gui:bracsa2");
	...
	//КНОПКА "ОТ СЕБЯ/НА СЕБЯ"
	acss_TD[12] = TextDrawCreate(35.0000, 155.0000, "gui:bracsa3");
	...
	//КНОПКА "МАСШТАБ"
	acss_TD[13] = TextDrawCreate(35.0000, 200.0000, "gui:bracsa4");
	...
	//КНОПКА "ПОВОРОТ ПО ОСИ X"
	acss_TD[14] = TextDrawCreate(35.0000, 245.0000, "gui:bracsa5");
	...
	//КНОПКА "ПОВОРОТ ПО ОСИ Y"
	acss_TD[15] = TextDrawCreate(35.0000, 290.0000, "gui:bracsa6");
	...
	//КНОПКА "ПОВОРОТ ПО ОСИ Z"
	acss_TD[16] = TextDrawCreate(35.0000, 335.0000, "gui:bracsa7");
	...
	acss_TD[17] = TextDrawCreate(430.0000, 100.0000, "gui:bracsm1");
	...
	//РЕДАКТОР "ВВЕРХ/ВНИЗ"
	acss_TD[18] = TextDrawCreate(430.0000, 100.0000, "gui:bracsm2");
	...
	acss_TD[19] = TextDrawCreate(430.0000, 100.0000, "gui:bracsm3");
	...
	//РЕДАКТОР "МАСШТАБ"
	acss_TD[20] = TextDrawCreate(430.0000, 100.0000, "gui:bracsm4");
	...
	acss_TD[21] = TextDrawCreate(430.0000, 100.0000, "gui:bracsm5");
	...
	acss_TD[22] = TextDrawCreate(430.0000, 100.0000, "gui:bracsm6");
	...
	acss_TD[23] = TextDrawCreate(430.0000, 100.0000, "gui:bracsm7");
	...
	acss_TD[24] = TextDrawCreate(485.0000, 205.0000, "_");
TextDrawLetterSize(acss_TD[24], 2.8000, 2.0000); 
TextDrawTextSize(acss_TD[24], 20.0000, 20.0000); 
TextDrawAlignment(acss_TD[24], 2); 
TextDrawColor(acss_TD[24], -1);
TextDrawBackgroundColor(acss_TD[24], 255);
TextDrawFont(acss_TD[24], 1); 
TextDrawSetProportional(acss_TD[24], 1);
TextDrawSetShadow(acss_TD[24], 0);
TextDrawSetSelectable(acss_TD[24], true);

acss_TD[25] = TextDrawCreate(555.0000, 205.0000, "_");
TextDrawLetterSize(acss_TD[25], 2.8000, 2.0000);
TextDrawTextSize(acss_TD[25], 20.0000, 20.0000);
TextDrawAlignment(acss_TD[25], 2);
TextDrawColor(acss_TD[25], -1);
TextDrawBackgroundColor(acss_TD[25], 255);
TextDrawFont(acss_TD[25], 1); // Обычный шрифт
TextDrawSetProportional(acss_TD[25], 1);
...
{
	extract params -> new slot, modelid, bone; else return Send(playerid, -1, "{FFFF00}| {FFFFFF}Используйте: {FFFF00}/{FFFFFF}acs [слот] [модель] [кость]");
	if(!(0 <= slot <= 9)) return Send(playerid, -1, "{FFFF00}| {FFFFFF}Используйте: слот от 0 до 9");
	if(!(1 <= bone <= 18)) return Send(playerid, -1, "{FFFF00}| {FFFFFF}Используйте: кость от 1 до 18"), Send(playerid, -1, "{FFFF00}| {FFFFFF}Кости 1 - Спина, 2 - Голова, 3 - Левое предплечье, 4 - Правое предплечье, 5 - Левая рука, 6 - Правая рука."), Send(playerid, -1, "{FFFF00}| {FFFFFF}7 - Левое бедро, 8 - Правое бедро, 9 - Левая нога, 10 - Правая нога, 11 - Правая голень, 12 - Левая голень."), Send(playerid, -1, "{FFFF00}| {FFFFFF}13 - Левое предплечье, 14 - Правое предплечье, 15 - Левое плечо, 16 - Правое плечо, 17 - Шея, 18 - Челюсть.");
	...
	if(rows)
	{
		...
		(
			...
			"{FF0000}"PROJECT_NAME" {FFFFFF}| Кастомизация",
			"Вы действительно желаете {FFFF00}сохранить{FFFFFF} положение аксессуара и выйти?",
			"Да", "Нет"
		);
	}
	else
	{
		EditAccessory(playerid);
		...
		mysql_format(mysql, query, sizeof query, "INSERT INTO accessories (id, slot, modelid, bone, x, y, z, rX, rY, rZ, scale) VALUES (%d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d)", GetPlayerAccountID(playerid), slot, modelid, bone, x, y, z, rX, rY, rZ, scale);
		mysql_query(mysql, query, false);
	}
...
{
	extract params -> new slot; else return Send(playerid, -1, "Используйте: /editacs [слот]");
	...
	{
	    Send(playerid, -1, "{FFFF00}| {FFFFFF}В данном слоту нет аксессуара");
	}
...
{
	extract params -> new slot; else return Send(playerid, -1, "{FFFF00}| {FFFFFF}Используйте: {FFFF00}/{FFFFFF}delacs [слот]");
	...
	rows = cache_num_rows();
 	...
 	if(rows)
 	{
		...
		result = mysql_query(mysql, query, true);
		RemovePlayerAttachedObject(playerid, slot);
		Send(playerid, -1, "{FFFF00}| {FFFFFF}Аксессуар успешно удален");
	}
	else
	{
		Send(playerid, -1, "{FFFF00}| {FFFFFF}Нет аксессуара в этом слоту");
	}
