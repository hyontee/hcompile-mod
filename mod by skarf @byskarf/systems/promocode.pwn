...
{
    print("[SYSTEM] Система промокодов загружена.");
...
public OnPromoTimerTick(playerPromoSQLID)
{
    ...
    {
        printf("[PROMO] Ошибка: Не удалось найти данные player_promos (SQL ID: %d) для таймера. errno: %d", playerPromoSQLID, mysql_errno());
...
public CheckAndCreatePromoTables()
{
    ...
    if(mysql_errno() != 0)
    {
        printf("[PROMO] Таблица 'promocodes' не найдена. Создаем новую структуру...");
        ...
        if(mysql_errno() != 0)
        {
            printf("[PROMO] Ошибка при создании таблицы 'promocodes': %d", mysql_errno());
        }
        else
        {
            printf("[PROMO] Таблица 'promocodes' успешно создана с новой структурой.");
        }
    }
    cache_delete(cache);
    ...
    if(mysql_errno() != 0)
    {
        printf("[PROMO] Таблица 'activated_promos' не найдена. Создаем новую структуру...");
        ...
        if(mysql_errno() != 0)
        {
            printf("[PROMO] Ошибка при создании таблицы 'activated_promos': %d", mysql_errno());
        }
        else
        {
            printf("[PROMO] Таблица 'activated_promos' успешно создана.");
        }
    }
    cache_delete(cache);
    ...
    if(mysql_errno() != 0)
    {
        printf("[PROMO] Таблица 'promo_prizes' не найдена. Создаем...");
        ...
        if(mysql_errno() != 0)
        {
            printf("[PROMO] Ошибка при создании таблицы 'promo_prizes': %d", mysql_errno());
        }
        else
        {
            printf("[PROMO] Таблица 'promo_prizes' успешно создана.");
        }
    }
    cache_delete(cache);
    ...
    {
        printf("[PROMO] Таблица 'player_promos' не найдена или имеет старую структуру. Создаем новую...");
       
        if(mysql_errno() == 0) {
            mysql_query(mysql, "DROP TABLE IF EXISTS `player_promos`", false);
            printf("[PROMO] Старая таблица 'player_promos' удалена.");
        }
        ...
        if(mysql_errno() != 0)
        {
            printf("[PROMO] Ошибка при создании таблицы 'player_promos': %d", mysql_errno());
        }
        else
        {
            printf("[PROMO] Таблица 'player_promos' успешно создана с новой структурой.");
        }
    }
    cache_delete(cache);
    ...
    if(cache_num_rows() == 0)
    {
        printf("[PROMO] Столбец 'promo_id' в 'ownable_cars' не найден. Добавляем...");
        mysql_format(mysql, query, sizeof(query), "ALTER TABLE `ownable_cars` ADD `promo_id` INT NOT NULL DEFAULT '0' AFTER `create_time`");
        mysql_query(mysql, query, false);
        if(mysql_errno() != 0)
        {
            printf("[PROMO] Ошибка при добавлении столбца 'promo_id': %d", mysql_errno());
        }
        else
        {
            printf("[PROMO] Столбец 'promo_id' успешно добавлен.");
        }
    }
    cache_delete(cache);
    ...
    if(cache_num_rows() == 0)
    {
        printf("[PROMO] Столбец 'promo_status_car' в 'ownable_cars' не найден. Добавляем...");
        ...
        if(mysql_errno() != 0)
        {
            printf("[PROMO] Ошибка при добавлении столбца 'promo_status_car': %d", mysql_errno());
        }
        else
        {
            printf("[PROMO] Столбец 'promo_status_car' успешно добавлен.");
        }
    }
    cache_delete(cache);
}
...
public LoadPromoCodesFromDatabase()
{
    ...
    {
        printf("[PROMO] ВНИМАНИЕ: Количество промокодов (%d) превышает размер массива (%d). Загружено только %d.", g_iLoadedPromoCodes, MAX_PROMO_CODES_ARRAY_SIZE, MAX_PROMO_CODES_ARRAY_SIZE);
        g_iLoadedPromoCodes = MAX_PROMO_CODES_ARRAY_SIZE;
    }
    else if(g_iLoadedPromoCodes == 0) {
        printf("[PROMO] В базе данных нет промокодов для загрузки.");
    }
    ...
    cache_delete(result);
    printf("[PROMO] Все промокоды успешно загружены. Загружено: %d", g_iLoadedPromoCodes);
...
stock SetPlayerPromoTimer(playerid, playerPromoSQLID, durationHours)
{
    ...
    {
        printf("[PROMO] Игрок %s (ID: %d) достиг лимита активных временных призов (%d). Таймер для приза %d не установлен.",
    ...
    printf("[PROMO] Игроку %s (ID: %d) установлен таймер для приза из player_promos.id: %d на %d часов.",
...
stock StopPlayerPromoTimer(playerid)
{
    ...
    for(new i = 0; i < g_iPlayerPromoTimersCount[playerid]; i++)
    {
        ...
        if(mysql_errno() == 0 && cache_num_rows(result) > 0)
        {
            ...
            mysql_query(mysql, query, false);
            printf("[PROMO] Таймер для приза (player_promos.id: %d) игрока %s (ID: %d) остановлен и время сохранено. Осталось: %d секунд.",
...
stock RevokePromoPrize(accountId, prizeType, prizeValue, promoIdForCarRevoke)
{
    ...
    switch(prizeType)
    {
        ...
        case PRIZE_TYPE_CAR:
        {
            ...
            if(rows > 0)
            {
                ...
                printf("[PROMO] Автомобиль (SQL ID: %d, Model: %d) отозван у игрока %s (ID: %d) из-за истечения срока действия приза.",
    ...
    printf("[PROMO] Приз типа %d (value %d) отозван у игрока %s (ID: %d).", prizeType, prizeValue, GetPlayerNameEx(playerid), accountId);
    return 1;
}
...
stock ShowPromoCreationConfirmation(playerid)
{
    ...
    format(szMessage, sizeof(szMessage), "{FF6347}"PROJECT_NAME" {FFFFFF}| Подтверждение создания промокода:\n\n");
    ...
    ShowPlayerDialog(playerid, DIALOG_CONFIRM_PROMO_CREATION, DIALOG_STYLE_MSGBOX,
        "{FF6347}"PROJECT_NAME" {FFFFFF}| Подтверждение создания промокода", szMessage, "Создать", "Отмена"
    );
}
...
stock CreatePromoCodeInDatabase(playerid)
{
    ...
    if(mysql_errno() != 0)
    {
        ...
        printf("[PROMO] Ошибка создания промокода (promocodes table): %d", mysql_errno());
        cache_delete(result);
        return 0;
    }
    ...
    if(promoId == 0)
    {
        ...
        printf("[PROMO] Ошибка: cache_insert_id вернул 0 для промокода '%s'.", promoCode);
        return 0;
    }
    ...
    for(new i = 0; i < prizeCount; i++)
    {
        ...
        if(mysql_errno() != 0)
        {
            ...
            SendClientMessage(playerid, -1, szMsg);
            printf("[PROMO] Ошибка добавления приза (promo_prizes table) для промокода ID %d, приз %d: %d", promoId, i, mysql_errno());
        }
    }
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    switch(dialogid)
    {
        ...
        case DIALOG_ADDPROMO_CODE:
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_ADDPROMO_PRIZE_COUNT, DIALOG_STYLE_INPUT, "{FF6347}"PROJECT_NAME" {FFFFFF}| Создание промокода", "Введите количество призов (1-5):", "Далее", "Отмена");
            return 1;
        }
        case DIALOG_ADDPROMO_PRIZE_COUNT:
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_ADDPROMO_USES_LIMIT, DIALOG_STYLE_INPUT, "{FF6347}"PROJECT_NAME" {FFFFFF}| Создание промокода", "Введите лимит использований промокода (0 - безлимитно):", "Далее", "Отмена");
            return 1;
        }
        ...
        case DIALOG_ADDPROMO_USES_LIMIT:
        {
            ...
            format(szDialogTitle, sizeof(szDialogTitle), "{FF6347}"PROJECT_NAME" {FFFFFF}| Настройка приза %d/%d (Тип)",
        ...
        case DIALOG_ADDPROMO_SINGLE_PRIZE_TYPE:
        {
            ...
            new szDialogTitle[128]; 
            format(szDialogTitle, sizeof(szDialogTitle), "{FF6347}"PROJECT_NAME" {FFFFFF}| Настройка приза %d/%d",
                currentPrizeIndex + 1,
                PlayerPromoCreationData[playerid][NewPromoPrizeCount]);
        ...
        case DIALOG_ADDPROMO_SINGLE_PRIZE_VALUE:
        {
            ...
            new szDialogTitle[128];
            format(szDialogTitle, sizeof(szDialogTitle), "{FF6347}"PROJECT_NAME" {FFFFFF}| Настройка приза %d/%d (Тип длительности)",
                currentPrizeIndex + 1,
                PlayerPromoCreationData[playerid][NewPromoPrizeCount]);
        ...
        case DIALOG_ADDPROMO_SINGLE_PRIZE_DURATION:
        {
            ...
            if(durationType == 0) 
            {
                format(szDialogTitle, sizeof(szDialogTitle), "{FF6347}"PROJECT_NAME" {FFFFFF}| Настройка приза %d/%d (Длительность)",
                    currentPrizeIndex + 1,
                    PlayerPromoCreationData[playerid][NewPromoPrizeCount]);
    ...
    if(GetPromoCodeDataByString(inputtext, promoData))
    {
        if(promoData[UsesLeft] > 0 || promoData[PromoUses] == 0) 
        {
            ...
            if(mysql_errno() != 0 || cache_num_rows(result) == 0)
            {
                ...
                printf("[PROMO] Ошибка загрузки призов для промокода ID %d: %d", promoSqlId, mysql_errno());
                cache_delete(result);
                return 1;
            }
            ...
            if(rows == 1)
            {
                ...
                if(mysql_errno() != 0)
                {
                    printf("[PROMO] Ошибка записи активации приза %d для игрока %d: %d", prizeSqlId, accountId, mysql_errno());
                }
                else
                {
            ...
            else if(rows == 2)
            {
                ...
                if(mysql_errno() != 0)
                {
                    printf("[PROMO] Ошибка записи активации приза %d для игрока %d: %d", prizeSqlId2, accountId, mysql_errno());
                }
                else
                {
                    new playerPromoEntryID = cache_insert_id(insertResult);
            ...
            else if(rows == 3)
            {
                ...
                if(mysql_errno() != 0)
                {
                    printf("[PROMO] Ошибка записи активации приза %d для игрока %d: %d", prizeSqlId3, accountId, mysql_errno());
                }
                else
                {
                    new playerPromoEntryID = cache_insert_id(insertResult);
            ...
            else if(rows == 4)
            {
                ...
                if(mysql_errno() != 0)
                {
                    printf("[PROMO] Ошибка записи активации приза %d для игрока %d: %d", prizeSqlId4, accountId, mysql_errno());
                }
                else
                {
                    new playerPromoEntryID = cache_insert_id(insertResult);
            ...
            else if(rows == 5)
            {
                ...
                if(mysql_errno() != 0)
                {
                    printf("[PROMO] Ошибка записи активации приза %d для игрока %d: %d", prizeSqlId5, accountId, mysql_errno());
                }
                else
                {
                    new playerPromoEntryID = cache_insert_id(insertResult);
        ...
        if(creator_playerid != -1)
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_PROMO_PRIZE_INFO, DIALOG_STYLE_MSGBOX, "{FF6347}"PROJECT_NAME"{ffffff} | Активация промокода", szActivationMessage, "Ок", "");
...
CMD:addcpromo(playerid)
{
    ...
    ShowPlayerDialog(playerid, DIALOG_ADDPROMO_CODE, DIALOG_STYLE_INPUT, "{FF6347}"PROJECT_NAME" {FFFFFF}| Создание промокода", "Введите название промокода (без пробелов):", "Далее", "Отмена");
    return 1;
}
