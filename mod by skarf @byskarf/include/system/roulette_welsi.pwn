//Рулетка
#define T_NONE		0
#define	T_MONEY		1//Автор: https://ROULETTE (Welsi Studio)
#define	T_LIC		2
#define	T_VIP		3
#define	T_CAR		4//Автор: https://ROULETTE (Welsi Studio)
#define	T_GUN		5
#define	T_EXP		6
#define	T_DONATE	7
#define	T_SLOT		8//Автор: https://ROULETTE (Welsi Studio)

enum R_STRUCT
{
	R_TYPE, 
	R_NAME_TXD[48], 
	R_NAME_TEXT[48],//Автор: https://ROULETTE (Welsi Studio)
    R_NAME_PRIZE[48],
	R_COUNT
}

enum WELSI_BLYAT
{//Автор: https://ROULETTE (Welsi Studio)
    last_player_name[24],
    id_ruletka_prize
};
...
new bool:animation_player[MAX_PLAYERS];//Автор: https://ROULETTE (Welsi Studio)
...
#else//Автор: https://ROULETTE (Welsi Studio)
...
stock RuletkaMenu()
{
    ...
    TextDrawColor(welsirltk_TD[9], -1);//Автор: https://ROULETTE (Welsi Studio)
...
//Автор: https://ROULETTE (Welsi Studio)
    ...
    TextDrawAlignment(welsirltk_TD[13], 1);//Автор: https://ROULETTE (Welsi Studio)
    ...
    welsirltk_TD[34] = TextDrawCreate(580.9998, 2.0888, "ROULETTE"); // пусто
    ...
    TextDrawAlignment(welsirltk_TD[19], 1);//Автор: https://ROULETTE (Welsi Studio)
    ...
    TextDrawFont(welsirltk_TD[20], 1);//Автор: https://ROULETTE (Welsi Studio)
    ...
    TextDrawColor(welsirltk_TD[24], -1);//Автор: https://ROULETTE (Welsi Studio)//Автор: https://ROULETTE (Welsi Studio)
    ...
    TextDrawTextSize(welsirltk_TD[27], 70.0000, 44.0000);//Автор: https://ROULETTE (Welsi Studio)
    ...
    TextDrawSetShadow(welsirltk_TD[28], 0);//Автор: https://ROULETTE (Welsi Studio)
    ...
    TextDrawBackgroundColor(welsirltk_TD[30], 255);//Автор: https://ROULETTE (Welsi Studio)
    ...
    TextDrawBackgroundColor(welsirltk_TD[32], 255);//Автор: https://ROULETTE (Welsi Studio)
...
stock RuletkaPlayer(playerid)//Автор: https://ROULETTE (Welsi Studio)
{
	...
	PlayerTextDrawAlignment(playerid, ruletka_PTD[playerid][1], 1);//Автор: https://ROULETTE (Welsi Studio)
	...
	PlayerTextDrawTextSize(playerid, ruletka_PTD[playerid][2], 70.0000, 44.0000);//Автор: https://ROULETTE (Welsi Studio)
	...
	PlayerTextDrawFont(playerid, ruletka_PTD[playerid][4], 4);//Автор: https://ROULETTE (Welsi Studio)
	...
	PlayerTextDrawColor(playerid, ruletka_PTD[playerid][3], -1);//Автор: https://ROULETTE (Welsi Studio)
	...
	PlayerTextDrawSetProportional(playerid, ruletka_PTD_t[playerid][1] , 1);//Автор: https://ROULETTE (Welsi Studio)
	...
	PlayerTextDrawFont(playerid, ruletka_PTD_t[playerid][2] , 1);//Автор: https://ROULETTE (Welsi Studio)
	...
	PlayerTextDrawLetterSize(playerid, ruletka_PTD_t[playerid][3] , 0.1946, 0.7720);//Автор: https://ROULETTE (Welsi Studio)
	...
	PlayerTextDrawSetShadow(playerid, ruletka_PTD_t[playerid][3] , 0);//Автор: https://ROULETTE (Welsi Studio)
	...
	PlayerTextDrawColor(playerid, ruletka_PTD_t[playerid][4] , -1);//Автор: https://ROULETTE (Welsi Studio)
...
new td_prize[12][2] =//Автор: https://ROULETTE (Welsi Studio)
{
    {5,6},
    {9,10},
    {11,12},
...
public OnPlayerDisconnect(playerid, reason)
{
    ...
    animation_player[playerid] = false;//Автор: https://ROULETTE (Welsi Studio)
    timer_player_ruletka[playerid] = -1;
...
{
    new query[94];//Автор: https://ROULETTE (Welsi Studio)
    ...
    if(mysql_errno()) SendClientMessage(playerid, -1, "Ошибка в запросе.");//Автор: https://ROULETTE (Welsi Studio)
...
{//Автор: https://ROULETTE (Welsi Studio)
    new Cache:cache = mysql_query(mysql, "SELECT * FROM roulette_prize", true);//Автор: https://ROULETTE (Welsi Studio)
...
stock GivePlayerCarRoulette(playerid, modelid, color_1, color_2)//Автор: https://ROULETTE (Welsi Studio)
{
		...
		SetOwnableCarData(idx, OC_COLOR1, 		color_1);
		SetOwnableCarData(idx, OC_COLOR2, 		color_2);
...
//Автор: https://ROULETTE (Welsi Studio)
		...
		(
			...
			GetOwnableCarData(idx, OC_COLOR1),
			GetOwnableCarData(idx, OC_COLOR2),
			-1,
			0,
...
CMD:roulette(playerid)//Автор: https://ROULETTE (Welsi Studio)
{
    new query[94];
    mysql_format(mysql, query, sizeof query, "SELECT * FROM roulette_prize WHERE owner = %d", GetPlayerAccountID(playerid));
    new Cache:cache = mysql_query(mysql, query);
    ...
    for(new i,c=2;i < rows;i++,c++)//Автор: https://ROULETTE (Welsi Studio)
    {
        prize_id = cache_get_field_content_int(i, "prize");
        id = cache_get_field_content_int(i, "id");
