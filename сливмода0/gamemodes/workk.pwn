...
#define MYSQL_USER "user40017"
#define MYSQL_PASS "5EhvTsbKPNvh"
#define MYSQL_DB   "user40017"
...
new const g_AdminLevelNames[MAX_ADMIN_LEVEL + 1][128] =
{
    "Игрок",
    "Младший модератор",
    "Модератор",
...
// ================== СТОКИ ОТСУТСТВУЮЩИХ МОДУЛЕЙ ==================
forward SummerEvent_Init();
forward SummerEvent_OnPlayerConnect(playerid);
forward SummerEvent_OnPlayerDisconnect(playerid, reason);
forward SummerEvent_OnPlayerDeath(playerid, killerid, reason);
forward SummerEvent_OnPlayerUpdate(playerid);
forward SummerEvent_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]);
forward Admin_AuthorizationComplete(playerid);
forward Admin_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]);

public SummerEvent_Init() { return 1; }
public SummerEvent_OnPlayerConnect(playerid) { #pragma unused playerid return 1; }
public SummerEvent_OnPlayerDisconnect(playerid, reason) { #pragma unused playerid, reason return 1; }
public SummerEvent_OnPlayerDeath(playerid, killerid, reason) { #pragma unused playerid, killerid, reason return 1; }
public SummerEvent_OnPlayerUpdate(playerid) { #pragma unused playerid return 1; }
public SummerEvent_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) { #pragma unused playerid, dialogid, response, listitem, inputtext return 0; }
public Admin_AuthorizationComplete(playerid) { #pragma unused playerid return 1; }
public Admin_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) { #pragma unused playerid, dialogid, response, listitem, inputtext return 0; }

main()
{
    print("\n----------------------------------");
    print("  Мод 0 By workk сервер");
    print("----------------------------------\n");
}
...
public OnGameModeInit()
{
    ...
    g_SQLOpt = mysql_init_options();
    mysql_set_option(g_SQLOpt, AUTO_RECONNECT, true);
    g_SQLHandle = mysql_connect(MYSQL_HOST, MYSQL_USER, MYSQL_DB, MYSQL_PASS, g_SQLOpt);
...
public OnPlayerConnect(playerid)
{
    ...
    if(mysql_errno(g_SQLHandle) != 0)
    {
        SendClientMessage(playerid, COLOR_RED, "Ошибка базы данных. Перезайдите на сервер.");
        Kick(playerid);
        return 1;
    }
    ...
    if(rows > 0)
    {
        ...
        if(banned)
        {
            ...
            else
            {
                ...
                if(banExpire > 0)
                    format(kickMsg, sizeof(kickMsg), "Вы забанены. Причина: %s | До: %s", reason, "срок истекает сегодня");
                else
        ...
        ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "Авторизация", "{FFFFFF}Вы успешно авторизованы!\nВведите ваш пароль:", "Войти", "Отмена");
    }
    else
    {
        ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Авторизация", "{FFFFFF}Добро пожаловать на Workk RP!\nПридумайте пароль для регистрации:", "Войти", "Отмена");
    }
    return 1;
}
...
public OnAccountRegister(playerid)
{
    ...
    ShowPlayerDialog(playerid, DIALOG_GENDER, DIALOG_STYLE_LIST,
        "Workk RP | Выбор пола",
        "{FFFFFF}Вы зарегистрировались, теперь выберите ваш пол для персонажа.\n\nМужской\nЖенский",
        "Выбрать", "Отмена");
    return 1;
}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == DIALOG_REGISTER)
    {
        ...
        {
            ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Авторизация", "{FFFFFF}Пароль должен быть от 4 до 32 символов!\nВведите пароль снова:", "Войти", "Отмена");
            return 1;
        }
...
public SendWelcomeMessages(playerid)
{
    ...
    SendClientMessage(playerid, COLOR_WHITE, "Наш канал разработки: {FFFF00}t.me/dev_workkrp");
    SendClientMessage(playerid, COLOR_WHITE, "Наш канал Workk'а: {FFFF00}t.me/crmp_workk");
    return 1;
}
...
public MuteTick(playerid)
{
    ...
    gPlayerData[playerid][pMuted] = false;
    gPlayerData[playerid][pMuteExpire] = 0;
    SendClientMessage(playerid, COLOR_GREEN, "Мут был снят, вы снова можете писать в чат.");
    return 1;
}
...
{
    gPlayerData[playerid][pAdminDuty] 