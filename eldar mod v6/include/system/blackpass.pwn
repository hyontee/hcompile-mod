...
stock BlackPass_HasQueuedReward(playerid, itemid)
{
    ...
    {
        if(g_PlayerRewards[playerid][i][rCase] == BLACKPASS_REWARD_CASE_ID
...
stock BlackPass_QueueInventoryReward(playerid, itemid)
{
    ...
    new reward_name[64];
    if(!BlackPass_GetRewardInventoryData(itemid, reward_name, sizeof(reward_name), type, internal, count, rarity, price))
    {
        printf("[BLACKPASS][REWARD][ERROR] invalid reward data player=%d item=%d", playerid, itemid);
        return 0;
    }

    if(BlackPass_HasQueuedReward(playerid, itemid))
    {
        BlackPass_LogEvent(playerid, "reward_queue", itemid, type, internal, count, 1);
        printf("[BLACKPASS][REWARD] already queued player=%d item=%d", playerid, itemid);
        return 1;
    }

    if(!AddPlayerCaseReward(playerid, BLACKPASS_REWARD_CASE_ID, itemid))
    {
        printf("[BLACKPASS][REWARD][ERROR] queue failed player=%d item=%d count=%d", playerid, itemid, g_PlayerRewardsCount[playerid]);
        return 0;
    }

    BlackPass_LogEvent(playerid, "reward_queue", itemid, type, internal, count, 0);
    printf("[BLACKPASS][REWARD] queued player=%d item=%d type=%d internal=%d count=%d", playerid, itemid, type, internal, count);
    return 1;
}
...
{
    new queued_count = 0;
    new required_count = skip_level_reward ? 3 : 4;
    ...
    return (queued_count == required_count);
}

stock BlackPass_RecoverMissingQueuedRewards(playerid)
{
    if(!g_blackpass_player[playerid][BP_DATA_LOADED]) return 0;

    new bool:queued_standard[BLACKPASS_MAX_LEVELS + 1];
    new bool:queued_premium[BLACKPASS_MAX_LEVELS + 1];
    new bool:queued_deluxe[5];
    new query[256];

    mysql_format(mysql, query, sizeof(query),
        "SELECT reward_id FROM blackpass_logs WHERE account_id=%d AND season_number=%d AND action='reward_queue'",
        GetPlayerAccountID(playerid),
        BLACKPASS_SEASON_NUMBER
    );
    ...
    if(mysql_errno())
    {
        printf("[BLACKPASS][REWARD][ERROR] recovery log load failed player=%d errno=%d", playerid, mysql_errno());
        cache_delete(result);
        return 0;
    }
    ...
    {
        new itemid = cache_get_field_content_int(row, "reward_id");
        if(itemid >= BLACKPASS_STANDARD_REWARD_BASE + 1 && itemid <= BLACKPASS_STANDARD_REWARD_BASE + BLACKPASS_MAX_LEVELS)
        {
            queued_standard[itemid - BLACKPASS_STANDARD_REWARD_BASE] = true;
        }
        else if(itemid >= BLACKPASS_PREMIUM_REWARD_BASE + 1 && itemid <= BLACKPASS_PREMIUM_REWARD_BASE + BLACKPASS_MAX_LEVELS)
        {
            queued_premium[itemid - BLACKPASS_PREMIUM_REWARD_BASE] = true;
        }
        else if(itemid >= BLACKPASS_DELUXE_REWARD_BASE + 1 && itemid <= BLACKPASS_DELUXE_REWARD_BASE + 4)
        {
            queued_deluxe[itemid - BLACKPASS_DELUXE_REWARD_BASE] = true;
        }
    }
    cache_delete(result);
    ...
    new recovered = 0;
    new current_level = BlackPass_GetCurrentLevel(playerid);
    for(new level = 1; level <= current_level; level++)
    {
        if(g_blackpass_claimed_standard[playerid][level] && !queued_standard[level])
        {
            if(BlackPass_QueueLevelReward(playerid, level, false))
            {
                queued_standard[level] = true;
                recovered++;
            }
        }

        if(g_blackpass_claimed_premium[playerid][level] && !queued_premium[level])
        {
            if(BlackPass_QueueLevelReward(playerid, level, true))
            {
                queued_premium[level] = true;
                recovered++;
            }
        }
    }

    if(g_blackpass_player[playerid][BP_PREMIUM_STATUS] == BLACKPASS_STATUS_DELUXE)
    {
        new bool:all_deluxe_queued = true;
        for(new i = 1; i <= 3; i++)
        {
            if(!queued_deluxe[i])
            {
                if(BlackPass_QueueInventoryReward(playerid, BlackPass_GetDeluxeInventoryItemId(i)))
                {
                    queued_deluxe[i] = true;
                    recovered++;
                }
                else
                {
                    all_deluxe_queued = false;
                }
            }
        }

        if(all_deluxe_queued && !g_blackpass_player[playerid][BP_DELUXE_REWARDS_CLAIMED])
        {
    ...
    if(recovered > 0)
    {
        printf("[BLACKPASS][REWARD] recovered player=%d rewards=%d", playerid, recovered);
    }
    return recovered;
}
...
stock BlackPass_AutoClaimRewardsRange(playerid, start_level, end_level, bool:claim_standard, bool:claim_premium)
{
    ...
    for(new level = start_level; level <= end_level; level++)
    {
        if(claim_standard && !g_blackpass_claimed_standard[playerid][level])
        {
            ...
            {
                printf("[BLACKPASS][REWARD][ERROR] auto claim standard failed player=%d level=%d", playerid, level);
            }
        }
        ...
        if(claim_premium && !g_blackpass_claimed_premium[playerid][level])
        {
            ...
            {
                printf("[BLACKPASS][REWARD][ERROR] auto claim premium failed player=%d level=%d", playerid, level);
            }
        }
    }
    return 1;
...
stock BlackPass_ClaimTaskReward(playerid, task_id)
{
    ...
    {
        ShowNotificationNew(playerid, 3, 3, 0, 0, "Вы успешно получили новый уровень Black Pass.", "qq");
...
stock BlackPass_OnPlayerLogged(playerid)
{
    ...
    new recovered = BlackPass_RecoverMissingQueuedRewards(playerid);
    if(recovered > 0)
    {
        BlackPass_ShowLootNotification(playerid, "\xcf\xf0\xee\xef\xf3\xf9\xe5\xed\xed\xfb\xe5 \xed\xe0\xe3\xf0\xe0\xe4\xfb \xe2\xee\xf1\xf1\xf2\xe0\xed\xee\xe2\xeb\xe5\xed\xfb \xe2 \xab\xc4\xee\xe1\xfb\xf7\xf3\xbb");
    }
