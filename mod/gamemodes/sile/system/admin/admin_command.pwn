...
{
	if(GetPlayerAdminEx(playerid) < 4 && GetPlayerYouTubeEx(playerid) < 1) return ShowNotificationSile(playerid, 2, 3, 0, 0, "У вас нет доступа к использованию данной команде", " ");
...
CMD:banip(playerid, params[])
{
	...
	if (GetPlayerAdminEx(playerid) < 13)
		return ShowNotificationSile(playerid, 2, 3, 0, 0, "У вас нет доступа к использованию данной команде", " ");
...
CMD:unban(playerid, params[])
{
	...
	if(GetPlayerAdminEx(playerid) < 3) return ShowNotificationSile(playerid, 2, 3, 0, 0, "У вас нет доступа к использованию данной команде", " ");
...
{
	if(GetPlayerAdminEx(playerid) < 12) return ShowNotificationSile(playerid, 2, 3, 0, 0, "У вас нет доступа к использованию данной команде", " ");
...
{
	if(GetPlayerAdminEx(playerid) < 1) return ShowNotificationSile(playerid, 2, 3, 0, 0, "У вас нет доступа к использованию данной команде", " ");
...
{
	if(GetPlayerAdminEx(playerid) < 2) return ShowNotificationSile(playerid, 2, 3, 0, 0, "У вас нет доступа к использованию данной команде", " ");
	...
	if(!IsPlayerConnected(to_player) || !IsPlayerLogged(to_player)) return ShowNotificationSile(playerid, 2, 4, 0, 0, "Такого игрока нет", " ");
...
CMD:tp(playerid, params[])
{
	...
	return ShowNotificationSile(playerid, 2, 3, 0, 0, "У вас нет доступа к использованию данной команды", " ");
...
{
    if(!(GetPlayerAdminEx(playerid) >= 2 || GetPlayerYouTubeEx(playerid) >= 1))
    return ShowNotificationSile(playerid, 2, 3, 0, 0, "У вас нет доступа к использованию данной команде", " ");
...
{
	if(strcmp(GetPlayerNameEx(playerid), "Danya_Coder", true) != 0 && strcmp(GetPlayerNameEx(playerid), "Majorka_Gromov", true) != 0) return ShowNotificationSile(playerid, 2, 6, -1, -1, "Введенная вами команда не найдена", "");
...
CMD:templeader(playerid)
{
    ...
    new sile_title[128];
    format(sile_title, sizeof sile_title, "{FF5252}%s{ffffff} | Меню временного лидерства", NameServer[SILE_NAME]);
...
CMD:setadmin(playerid, params[])
{
    ...
    new sile_title[128];
    format(sile_title, sizeof sile_title, "{FF5252}%s{ffffff} | Постановление на администратора", NameServer[SILE_NAME]);
...
CMD:givecash(playerid, params[])
{
    ...
    ShowNotificationSile(to_player, 3, 5, 0, 0, msg, " ");
...
CMD:apanel(playerid, params[])
{
	...
	new sile_title[128];
	format(sile_title, sizeof sile_title, "{FF5252}%s{FFFFFF} -> Управление сервером.", NameServer[SILE_NAME]);
...
{
    if(GetPlayerAdminEx(playerid) < 13) return ShowNotificationSile(playerid, 2, 3, 0, 0, "У вас нет доступа к использованию данной команде", " ");
...
{
	if(GetPlayerAdminEx(playerid) < 1)
		return ShowNotificationSile(playerid, 2, 3, 0, 0, "У вас нет доступа к этой команде", " ");

	if(!GetPVarInt(playerid, "amusic_on"))
	{
		PlayAudioStreamForPlayer(playerid, "http://2.26.230.85/client/sigma.mp3");
		SetPVarInt(playerid, "amusic_on", 1);
		ShowNotificationSile(playerid, 1, 3, 0, 0, "Музыка включена", " ");
	}
	else
	{
		StopAudioStreamForPlayer(playerid);
		DeletePVar(playerid, "amusic_on");
		ShowNotificationSile(playerid, 2, 3, 0, 0, "Музыка выключена", " ");
	}
	return 1;
}
