...
stock MoveOwnableCar(playerid, database_car, fam)
{
    ...
    // ==================== Тюнинг личного авто ====================
    // Переносим весь тюнинг, сохранённый у личной машины (ownable_cars),
    // в семейную (family_cars), чтобы он не терялся при передаче в семью.
    ...
    new vinil_current, exhsuast_current, strab_current;
    new enable_neon, horn_current, horn_id;
    ...
    new pdvradar, launch_v, nitro_v, nitro_level;
    new wheel_offset_ft, wheel_offset_rr, align_ft, align_rr;
    new width_ft, width_rr, wheel_radius, wheel_colors, wheel_comp;
    new lights_color, underlights_color, underlights_color_lf, underlights_color_rt;
    new body_colors, tonir_ft, tonir_rr;
    new vynil_name, vinyl_id;
    new supercharger_v, pt_turbo, color_body_r, color_body_g, color_wheels;
    new toner_front, toner_rear, light_color;
    new neon_center, neon_left, neon_right;
    ...
    vinil_current    = cache_get_field_content_int(0, "vinil_current");
    exhsuast_current = cache_get_field_content_int(0, "exhsuast_current");
    strab_current    = cache_get_field_content_int(0, "strab_current");
    enable_neon      = cache_get_field_content_int(0, "EnableNeon");
    horn_current     = cache_get_field_content_int(0, "horn_current");
    horn_id          = cache_get_field_content_int(0, "horn_id");
    ...
    pdvradar         = cache_get_field_content_int(0, "pdvradar");
    ...
    wheel_offset_ft  = cache_get_field_content_int(0, "wheel_offset_ft");
    wheel_offset_rr  = cache_get_field_content_int(0, "wheel_offset_rr");
    align_ft         = cache_get_field_content_int(0, "align_ft");
    align_rr         = cache_get_field_content_int(0, "align_rr");
    width_ft         = cache_get_field_content_int(0, "width_ft");
    width_rr         = cache_get_field_content_int(0, "width_rr");
    wheel_radius     = cache_get_field_content_int(0, "wheel_radius");
    wheel_colors     = cache_get_field_content_int(0, "wheel_colors");
    wheel_comp       = cache_get_field_content_int(0, "wheel_comp");
    lights_color     = cache_get_field_content_int(0, "lights_color");
    underlights_color    = cache_get_field_content_int(0, "underlights_color");
    underlights_color_lf = cache_get_field_content_int(0, "underlights_color_lf");
    underlights_color_rt = cache_get_field_content_int(0, "underlights_color_rt");
    body_colors      = cache_get_field_content_int(0, "body_colors");
    tonir_ft         = cache_get_field_content_int(0, "tonir_ft");
    tonir_rr         = cache_get_field_content_int(0, "tonir_rr");
    vynil_name       = cache_get_field_content_int(0, "vynil_name");
    vinyl_id         = cache_get_field_content_int(0, "diski");
    supercharger_v   = cache_get_field_content_int(0, "supercharger");
    pt_turbo         = cache_get_field_content_int(0, "pt_turbo");
    color_body_r     = cache_get_field_content_int(0, "color_body_r");
    color_body_g     = cache_get_field_content_int(0, "color_body_g");
    color_wheels     = cache_get_field_content_int(0, "color_wheels");
    toner_front      = cache_get_field_content_int(0, "toner_front");
    toner_rear       = cache_get_field_content_int(0, "toner_rear");
    light_color      = cache_get_field_content_int(0, "light_color");
    neon_center      = cache_get_field_content_int(0, "neon_center");
    neon_left        = cache_get_field_content_int(0, "neon_left");
    neon_right       = cache_get_field_content_int(0, "neon_right");
    // ===============================================================
...
(
    ...
    comfort1, sport1, drift1, sport_plus1, comfort2, sport2, drift2, sport_plus2,
    comfort3, sport3, drift3, sport_plus3, comfort4, sport4, drift4, sport_plus4,
    vinil_current, exhsuast_current, strab_current, enable_neon, horn_current, horn_id,
    highlight_type, stroboscope_type, exhaust_type, is_hydro_active, is_hydro_bought,
    is_farlight_active, is_farlight_bought, is_pnevmo_bought, pnevmo_selected, pdvradar,
    launch_v, nitro_v, nitro_level, wheel_offset_ft, wheel_offset_rr, align_ft, align_rr,
    width_ft, width_rr, wheel_radius, wheel_colors, wheel_comp, lights_color,
    underlights_color, underlights_color_lf, underlights_color_rt, body_colors, tonir_ft, tonir_rr,
    vynil_name, vinyl_id, supercharger_v, pt_turbo, color_body_r, color_body_g, color_wheels,
    toner_front, toner_rear, light_color, neon_center, neon_left, neon_right
);
...
// ==============================================================
// Тюнинг семейных автомобилей.
// Таблица family_cars не хранит отдельные параметры тюнинга
// (как у ownable_cars), поэтому на данный момент эти хуки служат
// плейсхолдером для будущего расширения (когда тюнинг появится
// и у семейных машин).
// ==============================================================
stock LoadVehicleTuningFromCache(vehicleid, familycarid, Cache:result)
{
    ...
    // ==================== Тюнинг семейного авто ====================
    // Применяем тюнинг, ранее перенесённый в family_cars из личной машины
    // (см. MoveOwnableCar), чтобы он не терялся при передаче в семью
    // и восстанавливался при каждой загрузке машины с гаража/базы.
...
stock SyncVehicleTuningForPlayer(playerid, vehicleid)
{
    ...
    // Принудительно обновляем состояние авто у игрока, которому оно застримилось.
        ...
        case 3: // Магазин
{
    ...
    {
        // Проверка на наличие свободного места и выдача предмета
        ...
        case 5:
{
    ...
    if(fid == -1)
    {
        // Игрок не состоит в семье - не даём зайти в меню семьи и не шлём ответ клиенту,
        // чтобы клиент не переспрашивал t:5 в цикле
        return 1;
    }
...
{
    SendClientMessage(playerid, -1, "[DEBUG] Команда /faminvite вызвана");

    new f = GetPlayerIdFamily(playerid);
    if(f == -1)
    {
        SendClientMessage(playerid, -1, ""c_f"У вас нету семьи.");
        return 1;
    }
    SendClientMessage(playerid, -1, "[DEBUG] Семья найдена, ID: " #f);

    new to_player;
    if(sscanf(params, "d", to_player))
    {
        SendClientMessage(playerid, -1, ""c_f_r"/faminvite [ID игрока]");
        return 1;
    }
    SendClientMessage(playerid, -1, "[DEBUG] ID игрока: " #to_player);

    if(IsPlayerConnected(to_player) == 0)
    {
        SendClientMessage(playerid, -1, ""c_f_r"Игрока нет в сети.");
        return 1;
    }
    SendClientMessage(playerid, -1, "[DEBUG] Игрок в сети");

    // Проверка доступа - ИСПРАВЛЯЕМ КАК В ПРЕДЫДУЩЕМ ОТВЕТЕ
    if(family_rang_dostup[f][GetPlayerRangFamily(playerid)-1][0] == 1)
    {
        SendClientMessage(playerid, -1, ""c_f_r"Вашему рангу недоступно принимать игроков.");
        return 1;
    }
    SendClientMessage(playerid, -1, "[DEBUG] Доступ есть");

    if(GetPlayerIdFamily(to_player) != -1)
    {
        SendClientMessage(playerid, -1, ""c_f_r"У игрока уже есть семья.");
        return 1;
    }
    SendClientMessage(playerid, -1, "[DEBUG] Игрок не в семье");
    ...
    SendClientMessage(playerid, -1, "[DEBUG] Состав не полон");

    if(!IsPlayerInRangeOfPlayer(playerid, to_player, 5.0))
    {
        SendClientMessage(playerid, 0xCECECEFF, "Игрок слишком далеко");
        return 1;
    }
    SendClientMessage(playerid, -1, "[DEBUG] Игрок в радиусе");

    format(f_string144, sizeof f_string144, "{FFFF00}%s{FFFFFF} приглашает вас в семью %s",
    GetPlayerNameEx(playerid), GetFamily(f, family_name));

    SendClientMessage(playerid, -1, "[DEBUG] Показываем диалог игроку " #to_player);
    ...
    SendClientMessage(playerid, -1, ""c_f"Приглашение отправлено!");
