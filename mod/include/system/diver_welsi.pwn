...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    ...
    for(new i = 0; i < MAX_I_DIVER; i++)
    {
        if(areaid == items_order[i][I_DV_AREA])
        {
            ...
            {
                ShowNotificationLonexs(playerid, 2, 6, 0, 0, "Устройтесь на работу водолаза.", "");
                return 0;
            }

            if(!GetPlayerDiver(playerid, DIVER_ORDER))
            {
                ShowNotificationLonexs(playerid, 2, 6, 0, 0, "У вас нет активного заказа. Возьмите заказ у капитана.", "");
                return 0;
            }

            if(items_order[i][I_DV_ORDER] != GetPlayerDiver(playerid, DV_ID))
            {
                ShowNotificationLonexs(playerid, 2, 6, 0, 0, "Этот предмет не из вашего заказа.", "");
                return 0;
            }
    ...
    if(dialogid == 2770)
    {
        if(response)
        {
            switch(listitem+1)
            {
                case 1:
                {
                    if(!GetPlayerDiver(playerid, DIVER_JOOB)) return ShowNotificationLonexs(playerid, 2, 6, 0, 0, "Сначала возьмите смену.", "");

                    if(GetPlayerDiver(playerid, DIVER_ORDER)) return ShowNotificationLonexs(playerid, 2, 6, 0, 0, "Сначала возьмите смену.", "");
                    ...
                    if(r_orders == -1) return ShowNotificationLonexs(playerid, 1, 6, 0, 0, "Немного подождите...", "");
                    ...
                    ShowNotificationLonexs(playerid, 1, 6, 0, 0, "Вы взяли заказ", "");
                ...
                case 2:
                {
                    if(GetPlayerDiver(playerid, DIVER_JOOB))
                    {
                        ...
                        if(GetPlayerDiver(playerid, DV_ID_VEH) != -1)
                        {
                            ...
                            SetPlayerDiver(playerid, DV_ID_VEH, -1);
                            ShowNotificationLonexs(playerid, 2, 6, 0, 0, "Вы уволились с работы 'Водолаз'.", "");
                            SendDestroyWayPoint(playerid, 5);
                        }
                    }
                    else
                    {
                        ShowNotificationLonexs(playerid, 1, 6, 0, 0, "Вы устроились на работу 'Водолаз' ", "");
               ...
               case 3:
                {
                    if(!GetPlayerDiver(playerid, DIVER_JOOB)) return ShowNotificationLonexs(playerid, 2, 6, 0, 0, "Сначала начните смену.", "");
                    ...
                    if(GetPlayerDiver(playerid, DV_ID_VEH) != -1)
                    {
                        DestroyVehicle(GetPlayerDiver(playerid, DV_ID_VEH));
                        SetPlayerDiver(playerid, DV_ID_VEH, -1);
                        ShowNotificationLonexs(playerid, 1, 6, 0, 0, "Вы вернули лодку", "");
                    }

                    else
                    {
                        if(!(GetPlayerMoneyEx(playerid) >= 20000)) return ShowNotificationLonexs(playerid, 1, 6, 0, 0, "Вам необходимо 20.000 рублей.", "");
                        new vehicle;

                        if(GetPVarInt(playerid, "shpere_diver") == sphere_actor[0])
                        {
                            new r = random(3);
                        	ShowNotificationLonexs(playerid, 1, 6, 0, 0, "Вы арендовали лодку", "");
    ...
    if(response)
    {
        switch(listitem + 1)
        {
            case 1:  // Колесо
            {
                ...
                {
                    ShowNotificationLonexs(playerid, 2, 6, 0, 0, "У вас нет колёс для продажи.", "");
                    return 1;
                }
                ...
                ShowNotificationLonexs(playerid, 1, 6, 0, 0, str, "");
            ...
            case 2:  // Аккумулятор
            {
                ...
                {
                    ShowNotificationLonexs(playerid, 2, 6, 0, 0, "У вас нет Аккумулятора для продажи.", "");
                    return 1;
                }
            ...
            case 3:  // Стиральная машина
            {
                ...
                {
                    ShowNotificationLonexs(playerid, 2, 6, 0, 0, "У вас нет Стиральной машины для продажи.", "");
                    return 1;
                }
            ...
            case 4:  // Ваза
            {
                ...
                {
                    ShowNotificationLonexs(playerid, 2, 6, 0, 0, "У вас нет Ваз для продажи.", "");
                    return 1;
                }
            ...
            case 5:  // Бензопил
            {
                ...
                {
                    ShowNotificationLonexs(playerid, 2, 6, 0, 0, "У вас нет Бензопилы для продажи.", "");
                    return 1;
                }
...
CMD:takeitems(playerid)
{
    ...
    if(item_id == -1 || item_id >= MAX_I_DIVER)
        return ShowNotificationLonexs(playerid, 2, 6, 0, 0, "Вы не находитесь рядом с предметом.", "");

    if(items_order[item_id][I_DV_OBJECT] == -1)
        return ShowNotificationLonexs(playerid, 2, 6, 0, 0, "Этот предмет уже подобран.", "");

    if(!GetPlayerDiver(playerid, DIVER_JOOB))
        return ShowNotificationLonexs(playerid, 2, 6, 0, 0, "Устройтесь на работу водолаза.", "");

    if(!GetPlayerDiver(playerid, DIVER_ORDER))
        return ShowNotificationLonexs(playerid, 2, 6, 0, 0, "У вас нет активного заказа.", "");

    if(items_order[item_id][I_DV_ORDER] != GetPlayerDiver(playerid, DV_ID))
        return ShowNotificationLonexs(playerid, 2, 6, 0, 0, "Этот предмет не из вашего заказа.", "");
    ...
    switch(items_order[item_id][I_DV_TYPE])
    {
        case I_DIVER_BOX:
        {
            AddPlayerDiver(playerid, DV_BOX, +, 1);
            UpdatePlayerDatabaseInt(playerid, "box", GetPlayerDiver(playerid, DV_BOX));
            ShowNotificationLonexs(playerid, 1, 6, 0, 0, "Вы подобрали 'Шкатулку'", "");
        }
        case I_DIVER_TIRE:
        {
            AddPlayerDiver(playerid, DV_TIRE, +, 1);
            UpdatePlayerDatabaseInt(playerid, "tire", GetPlayerDiver(playerid, DV_TIRE));
            ShowNotificationLonexs(playerid, 1, 6, 0, 0, "Вы подобрали 'Шину'", "");
        }
        case I_DIVER_GOLD:
        {
            AddPlayerDiver(playerid, DV_GOLD, +, 1);
            UpdatePlayerDatabaseInt(playerid, "gold", GetPlayerDiver(playerid, DV_GOLD));
            ShowNotificationLonexs(playerid, 1, 6, 0, 0, "Вы подобрали 'Слиток золота'", "");
        }
        case I_DIVER_TV:
        {
            AddPlayerDiver(playerid, DV_TV, +, 1);
            UpdatePlayerDatabaseInt(playerid, "television", GetPlayerDiver(playerid, DV_TV));
            ShowNotificationLonexs(playerid, 1, 6, 0, 0, "Вы подобрали 'Телевизор'", "");
        }
        case I_DIVER_BOOT:
        {
            AddPlayerDiver(playerid, DV_BOOT, +, 1);
            UpdatePlayerDatabaseInt(playerid, "boot", GetPlayerDiver(playerid, DV_BOOT));
            ShowNotificationLonexs(playerid, 1, 6, 0, 0, "Вы подобрали 'Сапог с кладом'", "");
        }
    }
    ...
    if(remaining_items == 0)
    {
        ...
        ShowNotificationLonexs(playerid, 1, 6, 0, 0, "Вы собрали все предметы! Возвращайтесь на пирс.", "");
    }
    else
    {
        new str[128];
