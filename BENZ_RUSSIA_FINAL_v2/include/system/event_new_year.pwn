...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    ...
    if(q_1_new[playerid][0][1] <= areaid && areaid <= q_1_new[playerid][4][1])
    {
    ...
    if(gift_box[0][G_ID_SPHERE] <= areaid && areaid <= gift_box[sizeof gift_box - 1][G_ID_SPHERE])
    {
    ...
    if(m_g_player_gift[playerid][0][1] <= areaid && areaid <= m_g_player_gift[playerid][9][1])
    {
    ...
    if(q_5_sphere[playerid][0] <= areaid && areaid <= q_5_sphere[playerid][2])
    {
    ...
    if(m_g_2_player_gift[playerid][0][1] <= areaid && areaid <= m_g_2_player_gift[playerid][14][1])
    {
    ...
    if(q_2_player[playerid][0][Q_2_SPHERE] <= areaid && areaid <= q_2_player[playerid][3][Q_2_SPHERE])
    {
...
stock UnLoadPlayerData_NewYear(playerid)
{
 //Автор данного ивента https://t.me/welsistudio (Welsi Studio)
    ...
    if(GetPVarInt(playerid, "quest_5_new_year"))    RemoveQuestNewYear(playerid, 5);//Автор данного ивента https://t.me/welsistudio (LAIRD_SYSTEM)
...
{
    if(count >= 0 && count <= sizeof q_3_dialog)
    {
        if(count == 7)
        {
