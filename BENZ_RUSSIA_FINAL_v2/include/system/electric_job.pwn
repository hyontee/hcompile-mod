...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    ...
    if(orders_electric[0][E_SPHERE] <= areaid && areaid <= orders_electric[sizeof orders_electric - 1][E_SPHERE])
    {
        if(player_electric[playerid])
        {
            ...
            if(player_orders_electric[playerid] == orders || (player_orders_electric[playerid] >= 0 && player_orders_electric[playerid] <= 3))
            {
...
CMD:eorders(playerid)
{
    ...
    new Float:salary = orders_electric[player_orders_electric[playerid]][E_MULTIPLIER] * (floatround(dist) * 8) + 3000;
