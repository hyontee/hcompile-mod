...
stock Cases_OnPacketIncoming(playerid, const jsonData[])
{
    ...
    // Проверяем, не пустой ли json пришел в функцию
    if(json != Node:0) {
        ...
        case CASES_TYPE_GO_DONATE: {
            ...
            if(donateType == 2) SendClientMessage(playerid, 0xFFFF00FF, "Переход в раздел BC");
        }
...
stock Cases_OpenCase(playerid, caseId, openType)
{
	...
	if(pCasesCounts[playerid][idx] > 0) {
		if(pCasesCounts[playerid][idx] < openCount) {
			new Node:json_err = JSON_Object(
				"t", JSON_Int(CASES_TYPE_OPEN),
				"s", JSON_Int(-1),
				"d", JSON_Int(1)
			);
			SendPlayerGUIPacket(playerid, GUICases, json_err);
			JSON_Cleanup(json_err);
			return 0;
		}
		useOwnedCases = 1;
	}
	...
	for(new r = 0; r < openCount; r++) {
		...
		// Ищем награду в CaseAwards по aId и записываем в базу
		for(new ai = 0; ai < CaseData[idx][cAwardsCount]; ai++) {
			if(CaseAwards[idx][ai][aId] == rewardIds[r]) {
				AddCasePrizeToRewards(
					playerid,
					CaseAwards[idx][ai][aType],
					CaseAwards[idx][ai][aInternalId],
					CaseAwards[idx][ai][aCount]
				);
				break;
			}
		}
	...
	new Node:json_out;
	JSON_Parse(jsonStr, json_out);
	SendPlayerGUIPacket(playerid, GUICases, json_out);
	JSON_Cleanup(json_out);
...
stock Cases_CheckDustReward(playerid)
{
	...
	if(pCasesDust[playerid] >= 2000) {
		...
		// Give special case (case ID 5 =            , index 4)
...
{
	#pragma unused json
	if(pCasesGUIOpen[playerid] == 0) return 1;

	// Очищаем массив временных наград
	for(new r = 0; r < 10; r++) pCasesPendingRewards[playerid][r] = 0;
	pCasesPendingCount[playerid] = 0;

	SendClientMessage(playerid, 0x00FF00FF, "[Кейсы] Награды успешно перенесены в /rewards!");
	pCasesGUIOpen[playerid] = 0;
	
	// Отправляем пустой статус успеха клиенту, чтобы закрыть окно
	new Node:res = JSON_Object("t", JSON_Int(CASES_TYPE_TAKE_REWARDS), "s", JSON_Int(1));
	SendPlayerGUIPacket(playerid, GUICases, res);
	JSON_Cleanup(res);
	return 1;
}
...
{
	if(GetPlayerAdminEx(playerid) < 6) return SendClientMessage(playerid, 0xFF0000FF, "                 !");

	extract params -> new to_player, amount; else return SendClientMessage(playerid, 0xCECECEFF, "           : /givedust [id       ] [          ]");

	if(!IsPlayerConnected(to_player) || !IsPlayerLogged(to_player))
		return SendClientMessage(playerid, 0xCECECEFF, "                 ");

	if(!(1 <= amount <= 10000)) return SendClientMessage(playerid, 0xCECECEFF, "                  1    10000                 ");

	pCasesDust[to_player] += amount;

	new reason[144];
	format(reason, sizeof reason, "              %s           %d     ", GetPlayerNameEx(playerid), amount);
	SendClientMessage(to_player, 0x00FF00FF, reason);

	format(reason, sizeof reason, "          %d             %s", amount, GetPlayerNameEx(to_player));
	...
	format(reason, sizeof reason, "      %s[acc:%d] %d     ", GetPlayerNameEx(to_player), GetPlayerAccountID(to_player), amount);
...
CMD:givecases(playerid, params[])
{
	...
	extract params -> new to_player, case_id, amount; else return SendClientMessage(playerid, 0xCECECEFF, "           : /givecases [id       ] [id      ] [          ]");

	if(!IsPlayerConnected(to_player) || !IsPlayerLogged(to_player))
		return SendClientMessage(playerid, 0xCECECEFF, "                 ");

	new caseIdx = Cases_GetIndex(case_id);
	if(caseIdx == -1) return SendClientMessage(playerid, 0xCECECEFF, "         ID      ");
	...
	new reason[144];
	format(reason, sizeof reason, "              %s           %d        (ID %d)", GetPlayerNameEx(playerid), amount, case_id);
	SendClientMessage(to_player, 0x00FF00FF, reason);

	format(reason, sizeof reason, "          %d        (ID %d)        %s", amount, case_id, GetPlayerNameEx(to_player));
	SendClientMessage(playerid, 0x00FF00FF, reason);

	format(reason, sizeof reason, "[A] %s[%d]             %s[%d]      -   %d (ID %d),                  %d",
	GetPlayerNameEx(playerid), playerid, GetPlayerNameEx(to_player), to_player, amount, case_id, pCasesCounts[to_player][caseIdx]);
	SendMessageToAdmins(reason, 0x999999FF);

	format(reason, sizeof reason, "      %s[acc:%d] %d        (ID %d)", GetPlayerNameEx(to_player), GetPlayerAccountID(to_player), amount, case_id);
	SendLog(playerid, LOG_TYPE_SUPERADMIN_ACTION, reason);
...
CMD:mybc(playerid, params[])
{
	...
	format(str, sizeof(str), "BC: %d |     : %d |              : %d", GetPlayerDonateRub(playerid), pCasesDust[playerid], pCasesOpened[playerid]);
	...
	for(new i = 0; i < 6; i++) { //                   6       
		if(pCasesCounts[playerid][i] > 0) {
			...
			format(str, sizeof(str), "     %d:         %d    ,                 : %d/5", CaseData[i][cId], pCasesCounts[playerid][i], bonusClaimed);
...
{
	if(GetPlayerAdminEx(playerid) < 1) return SendClientMessage(playerid, 0xFF0000FF, "                 !");
	
	new str[128];
	SendClientMessage(playerid, 0xFFFF00FF, "=== Debug        ===");
	format(str, sizeof(str), "              : %d | GUI       : %d | Pending       : %d", 
...
stock AddCasePrizeToRewards(playerid, type, item_id, amount)
{
	new acc_id = GetPlayerAccountID(playerid);
	if(acc_id <= 0) return 0;

	new query[256];
	// Используем g_InventoryMySQL или mysql (проверь имя переменной в моде)
	mysql_format(g_InventoryMySQL, query, sizeof(query), 
		"INSERT INTO `user_rewards` (player_id, item_type, item_id, quantity, claimed) VALUES (%d, %d, %d, %d, 0)",
		acc_id, type, item_id, amount);
	
	mysql_tquery(g_InventoryMySQL, query);
	return 1;
}