...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
  ...
  //          if(g_doubling[DOUBLING_WAGE]) salary = salary * 2;
            ...
            ShowClientNotification(playerid, 1, 6, 0, 0, "Вы успешно доставили груз", "");
            ShowClientNotification(partner, 1, 6, 0, 0, "Вы успешно доставили груз", "");
...
CMD:colinv(playerid, params[])
{
    ...
    if(player_collector[playerid][order_collector] != -1) 
        return ShowClientNotification(playerid, 2, 6, 0, 0, "Сначала завершите текущий заказ.", "");
    ...
    if(player_collector[playerid][player_partner] != -1)
    {
        ...
        player_collector[playerid][player_partner] = -1;
        ShowClientNotification(playerid, 1, 6, 0, 0, "Вы отказались от своего напарника.", "");
        
    }

    if(sscanf(params, "u", params[0]))
    return ShowClientNotification(playerid, 2, 6, 0, 0, "Используйте '/colinv' [id игрока].", "");

    if(!IsPlayerLogged(partner) || !IsPlayerConnected(partner)) 
        return ShowClientNotification(playerid, 2, 6, 0, 0, "Не игрока с данным ID.", "");

    if(!player_collector[partner][job_active])
        return ShowClientNotification(playerid, 2, 6, 0, 0, "Игрок не работает в инкассации.", "");

    if(!player_collector[playerid][job_active])
        return ShowClientNotification(playerid, 1, 6, 0, 0, "Вы не искассатор.", "");

    new Float:x, Float:y, Float:z;
    GetPlayerPos(partner, x, y, z);
    new Float:distance = GetPlayerDistanceFromPoint(playerid, x, y, z);

    if(distance >= 7.0) return ShowClientNotification(playerid, 2, 6, 0, 0, "Игрок слишком далеко от вас.", "");

    if(player_collector[partner][player_partner] != -1)
        return ShowClientNotification(playerid, 2, 6, 0, 0, "У игрока уже есть напарник.", "");

    new text[52];
    format(text, sizeof text, "%s приглашает вас в напарники, /joincoll", GetPlayerNameEx(playerid));

    ShowClientNotification(partner, 2, 6, 0, 0, text, "");
