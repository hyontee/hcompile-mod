...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
        ...
        if(response)
        {
            ...
            if(GetPlayerMoneyEx(playerid) >= 3_000_000)
            {
                ...
                if(!mysql_errno())
                {
                    ...
                    AddFamily(free_id, family_slot_veh, +, 1);
                }
    ...
    if(dialogid == 2240)
    {
        if(response)
        {
            ...
            else FSendClientMessage(f_string144, playerid, -1, ""c_f_r"У вас не хватает %d донат-рублей", 50-GetPlayerDonateRub(playerid));
        }
    }
    if(dialogid == 2241)
    {
...
stock MoveOwnableCar(playerid, database_car, fam)
{
    ...
    // ? ПОЛУЧАЕМ ТЮНИНГ
    new t_neon1 = cache_get_field_content_int(0, "tuning_neon1");
    new t_neon2 = cache_get_field_content_int(0, "tuning_neon2");
    new t_neon3 = cache_get_field_content_int(0, "tuning_neon3");
    new t_tint = cache_get_field_content_int(0, "tuning_tint");
    new t_vinyl = cache_get_field_content_int(0, "tuning_vinyl");
    new t_toner_front = cache_get_field_content_int(0, "tuning_toner_front");
    new t_toner_rear = cache_get_field_content_int(0, "tuning_toner_rear");
    new t_toner_front_side = cache_get_field_content_int(0, "tuning_toner_front_side");
    new t_toner_rear_side = cache_get_field_content_int(0, "tuning_toner_rear_side");
    new Float:t_susp_force = cache_get_field_content_float(0, "tuning_suspension_force");
    new Float:t_susp_bias = cache_get_field_content_float(0, "tuning_suspension_bias");
    new Float:t_wheel_size = cache_get_field_content_float(0, "tuning_wheel_size");
    new Float:t_wheel_front = cache_get_field_content_float(0, "tuning_wheel_add_front");
    new Float:t_wheel_rear = cache_get_field_content_float(0, "tuning_wheel_add_rear");
    new t_hydraulics = cache_get_field_content_int(0, "tuning_hydraulics");
    new t_launch = cache_get_field_content_int(0, "tuning_launch_control");
    new t_strob = cache_get_field_content_int(0, "tuning_stroboscope");
    new t_siren = cache_get_field_content_int(0, "tuning_siren");
    new t_plate_type = cache_get_field_content_int(0, "tuning_plate_type");
    new t_plate_num[9], t_plate_reg[4];
    cache_get_field_content(0, "tuning_plate_number", t_plate_num, mysql, 9);
    cache_get_field_content(0, "tuning_plate_region", t_plate_reg, mysql, 4);
    ...
    (
        ...
        tuning_neon1,tuning_neon2,tuning_neon3,tuning_tint,tuning_vinyl,\
        tuning_toner_front,tuning_toner_rear,tuning_toner_front_side,tuning_toner_rear_side,\
        tuning_suspension_force,tuning_suspension_bias,tuning_wheel_size,tuning_wheel_add_front,tuning_wheel_add_rear,\
        tuning_hydraulics,tuning_launch_control,tuning_stroboscope,tuning_siren,\
        tuning_plate_type,tuning_plate_number,tuning_plate_region) \
        VALUES \
        (%d,%d,%d,%d,%f,%f,%f,%f,%d,'%s',\
        %d,%d,%d,%d,%d,\
        %d,%d,%d,%d,\
        ...
        %d,%d,%d,%d,%d,\
        %d,'%e')",
        fam, model_id, color_1, color_2, pos_x, pos_y, pos_z, angle, gettime(), number,
        t_neon1, t_neon2, t_neon3, t_tint, t_vinyl,
        t_toner_front, t_toner_rear, t_toner_front_side, t_toner_rear_side,
        t_susp_force, t_susp_bias, t_wheel_size, t_wheel_front, t_wheel_rear,
        t_hydraulics, t_launch, t_strob, t_siren,
        t_plate_type, t_plate_num, t_plate_reg
    );
...
stock MoveFamilyCar(database_car, playerid)
{
    ...
    new comfort = cache_get_field_content_int(0, "comfort"),
...
(
    ...
    (owner_id,model_id,color_1,color_2,pos_x,pos_y,pos_z,angle,wheels_kl,wheels_size,wheels_raz,wheels_otkl,gidra,comfort,sport,sport_plus,drift,create_time,number) \
    VALUES \
    (%d,%d,%d,%d,%f,%f,%f,%f,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,'%s')", // Исправленная строка форматирования
...
stock LoadFamilyCar(database)
{
    ...
    SetCarFamily(free_id, V_F_COMFORT_PL,     cache_get_field_content_int(0, "comfort"));
    SetCarFamily(free_id, V_F_SPORT,          cache_get_field_content_int(0, "sport"));
    SetCarFamily(free_id, V_F_SPORT_PL,       cache_get_field_content_int(0, "sport_plus"));
    SetCarFamily(free_id, V_F_DRIFT,          cache_get_field_content_int(0, "drift"));
    ...
    if(vehicleid != INVALID_VEHICLE_ID)
    {
        ...
        if(strcmp(vehicle_family[index][V_F_NUMBER], "------"))
    	{
          SetVehicleNumberPlateForPlayer(playerid, vehicleid, 1, GetCarFamily(index, V_F_NUMBER), "29");
        }
    
        format(vehicle_family[free_id][V_F_NUMBER], 12, vehicle_family[free_id][V_F_NUMBER]);
        SetVehicleRuNumberPlate(vehicleid, vehicle_family[free_id][V_F_NUMBER], "29");
...
cmd:buyslotfam(playerid)
{
    ...
    (
        ...
        "в вашу семью? Стоимость слота - {FFFF00}50 донат-руб.",
        "Далее", "Выйти"
    );
    return 1;
}
