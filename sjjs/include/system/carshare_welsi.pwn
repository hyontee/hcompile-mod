...
{
    print("[KIRILL_SYSTEM] Система каршеринга загружена.");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == 2313)
    {
        if(response)
        {
            ...
            SetVehicleVirtualWorld(player_vehicle_carshare[playerid], GetPlayerVirtualWorld(playerid));
            LinkVehicleToInterior(player_vehicle_carshare[playerid], GetPlayerInterior(playerid));
            SetTimerEx("DelayedPutPlayerInVehicle", 300, false, "ii", playerid, player_vehicle_carshare[playerid]);
