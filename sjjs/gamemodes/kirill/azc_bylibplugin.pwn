...
// AZC bylibplugin integration
// Native Black Russia fuel GUI + working server refuel logic.
...
stock AZC_ShowFuelStation(playerid, vehicleid, stationid)
{
    ...
    ShowPlayerGUI(playerid, BrDialogFuelFill, json);
    JSON_Cleanup(json);
    return 1;
}
...
stock AZC_DoRefuel(playerid, liters, fuel_type)
{
    ...
    new gametext[64];
    format(gametext, sizeof gametext, "~g~Заправлено %d л~n~~w~Списано %d руб.", liters, price);
    GameTextForPlayer(playerid, gametext, 3000, 5);
...
stock AZC_HandleFuelGui(playerid, Node:json)
{
    ...
    {
        SendClientMessage(playerid, 0xCECECEFF, "Вы должны быть в машине!");
        return 1;
    }

    if(!IsPlayerDriver(playerid))
    {
        SendClientMessage(playerid, 0xCECECEFF, "Вы не за рулём!");
        return 1;
    }

    if(IsABike(vehicleid))
    {
        SendClientMessage(playerid, 0xCECECEFF, "Этот транспорт нельзя заправить!");
        return 1;
    }

    if(stationid < 0)
        stationid = GetNearestFuelStation(playerid, 10.0);

    if(stationid < 0)
    {
        SendClientMessage(playerid, 0xCECECEFF, "АЗС не найдена!");
        return 1;
    }

    new Float:curFuel = GetVehicleData(vehicleid, V_FUEL);
    new Float:newFuel = curFuel + float(liters);
    if(newFuel > 150.0) newFuel = 150.0;
    ...
    SetVehicleData(vehicleid, V_FUEL, newFuel);
    GivePlayerMoneyEx(playerid, -price);

    new msg[128];
    format(msg, sizeof msg, "Вы заправили %d литров за %d руб.", liters, price);
    SendClientMessage(playerid, 0x66CC00FF, msg);
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if((newkeys & KEY_CROUCH) && !(oldkeys & KEY_CROUCH))
    {
        ...
        {
            if(GarageGUI_OnPlayerKeyStateChange(playerid, newkeys, oldkeys))
                return 1;

            // Открываем GUI заправки только если реально рядом есть АЗС.
            // Иначе гудок должен просто гудеть, без сообщений в чат.
            new vehicleid = GetPlayerVehicleID(playerid);
            new stationid = GetNearestFuelStation(playerid, 10.0);

            if(stationid != -1 && !IsABike(vehicleid))
            {
                callcmd::azs(playerid, "");
            }
            return 1;
        }
    }
