...
stock FractionGui_AddQuestProgress(playerid, amount = 1)
{
    ...
    if(!FractionGui_IsOfficialTeam(GetPlayerData(playerid, P_TEAM))) return 0;
...
public FractionGui_MinuteTick()
{
    foreach(new playerid : Player)
    {
        ...
        new teamid = GetPlayerData(playerid, P_TEAM);
...
stock FractionGui_FillRanks(playerid, Node:json)
{
    ...
    JSON_SetInt(json, "fraction_id", GetPlayerData(playerid, P_TEAM));
    ...
    for(new r = 1; r <= 10; r++)
        rank_names_json = JSON_Append(rank_names_json, JSON_Array(JSON_String(GetTeamRankName(GetPlayerData(playerid, P_TEAM), r))));
...
stock FractionGui_BuildRoster(playerid, Node:response, const search[] = "")
{
    ...
    {
        if(!IsPlayerConnected(targetid) || !IsPlayerLogged(targetid)) continue;
        if(GetPlayerData(targetid, P_TEAM) != teamid) continue;
...
stock FractionGui_FindRosterAccount(playerid, Node:json, bool:allow_saved = false)
{
       ...
       JSON_GetInt(json, "id", value))
    {
        ...
        if(IsPlayerConnected(value) && GetPlayerData(value, P_TEAM) == teamid)
...
{
    if(account_id <= 0 || target_team != GetPlayerData(playerid, P_TEAM)) return false;
...
stock FractionGui_FillPlayerCard(playerid, account_id, Node:response)
{
    ...
    mysql_format(mysql, query, sizeof(query),
        ...
        account_id, GetPlayerData(playerid, P_TEAM));
    new Cache:cache = mysql_query(mysql, query, true);
    if(!cache_num_rows())
    {
        cache_delete(cache);
    ...
    if(online)
    {
        teamid = GetPlayerData(targetid, P_TEAM);
        rank = GetPlayerJob(targetid);
        level = GetPlayerLevel(targetid);
        warn_count = GetPlayerData(targetid, P_OWARN);
...
stock FractionGui_FillQuest(playerid, Node:response)
{
    ...
    new desc[160];
    FractionGui_GetQuestDescription(GetPlayerData(playerid, P_TEAM), desc, sizeof(desc));
