// Not defined anywhere else in the project - default to the new notification engine.
...
stock Garage_OnPlayerSpawn(playerid)
{
    LoadCarGarage[playerid] = 0;
    ...
    // Re-assert the selected garage after the entire spawn sequence.
...
stock Garage_OnPlayerDeath(playerid, killerid, reason)
{
...
stock Garage_OnPlayerPickUpPickupEx(playerid, pickupid, action_type, action_id)
{
...
stock Garage_OnKeyStateChange(playerid, newkeys, oldkeys)
{
...
// New explicit ALT-key interact: open garage info at the entrance, or exit the interior on foot.
stock Garage_TryInteract(playerid)
{
    if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER)
    {
        new garageid = GetNearestGarageEntrance(playerid, 5.0);
        if(garageid != -1)
        {
            ShowPlayerGarageInfo(playerid, garageid);
            return 1;
        }
    }
