...
stock ShowTuningTireGUI(playerid)
{
    if(!IsPlayerInAnyVehicle(playerid))
        return SendClientMessage(playerid, -1, "Вы должны быть в автомобиле!");
    ...
    JSON_SetInt(tuning_response, "m", GetPlayerMoneyEx(playerid));
        ...
        case 7:
        {
            ...
            if(GetPlayerMoneyEx(playerid) >= price)
            {
                ...
                new save_query[160];
                mysql_format(mysql, save_query, sizeof(save_query),
                    "INSERT INTO `vehicle_tuning_parts` (`car_id`,`part_type`,`part_id`) VALUES (%d,7,%d) ON DUPLICATE KEY UPDATE `part_id`=%d",
                    sql_id, detail_id, detail_id);
        ...
        case 3:
        {
            ...
            {
                GivePlayerMoneyEx(playerid, -price);
                SetOwnableCarData(GetVehicleData(GetPlayerVehicleID(playerid), V_ACTION_ID), OC_VINILCAR, vinyl_id);
                
                new save_query[128];
                mysql_format(mysql, save_query, sizeof(save_query),
                    "UPDATE `ownable_cars` SET `vinilcar`=%d WHERE `id`=%d",
                    vinyl_id, sql_id);
                mysql_tquery(mysql, save_query);
        ...
        case 5:
        {
            ...
            new save_query5[160];
            mysql_format(mysql, save_query5, sizeof(save_query5),
                "INSERT INTO `vehicle_tuning_parts` (`car_id`,`part_type`,`part_id`) VALUES (%d,5,%d) ON DUPLICATE KEY UPDATE `part_id`=%d",
                sql_id, detail_id, detail_id);
            mysql_tquery(mysql, save_query5);
        ...
        case 18:
        {
            ...
            new save_query18[128];
            mysql_format(mysql, save_query18, sizeof(save_query18),
                "DELETE FROM `vehicle_tuning_parts` WHERE `car_id`=%d AND `part_type`=%d",
                sql_id, selector_id);
            mysql_tquery(mysql, save_query18);
        ...
        case 12:
        {
            ...
            if(GetPlayerMoneyEx(playerid) >= price)
            {
                ...
                new save_query12[160];
                mysql_format(mysql, save_query12, sizeof(save_query12),
                    "INSERT INTO `vehicle_tuning_parts` (`car_id`,`part_type`,`part_id`) VALUES (%d,12,%d) ON DUPLICATE KEY UPDATE `part_id`=%d",
                    sql_id, collapse_value, collapse_value);
                mysql_tquery(mysql, save_query12);
