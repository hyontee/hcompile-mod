...
enum // типы бизнесов
{
	...
	BUSINESS_TYPE_ACCESSORY_SHOP = 13, 	// магазин аксессуаров
};
...
// ПРАВИЛЬНЫЕ КОСТИ ДЛЯ ПРИКРЕПЛЕННЫХ ОБЪЕКТОВ В SA-MP
// 1 - Позвоночник (SPINE) - НЕ ДВИГАЕТСЯ ПРИ БЕГЕ!
// 2 - Голова (HEAD)
// 3 - Левое плечо (LEFT_SHOULDER)
// 4 - Правое плечо (RIGHT_SHOULDER)  
// 5 - Левая рука (LEFT_ARM)
// 6 - Правая рука (RIGHT_ARM)
// 7 - Левое бедро (LEFT_LEG)
// 8 - Правое бедро (RIGHT_LEG)
// 9 - Левая ступня (LEFT_FOOT)
// 10 - Правая ступня (RIGHT_FOOT)
// 11 - Левая кисть (LEFT_HAND)
// 12 - Правая кисть (RIGHT_HAND)

new accessory[122][ACCESSORY_M_STRCT] =
{
    {4196 , 1,"Крылья бабочки "                       ,15000000 },
    {4197 , 2,"Маска пчелы"                       ,10000000},
    {18576 , 2,"Вертолет"                       ,300000},
    {13744 , 2,"Бабочки"                       ,500000000},
    {5378 , 2,"Меч короля пиратов"                       ,10000000},
    {916 , 2,"Спортивная сумка Supreme"                       ,3000000},
    {18509 , 2,"Кейс макита"                       ,10000000},
    {907 , 2,"Рюкзак LV"                       ,150000},
...
{
    button_PTD[playerid][0] = CreatePlayerTextDraw(playerid, 309.4163, 406.8438, "‰EHA:10000000");
...
{
    button_TD[0] = TextDrawCreate(579.1665, 374.5928, "_");
    ...
    button_TD[1] = TextDrawCreate(580.8331, 374.5928, "_");
    ...
    button_TD[2] = TextDrawCreate(580.4165, 405.7038, "_");
    ...
    button_TD[3] = TextDrawCreate(31.6666, 374.5927, "_");
    ...
    button_TD[4] = TextDrawCreate(584.9999, 388.0742, ">>>");
    ...
    button_TD[5] = TextDrawCreate(33.3333, 374.5927, "_");
    ...
    button_TD[6] = TextDrawCreate(32.9166, 405.7038, "_");
    ...
    button_TD[7] = TextDrawCreate(37.9166, 388.0742, "<<<");
    ...
    button_TD[8] = TextDrawCreate(257.4165, 378.2438, "_");
    ...
    button_TD[9] = TextDrawCreate(259.0829, 380.6067, "_");
    ...
    button_TD[10] = TextDrawCreate(258.2500, 388.2810, "_");
    ...
    button_TD[11] = TextDrawCreate(321.9993, 391.0805, "‹‘XOѓ");
    ...
    button_TD[12] = TextDrawCreate(260.1662, 391.0807, "KYЊ…Џ’");
...
public OnPlayerClickTextDraw(playerid, Text:clickedid)
{
    ...
    if(clickedid == button_TD[12]) // купить
    {
        ...
        if(GetPlayerMoneyEx(playerid) >= price)
        {
            ...
            if(!mysql_errno())
            {
                ...
                mysql_query(mysql, query, false);
                GivePlayerMoneyEx(playerid, -price);
                SendClientMessage(playerid, -1, "Вы купили аксессуар. Он был добавлен /inv");
	...
	if(clickedid == acss_TD[1])
	{
		...
		if(GetPVarInt(playerid, "hud_hidden") == 1)
		{
		    SetPVarInt(playerid, "hud_hidden", 0);
...
{
    print("[West_Dev] Система аксессуаров загружена!");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
           ...
           if(!GetPVarInt(playerid, "acs_use"))
            {
                switch(listitem)
                {
                    case 0: ShowBoneSelectionDialog(playerid, id);
    ...
    if(dialogid == 1193)
    {
        if(response)
        {
            new bone = listitem + 1;
            new id = GetPVarInt(playerid, "acs_id_sql");
            UseAccessoryWithBone(playerid, id, bone);
        }
    }
...
stock ShowBoneSelectionDialog(playerid, database_id)
{
    SetPVarInt(playerid, "acs_id_sql", database_id);
    
    new string[1024];
    strcat(string, "{FFFF00}Выберите часть тела для аксессуара\n\n");
    strcat(string, "{FFFFFF}1 - Спина (позвоночник) - {999999}НЕ ДВИГАЕТСЯ ПРИ БЕГЕ!\n");
    strcat(string, "{FFFFFF}2 - Голова\n");
    strcat(string, "3 - Левое плечо\n");
    strcat(string, "4 - Правое плечо\n");
    strcat(string, "5 - Левая рука\n");
    strcat(string, "6 - Правая рука\n");
    strcat(string, "7 - Левое бедро\n");
    strcat(string, "8 - Правое бедро\n");
    strcat(string, "9 - Левая нога\n");
    strcat(string, "10 - Правая нога\n");
    
    Dialog(playerid, 1193, DIALOG_STYLE_LIST, "{FFCD00}Выберите кость", string, "Выбрать", "Отмена");
}

stock UseAccessoryWithBone(playerid, database, bone)
{
    ...
    {
        cache_delete(cache);
        return SendClientMessage(playerid, -1, "Слот занят другим аксессуаром.");
    }
    cache_delete(cache);

    mysql_format(mysql, string, sizeof string, "SELECT * FROM accessories_players WHERE `player_id` = %d AND `bone` = %d", GetPlayerAccountID(playerid), bone);
    cache = mysql_query(mysql, string, true);
    if(cache_num_rows())
    {
        cache_delete(cache);
        return SendClientMessage(playerid, -1, "На этой части тела уже есть аксессуар.");
    }
    cache_delete(cache);
...
{
    ShowBoneSelectionDialog(playerid, database);
    return 1;
}
...
stock EditAccessory(playerid, database)
{
	...
	if(rows)
	{
		...
		new name_str[64];
		format(name_str, sizeof name_str, "~w~%s", accessory[acs_id][NAME_ACCESSORY]);
		TextDrawSetString(acss_TD[26], name_str);
		TextDrawShowForPlayer(playerid, acss_TD[26]);
		...
		SetPVarInt(playerid, "hud_hidden", 1);
	}
...
stock GiveAccessory(playerid, acs)
{
    ...
    if(!mysql_errno())
    {
        format(string, sizeof string, "В инвентарь был добавлен {FFFF00}\"%s\"{FFFFFF} (/inv)", accessory[acs][NAME_ACCESSORY]);
...
stock CreateEditAccessoryTD()
{
	...
	acss_TD[26] = TextDrawCreate(520.0000, 115.0000, "_");
	TextDrawLetterSize(acss_TD[26], 0.3500, 1.6000);
	TextDrawAlignment(acss_TD[26], 2);
	TextDrawColor(acss_TD[26], 0xFFFFFFFF);
	TextDrawBackgroundColor(acss_TD[26], 255);
	TextDrawFont(acss_TD[26], 2);
	TextDrawSetProportional(acss_TD[26], 1);
	TextDrawSetShadow(acss_TD[26], 1);
	TextDrawSetOutline(acss_TD[26], 1);
}
