...
stock Music_GetFreeTrackName(trackid, name[], size = sizeof(name))
{
    ...
    switch(trackid)
    {
        case 0: format(name, size, "           ");
        case 1: format(name, size, "             ");
        case 2: format(name, size, "DFM");
        case 3: format(name, size, "      Record");
        case 4: format(name, size, "         ");
        case 5: format(name, size, "           ");
        case 6: format(name, size, "      ENERGY");
        case 7: format(name, size, "              ");
        case 8: format(name, size, "      FM");
        case 9: format(name, size, "Relax FM");
        case 10: format(name, size, "Like FM");
        case 11: format(name, size, "      Maximum");
        default: format(name, size, "            ");
    }
    return 1;
}
...
stock Music_GetAlbumName(albumid, name[], size = sizeof(name))
{
    ...
    switch(albumid)
    {
        case 0: format(name, size, "MORGENSHTERN -       ");
        case 1: format(name, size, "Miyagi -       ");
    ...
    switch(albumid)
    {
        case 0:
        {
            switch(trackid)
            {
                ...
                case 1: format(name, size, "MORGENSHTERN - Cristal &  Ψ ");
                case 2: format(name, size, "MORGENSHTERN -            ");
                case 3: format(name, size, "MORGENSHTERN -      ");
                case 4: format(name, size, "MORGENSHTERN -    ");
            }
        }
        case 1:
        {
            switch(trackid)
            {
                case 0: format(name, size, "Miyagi -       ");
                case 1: format(name, size, "Miyagi -        ");
                case 2: format(name, size, "Miyagi -                        ");
                case 3: format(name, size, "Miyagi -              ");
                case 4: format(name, size, "Miyagi -               ");
            }
        }
        case 2:
        {
            switch(trackid)
            {
                case 0: format(name, size, "Miyagi & Andy Panda -        ");
                case 1: format(name, size, "Miyagi & Andy Panda -         ");
                case 2: format(name, size, "Miyagi & Andy Panda - Get Up");
                case 3: format(name, size, "Miyagi & Andy Panda -             ");
        ...
        case 4:
        {
            switch(trackid)
            {
                case 0: format(name, size, "GAZAN -        ");
                case 1: format(name, size, "GAZAN - Six Seven");
                case 2: format(name, size, "9       -     ");
                case 3: format(name, size, "DEAD BLONDE -         ");
                case 4: format(name, size, "             -           ");
            }
        }
        case 5:
        {
            switch(trackid)
            {
                case 0: format(name, size, "           -               ");
                case 1: format(name, size, "           -             ");
                case 2: format(name, size, "           -             ");
                case 3: format(name, size, "           -              ");
                case 4: format(name, size, "           -                       ");
            }
        }
        default: format(name, size, "           -       ");
...
stock bool:Music_PlayerHasAlbum(playerid, albumid)
{
    ...
    return g_music_premium_tracks[playerid];
}
...
stock Music_TakeDonate(playerid, amount)
{
    ...
    GivePlayerDonateRub(playerid, -amount, "                            ", true, true);
    return 1;
}
...
stock Music_Play(playerid, tabid, bool:premium, const track_name[], const url[])
{
    ...
    new msg[160];
    format(msg, sizeof msg, ""SC"     {FFFF00}%s {FFFFFF}       .", track_name);
...
stock Music_PlayPaidTrack(playerid, albumid, trackid)
{
    ...
    {
        new msg[144];
        format(msg, sizeof msg, ""SC"                          /            {FFFF00}%d       {FFFFFF}.", MUSIC_ALBUM_PRICE);
        SendClientMessage(playerid, -1, msg);
...
stock Music_OpenGUI(playerid)
{
    ...
    {
        SendClientMessage(playerid, -1, ""SC"          {FFFF00}       {FFFFFF}                  {FFFF00}          ");
        return 1;
    }
...
stock Music_Stop(playerid)
{
    ...
    {
        ShowNotificationKirill(playerid, 2, 6, 0, 0, "                  ", "");
        return 1;
    }
...
stock Music_HandlePremiumAlbumClick(playerid, albumid)
{
    ...
    {
        new msg[144];
        format(msg, sizeof msg, ""SC"                   .       {FFFF00}%d       {FFFFFF},       {FFFF00}%d{FFFFFF}.", MUSIC_ALBUM_PRICE, GetPlayerDonateRub(playerid));
        SendClientMessage(playerid, -1, msg);
        return Music_ShowPremiumAlbums(playerid);
    }
...
public InMusicTimer(playerid)
{
    if(IsPlayerInAnyVehicle(playerid))
    {
        SendClientMessage(playerid, -1, ""SC"              {FFFF00}       {FFFFFF}                                    {FFFF00}/music");
    }
    else
    {
