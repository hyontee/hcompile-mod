...
public GreenZoneCreate()
{
    for(new i = 0; i < MAX_NEW_GREEN_ZONES; i++)
    {
        ...
        // printf("Создана зона %d", GREENZONE_FLAGS[i]);
    }
}
...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    for(new i = 0; i < MAX_NEW_GREEN_ZONES; i++)
    {
        if(areaid == GREENZONE_FLAGS[i])
        {
            ...
            break; // Выходим из цикла, как только нашли зону
        }
    }
    ...
    for(new i = 0; i < MAX_NEW_GREEN_ZONES; i++)
    {
        if(areaid == GREENZONE_FLAGS[i])
        {
            ...
            if(StoredWeaponData[playerid][StoredWeaponID] != 0)
            {
                ...
                SendClientMessage(playerid, 0x00FF00FF, "{34C924}| {FFFFFF}Вам возвращено оружие, которое было у Вас до входа в зеленую зону.");
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(newkeys == KEY_FIRE)
    {
        if(player_order[playerid] != -1)
        {
            for(new i = 0; i < MAX_NEW_GREEN_ZONES; i++)
            {
                if(IsPlayerInDynamicArea(playerid, GREENZONE_FLAGS[i]))
                {
                    new weaponID = GetPlayerWeapon(playerid);
                    new weaponAmmo = GetPlayerAmmo(playerid);
    
                    if(weaponID > 0)
                    {
                        StoredWeaponData[playerid][StoredWeaponID] = weaponID;
                        StoredWeaponData[playerid][StoredWeaponAmmo] = weaponAmmo;
                    
                        ResetPlayerWeapons(playerid);
                    
                        SendClientMessage(playerid, 0xFF0000FF, "{ff2400}| {ffffff}Атака в зеленой зоне запрещена! Ваше оружие было временно забрано.");
                    }
                
                    TogglePlayerControllable(playerid, false);
                    ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{FF6347}SERVER_NAME {FFFFFF}| Зеленая зона", 
                        "Атаковать в зеленой зоне запрещено!\nВы заморожены на 5 секунд.", 
                        "Понял", "");
                    SetTimerEx("UnfreezePlayerGreenZone", 5000, false, "i", playerid);
                    SendClientMessage(playerid, 0xFF0000FF, "{ff2400}| {ffffff}Атака в зеленой зоне запрещена! Вы заморожены на 5 секунд.");
                }
            }
        }
    }
    #if defined greenzone_OnPlayerKeyStateChange
        return greenzone_OnPlayerKeyStateChange(playerid, newkeys, oldkeys);
    #else
        return 1;
    #endif
}
...
#endif
#define OnPlayerKeyStateChange greenzone_OnPlayerKeyStateChange
#if defined greenzone_OnPlayerKeyStateChange
    forward greenzone_OnPlayerKeyStateChange(playerid, newkeys, oldkeys);
#endif
...
public UnfreezePlayerGreenZone(playerid)
{
    ...
    SendClientMessage(playerid, 0x00FF00FF, "{34C924}| {FFFFFF}Вы снова можете двигаться.");
}
