    ...
    switch(type)
    {
        ...
        case 2:
        {
            if (GetPlayerMoney(playerid) < 20000)
            {
                new Node:response = JSON_Object();
                JSON_SetInt(response, "t", 2);
                ShowNotificationKirill(playerid, 2, 4, 1, 1, "Недостаточно денег", "");
                SendPacketToClient(playerid, 1, response);
                ~response;
                return 0;
            }
            ...
            while (!unique && attempts < maxAttempts)
            {
                ...
                if (!unique && attempts >= maxAttempts)
                {
                    ...
                    ShowNotificationKirill(playerid, 2, 4, 1, 1, "Ошибка генерации номера, попробуйте позже", "");
...
stock RemovePlateFromVehicle(playerid)
{
    ...
    new inventory_plate_text[32];
    if(inventory_type == 59 || inventory_type == 83)
    {
        JoinPlate(plate_text_value, region_value, inventory_plate_text, sizeof(inventory_plate_text));
    }
    else
    {
        format(inventory_plate_text, sizeof(inventory_plate_text), "%s", plate_text_value);
    }
    ...
    Inventory_AddItem(playerid, inventory_type, freeSlot, 1, inventory_plate_text);
    SaveInventoryItem(playerid, freeSlot);
