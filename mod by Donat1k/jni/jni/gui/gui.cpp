...
void CGUI::Render()
{
	...
	if (this->m_bMemDebugOn) {
		...
		ImGui::Text(" ");
		ImGui::Text("vortex-mobile", *(uint32_t*)(g_libGTASA + 0x670680));//merixton soset
		ImGui::Text(" ");
...
void CGUI::RenderPosition()
{
    ...
    // Размер шрифта
    float size_font = 34.0f;
    ImFont* font = fontmoney;
    
    // Текст для отображения
    char text[256];
    sprintf(text, "weferon kryt", __DATE__);
    
    // Вычисляем размер текста
    ImVec2 text_size = ImGui::CalcTextSize(text);
    
    // Центрируем текст вверху экрана с небольшим отступом от верхнего края
    float padding_top = 10.0f; // Отступ от верхнего края
    
    ImVec2 pos = ImVec2(
        (io.DisplaySize.x - text_size.x) * 0.5f,  // Центрирование по горизонтали
    ...
    // Рендерим обводку (8 направлений для лучшей обводки)
    const float offsets[][2] = {
        {-outline_thickness, 0},  // лево
        {outline_thickness, 0},   // право
        {0, -outline_thickness},  // верх
        {0, outline_thickness},   // низ
        {-outline_thickness, -outline_thickness}, // лево-верх
        {outline_thickness, -outline_thickness},  // право-верх
        {-outline_thickness, outline_thickness},  // лево-низ
        {outline_thickness, outline_thickness}    // право-низ
    };
    ...
    for (int i = 0; i < 8; i++) {
        ImGui::GetBackgroundDrawList()->AddText(
            ...
            outline_color, text, text + strlen(text)
        );
    }
    
    // Рендерим основной текст
    ImGui::GetBackgroundDrawList()->AddText(
        font, size_font, pos, text_color, text, text + strlen(text)
    );
}
