...
#define LEVEL_START_TC  2 //  ИЗМЕНЕНО: теперь с 2 уровня можно работать
...
public:CREATE_TABLE_COMPANY()
{
    ...
    if(mysql_errno()) mysql_query(mysql, "ALTER TABLE `accounts` ADD `progress_company` FLOAT NOT NULL DEFAULT '1.0' AFTER `job`, ADD `t_company` INT NOT NULL DEFAULT '-1' AFTER `progress_company`, ADD `tc_owner` INT NOT NULL DEFAULT '-1' AFTER `t_company`", false);
...
stock GivePlayerExpTC(playerid, count)
{
    ...
    if(!mysql_errno())
    {
        ...
        if(level_up) 
        {
            format(message, sizeof message, ""tc_msg"Поздравляем! Вы получили {FFAE00}%d{FFFFFF} уровень дальнобойщика. Ваш заработок увеличен", player_tc[playerid][p_TC_Level]);
            SendClientMessage(playerid, -1, message);
        }

        format(message, sizeof message, ""tc_msg"На данный момент у вас {FFAE00}%d{FFFFFF} уровень и {FFAE00}%d{FFFFFF} опыта дальнобойщика.", player_tc[playerid][p_TC_Level], player_tc[playerid][p_TC_Exp]);
        SendClientMessage(playerid, -1, message);
    }
...
public OnGameModeInit()
{
    ...
    for(new i; i < 2; i++)
    {
        for(new e, order_lvl[3]; e < 3; e++)
        {
            ...
            for(new o, type; o < sizeof orders_tc; o++)
            {
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == 2902)
    {
        ...
        { 
            format(level_txt, sizeof level_txt, "Ваш уровень дальнобойщика {FFAE00}позволяет{FFFFFF} принять этот заказ"); 
            SetPVarInt(playerid, "dostup_order", 0); 
        }
        else 
        { 
            format(level_txt, sizeof level_txt, "Ваш уровень дальнобойщика {FFAE00}не позволяет{FFFFFF} принять этот заказ"); 
        ...
        format(string, sizeof string, 
            ...
            {FFFFFF}%s\n\n\
            {FFFFFF}Оплата за заказ: {FFAE00}%d руб%s", 
            name_order_TC[orders_tc[id][o_TC_Type]-1],
            orders_tc[id][o_TC_Finish], level, level_txt, salary, salary_txt
        );

        Dialog(playerid, 2903, DIALOG_STYLE_MSGBOX, "{FFAE00}Информация о заказ", string, "Взять", "Назад");
    ...
    if(dialogid == 2903)
    {
        ...
        else
        {
            ...
            if(type_trailer[type] != -1)
            {
                ...
                spawn_trailer_tc[plant][r][2], spawn_trailer_tc[plant][r][3], random(225), 0, 0);
            } 
            else 
            {
...
//  ИСПРАВЛЕННАЯ ФУНКЦИЯ (добавлен return)
stock ShowDialogOrders(playerid, Locality)
{
    ...
    if(GetPlayerLevel(playerid) < LEVEL_START_TC)
    {
        ...
        SendClientMessage(playerid, -1, string);
        return 1; //  ВАЖНО: блокируем открытие диалога
    }
