    ...
    switch(t)
    {
        case -1:
                ...
                case 1:
                {
                    ...
                    {
                        ShowNotificationSile(playerid, 2, 5, -1, -1, "Пароль должен быть от 6 до 32 символов", "");
                        JSON_Cleanup(JSONObject);
                        return 1;
                    }
                ...
                case 3:
                {
                    ...
                    {
                        ShowNotificationSile(playerid, 2, 5, -1, -1, "Ошибка создания аккаунта!", "");
                        Kick(playerid);
                    }
        ...
        case 4:
                {
                    ...
                    {
                        ShowNotificationSile(playerid, 2, 5, -1, -1, "Указанный Nick-Name не найден!", "");
                        JSON_Cleanup(JSONObject);
                        JSON_Cleanup(json);
                        return;
                    }
        ...
        case 6:
                {
                    ...
                    {
                        if(GetPlayerIpEx(playerid) != GetPlayerData(playerid, P_LAST_IP)
                           && strcmp(GetPlayerData(playerid, P_SETTING_PIN), "None") != 0)
                        {
                            new Node:pinJson = JSON_Object();
                            JSON_SetInt(pinJson, "t", 1);
                            JSON_SetInt(pinJson, "r", 1);
                            SendPacketToClient(playerid, 38, pinJson);
                            JSON_Cleanup(pinJson);
                        }
                        else
                        {
                    ...
                    {
                        ShowNotificationSile(playerid, 2, 5, -1, -1, "Неверный пароль!", "");
                    }
                ...
                case 8:
                {
                    new input_code[32], tt;
                    JSON_GetString(JSONObject, "r", input_code);
                    JSON_GetInt(JSONObject, "tt", tt);
                    if(tt == 1)
                    {
                        if(strcmp(input_code, GetPlayerData(playerid, P_SETTING_PIN)) != 0)
                        {
                            HidePlayerGUI(playerid, 38);
                            SendClientMessage(playerid, -1, "Вы ввели не верный PIN-Code, и были кикнуты!");
                            SetTimerEx("kickpincode", 150, false, "");
                        }
                        else
                        {
                            SetPlayerData(playerid, P_AUTH_TIME, -1);
...
forward kickpincode(playerid);
public kickpincode(playerid)
{
    Kick(playerid);
}