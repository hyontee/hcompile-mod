...
#define DIALOG_ACS_ADMIN_PLAYERS      (31890)
#define DIALOG_ACS_ADMIN_BROWSER      (31891)
#define ACS_ADMIN_ITEMS_PER_PAGE      (15)

new gAcsAdminTarget[MAX_PLAYERS];
new gAcsAdminPage[MAX_PLAYERS];

// Single authoritative accessory loader. The token prevents repeated spawn/auth
// callbacks from applying the same attach slots several times.
new gAcsLoadToken[MAX_PLAYERS];
new bool:gAcsLoadedSlot[MAX_PLAYERS][MAX_PLAYER_ATTACHED_OBJECTS];
...
// The large 1073-entry array also contains skins, rewards and service models.
// Only entries from the real wearable catalogue may be streamed as attachments.
stock bool:AccessoryShop_IsWearableModel(modelid)
{
    if(modelid <= 0) return false;

    for(new i; i < sizeof(accessory_shop_data); i++)
    {
        if(accessory_shop_data[i][ID_ACCESSORY] != modelid) continue;

        new bone = accessory_shop_data[i][BONE_ACCESSORY];
        if(1 <= bone <= 18 && strlen(accessory_shop_data[i][NAME_ACCESSORY]))
            return true;
    }
    return false;
}

stock AcsDefaultSlotForBone(bone)
{
    switch(bone)
    {
        case 1: return 1;
...
stock AcsDeleteBrokenRow(playerid, slot, acs_id)
{
    ...
    {
        mysql_format(mysql, query, sizeof query,
            "DELETE FROM accessories_players WHERE player_id=%d AND acs_id=%d",
            account_id, acs_id);
    }
...
stock bool:AcsFloatIsSafe(Float:value)
{
    if(floatcmp(value, value) != 0) return false;
    new raw_value = _:value;
    if(raw_value == 0x7F800000 || raw_value == 0xFF800000) return false;
    return true;
}

stock bool:AcsSafeAttach(playerid, slot, modelid, bone,
    Float:x, Float:y, Float:z, Float:rX, Float:rY, Float:rZ,
    Float:scale_x, Float:scale_y, Float:scale_z)
{
    if(!IsPlayerConnected(playerid)) return false;
    if(slot < 0 || slot >= MAX_PLAYER_ATTACHED_OBJECTS) return false;
    if(!AccessoryShop_IsWearableModel(modelid))
    {
        printf("[ACS][CRASH-FIX] blocked attach model: player=%d slot=%d model=%d bone=%d", playerid, slot, modelid, bone);
        return false;
    }
    if(bone < 1 || bone > 18)
    {
        printf("[ACS][CRASH-FIX] blocked attach bone: player=%d slot=%d model=%d bone=%d", playerid, slot, modelid, bone);
        return false;
    }

    if(!AcsFloatIsSafe(x) || floatabs(x) > 10.0) x = 0.0;
    if(!AcsFloatIsSafe(y) || floatabs(y) > 10.0) y = 0.0;
    if(!AcsFloatIsSafe(z) || floatabs(z) > 10.0) z = 0.0;
    if(!AcsFloatIsSafe(rX) || floatabs(rX) > 3600.0) rX = 0.0;
    if(!AcsFloatIsSafe(rY) || floatabs(rY) > 3600.0) rY = 0.0;
    if(!AcsFloatIsSafe(rZ) || floatabs(rZ) > 3600.0) rZ = 0.0;
    if(!AcsFloatIsSafe(scale_x) || scale_x < 0.05 || scale_x > 5.0) scale_x = 1.0;
    if(!AcsFloatIsSafe(scale_y) || scale_y < 0.05 || scale_y > 5.0) scale_y = 1.0;
    if(!AcsFloatIsSafe(scale_z) || scale_z < 0.05 || scale_z > 5.0) scale_z = 1.0;

    if(IsPlayerAttachedObjectSlotUsed(playerid, slot))
        RemovePlayerAttachedObject(playerid, slot);

    if(!SetPlayerAttachedObject(playerid, slot, modelid, bone,
        x, y, z, rX, rY, rZ, scale_x, scale_y, scale_z))
    {
        printf("[ACS][CRASH-FIX] native attach failed: player=%d slot=%d model=%d bone=%d", playerid, slot, modelid, bone);
        return false;
    }
    return true;
}
...
stock AccessoryShop_GetInventoryItemByModel(modelid)
{
    ...
    new mapped_item = Inv11_GetAccItemByModel(modelid);
    if(mapped_item > 0)
    {
        return mapped_item;
    }

    // Generic accessory inventory item. The actual BR model is stored in model_id/extra_1.
    if(AccessoryGetIndexByModel(modelid) != -1)
    {
        return 136;
    }
...
stock bool:AcsInventory_GiveByIndex(playerid, index, &inventory_slot, &inventory_item_id)
{
    inventory_slot = -1;
    inventory_item_id = -1;

    if(index < 0 || index >= sizeof(accessory))
    {
        return false;
    }

    new modelid = accessory[index][ID_ACCESSORY];
    inventory_item_id = AccessoryShop_GetInventoryItemByModel(modelid);
    if(inventory_item_id <= 0)
    {
        printf("[ACS][INV][ERROR] cannot map model to inventory item: player=%d idx=%d model=%d", playerid, index, modelid);
        return false;
    }

    inventory_slot = Inventory11_AddItemToDatabase(playerid, inventory_item_id, modelid, 1, modelid, 0);
    if(inventory_slot == -1)
    {
        printf("[ACS][INV][ERROR] Inventory11_AddItem failed: player=%d idx=%d model=%d item=%d", playerid, index, modelid, inventory_item_id);
        return false;
    }
...
stock bool:AccessoryShop_GiveInventoryItem(playerid, index)
{
    ...
    new slot = Inventory11_AddItemToDatabase(playerid, inv_item_id, modelid, 1, modelid, 0);
    if(slot == -1)
    {
...
stock GetAccessoryBoneName(bone, output[], size)
{
    switch(bone)
    {
        case 1: format(output, size, "Спина");
        case 2: format(output, size, "Голова");
        case 3: format(output, size, "Левое плечо");
        case 4: format(output, size, "Правое плечо");
        case 5: format(output, size, "Левая рука");
        case 6: format(output, size, "Правая рука");
        case 7: format(output, size, "Левое бедро");
        case 8: format(output, size, "Правое бедро");
        case 9: format(output, size, "Левая нога");
        case 10: format(output, size, "Правая нога");
        case 11: format(output, size, "Правая голень");
        case 12: format(output, size, "Левая голень");
        case 13: format(output, size, "Левое предплечье");
        case 14: format(output, size, "Правое предплечье");
        case 15: format(output, size, "Левая ключица");
        case 16: format(output, size, "Правая ключица");
        case 17: format(output, size, "Шея");
        case 18: format(output, size, "Челюсть");
        default: format(output, size, "Кость %d", bone);
    }
    return 1;
}

stock bool:AcsAdmIsReal(index)
{
    if(index < 0 || index >= sizeof(accessory_shop_data))
    {
        return false;
    }

    new modelid = accessory_shop_data[index][ID_ACCESSORY];
    new bone = accessory_shop_data[index][BONE_ACCESSORY];
    ...
    if(!strlen(accessory_shop_data[index][NAME_ACCESSORY]))
    {
        return false;
    }

    return (FindAccsDataIndexByModel(modelid) != -1);
}

stock AcsAdmGetTotal()
{
    new total;
...
stock AcsAdmShowPlayers(playerid)
{
    ...
    gAcsAdminTarget[playerid] = INVALID_PLAYER_ID;
    gAcsAdminPage[playerid] = 0;

    new dialog_text[2048];
    new row_text[96];
    new shown;

    format(dialog_text, sizeof(dialog_text), "ID\tИгрок\tУровень\n");
    ...
    foreach(new i : Player)
    {
        ...
        format(row_text, sizeof(row_text), "%d\t%s\t%d\n", i, GetPlayerNameEx(i), GetPlayerLevel(i));
        strcat(dialog_text, row_text);
        SetPlayerListitemValue(playerid, shown, i);
        shown++;
    }

    if(!shown)
    {
        SendClientMessage(playerid, 0x999999FF, "Нет авторизованных игроков для выдачи аксессуара.");
        return 0;
    }

    Dialog(
        playerid,
        DIALOG_ACS_ADMIN_PLAYERS,
        DIALOG_STYLE_TABLIST_HEADERS,
        "{FFFF00}Выдача аксессуара | Выберите игрока",
        dialog_text,
        "Выбрать",
        "Закрыть"
    );
...
stock AcsAdmShowList(playerid, page)
{
    if(GetPlayerAdminEx(playerid) < 13)
    {
        return 0;
    ...
    new targetid = gAcsAdminTarget[playerid];
    if(targetid == INVALID_PLAYER_ID || !IsPlayerConnected(targetid) || !IsPlayerLogged(targetid))
    {
        SendClientMessage(playerid, 0x999999FF, "Выбранный игрок вышел с сервера.");
        return AcsAdmShowPlayers(playerid);
    }

    new total = AcsAdmGetTotal();
    if(total <= 0)
    {
        SendClientMessage(playerid, 0x999999FF, "В моде не найдено корректных аксессуаров.");
        return 0;
    }

    new max_page = (total - 1) / ACS_ADMIN_ITEMS_PER_PAGE;
    if(page < 0) page = 0;
    if(page > max_page) page = max_page;

    gAcsAdminPage[playerid] = page;
    ...
    new shown;
    new valid_pos;
    new start_pos = page * ACS_ADMIN_ITEMS_PER_PAGE;

    format(dialog_text, sizeof(dialog_text), "ACS ID\tMODEL ID\tАксессуар\tКрепление\n");
    ...
    for(new i; i < sizeof(accessory_shop_data); i++)
    {
        ...
        if(shown >= ACS_ADMIN_ITEMS_PER_PAGE)
        {
            break;
        }

        GetAccessoryBoneName(accessory_shop_data[i][BONE_ACCESSORY], bone_name, sizeof(bone_name));
        format(
            row_text,
            sizeof(row_text),
            "%d\t%d\t%s\t%s\n",
            i,
            accessory_shop_data[i][ID_ACCESSORY],
            accessory_shop_data[i][NAME_ACCESSORY],
            bone_name
        );
        strcat(dialog_text, row_text);
        SetPlayerListitemValue(playerid, shown, i);
    ...
    if(page > 0)
    {
        strcat(dialog_text, "{AAAAAA}<< Предыдущая страница\t-\t-\t-\n");
        SetPlayerListitemValue(playerid, shown, -1);
        shown++;
    }

    if(page < max_page)
    {
        strcat(dialog_text, "{AAAAAA}Следующая страница >>\t-\t-\t-\n");
        SetPlayerListitemValue(playerid, shown, -2);
        shown++;
    }

    strcat(dialog_text, "{FFD700}Выбрать другого игрока\t-\t-\t-\n");
    SetPlayerListitemValue(playerid, shown, -3);

    new dialog_title[144];
    format(
        dialog_title,
        sizeof(dialog_title),
        "{FFFF00}Аксессуары для %s[%d] | %d/%d",
...
stock AcsAdmGiveSelected(playerid, targetid, acs_id)
{
    if(GetPlayerAdminEx(playerid) < 13)
    {
        return 0;
    ...
    {
        SendClientMessage(playerid, 0x999999FF, "Игрок вышел с сервера.");
        return 0;
    }

    if(!AcsAdmIsReal(acs_id))
    {
        SendClientMessage(playerid, 0x999999FF, "Этот элемент не является корректным аксессуаром.");
        return 0;
    }
    ...
    if(!AcsInventory_GiveByIndex(targetid, acs_id, inventory_slot, inventory_item_id))
    {
        SendClientMessage(playerid, 0x999999FF, "Не удалось выдать аксессуар: проверьте место и вес инвентаря.");
        return 0;
    }
    ...
    format(
        message,
        sizeof(message),
        "Вы выдали %s[%d] аксессуар «%s» (ACS:%d, MODEL:%d, слот:%d).",
        GetPlayerNameEx(targetid),
        targetid,
        accessory_shop_data[acs_id][NAME_ACCESSORY],
        acs_id,
        accessory_shop_data[acs_id][ID_ACCESSORY],
        inventory_slot
    );
    ...
    format(
        message,
        sizeof(message),
        "Администратор %s выдал вам аксессуар «%s». Откройте инвентарь и нажмите «Надеть».",
        GetPlayerNameEx(playerid),
        accessory_shop_data[acs_id][NAME_ACCESSORY]
    );
    SendClientMessage(targetid, 0x66CC00FF, message);

    format(
        message,
        sizeof(message),
        "[A] %s[%d] выдал %s[%d] аксессуар ACS:%d MODEL:%d ITEM:%d INV_SLOT:%d",
        GetPlayerNameEx(playerid),
        playerid,
        GetPlayerNameEx(targetid),
        targetid,
...
forward AcsSafeLoadDelay(playerid, token);
public AcsSafeLoadDelay(playerid, token)
{
    if(!IsPlayerConnected(playerid) || !IsPlayerLogged(playerid)) return 1;
    if(token != gAcsLoadToken[playerid]) return 1;
    return LoadAccessory(playerid);
}

public OnPlayerSpawn(playerid)
{
    // Wait until the spawn and skin packets are stable before attaching objects.
    gAcsLoadToken[playerid]++;
    SetTimerEx("AcsSafeLoadDelay", 700, false, "ii", playerid, gAcsLoadToken[playerid]);

    if(GetPlayerAdminEx(playerid) >= 13 && !GetPVarInt(playerid, "acs_admin_hotkey_hint"))
    {
        SetPVarInt(playerid, "acs_admin_hotkey_hint", 1);
        SendClientMessage(playerid, 0xFFD700FF, "[ACS] ALT + Y — открыть меню выдачи аксессуаров.");
    }
...
public OnPlayerClickTextDraw(playerid, Text:clickedid)
{
    ...
    if(clickedid == acss_fix_TD[24] || clickedid == acss_fix_TD[25])
    {
        ...
        if(!AcsSafeAttach(
            playerid,
            GetPVarInt(playerid, "slot"),
            GetSafeAccessoryEditModel(playerid),
            GetPVarInt(playerid, "bone"),
            GetPVarFloat(playerid, "edit_x"),
            GetPVarFloat(playerid, "edit_y"),
            GetPVarFloat(playerid, "edit_z"),
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == DIALOG_ACS_ADMIN_PLAYERS)
    {
        if(GetPlayerAdminEx(playerid) < 13)
        {
            return 1;
        ...
        new targetid = GetPlayerListitemValue(playerid, listitem);
        if(targetid == INVALID_PLAYER_ID || !IsPlayerConnected(targetid) || !IsPlayerLogged(targetid))
        {
            SendClientMessage(playerid, 0x999999FF, "Игрок вышел с сервера.");
            AcsAdmShowPlayers(playerid);
            return 1;
        }

        gAcsAdminTarget[playerid] = targetid;
        gAcsAdminPage[playerid] = 0;
        AcsAdmShowList(playerid, 0);
        return 1;
    }

    if(dialogid == DIALOG_ACS_ADMIN_BROWSER)
    {
        if(GetPlayerAdminEx(playerid) < 13)
        {
            return 1;
        ...
        new selected = GetPlayerListitemValue(playerid, listitem);

        if(selected == -1)
        {
            AcsAdmShowList(playerid, gAcsAdminPage[playerid] - 1);
            return 1;
        }
        ...
        new targetid = gAcsAdminTarget[playerid];
        AcsAdmGiveSelected(playerid, targetid, selected);
        AcsAdmShowList(playerid, gAcsAdminPage[playerid]);
        return 1;
    }
...
stock UseAccessory(playerid, database)
{
    ...
    if(!AcsSafeAttach(playerid, slot, GetSafeAccessoryModel(id_acs), bone,
...
stock GetSafeAccessoryModel(acs_id)
{
    if(acs_id < 0 || acs_id >= sizeof(accessory)) return 0;
    return accessory[acs_id][ID_ACCESSORY];
}

stock GetSafeAccessoryEditModel(playerid)
{
    new modelid = GetPVarInt(playerid, "edit_modelid");
    if(modelid > 0) return modelid;

    new acs_id = GetPVarInt(playerid, "acs_id");
    return GetSafeAccessoryModel(acs_id);
}
...
stock EditAccessory(playerid, database)
{
	...
	if(rows)
	{
        ...
        DeletePVar(playerid, "inv_acs_db_id");
        SetPVarInt(playerid, "edit_modelid", GetSafeAccessoryModel(acs_id));
...
stock AccessoryGetIndexByModel(modelid)
{
    ...
    {
        if(GetSafeAccessoryModel(i) == modelid)
        {
            return i;
        }
    }
...
stock bool:AccessoryUseAndEditByModel(playerid, modelid)
{
    ...
    if(!AccessoryShop_IsWearableModel(modelid))
    {
        SendClientMessage(playerid, -1, "{FF0000}Этот предмет не является надеваемым аксессуаром.");
        printf("[ACS][CRASH-FIX] rejected non-accessory equip: player=%d model=%d", playerid, modelid);
        return false;
    }
    ...
    if(!AcsSafeAttach(playerid, slot, modelid, bone, x, y, z, rX, rY, rZ, scale, scale, scale))
    {
        return false;
    }
...
stock AccessoryEditFromInventory(playerid, database)
{
    ...
    if(!AcsSafeAttach(playerid, slot, modelid, bone,
        GetPVarFloat(playerid, "edit_x"),
        GetPVarFloat(playerid, "edit_y"),
        GetPVarFloat(playerid, "edit_z"),
        GetPVarFloat(playerid, "edit_rX"),
...
{
    new inventory_slot, inventory_item_id;
    if(!AcsInventory_GiveByIndex(playerid, acs, inventory_slot, inventory_item_id))
    {
        return 0;
    }

    new string[192];
    format(string, sizeof string, "Получен {FFFF00}«%s»{FFFFFF}. Откройте инвентарь и нажмите «Надеть»", accessory[acs][NAME_ACCESSORY]);
...
public:LoadAccessory(playerid)
{
    ...
    // On the first load of this connection remove stale slots created by the
    // legacy direct-model loader. Later reloads only touch our own slots.
    if(!GetPVarInt(playerid, "acs_safe_cleaned"))
    {
        for(new slot; slot < MAX_PLAYER_ATTACHED_OBJECTS; slot++)
        {
            if(IsPlayerAttachedObjectSlotUsed(playerid, slot))
                RemovePlayerAttachedObject(playerid, slot);

            gAcsLoadedSlot[playerid][slot] = false;
            for(new field; field < 11; field++)
                gPlayerTempAccData[playerid][slot][field] = 0.0;
        }
        SetPVarInt(playerid, "acs_safe_cleaned", 1);
    }
    else
    {
        for(new slot; slot < MAX_PLAYER_ATTACHED_OBJECTS; slot++)
        {
            if(!gAcsLoadedSlot[playerid][slot]) continue;
    ...
    mysql_format(mysql, query, sizeof query,
        "SELECT slot,acs_id,bone,x,y,z,rX,rY,rZ,scale FROM accessories_players WHERE player_id=%d AND bone>0 ORDER BY slot ASC",
        account_id);
    result = mysql_query(mysql, query, true);
    rows = cache_num_rows();

    new bool:used_slot[MAX_PLAYER_ATTACHED_OBJECTS];
    new loaded, removed;

    for(new row; row < rows; row++)
    {
        new slot = cache_get_field_content_int(row, "slot");
        new acs_id = cache_get_field_content_int(row, "acs_id");
        new bone = cache_get_field_content_int(row, "bone");

        if(acs_id < 0 || acs_id >= sizeof(accessory))
        {
            printf("[ACS][CRASH-FIX] remove invalid index: player=%d account=%d row=%d acs=%d slot=%d",
                playerid, account_id, row, acs_id, slot);
            AcsDeleteBrokenRow(playerid, slot, acs_id);
            removed++;
            continue;
        }

        new modelid = GetSafeAccessoryModel(acs_id);
        if(!AccessoryShop_IsWearableModel(modelid))
        {
            printf("[ACS][CRASH-FIX] remove non-accessory: player=%d account=%d row=%d acs=%d model=%d slot=%d",
                playerid, account_id, row, acs_id, modelid, slot);
            AcsDeleteBrokenRow(playerid, slot, acs_id);
            removed++;
            continue;
        }
        ...
        if(bone < 1 || bone > 18)
            bone = accessory[acs_id][BONE_ACCESSORY];

        if(bone < 1 || bone > 18)
        {
            printf("[ACS][CRASH-FIX] remove invalid bone: player=%d account=%d row=%d acs=%d model=%d bone=%d",
                playerid, account_id, row, acs_id, modelid, bone);
            AcsDeleteBrokenRow(playerid, slot, acs_id);
            removed++;
            continue;
        }
        ...
        if(slot < 0 || slot >= MAX_PLAYER_ATTACHED_OBJECTS)
            slot = AcsDefaultSlotForBone(bone);

        if(slot < 0 || slot >= MAX_PLAYER_ATTACHED_OBJECTS || used_slot[slot])
        {
            printf("[ACS][CRASH-FIX] skip invalid/duplicate slot: player=%d account=%d row=%d acs=%d model=%d slot=%d",
                playerid, account_id, row, acs_id, modelid, slot);
            continue;
        }

        new Float:x = cache_get_field_content_float(row, "x");
        new Float:y = cache_get_field_content_float(row, "y");
        new Float:z = cache_get_field_content_float(row, "z");
        new Float:rX = cache_get_field_content_float(row, "rX");
        new Float:rY = cache_get_field_content_float(row, "rY");
        new Float:rZ = cache_get_field_content_float(row, "rZ");
        new Float:scale = cache_get_field_content_float(row, "scale");
        ...
        if(!AcsFloatIsSafe(rY) || floatabs(rY) > 3600.0) rY = 0.0;
        if(!AcsFloatIsSafe(rZ) || floatabs(rZ) > 3600.0) rZ = 0.0;
        if(!AcsFloatIsSafe(scale) || scale < 0.05 || scale > 5.0) scale = 1.0;

        if(loaded >= 2)
        {
            printf("[ACS][CRASH-FIX] skip extra equipped row: player=%d account=%d row=%d model=%d slot=%d",
                playerid, account_id, row, modelid, slot);
            continue;
        }

        if(!AcsSafeAttach(playerid, slot, modelid, bone,
            x, y, z, rX, rY, rZ, scale, scale, scale))
        {
            removed++;
            AcsDeleteBrokenRow(playerid, slot, acs_id);
            continue;
        ...
        used_slot[slot] = true;
        gAcsLoadedSlot[playerid][slot] = true;
        gPlayerTempAccData[playerid][slot][0] = float(modelid);
        gPlayerTempAccData[playerid][slot][1] = float(bone);
        gPlayerTempAccData[playerid][slot][2] = x;
        gPlayerTempAccData[playerid][slot][3] = y;
    ...
    cache_delete(result);
    printf("[ACS][CRASH-FIX] safe load: player=%d account=%d loaded=%d removed=%d rows=%d",
        playerid, account_id, loaded, removed, rows);
    return 1;
}
