...
//эти строки добавить в LoadOwnableCar (где находиться подобные строки)
		...
		SetOwnableCarData(idx, 	OC_COMFORT_PL,	cache_get_field_content_int(1, "comfort"));
		SetOwnableCarData(idx, 	OC_SPORT,	cache_get_field_content_int(1, "sport"));
		SetOwnableCarData(idx, 	OC_SPORT_PL,	cache_get_field_content_int(1, "sport_plus"));
		SetOwnableCarData(idx, 	OC_DRIFT,	cache_get_field_content_int(1, "drift"));
...
new Float:coord_enter_stage[1][1] =
{
...
new Float:coord_exit_stage[1][1] =
{
...
new player_select_stage[1], player_price_stage[1];
new Text:stg_TD[1];
new PlayerText:stg_PTD[1][1];
new stage_td_select[1][1][1] =
{
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == 4646)
    {
        if(response)
        {
            ...
            {
                new string[124];
                format(string, sizeof string, "Вы успешно включили прошивку: {FFFF00}", name_stage[listitem]);
                SendClientMessage(playerid, -1, string);
