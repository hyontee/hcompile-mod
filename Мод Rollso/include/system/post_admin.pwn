...
new admin_player_post[13];

#define SetPlayerAdminPost(%0,%1)    admin_player_post[%13] = %1
#define GetPlayerAdminPost(%0)    admin_player_post[%13]
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == 1990)
    {
        if(response)
        {
            ...
            Dialog(playerid, 1991, DIALOG_STYLE_LIST, "{FFCD55}SILE RUSSIA | {FFFFFF}Выдача префикса администратору", dialog, "Назначить", "Выйти");
        }
    }
    if(dialogid == 1991)
    {
