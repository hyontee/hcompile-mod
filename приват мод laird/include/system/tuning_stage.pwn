#if !defined YES_CARPASS
    #define YES_CARPASS 100 // TODO: замени на правильный ID типа уведомления, если он объявлен в другом файле проекта
#endif

#if !defined _HAS_SetVehicleSuspensionBias
    #define _HAS_SetVehicleSuspensionBias
    stock SetVehicleSuspensionBias(vehicleid, Float:value)
    {
        // TODO: заглушка. Настоящей функции для этого нигде в проекте не нашлось
        // (нет плагина для хэндлинга среди подключённых). Если найдёшь готовую
        // реализацию в другом файле - удали эту заглушку и подключи тот файл раньше.
        return 1;
    }
#endif
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	if(newkeys == KEY_CROUCH)
    {
       if(IsPlayerInRangeOfPoint(playerid,  5.0, -426.235290,1005.412658,12.307508))
       {
        ...
        else if(IsPlayerInRangeOfPoint(playerid,  5.0, -420.054534,1005.409240,12.307003))
        {
          if(!IsPlayerInVehicle(playerid, GetPlayerOwnableCar(playerid))) return SendClientMessage(playerid, -1, ""USC"Вы должны быть за рулем своего автомобиля");
		  new vehicleID = GetPlayerVehicleID(playerid);
        ...
        else if(IsPlayerInRangeOfPoint(playerid,  5.0, -413.549285,1005.543029,12.326748))
        {
          if(!IsPlayerInVehicle(playerid, GetPlayerOwnableCar(playerid))) return SendClientMessage(playerid, -1, ""USC"Вы должны быть за рулем своего автомобиля");
		  new vehicleID = GetPlayerVehicleID(playerid);
...
cmd:carpass(playerid, params[])
{
    ...
    ShowNewNotification(playerid, 4, 5, YES_CARPASS, 1, "Посмотреть тех.паспорт", "");
