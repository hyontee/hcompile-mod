...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
            ...
            switch(listitem)
            {
                case 0:
                {
                    ...
                    (
                        ...
                        ""BR""FAN4IIK_NAME" {FFFFFF}-> Информация",
                ...
                case 2:
                {
                    ...
                    DialogCollector(playerid, -1, DIALOG_STYLE_TABLIST_HEADERS, ""BR""FAN4IIK_NAME" {FFFFFF}| Рейтинг лучших инкассаторов", dialog, "Назад", "Закрыть");
                ...
                case 3:
                {
                    ...
                    (
                        ...
                        ""BR""FAN4IIK_NAME"{FFFFFF}-> Ежедневные задания",
                        dialog,
                        "Далее", "Отмена"
                    );
                }
                case 4:
                {
                    ...
                    Dialog(playerid, -1, DIALOG_STYLE_MSGBOX, ""BR""FAN4IIK_NAME"{FFFFFF}-> Личная карточка", text, "Назад", "Закрыть");
                }
            }
        }
    }
    ...
    if(dialogid == 1812)
    {
        if(response)
        {
            ...
            (
                playerid, 1812, DIALOG_STYLE_MSGBOX,
                ""BR""FAN4IIK_NAME" {FFFFFF}-> Инкассатор",
        ...
        if(response)
        {
            ...
            switch(listitem)
            {
                ...
                case 0:
                {
                        ...
                        format(playertextt, sizeof playertextt, ""BR""FAN4IIK_NAME"{FFFFFF}-> Личная карточка игрока %s", GetPlayerNameEx(playerid));
                ...
                case 1:
                {
                    ...
                    Dialog(playerid, -1, DIALOG_STYLE_MSGBOX, ""BR""FAN4IIK_NAME"{FFFFFF}-> Данные о текущем заказе", text, "Назад", "Закрыть");     
                }
            }
        }
    }
            ...
            switch(listitem)
            {
                case 0:
                {
                    ...
                    (
                        ...
                        ""BR" "FAN4IIK_NAME" {FFFFFF}| Вместе веселее",
                ...
                case 1:
                {
                    ...
                    (
                        playerid, 1815, DIALOG_STYLE_MSGBOX,
                        ""BR" "FAN4IIK_NAME" {FFFFFF}| Достойный инкассатор",
                ...
                case 2:
                {
                    ...
                    (
                        playerid, 1815, DIALOG_STYLE_MSGBOX,
                        ""BR" "FAN4IIK_NAME" {FFFFFF}| Это высокая позиция",
...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    if(areaid == sphere_collector)
    {
        ...
        (
            ...
            ""BR""FAN4IIK_NAME"{FFFFFF}-> Инкассация",
  ...
  //          if(g_doubling[DOUBLING_WAGE]) salary = salary * 2;
            ...
            ShowNotificationFan4iik(playerid, 1, 6, 0, 0, "Вы успешно доставили груз", "");
            ShowNotificationFan4iik(partner, 1, 6, 0, 0, "Вы успешно доставили груз", "");
...
CMD:colinv(playerid, params[])
{
    ...
    if(player_collector[playerid][order_collector] != -1) 
        return ShowNotificationFan4iik(playerid, 2, 6, 0, 0, "Сначала завершите текущий заказ.", "");
    ...
    if(player_collector[playerid][player_partner] != -1)
    {
        ...
        player_collector[playerid][player_partner] = -1;
        ShowNotificationFan4iik(playerid, 1, 6, 0, 0, "Вы отказались от своего напарника.", "");
        
    }

    if(sscanf(params, "u", params[0]))
    return ShowNotificationFan4iik(playerid, 2, 6, 0, 0, "Используйте '/colinv' [id игрока].", "");

    if(!IsPlayerLogged(partner) || !IsPlayerConnected(partner)) 
        return ShowNotificationFan4iik(playerid, 2, 6, 0, 0, "Не игрока с данным ID.", "");

    if(!player_collector[partner][job_active])
        return ShowNotificationFan4iik(playerid, 2, 6, 0, 0, "Игрок не работает в инкассации.", "");

    if(!player_collector[playerid][job_active])
        return ShowNotificationFan4iik(playerid, 1, 6, 0, 0, "Вы не искассатор.", "");

    new Float:x, Float:y, Float:z;
    GetPlayerPos(partner, x, y, z);
    new Float:distance = GetPlayerDistanceFromPoint(playerid, x, y, z);

    if(distance >= 7.0) return ShowNotificationFan4iik(playerid, 2, 6, 0, 0, "Игрок слишком далеко от вас.", "");

    if(player_collector[partner][player_partner] != -1)
        return ShowNotificationFan4iik(playerid, 2, 6, 0, 0, "У игрока уже есть напарник.", "");

    new text[52];
    format(text, sizeof text, "%s приглашает вас в напарники, /joincoll", GetPlayerNameEx(playerid));

    ShowNotificationFan4iik(partner, 2, 6, 0, 0, text, "");
...
cmd:collist(playerid)
{
    ...
    Dialog(playerid, -1, DIALOG_STYLE_LIST, ""BR""FAN4IIK_NAME" -> Инкассаторы онлайн", dialog, "Далее", "Назад");
...
cmd:cmenu(playerid)
{
    ...
    (
        ...
        ""BR""FAN4IIK_NAME" {FFFFFF}-> Информация о работе инкассатора",
