...
stock Cases_OnPlayerConnect(playerid)
{
	...
	for(new i = 0; i < MAX_CASES; i++) {
		SetPlayerCaseCountByType(playerid, i, 0);
...
stock Cases_CheckDustReward(playerid)
{
    ...
    while(pCasesDust[playerid] >= 2000)
    {
        ...
        SetPlayerCaseCountByType(playerid, 5, 1);
        rewarded++;
    }
...
stock Cases_SavePlayer(playerid)
{
    ...
    for(new i = 0; i < MAX_CASES; i++)
    {
        if(i > 0) strcat(ccStr, ",");
        new tmp[16];
        format(tmp, sizeof(tmp), "%d", GetPlayerCaseCountByType(playerid, i));
        strcat(ccStr, tmp);
    }
    ...
    if(cache_num_rows())
    {
        ...
        if(strlen(ccStr) > 0)
        {
            ...
            while(pos < len && idx < MAX_CASES)
            {
                ...
                SetPlayerCaseCountByType(playerid, idx++, strval(tmp));
                pos = end + 1;
            }
        }
...
CMD:givecases(playerid, params[])
{
    ...
    if(sscanf(params, "udd", to_player, type_case, count))
        return SendClientMessage(playerid, COLOR_GREY, "»ÒÔÓÎ¸ÁÛÈÚÂ: /givecases [id] [type_case:1-6] [count]");

    if(!IsPlayerConnected(to_player)) return SendClientMessage(playerid, COLOR_RED, "»„ÓÍ ÌÂ Ì‡È‰ÂÌ.");
    if(type_case < 1 || type_case > 6) return SendClientMessage(playerid, COLOR_RED, "type_case ‰ÓÎÊÂÌ ·˚Ú¸ ÓÚ 1 ‰Ó 6.");
    if(count <= 0) return SendClientMessage(playerid, COLOR_RED, "count ‰ÓÎÊÂÌ ·˚Ú¸ ·ÓÎ¸¯Â 0.");

    AddPlayerCaseCountByType(to_player, type_case, count);
    SavePlayerAccount(to_player);

    new str[128];
    format(str, sizeof(str), "¬˚ ‚˚‰‡ÎË %d ÍÂÈÒ(Ó‚) ÚËÔ‡ %d Ë„ÓÍÛ %s.", count, type_case, GetPlayerNameEx(to_player));
    SendClientMessage(playerid, COLOR_WHITE, str);

    format(str, sizeof(str), "¿‰ÏËÌËÒÚ‡ÚÓ ‚˚‰‡Î ‚‡Ï %d ÍÂÈÒ(Ó‚) ÚËÔ‡ %d.", count, type_case);
...
public Cases_OnRewardTaken(playerid, rewardType, rewardValue, rewardCount, rewardRarity, isBonus)
{
    ...
    new qty = (rewardCount > 0) ? rewardCount : 1;
    new name[48];

    switch(rewardType)
    {
        case REWARD_TYPE_EXP: // 1
        {
            format(name, sizeof(name), "Œœ€“ »«  ≈…—¿ x%d", qty);
            BPR_GiveReward(playerid, 1, 0, name, rewardRarity, qty, 0, 0, -1);
        }
        case REWARD_TYPE_MONEY: // 2
        {
            format(name, sizeof(name), "ƒ≈Õ‹√» »«  ≈…—¿ x%d", qty);
            BPR_GiveReward(playerid, 2, 0, name, rewardRarity, qty, 0, 0, -1);
        }
        case REWARD_TYPE_BC: // 3
        {
            format(name, sizeof(name), "BC »«  ≈…—¿ x%d", qty);
            BPR_GiveReward(playerid, 3, 0, name, rewardRarity, qty, 0, 0, -1);
        }
        case REWARD_TYPE_CASE: // 4 ó rewardValue = caseId
        {
            format(name, sizeof(name), " ≈…— »«  ≈…—¿");
            BPR_GiveCase(playerid, rewardValue, name, rewardRarity, qty);
        }
        case REWARD_TYPE_VEHICLE: // 5 ó rewardValue = ÏÓ‰ÂÎ¸ Ï‡¯ËÌ˚, ‚˚‰‡∏Ï Ò‡ÁÛ ‚ „‡‡Ê
        {
            format(name, sizeof(name), "¿¬“ŒÃŒ¡»À‹ »«  ≈…—¿");
            BlackPass_GiveCarDirect(playerid, rewardValue, name);
        }
        case REWARD_TYPE_VIP: // 9 ó rewardValue = ÛÓ‚ÂÌ¸, rewardCount = ‰ÌÂÈ
        {
            format(name, sizeof(name), "VIP »«  ≈…—¿ Õ¿ %d ƒÕ.", qty);
            BPR_GiveReward(playerid, BPR_TYPE_VIP, rewardValue, name, rewardRarity, 1, qty, 0, -1);
        }
        case REWARD_TYPE_ITEM: // 11 ó rewardValue = item_id
        {
            format(name, sizeof(name), "œ–≈ƒÃ≈“ »«  ≈…—¿");
            BPR_GiveReward(playerid, BPR_TYPE_INVENTORY, rewardValue, name, rewardRarity, qty, 365, 0, -1);
        }
        case REWARD_TYPE_DUST: // 21
        {
            format(name, sizeof(name), "œ€À‹ »«  ≈…—¿ x%d", qty);
            BPR_GiveReward(playerid, 21, 0, name, rewardRarity, qty, 0, 0, -1);
        }
        case REWARD_TYPE_BP_EXP: // 10
        {
            format(name, sizeof(name), "BP Œœ€“ »«  ≈…—¿ x%d", qty);
            BPR_GiveReward(playerid, 10, 0, name, rewardRarity, qty, 0, 0, -1);
        }
        case REWARD_TYPE_EVENT_RES: // 23
        {
            format(name, sizeof(name), "»¬≈Õ“-–≈—”–— »«  ≈…—¿ x%d", qty);
            BPR_GiveReward(playerid, 23, 0, name, rewardRarity, qty, 0, 0, -1);
        }
        default:
        {
            format(name, sizeof(name), "Õ¿√–¿ƒ¿ »«  ≈…—¿ x%d", qty);
            BPR_GiveReward(playerid, rewardType, rewardValue, name, rewardRarity, qty, 30, 0, -1);
        }
    }
    return 1;
}
