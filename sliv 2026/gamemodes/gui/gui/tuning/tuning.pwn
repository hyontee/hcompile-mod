...
stock ShowTuningWheelsGUI(playerid)
{
    if(!IsPlayerInAnyVehicle(playerid))
        return SendClientMessage(playerid, -1, "Вы должны быть в автомобиле!");
...
// ВНИМАНИЕ: значение "o":3 для техцентра/диагностики подобрано по аналогии
// (1 = стайлинг, 2 = шиномонтаж), в исходном коде явного подтверждения для
// третьей вкладки не было. Если панель откроется не на той вкладке — скажи,
// поправим значение.
stock ShowTuningTechGUI(playerid)
{
    ...
    new Float:x = 1005.0, Float:y = 995.0, Float:z = 1497.5;
    SetVehiclePos(vehicleid, x, y, z);
    SetVehicleZAngle(vehicleid, 180.0);
    ...
    SetPlayerCameraPos(playerid, 1009.0, 991.0, 1498.2);
    SetPlayerCameraLookAt(playerid, 1005.6, 995.6, 1497.7);
...
stock CloseTuningGUI(playerid)
{
    ...
    switch(g_TuningType[playerid])
    {
        ...
        case 2:
        {
            SetVehiclePos(vehicleid, 2160.223144, -862.804687, 28.592450);
    ...
    switch(type) 
    {
        case 0:
        {
            ...
            switch(g_TuningType[playerid])
            {
                ...
                case 2:
                {
                    SetVehiclePos(GetPlayerVehicleID(playerid), 2160.223144, -862.804687, 28.592450);
