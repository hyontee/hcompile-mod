    ...
    if(dialogid == 2770)
    {
        if(response)
        {
            switch(listitem+1)
            {
                case 1:
                {
                    if(!GetPlayerDiver(playerid, DIVER_JOOB)) return ShowNotificationLaird(playerid, 2, 6, 0, 0, "Сначала возьмите смену.", "");

                    if(GetPlayerDiver(playerid, DIVER_ORDER)) return ShowNotificationLaird(playerid, 2, 6, 0, 0, "Сначала возьмите смену.", "");
                    ...
                    if(r_orders == -1) return ShowNotificationLaird(playerid, 1, 6, 0, 0, "Немного подождите...", "");
