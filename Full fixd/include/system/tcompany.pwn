...
new Text3D:g_tcompany_unload_label = Text3D:-1;
new g_tcompany_unload_pickup = -1;

#define TCOMPANY_UNLOAD_X (-1006.630676)
#define TCOMPANY_UNLOAD_Y (2150.311767)
#define TCOMPANY_UNLOAD_Z (44.586875)
#define TCOMPANY_LOAD_X (2325.356933)
#define TCOMPANY_LOAD_Y (1747.288696)
#define TCOMPANY_LOAD_Z (0.506430)
#define TCOMPANY_PAY (45000)
#define TCOMPANY_TRAILER_MODEL (435)

new g_tcompany_vehicle_trailer[MAX_VEHICLES];
new bool:g_tcompany_order_active[MAX_PLAYERS];
new g_tcompany_order_stage[MAX_PLAYERS];
new g_tcompany_order_vehicle[MAX_PLAYERS];
...
stock TCompany_GetAuctionBusinessId()
{
    for(new i = 0; i < g_business_loaded; i++)
    {
        if(GetBusinessData(i, B_TYPE) == BUSINESS_TYPE_TRUCKING_COMPANY)
            return i;
    }
    return -1;
}

stock bool:TCompany_IsTrailer(vehicleid)
{
    if(vehicleid <= 0 || vehicleid >= MAX_VEHICLES) return false;
    for(new i = 1; i < MAX_VEHICLES; i++)
    {
        if(g_tcompany_vehicle_trailer[i] == vehicleid)
            return true;
    }
    return false;
}

stock TCompany_GetTrailer(vehicleid)
{
    if(vehicleid <= 0 || vehicleid >= MAX_VEHICLES) return INVALID_VEHICLE_ID;
    return g_tcompany_vehicle_trailer[vehicleid];
}

stock TCompany_DestroyTrailer(vehicleid)
{
    if(vehicleid <= 0 || vehicleid >= MAX_VEHICLES) return 0;
    new trailerid = g_tcompany_vehicle_trailer[vehicleid];
    if(trailerid > 0 && IsValidVehicle(trailerid)) DestroyVehicle(trailerid);
    g_tcompany_vehicle_trailer[vehicleid] = INVALID_VEHICLE_ID;
    return 1;
}

stock TCompany_AttachTrailer(vehicleid)
{
    if(!TCompany_IsVehicle(vehicleid)) return 0;
    TCompany_DestroyTrailer(vehicleid);
    new Float:x, Float:y, Float:z, Float:a;
    GetVehiclePos(vehicleid, x, y, z);
    GetVehicleZAngle(vehicleid, a);
    x += floatsin(-a, degrees) * -8.0;
    y += floatcos(-a, degrees) * -8.0;
    z += 0.2;
    new trailerid = CreateVehicle(TCOMPANY_TRAILER_MODEL, x, y, z, a, 1, 1, -1, 0, VEHICLE_ACTION_TYPE_TRUCKER, VEHICLE_ACTION_ID_NONE);
    if(trailerid == INVALID_VEHICLE_ID) return 0;
    g_tcompany_vehicle_trailer[vehicleid] = trailerid;
    AttachTrailerToVehicle(trailerid, vehicleid);
    return trailerid;
}

forward TCompany_ReattachTrailer(vehicleid);
public TCompany_ReattachTrailer(vehicleid)
{
    if(!TCompany_IsVehicle(vehicleid)) return 1;
    if(!IsValidVehicle(vehicleid)) return 1;
    TCompany_AttachTrailer(vehicleid);
    return 1;
}

stock TCompany_IsPlayerInCompanyTruck(playerid)
{
    new vehicleid = GetPlayerVehicleID(playerid);
    if(vehicleid == INVALID_VEHICLE_ID || vehicleid <= 0) return 0;
    if(!TCompany_IsVehicle(vehicleid)) return 0;
    if(GetPlayerVehicleSeat(playerid) != 0) return 0;
    if(!TCompany_CanUseVehicle(playerid, vehicleid)) return 0;
    return vehicleid;
}

stock TCompany_SetOrderCheckpoint(playerid, stage)
{
    if(stage == 1)
    {
        SetPlayerCheckpoint(playerid, TCOMPANY_LOAD_X, TCOMPANY_LOAD_Y, TCOMPANY_LOAD_Z, 7.0);
        EnablePlayerGPS(playerid, 55, TCOMPANY_LOAD_X, TCOMPANY_LOAD_Y, TCOMPANY_LOAD_Z, "GPS: ");
        SendClientMessage(playerid, 0x66CC00FF, "Заказ ТК получен. Подъезжайте к точке загрузки и подайте гудок.");
    }
    else
    {
        SetPlayerCheckpoint(playerid, TCOMPANY_UNLOAD_X, TCOMPANY_UNLOAD_Y, TCOMPANY_UNLOAD_Z, 7.0);
        EnablePlayerGPS(playerid, 55, TCOMPANY_UNLOAD_X, TCOMPANY_UNLOAD_Y, TCOMPANY_UNLOAD_Z, "GPS: ");
        SendClientMessage(playerid, 0x66CC00FF, "Груз загружен. Доставьте его на выгрузку и подайте гудок.");
    }
    return 1;
}

stock TCompany_StartOrder(playerid)
{
    ...
    new vehicleid = TCompany_IsPlayerInCompanyTruck(playerid);
    if(!vehicleid)
        return SendClientMessage(playerid, 0xCECECEFF, "Служебный грузовик ТК нужно взять для выполнения заказа.");
    if(g_tcompany_order_active[playerid])
        return SendClientMessage(playerid, 0xFF9933FF, "Грузовой заказ уже активен.");
    if(TCompany_GetTrailer(vehicleid) == INVALID_VEHICLE_ID || !IsTrailerAttachedToAnyVehicle(TCompany_GetTrailer(vehicleid)))
        TCompany_AttachTrailer(vehicleid);
    g_tcompany_order_active[playerid] = true;
    g_tcompany_order_stage[playerid] = 1;
    g_tcompany_order_vehicle[playerid] = vehicleid;
    TCompany_SetOrderCheckpoint(playerid, 1);
    return 1;
}

stock TCompany_CancelOrder(playerid)
{
    if(g_tcompany_order_active[playerid]) DisablePlayerCheckpoint(playerid);
    g_tcompany_order_active[playerid] = false;
    g_tcompany_order_stage[playerid] = 0;
    g_tcompany_order_vehicle[playerid] = INVALID_VEHICLE_ID;
    return 1;
}

stock TCompany_OnCheckpoint(playerid)
{
    if(!g_tcompany_order_active[playerid]) return 0;
    if(g_tcompany_order_stage[playerid] == 1)
        SendClientMessage(playerid, 0xFFCC66FF, "Точка загрузки. Заезжайте на место и подайте гудок.");
    else
        SendClientMessage(playerid, 0xFFCC66FF, "Вы на точке выгрузки. Подайте гудок для завершения заказа.");
    return 1;
}

stock TCompany_TryHorn(playerid)
{
    if(!g_tcompany_order_active[playerid]) return 0;
    new vehicleid = TCompany_IsPlayerInCompanyTruck(playerid);
    if(!vehicleid || vehicleid != g_tcompany_order_vehicle[playerid]) return 1;
    new trailerid = TCompany_GetTrailer(vehicleid);
    if(trailerid == INVALID_VEHICLE_ID || !IsTrailerAttachedToAnyVehicle(trailerid))
    {
        TCompany_AttachTrailer(vehicleid);
        return SendClientMessage(playerid, 0xFF9933FF, "Прицеп был отсоединен. Прицеп восстановлен.");
    }
    if(g_tcompany_order_stage[playerid] == 1)
    {
        if(!IsPlayerInRangeOfPoint(playerid, 9.0, TCOMPANY_LOAD_X, TCOMPANY_LOAD_Y, TCOMPANY_LOAD_Z)) return 0;
        g_tcompany_order_stage[playerid] = 2;
        TCompany_SetOrderCheckpoint(playerid, 2);
        return 1;
    }
    if(!IsPlayerInRangeOfPoint(playerid, 9.0, TCOMPANY_UNLOAD_X, TCOMPANY_UNLOAD_Y, TCOMPANY_UNLOAD_Z)) return 0;
    GivePlayerMoneyEx(playerid, TCOMPANY_PAY, "Выполнение заказа ТК", true, true);
    TCompany_AddWorkPoints(playerid, TCOMPANY_PAY);
    TCompany_CancelOrder(playerid);
    SendClientMessage(playerid, 0x66CC00FF, "Заказ ТК успешно выполнен! Вы получили +45.000 рублей.");
    return 1;
}

stock TCompany_OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
    if(!TCompany_IsVehicle(vehicleid)) return 0;
    if(ispassenger) return 1;
    if(!TCompany_CanUseVehicle(playerid, vehicleid))
    {
        RemovePlayerFromVehicle(playerid);
        SendClientMessage(playerid, 0xFF6666FF, "Это транспорт ТК. Доступ разрешен сотрудникам компании.");
        return 1;
    }
    ...
    if(TCompany_GetTrailer(vehicleid) == INVALID_VEHICLE_ID || !IsTrailerAttachedToAnyVehicle(TCompany_GetTrailer(vehicleid))) TCompany_AttachTrailer(vehicleid);
    SendClientMessage(playerid, 0x66CC00FF, "Вы взяли служебный грузовик ТК. Создайте заказ через /cargo.");
    return 1;
}

stock TCompany_OnVehicleDeath(vehicleid)
{
    if(!TCompany_IsVehicle(vehicleid)) return 0;
    TCompany_DestroyTrailer(vehicleid);
    SetVehicleToRespawn(vehicleid);
    SetTimerEx("TCompany_ReattachTrailer", 1200, false, "i", vehicleid);
    foreach(new i : Player)
    {
        if(GetPlayerData(i, P_JOB_CAR) != vehicleid) continue;
        TCompany_CancelOrder(i);
        SetPlayerData(i, P_JOB_CAR, INVALID_VEHICLE_ID);
        SendClientMessage(i, 0xFF6666FF, "Грузовик ТК был поврежден и восстановлен.");
    }
    return 1;
}

stock TCompany_OnPlayerDisconnect(playerid)
{
    TCompany_CancelOrder(playerid);
    return 1;
}

stock TCompany_OnTrailerUpdate(playerid, trailerid)
{
    if(!TCompany_IsTrailer(trailerid)) return 0;
    return 1;
}
...
stock TCompany_InitWorld()
{
    ...
    g_tcompany_unload_pickup = CreateDynamicPickup(1239, 23, TCOMPANY_UNLOAD_X, TCOMPANY_UNLOAD_Y, TCOMPANY_UNLOAD_Z, 0, 0, -1, 25.0);
    g_tcompany_unload_label = CreateDynamic3DTextLabel("{FFCC00}Выгрузка заказов Транспортной компании\n{FFFFFF}Подайте гудок для выгрузки", 0xFFFFFFFF, TCOMPANY_UNLOAD_X, TCOMPANY_UNLOAD_Y, TCOMPANY_UNLOAD_Z + 1.0, 15.0);
...
CMD:orders(playerid, params[])
{
    #pragma unused params
    if(TCompany_GetPlayerCompany(playerid) == -1)
        return SendClientMessage(playerid, 0xCECECEFF, "Вы не состоите в ТК.");
    if(g_tcompany_order_active[playerid])
    {
        if(g_tcompany_order_stage[playerid] == 1)
            return SendClientMessage(playerid, 0xFFCC66FF, "В работе: доставка к загрузке.");
        return SendClientMessage(playerid, 0xFFCC66FF, "В работе: груз едет на выгрузку.");
    }
    return SendClientMessage(playerid, 0xCECECEFF, "Заказов нет. Создайте заказ через /cargo.");
}
...
CMD:tc(playerid, params[])
{
    ...
    if(sscanf(params, "s[128]", message))
        return SendClientMessage(playerid, 0xCECECEFF, "Синтаксис: /tc [сообщение]");
    new companyid = TCompany_GetPlayerCompany(playerid);
    if(companyid == -1)
        return SendClientMessage(playerid, 0xCECECEFF, "Вы не сотрудник ТК.");
    new fmt[180];
    format(fmt, sizeof fmt, "{FFCC00}[ТК] {FFFFFF}%s: %s", GetPlayerNameEx(playerid), message);
    foreach(new i : Player)
    {
        if(TCompany_GetPlayerCompany(i) == companyid)
            SendClientMessage(i, 0xFFCC66FF, fmt);
    }
    return 1;
}
        ...
        case DIALOG_TCOMPANY_MENU:
        {
            ...
            if(is_owner)
            {
                switch(listitem)
                {
                    ...
                    case 3:
                        return TCompany_StartOrder(playerid);
                    case 4:
                    {
                    ...
                    case 5: Dialog(playerid, DIALOG_TCOMPANY_RENAME, DIALOG_STYLE_INPUT, "Название компании", "{FFFFFF}Введите новое название транспортной компании (3-23 символа):", "Изменить", "Назад");
                    case 6: Dialog(playerid, DIALOG_TCOMPANY_SELL, DIALOG_STYLE_MSGBOX, "Продажа ТК государству", "{FFFFFF}Продать транспортную компанию государству за {66CC00}70.000.000 руб.{FFFFFF}?\nСотрудники будут уволены, ТК снова попадёт на аукцион.", "Продать", "Отмена");
                }
            }
            else
            {
                switch(listitem)
                {
                    ...
                    case 1: TCompany_ShowStaff(playerid, businessid);
                    case 2: TCompany_ShowFleet(playerid, businessid);
                    case 3: return TCompany_StartOrder(playerid);
                    case 4: Dialog(playerid, DIALOG_TCOMPANY_LEAVE, DIALOG_STYLE_MSGBOX, "Транспортная компания", "{FFFFFF}Вы действительно хотите покинуть транспортную компанию?", "Покинуть", "Отмена");
                }
            }
            return 1;
        }
...
stock TCompany_LoadVehicles()
{
    for(new v; v < MAX_VEHICLES; v++)
    {
        ...
        g_tcompany_vehicle_trailer[v] = INVALID_VEHICLE_ID;
    }
