...
{
    //19133
    CreateDynamicPickupEx(1318, 23, -2341.432128,1581.900878,9.899374);
    CreateDynamicPickupEx(1318, 23, 2500.420898,2500.220703,1500.997192);
    CreateDynamicPickupEx(1314, 23, 2534.543945,2526.558593,1501.286254);
    CreateDynamicPickupEx(1318, 23, -2362.439453,1591.376220,9.891562);
    p_wedding = CreateDynamicSphere(-2341.432128,1581.900878,9.899374, 1.0);
    ...
    p_wedding_car =  CreateDynamicSphere(-2362.439453,1591.376220,9.891562, 3.0);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
            ...
            switch(listitem)
            {
                case 0:
                {
                    ...
                    (
                        ...
                        "{FF6347}"SERVER_NAME"{ffffff} | Подробнее о свадьбах",
                ...
                case 1:
                {
                        ...
                        (
                            ...
                            "{FF6347}"SERVER_NAME"{ffffff} | Обручальные кольца",
    ...
    if(dialogid == 1402)
    {
        if(response)
        {
            ...
            Dialog
            (
                playerid, 1405, DIALOG_STYLE_MSGBOX,
...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    ...
    if(areaid == p_wedding_menu)
    {
        Dialog
        (
            playerid, 1430, DIALOG_STYLE_LIST, 
    ...
    if(areaid == p_wedding_car)
    {
        Dialog
        (
            playerid, 1404, DIALOG_STYLE_LIST, 
...
CMD:wedding(playerid, params[])
{
    ...
    if(GetPlayerVirtualWorld(playerid) != 143) 
        return Send(playerid, -1, ""USC" Вы должны находиться в ЗАГСе");

    new Float: to_x, Float: to_y, Float: z;
    GetPlayerPos(to_wife, to_x, to_y, z);

    // ИСПРАВЛЕНО: Проверка дистанции (если далеко — отказ)
    if(GetPlayerVirtualWorld(playerid) != GetPlayerVirtualWorld(to_wife) || GetPlayerDistanceFromPoint(playerid, to_x, to_y, z) > 10.0) 
        return Send(playerid, -1, "Игрок далеко от вас");

    if(GetPlayerData(to_wife, P_WIFE) || GetPlayerData(playerid, P_WIFE)) 
        return Send(playerid, -1, "У вас или у игрока уже есть вторая половинка");
   
    // Записываем, кто предлагает брак
    ...
    // Используем старую функцию, которая сама вызовет команду согласия при клике на ">>"
    format(string128, sizeof string128, "Игрок %s делает вам предложение!", GetPlayerNameEx(playerid));
    ShowNotification(to_wife, 4, string128, 8, "/yes_wedding", ">>");
    return 1;
}
...
CMD:yes_wedding(playerid)
{
    ...
    // Проверяем, на месте ли еще инициатор
    if(!IsPlayerConnected(to_player)) {
        DeletePVar(playerid, "player_wedding");
        return Send(playerid, -1, "Игрок, предложивший брак, вышел из игры");
    }

    if(GetPlayerVirtualWorld(playerid) != GetPlayerVirtualWorld(to_player))
        return Send(playerid, -1, "Игрок далеко от вас");

    if(GetPlayerData(to_player, P_WIFE) || GetPlayerData(playerid, P_WIFE)) {
        DeletePVar(playerid, "player_wedding");
        return Send(playerid, -1, "У вас или у партнера уже есть вторая половинка");
    }

    // === ЛОГИКА СОХРАНЕНИЯ БРАКА ===
    SetPlayerData(playerid, P_WIFE, GetPlayerAccountID(to_player)); 
    SetPlayerData(to_player, P_WIFE, GetPlayerAccountID(playerid));

    // Выводим сообщения об успехе молодоженам
    format(string128, sizeof string128, "Поздравляем! Вы успешно вступили в брак с %s!", GetPlayerNameEx(to_player));
    Send(playerid, 0x00FF00FF, string128);

    format(string128, sizeof string128, "Поздравляем! Вы успешно вступили в брак с %s!", GetPlayerNameEx(playerid));
    Send(to_player, 0x00FF00FF, string128);

    // === ОБЩЕЕ ОБЪЯВЛЕНИЕ В ЧАТ (СВЕТЛО-МАЛИНОВЫЙ ЦВЕТ) ===
    // ЦВЕТ: 0xDC143CFF (Crimson / Малиновый) или 0xFF007FFF (Rose)
    new wedding_string[144];
    format(wedding_string, sizeof wedding_string, "[Внимание] У нас новая семейная пара: %s и %s! Пожелаем им счастья и благополучия!", GetPlayerNameEx(to_player), GetPlayerNameEx(playerid));
    SendClientMessageToAll(0xDC143CFF, wedding_string);
...
CMD:divorce(playerid)
{
    ...
    Dialog(playerid, 1403, DIALOG_STYLE_MSGBOX, "Развод", "Вы действительно желаете расторгнуть брак со своей второй половинкой?", "Да", "Отмена");
    return 1;
}
