...
#define FIXED_BUSINESS_COUNT 5
#define FIXED_BUSINESS_0 53
#define FIXED_BUSINESS_1 54
#define FIXED_BUSINESS_2 55
#define FIXED_BUSINESS_3 57
...
public OnGameModeInit()
{
    ...
    mysql_query(mysql, "CREATE TABLE IF NOT EXISTS auction_fixed_business (business_id INT NOT NULL PRIMARY KEY, owner_rate INT NOT NULL DEFAULT -1, current_rate INT NOT NULL DEFAULT 0, timer_end INT NOT NULL DEFAULT 0)", false);
    if(mysql_errno()) print("[ERROR AUCTION MYSQL] FIXED BUSINESS TABLE");
...
public OnPlayerClickPlayerTextDraw(playerid, PlayerText:playertextid)
{
    ...
    if(playertextid == auction_PTD[playerid][37])
    {
        ...
        switch(auction_slot[id_slot][SLOT_TYPE])
        {
            case TYPE_BUSINESS:
            {
                new target_business = auction_slot[id_slot][SERVER_ID];
                {
...
public:AuctionUpdate()
{
    if(!load_business_action)
    {
        ...
        // Businesses 53/54/55/57 are regular /buybiz businesses, not auction lots.
...
stock create_auction_slot(playerid, type, opisanie[124], start_rate, data_base, server_id, number_vehicle[7] = "", number_SUM = 0)
{
    ...
    for(new s = FIXED_BUSINESS_COUNT; s < MAX_AUCTION_SLOT;s++)
    {
...
stock finish_auction_slot(slot)
{
    ...
    switch(auction_slot[slot][SLOT_TYPE])
    {
        case TYPE_BUSINESS:
        {
            ...
            {
                new fixed_sql = auction_slot[slot][DATABASE_ID];
                reset_slot_auction(slot);
                DeleteFixedBusinessAuction(fixed_sql);
                return 1;
            }
...
stock AuctionBusiness(sql_id, slot)
{
        ...
        if(businessid >= 0 && businessid < MAX_BUSINESS && GetBusinessData(businessid, B_TYPE) == -999999)
        {
            if(0)
            {
                foreach(new i : Player)
                {
                    if(GetPlayerAccountID(i) == sql_id)
                        SendClientMessage(i, 0x66CC00FF, "Поздравляем! Вы выиграли транспортную компанию на аукционе. Управление: /tcompany");
                }
                reset_slot_auction(slot);
                return 1;
            }
...
public:LoadBusinessToAuction()
{
    ...
    // Fixed business auctions are tied to the real business array indexes,
    // not to the order of rows returned by MySQL. This prevents the auction
    // list from becoming empty when there are skipped/static business rows.
    new fixed_businesses[FIXED_BUSINESS_COUNT] =
    {
        FIXED_BUSINESS_0,
        FIXED_BUSINESS_1,
        FIXED_BUSINESS_2,
    ...
    for(new f; f < FIXED_BUSINESS_COUNT; f++)
    {
        new businessid = fixed_businesses[f];

        if(businessid < 0 || businessid >= MAX_BUSINESS) continue;

        new business_sql = GetBusinessData(businessid, B_SQL_ID);
        if(business_sql <= 0) continue;

        // An already owned business must not remain in the auction.
        if(IsBusinessOwned(businessid))
        {
            if(auction_slot[f][SLOT_TYPE] == TYPE_BUSINESS)
                reset_slot_auction(f);

            DeleteFixedBusinessAuction(business_sql);
            continue;
        }

        // Do not create duplicate slots. Fixed businesses always use slots 0..3.
        if(auction_slot[f][SLOT_TYPE] != TYPE_BUSINESS)
            reset_slot_auction(f);

        auction_slot[f][SLOT_TYPE] = TYPE_BUSINESS;
        auction_slot[f][OWNER_SLOT_SQL] = -2;
        auction_slot[f][DATABASE_ID] = business_sql;
        auction_slot[f][SERVER_ID] = businessid;
        auction_slot[f][START_RATE] = GetBusinessData(businessid, B_PRICE);
        if(auction_slot[f][START_RATE] <= 0)
            auction_slot[f][START_RATE] = 5000000;

        format(auction_slot[f][OWNER_SLOT_NAME], 24, "\xc3\xee\xf1\xf3\xe4\xe0\xf0\xf1\xf2\xe2\xee");

        switch(businessid)
        {
            case FIXED_BUSINESS_0: format(auction_slot[f][SLOT_NAME], 24, "\xd2\xe5\xf5\xed\xe8\xf7\xe5\xf1\xea\xe8\xe9 \xf6\xe5\xed\xf2\xf0");
            case FIXED_BUSINESS_1: format(auction_slot[f][SLOT_NAME], 24, "\xd8\xe8\xed\xee\xec\xee\xed\xf2\xe0\xe6\xed\xfb\xe9 \xf6\xe5\xed\xf2\xf0");
            case FIXED_BUSINESS_2: format(auction_slot[f][SLOT_NAME], 24, "\xd1\xf2\xe0\xe9\xeb\xe8\xed\xe3 \xf6\xe5\xed\xf2\xf0");
            case FIXED_BUSINESS_3: format(auction_slot[f][SLOT_NAME], 24, "\xd1\xd2\xce");
        }

        format(auction_slot[f][OPISANIE_SLOT], 124, "\xc1\xe8\xe7\xed\xe5\xf1 (%d)", businessid);

        // Load the persistent bid/timer for this exact business.
        new query[256], Cache:result;
        mysql_format(mysql, query, sizeof query,
            "SELECT owner_rate,current_rate,timer_end FROM auction_fixed_business WHERE business_id=%d LIMIT 1",
            business_sql);
        result = mysql_query(mysql, query, true);
        if(mysql_errno())
        {
            print("[ERROR AUCTION MYSQL] FIXED BUSINESS LOAD");
            cache_delete(result);
            continue;
        }
        ...
        if(cache_num_rows())
        {
            auction_slot[f][OWNER_RATE] = cache_get_field_content_int(0, "owner_rate");
            auction_slot[f][CURRENT_RATE] = cache_get_field_content_int(0, "current_rate");
            auction_slot[f][TIMER_END] = cache_get_field_content_int(0, "timer_end");

            if(auction_slot[f][CURRENT_RATE] <= 0)
                auction_slot[f][CURRENT_RATE] = auction_slot[f][START_RATE];
        }
        else
        {
            auction_slot[f][OWNER_RATE] = -1;
            auction_slot[f][CURRENT_RATE] = auction_slot[f][START_RATE];
            auction_slot[f][TIMER_END] = 0;
        }
        cache_delete(result);

        // Create a timer only when there is no valid saved timer.
        // A valid timer is never reset on player/server reconnect.
        if(auction_slot[f][TIMER_END] <= gettime())
        {
            new hours, minutes, seconds;
            gettime(hours, minutes, seconds);
            ...
            new m = 59 - minutes;
            new sec = 59 - seconds;
            new duration = (h * 3600) + (m * 60) + sec - 10;

            if(duration <= 3600)
                duration = 1800;

            auction_slot[f][TIMER_END] = gettime() + duration;
            auction_slot[f][OWNER_RATE] = -1;
            auction_slot[f][CURRENT_RATE] = auction_slot[f][START_RATE];
        }

        SaveFixedBusinessAuction(f);
    }

    return 1;
}

stock bool:IsFixedBusinessAuctionIndex(businessid)
{
    if(businessid == FIXED_BUSINESS_0 || businessid == FIXED_BUSINESS_1 || businessid == FIXED_BUSINESS_2 || businessid == FIXED_BUSINESS_3)
        return true;
    if(businessid >= 0 && businessid < MAX_BUSINESS && GetBusinessData(businessid, B_TYPE) == -999999)
        return true;
    return false;
...
stock GetFixedBusinessAuctionSlot(business_sql)
{
    for(new businessid = 0; businessid < MAX_BUSINESS; businessid++)
    {
        if(!IsFixedBusinessAuctionIndex(businessid)) continue;
        if(GetBusinessData(businessid, B_SQL_ID) != business_sql) continue;

        switch(businessid)
        {
            case FIXED_BUSINESS_0: return 0;
            case FIXED_BUSINESS_1: return 1;
            case FIXED_BUSINESS_2: return 2;
            case FIXED_BUSINESS_3: return 3;
        }
        if(GetBusinessData(businessid, B_TYPE) == -999999)
            return 4;
    }
    return -1;
}

stock bool:IsFixedBusinessAuction(business_sql)
{
    return GetFixedBusinessAuctionSlot(business_sql) != -1;
}

stock SaveFixedBusinessAuction(slot)
{
    if(auction_slot[slot][SLOT_TYPE] != TYPE_BUSINESS) return 1;
    if(!IsFixedBusinessAuction(auction_slot[slot][DATABASE_ID])) return 1;

    new query[512];
    mysql_format(mysql, query, sizeof query,
        "INSERT INTO auction_fixed_business (business_id,owner_rate,current_rate,timer_end) VALUES (%d,%d,%d,%d) ON DUPLICATE KEY UPDATE owner_rate=VALUES(owner_rate),current_rate=VALUES(current_rate),timer_end=VALUES(timer_end)",
        auction_slot[slot][DATABASE_ID], auction_slot[slot][OWNER_RATE], auction_slot[slot][CURRENT_RATE], auction_slot[slot][TIMER_END]);
    mysql_query(mysql, query, false);
    if(mysql_errno()) return print("[ERROR AUCTION MYSQL] FIXED SAVE");
    return 1;
}

stock DeleteFixedBusinessAuction(business_sql)
{
    if(!IsFixedBusinessAuction(business_sql)) return 1;

    new query[128];
    mysql_format(mysql, query, sizeof query, "DELETE FROM auction_fixed_business WHERE business_id=%d", business_sql);
    mysql_query(mysql, query, false);
    if(mysql_errno()) return print("[ERROR AUCTION MYSQL] FIXED DELETE");
    return 1;
}
