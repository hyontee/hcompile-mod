...
public OnGameModeInit()
{
    ...
    CAPTURE_3DTEXT[0] = Create3DTextLabel("{FFF000}Вышка сотовой связи №1\n{7FFFD4}Данным предприятием владеет семья", -1, text_3d_capt[0][0], text_3d_capt[0][1], text_3d_capt[0][2]+1.0, 10.0, 0, 1);
    CAPTURE_3DTEXT[3] = Create3DTextLabel("{FFF000}Вышка сотовой связи №2\n{7FFFD4}Данным предприятием владеет семья", -1, text_3d_capt[3][0], text_3d_capt[3][1], text_3d_capt[3][2]+1.0, 10.0, 0, 1);
    CAPTURE_3DTEXT[4] = Create3DTextLabel("{FFF000}Вышка сотовой связи №3\n{7FFFD4}Данным предприятием владеет семья", -1, text_3d_capt[4][0], text_3d_capt[4][1], text_3d_capt[4][2]+1.0, 10.0, 0, 1);
    CAPTURE_3DTEXT[1] = Create3DTextLabel("{FFF000}Ферма Гарель\n{7FFFD4}Данным предприятием владеет семья", -1, text_3d_capt[1][0], text_3d_capt[1][1], text_3d_capt[1][2]+1.0, 10.0, 0, 1);
    CAPTURE_3DTEXT[2] = Create3DTextLabel("{FFF000}Завод Бусаево\n{7FFFD4}Данным предприятием владеет семья", -1, text_3d_capt[2][0], text_3d_capt[2][1], text_3d_capt[2][2]+1.0, 10.0, 0, 1);
...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    if(areaid == CAPTURE_AREA[0] || areaid == CAPTURE_AREA[1] || areaid == CAPTURE_AREA[2] || areaid == CAPTURE_AREA[3] || areaid == CAPTURE_AREA[4])
    {
        ...
        if(f < 0 || f >= MAX_FAMILY)
        {
            printf("[ERROR] OnPlayerEnterDynamicArea: выход за границы! f:%d (MAX:%d)", f, MAX_FAMILY);
            #if defined capt_OnPlayerEnterDynamicArea
                return capt_OnPlayerEnterDynamicArea(playerid, STREAMER_TAG_AREA:areaid);
            #else
                return 0;
...
public OnPlayerLeaveDynamicArea(playerid, areaid)
{
    if(areaid == CAPTURE_AREA[0] || areaid == CAPTURE_AREA[1] || areaid == CAPTURE_AREA[2] || areaid == CAPTURE_AREA[3] || areaid == CAPTURE_AREA[4]) 
    {
        ...
        if(f < 0 || f >= MAX_FAMILY || type_capt < 0 || type_capt >= 5)
        {
            printf("[ERROR] Выход за границы массива! f:%d (MAX:%d), type_capt:%d", f, MAX_FAMILY, type_capt);
...
stock StartFCapture(type_capt)
{
    ...
    for(new i;i < welsi;i++)
    {
        ...
        format(string, sizeof string, "{7FFFD4}%s\n{7FFFD4}Данным предприятием владеет семья", name_object[e[i]]);
        ...
        format(string, sizeof string, "{7FFFD4}|{7FFFD4} Началось сражение за {7FFFD4}%s{7FFFD4}, найдите место проведение с помощью — карты", name_object[e[i]]);
    ...
    if(GetPlayerIdFamily(playerid) != -1 && CAPTURE_STATUS[0] || CAPTURE_STATUS[1] || CAPTURE_STATUS[2])
    {
        if(CAPTURE_PLAYER_EXP[playerid] != 0)
        {
            ...
            if(killerid != INVALID_PLAYER_ID)
            {
                ...
                // ? КРИТИЧЕСКАЯ ПРОВЕРКА:
                if(f != -1 && f >= 0 && f < MAX_FAMILY && capt != -1)
                {
...
{   
    for(new i;i < sizeof(fam_TD);i++)
    {
        ...
        fam_TD[i][2] = TextDrawCreate(10.3332, 252.2221, "очки вашей семьи:"); // пусто
        ...
        fam_TD[i][3] = TextDrawCreate(10.3332, 266.7406, "лидер сражения:"); // пусто
...
stock StartFCont(type_capt)
{
    ...
    for(new i;i < welsi;i++)
    {
        ...
        format(string, sizeof string, "{7FFFD4}|{7FFFD4} Началось сражение за {7FFFD4}Семейные контейнеры{7FFFD4}, найдите место проведение с помощью — карты");
