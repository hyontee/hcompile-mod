...
#define SC              "{FFFFFF}| {ffffff}"
#define USC             "{FFFFFF}| {ffffff}"
...
{
    print("[LAIRD_SYSTEM] Система AIRDROP'а загружена\n Welsi(t.me/welsistudio)");
...
stock AirDrop()
{
    ...
    SendClientMessageToAll(-1, ""SC" На сервере произошло события {FFFFFF}AIRDROP {686868}(длиться 40 минут)");
    SendClientMessageToAll(-1, ""SC" Ищите на карте {FFFFFF}ящики {FFFFFF}с призами и ломайте чтобы получить приз");
    SendClientMessageToAll(-1, ""SC" Чтобы узнать информацию о {FFFFFF}ящиках{FFFFFF} используйте {FFFFFF}/airdrop");
    ...
    if(air_sphere[0] <= areaid <= air_sphere[sizeof air_sphere - 1])
    {
       ...
       {    
            if(airdrop[id_air][AIR_STATE] == AIR_STATE_CRASH)
            {
                ...
                "Вы открыли ящик[%d]\n\
                Вам выпало {FFFFFF}%d{FFFFFF} рублей, {FFFFFF}%d {FFFFFF}донат-рублей, {FFFFFF}%d {FFFFFF}очков опыта\n\
...
stock UnAirDrop()
{
    ...
    for(new i = 0; i < 5; i++)
    {
        SendClientMessageToAll(-1, "");
           ...
           //Автор: Welsi Тг канал:t.me/welsistudio
    SendClientMessageToAll(-1, ""SC" На сервере завершено событие {FFFFFF}AIRDROP");
    SendClientMessageToAll(-1, ""SC" Все {FFFFFF}ящики {FFFFFF}с призами на карте были удалены");
    SendClientMessageToAll(-1, ""SC" Следущий {FFFFFF}AIRDROP {FFFFFF}произойдет в {FFFFFF}XX:00");
