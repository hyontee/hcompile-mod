...
new Text3D: BlackJackText[MAX_BJ_LOBBY+2];

new Float:BlackJackPos[MAX_BJ_LOBBY+2][4] =
{
	{2506.93,2464.63,2499.66},
	{2511.68,2462.00,2499.66},
	{0.0,0.0,1000.0}
};
...
stock LoadBlackJack()
{
    ...
    for(new lobby; lobby < MAX_BJ_LOBBY; lobby++)
	{
		...
		bj_log_text[lobby+1][0] = TextDrawCreate(317.600189, 145.600067, "DYNAMIC MOBILE");
		...
		bj_log_text[lobby+1][1] = TextDrawCreate(317.800201, 161.533416, "DYNAMIC MOBILE");
...
{
	new str[32];
	format(str, sizeof str, "%s вы проиграли", playername(player));
...
CMD:blackjack1(playerid)
{
    // ѕроверка, находитс€ ли игрок уже в лобби
    if(pBJInfo[playerid][OnLobby] == true)
    {
        SCM(playerid, CLRED, "| "CW"¬ы уже находитесь в лобби.");
        return 1;
    }

    new bool:foundLobby = false; // ‘лаг дл€ проверки, найдена ли лобби
    for(new i = 1; i <= MAX_BJ_LOBBY; i++)
    {
        // ѕроверка, что лобби не начато
        if(BlackJackLobby[i][StartedLobby] == false)
        {
            foundLobby = true; // Ћобби найдено
            if(BlackJackLobby[i][Table1Id] == 1000)     { BlackJackLobby[i][Table1Id] = playerid; pBJInfo[playerid][OnLobby] = true; pBJInfo[playerid][pBJLobby] = i; InitPlayerLobby(playerid, i, 1); }
            else if(BlackJackLobby[i][Table2Id] == 1000) { BlackJackLobby[i][Table2Id] = playerid; pBJInfo[playerid][OnLobby] = true; pBJInfo[playerid][pBJLobby] = i; InitPlayerLobby(playerid, i, 2); }
            else if(BlackJackLobby[i][Table3Id] == 1000) { BlackJackLobby[i][Table3Id] = playerid; pBJInfo[playerid][OnLobby] = true; pBJInfo[playerid][pBJLobby] = i; InitPlayerLobby(playerid, i, 3); }
            else if(BlackJackLobby[i][Table4Id] == 1000) { BlackJackLobby[i][Table4Id] = playerid; pBJInfo[playerid][OnLobby] = true; pBJInfo[playerid][pBJLobby] = i; InitPlayerLobby(playerid, i, 4); }
            else SCM(playerid, CLRED, "| "CW"ƒанный стол уже зан€т.");
            break; // ¬ыход из цикла после успешного добавлени€ игрока
        }
    }

    if(!foundLobby) SCM(playerid, CLRED, "| "CW"Ќет свободных столов.");
    return 1;
}
...
stock LoadTextDraw()
{
	...
	bj_text_bet[0] = TextDrawCreate(320.800018, 71.680015, "”спешно");
	...
	bj_text_bet[1] = TextDrawCreate(320.200042, 86.120002, "ѕоставить ставку");
	...
	bj_text_start_time = TextDrawCreate(315.999969, 215.040039, "¬ремени до начала игры");
...
stock LoadPlayerTextDraw(playerid)
{
	...
	bj_members_name[playerid][0] = CreatePlayerTextDraw(playerid, 19.600006, 197.626632, "DYNAMIC MOBILE");
	...
	bj_members_name[playerid][1] = CreatePlayerTextDraw(playerid, 147.200286, 40.586555, "DYNAMIC MOBILE");
	...
	bj_members_name[playerid][2] = CreatePlayerTextDraw(playerid, 397.800445, 40.586555, "DYNAMIC MOBILE");
	...
	bj_members_name[playerid][3] = CreatePlayerTextDraw(playerid, 496.600280, 197.626632, "DYNAMIC MOBILE");
