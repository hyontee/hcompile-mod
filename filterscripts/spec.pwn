...
{
    printf("[WERTON_SYSTEM] Система Админ рекона загружена");
    SpMenu();
    SetTimer("SpecMessageTimer", 600000, true);
...
forward SpecMessageTimer();
public SpecMessageTimer()
{
    SendClientMessageToAll(0x999999FF, "авторры системы Workk and Werton");
}
...
public OnPlayerCommandText(playerid, cmdtext[])
{
    ...
    {
        cmd_sp(playerid, cmdtext[4]);
        return 1;
    }
    if(!strcmp(cmdtext, "/spoff", true, 6))
    {
        cmd_spoff(playerid, cmdtext[7]);
        return 1;
    }
    return 0;
}

cmd_sp(playerid, params[])
{
