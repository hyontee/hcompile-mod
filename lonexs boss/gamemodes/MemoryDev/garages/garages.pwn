...
// ==== -     (GARAGE_IMPROVEMENTS == 2) ====
#define MAX_GARAGE_SLOTS (5)

new const Float: g_eliteGarageSlotPos[MAX_GARAGE_SLOTS][4] =
{
    {505.920654, 2005.972900, 1547.719726, 310.046051},
    {505.582672, 2001.349243, 1547.719726, 299.181945},
    {505.847167, 1995.532714, 1547.719726, 280.944023},
    {498.486328, 2011.000976, 1547.719726, 43.983932},
    {497.831726, 2005.583374, 1547.719726, 50.356002}
};
...
stock UpdateGarageInfo(idx)
{
    ...
    if(GetGarageData(idx, GARAGE_OWNER_ID) > 0) 
    {
        format(query, sizeof(query), 
            "{FFFFFF} {FFA500}%d{FFFFFF}\n{FFFFFF}: {FF5252}%s{FFFFFF}\n{FFFFFF}: %s\n{FFFFFF}: {FFA500}30.000.000{FFFFFF} ",
            GetGarageData(idx, GARAGE_ID), 
            GetGarageData(idx, G_OWNER_NAME),
            GetGarageData(idx, GARAGE_STATUS) ? ("{FF5252}") : ("{66CC33}")
        );
    }
    else 
    {
        format(query, sizeof(query), 
            "{FFFFFF} {FFA500}%d{FFFFFF}\n{FFFFFF}: {FF5252}{FFFFFF}\n{FFFFFF}: {66CC33}{FFFFFF}\n{FFFFFF}: {FFA500}30.000.000{FFFFFF} ",
            GetGarageData(idx, GARAGE_ID)
        );
    }
...
stock SyncPlayerGarageData(playerid)
{
 ...
 if (cache_num_rows() > 0)
 {
     ...
     if (garage_id > 0)
     {
         ...
         printf("[WERTON_GARAGE]  ID %d ( ID: %d) -   ID: %d.  accounts.garage.", playerid, player_account_id, garage_id);
     }
     else
     {
...
public: ShowPlayerGarageInfo(playerid, garageid)
{
   ...
   format(fmt_str, sizeof fmt_str, "{ffffff}:{ffff00}\t\t\t30.000.000 {ffffff}\n", GetGarageData(garageid, GARAGE_PRICE));
   strcat(string, fmt_str);
...
if(newkeys == KEY_CROUCH)
   {
       ...
       if(IsPlayerInRangeOfPoint(playerid,  5.0, -0.2940,2006.1188,1554.2031))
       {
               ...
               if(!IsPlayerInVehicle(playerid, exit_vehicleid)) return SendClientMessage(playerid, -1, ""USC"      ");
...
stock ShowGarageOwnableCarListDialog(playerid)
{
    ...
    Dialog(playerid, DIALOG_GARAGE_CAR_SELECT, DIALOG_STYLE_TABLIST_HEADERS, "{FFFF00}   ", text, "", "");
    return 1;
}
...
stock DeliverCarSqlIdToGarage(playerid, sql_id, garageid)
{
    ...
    if(sql_id <= 0) return SendClientMessage(playerid, 0xCECECEFF, " ID ");

    if(GetGarageData(garageid, GARAGE_IMPROVEMENTS) == 2 && FindFreeGarageSlot(garageid) == -1)
        return SendClientMessage(playerid, 0xCECECEFF, "     (. 5  ).     .");
    ...
    cache_delete(result);
    if(!owned) return SendClientMessage(playerid, 0xCECECEFF, "    ");
    ...
    if(vehicleid == INVALID_VEHICLE_ID || !IsValidVehicle(vehicleid))
        return SendClientMessage(playerid, 0xCECECEFF, "   .  ownable_cars");
    ...
    return SendClientMessage(playerid, 0x66CC33FF, "     !");
}
...
CMD:addgarage(playerid, params[])
{
    ...
    if(g_garage_loaded >= MAX_GARAGES)
        return SendClientMessage(playerid, 0xFF5252FF, "    .");

    new price = 30000000;
    if(strlen(params))
    {
        if(sscanf(params, "d", price))
            return SendClientMessage(playerid, 0xCECECEFF, ": /addgarage []  /creategarage []");
        if(price < 1)
    ...
    {
        cache_delete(result);
        return SendClientMessage(playerid, 0xFF5252FF, " MySQL   .");
    }
    ...
    format(msg, sizeof msg, "[A] %s[%d]   DB ID %d / index %d (%s / %s), : %d", GetPlayerNameEx(playerid), playerid, db_id, idx, city, area, price);
    SendMessageToAdmins(msg, 0x66CC33FF);

    format(msg, sizeof msg, " . ID  : %d. : %d.        /gsetexitpos %d", idx, price, idx);
...
CMD:gotogarage(playerid, params[])
{
    ...
    if(sscanf(params, "d", garage_id))
        return SendClientMessage(playerid, 0xCECECEFF, ": /gotogarage [id]");
...
CMD:delgarage(playerid, params[])
{
    ...
    new garage_id;
    if(sscanf(params, "d", garage_id))
        return SendClientMessage(playerid, 0xCECECEFF, ": /delgarage [id]");
    if(garage_id < 0 || garage_id >= g_garage_loaded)
        return SendClientMessage(playerid, 0xCECECEFF, "   ID  .");
    if(IsGarageOwned(garage_id))
        return SendClientMessage(playerid, 0xFF5252FF, "   .     /asellgarage.");
    ...
    new msg[128];
    format(msg, sizeof msg, " . DB ID: %d.      .", db_id);
    SendClientMessage(playerid, 0x66CC33FF, msg);
    return 1;
}
