...
#define NEW_YEAR_WAYPOINT_ID 2026
#define NEW_YEAR_MUSIC_URL "https://ru.hitmoz.org/get/music/20170903/Wham_-_Last_Christmas_48076479.mp3"

new const Float:winter_lights_route[][3] =
{
    {-1375.116943, -277.131561, 256.216766},
    {-540.210021, -1601.124511, 41.343948},
    {-2034.014404, -1353.431396, 48.192638},
    {-2156.958007, 998.339843, 2.630023},
    {-1415.527343, -321.201171, 253.228469}
};

new shpere_ded_moroz = -1, Float:ded_coord[4] = {-1368.908081,-293.392639,256.228637,223.291610}; //дед мороз

new Float:obmen_npc[4] = {-1415.527343,-321.201171,253.228469,326.760772}, shpere_obmen_npc = -1; // обмен
new Float:uncle_claus_coord[4] = {-1383.742431,-286.618164,256.228637,145.0}, shpere_uncle_claus = -1; // Дядя Клаус
...
enum E_WINTER_SKIN_PRIZE
{
    WINTER_SKIN_NAME[22],
    WINTER_SKIN_ID
}
new const skin_prize[][E_WINTER_SKIN_PRIZE] =
{
    {"Мальчик", 89},
    {"Шейх", 94},
...
{
    SendClientMessage(playerid, -1, "{7DDCFF}[Новый год] {FFFFFF}Ивент и мини-игры: {FFFF00}/newyear {FFFFFF}| Зимнее задание: {FFFF00}/winterquest");
    return 1;
}

public OnPlayerDisconnect(playerid, reason)
{
    if(GetPVarInt(playerid, "winter_lights_active")) StopWinterLightsQuest(playerid);
    ToggleInteractionWindow(playerid, false);
    DeletePVar(playerid, "newyear_npc_area");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == 4460)
    {
        if(response) callcmd::winterquest(playerid, "");
        else ToggleInteractionWindow(playerid, false);
    }
    ...
    if(dialogid == 4432)
    {
        if(response)
        {
            ...
            SendClientMessage(playerid, -1, ""SC" Следуйте по меткам на мини-карте");
    ...
    if(dialogid == 4446)
    {
        if(response)
        {
            if(GetPlayerNewYear(playerid, NEW_QUEST_1) == 0) return SendClientMessage(playerid, -1, ""USC" Сначала пройдите предыдущие задание");
    ...
    if(dialogid == 4447)
    {
        if(response)
        {
            if(GetPlayerNewYear(playerid, NEW_QUEST_2) == 0) return SendClientMessage(playerid, -1, ""USC" Сначала пройдите предыдущие задание");
    ...
    if(dialogid == 4448)
    {
        if(response)
        {
            if(GetPlayerNewYear(playerid, NEW_QUEST_3) == 0) return SendClientMessage(playerid, -1, ""USC" Сначала пройдите предыдущие задание");
    ...
    if(dialogid == 4449)
    {
        if(response)
        {
            if(GetPlayerNewYear(playerid, NEW_QUEST_4) == 0) return SendClientMessage(playerid, -1, ""USC" Сначала пройдите предыдущие задание");
    ...
    if(dialogid == 4453)
    {
        for(new p; p < 11;p++)
        {
            SetDynamicActorPos(q_5_new[playerid][p], end_actor[p][0], end_actor[p][1], end_actor[p][2]);
            SetDynamicActorFacingAngle(q_5_new[playerid][p], end_actor[p][3]);
        }

        for(new p; p < 11;p++)
        {
            ClearDynamicActorAnimations(q_5_new[playerid][p]);
        }
    ...
    if(dialogid == 4438) //box open
    {
        if(response)
        {
            if(1 <= GetPVarInt(playerid, "gift_skin") <= sizeof skin_prize)
            {
                SetPlayerData(playerid, P_SKIN, skin_prize[GetPVarInt(playerid, "gift_skin") - 1][WINTER_SKIN_ID]);
                UpdatePlayerDatabaseInt(playerid, "skin", skin_prize[GetPVarInt(playerid, "gift_skin") - 1][WINTER_SKIN_ID]);
                SetPlayerSkinInit(playerid);
...
{
    if(GetPVarInt(playerid, "winter_lights_active"))
    {
        WinterLightsNextPoint(playerid);
        return 1;
    }
...
stock IsNewYearQuestArea(playerid, quest, areaid)
{
    switch(quest)
    {
        case 1:
        {
            for(new i; i < 5; i++)
                if(q_1_new[playerid][i][1] != -1 && areaid == q_1_new[playerid][i][1]) return 1;
        }
        case 2:
        {
            for(new i; i < 4; i++)
                if(q_2_player[playerid][i][Q_2_SPHERE] != -1 && areaid == q_2_player[playerid][i][Q_2_SPHERE]) return 1;
        }
        case 5:
        {
            for(new i; i < 3; i++)
                if(q_5_sphere[playerid][i] != -1 && areaid == q_5_sphere[playerid][i]) return 1;
        }
    }
    return 0;
}
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    ...
    if(areaid == shpere_ded_moroz || areaid == shpere_obmen_npc || areaid == shpere_uncle_claus)
    {
        SetPVarInt(playerid, "newyear_npc_area", areaid);
        ToggleInteractionWindow(playerid, true);
        return 1;
    }
    if(GetPVarInt(playerid, "quest_3_new_year") && areaid == q_3_new[playerid][1])
    {
    ...
    if(GetPVarInt(playerid, "quest_4_new_year") && areaid == q_4_new[playerid][1])
    {
    ...
    if(GetPVarInt(playerid, "quest_1_new_year") && IsNewYearQuestArea(playerid, 1, areaid))
    {
        if(!(GetPVarInt(playerid, "quest_1_new_year"))) return 0;

        if(GetPlayerVehicleID(playerid)) return SendClientMessage(playerid, -1, ""USC" Сначала выйдите из транспорта");
    ...
    if(IsPlayerInRangeOfAnyGiftBox(playerid) != -1)
    {
        new id = IsPlayerInRangeOfAnyGiftBox(playerid);
        if(!(0 <= id < MAX_BOX_GIFT) || !gift_box[id][GIFT_SPAWN]) return 0;
    ...
    if(GetPlayerNewYear(playerid, MINI_GAME_GIFT) && InPlayerGameGift(playerid, 1) != -1)
    {
        new id = InPlayerGameGift(playerid, 1);
        if(!(0 <= id < sizeof spawn_mini_game_gift)) return 0;
        ...
        if(GetPlayerNewYear(playerid, MINI_GAME_GIFT) == 1)
        {
            ...
            DestroyDynamicPickup(m_g_player_gift[playerid][id][0]);
    ...
    if(GetPVarInt(playerid, "quest_5_new_year") && IsNewYearQuestArea(playerid, 5, areaid))
    {
    ...
    if(GetPlayerNewYear(playerid, MINI_GAME_GIFT) && InPlayerGameGift(playerid, 2) != -1)
    {
        new id = InPlayerGameGift(playerid, 2);
        if(!(0 <= id < sizeof spawn_mini_game_2_gift)) return 0;
        ...
        if(GetPlayerNewYear(playerid, MINI_GAME_GIFT) == 1)
        {
            ...
            DestroyDynamicPickup(m_g_2_player_gift[playerid][id][0]);
    ...
    if(GetPVarInt(playerid, "quest_2_new_year") && IsNewYearQuestArea(playerid, 2, areaid))
    {
...
//Автор данного ивента https://t.me/welsistudio (Welsi Studio)
        ...
        else
        {
            ...
            SendClientMessage(playerid, -1, ""USC" К сожалению... это оказалась палка");
        }
    }
...
public OnPlayerLeaveDynamicArea(playerid, areaid)
{
    ...
    if(areaid == GetPVarInt(playerid, "mini_game_sphere"))
    {
        ...
        if(GetPlayerNewYear(playerid, MINI_GAME_GIFT))
        {
            ...
            {
                if(IsValidDynamicArea(areaid)) DestroyDynamicArea(areaid);
...
public OnGameModeInit()
{
    ...
    CreateActorEx("Дед Мороз", "{C0C0C0}[Новогодние задания]", 24, ded_coord[0], ded_coord[1], ded_coord[2], ded_coord[3]);
    shpere_ded_moroz = CreateDynamicSphere(ded_coord[0], ded_coord[1], ded_coord[2], 2.0);
    CreateActorEx("Местный барыга", "{C0C0C0}[Обмен снежинок]", 80, obmen_npc[0], obmen_npc[1], obmen_npc[2], obmen_npc[3]);
    shpere_obmen_npc = CreateDynamicSphere(obmen_npc[0], obmen_npc[1], obmen_npc[2], 3.0);
    CreateActorEx("Дядя Клаус", "{C0C0C0}[Огни зимней ночи]", 113, uncle_claus_coord[0], uncle_claus_coord[1], uncle_claus_coord[2], uncle_claus_coord[3]);
    shpere_uncle_claus = CreateDynamicSphere(uncle_claus_coord[0], uncle_claus_coord[1], uncle_claus_coord[2], 2.5);
    Create3DTextLabel("Новогодние{FFFF00} Мини-игры\n{C0C0C0}[Подойдите для взаимодействия]", -1, -1376.658691,-277.933319,256.216766, 7.0, 0);
...
stock UnLoadPlayerData_NewYear(playerid)
{
 //Автор данного ивента https://t.me/welsistudio (Welsi Studio)
    ...
    if(GetPlayerNewYear(playerid, MINI_GAME_GIFT))
    {
        if(GetPVarType(playerid, "id_timer_mini_game"))
        {
            KillTimer(GetPVarInt(playerid, "id_timer_mini_game"));
            DeletePVar(playerid, "id_timer_mini_game");
        }
        if(IsValidDynamicArea(GetPVarInt(playerid, "mini_game_sphere")))
            DestroyDynamicArea(GetPVarInt(playerid, "mini_game_sphere"));
        DeletePVar(playerid, "mini_game_sphere");

        for(new idx; idx < sizeof spawn_mini_game_gift; idx++)
        {
            if(IsValidDynamicPickup(m_g_player_gift[playerid][idx][0])) DestroyDynamicPickup(m_g_player_gift[playerid][idx][0]);
            if(IsValidDynamicArea(m_g_player_gift[playerid][idx][1])) DestroyDynamicArea(m_g_player_gift[playerid][idx][1]);
            m_g_player_gift[playerid][idx][0] = -1;
            m_g_player_gift[playerid][idx][1] = -1;
        }
        for(new idx; idx < sizeof spawn_mini_game_2_gift; idx++)
        {
            if(IsValidDynamicPickup(m_g_2_player_gift[playerid][idx][0])) DestroyDynamicPickup(m_g_2_player_gift[playerid][idx][0]);
            if(IsValidDynamicArea(m_g_2_player_gift[playerid][idx][1])) DestroyDynamicArea(m_g_2_player_gift[playerid][idx][1]);
            m_g_2_player_gift[playerid][idx][0] = -1;
            m_g_2_player_gift[playerid][idx][1] = -1;
        }
...
stock quest_complete(playerid, id_q)
{
    ...
    SendCreateWayPoint(playerid, NEW_YEAR_WAYPOINT_ID, ded_coord[0], ded_coord[1], ded_coord[2], 85, 27, 30000.0, 0);
...
stock LoadQuestNewYear(playerid, number_quest)
{
    ...
    switch(number_quest)
    {
        case 1:
        {
            ...
            for(new a;a < 5;a++)
            {
                q_1_new[playerid][a][0] = CreateDynamicActor(quest_1_ACTOR[a][SKIN_QUEST_ONE], quest_1_ACTOR[a][X_QUEST_ONE], quest_1_ACTOR[a][Y_QUEST_ONE], quest_1_ACTOR[a][Z_QUEST_ONE], quest_1_ACTOR[a][A_QUEST_ONE], true, 100.0, virtual, -1, playerid);
        ...
        { 
            ...
            for(new a;a < 4;a++)
            {
                ...
                q_2_player[playerid][a][Q_2_3DTEXT] = CreateDynamic3DTextLabel("Что-то похожее на посох\n{C0C0C0}[Возможное местоположение посоха]", -1, spawn_staff_q_2[a][0], spawn_staff_q_2[a][1], spawn_staff_q_2[a][2], 17.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, virtual, -1, playerid);
            }

            player_gang_zone[playerid] = GangZoneCreate(-2076.646972, -1425.330444, -2009.189331, -1343.253417);
        ...
        case 3:
        {
            ...
            SendCreateWayPoint(playerid, NEW_YEAR_WAYPOINT_ID, -540.210021, -1601.124511, 41.343948, 85, 27, 30000.0, 0);
            ...
            q_3_new[playerid][0] = CreateDynamicActor(245, -540.210021, -1601.124511, 41.343948, 324.716491, true, 100.0, virtual, -1, playerid);
        ...
        case 4:
        {
            ...
            q_4_new[playerid][0] = CreateDynamicActor(39, -2156.958007, 998.339843, 2.630023, 197.331619, true, 100.0, virtual, -1, playerid);
        ...
        case 5:
        {
            ...
            for(new e;e < sizeof spawn_actor_5;e++)
            {
                ...
                q_5_new[playerid][e] = CreateDynamicActor(spawn_actor_5[e][Q_5_SKIN], spawn_actor_5[e][Q_5_X], spawn_actor_5[e][Q_5_Y], spawn_actor_5[e][Q_5_Z], spawn_actor_5[e][Q_5_A], true, 100.0, virtual, 1, playerid);
            }
            //Автор данного ивента https://t.me/welsistudio (Welsi Studio)

            SendCreateWayPoint(playerid, NEW_YEAR_WAYPOINT_ID, spawn_actor_5[0][Q_5_X], spawn_actor_5[0][Q_5_Y], spawn_actor_5[0][Q_5_Z], 85, 27, 30000.0, 1);
        }
    }
}
...
{
    SendDestroyWayPoint(playerid, NEW_YEAR_WAYPOINT_ID);
    ...
    switch(number_quest)
    {
        case 1:
        {
            ...
            for(new a; a<5;a++)
            {
                DestroyDynamicActor(q_1_new[playerid][a][0]);
        ...
        case 2:
        {
            ...
            for(new a;a < 4;a++)
            {
                DestroyDynamic3DTextLabel(q_2_player[playerid][a][Q_2_3DTEXT]);
                DestroyDynamicArea(q_2_player[playerid][a][Q_2_SPHERE]);
                q_2_player[playerid][a][Q_2_3DTEXT] = -1; 
        ...
        case 3:
        {
            ...
            DestroyDynamicActor(q_3_new[playerid][0]);
        ...
        case 4:
        {
            ...
            DestroyDynamicActor(q_4_new[playerid][0]);
        ...
        case 5:
        {
            ...
            {
                DestroyDynamicActor(q_5_new[playerid][end]);
...
{
   if(!IsPlayerConnected(playerid) || !(0 <= gift < MAX_BOX_GIFT)) return 0;
   ...
   {    
        ...
        SetPVarInt(playerid, "gift_skin", rand + 1);

        format(text, sizeof text, "Поздравляем вы получили одежду {FFFF00}\"%s\" {FFFFFF}\nВы хотите его забрать?\nЧтобы забрать нажмите {15FF00}\"Забрать\"", skin_prize[rand][WINTER_SKIN_NAME]);
...
public: LoseMiniGame(playerid, game)
{
    ...
    SendClientMessage(playerid, -1, ""USC" К сожалению... Вы не успели за данное время забрать все подарки");
    ...
    if(game == 1)
    {
        for(new a;a < sizeof spawn_mini_game_gift;a++)
        {
            DestroyDynamicPickup(m_g_player_gift[playerid][a][0]);
            m_g_player_gift[playerid][a][1] = -1;
        }
    ...
    else if(game == 2)
    {
        ...
        for(new a;a < sizeof spawn_mini_game_2_gift;a++)
        {
            DestroyDynamicPickup(m_g_2_player_gift[playerid][a][0]);
            m_g_2_player_gift[playerid][a][1] = -1;
        }
...
stock WinMiniGame(playerid, game)
{
    ...
    if(game == 1)
    {
        ...
        SendClientMessage(playerid, -1, ""SC" Вы успешно выиграли в мини-игре {FFFF00}\"Собери подарки\"");
    ...
    else if(game == 2)
    {
        ...
        SendClientMessage(playerid, -1, ""SC" Вы успешно выиграли в мини-игре {FFFF00}\"Езда на снегоходе\"");
        format(text, sizeof text, "Вы получили %d подарков", gift);
...
stock LoadMiniGame(playerid, game)
{
    ...
    if(game == 1)
    {
        ...
        SendClientMessage(playerid, -1, ""SC" Приготовьтесь. Скоро начнется игра");

        SetTimerEx("StartMiniGame", 3000, false, "ii", playerid, 1);

        for(new a;a < sizeof spawn_mini_game_gift;a++)
        {
            m_g_player_gift[playerid][a][0] = CreateDynamicPickup(19055, 23, spawn_mini_game_gift[a][0], spawn_mini_game_gift[a][1], spawn_mini_game_gift[a][2], virtual, 1, playerid);
        }
    ...
    else if(game == 2)
    {
        ...
        for(new a;a < sizeof spawn_mini_game_2_gift;a++)
        {
            m_g_2_player_gift[playerid][a][0] = CreateDynamicPickup(19055, 23, spawn_mini_game_2_gift[a][0], spawn_mini_game_2_gift[a][1], spawn_mini_game_2_gift[a][2] + 1.0, virtual, -1, playerid);
        }
...
stock HandleNewYearNpcInteraction(playerid)
{
    new areaid = GetPVarInt(playerid, "newyear_npc_area");
    if(areaid == shpere_ded_moroz && IsPlayerInDynamicArea(playerid, shpere_ded_moroz))
    {
        ToggleInteractionWindow(playerid, false);
        if(GetPlayerNewYear(playerid, NEW_QUEST_1) == 0)
        {
            Dialog(playerid, 4430, DIALOG_STYLE_MSGBOX, "Дед Мороз",
                "Здравствуй! Я приготовил задания, подарки и зимние мини-игры.\nПоможешь вернуть праздник?",
                "Посмотреть", "Задания");
        }
        else ShowDialogQuestNewYear(playerid);
        return 1;
    }
    if(areaid == shpere_obmen_npc && IsPlayerInDynamicArea(playerid, shpere_obmen_npc))
    {
        ToggleInteractionWindow(playerid, false);
        new title[48];
        format(title, sizeof title, "Обмен снежков | Баланс: %d", GetPlayerNewYear(playerid, SNOW_COINS));
        Dialog(playerid, 4440, DIALOG_STYLE_LIST, title,
            "{FFFF00}1. {FFFFFF}Обмен на рубли\n{FFFF00}2. {FFFFFF}Обмен на донат\n{FFFF00}3. {FFFFFF}Покупка одежды\n{FFFF00}4. {FFFFFF}Покупка транспорта\n{FFFF00}5. {FFFFFF}Слот транспорта\n{FFFF00}6. {FFFFFF}Покупка опыта",
            "Выбрать", "Закрыть");
        return 1;
    }
    if(areaid == shpere_uncle_claus && IsPlayerInDynamicArea(playerid, shpere_uncle_claus))
    {
        ToggleInteractionWindow(playerid, false);
        Dialog(playerid, 4460, DIALOG_STYLE_MSGBOX, "Дядя Клаус",
            "Ночью погасли пять праздничных огней. Проедь зимний маршрут,\nзажги их снова и получи {FFFF00}250 снежков{FFFFFF}. Музыка уже готова!",
            "Начать", "Позже");
        return 1;
    }
    return 0;
}
stock ShowWinterLightsPoint(playerid)
{
    new stage = GetPVarInt(playerid, "winter_lights_stage");
    if(!(1 <= stage <= sizeof winter_lights_route)) return 0;

    new point = stage - 1, message[96];
    SendDestroyWayPoint(playerid, NEW_YEAR_WAYPOINT_ID);
    SendCreateWayPoint(playerid, NEW_YEAR_WAYPOINT_ID,
        winter_lights_route[point][0], winter_lights_route[point][1], winter_lights_route[point][2],
        85, 27, 30000.0, 0);
    SetPlayerCheckpoint(playerid,
        winter_lights_route[point][0], winter_lights_route[point][1], winter_lights_route[point][2], 6.0);

    format(message, sizeof message, "{7DDCFF}[Зимний маршрут] {FFFFFF}Точка %d из %d отмечена на карте", stage, sizeof winter_lights_route);
    SendClientMessage(playerid, -1, message);
    return 1;
}

stock StopWinterLightsQuest(playerid, bool:completed = false)
{
    DisablePlayerCheckpoint(playerid);
    SendDestroyWayPoint(playerid, NEW_YEAR_WAYPOINT_ID);
    StopAudioStreamForPlayer(playerid);
    DeletePVar(playerid, "winter_lights_active");
    DeletePVar(playerid, "winter_lights_stage");

    if(completed)
    {
        SetPVarInt(playerid, "winter_lights_done", 1);
        AddPlayerNewYear(playerid, SNOW_COINS, +, 250);
        UpdatePlayerDatabaseInt(playerid, "snowcoins", GetPlayerNewYear(playerid, SNOW_COINS));
        ShowNotificationNew(playerid, 3, 7, 0, 0, "Зимний маршрут завершён! Получено 250 снежков", "OK");
        SendClientMessage(playerid, -1, "{7DDCFF}[Зимний маршрут] {FFFFFF}Вы зажгли все праздничные огни!");
    }
    return 1;
}

stock WinterLightsNextPoint(playerid)
{
    new stage = GetPVarInt(playerid, "winter_lights_stage");
    if(!(1 <= stage <= sizeof winter_lights_route)) return StopWinterLightsQuest(playerid);

    DisablePlayerCheckpoint(playerid);
    SendDestroyWayPoint(playerid, NEW_YEAR_WAYPOINT_ID);
    GameTextForPlayer(playerid, "~b~~h~NOVOGODNIY OGON ZAZHZHEN!", 2500, 3);

    if(stage == sizeof winter_lights_route)
        return StopWinterLightsQuest(playerid, true);

    SetPVarInt(playerid, "winter_lights_stage", stage + 1);
    return ShowWinterLightsPoint(playerid);
}

CMD:winterquest(playerid)
{
    if(GetPVarInt(playerid, "winter_lights_active"))
        return SendClientMessage(playerid, -1, "{FFCC00}[Зимний маршрут] {FFFFFF}Вы уже выполняете задание. Используйте /exitsnowquest для отмены");
    if(GetPVarInt(playerid, "winter_lights_done"))
        return SendClientMessage(playerid, -1, "{FFCC00}[Зимний маршрут] {FFFFFF}Сегодня вы уже зажгли праздничные огни");
    if(GetPVarInt(playerid, "quest_1_new_year") || GetPVarInt(playerid, "quest_2_new_year") ||
       GetPVarInt(playerid, "quest_3_new_year") || GetPVarInt(playerid, "quest_4_new_year") || GetPVarInt(playerid, "quest_5_new_year"))
        return SendClientMessage(playerid, -1, "{FFCC00}[Зимний маршрут] {FFFFFF}Сначала завершите текущее новогоднее задание");

    SetPVarInt(playerid, "winter_lights_active", 1);
    SetPVarInt(playerid, "winter_lights_stage", 1);
    PlayerPlayStream(playerid, NEW_YEAR_MUSIC_URL);
    SendClientMessage(playerid, -1, "{7DDCFF}[Зимний маршрут] {FFFFFF}Зажгите пять праздничных огней и получите 250 снежков");
    SendClientMessage(playerid, -1, "{7DDCFF}[Зимний маршрут] {FFFFFF}Музыка включена. Первая точка отмечена на карте");
    return ShowWinterLightsPoint(playerid);
}
CMD:exitsnowquest(playerid)
{
    ...
    else if(GetPVarInt(playerid, "winter_lights_active")) StopWinterLightsQuest(playerid);
...
CMD:newyear(playerid)
{
    SendDestroyWayPoint(playerid, NEW_YEAR_WAYPOINT_ID);
    SendCreateWayPoint(playerid, NEW_YEAR_WAYPOINT_ID, ded_coord[0], ded_coord[1], ded_coord[2], 85, 27, 30000.0, 0);
    SendClientMessage(playerid, -1, "{7DDCFF}[Новый год] {FFFFFF}Дед Мороз отмечен на карте. Дополнительное задание: /winterquest");
