...
{
    print("[MISTERS_SYSTEM] Команда для обмена загружена.");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == 1612)
    {
        if(response)
        {
            ...
            ShowNewNotification(playerid, 5, 5, 1, 10, "Выберите тип имущества игрока", "Ок");
...
CMD:changeprop(playerid, params[])
{
	...
	ShowNotificationMisters(to_player, 4, 5, OFFER_YES_CHANGE, 0, text, ">>");
