...
stock ShowDialogOrders(playerid, Locality)
{
    ...
    DialogTruck(playerid, 2902, DIALOG_STYLE_TABLIST_HEADERS, "{FFAE00}Список актуальных заказов", dialog, "Далее", "Выйти");
    return 1;
}

CMD:setlvltc(playerid, params[])
{
	if(GetPlayerAdminEx(playerid) >= 10) // 6 - начиная с какого уровня администратора это доступно?
	{
		if(!strlen(params))
