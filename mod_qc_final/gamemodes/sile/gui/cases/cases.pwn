...
stock Cases_SendPacketToClient(playerid, const jsonData[])
{
	...
	switch(actionType) {
		...
		case CASES_TYPE_GO_DONATE: {
			...
			if(donateType == 2) {
				Cases_Notify(playerid, 2, "Недостаточно BC", "Откройте донат-магазин для покупки BC.");
			}
		}
...
stock Cases_OpenCase(playerid, caseId, openType)
{
    ...
    Cases_SendBonusPulse(playerid, idx);
...
stock Cases_CheckDustReward(playerid)
{
    ...
    {
        new str[128];
        format(str, sizeof(str), "Особый кейс: %d шт.", rewarded);
        Cases_Notify(playerid, 3, "Вы получили особый кейс за пыль", str);
...
stock Cases_GetRewardDisplayName(rewardType, rewardValue, rewardCount, dest[], destSize)
{
    switch(rewardType)
    {
        ...
        {
            format(dest, destSize, "VIP на %d дн.", rewardCount);
            return 1;
        }
...
stock Cases_SaveRewardToBPR(playerid, rewardType, rewardValue, rewardCount, rewardRarity)
{
    new rewardName[64];
    ...
    switch(rewardType)
    {
        case REWARD_TYPE_CASE:
        {
            BPR_GiveCase(playerid, rewardValue, rewardName, rewardRarity, rewardCount);
            return 1;
        }
        case REWARD_TYPE_VEHICLE:
        {
            BPR_GiveCar(playerid, rewardValue, rewardName, rewardRarity, 365, 500);
            return 1;
        }
        case REWARD_TYPE_ITEM:
        {
            BPR_GiveAccessory(playerid, rewardValue, rewardCount, rewardName, rewardRarity, 365, 300);
            return 1;
        }
        case REWARD_TYPE_DUST:
        {
            BPR_GiveCurrency(playerid, rewardType, rewardName, rewardCount, 0);
            return 1;
        }
        case REWARD_TYPE_EXP, REWARD_TYPE_MONEY, REWARD_TYPE_BC, REWARD_TYPE_VIP, REWARD_TYPE_BP_EXP, REWARD_TYPE_EVENT_RES:
        {
            BPR_GiveCurrency(playerid, rewardType, rewardName, rewardCount, 0);
            return 1;
        }
...
stock Cases_TakeRewards(playerid, const jsonData[])
{
    ...
    for(new i = 0; i < takeCount; i++)
    {
        ...
        {
            Cases_SaveRewardToBPR(playerid, rewardType, rewardValue, rewardCount, rewardRarity);
        }
    }
...
CMD:setcase(playerid, params[])
{
    ...
    if(sscanf(params, "udd", to_player, case_type, amount))
        return SendClientMessage(playerid, COLOR_GREY, "Используйте: /setcase [id игрока] [тип кейса 1-6] [количество]");

    if(!IsPlayerConnected(to_player))
        return SendClientMessage(playerid, COLOR_RED, "Игрок не найден.");

    if(case_type < 1 || case_type > 6)
        return SendClientMessage(playerid, COLOR_RED, "Тип кейса должен быть от 1 до 6.");
...
CMD:giveallcases(playerid, params[])
{
    ...
    for(new case_type = 1; case_type <= 6; case_type++)
    {
...
CMD:givecases(playerid, params[])
{
    ...
    if(type_case < 1 || type_case > 6) return SendClientMessage(playerid, COLOR_RED, "type_case должен быть от 1 до 6.");
