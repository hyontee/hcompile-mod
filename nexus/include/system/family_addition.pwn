...
new Text:fam_TD[5][9];
new PlayerText:fam_PTD[MAX_PLAYERS][5]; // было Text:fam_PTD[MAX_FAM][5] — глобальный textdraw на каждую из 5000 семей (25000 TD, лимит SA-MP 2048); заменено на PlayerTextDraw по игроку (лимит 256 на игрока)
new bool:fam_PTD_created[MAX_PLAYERS];
...
public OnGameModeInit()
{
    ...
    // fam_PTD теперь PlayerTextDraw, создаётся лениво на игрока в ShowFamilyCaptPanel — массовое создание при старте убрано
...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    if(areaid == CAPTURE_AREA[0] || areaid == CAPTURE_AREA[1] || areaid == CAPTURE_AREA[2] || areaid == CAPTURE_AREA[3] || areaid == CAPTURE_AREA[4])
    {
        ...
        {
            new t = GetPlayerCaptArea(playerid);

            // КРИТИЧЕСКАЯ ПРОВЕРКА: без неё возможен вылет клиента (t = -1 -> fam_TD[-1])
            if(t == -1)
            {
                printf("[DEBUG] OnPlayerEnterDynamicArea: GetPlayerCaptArea вернул -1 для игрока %d (areaid:%d)", playerid, areaid);
                #if defined capt_OnPlayerEnterDynamicArea
                    return capt_OnPlayerEnterDynamicArea(playerid, STREAMER_TAG_AREA:areaid);
                #else
                    return 0;
            ...
            ShowFamilyCaptPanel(playerid, t);
        }
    } 
...
public OnPlayerDisconnect(playerid, reason)
{
    ...
    fam_PTD_created[playerid] = false; // SA-MP сам уничтожает PlayerTextDraw при выходе, но флаг нужно сбросить, иначе следующий игрок с этим playerid унаследует "созданное" состояние для несуществующих TD
...
{
    // Защита от некорректного индекса зоны захвата (см. фикс краша при входе в зону)
    if(t < 0 || t >= 5) return 0;

    new f = GetPlayerIdFamily(playerid);
    if(f < 0 || f >= MAX_FAM) return 0; // защита от некорректного f

    show_panel_capt[playerid] = true;

    if(!fam_PTD_created[playerid]) TextDrawExpFamily(playerid); // ленивое создание PlayerTextDraw

    new stringcapture_show[8];
    format(stringcapture_show, sizeof stringcapture_show, "%d", CAPTURE_FAMILY_EXP[f][t]);
    PlayerTextDrawSetString(playerid, fam_PTD[playerid][t], stringcapture_show);
...
{
    // Защита от некорректного индекса зоны захвата
    if(t < 0 || t >= 5) return 0;
    ...
    if(!fam_PTD_created[playerid]) return 1; // ещё не создавался — скрывать нечего

    PlayerTextDrawHide(playerid, fam_PTD[playerid][t]);
    return 1;
}
...
public:CaptureUpd(type)
{
    ...
    {
        if(!show_panel_capt[p] || !fam_PTD_created[p]) continue;

        new pf = GetPlayerIdFamily(p);
        if(pf < 0 || pf >= MAX_FAM) continue;
        if(CAPTURE_FAMILY_EXP[pf][type] == 0) continue;

        format(stringcapture, sizeof stringcapture, "%d", CAPTURE_FAMILY_EXP[pf][type]);
        PlayerTextDrawSetString(p, fam_PTD[p][type], stringcapture);
    }
...
stock TextDrawExpFamily(playerid)
{
    for(new i;i < 5;i++)
    {
        fam_PTD[playerid][i] = CreatePlayerTextDraw(playerid, 99.3332, 252.6367, "0"); // пусто
...
{
    // Р’РёР·СѓР°Р»СЊРЅС‹Р№ РѕР±СЉРµРєС‚ СЏС‰РёРєР° РЅР° РїРµСЂСЃРѕРЅР°Р¶Рµ СѓР±СЂР°РЅ РїРѕ РїСЂРѕСЃСЊР±Рµ вЂ” РјРµС…Р°РЅРёРєР° РґРѕСЃС‚Р°РІРєРё РЅРµ РёР·РјРµРЅРёР»Р°СЃСЊ.
