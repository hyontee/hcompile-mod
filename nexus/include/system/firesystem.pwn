...
new const Float:fire_service_vehicle_exit_pos[FIRE_SERVICE_VEHICLE_COUNT][4] =
{
    {-2554.856201, -296.838470, 27.292865, 270.034210},
    {-2554.856201, -303.338470, 27.292865, 270.034210}
};
...
{
    return ShowNotificationSile(playerid, type, 4, -1, 0, text, "");
}
...
stock FireSystemTryExtinguish(playerid)
{
    ...
    for(new slot; slot < FIRE_CALL_OBJECT_COUNT; slot++)
    {
        ...
        ShowNotificationSile(playerid, 3, 5, -1, 0, "Очаг успешно потушен.", "");
...
stock FireSystemCreateDatabase()
{
    ...
    if(!FireSystemEnsureDBColumn("employed", "TINYINT(1) NOT NULL DEFAULT 0") ||
...
stock FireSystemSavePlayerData(playerid)
{
    ...
    if(!fire_db_ready && !FireSystemCreateDatabase())
        return 0;
...
stock FireSystemGetServiceVehicleSlot(vehicleid)
{
...
{
    return FireSystemGetServiceVehicleSlot(vehicleid) != -1;
}
...
stock FireSystemInvitePartner(playerid, targetid)
{
    ...
    ShowNotificationSile(targetid, 4, 6, playerid, 0, text, ">>");
    return 1;
}
...
stock FireSystem_HandleNotifyClick(playerid, notify_type, notify_id, notify_sub_id)
{
    ...
    {
        FireSystemResetPartnerInvite(playerid);
        ShowNotificationSile(playerid, 2, 6, 0, 0, "Игрок недоступен", " ");
        return 1;
    }
    ...
    {
        FireSystemResetPartnerInvite(playerid);
        ShowNotificationSile(playerid, 2, 6, 0, 0, "Предложение устарело", " ");
        return 1;
    }
    ...
    {
        FireSystemResetPartnerInvite(playerid);
        ShowNotificationSile(playerid, 2, 6, 0, 0, "Игрок находится слишком далеко", " ");
        return 1;
    }
    ...
    {
        FireSystemResetPartnerInvite(playerid);
        ShowNotificationSile(playerid, 2, 6, 0, 0, "Сначала начните рабочий день МЧС", " ");
        return 1;
    }
    ...
    {
        FireSystemResetPartnerInvite(playerid);
        ShowNotificationSile(playerid, 2, 6, 0, 0, "У Вас уже есть активный вызов или напарник", " ");
        return 1;
    }
    ...
    if(callid != -1 && fire_call_owner[callid] == inviter)
    {
        ...
        {
            FireSystemBreakPartnerLink(playerid);
            ShowNotificationSile(playerid, 2, 6, 0, 0, "У игрока уже есть напарник", " ");
            return 1;
        }
        ...
        {
            FireSystemBreakPartnerLink(playerid);
            ShowNotificationSile(playerid, 2, 6, 0, 0, "Ваш ранг слишком мал для этого вызова", " ");
            return 1;
        }
...
stock FireSystemGetAssignedVehicleOwner(vehicleid)
{
...
stock FireSystemReturnAssignedVehicle(playerid)
{
...
stock FireSystemMoveServiceVehicleOutside(playerid, vehicleid)
{
    new slot = FireSystemGetServiceVehicleSlot(vehicleid);
    if(slot == -1)
        return 0;

    new ownerid = FireSystemGetAssignedVehicleOwner(vehicleid);
    if(ownerid != INVALID_PLAYER_ID && ownerid != playerid)
    {
        ShowNotificationSile(playerid, 2, 5, -1, 0, "Этот транспорт уже использует другой сотрудник МЧС.", "");
        FireSystemEjectFromVehicle(playerid, vehicleid);
        fire_player_driver_vehicle[playerid] = INVALID_VEHICLE_ID;
        return 0;
    }

    if(fire_player_gazel_vehicle[playerid] != INVALID_VEHICLE_ID && fire_player_gazel_vehicle[playerid] != vehicleid)
    {
        ShowNotificationSile(playerid, 2, 5, -1, 0, "Сначала верните ранее взятый транспорт МЧС.", "");
        FireSystemEjectFromVehicle(playerid, vehicleid);
        fire_player_driver_vehicle[playerid] = INVALID_VEHICLE_ID;
        return 0;
    }
    ...
    (
        vehicleid,
        fire_service_vehicle_exit_pos[slot][0],
        fire_service_vehicle_exit_pos[slot][1],
        fire_service_vehicle_exit_pos[slot][2]
    );
    SetVehicleZAngle(vehicleid, fire_service_vehicle_exit_pos[slot][3]);

    foreach(new occupantid : Player)
    {
        if(GetPlayerVehicleID(occupantid) == vehicleid)
        {
            SetPlayerInterior(occupantid, 0);
            SetPlayerVirtualWorld(occupantid, 0);
        }
    }
    ...
    ShowNotificationSile(playerid, 3, 5, -1, 0, "Служебный транспорт МЧС выведен на улицу.", "");
    return 1;
}
...
stock FireSystemEndWorkDay(playerid)
{
    ...
    fire_player_on_duty[playerid] = false;
    FireSystemReturnAssignedVehicle(playerid);
    fire_player_return_textdraw_created[playerid] = false;
...
{
    return ShowNotificationSile(playerid, type, 4, -1, button, text, "");
}

stock FireSystemSendNotEmployedNotify(playerid)
{
    return ShowNotificationSile(playerid, 2, 5, 0, 0, "Вы не работаете в МЧС.", "");
}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == FIRE_DIALOG_GAZEL_RENT)
    {
        ...
        FireSystemMoveServiceVehicleOutside(playerid, vehicleid);
        return 1;
    }
...
public FireSystemVehicleReturnTimer(playerid)
{
    ...
    if(fire_player_return_time[playerid] <= 0)
    {
        ...
        SetVehicleVirtualWorld(vehicleid, 0);
        SetVehicleToRespawn(vehicleid);
        ShowNotificationSile(playerid, 2, 5, -1, 0, "Вы не вернулись в автомобиль МЧС.", "");
        return 1;
    }
...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
    ...
    if(newstate == PLAYER_STATE_DRIVER && oldstate != PLAYER_STATE_DRIVER)
    {
        ...
        if(FireSystemIsGazelleVehicle(vehicleid))
        {
            if(!fire_player_on_duty[playerid])
            {
                ShowNotificationSile(playerid, 2, 5, -1, 0, "Начните рабочий день.", "");

                if(!fire_player_employed[playerid])
                {
                    ShowNotificationSile(playerid, 2, 5, -1, 0, "Транспорт доступен только для сотрудникам МЧС.", "");
                }
            ...
            if(fire_player_gazel_vehicle[playerid] == vehicleid)
            {
                ...
                if(vehicle_z > 1000.0)
                {
                    return FireSystemMoveServiceVehicleOutside(playerid, vehicleid);
                }
...
{
    if(!FireSystemCreateDatabase())
        print("[FIRESYSTEM] database initialization failed; player data will retry on login");
