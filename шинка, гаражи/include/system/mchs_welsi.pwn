...
#define MCHS_SCM SendClientMessage
#define MCHS_SC              "{ffff00}| {ffffff}"
#define MCHS_USC             "{ff2400}| {ffffff}"
#define		MCHS_DSI		DIALOG_STYLE_INPUT
#define 	MCHS_DSM		DIALOG_STYLE_MSGBOX
#define 	MCHS_DSL		DIALOG_STYLE_LIST
#define 	MCHS_DSP		DIALOG_STYLE_PASSWORD
#define 	MCHS_DSTH    DIALOG_STYLE_TABLIST_HEADERS
...
stock MCHS_Init()
{
    ...
    mysql_query(mysql, "CREATE TABLE IF NOT EXISTS `order_mchs` (`id` INT NOT NULL PRIMARY KEY, `id_player` INT NOT NULL DEFAULT 0, `state` INT NOT NULL DEFAULT 1) ENGINE=InnoDB DEFAULT CHARSET=cp1251", false);
    if(mysql_errno()) print("[MCHS] Не удалось создать таблицу order_mchs");

    SetTimer("LoadOrdersMchs", 5000, false);
    SetTimer("TimerMchs", 60000*5, true);
}

stock MCHS_OnPlayerDisconnect(playerid, reason)
{
...
stock MCHS_OnPlayerConnect(playerid)
{
    player_order[playerid] = 0;
    player_veh_mchs[playerid] = -1;
}

stock MCHS_OnPlayerEnterDynamicArea(playerid, STREAMER_TAG_AREA:areaid)
{
    if(areaid == exit_mchs)
    {
    ...
    {
        if(!player_mchs_active{playerid}) return MCHS_SCM(playerid, -1,""MCHS_USC" Сначало начните смену");
        Dialog(playerid, 35802, MCHS_DSM, "Инвентарная МЧС", "Вы хотите взять огнетушитель", "Взять", "Назад");
        return 1;
    }
    if(areaid == veh_mchs)
    {
        if(!player_mchs_active{playerid}) return MCHS_SCM(playerid, -1,""MCHS_USC" Сначало начните смену");
        Dialog(playerid, 35803, MCHS_DSM, "Рабочий транспорт МЧС", "Вы хотите взять рабочий транспорт", "Взять", "Назад");
        return 1;
    }
}

stock MCHS_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid < 35800 || dialogid > 35803) return 0;
    if(dialogid == 35800)
    {
        if(response)
        {
            ...
            if(mysql_errno()) return MCHS_SCM(playerid, -1, "Ошибка в запросе 1");
            ...
            switch(listitem)
            {
                case 0:
                {
                    ...
                    else
                    {
                        ...
                        MCHS_SCM(playerid, -1, ""MCHS_SC" Вы начали смену в МЧС");
                    }
                }
                case 1:
                {
    ...
    if(dialogid == 35802)
    {
        if(response)
        {
            ...
            MCHS_SCM(playerid, -1, ""MCHS_SC" Вы взяли огнетушитель");
        }
    }
    if(dialogid == 35803)
    {
        if(response)
        {
            ...
            MCHS_SCM(playerid, -1, ""MCHS_SC" Вы взяли рабочий транспорт МЧС");
            MCHS_SCM(playerid, -1, ""MCHS_SC" Используйте /meslist чтобы взять заказ");
        }
    }
    return 1;
}

stock MCHS_OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(PRESSED(KEY_FIRE))
	{
        if(press_fire{playerid}) return MCHS_SCM(playerid, -1, ""MCHS_USC" Подождите немного...");
			...
			if(IsPlayerInRangeOfAnyFire(playerid) >= 1)
			{
                ...
                if(all_fire[IsPlayerInRangeOfAnyFire(playerid)][F_ID_Z] == player_order[playerid])
                {
                    ...
                    { 
                        ...
                        MCHS_SCM(playerid, -1, ""MCHS_SC" Вы потушили огонь");

                        if(Order(id))
                        {
                            MCHS_SCM(playerid, -1, ""MCHS_SC" Вы выполнили заказ");
                            ...
                            if(mysql_errno()) return MCHS_SCM(playerid, -1, "Ошибка в запросе 2");
                        }
                        return 1;
                    }
                ...
                else MCHS_SCM(playerid, -1, ""MCHS_USC" Эти очаги не с вашего заказа");
			}
			else MCHS_SCM(playerid, -1, ""MCHS_USC" Вы не у очага огня");
		}
	}
    return 0;
}
...
{
    if(!player_mchs_active{playerid}) return MCHS_SCM(playerid, -1, ""MCHS_USC" Сначало устройтесь в МЧС");
    new Cache:result;
    ...
    if(!rows) return MCHS_SCM(playerid, 0xFFFFFFFF3, "На данный момент заказов нет. Ждите пейдей");

    if(player_order[playerid]) return MCHS_SCM(playerid, -1, ""MCHS_USC" У вас уже есть заказ");
    ...
    if(rows)
    {
        ...
        DialogMhsc(playerid, 35800, MCHS_DSTH, "Доступные заказы", list, "Взять", "Назад");
