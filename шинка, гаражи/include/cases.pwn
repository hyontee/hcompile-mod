...
new pCasesPendingCaseIdx[MAX_PLAYERS][10]; // Exact case index for each pending reward
new pCasesPendingAwardIdx[MAX_PLAYERS][10]; // Exact award index for each pending reward
...
stock Cases_GivePlayerCaseById(playerid, caseId, amount)
{
    ...
    pCasesCounts[playerid][idx] += amount;
    Cases_SavePlayer(playerid);
    if(pCasesGUIOpen[playerid]) Cases_UpdateGUI(playerid);
    return 1;
}
...
{
    pCasesBC[playerid] = GetPlayerDonateRub(playerid);
    ...
    for(new i = 0; i < 10; i++) {
        ...
        pCasesPendingCaseIdx[playerid][i] = -1;
        pCasesPendingAwardIdx[playerid][i] = -1;
    }
...
{
    // Only selection state is required here. Rebuilding all case/bonus
    // arrays was causing an AMX stack/heap collision.
    new Node:json = JSON_Object(
        "t", JSON_Int(CASES_TYPE_SELECT),
        "s", JSON_Int(1),
        "cs", JSON_Int(pCasesSelected[playerid]),
        "bc", JSON_Int(GetPlayerDonateRub(playerid)),
        "pc", JSON_Int(pCasesDust[playerid]),
...
{
    // Parse first. Case selection must not be blocked by the opening cooldown.
    new Node:json;
    ...
    // Anti-spam applies only to opening a case.
    if(actionType == CASES_TYPE_OPEN) {
    ...
    switch(actionType) {
        ...
        case CASES_TYPE_GO_DONATE: {
            ...
            if(donateType == 2) {
                SendClientMessage(playerid, 0xFFFF00FF, "\xcf\xe5\xf0\xe5\xe9\xe4\xe8\xf2\xe5 \xe2 \xf0\xe0\xe7\xe4\xe5\xeb \xe4\xee\xed\xe0\xf2\xe0 \xe4\xeb\xff \xef\xee\xea\xf3\xef\xea\xe8 BC");
            }
        }
...
stock Cases_OpenCase(playerid, caseId, openType)
{
    ...
    // OPEN contains the case id explicitly. Keep server-side selection in sync.
    pCasesSelected[playerid] = caseId;
    ...
    // Deduct BC through the main gamemode balance.
    GivePlayerDonateRub(playerid, -price, "Cases: open", true, true);
    pCasesBC[playerid] = GetPlayerDonateRub(playerid);
    
    // Generate rewards
    new openCount = (openType == CASES_OPEN_ONE) ? 1 : 10;
    // Generate directly into the player pending reward array.
    // Avoid a local rewardIds[10] array to reduce AMX stack usage.
    for(new r = 0; r < openCount; r++) {
        pCasesPendingRewards[playerid][r] = Cases_GetRandomReward(idx);
        pCasesPendingCaseIdx[playerid][r] = idx;
        pCasesPendingAwardIdx[playerid][r] = -1;

        // Store the exact award index selected by the server.
        // Reward IDs are local to a case, so resolving them later by ID
        // can produce a wrong reward when the client sends a stale/mismatched ID.
        for(new a = 0; a < CaseData[idx][cAwardsCount]; a++) {
            if(CaseAwards[idx][a][aId] == pCasesPendingRewards[playerid][r]) {
                pCasesPendingAwardIdx[playerid][r] = a;
                break;
            }
        }
    }
    ...
    // IMPORTANT: the mobile client expects the reward response in the legacy
    // string format. Do not build `pr` through JSON_SetArray here: on some
    // client builds that produces a different JSON node representation and
    // the launcher crashes while parsing the reward screen.
    new prStr[128];
    prStr[0] = '\0';
    for(new r = 0; r < openCount; r++) {
        new tmp[16];
        if(r > 0) strcat(prStr, ",");
        format(tmp, sizeof(tmp), "%d", pCasesPendingRewards[playerid][r]);
        strcat(prStr, tmp);
    }

    for(new r = 0; r < openCount; r++) {
        new debugAward = pCasesPendingAwardIdx[playerid][r];
        if(debugAward >= 0) {
            printf("[Cases] Reward %d: case=%d awardId=%d type=%d internal=%d count=%d",
                r + 1, caseId,
                CaseAwards[idx][debugAward][aId],
                CaseAwards[idx][debugAward][aType],
                CaseAwards[idx][debugAward][aInternalId],
                CaseAwards[idx][debugAward][aCount]);
        }
    }
    ...
    format(jsonStr, sizeof(jsonStr),
        ...
        pCasesOpened[playerid], pCasesSelected[playerid], openType, prStr);

    SendPacketToClientString(playerid, GUICases, jsonStr);
    printf("[Cases] Player %d opened case %d (%d reward(s))", playerid, caseId, openCount);
    return 1;
}
...
stock Cases_FindPendingReward(playerid, rewardId, &pendingIndex)
{
    for(new i = 0; i < pCasesPendingCount[playerid]; i++) {
        if(pCasesPendingRewards[playerid][i] == rewardId &&
            pCasesPendingAwardIdx[playerid][i] >= 0 &&
            pCasesPendingCaseIdx[playerid][i] >= 0) {
            pendingIndex = i;
            return 1;
        }
    }
...
stock Cases_ClearPendingReward(playerid, pendingIndex)
{
    if(pendingIndex < 0 || pendingIndex >= 10) return 0;

    pCasesPendingRewards[playerid][pendingIndex] = 0;
    pCasesPendingCaseIdx[playerid][pendingIndex] = -1;
    pCasesPendingAwardIdx[playerid][pendingIndex] = -1;
    return 1;
}
    ...
    if(JSON_GetArray(json, "bt1", bt1Array) == 0) {
        ...
        for(new i = 0; i < length; i++) {
            ...
            if(JSON_ArrayObject(bt1Array, i, itemNode) == 0) {
                new rewardId = 0;
                JSON_GetNodeInt(itemNode, rewardId);
                if(rewardId > 0) {
                    new pendingIndex = -1;
                    if(Cases_FindPendingReward(playerid, rewardId, pendingIndex)) {
                        new caseIdx = pCasesPendingCaseIdx[playerid][pendingIndex];
                        new awardIdx = pCasesPendingAwardIdx[playerid][pendingIndex];

                        // Give exactly the award selected by the server.
                        // Do not search all cases by rewardId: reward IDs are
                        // local to each case and may repeat (1..37).
                        if(Cases_GiveRewardInternal(playerid, caseIdx, awardIdx)) {
                            Cases_ClearPendingReward(playerid, pendingIndex);
                        }
                    } else {
                        printf("[Cases] WARNING: pending reward %d not found for player %d", rewardId, playerid);
                    }
                }
            }
        }
            ...
            if(JSON_ArrayObject(bt2Array, i, itemNode) == 0) {
                ...
                if(rewardId > 0) {
                    ...
                    if(Cases_FindPendingReward(playerid, rewardId, pendingIndex)) {
                        new caseIdx = pCasesPendingCaseIdx[playerid][pendingIndex];
                        new awardIdx = pCasesPendingAwardIdx[playerid][pendingIndex];
                        new dustGained = CaseAwards[caseIdx][awardIdx][aPriceSprayed];

                        if(dustGained > 0) {
                            Cases_GiveDust(playerid, dustGained);
                            totalDustGained += dustGained;
                        }

                        Cases_ClearPendingReward(playerid, pendingIndex);
                    } else {
                        printf("[Cases] WARNING: pending spray reward %d not found for player %d", rewardId, playerid);
                    }
                }
            }
        }
...
stock Cases_GiveRewardInternal(playerid, caseIdx, awardIdx)
{
    ...
    new rarity = CaseAwards[caseIdx][awardIdx][aRarity];

    if(funcidx("Cases_OnRewardTaken") != -1)
        return CallLocalFunction("Cases_OnRewardTaken", "iiiiii", playerid, rewardType, internalId, count, rarity, 0);
...
stock Cases_GetBonus(playerid, bonusId)
{
    ...
    // Give the bonus reward through the host gamemode reward pipeline.
    ...
    new rarity = CaseBonus[caseIdx][i][bRarity];

    if(funcidx("Cases_OnRewardTaken") != -1)
    {
        if(!CallLocalFunction("Cases_OnRewardTaken", "iiiiii", playerid, bonusType, internalId, count, rarity, 1))
            return 0;
    }
    else return 0;
...
{
    if(GetPlayerAdminEx(playerid) < 13) return SendClientMessage(playerid, 0xFF0000FF, "Недостаточно прав!");
    ...
    GivePlayerDonateRub(targetid, amount, "Cases: givebc", true, true);
    pCasesBC[targetid] = GetPlayerDonateRub(targetid);
...
stock Cases_DBInit()
{
    mysql_tquery(mysql,
        ...
        `bc_balance` INT NOT NULL DEFAULT 0, \
    ...
    mysql_tquery(mysql,
        "SELECT COUNT(*) AS cnt FROM INFORMATION_SCHEMA.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'player_cases' AND COLUMN_NAME = 'bc_balance'",
        "Cases_DBCheckBC", "");
    return 1;
}

public Cases_DBCheckBC()
{
    if(cache_num_rows() && cache_get_field_content_int(0, "cnt") == 0)
        mysql_tquery(mysql, "ALTER TABLE `player_cases` ADD COLUMN `bc_balance` INT NOT NULL DEFAULT 0 AFTER `user_id`");
    return 1;
}
...
stock Cases_SavePlayer(playerid)
{
    ...
    mysql_format(mysql, query, sizeof(query),
        ...
        GetPlayerDonateRub(playerid), pCasesDust[playerid], pCasesOpened[playerid],
        pCasesSelected[playerid], pCasesTutorial[playerid], ccStr, cbStr);
...
public Cases_OnPlayerLoad(playerid)
{
    if(cache_num_rows())
    {
        ...
        if(Cases_GetIndex(pCasesSelected[playerid]) == -1) pCasesSelected[playerid] = 1;
        ...
        if(strlen(ccStr) > 0)
        {
            ...
            new legacyLayout = 1;
            new commaCount = 0;
            for(new ci = 0; ci < len; ci++) if(ccStr[ci] == ',') commaCount++;
            if(commaCount >= MAX_CASES - 1) legacyLayout = 0;
            while(pos < len && idx < MAX_CASES)
            {
                ...
                {
                    new legacyIndex = idx;
                    new targetIndex = (legacyIndex == 5) ? 7 : legacyIndex;
                    if(targetIndex < MAX_CASES) pCasesCounts[playerid][targetIndex] = strval(tmp);
                }
                else pCasesCounts[playerid][idx] = strval(tmp);
                idx++;
                pos = end + 1;
            }
        }
        ...
        if(strlen(cbStr) > 0)
        {
            ...
            new legacyBonusLayout = 1;
            new commaCount = 0;
            for(new bi = 0; bi < len; bi++) if(cbStr[bi] == ',') commaCount++;
            if(commaCount >= MAX_CASES * MAX_BONUS_PER_CASE - 1) legacyBonusLayout = 0;
            while(pos < len && idx < MAX_CASES * MAX_BONUS_PER_CASE)
            {
                new end = pos;
                while(end < len && cbStr[end] != ',') end++;
                strmid(tmp, cbStr, pos, end, sizeof(tmp));
                if(legacyBonusLayout)
                {
                    new legacyCase = idx / MAX_BONUS_PER_CASE;
                    new bonusIndex = idx % MAX_BONUS_PER_CASE;
                    new targetCase = (legacyCase == 5) ? 7 : legacyCase;
                    if(targetCase < MAX_CASES) pCasesBonusStatus[playerid][targetCase][bonusIndex] = strval(tmp);
                }
                else pCasesBonusStatus[playerid][idx / MAX_BONUS_PER_CASE][idx % MAX_BONUS_PER_CASE] = strval(tmp);
                idx++;
                pos = end + 1;
            }
        }
