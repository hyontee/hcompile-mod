...
CMD:givebc(playerid, params[])
{
	...
	extract params -> new to_player, amount; else return SendClientMessage(playerid, 0xCECECEFF, "Используйте: /givebc [id игрока] [количество]");

	if(!IsPlayerConnected(to_player) || !IsPlayerLogged(to_player))
		return SendClientMessage(playerid, 0xCECECEFF, "Такого игрока нет");

	if(!(1 <= amount <= 10000)) return SendClientMessage(playerid, 0xCECECEFF, "Можно выдавать от 1 до 10000 BC за один раз");

	pCasesBC[to_player] += amount;

	new reason[144];
	format(reason, sizeof reason, "Администратор %s выдал Вам %d BC", GetPlayerNameEx(playerid), amount);
	SendClientMessage(to_player, 0x00FF00FF, reason);

	format(reason, sizeof reason, "Вы выдали %d BC игроку %s", amount, GetPlayerNameEx(to_player));
	SendClientMessage(playerid, 0x00FF00FF, reason);

	format(reason, sizeof reason, "[A] %s[%d] выдал BC %s[%d] в кол-во %d, в итоге у игрока %d BC",
	GetPlayerNameEx(playerid), playerid, GetPlayerNameEx(to_player), to_player, amount, pCasesBC[to_player]);
	SendMessageToAdmins(reason, 0x999999FF);

	format(reason, sizeof reason, "Выдал %s[acc:%d] %d BC", GetPlayerNameEx(to_player), GetPlayerAccountID(to_player), amount);
...
stock Cases_SavePlayer(playerid)
{
    ...
    mysql_format(mysql, query, sizeof(query),
        ...
        GetPlayerAccountID(playerid), pCasesBC[playerid], pCasesDust[playerid], pCasesOpened[playerid],
