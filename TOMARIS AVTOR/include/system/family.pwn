...
public: UnLoadPlayerFamily(playerid)
{
    ...
    new family_id = GetPlayerFamily(playerid, ID_SQL_FAMILY);
    new family_rang = GetPlayerFamily(playerid, RANG_FAMILY);
    new family_mute = GetPlayerFamily(playerid, MUTE_FAMILY);

    UpdatePlayerDatabaseInt(playerid, "family_id", family_id);
    UpdatePlayerDatabaseInt(playerid, "family_rang", family_rang);
    UpdatePlayerDatabaseInt(playerid, "family_mute", family_mute);
        ...
        if(response)
        {
            ...
            if(GetPlayerMoneyEx(playerid) >= 3_000_000)
            {
                ...
                if(!mysql_errno())
                {
                    ...
                    ShowNotificationLaird(playerid, 2, 5, 0, 0, "Вы потратили 3000000 рублей", " ");
...
stock LoadFamilyCar(database)
{
    ...
    if(vehicleid != INVALID_VEHICLE_ID)
    {
        ...
        // ? ЗАГРУЖАЕМ ТЮНИНГ ИЗ КЭША
        LoadVehicleTuningFromCache(vehicleid, 0, result);
        
        // ? ПРИМЕНЯЕМ ДЛЯ ВСЕХ ИГРОКОВ
