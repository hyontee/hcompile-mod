...
{
    GARAGE_ID,                 // айди гаража
    GARAGE_OWNER_ID,           // айди владельца в бд
    GARAGE_PRICE,              // цена гаража
    GARAGE_STATUS,             // lock статус гаража, 0 - открыт 1 - закрыт
    Float: GARAGE_X,	       // позиция пикапа входа
	Float: GARAGE_Y,		   // позиция пикапа входа
	Float: GARAGE_Z,	       // позиция пикапа входа
	Float: GARAGE_EXIT_X,      // позиция после выхода из гаража
	Float: GARAGE_EXIT_Y,      // позиция после выхода из гаража
	Float: GARAGE_EXIT_Z,      // позиция после выхода из гаража
	Float: GARAGE_EXIT_ANGLE,  // угол поворота
    G_ENTER_PICKUP,            // пикап входа
    GARAGE_ZONE,               // зона входа
...
g_garage[idx][GARAGE_ZONE] = CreateDynamicSphere(
    g_garage[idx][GARAGE_X],
    g_garage[idx][GARAGE_Y],
    g_garage[idx][GARAGE_Z],
    2.0
);
printf("[DEBUG] Создана зона гаража %d на координатах: %f, %f, %f", idx, GetGarageData(idx, GARAGE_X), GetGarageData(idx, GARAGE_Y), GetGarageData(idx, GARAGE_Z));
			...
			new garageid = GetPlayerGarage(playerid);
					if(response)
					{
							switch(listitem)
							{
								...
								case 2:
{
    printf("[DEBUG] Нажат пункт Улучшения");
    printf("[DEBUG] garageid = %d", garageid);
    printf("[DEBUG] GARAGE_IMPROVEMENTS = %d", GetGarageData(garageid, GARAGE_IMPROVEMENTS));
    
    if(GetGarageData(garageid, GARAGE_IMPROVEMENTS) == 1)
    {
        printf("[DEBUG] Открываем DIALOG_GARAGE_IMPROVEMENT");
        Dialog(playerid, DIALOG_GARAGE_IMPROVEMENT, DIALOG_STYLE_LIST, 
            "{ff0000}Улучшения",
            "{FFFFFF}Название\t\t\t\t{FFFFFF}Цена\t\t\t\t{FFFFFF}Статус\n"\
            "1. Возможность проживания в гараже\t30000 руб.\t\t{66CC00}имеется\n"\
            "2. Сейф в гараже\t\t\t\t40000 руб.\t\t{66CC00}имеется\n"\
            "3. Улучшить до элитного\t\t\t5000000 руб.\t\t{ff0000}не имеется",
            "Далее", "Выйти"
        );
    }
    else if(GetGarageData(garageid, GARAGE_IMPROVEMENTS) == 2)
    {
        printf("[DEBUG] Уже максимальное улучшение");
        SendClientMessage(playerid, -1, "У вас уже максимальное улучшение");
    }
    else
    {
        printf("[DEBUG] GARAGE_IMPROVEMENTS = %d - не 1 и не 2", GetGarageData(garageid, GARAGE_IMPROVEMENTS));
        // Принудительно устанавливаем значение 1
        g_garage[garageid][GARAGE_IMPROVEMENTS] = 1;
        Dialog(playerid, DIALOG_GARAGE_IMPROVEMENT, DIALOG_STYLE_LIST, 
            "{ff0000}Улучшения",
            "{FFFFFF}Название\t\t\t\t{FFFFFF}Цена\t\t\t\t{FFFFFF}Статус\n"\
            "1. Возможность проживания в гараже\t30000 руб.\t\t{66CC00}имеется\n"\
...
{
    printf("[DEBUG] EnterPlayerToGarage: garageid=%d, improvements=%d", garageid, GetGarageData(garageid, GARAGE_IMPROVEMENTS));
    ...
    {
        // Если улучшений нет (значение 0 или другое) - используем стандартный вход
        SetPlayerPosEx(playerid, 1.265544, 1996.246215, 1554.203125, 359.354705, 1, garageid + 2000);
        SetPlayerInGarage(playerid, garageid);
        SendClientMessage(playerid, -1, "Вы вошли в гараж (стандартный)");
    }
}
...
CMD:gsetexitpos(playerid, params[])
{
    ...
    format(fmt_text, sizeof fmt_text,
        ...
        POS[0], POS[1], POS[2], POS[3], GetGarageData(garage_id, GARAGE_ID)
    );
    mysql_query(mysql, fmt_text, false);

    // ========== ОБНОВЛЯЕМ МАССИВ ==========
    g_garage[garage_id][GARAGE_EXIT_X] = POS[0];
    g_garage[garage_id][GARAGE_EXIT_Y] = POS[1];
    g_garage[garage_id][GARAGE_EXIT_Z] = POS[2];
    g_garage[garage_id][GARAGE_EXIT_ANGLE] = POS[3];
    // =====================================

    SendClientMessage(playerid, 0x66CC33FF, "Вы успешно изменили координаты выхода у гаража");
    return 1;
}
...
CMD:g(playerid)
{
    ...
    new carid = GetPlayerOwnableCar(playerid);
    if(carid == INVALID_VEHICLE_ID)
        return SendClientMessage(playerid, -1, "У вас нет личного транспорта");
    
    if(!IsPlayerInVehicle(playerid, carid))
        return SendClientMessage(playerid, -1, "Вы должны быть за рулем");
    
    if(GetPlayerInGarage(playerid) != -1)
    {
        // Выезд
        SetVehiclePos(carid,
            g_garage[garageid][GARAGE_EXIT_X],
            g_garage[garageid][GARAGE_EXIT_Y],
            g_garage[garageid][GARAGE_EXIT_Z]
        );
        SetVehicleZAngle(carid, g_garage[garageid][GARAGE_EXIT_ANGLE]);
    ...
    {
        new Float:gx = g_garage[garageid][GARAGE_X];
        new Float:gy = g_garage[garageid][GARAGE_Y];
        new Float:gz = g_garage[garageid][GARAGE_Z];
        
        if(!IsPlayerInRangeOfPoint(playerid, 10.0, gx, gy, gz))
    return SendClientMessage(playerid, -1, "Вы не рядом со своим гаражом");
        ...
        SetVehicleVirtualWorld(carid, garageid + 2000);
        LinkVehicleToInterior(carid, 1);
        PutPlayerInVehicle(playerid, carid, 0);
        SetPlayerVirtualWorld(playerid, garageid + 2000);
        SetPlayerInterior(playerid, 1);
        SetPlayerInGarage(playerid, garageid);
        SendClientMessage(playerid, -1, "Вы въехали в гараж");
    }
    return 1;
}
