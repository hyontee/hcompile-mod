// ============================================================================
// dtp_vehiclebreak.pwn — PRO MAX FULL (v17) — ИСПРАВЛЕННАЯ ВЕРСИЯ
// ----------------------------------------------------------------------------
// ИСПРАВЛЕНИЕ: Добавлена защита от вылетов при посадке в машину.
// Теперь система ДТП не работает первые 2-3 секунды после того, как игрок сел.
// ============================================================================
...
// Доп. усиление, если одновременно меняется damageStatus
#define DTP_USE_DAMAGESTATUS_BOOST  (1)
#define DTP_DAMAGESTATUS_BOOST_PCT  (1.15)
...
new Float:dtp_hp[MAX_VEHICLES];
new Float:dtp_maxhp[MAX_VEHICLES];
new Float:dtp_minhp[MAX_VEHICLES];
new dtp_broken[MAX_VEHICLES];
new dtp_broken_announced[MAX_VEHICLES];

new dtp_vehicle_last_tick[MAX_VEHICLES];

new Float:dtp_last_kmh_p[MAX_PLAYERS];
new dtp_player_last_tick[MAX_PLAYERS];
...
new dtp_dmgstat_recent[MAX_VEHICLES];
...
{
    // ========== ЗАЩИТА: не ограничиваем скорость если игрок только что сел ==========
    for(new i = 0; i < MAX_PLAYERS; i++)
    {
        if(IsPlayerConnected(i) && GetPlayerVehicleID(i) == vehicleid)
        {
            // Если переменная playerEnterCount существует и больше 0 — пропускаем
            #if defined playerEnterCount
                if(playerEnterCount{i} > 0 && playerEnterCount{i} < 100)
                {
                    return 1;
                }
            #endif
...
// ============================================================================
// ОСНОВНАЯ РАБОТА — ИСПРАВЛЕННАЯ ВЕРСИЯ
// ============================================================================
...
public OnPlayerUpdate(playerid)
{
    ...
    {
        // ========== ЗАЩИТА: если игрок только что сел в машину — пропускаем обработку ДТП ==========
        #if defined playerEnterCount
            if(playerEnterCount{playerid} > 0 && playerEnterCount{playerid} < 100)
            {
                #if defined DTP_OnPlayerUpdate
                    return DTP_OnPlayerUpdate(playerid);
                #else
