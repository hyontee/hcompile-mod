...
CMD:music(playerid) // уберите /music старый пж
{
    ...
    {
        SendClientMessage(playerid, -1, ""SC"Включение {FF5252}музыки {FFFFFF}доступно только в {FF5252}транспорте");
    }
    else
    {
        new title[128];
        format(title, sizeof(title), "{FF5252}%s{ffffff} | Выберите нужное действие", NameServer[MISTERS_NAME]);
    ...
    if(response)
    {
        ...
        switch(listitem)
        {
          case 0:
          {
             ...
             SendClientMessage(playerid, -1, ""SC"Радио {FF5252}«DFM» {FFFFFF}успешно включено");
          }
          case 1:
          {
             PlayAudioStreamURL(playerid, "http://hitfm.hostingradio.ru/hitfm128.mp3");
             SendClientMessage(playerid, -1, ""SC"Радио {FF5252}«Hit-FM» {FFFFFF}успешно включено");
           } 
           case 2:
           {
              PlayAudioStreamURL(playerid, "http://rusradio.hostingradio.ru/rusradio128.mp3");
             SendClientMessage(playerid, -1, ""SC"Радио {FF5252}«Русское радио» {FFFFFF}успешно включено");
            }
          } 
      }
} 
    ...
    if(response)
    {
        switch(listitem)
        {
          ...
          case 2:
          {
             ...
             {
               ShowNewNotification(playerid, 2, 6, 0, 0, "Музыка не включена", "");
             } 
             else
             {
              StopAudioStream(playerid);
              ShowNewNotification(playerid, 2, 6, 0, 0, "Музыка выключена", "");
       ...
       Action(playerid, "Включил(-а) музыку в транспортном средстве", _, false);
        switch(listitem)
        {
            case 0:
            {
                ...
                SendClientMessage(playerid, -1, ""SC"Песня {FF5252}'Катюха' {FFFFFF}включена.");
            }
            case 1:
            {
                ...
                SendClientMessage(playerid, -1, ""SC"Песня {FF5252}'Brainrot Rap' {FFFFFF}включена.");
            }
            case 2:
            {
                PlayAudioStreamURL(playerid, "https://rus.hitаmotop.com/get/music/20170830/Miyagi_Rem_Digga_-_I_Got_Love_47828425.mp3");
                SendClientMessage(playerid, -1, ""SC"Песня {FF5252}'Miyagi & Эндшпиль - I Got Love' {FFFFFF}включена.");
            }
            case 3:
            {
                PlayAudioStreamURL(playerid, "https://rus.hitmotop.com/get/music/20250815/bridge_-_LABUBA_79438066.mp3");
                SendClientMessage(playerid, -1, ""SC"Песня {FF5252}'ЛАБУБА' {FFFFFF}включена.");
            }
            case 4:
            {
            ...
            case 15:
            {
                ...
                SendClientMessage(playerid, -1, ""SC"Песня {FF5252}'Пацанский фонк - ЕГОР КРИД' {FFFFFF}включена.");
            }
            case 16:
            {
...
public: InCarTimer(playerid)
{
    if(IsPlayerInAnyVehicle(playerid))
    {
       SendClientMessage(playerid, -1, ""SC"Для включения музыки в транспортном средстве используйте {FF5252}\"/music\"{FFFFFF}.");
    }
    else
    {
...
public: InMusicTimer(playerid)
{
    if(IsPlayerInAnyVehicle(playerid))
    {
       //SendClientMessage(playerid, -1, ""SC"Для включения {FF5252}музыки {FFFFFF}в транспортном средстве используйте {FF5252}/music");
    }
    else
    {
...
{
    new title[128];
    format(title, sizeof(title), "{FF5252}%s{ffffff} | Выберите нужную радиостанцию", NameServer[MISTERS_NAME]);
...
{
    new title[128];
    format(title, sizeof(title), "{FF5252}%s{ffffff} | Аудио-система автомобиля", NameServer[MISTERS_NAME]);
