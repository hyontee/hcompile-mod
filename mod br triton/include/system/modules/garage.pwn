...
{
    GARAGE_ID,                 //   (by wrton) 
    GARAGE_OWNER_ID,           //     (by wrton) 
    GARAGE_PRICE,              //   (by wrton) 
    GARAGE_STATUS,             // lock  , 0 -  1 -  (by wrton) 
    Float: GARAGE_X,	       //    (by wrton) 
	Float: GARAGE_Y,		   //    (by wrton) 
	Float: GARAGE_Z,	       //    (by wrton) 
	Float: GARAGE_EXIT_X,      //      (by wrton) 
	Float: GARAGE_EXIT_Y,      //      (by wrton) 
	Float: GARAGE_EXIT_Z,      //      (by wrton) 
	Float: GARAGE_EXIT_ANGLE,  //   (by wrton) 
    G_ENTER_PICKUP,            //   (by wrton) 
    Text3D: G_LABEL,	       // 3 
    G_OWNER_NAME[20 + 1],	   //  
    GARAGE_IMPROVEMENTS, 
};
...
//#define DIALOG_OWNABLE_CAR_GARAGE_LIST        22373   //    
...
public SYS_GARAGE_OnGameModeInit()
{
    SetTimer("CheckAndCreateGaragesTables", 1500, false);
    print("[MELLISS SYSTEM]:   !");
	SetTimer("LoadGarages", 3000, false);

    #if defined garage_SYS_GARAGE_OnGameModeInit
        return garage_SYS_GARAGE_OnGameModeInit();
    #else
        return 1;
    #endif
}
   #if defined _ALS_SYS_GARAGE_OnGameModeInit
    #undef SYS_GARAGE_OnGameModeInit
#else
    #define _ALS_SYS_GARAGE_OnGameModeInit
#endif
#define SYS_GARAGE_OnGameModeInit garage_SYS_GARAGE_OnGameModeInit
#if defined garage_SYS_GARAGE_OnGameModeInit
    forward garage_SYS_GARAGE_OnGameModeInit();
#endif
...
public: LoadGarages()
{
 ...
 rows = cache_num_rows();
    ...
    {
        printf("[WERTON_GARAGES]:      : %s", mysql_errno(mysql));
        return 0;
    }
 ...
 CreateDynamic3DTextLabel("{ffffff}  {ff0000}\n{ffffff} ", -1, -0.2940,2006.1188,1554.2031, 9.0, _, _, _, -1, -1);
 CreateDynamic3DTextLabel("{ffffff}  {ff0000}\n{ffffff} ", -1, 492.663848,1988.835205,1548.586645, 9.0, _, _, _, -1, -1);
 printf("[WERTON_GARAGES]:  : %d", g_garage_loaded);
}
...
stock UpdateGarageInfo(idx)
{
    ...
    {
            format(query, sizeof(query), "{ffff00}: %d\n{ffffff}: {ff0000}%s\n{66CC00} ", GetGarageData(idx, GARAGE_ID), GetGarageData(idx, G_OWNER_NAME));
    }
    else 
    {
        format(query, sizeof(query), "{ffff00}: %d\n{ffffff} ", GetGarageData(idx, GARAGE_ID));
    }
...
public: LoadPlayerData(playerid)
{
    ...
    if (cache_num_rows() > 0)
    {
        ...
        if (garage_id > 0)
        {
            ...
            printf("[WERTON_GARAGE OnPlayerConnect]:  ID %d ( ID: %d) -   ID: %d.  accounts.garage.", playerid, player_account_id, garage_id);
        }
    }
	}
...
public SYS_GARAGE_OnPlayerSpawn(playerid)
{
    LoadCarGarage[playerid] = 0;
    
    SetPlayerData(playerid, P_IN_GARAGE, 		-1);
    #if defined garage_SYS_GARAGE_OnPlayerSpawn
        return garage_SYS_GARAGE_OnPlayerSpawn(playerid);
    #else
        return 1;
    #endif
}
#if defined _ALS_SYS_GARAGE_OnPlayerSpawn
    #undef SYS_GARAGE_OnPlayerSpawn
#else
    #define _ALS_SYS_GARAGE_OnPlayerSpawn
#endif
#define SYS_GARAGE_OnPlayerSpawn garage_SYS_GARAGE_OnPlayerSpawn
#if defined garage_SYS_GARAGE_OnPlayerSpawn
    forward garage_SYS_GARAGE_OnPlayerSpawn(playerid);
#endif

public SYS_GARAGE_OnPlayerDeath(playerid, killerid, reason)
{
    LoadCarGarage[playerid] = 0;
    
    SetPlayerData(playerid, P_IN_GARAGE, 		-1);
    #if defined garage_SYS_GARAGE_OnPlayerSpawn
        return garage_SYS_GARAGE_OnPlayerSpawn(playerid, killerid, reason);
    #else
        return 1;
    #endif
}
#if defined _ALS_SYS_GARAGE_OnPlayerDeath
    #undef SYS_GARAGE_OnPlayerDeath
#else
    #define _ALS_SYS_GARAGE_OnPlayerDeath
#endif
#define SYS_GARAGE_OnPlayerDeath garage_SYS_GARAGE_OnPlayerDeath
#if defined garage_SYS_GARAGE_OnPlayerDeath
    forward garage_SYS_GARAGE_OnPlayerDeath(playerid, killerid, reason);
#endif

public SYS_GARAGE_OnPlayerPickUpPickupEx(playerid, pickupid, action_type, action_id)
{
	        switch(action_type)
			{
    ...
    #if defined garage_SYS_GARAGE_OnPlayerPickUpPickupEx
        return garage_SYS_GARAGE_OnPlayerPickUpPickupEx(playerid, pickupid, action_type, action_id);
    #else
        return 1;
    #endif
}
#if defined _ALS_SYS_GARAGE_OnPlayerPickUpPickupEx
    #undef SYS_GARAGE_OnPlayerPickUpPickupEx
#else
    #define _ALS_SYS_GARAGE_OnPlayerPickUpPickupEx
#endif
#define SYS_GARAGE_OnPlayerPickUpPickupEx garage_SYS_GARAGE_OnPlayerPickUpPickupEx
#if defined garage_SYS_GARAGE_OnPlayerPickUpPickupEx
    forward garage_SYS_GARAGE_OnPlayerPickUpPickupEx(playerid, pickupid, action_type, action_id);
#endif
...
public: ShowPlayerGarageInfo(playerid, garageid)
{
			...
			if(IsGarageOwned(garageid))
			{
				...
				format(fmt_str, sizeof fmt_str, "{ffffff} :\t\t\t%d\n", garageid);
				strcat(string, fmt_str);

				if(!IsGarageOwned(garageid)) strcat(string, "\n");

			format(fmt_str, sizeof fmt_str, "{ffffff}:{ffff00}\t\t\t%d {ffffff}\n", GetGarageData(garageid, GARAGE_PRICE));
			strcat(string, fmt_str);

			if(IsGarageOwned(garageid))
			{
				Dialog(playerid, DIALOG_GARAGE_ENTER, DIALOG_STYLE_MSGBOX, "{FF9900} ", string, "", "");
			}
			else Dialog(playerid, DIALOG_GARAGE_BUY, DIALOG_STYLE_MSGBOX, "{33CC00} ", string, "", "");
}

public SYS_GARAGE_OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
// , 
if(newkeys == KEY_CROUCH)
   ...
   #if defined garage_SYS_GARAGE_OnPlayerKeyStateChange
        return garage_SYS_GARAGE_OnPlayerKeyStateChange(playerid, newkeys, oldkeys);
    #else
        return 0;
    #endif
}
   #if defined _ALS_SYS_GARAGE_OnPlayerKeyStateChange
    #undef SYS_GARAGE_OnPlayerKeyStateChange
#else
    #define _ALS_SYS_GARAGE_OnPlayerKeyStateChange
#endif
#define SYS_GARAGE_OnPlayerKeyStateChange garage_SYS_GARAGE_OnPlayerKeyStateChange
#if defined garage_SYS_GARAGE_OnPlayerKeyStateChange
    forward garage_SYS_GARAGE_OnPlayerKeyStateChange(playerid, newkeys, oldkeys);
#endif

public SYS_GARAGE_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
   if(dialogid == DIALOG_GARAGE_IMPROVEMENT)
	{
	    if(response)
		{
			switch(listitem)
			{
                ...
                {
					  if(GetPlayerMoneyEx(playerid) < 5000000) return SendClientMessage(playerid, 0xCECECEFF, "      5.000.000 !");
					  ShowPlayerDialog(playerid, DIALOG_GARAGE_IMPROVEMENT_CONFIRM, DIALOG_STYLE_MSGBOX, "{ff0000}", "        \n      \n\n  : 5.000.000 .\n ,   ?", "", "");
				}
				}
			}
		}
	...
	new garageid = GetPlayerGarage(playerid);
	   ...
	   if(response)
	   {
        ...
        {
		    ShowNotification(playerid, 3, "  !", 3, "", "");
		    ShowNotification(playerid, 0, "  5000000 ", 3, "", "");
			GivePlayerMoneyEx(playerid, -5000000);
	    }
	    else
	    {
        	ShowNewNotification(playerid, 3, 5, 1, 10, "  !", "");
            GivePlayerMoneyEx(playerid, -5000000);
        }
	    }
 	   else 
        {
	       Dialog
               (
                    ...
                    "{ffffff}1.    \t{ffffff}30000 .\t\t{66CC00}\n"\
                    "{ffffff}2.   {ffffff}\t\t\t\t\t\t\t\t\t40000 .\t\t\t\t\t{66CC00}\n"\
                    "{ffffff}3.   \t\t\t\t\t{ffffff}5000000 .\t\t\t\t\t{ff0000} ",
                    "", ""
                );
        }
    }
                               ...
                               else if(GetGarageData(garageid, GARAGE_IMPROVEMENTS) == 2)
						   	{
								Dialog
                                    (
                                        ...
                                        "{ffffff}1.    \t{ffffff}30000 .\t\t{66CC00}\n"\
                                        "{ffffff}2.   {ffffff}\t\t\t\t\t\t\t\t\t40000 .\t\t\t\t\t{66CC00}\n"\
                                        "{ffffff}3.   \t\t\t\t\t{ffffff}5000000 .\t\t\t\t\t{66CC00}",
                                        "", ""
                                    );
                               }
								}
								...
								case 4:
								{
	                              ...
	                              if(GetPlayerGPSInfo(playerid, G_ENABLED) == GPS_STATUS_OFF)
						     	{
							    	...
							    	{
									   GivePlayerMoneyEx(playerid, -300, "   GPS", true, true);
								...
								case 5:
								{
								  SendClientMessage(playerid, 0x999999FF, "  /sellmygarage");
                                }
							}
					}
					else 
     	...
     	if(response)
		 {
              if(GetPlayerGarage(playerid) == -1)
          	{
						...
						else
						{
							...
							format(fmt_str, sizeof fmt_str, "   . : %s", GetGarageData(garageid, G_OWNER_NAME));
    ...
    #if defined garage_SYS_GARAGE_OnDialogResponse
    return garage_SYS_GARAGE_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
    #else
        return 1;
    #endif
}
#if defined _ALS_SYS_GARAGE_OnDialogResponse
#undef SYS_GARAGE_OnDialogResponse
#else
#define _ALS_SYS_GARAGE_OnDialogResponse
#endif
#define SYS_GARAGE_OnDialogResponse garage_SYS_GARAGE_OnDialogResponse
#if defined garage_SYS_GARAGE_OnDialogResponse
forward garage_SYS_GARAGE_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]);
#endif
...
new garage_percent = (garage_price * 30) / 100;
		...
		if(to_player == INVALID_PLAYER_ID)
		{
			...
			format(query, sizeof query, "     30     {99CC00}(%d )", garage_percent);
	...
	if(!IsGarageOwned(garageid) && GetPlayerGarage(playerid) == -1)
	{
		...
		if(GetPlayerMoneyEx(playerid) >= price)
		{
			...
			if(!mysql_errno())
			{
				...
				SendClientMessage(playerid, 0x66CC00FF, " {0099FF}/garage {66CC00}   ");
...
CMD:addgarage(playerid, params[])
{
	...
	extract params -> new price; else return SendClientMessage(playerid, 0xCECECEFF, ": /addgarage []");
	...
	SendClientMessage(playerid, 0x3399FFFF, " {FFFF00}/gsetexitpos{3399FF},      ");
...
CMD:gsetexitpos(playerid, params[])
{
	...
	extract params -> new garage_id; else return SendClientMessage(playerid, 0xCECECEFF, ": /gsetexitpos [id ]");

	if(!(0 <= garage_id <= g_garage_loaded - 1)) return SendClientMessage(playerid, 0x999999FF, "     ");
	...
	format(fmt_text, sizeof fmt_text, "       %d", garage_id);
...
CMD:garage(playerid, params[])
{
	...
	if(garageid != -1)
	{
		...
		(
			...
			area,
			GetGarageData(garageid, GARAGE_PRICE),
			GetGarageData(garageid, GARAGE_STATUS) ? ("{CC3333} ") : ("{66CC33} ")
		);
		Dialog(playerid, DIALOG_GARAGE_INFO, DIALOG_STYLE_MSGBOX, "{33AACC}  ", fmt_str, "", "");
...
{
    if (GetPlayerAdminEx(playerid) < 6) return SendClientMessage(playerid, 0x999999FF, "       .");
    ...
    {
        SendClientMessage(playerid, 0xCECECEFF, ": /asellgarage [ID ] [ ()]");
        return 1;
    }

    if (garageid < 1 || garageid > MAX_GARAGES)
    {
        SendClientMessage(playerid, 0x999999FF, " ID . ID    1  MAX_GARAGES.");
        return 1;
    }
    ...
    if (mysql_errno(mysql) != 0)
    {
        printf("[MySQL Error]:    owner_id   %d: %s", garageid, mysql_errno(mysql));
    ...
    {
        SendClientMessage(playerid, 0xCECECEFF, "   ID     .");
        return 1;
    }
    ...
    if (owner_account_id > 0)
    {
        ...
        if (mysql_errno(mysql) != 0)
        {
            printf("[MySQL Error]:       %d: %s", owner_account_id, mysql_errno(mysql));
            SendClientMessage(playerid, COLOR_RED, "      .");
            format(former_owner_name, sizeof(former_owner_name), "  (ID: %d)", owner_account_id);
        }
        ...
        if (mysql_errno(mysql) != 0)
        {
            printf("[MySQL Error]:    garages.owner_id   %d: %s", garageid, mysql_errno(mysql));
            SendClientMessage(playerid, COLOR_RED, "     .");
            return 1;
        }
        ...
        if (mysql_errno(mysql) != 0)
        {
            printf("[MySQL Error]:    accounts.garage   %d: %s", owner_account_id, mysql_errno(mysql));
            SendClientMessage(playerid, COLOR_RED, "      .");
            return 1;
        }

        SendClientMessage(playerid, 0x00FF00FF, !"  .");
        printf("[ADMIN]:  %s (ID: %d)   ID: %d,    ID: %d",
               GetPlayerNameEx(playerid), playerid, garageid, owner_account_id);

        new broadcast_message[MAX_PLAYER_NAME + MAX_PLAYER_NAME + 150];
        format(broadcast_message, sizeof(broadcast_message), " %s    %s", GetPlayerNameEx(playerid), former_owner_name);

        if(strlen(reason) > 0)
        {
            new reason_text[sizeof(reason) + 16];
            format(reason_text, sizeof(reason_text), ". : %s", reason);
