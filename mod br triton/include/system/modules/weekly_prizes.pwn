...
{
    {"100 -.", WP_RUB, 100},
    {"500000 .", WP_MONEY, 500000},
    {"150 -.", WP_RUB, 150},
    {"VIP  (3 .)", WP_VIP, 3},
    {"30000000 .", WP_MONEY, 30000000},
    {"500 -.", WP_RUB, 500},
    {"3   ", WP_SLOT, 3}
};
...
new player_wp[MAX_PLAYERS][8]; // 7 (8) -        
...
public:CREATE_ACCOUNTS_TABLE_WP()
{
    ...
    if(mysql_errno())
    {
        ...
        if(!mysql_errno()) print("[LAIRD_SYSTEM]     ");
    }
...
public SYS_WEEKLY_PRIZES_OnPlayerConnect(playerid)
{
...
#if defined _ALS_SYS_WEEKLY_PRIZES_OnPlayerConnect
    #undef SYS_WEEKLY_PRIZES_OnPlayerConnect
#else
    #define _ALS_SYS_WEEKLY_PRIZES_OnPlayerConnect
#endif
#define SYS_WEEKLY_PRIZES_OnPlayerConnect weekly_OnPlayerConnect
#if defined weekly_OnPlayerConnect
    forward weekly_OnPlayerConnect(playerid);
#endif

public SYS_WEEKLY_PRIZES_OnPlayerDisconnect(playerid, reason)
{
...
#if defined _ALS_SYS_WEEKLY_PRIZES_OnPlayerDisconnect
    #undef SYS_WEEKLY_PRIZES_OnPlayerDisconnect
#else
    #define _ALS_SYS_WEEKLY_PRIZES_OnPlayerDisconnect
#endif
#define SYS_WEEKLY_PRIZES_OnPlayerDisconnect weekly_OnPlayerDisconnect
#if defined weekly_OnPlayerDisconnect
    forward weekly_OnPlayerDisconnect(playerid, reason);
#endif

public SYS_WEEKLY_PRIZES_OnGameModeInit()
{
   ...
   #if defined _ALS_SYS_WEEKLY_PRIZES_OnGameModeInit
    #undef SYS_WEEKLY_PRIZES_OnGameModeInit
#else
    #define _ALS_SYS_WEEKLY_PRIZES_OnGameModeInit
#endif
#define SYS_WEEKLY_PRIZES_OnGameModeInit weekly_OnGameModeInit
#if defined weekly_OnGameModeInit
...
public:CheckWeeklyPrizes()
{
    ...
    foreach(new i : Player)
    {
        ...
        {
            SendClientMessage(i, -1, ",   2      ! ({FFFF00}/everyprize{FFFFFF})");
            player_wp[i][pday]++;
            player_wp[i][7]++;
...
cmd:everyprize(playerid)
{
    ...
    for(new i, prize[28], list[84]; i < 7;i ++)
    {
        ...
        {
            case STATUS_WP_WAITING: format(prize, sizeof prize, "{787878}[ 2 ]");
            case STATUS_WP_MAYBE: format(prize, sizeof prize, "{E6E32C}[ ]");
            case STATUS_WP_TOOK_IT: format(prize, sizeof prize, "{39D055}[]");
    ...
    ShowPlayerDialog(playerid, 3882, DIALOG_STYLE_LIST, " ", dialog, "", "");

    return 1;
}

public SYS_WEEKLY_PRIZES_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == 3882)
    {
        if(response)
        {
            ...
            if(day && player_wp[playerid][day] == STATUS_WP_MAYBE && player_wp[playerid][day-1] == STATUS_WP_MAYBE) // status_maybe
                return SendClientMessage(playerid, -1, "{E6E32C}[ ] {FFFFFF}  ,  ");

            if(day && !player_wp[playerid][day] && !player_wp[playerid][day-1])
                return SendClientMessage(playerid, -1, "{E6E32C}[ ] {FFFFFF}       ,   ");

            if(player_wp[playerid][day] == STATUS_WP_TOOK_IT) return SendClientMessage(playerid, -1, "{E6E32C}[ ] {FFFFFF}    ");

            switch(player_wp[playerid][day])
            {
                case STATUS_WP_WAITING: SendClientMessage(playerid, -1, "{E6E32C}[ ] {FFFFFF}    2 ");
                case STATUS_WP_MAYBE:
                {
                    ...
                    new string[144];
                    format(string, sizeof string, "{E6E32C}[ ] {FFFFFF} !   {E6E32C}%s{FFFFFF}", weekly_prizes[day][WP_Name]);
                ...
                case STATUS_WP_TOOK_IT: SendClientMessage(playerid, -1, "{E6E32C}[ ] {FFFFFF}    ");
            }
        }
    }
...
#if defined _ALS_SYS_WEEKLY_PRIZES_OnDialogResponse
#undef SYS_WEEKLY_PRIZES_OnDialogResponse
#else
#define _ALS_SYS_WEEKLY_PRIZES_OnDialogResponse
#endif
#define SYS_WEEKLY_PRIZES_OnDialogResponse weekly_OnDialogResponse
#if defined weekly_OnDialogResponse
