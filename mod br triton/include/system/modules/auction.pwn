...
public SYS_AUCTION_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == 5990)
    {
        if(response)
        {
            ...
            if(!(rate >= price)) return SendClientMessage(playerid, -1, ""SC"        .");
        ...
        switch(GetPVarInt(playerid, "type_param"))
        {
            case TYPE_VEHICLE_NUMBER:
            {
                ...
                if(num[0] == 0)
                {
                    ...
                    format(text, sizeof text, "       %s", num_car);
            ...
            case TYPE_CLOTHES:
            {
                ...
                format(text, sizeof text, "      %s", GetSkinInfo(server, S_NAME));
        ...
        if(response)
        {
            ...
            else if(player_info_panel[playerid][filter_type] == 2)
            {
                ...
                if(sscanf(inputtext, "P<,>dd", min_price, max_price))
                {
                    ...
                    return SYS_AUCTION_OnDialogResponse(playerid, 5993, 1, 1, "");
                }
                else
                {
...
#if defined _ALS_SYS_AUCTION_OnDialogResponse
#undef SYS_AUCTION_OnDialogResponse
#else
#define _ALS_SYS_AUCTION_OnDialogResponse
#endif
#define SYS_AUCTION_OnDialogResponse auc_OnDialogResponse
#if defined auc_OnDialogResponse
...
public SYS_AUCTION_OnGameModeInit()
{
   ...
   #if defined _ALS_SYS_AUCTION_OnGameModeInit
    #undef SYS_AUCTION_OnGameModeInit
#else
    #define _ALS_SYS_AUCTION_OnGameModeInit
#endif
#define SYS_AUCTION_OnGameModeInit auc_OnGameModeInit
#if defined auc_OnGameModeInit
    forward auc_OnGameModeInit();
#endif

public SYS_AUCTION_OnPlayerConnect(playerid)
{
...
#if defined _ALS_SYS_AUCTION_OnPlayerConnect
    #undef SYS_AUCTION_OnPlayerConnect
#else
    #define _ALS_SYS_AUCTION_OnPlayerConnect
#endif
#define SYS_AUCTION_OnPlayerConnect auc_OnPlayerConnect
#if defined auc_OnPlayerConnect
    forward auc_OnPlayerConnect(playerid);
#endif

cmd:tpa(playerid){
    CallLocalFunction("SYS_AUCTION_OnPlayerEnterDynamicArea", "ii", playerid, enter_auction);
    return 1;
}

public SYS_AUCTION_OnPlayerEnterDynamicArea(playerid, areaid)
{
   ...
   #if defined _ALS_SYS_AUCTION_OnPlayerEnterDynamicArea
    #undef SYS_AUCTION_OnPlayerEnterDynamicArea
#else
    #define _ALS_SYS_AUCTION_OnPlayerEnterDynamicArea
#endif
#define SYS_AUCTION_OnPlayerEnterDynamicArea auc_OnPlayerEnterDynamicArea
#if defined auc_OnPlayerEnterDynamicArea
...
public SYS_AUCTION_OnPlayerClickPlayerTextDraw(playerid, PlayerText:playertextid)
{
    ...
    if(playertextid == auction_PTD[playerid][30])
    {
        ...
        switch(GetPVarInt(playerid, "type_param"))
        {
            ...
            case TYPE_VEHICLE_NUMBER:
            {
                ...
                if(!cache_num_rows()) return SendClientMessage(playerid, -1, ""USC"      ");
    ...
    if(playertextid == auction_PTD[playerid][37])
    {
        ...
        switch(auction_slot[id_slot][SLOT_TYPE])
        {
            case TYPE_BUSINESS:
            {
                if(GetPlayerBusiness(playerid) != -1) return SendClientMessage(playerid, -1, ""SC"     ");
            ...
            case TYPE_HOUSE:
            {
                if(GetPlayerHouse(playerid) != -1) return SendClientMessage(playerid, -1, ""SC"     ");
            ...
            case TYPE_VEHICLE_NUMBER:
            {
                ...
                if(!rows) return SendClientMessage(playerid, -1, ""SC"       ");
            ...
            case TYPE_SUM_NUMBER:
            {
                if(GetPlayerPhone(playerid)) 
                {
                    for(new i; i < 5; i++) SendClientMessage(playerid, -1, ""USC"     -.      ");
                }
            }
        }
    ...
    #undef SYS_AUCTION_OnPlayerClickPlayerTextDraw
#else
...
#endif
#define	SYS_AUCTION_OnPlayerClickPlayerTextDraw auc_OnPlayerClickPlayer
...
stock create_auction_slot(playerid, type, opisanie[124], start_rate, data_base, server_id, number_vehicle[7] = "", number_SUM = 0)
{
    ...
    if(stop_create) return SendClientMessage(playerid, -1, ""USC"     .");
    ...
    switch(type)
    {
        ...
        case TYPE_HOUSE:
        {
            for(new s;s < MAX_HOUSES;s++)
            {
                ...
                {
                    auction_slot[id][SERVER_ID] = s;
                    format(auction_slot[id][SLOT_NAME], 24, "_(%d)", s);
                    break;
                }
            }
        }
...
stock finish_auction_slot(slot)
{
    ...
    switch(auction_slot[slot][SLOT_TYPE])
    {
        case TYPE_BUSINESS:
        {
            foreach(new i : Player)
            {
                if(auction_slot[slot][OWNER_RATE] == GetPlayerAccountID(i)) SendClientMessage(i, -1, ""SC" !     ");
            }
        ...
        case TYPE_VEHICLE_NUMBER:
        {
            foreach(new i : Player)
            {
                if(GetPlayerAccountID(i) == auction_slot[slot][OWNER_RATE]) SendClientMessage(i, -1, ""SC" !     ");
                if(GetPlayerAccountID(i) == auction_slot[slot][OWNER_SLOT_SQL]) SendClientMessage(i, -1, ""SC" !     ");
            }
...
stock AuctionBusiness(sql_id, slot)
{
        ...
        if(!mysql_errno())
        {
            ...
            SendClientMessage(player, 0x66CC00FF, " {0099FF}/business {66CC00}   ");
...
public: SetBusinessName()
{
    ...
    mysql_format(mysql, text, sizeof text, "UPDATE business SET `name`='' WHERE `type` = 7");
    mysql_query(mysql, text, false);
...
public:LoadBusinessToAuction()
{
    ...
    for(new b;b < memory;b++)
    {
        ...
        format(auction_slot[b][OWNER_SLOT_NAME], 24, "");
        ...
        for(new sql;sql < MAX_BUSINESS;sql++)
        {
            if(GetBusinessData(sql, B_SQL_ID) == auction_slot[b][DATABASE_ID]) 
            {
                ...
                format(auction_slot[b][OPISANIE_SLOT], 124, " (%d)", sql);
                break;
            }
        }
...
stock defualt_slot()
{
    for(new DEATH; DEATH < MAX_AUCTION_SLOT; DEATH++)
    {
        ...
        format(auction_slot[DEATH][SLOT_NAME], 24, "-");
