...
public SYS_MCHS_OnGameModeInit()
{
    ...
    CreateActorEx(" ", "  ", 261, -2615.917724,-282.265045,1252.043823,265.929382, 1, 1);
    ...
    CreateDynamic3DTextLabel("   ", 0xFFEE00FF3, -2608.042480,-282.297698,1252.043823, 15.0);
    CreateDynamic3DTextLabel("    ", 0xFFEE00FF3, -2555.758544,-277.883331,27.260526, 15.0);
...
#if defined _ALS_SYS_MCHS_OnGameModeInit
    #undef SYS_MCHS_OnGameModeInit
#else
    #define _ALS_SYS_MCHS_OnGameModeInit
#endif
#define SYS_MCHS_OnGameModeInit mchs_OnGameModeInit
#if defined mchs_OnGameModeInit
    forward mchs_OnGameModeInit();
#endif

public SYS_MCHS_OnPlayerDisconnect(playerid, reason)
{
...
#if defined _ALS_SYS_MCHS_OnPlayerDisconnect
    #undef SYS_MCHS_OnPlayerDisconnect
#else
    #define _ALS_SYS_MCHS_OnPlayerDisconnect
#endif
#define SYS_MCHS_OnPlayerDisconnect mchs_OnPlayerDisconnect
#if defined mchs_OnPlayerDisconnect
    forward mchs_OnPlayerDisconnect(playerid, reason);
#endif


public SYS_MCHS_OnPlayerConnect(playerid)
{
    player_order[playerid] = 0;
...
#if defined _ALS_SYS_MCHS_OnPlayerConnect
    #undef SYS_MCHS_OnPlayerConnect
#else
    #define _ALS_SYS_MCHS_OnPlayerConnect
#endif
#define SYS_MCHS_OnPlayerConnect mchs_OnPlayerConnect
#if defined mchs_OnPlayerConnect
    forward mchs_OnPlayerConnect(playerid);
#endif

public SYS_MCHS_OnPlayerEnterDynamicArea(playerid, areaid)
{
    if(areaid == exit_mchs)
    {
    ...
    {
        if(!player_mchs_active{playerid}) return SCM(playerid, -1,""USC"   ");
        Dialog(playerid, 3502, DSM, " ", "   ", "", "");
    }
    if(areaid == veh_mchs)
    {
        if(!player_mchs_active{playerid}) return SCM(playerid, -1,""USC"   ");
        Dialog(playerid, 3503, DSM, "  ", "    ", "", "");
    }
...
#if defined _ALS_SYS_MCHS_OnPlayerEnterDynamicArea
    #undef SYS_MCHS_OnPlayerEnterDynamicArea
#else
    #define _ALS_SYS_MCHS_OnPlayerEnterDynamicArea
#endif
#define SYS_MCHS_OnPlayerEnterDynamicArea mchs_OnPlayerEnterDynamicArea
#if defined mchs_OnPlayerEnterDynamicArea
    forward mchs_OnPlayerEnterDynamicArea(playerid, STREAMER_TAG_AREA:areaid);
#endif

public SYS_MCHS_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == 3500)
    {
        if(response)
        {
            ...
            EnablePlayerGPS(playerid, 55, coord_order[id][0],  coord_order[id][1], coord_order[id][2], "    !");
            ...
            if(mysql_errno()) return SCM(playerid, -1, "   1");
...
#if defined _ALS_SYS_MCHS_OnDialogResponse
#undef SYS_MCHS_OnDialogResponse
#else
#define _ALS_SYS_MCHS_OnDialogResponse
#endif
#define SYS_MCHS_OnDialogResponse mchs_OnDialogResponse
#if defined mchs_OnDialogResponse
forward mchs_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]);
#endif

public SYS_MCHS_OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(PRESSED(KEY_FIRE))
	{
        if(press_fire{playerid}) return SCM(playerid, -1, ""USC"  ...");
                ...
                if(all_fire[IsPlayerInRangeOfAnyFire(playerid)][F_ID_Z] == player_order[playerid])
                {
                    ...
                    { 
                        ...
                        if(Order(id))
                        {
                            ...
                            GivePlayerMoneyEx(playerid, 10000); // 10000   
                            endOrders(id);
                            player_order[playerid] = 0;
                            mysql_format(mysql, String128m, sizeof String128m, "DELETE FROM order_mchs WHERE id = %d", id -1);
	                        mysql_query(mysql, String128m, false);

                            if(mysql_errno()) return SCM(playerid, -1, "   2");
                        }
                        return 1;
                    }
                    
                    all_fire[id_fire][F_SCORE] ++;
                    format(text, sizeof text, "~y~: ~w~%d/10", all_fire[id_fire][F_SCORE]);

                    GameTextForPlayer(playerid, text, 3000, 5);

                    return 0;
                }
                else SCM(playerid, -1, ""USC"      ");
			}
			else SCM(playerid, -1, ""USC"     ");
		}
	}
...
#if defined _ALS_SYS_MCHS_OnPlayerKeyStateChange
    #undef SYS_MCHS_OnPlayerKeyStateChange
#else
    #define _ALS_SYS_MCHS_OnPlayerKeyStateChange
#endif
#define SYS_MCHS_OnPlayerKeyStateChange mchs_OnPlayerKeyStateChange
#if defined mchs_OnPlayerKeyStateChange
...
{
    SYS_MCHS_OnPlayerKeyStateChange(playerid, KEY_FIRE, KEY_FIRE);
    return 1;
}
...
CMD:meslist(playerid)
{
    ...
    if(!rows) return SCM(playerid, 0xFFFFFFFF3, "    .  ");

    if(player_order[playerid]) return SCM(playerid, -1, ""USC"     ");

    new id, text[185], list[sizeof text * 10 + 50] = "{FFFFFF} \t\n";
    new Float:dista;
    ...
    if(rows)
    {
        for(new r; r< rows; r++)
        {
            ...
            format(text, sizeof text, "{FFFFFF} [%d]\t{B3B3B3}%.2f\n", id, dista);
            strcat(list, text);
            SetPlayerListitemValue(playerid, r, id);
        }

        DialogMhsc(playerid, 3500, DSTH, " ", list, "", "");
