...
new vehicle_get_race[3] = {415, 466, 2582};
...
public SYS_DUEL_RACE_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == 1877)
    {
        if(response)
        {
            ...
            else
            {
                ...
                if(money > 10_000_000)
                {
                    ...
                    SendClientMessage(playerid, -1, ""SC"   10.000.000 ");
                    SYS_DUEL_RACE_OnDialogResponse(playerid, 1877, 1, -1, "");
                    return 1;
                }
    ...
    #if defined race_SYS_DUEL_RACE_OnDialogResponse
return race_SYS_DUEL_RACE_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
#endif
}
#if defined _ALS_SYS_DUEL_RACE_OnDialogResponse
#undef SYS_DUEL_RACE_OnDialogResponse
#else
#define _ALS_SYS_DUEL_RACE_OnDialogResponse
#endif
#define SYS_DUEL_RACE_OnDialogResponse race_SYS_DUEL_RACE_OnDialogResponse
#if defined race_SYS_DUEL_RACE_OnDialogResponse
forward race_SYS_DUEL_RACE_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]);
#endif

public SYS_DUEL_RACE_OnPlayerCommandText(playerid, cmdtext[])
{
    if(player_active_race[playerid])
    {
        if(!GetPlayerAdminEx(playerid)) return SendClientMessage(playerid, -1, ""USC"      .      ");
    } 
    
    #if defined race_SYS_DUEL_RACE_OnPlayerCommandText
        return race_SYS_DUEL_RACE_OnPlayerCommandText(playerid, cmdtext);
    #else
        return 0;
    #endif
}
   #if defined _ALS_SYS_DUEL_RACE_OnPlayerCommandText
    #undef SYS_DUEL_RACE_OnPlayerCommandText
#else
    #define _ALS_SYS_DUEL_RACE_OnPlayerCommandText
#endif
#define SYS_DUEL_RACE_OnPlayerCommandText race_SYS_DUEL_RACE_OnPlayerCommandText
#if defined race_SYS_DUEL_RACE_OnPlayerCommandText
    forward race_SYS_DUEL_RACE_OnPlayerCommandText(playerid, cmdtext[]);
#endif

public SYS_DUEL_RACE_OnPlayerEnterRaceCheckpoint(playerid)
{
    if(player_active_race[playerid])
    {
        ...
        switch(player_type_race[playerid])
        {
            ...
            case 2:
            {
                switch(count_cp)
                {
                    case 9:
                    {
                        SendClientMessage(playerid, -1, "{FFF000}   ");
    ...
    #undef SYS_DUEL_RACE_OnPlayerEnterRaceCheckpoint
#else
...
#endif
#define	SYS_DUEL_RACE_OnPlayerEnterRaceCheckpoint re_OnPlayerEnterRaceCP

public SYS_DUEL_RACE_OnPlayerEnterDynamicArea(playerid, areaid)
{

    #if defined race_SYS_DUEL_RACE_OnPlayerEnterDynamicArea
        return race_SYS_DUEL_RACE_OnPlayerEnterDynamicArea(playerid, STREAMER_TAG_AREA:areaid);
    #else
        return 0;
    #endif
}
   #if defined _ALS_SYS_DUEL_RACE_OnPlayerEnterDynamicArea
    #undef SYS_DUEL_RACE_OnPlayerEnterDynamicArea
#else
    #define _ALS_SYS_DUEL_RACE_OnPlayerEnterDynamicArea
#endif
#define SYS_DUEL_RACE_OnPlayerEnterDynamicArea race_SYS_DUEL_RACE_OnPlayerEnterDynamicArea
#if defined race_SYS_DUEL_RACE_OnPlayerEnterDynamicArea
    forward race_SYS_DUEL_RACE_OnPlayerEnterDynamicArea(playerid, STREAMER_TAG_AREA:areaid);
#endif


public SYS_DUEL_RACE_OnGameModeInit()
{
    print("[LAIRD_SYSTEM]   \");
    TextDrawRace();
    #if defined race_SYS_DUEL_RACE_OnGameModeInit
        return race_SYS_DUEL_RACE_OnGameModeInit();
    #else
        return 1;
    #endif
}
   #if defined _ALS_SYS_DUEL_RACE_OnGameModeInit
    #undef SYS_DUEL_RACE_OnGameModeInit
#else
    #define _ALS_SYS_DUEL_RACE_OnGameModeInit
#endif
#define SYS_DUEL_RACE_OnGameModeInit race_SYS_DUEL_RACE_OnGameModeInit
#if defined race_SYS_DUEL_RACE_OnGameModeInit
    forward race_SYS_DUEL_RACE_OnGameModeInit();
#endif

public SYS_DUEL_RACE_OnPlayerDisconnect(playerid, reason)
{
    if(player_active_race[playerid])
    {
        ResultDuelRace(player_opponent[playerid], playerid);
        SendClientMessage(player_opponent[playerid], -1, ""SC"     .   ");
    }

    #if defined race_SYS_DUEL_RACE_OnPlayerDisconnect
        return race_SYS_DUEL_RACE_OnPlayerDisconnect(playerid, reason);
    #else
        return 1;
    #endif
}
#if defined _ALS_SYS_DUEL_RACE_OnPlayerDisconnect
    #undef SYS_DUEL_RACE_OnPlayerDisconnect
#else
    #define _ALS_SYS_DUEL_RACE_OnPlayerDisconnect
#endif
#define SYS_DUEL_RACE_OnPlayerDisconnect race_SYS_DUEL_RACE_OnPlayerDisconnect
#if defined race_SYS_DUEL_RACE_OnPlayerDisconnect
    forward race_SYS_DUEL_RACE_OnPlayerDisconnect(playerid, reason);
#endif

public SYS_DUEL_RACE_OnPlayerConnect(playerid)
{
    player_vehicle_race[playerid] = -1;
    RemovePlayerNew(playerid);

    #if defined race_SYS_DUEL_RACE_OnPlayerConnect
        return race_SYS_DUEL_RACE_OnPlayerConnect(playerid);
    #else
        return 1;
    #endif
}
#if defined _ALS_SYS_DUEL_RACE_OnPlayerConnect
    #undef SYS_DUEL_RACE_OnPlayerConnect
#else
    #define _ALS_SYS_DUEL_RACE_OnPlayerConnect
#endif
#define SYS_DUEL_RACE_OnPlayerConnect race_SYS_DUEL_RACE_OnPlayerConnect
#if defined race_SYS_DUEL_RACE_OnPlayerConnect
    forward race_SYS_DUEL_RACE_OnPlayerConnect(playerid);
#endif

public SYS_DUEL_RACE_OnPlayerSpawn(playerid)
{
    if(player_active_race[playerid]) RemoveDuelRace(playerid, player_opponent[playerid]);
    #if defined race_SYS_DUEL_RACE_OnPlayerSpawn
        return race_SYS_DUEL_RACE_OnPlayerSpawn(playerid);
    #else
        return 1;
    #endif
}
#if defined _ALS_SYS_DUEL_RACE_OnPlayerSpawn
    #undef SYS_DUEL_RACE_OnPlayerSpawn
#else
    #define _ALS_SYS_DUEL_RACE_OnPlayerSpawn
#endif
#define SYS_DUEL_RACE_OnPlayerSpawn race_SYS_DUEL_RACE_OnPlayerSpawn
#if defined race_SYS_DUEL_RACE_OnPlayerSpawn
    forward race_SYS_DUEL_RACE_OnPlayerSpawn(playerid);
#endif


stock TextDrawRace()
{
    race_time_TD[0] = TextDrawCreate(294.6666, 39.4222, "_"); // 
    ...
    race_time_TD[2] = TextDrawCreate(298.9998, 40.2518, "OCAOC_PEMEH:"); // 
    ...
    race_time_TD[1] = TextDrawCreate(295.6667, 40.2519, "_"); // 
...
{
    race_time_PTD[playerid][0] = CreatePlayerTextDraw(playerid, 320.6665, 49.7925, "LOAD..."); // 
...
CMD:gonki(playerid, params[])
{
    if(GetPVarInt(playerid, "player_time_opponent"))
        DeletePVar(playerid, "player_time_opponent");

    if(player_active_race[playerid])
        return SendClientMessage(playerid, -1, ""USC"    ");

    if(!strlen(params))
        return SendClientMessage(playerid, 0xCECECEFF, ": /rduel [id ]");

    if(GetPlayerSuspect(playerid))
        return SendClientMessage(playerid, 0x999999FF, "     ");

    extract params -> new playerr_opponent;

    if(GetPlayerSuspect(playerr_opponent))
        return SendClientMessage(playerid, 0x999999FF, "   ");
    ...
    // ??      
    new duel_cost = 1000;
    if(GetPlayerMoney(playerid) < duel_cost)
        return SendClientMessage(playerid, 0xFF0000FF, "      ( 1000 )");

    new fmt_text[100];
    format(fmt_text, sizeof fmt_text, "    %s", GetPlayerNameEx(playerr_opponent));
    GivePlayerMoneyEx(playerid, -duel_cost, fmt_text, true, true);

    // ??    
    format(fmt_text, sizeof fmt_text, "-%d  (  )", duel_cost);
    ...
    ShowNotification(playerr_opponent, 4, fmt_text, 6, "/yesgonki", ">>");

    format(fmt_text, sizeof fmt_text, "%s    .  /yesgonki,  .", GetPlayerNameEx(playerid));
    SendClientMessage(playerr_opponent, 0x00FF00FF, fmt_text);
...
CMD:yesgonki(playerid)
{
    new player_invite = -1;

    foreach(new p : Player)
    {
        if(GetPVarInt(p, "player_time_opponent") == playerid)
            player_invite = p;
    }
    ...
    if(player_invite != -1)
    {
        ...
        format(text, sizeof text, "  | {FFFF00}%s", GetPlayerNameEx(playerid));
...
stock LoadDuelRace(type, p1, p2)
{
    ...
    if(!IsPlayerConnected(p1)) return SendClientMessage(p2, -1, ""SC"  .   ");
    if(!IsPlayerConnected(p2)) return SendClientMessage(p1, -1, ""SC"  .   ");
...
{  
    ...
    SendClientMessage(p1, -1, ""SC"  ");
    SendClientMessage(p2, -1, ""SC"  ");
...
public:ChangeTimerDuelRace(playerid)
{
    if(race_countdown[playerid] != 0)
    {
        ...
        format(fmt_str, sizeof fmt_str,"%d_ceky", race_countdown[playerid]);
...
stock ResultDuelRace(winner, loser)
{
    ...
    format(text, sizeof text, " {FFFF00}%s {FFFFFF}   .", GetPlayerNameEx(winner));
    ...
    if(money != 0)
    {
        ...
        {
            new prize[27];
            format(prize, sizeof prize, " :{FFFF00} %d", money);
            strcat(text, prize);
            GivePlayerMoneyEx(winner, money);
            GivePlayerMoneyEx(loser, -money);
        }
        else SendClientMessage(winner, -1, ""SC"     .");
    }
