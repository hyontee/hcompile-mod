...
new car_market_type[88][STRUCT_VEH_MARKET] = //   88 
{
    //  ( )
    {462, VEH_BIKE},
    {468, VEH_BIKE},
...
public SYS_USED_VEH_MARKET_OnPlayerSpawn(playerid)
{
...
#if defined _ALS_SYS_USED_VEH_MARKET_OnPlayerSpawn
    #undef SYS_USED_VEH_MARKET_OnPlayerSpawn
#else
    #define _ALS_SYS_USED_VEH_MARKET_OnPlayerSpawn
#endif
#define SYS_USED_VEH_MARKET_OnPlayerSpawn by_OnPlayerSpawn
#if defined by_OnPlayerSpawn
    forward by_OnPlayerSpawn(playerid);
#endif

public SYS_USED_VEH_MARKET_OnPlayerDisconnect(playerid, reason)
{
    player_exit_from_used_market(playerid);
...
#if defined _ALS_SYS_USED_VEH_MARKET_OnPlayerDisconnect
    #undef SYS_USED_VEH_MARKET_OnPlayerDisconnect
#else
    #define _ALS_SYS_USED_VEH_MARKET_OnPlayerDisconnect
#endif
#define SYS_USED_VEH_MARKET_OnPlayerDisconnect by_OnPlayerDisconnect
#if defined by_OnPlayerDisconnect
    forward by_OnPlayerDisconnect(playerid, reason);
#endif

public SYS_USED_VEH_MARKET_OnGameModeInit()
{
    print("[LAIRD_SYSTEM]   \n: Welsi Studio");
    CreateUsedMarket();
   ...
   #if defined _ALS_SYS_USED_VEH_MARKET_OnGameModeInit
    #undef SYS_USED_VEH_MARKET_OnGameModeInit
#else
    #define _ALS_SYS_USED_VEH_MARKET_OnGameModeInit
#endif
#define SYS_USED_VEH_MARKET_OnGameModeInit by_OnGameModeInit
#if defined by_OnGameModeInit
    forward by_OnGameModeInit();
#endif

public SYS_USED_VEH_MARKET_OnPlayerEnterDynamicArea(playerid, areaid)
{
    if(areaid == pickup_used_market)
    {
        ...
        (
            text, sizeof text, 
            " \t- \t\n"\
            "{FF6347}1. {FFFFFF} \t{FF6347}%d\t{7A7A7A}  \n"\
            "{FF6347}2. {FFFFFF} \t{FF6347}%d\t{7A7A7A}  \n"\
            "{FF6347}3. {FFFFFF} \t{FF6347}%d\t{7A7A7A}  \n"\
            "{FF6347}4. {FFFFFF}\t{FF6347}%d\t{7A7A7A}  ",
            small_veh_to_market,
   ...
   #if defined _ALS_SYS_USED_VEH_MARKET_OnPlayerEnterDynamicArea
    #undef SYS_USED_VEH_MARKET_OnPlayerEnterDynamicArea
#else
    #define _ALS_SYS_USED_VEH_MARKET_OnPlayerEnterDynamicArea
#endif
#define SYS_USED_VEH_MARKET_OnPlayerEnterDynamicArea by_OnPlayerEnterDynamicArea
#if defined by_OnPlayerEnterDynamicArea
    forward by_OnPlayerEnterDynamicArea(playerid, STREAMER_TAG_AREA:areaid);
#endif

public SYS_USED_VEH_MARKET_OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
   ...
   #if defined _ALS_SYS_USED_VEH_MARKET_OnPlayerKeyStateChange
    #undef SYS_USED_VEH_MARKET_OnPlayerKeyStateChange
#else
    #define _ALS_SYS_USED_VEH_MARKET_OnPlayerKeyStateChange
#endif
#define SYS_USED_VEH_MARKET_OnPlayerKeyStateChange by_OnPlayerKeyStateChange
#if defined by_OnPlayerKeyStateChange
    forward by_OnPlayerKeyStateChange(playerid, newkeys, oldkeys);
#endif

public SYS_USED_VEH_MARKET_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == 3456)
    {
        if(response)
...
#if defined _ALS_SYS_USED_VEH_MARKET_OnDialogResponse
#undef SYS_USED_VEH_MARKET_OnDialogResponse
#else
#define _ALS_SYS_USED_VEH_MARKET_OnDialogResponse
#endif
#define SYS_USED_VEH_MARKET_OnDialogResponse by_OnDialogResponse
#if defined by_OnDialogResponse
...
stock CreateUsedMarket()
{
    for(new c;c < 8;c++)
    {
        ...
        Create3DTextLabel("{FFFF00}\n\n{FFFFFF} {FF0000}''{FFFFFF}   ", -1, coord_veh_enter_used_market[c][0], coord_veh_enter_used_market[c][1], coord_veh_enter_used_market[c][2] + 1.7, 20.0, 0);
    }
    ...
    for(new c;c < 8;c++)
    {
        ...
        CreateDynamic3DTextLabel("{FFFF00}\n\n{FFFFFF} {FF0000}''{FFFFFF}   ", -1, pos_exit_veh_us_mrkt_to_int[c][0], pos_exit_veh_us_mrkt_to_int[c][1], pos_exit_veh_us_mrkt_to_int[c][2] + 1.7, 20.0);
    }
    for(new c;c < 2;c++)
    {
    ...
    Create3DTextLabel("{FFFF00}\n\n{FFFFFF}   ", -1, 1980.876953,-545.951843,12.500099 + 0.5, 10.0, 0);
