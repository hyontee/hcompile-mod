...
//     
//                                  
//B  --         SetVehiclerMass                 --  (100...50000)           
//C  --         SetVehiclerMassTurn             --   
//J  --         SetVehicleTractionCoef          --     (0.5...2.0)
//K  --         SetVehicleTractionLoss          --   (  0.5...2.0)
//L  --         SetVehicleTractionBias          --   (0.1...0.6)
//M  --         SetVehicleGear                  ---   ( 5)
//N  --         SetVehicleMaxSpeed              -  
//O  --         SetVehicleAcceleration            ()
//P  --         SetVehicleEngInert                 (   0.0...250.0)
//S  --         SetVehicleBrakeCoef             -   (1.0...20.0)
//f  --         SetVehicleSuspensionBias        -- 
//e  --         SetVehicleSuspinsionLower       -- 

new setting_vehicle[210][STRUCT_VEHICLE] = 
{ 
    //  B         C       J      K     L        M   N      O     P          S             e    f   		
    {1700.0 ,   5008.0 ,  0.75, 0.85, 0.5 , 	5, 280.0, 18.7, 35.0 ,	    6.2  ,  	-0.14, 0.5 },   //400   == id  
...
cmd:stage(playerid)
{
    ...
    if(id == -1) return SendClientMessage(playerid, -1, "   .");
    ...
    { 
        ...
        {
            format(string, sizeof string, "{00FF00}");
    ...
    Dialog(playerid, 4646, DIALOG_STYLE_LIST, "", string, "", "");
...
public SYS_TUNING_STAGE_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == 4646)
    {
        if(response)
        {
            ...
            switch(listitem+1)
            {
                case 1:if(!GetOwnableCarData(id, OC_COMFORT_PL)) return SendClientMessage(playerid, -1, "     {FFFF00}Stage 1");
                case 2:if(!GetOwnableCarData(id, OC_SPORT)) return SendClientMessage(playerid, -1, "     {FFFF00}Stage 2");
                case 3:if(!GetOwnableCarData(id, OC_SPORT_PL)) return SendClientMessage(playerid, -1, "     {FFFF00}Stage 3");
                case 4:if(!GetOwnableCarData(id, OC_DRIFT)) return SendClientMessage(playerid, -1, "     {FFFF00}Drift");
            }

            if(stage_vehicle[veh] == listitem+1) 
            {           
                new string[124];
                format(string, sizeof string, "   : {FFFF00}%s", name_stage[listitem]);
    ...
    #if defined stage_SYS_TUNING_STAGE_OnDialogResponse
return stage_SYS_TUNING_STAGE_OnDialogResponse(playerid, dialogid, response, listitem, inputtext);
#endif
}
#if defined _ALS_SYS_TUNING_STAGE_OnDialogResponse
#undef SYS_TUNING_STAGE_OnDialogResponse
#else
#define _ALS_SYS_TUNING_STAGE_OnDialogResponse
#endif
#define SYS_TUNING_STAGE_OnDialogResponse stage_SYS_TUNING_STAGE_OnDialogResponse
#if defined stage_SYS_TUNING_STAGE_OnDialogResponse
forward stage_SYS_TUNING_STAGE_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]);
#endif

public SYS_TUNING_STAGE_OnGameModeInit()
{
    print("[W_SYSTEM]   .");
    ...
    CreateDynamic3DTextLabel("{f6fa05} \n{ffffff}: {e81717}'' {ffffff}  ", -1, -426.235290,1005.412658,12.307508, 9.0);
    CreateDynamic3DTextLabel("{f6fa05} \n{ffffff}: {e81717}'' {ffffff}  ", -1, -420.054534,1005.409240,12.307003, 9.0);
    CreateDynamic3DTextLabel("{f6fa05} \n{ffffff}: {e81717}'' {ffffff}  ", -1, -413.549285,1005.543029,12.326748, 9.0);

    #if defined stage_SYS_TUNING_STAGE_OnGameModeInit
        return stage_SYS_TUNING_STAGE_OnGameModeInit();
    #else
        return 1;
    #endif
}
   #if defined _ALS_SYS_TUNING_STAGE_OnGameModeInit
    #undef SYS_TUNING_STAGE_OnGameModeInit
#else
    #define _ALS_SYS_TUNING_STAGE_OnGameModeInit
#endif
#define SYS_TUNING_STAGE_OnGameModeInit stage_SYS_TUNING_STAGE_OnGameModeInit
#if defined stage_SYS_TUNING_STAGE_OnGameModeInit
    forward stage_SYS_TUNING_STAGE_OnGameModeInit();
#endif


public SYS_TUNING_STAGE_OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
    if(GetVehicleData(vehicleid, V_ACTION_TYPE) == VEHICLE_ACTION_TYPE_OWNABLE_CAR)
    {
        new idx = GetVehicleData(vehicleid, V_ACTION_ID), string[158] = "    :{FFFF00}";
        ...
        if(GetOwnableCarData(idx, OC_COMFORT_PL) || GetOwnableCarData(idx, OC_SPORT) ||GetOwnableCarData(idx, OC_SPORT_PL) || GetOwnableCarData(idx, OC_DRIFT))
        {
            ...
            SendClientMessage(playerid, -1, " /stage   ");
            SendClientMessage(playerid, -1, string);
        }
    }


    #if defined stage_SYS_TUNING_STAGE_OnPlayerEnterVehicle
        return stage_SYS_TUNING_STAGE_OnPlayerEnterVehicle(playerid, vehicleid, ispassenger);
    #else
        return 1;
    #endif
}
   #if defined _ALS_SYS_TUNING_STAGE_OnPlayerEnterVehicle
    #undef SYS_TUNING_STAGE_OnPlayerEnterVehicle
#else
    #define _ALS_SYS_TUNING_STAGE_OnPlayerEnterVehicle
#endif
#define SYS_TUNING_STAGE_OnPlayerEnterVehicle stage_SYS_TUNING_STAGE_OnPlayerEnterVehicle
#if defined stage_SYS_TUNING_STAGE_OnPlayerEnterVehicle
    forward stage_SYS_TUNING_STAGE_OnPlayerEnterVehicle(playerid, vehicleid, ispassenger);
#endif
...
public SYS_TUNING_STAGE_OnPlayerConnect(playerid)
{
    CreatePlayerTextDrawStage(playerid);
    #if defined stage_SYS_TUNING_STAGE_OnPlayerConnect
        return stage_SYS_TUNING_STAGE_OnPlayerConnect(playerid);
    #else
        return 1;
    #endif
}
#if defined _ALS_SYS_TUNING_STAGE_OnPlayerConnect
    #undef SYS_TUNING_STAGE_OnPlayerConnect
#else
    #define _ALS_SYS_TUNING_STAGE_OnPlayerConnect
#endif
#define SYS_TUNING_STAGE_OnPlayerConnect stage_SYS_TUNING_STAGE_OnPlayerConnect
#if defined stage_SYS_TUNING_STAGE_OnPlayerConnect
    forward stage_SYS_TUNING_STAGE_OnPlayerConnect(playerid);
#endif

public SYS_TUNING_STAGE_OnPlayerClickTextDraw(playerid, Text:clickedid)
{
    ...
    if(clickedid == stg_TD[3])
    {
        ...
        if(GetPlayerOwnableCar(playerid) != veh) return SendClientMessage(playerid, -1, "      .");
        ...
        switch(player_select_stage[playerid])
        {
            ...
            case 1:
            {
                if(GetOwnableCarData(id, OC_COMFORT_PL)) return SendClientMessage(playerid, -1, "    Stage 1");
            ...
            case 2:
            {
                if(GetOwnableCarData(id, OC_SPORT)) return SendClientMessage(playerid, -1, "    Stage 2");
            ...
            case 3:
            {
                if(GetOwnableCarData(id, OC_SPORT_PL)) return SendClientMessage(playerid, -1, "    Stage 3");
            ...
            case 4:
            {
                if(GetOwnableCarData(id, OC_DRIFT)) return SendClientMessage(playerid, -1, "    Drift");
        ...
        if(1 <= player_select_stage[playerid] <= 4) 
        {
            ...
            if(!mysql_errno())
            {
                ...
                format(string, sizeof string, "  {FFFF00}%s{FFFFFF}  {FFFF00}%d {FFFFFF}.   !", 
    ...
    #if defined stage_SYS_TUNING_STAGE_OnPlayerClickTextDraw
        return stage_SYS_TUNING_STAGE_OnPlayerClickTextDraw(playerid, clickedid);
    #else
        return 1;
    #endif
}
#if defined _ALS_SYS_TUNING_STAGE_OnPlayerClickTextDraw
    #undef SYS_TUNING_STAGE_OnPlayerClickTextDraw
#else
    #define _ALS_SYS_TUNING_STAGE_OnPlayerClickTextDraw
#endif
#define SYS_TUNING_STAGE_OnPlayerClickTextDraw stage_SYS_TUNING_STAGE_OnPlayerClickTextDraw
#if defined stage_SYS_TUNING_STAGE_OnPlayerClickTextDraw
    forward stage_SYS_TUNING_STAGE_OnPlayerClickTextDraw(playerid, Text:clickedid);
#endif

public SYS_TUNING_STAGE_OnPlayerClickPlayerTextDraw(playerid, PlayerText:playertextid)
{
    ...
    #undef SYS_TUNING_STAGE_OnPlayerClickPlayerTextDraw
#else
...
#endif
#define	SYS_TUNING_STAGE_OnPlayerClickPlayerTextDraw stage_OnPlayerClickPlayerTextD
...
stock ShowPanelStage(playerid, panel)
{
    ...
    format(rubl, sizeof rubl, "%d_", percent);
...
/*stock ShowPanelStage(playerid, panel)
{
    ...
    player_price_stage[playerid] = percent;
    new string[18];
    format(string, sizeof string, "%d_", percent);
...
{
	if(!strlen(params))
		return SendClientMessage(playerid, 0xCECECEFF, ": /carpass [ID-]");

    if(GetPlayerVehicleID(playerid) != GetPlayerOwnableCar(playerid)) return SendClientMessage(playerid, -1, "     ");

    extract params -> new to_player;

    new string[124];
    format(string, sizeof string, "{0091FF} %s   .  ", GetPlayerNameEx(playerid));
    SendClientMessage(to_player, -1, string);

    format(string, sizeof string, "{0091FF}  %s  .  ", GetPlayerNameEx(playerid));
    SendClientMessage(playerid, -1, string);

    ShowNotification(to_player, 4, " .", 5, "/showcar_p_ass", ">>");
...
cmd:showcar_p_ass(playerid)
{
    ...
    if(GetPlayerVehicleID(to_player) != GetPlayerOwnableCar(to_player)) return SendClientMessage(playerid, -1, "     ");
    ...
    "{FFFFFF}: \t\t\t\t\t{FFFF00}%d .\n"\
    "{FFFFFF}.: \t\t\t\t\t{FFFF00}%d .\n"\
    "{FFFFFF} Stage 1: \t\t\t\t\t{FFFF00}%s\n"\
    "{FFFFFF} Stage 2: \t\t\t\t\t{FFFF00}%s\n"\
    "{FFFFFF} Stage 3: \t\t\t\t\t{FFFF00}%s\n", 
    ...
    GetOwnableCarData(idx, OC_COMFORT_PL) ? ("") : (""),
    GetOwnableCarData(idx, OC_SPORT) ? ("") : (""),
    GetOwnableCarData(idx, OC_SPORT_PL) ? ("") : ("")
    );

    Dialog(playerid, -1, DIALOG_STYLE_LIST, "{FFD900}. ", string, "", "");
