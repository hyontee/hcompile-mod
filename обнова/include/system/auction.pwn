...
stock AuctionManager_GetTodayFinishTime()
{
    ...
    if(hour >= 23)
        return gettime() + ((47 - hour) * 3600) - (minute * 60) - second;
    return gettime() + ((23 - hour) * 3600) - (minute * 60) - second;
}

stock AuctionBusiness_IsCasino(businessid)
{
    return 0 <= businessid < g_business_loaded
        && GetBusinessData(businessid, B_SQL_ID) > 0
        && 1 <= GetBusinessData(businessid, B_TYPE) <= BUSINESS_TYPE_ACCESSORY_SHOP;
}
...
stock BusinessManager_GetTypeName(businessid, output[], size)
{
    switch(GetBusinessData(businessid, B_TYPE))
    {
        ...
        case BUSINESS_TYPE_CAR_MARKET: format(output, size, "Автосалон");
...
stock AuctionBusiness_FormatLotName(source, server_id, output[], size)
{
    ...
    if(!AuctionBusiness_IsCasino(server_id))
...
{
    if(!AuctionBusiness_IsCasino(businessid))
...
{
    if(!AuctionBusiness_IsCasino(businessid)) return -1;
            ...
            if(GetPlayerMoneyEx(playerid) >= rate)
            {
                ...
                if(auction_slot[id][AUCTION_DB_ID] > 0)
                {
                    ...
                    mysql_format(mysql, bid_query, sizeof bid_query,
                        "UPDATE auction_lots SET current_price=%d,highest_bidder_account_id=%d,highest_bidder_name='%e' WHERE id=%d AND status='active'",
                        rate, GetPlayerAccountID(playerid), GetPlayerNameEx(playerid), auction_slot[id][AUCTION_DB_ID]);
...
stock finish_auction_slot(slot)
{
    ...
    {
        if(auction_slot[slot][SLOT_TYPE] == TYPE_BUSINESS && auction_slot[slot][AUCTION_DB_ID] > 0)
        {
    ...
    if(auction_slot[slot][SLOT_TYPE] == TYPE_BUSINESS)
    {
        ...
        {
            item_valid = AuctionBusiness_IsCasino(businessid)
        ...
        {
            if(GetPlayerAccountID(i) == auction_slot[slot][OWNER_RATE] && GetPlayerBusiness(i) != -1)
            {
                AuctionRefundBid(slot, "У вас уже есть бизнес, ставка возвращена.");
                if(auction_slot[slot][AUCTION_DB_ID] > 0)
                {
                    new limit_query[256];
                    mysql_format(mysql, limit_query, sizeof limit_query,
                        "UPDATE auction_lots SET status='cancelled_buyer_limit' WHERE id=%d",
                        auction_slot[slot][AUCTION_DB_ID]);
                    mysql_query(mysql, limit_query, false);
                }
                auction_slot_busy[slot] = false;
                reset_slot_auction(slot);
                return 1;
        ...
        if(auction_slot[slot][BUSINESS_SOURCE_SLOT] == AUCTION_BUSINESS_SOURCE_FUEL)
            ...
            transfer_result = AuctionTransferCasino(businessid, auction_slot[slot][DATABASE_ID],
                auction_slot[slot][OWNER_SLOT_SQL], auction_slot[slot][OWNER_RATE]);
        if(!transfer_result)
        {
...
stock reset_slot_auction(slot)
{
    ...
    new bool:update_business_label =
        auction_slot[slot][SLOT_TYPE] == TYPE_BUSINESS && AuctionBusiness_IsCasino(businessid);
...
stock AuctionManager_InitDatabase()
{
    ...
    mysql_query(mysql, "ALTER TABLE business ADD COLUMN ownership_started_at INT NOT NULL DEFAULT 0", false);
    return 1;
}
...
stock AuctionRefundBid(slot, message[])
{
    ...
    AuctionGiveAccountMoney(accountid, amount, "Возврат ставки аукциона");
    foreach(new i : Player)
    {
...
stock AuctionTransferCasino(businessid, business_sql_id, seller_account_id, buyer_account_id)
{
    if(!AuctionBusiness_IsCasino(businessid)
    ...
    mysql_format(mysql, transfer_query, sizeof transfer_query,
        "UPDATE business SET owner_id=%d,ownership_started_at=%d WHERE id=%d AND owner_id=%d",
        buyer_account_id, gettime(), business_sql_id, seller_account_id);
    ...
    {
        mysql_format(mysql, transfer_query, sizeof transfer_query,
            "UPDATE accounts SET business=-1 WHERE id=%d", seller_account_id);
        mysql_query(mysql, transfer_query, false);
        if(mysql_errno())
        {
            mysql_query(mysql, "ROLLBACK", false);
...
stock AuctionTransferFuelStation(stationid, station_sql_id, seller_account_id, buyer_account_id)
{
    ...
    mysql_query(mysql, query, false);
    if(mysql_errno()) return mysql_query(mysql, "ROLLBACK", false), 0;
...
stock AuctionManager_RestoreCasinoLots()
{
    new query[256], Cache:result;
    mysql_format(mysql, query, sizeof query,
        "SELECT * FROM auction_lots WHERE item_type=%d AND status IN ('active','processing') ORDER BY id ASC",
        TYPE_BUSINESS);
    result = mysql_query(mysql, query, true);
    if(mysql_errno())
    {
        cache_delete(result);
        return print("[AUCTION] Failed to restore casino lots");
    }

    new rows = cache_num_rows();
    for(new row; row < rows; row++)
    {
        new business_sql_id = cache_get_field_content_int(row, "item_database_id");
        new stored_type = cache_get_field_content_int(row, "business_type");
        new business_source = stored_type == 1000
            ? AUCTION_BUSINESS_SOURCE_FUEL
            : AUCTION_BUSINESS_SOURCE_REGULAR;
        new businessid = -1;
        if(business_source == AUCTION_BUSINESS_SOURCE_FUEL)
        {
            for(new stationid; stationid < g_fuel_station_loaded; stationid++)
            {
                if(GetFuelStationData(stationid, FS_SQL_ID) == business_sql_id)
                {
        ...
        else
        {
            ...
            {
                if(GetBusinessData(b, B_SQL_ID) == business_sql_id)
                {
                    businessid = b;
                    break;
                }
        ...
        if(businessid == -1) continue;
        if(business_source == AUCTION_BUSINESS_SOURCE_REGULAR && IsBusinessOnAuction(businessid)) continue;
        new slot = -1;
        for(new s; s < MAX_AUCTION_SLOT; s++)
        {
            if(!auction_slot[s][SLOT_TYPE])
        ...
        auction_slot[slot][DATABASE_ID] = business_sql_id;
        ...
        auction_slot[slot][BUSINESS_TYPE_SLOT] = stored_type;
        ...
        AuctionBusiness_FormatLotName(business_source,
        ...
        if(business_source == AUCTION_BUSINESS_SOURCE_FUEL)
            CallLocalFunction("UpdateFuelStationLabel", "i", businessid);
        else
            CallLocalFunction("UpdateBusinessLabel", "i", businessid);
    }
    cache_delete(result);
