...
stock Music_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
        ...
        switch(listitem)
        {
          ...
          case 2:
          {
             ...
             else
             {
              ...
              ShowNotificationNew(playerid, 2, 6, 0, 0, "Музыка выключена", "");
...
stock Music_OnPlayerExitVehicle(playerid, vehicleid)
{
...
stock Music_OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
    if(!ispassenger)
	{
...
public: InCarTimer(playerid)
{
    if(IsPlayerInAnyVehicle(playerid))
    {
       ShowNotificationNew(playerid, 6, 6, 0, 0, "Двигатель автомобиля выключен", "Включить");
    }
    else
    {
...
stock Music_OnPlayerDisconnect(playerid, reason)
{
   if(musicOn[playerid] > 0)
   {
