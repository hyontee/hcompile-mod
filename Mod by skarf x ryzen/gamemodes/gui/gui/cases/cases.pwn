...
stock Cases_AddToPlayer(playerid, caseIdx, amount)
{
    ...
    if(amount <= 0) return 0;
    SetPlayerCaseCountByType(playerid, CaseData[caseIdx][cId], amount);
    return 1;
}
...
stock Cases_OnPlayerConnect(playerid)
{
	...
	for(new i = 0; i < MAX_CASES; i++) {
		SetPlayerCaseCountByType(playerid, i + 1, 0);
...
stock Cases_SavePlayer(playerid)
{
    ...
    for(new i = 0; i < MAX_CASES; i++)
    {
        if(i > 0) strcat(ccStr, ",");
        new tmp[16];
        format(tmp, sizeof(tmp), "%d", GetPlayerCaseCountByType(playerid, i + 1));
        strcat(ccStr, tmp);
    }
    ...
    if(cache_num_rows())
    {
        ...
        if(strlen(ccStr) > 0)
        {
            ...
            while(pos < len && idx < MAX_CASES)
            {
                ...
                SetPlayerCaseCountByType(playerid, idx++ + 1, strval(tmp));
                pos = end + 1;
            }
        }
