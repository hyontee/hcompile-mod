...
new player_rented_vehicle[MAX_PLAYERS] = {INVALID_VEHICLE_ID, ...}; // Новая переменная
...
public OnPlayerDisconnect(playerid, reason)
{
    ...
    // Удаляем арендованный транспорт игрока
    if(player_rented_vehicle[playerid] != INVALID_VEHICLE_ID)
    {
        DestroyVehicle(player_rented_vehicle[playerid]);
        player_rented_vehicle[playerid] = INVALID_VEHICLE_ID;
    }
...
// НОВЫЙ: Обработчик выхода из ТС
public OnPlayerStateChange(playerid, newstate, oldstate)
{
    if(oldstate == PLAYER_STATE_DRIVER && player_rented_vehicle[playerid] != INVALID_VEHICLE_ID)
    {
        new vehicleid = GetPlayerVehicleID(playerid);
        if(vehicleid == player_rented_vehicle[playerid])
        {
            DestroyVehicle(player_rented_vehicle[playerid]);
            player_rented_vehicle[playerid] = INVALID_VEHICLE_ID;
            Send(playerid, -1, ""we_c"| Арендованный транспорт был удалён.");
        }
    }
    #if defined wed_OnPlayerStateChange
        return wed_OnPlayerStateChange(playerid, newstate, oldstate);
    #else
        return 1;
    #endif
}
...
#endif
#define OnPlayerStateChange wed_OnPlayerStateChange
#if defined wed_OnPlayerStateChange
    forward wed_OnPlayerStateChange(playerid, newstate, oldstate);
#endif
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    // ИЗМЕНЕННЫЙ диалог аренды
    if(dialogid == 1404)
    {
        if(response)
        {
            // Проверяем, есть ли у игрока уже арендованный транспорт
            if(player_rented_vehicle[playerid] != INVALID_VEHICLE_ID)
            {
                DestroyVehicle(player_rented_vehicle[playerid]);
                player_rented_vehicle[playerid] = INVALID_VEHICLE_ID;
                Send(playerid, -1, ""we_c"| Ваш предыдущий арендованный транспорт был удалён.");
            }
            ...
            switch(listitem)
            {
                case 0:
                {
                    if(GetPlayerMoneyEx(playerid) < 100000) 
                        return Send(playerid, -1, ""USC" Недостаточно средств. Нужно 100.000 рублей");
                    vehicleid = CreateVehicle(409, x, y, z, 0.0, 0, 0, -1);
                    price = 100000;
                }
                case 1:
                {
                    if(GetPlayerMoneyEx(playerid) < 50000) 
                        return Send(playerid, -1, ""USC" Недостаточно средств. Нужно 50.000 рублей");
                    vehicleid = CreateVehicle(411, x, y, z, 0.0, 0, 0, -1);
                    price = 50000;
                }
                case 2:
                {
                    if(GetPlayerMoneyEx(playerid) < 60000) 
                        return Send(playerid, -1, ""USC" Недостаточно средств. Нужно 60.000 рублей");
                    vehicleid = CreateVehicle(517, x, y, z, 0.0, 0, 0, -1);
                    price = 60000;
                }
            }
            ...
            {
                player_rented_vehicle[playerid] = vehicleid;
                GivePlayerMoneyEx(playerid, -price);
                PutPlayerInVehicle(playerid, vehicleid, 0);
                Send(playerid, -1, ""SC" Вы успешно арендовали транспорт! Выход из него удалит его.");
            }
            return 1;
        }
    }
    if(dialogid == 1405)
    {
        if(response)
        {
            ...
            sscanf(GetPlayerData(playerid, P_NAME),"p<_>s[10]s[10]", name, surname);

            sscanf(GetPlayerData(to_player, P_NAME),"p<_>s[10]s[10]", to_name, to_surname);
