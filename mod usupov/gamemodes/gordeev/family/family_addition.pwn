...
#define START_OBJECT_3  45 //Предприятие Бусаево (время указано в минутах часа 00:XX)
...
new CAPTURE_PANEL_TYPE[MAX_PLAYERS];
...
public OnGameModeInit()
{
    ...
    CAPTURE_3DTEXT[0] = Create3DTextLabel("{1E90FF}«Сотовоя вышка №1»\n{FFFFFF}Контролирует семья", -1, text_3d_capt[0][0], text_3d_capt[0][1], text_3d_capt[0][2]+1.0, 10.0, 0, 1);
    CAPTURE_3DTEXT[3] = Create3DTextLabel("{1E90FF}«Сотовоя вышка №2»\n{FFFFFF}Контролирует семья", -1, text_3d_capt[3][0], text_3d_capt[3][1], text_3d_capt[3][2]+1.0, 10.0, 0, 1);
    CAPTURE_3DTEXT[4] = Create3DTextLabel("{1E90FF}«Сотовоя вышка №3»\n{FFFFFF}Контролирует семья", -1, text_3d_capt[4][0], text_3d_capt[4][1], text_3d_capt[4][2]+1.0, 10.0, 0, 1);
    CAPTURE_3DTEXT[1] = Create3DTextLabel("{1E90FF}«Предприятие Ферма Гарель»\n{FFFFFF}Контролирует семья", -1, text_3d_capt[1][0], text_3d_capt[1][1], text_3d_capt[1][2]+1.0, 10.0, 0, 1);
    CAPTURE_3DTEXT[2] = Create3DTextLabel("{1E90FF}«Предприятие Бусаево»\n{FFFFFF}Контролирует семья", -1, text_3d_capt[2][0], text_3d_capt[2][1], text_3d_capt[2][2]+1.0, 10.0, 0, 1);
    SetTimer("FamilyTimer", 60*1000, true);

    for(new i = 0; i < MAX_FAM; i++)
    {
        for(new j = 0; j < 5; j++)
        {
            fam_PTD[i][j] = Text:INVALID_TEXT_DRAW;
        }
    }

    for(new i = 0; i < MAX_PLAYERS; i++)
    {
        CAPTURE_PANEL_TYPE[i] = -1;
    }
...
{
    new type_capt = GetCaptureTypeByArea(areaid);
    ...
    if(type_capt != -1)
    {
        ...
        if(f == -1) 
        {
            printf("[DEBUG] Игрок %d вошёл в зону захвата без семьи (f=-1)", playerid);
        ...
        if(f < 0 || f >= MAX_FAM)
        {
            printf("[ERROR] OnPlayerEnterDynamicArea: выход за границы! f:%d (MAX:%d)", f, MAX_FAM);
            #if defined capt_OnPlayerEnterDynamicArea
                return capt_OnPlayerEnterDynamicArea(playerid, STREAMER_TAG_AREA:areaid);
            #else
                return 0;
        ...
        if(!show_panel_capt[playerid]) 
            ShowFamilyCaptPanel(playerid, type_capt);
    } 
...
stock GetCaptureTypeByArea(areaid)
{
...
public OnPlayerLeaveDynamicArea(playerid, areaid)
{
    ...
    {
        new type_capt = -1, f = GetPlayerIdFamily(playerid);
        
        printf("[DEBUG] OnPlayerLeaveDynamicArea | Player: %d | Area: %d | Family: %d", playerid, areaid, f);
...
stock StartFCapture(type_capt)
{
    ...
    for(new i;i < welsi;i++)
    {
        ...
        format(string, sizeof string, "{1E90FF}%s\n{FFFFFF}Контролирует семья", name_object[e[i]]);
        ...
        format(string, sizeof string, "{1E90FF}|{FFFFFF} Началось сражение за {1E90FF}%s{FFFFFF}, найдите место проведение с помощью — карты", name_object[e[i]]);
...
{
    if(CAPTURE_PANEL_TYPE[playerid] != -1)
    {
        HideFamilyCaptPanel(playerid, CAPTURE_PANEL_TYPE[playerid]);
    }
...
{
    if(GetPlayerIdFamily(playerid) != -1 && (CAPTURE_STATUS[0] || CAPTURE_STATUS[1] || CAPTURE_STATUS[2] || CAPTURE_STATUS[3] || CAPTURE_STATUS[4]))
    {
        if(CAPTURE_PLAYER_EXP[playerid] != 0)
        {
            new capt = GetPlayerCaptArea(playerid);
...
{
    new f = GetPlayerIdFamily(playerid);
    if(f == -1 || t < 0 || t >= sizeof(fam_TD)) return 0;

    show_panel_capt[playerid] = true;
    CAPTURE_PANEL_TYPE[playerid] = t;

    TextDrawExpFamily(f);
    for(new i = 0; i < 9; i++) TextDrawShowForPlayer(playerid, fam_TD[t][i]);
    if(fam_PTD[f][t] != Text:INVALID_TEXT_DRAW)
    {
...
{
    new f = GetPlayerIdFamily(playerid);
    if(t < 0 || t >= sizeof(fam_TD)) return 0;

    show_panel_capt[playerid] = false;
    if(CAPTURE_PANEL_TYPE[playerid] == t) CAPTURE_PANEL_TYPE[playerid] = -1;

    for(new i = 0; i < 9; i++) TextDrawHideForPlayer(playerid, fam_TD[t][i]);
    if(f != -1 && fam_PTD[f][t] != Text:INVALID_TEXT_DRAW)
    {
...
cmd:endfevent(playerid, params[])
{
    ...
    if(CAPTURE_STATUS[3]) EndFCapture(3);
    if(CAPTURE_STATUS[4]) EndFCapture(4);
...
{
    if(type < 0 || type >= sizeof(CAPTURE_STATUS)) return 0;

    if(CAPTURE_TIMER[type] != -1)
    {
    ...
    {
        if(CAPTURE_PANEL_TYPE[i] == type) HideFamilyCaptPanel(i, type);

        if(GetPlayerIdFamily(i) == -1) continue;

        if(CAPTURE_GANG_ZONE[type] != -1)
        {
            GangZoneStopFlashForPlayer(i, CAPTURE_GANG_ZONE[type]);
            GangZoneHideForPlayer(i, CAPTURE_GANG_ZONE[type]);
        }

        if(IsPlayerInDynamicArea(i, CAPTURE_AREA[type])) CAPTURE_PLAYER_EXP[i] = 0;
    }

    new string[245], f = CAPTURE_LEADER[type][0];

    if(f >= 0 && f < MAX_FAM && CAPTURE_LEADER[type][1] > 0) 
    {
        ...
        format(string, sizeof string, "{1E90FF}%s\n{FFFFFF}Контролирует семья\n%s", name_object[type], GetFamily(f, family_name));
        Update3DTextLabelText(CAPTURE_3DTEXT[type], -1, string);

    }

    if(IsValidDynamicArea(CAPTURE_AREA[type])) DestroyDynamicArea(CAPTURE_AREA[type]);
    CAPTURE_AREA[type] = -1;

    if(CAPTURE_GANG_ZONE[type] != -1) GangZoneDestroy(CAPTURE_GANG_ZONE[type]);
    CAPTURE_GANG_ZONE[type] = -1;

    CAPTURE_SECOND[type] = 0;
    CAPTURE_STATUS[type] = false;
    
    for(new i = 0; i < MAX_FAM; i++) CAPTURE_FAMILY_EXP[i][type] = 0;
...
public:CaptureUpd(type)
{
    ...
    foreach(new i : Player)
    {
        if(GetPlayerIdFamily(i) == -1 && CAPTURE_PANEL_TYPE[i] == type) HideFamilyCaptPanel(i, type);
    ...
    for(new i = 0; i < MAX_FAM; i++)
    {
        ...
        if(fam_PTD[i][type] == Text:INVALID_TEXT_DRAW)
        {
            TextDrawExpFamily(i);
        }
...
stock TextDrawExpFamily(fam)
{
    ...
    if(fam_PTD[fam][0] != Text:INVALID_TEXT_DRAW) return 1;
...
cmd:startfevent(playerid, params[])
{
    ...
    if(!t) return SendClientMessage(playerid, -1, "1 - Вышки / 2 - Ферма Гарель / 3 - Предприятие Бусаево / 4 - Семейные контейнеры");
...
cmd:testtower10(playerid, params[])
{
    if(GetPlayerAdminEx(playerid) < 6) return 1;

    if(CAPTURE_STATUS[0] || CAPTURE_STATUS[3] || CAPTURE_STATUS[4])
        return SendClientMessage(playerid, -1, "Сражение за вышки уже запущено.");
    ...
    format(stringcapture, sizeof stringcapture, "%d:%02d", ConvertUnixTime(CAPTURE_SECOND[0], CONVERT_TIME_TO_MINUTES), ConvertUnixTime(CAPTURE_SECOND[0], CONVERT_TIME_TO_SECONDS));
    TextDrawSetString(fam_TD[0][8], stringcapture);
    TextDrawSetString(fam_TD[3][8], stringcapture);
    TextDrawSetString(fam_TD[4][8], stringcapture);

    SendClientMessage(playerid, -1, "{66CC66}Тестовое сражение за вышки запущено на 10 секунд.");
    return 1;
}
...
stock EndFCont()
{
    for(new i;i < 2;i++)
    {
        ...
        if(FAM_CONT_OBJECT[i] != -1) DestroyObject(FAM_CONT_OBJECT[i]);
        FAM_CONT_OBJECT[i] = -1;

        if(IsValidDynamicArea(FAM_CONT_AREA[i])) DestroyDynamicArea(FAM_CONT_AREA[i]);
        FAM_CONT_AREA[i] = -1;

        if(IsValidDynamicArea(FAM_CONT_AREA_P[i])) DestroyDynamicArea(FAM_CONT_AREA_P[i]);
        FAM_CONT_AREA_P[i] = -1;

        if(FAM_CONT_GANG_ZONE[i] != -1) GangZoneDestroy(FAM_CONT_GANG_ZONE[i]);
        FAM_CONT_GANG_ZONE[i] = -1;

        FAM_CONT_BOX[i] = 0;
        FAM_CONT_SECOND[i] = 0;

        if(FAM_CONT_3DTEXT[i] != Text3D:-1) Delete3DTextLabel(FAM_CONT_3DTEXT[i]);
        FAM_CONT_3DTEXT[i] = Text3D:-1;

        if(FAM_CONT_PICKUP[i] != -1) DestroyDynamicPickup(FAM_CONT_PICKUP[i]);
        FAM_CONT_PICKUP[i] = -1;
    }
    return 1;
}
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
    if(GetVehicleData(vehicleid, V_ACTION_TYPE) == VEHICLE_ACTION_TYPE_FAMILY_CAR)
    {
        ...
        if(GetCarFamily(id, V_F_STATUS_CONT))
        {
            ...
            if(IsValidDynamic3DTextLabel(GetCarFamily(id, V_F_TEXT_CONT))) DestroyDynamic3DTextLabel(GetCarFamily(id, V_F_TEXT_CONT));
            SetCarFamily(id, V_F_AREA_CONT, -1);
            SetCarFamily(id, V_F_TEXT_CONT, Text3D:-1);
            SetCarFamily(id, V_F_STATUS_CONT, false);
        } 
    }
