...
new PromoLevelPrizes_3[2][3] = {
    {3, 5, 0},
    {0, 30000, 0}
};

new PromoLevelPrizes_4[2][3] = {
    {3, 5, 0},
    {0, 45000, 0}
};

new PromoLevelPrizes_5[2][3] = {
    {3, 7, 0},
    {0, 50000, 0}
};

new PromoLevelPrizes_6[2][3] = {
    {3, 10, 0},
    {0, 55000, 0}
};

new PromoLevelPrizes_7[2][3] = {
    {3, 10, 0},
    {0, 55000, 0}
};

new PromoLevelPrizes_8[2][3] = {
    {3, 12, 0},
    {0, 62000, 0}
};

new PromoLevelPrizes_9[2][3] = {
    {3, 14, 0},
    {0, 65000, 0}
};

new PromoLevelPrizes_10[2][3] = {
    {3, 20, 0},
    {0, 75000, 0}
};
...
#define DIALOG_DEVPROMO_MENU            12365 // Главное меню создания dev-промокода
#define DIALOG_DEVPROMO_SET_NAME        12366 // Ввод названия dev-промокода
#define DIALOG_DEVPROMO_SET_MONEY       12367 // Ввод виртов для dev-промокода
#define DIALOG_DEVPROMO_SET_DONATE      12368 // Ввод доната для dev-промокода
#define DIALOG_DEVPROMO_SET_EXP         12369 // Ввод EXP для dev-промокода
#define DIALOG_DEVPROMO_CONFIRM_CREATE  12370 // Подтверждение создания dev-промокода
#define DIALOG_DEVPROMO_ACTIVATE_INPUT  12371 // Ввод dev-промокода для активации
#define DIALOG_DEVPROMO_ACTIVATE_CONFIRM 12372 // Подтверждение активации dev-промокода
#define DIALOG_DEVPROMO_RESULT          12373 // Результат активации dev-промокода
...
enum E_DEV_PROMO_DATA
{
    DevPromoSQLID,
    String:DevPromoCode[MAX_PROMO_CODE_LENGTH],
    DevPromoMoney,
    DevPromoDonate,
    DevPromoExp
}
new PlayerDevPromoDraft[MAX_PLAYERS][E_DEV_PROMO_DATA];
new PlayerDevPromoPending[MAX_PLAYERS][E_DEV_PROMO_DATA];
...
stock GetPromoCodeDataByID(promoId, promoData[]);
stock GetPromoCodeDataByString(const promoCode[], promoData[]);
stock GetPromoCodeIndexByString(const promoCode[]);
stock GivePromoCar(playerid, modelid, promoSqlId);
stock ResetPlayerDevPromoDraft(playerid);
stock ResetPlayerDevPromoPending(playerid);
stock ShowDevPromoBuilderDialog(playerid);
stock ShowDevPromoSetAmountDialog(playerid, dialogid, title[], description[]);
stock IsUnsignedNumber(const text[]);
stock BuildDevPromoRewardText(dest[], size, money, donate, exp);
stock ShowDevPromoActivationDialog(playerid, const promoCode[]);
stock ActivatePendingDevPromo(playerid);
...
public CheckAndCreatePromoTables()
{
    ...
    mysql_format(mysql, query, sizeof(query), "SELECT * FROM `dev_promocodes` LIMIT 1");
    cache = mysql_query(mysql, query, true);
    if(mysql_errno() != 0)
    {
        printf("[WERTON_PROMO] Таблица 'dev_promocodes' не найдена. Создаем новую структуру...");
        mysql_format(mysql, query, sizeof(query), "CREATE TABLE IF NOT EXISTS `dev_promocodes` (`id` INT NOT NULL AUTO_INCREMENT, `code` VARCHAR(%d) NOT NULL UNIQUE, `money` INT NOT NULL DEFAULT 0, `donate` INT NOT NULL DEFAULT 0, `exp` INT NOT NULL DEFAULT 0, `created_by` INT NOT NULL DEFAULT 0, `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP, PRIMARY KEY (`id`)) ENGINE=InnoDB", MAX_PROMO_CODE_LENGTH);
        mysql_query(mysql, query, false);
        if(mysql_errno() != 0)
        {
            printf("[WERTON_PROMO] Ошибка при создании таблицы 'dev_promocodes': %d", mysql_errno());
        }
        else
        {
            printf("[WERTON_PROMO] Таблица 'dev_promocodes' успешно создана.");
        }
    }
    cache_delete(cache);

    mysql_format(mysql, query, sizeof(query), "SELECT * FROM `dev_promocode_activations` LIMIT 1");
    cache = mysql_query(mysql, query, true);
    if(mysql_errno() != 0)
    {
        printf("[WERTON_PROMO] Таблица 'dev_promocode_activations' не найдена. Создаем новую структуру...");
        mysql_format(mysql, query, sizeof(query), "CREATE TABLE IF NOT EXISTS `dev_promocode_activations` (`id` INT NOT NULL AUTO_INCREMENT, `account_id` INT NOT NULL, `promo_id` INT NOT NULL, `activated_at` DATETIME NOT NULL, PRIMARY KEY (`id`), UNIQUE KEY `account_promo` (`account_id`, `promo_id`), FOREIGN KEY (`promo_id`) REFERENCES `dev_promocodes`(`id`) ON DELETE CASCADE) ENGINE=InnoDB");
        mysql_query(mysql, query, false);
        if(mysql_errno() != 0)
        {
            printf("[WERTON_PROMO] Ошибка при создании таблицы 'dev_promocode_activations': %d", mysql_errno());
        }
        else
        {
            printf("[WERTON_PROMO] Таблица 'dev_promocode_activations' успешно создана.");
        }
    }
    cache_delete(cache);
}
...
public OnPlayerConnect(playerid)
{
    ...
    ResetPlayerDevPromoDraft(playerid);
    ResetPlayerDevPromoPending(playerid);
...
stock ResetPlayerDevPromoDraft(playerid)
{
    PlayerDevPromoDraft[playerid][DevPromoSQLID] = 0;
    PlayerDevPromoDraft[playerid][DevPromoCode][0] = EOS;
    PlayerDevPromoDraft[playerid][DevPromoMoney] = 0;
    PlayerDevPromoDraft[playerid][DevPromoDonate] = 0;
    PlayerDevPromoDraft[playerid][DevPromoExp] = 0;
    return 1;
}

stock ResetPlayerDevPromoPending(playerid)
{
    PlayerDevPromoPending[playerid][DevPromoSQLID] = 0;
    PlayerDevPromoPending[playerid][DevPromoCode][0] = EOS;
    PlayerDevPromoPending[playerid][DevPromoMoney] = 0;
    PlayerDevPromoPending[playerid][DevPromoDonate] = 0;
    PlayerDevPromoPending[playerid][DevPromoExp] = 0;
    return 1;
}
...
{
    new dialogText[512];
    new codeText[MAX_PROMO_CODE_LENGTH];

    if(strlen(PlayerDevPromoDraft[playerid][DevPromoCode]) == 0)
    {
        format(codeText, sizeof(codeText), "Не задан");
    }
    else
    {
        format(codeText, sizeof(codeText), "%s", PlayerDevPromoDraft[playerid][DevPromoCode]);
    }

    format(dialogText, sizeof(dialogText), "{FFFFFF}Название промокода: {FFFF00}%s\n{FFFFFF}Добавить вирты в промокод: {FFFF00}%d\n{FFFFFF}Добавить донат в промокод: {FFFF00}%d\n{FFFFFF}Добавить exp в промокод: {FFFF00}%d\n{66CC33}Создать промокод",
        codeText,
        PlayerDevPromoDraft[playerid][DevPromoMoney],
        PlayerDevPromoDraft[playerid][DevPromoDonate],
        PlayerDevPromoDraft[playerid][DevPromoExp]
    );

    ShowPlayerDialog(playerid, DIALOG_DEVPROMO_MENU, DIALOG_STYLE_LIST, "{FF6347}"SERVER_NAME"{FFFFFF} | Dev Promo", dialogText, "Выбрать", "Закрыть");
    return 1;
}

stock ShowDevPromoSetAmountDialog(playerid, dialogid, title[], description[])
{
    ShowPlayerDialog(playerid, dialogid, DIALOG_STYLE_INPUT, title, description, "Сохранить", "Назад");
    return 1;
}
...
stock BuildDevPromoRewardText(dest[], size, money, donate, exp)
{
    ...
    if(money > 0)
    {
        format(dest, size, "%s{FFFF00}- %d вирт\n", dest, money);
    }
    if(donate > 0)
    {
        format(dest, size, "%s{FFFF00}- %d доната\n", dest, donate);
    }
    if(exp > 0)
    {
        format(dest, size, "%s{FFFF00}- %d EXP\n", dest, exp);
    }

    if(strlen(dest) > 0 && dest[strlen(dest) - 1] == '\n')
    {
        dest[strlen(dest) - 1] = EOS;
    }
    return 1;
}
...
stock ShowDevPromoActivationDialog(playerid, const promoCode[])
{
    ...
    mysql_format(mysql, query, sizeof(query), "SELECT `id`, `code`, `money`, `donate`, `exp` FROM `dev_promocodes` WHERE `code` = '%e' LIMIT 1", promoCode);
    result = mysql_query(mysql, query, true);

    if(mysql_errno() != 0)
    {
        SendClientMessage(playerid, -1, "{ff2400}| {ffffff}Ошибка базы данных при проверке промокода.");
        cache_delete(result);
        return 1;
    }
    ...
    ResetPlayerDevPromoPending(playerid);
    PlayerDevPromoPending[playerid][DevPromoSQLID] = cache_get_field_content_int(0, "id");
    cache_get_field_content(0, "code", PlayerDevPromoPending[playerid][DevPromoCode], MAX_PROMO_CODE_LENGTH);
    PlayerDevPromoPending[playerid][DevPromoMoney] = cache_get_field_content_int(0, "money");
    PlayerDevPromoPending[playerid][DevPromoDonate] = cache_get_field_content_int(0, "donate");
    PlayerDevPromoPending[playerid][DevPromoExp] = cache_get_field_content_int(0, "exp");
    cache_delete(result);
    ...
    {
        SendClientMessage(playerid, -1, "{ff2400}| {ffffff}У этого промокода нет доступных наград.");
        ResetPlayerDevPromoPending(playerid);
        return 1;
    }

    mysql_format(mysql, query, sizeof(query), "SELECT 1 FROM `dev_promocode_activations` WHERE `account_id` = %d AND `promo_id` = %d LIMIT 1", accountId, PlayerDevPromoPending[playerid][DevPromoSQLID]);
    result = mysql_query(mysql, query, true);

    if(mysql_errno() != 0)
    {
        SendClientMessage(playerid, -1, "{ff2400}| {ffffff}Ошибка базы данных при проверке активации.");
        cache_delete(result);
        ResetPlayerDevPromoPending(playerid);
        return 1;
    }
    ...
    if(cache_num_rows(result) > 0)
    {
        SendClientMessage(playerid, -1, "{ff2400}| {ffffff}Вы уже активировали этот промокод.");
        cache_delete(result);
        ResetPlayerDevPromoPending(playerid);
        return 1;
    }
    ...
    BuildDevPromoRewardText
    (
        previewText,
        sizeof(previewText),
        PlayerDevPromoPending[playerid][DevPromoMoney],
        PlayerDevPromoPending[playerid][DevPromoDonate],
        PlayerDevPromoPending[playerid][DevPromoExp]
    );

    format(dialogText, sizeof(dialogText), "{ffffff}Вы активируете промокод: {ffff00}%s\n\n{ffffff}Вы получите:\n%s", PlayerDevPromoPending[playerid][DevPromoCode], previewText);
    ShowPlayerDialog(playerid, DIALOG_DEVPROMO_ACTIVATE_CONFIRM, DIALOG_STYLE_MSGBOX, "{FF6347}"SERVER_NAME"{ffffff} | Активация промокода", dialogText, "Активировать", "Отмена");
    return 1;
}
...
stock ActivatePendingDevPromo(playerid)
{
    new query[256];
    new rewardText[256];
    new szActivationMessage[512];
    new Cache:result;
    new accountId = GetPlayerAccountID(playerid);

    if(PlayerDevPromoPending[playerid][DevPromoSQLID] <= 0)
    {
        SendClientMessage(playerid, -1, "{ff2400}| {ffffff}Нет промокода для активации.");
        return 1;
    }

    mysql_format(mysql, query, sizeof(query), "INSERT INTO `dev_promocode_activations` (`account_id`, `promo_id`, `activated_at`) VALUES (%d, %d, NOW())", accountId, PlayerDevPromoPending[playerid][DevPromoSQLID]);
    result = mysql_query(mysql, query, true);

    if(mysql_errno() != 0)
    {
        SendClientMessage(playerid, -1, "{ff2400}| {ffffff}Не удалось активировать промокод. Попробуйте позже.");
        cache_delete(result);
        ResetPlayerDevPromoPending(playerid);
        return 1;
    }
    ...
    if(PlayerDevPromoPending[playerid][DevPromoMoney] > 0)
    {
        GivePlayerMoneyEx(playerid, PlayerDevPromoPending[playerid][DevPromoMoney], "Активация промокода", true, true);
    }
    if(PlayerDevPromoPending[playerid][DevPromoDonate] > 0)
    {
        GivePlayerDonateRub(playerid, PlayerDevPromoPending[playerid][DevPromoDonate], "Активация промокода", true, true);
    }
    if(PlayerDevPromoPending[playerid][DevPromoExp] > 0)
    {
        AddPlayerData(playerid, P_EXP, +, PlayerDevPromoPending[playerid][DevPromoExp]);
    ...
    format(szActivationMessage, sizeof(szActivationMessage), "{ffffff}Поздравляем!\n\n{ffffff}Вы активировали промокод: {ffff00}%s\n\n", PlayerDevPromoPending[playerid][DevPromoCode]);
    strcat(szActivationMessage, "{ffffff}Вы получили:\n");
    strcat(szActivationMessage, rewardText);

    ShowPlayerDialog(playerid, DIALOG_DEVPROMO_RESULT, DIALOG_STYLE_MSGBOX, "{FF6347}"SERVER_NAME"{ffffff} | Активация промокода", szActivationMessage, "Ок", "");
    ...
    switch(dialogid)
    {
        case DIALOG_DEVPROMO_MENU:
        {
            ...
            switch(listitem)
            {
                case 0:
                {
                    ShowPlayerDialog(playerid, DIALOG_DEVPROMO_SET_NAME, DIALOG_STYLE_INPUT, "{FF6347}"SERVER_NAME"{FFFFFF} | Название промокода", "Введите название нового промокода:", "Сохранить", "Назад");
                }
                case 1:
                {
                    ShowDevPromoSetAmountDialog(playerid, DIALOG_DEVPROMO_SET_MONEY, "{FF6347}"SERVER_NAME"{FFFFFF} | Вирты", "Введите количество виртов для промокода.\nЕсли указать 0, этот приз не будет выдан.");
                }
                case 2:
                {
                    ShowDevPromoSetAmountDialog(playerid, DIALOG_DEVPROMO_SET_DONATE, "{FF6347}"SERVER_NAME"{FFFFFF} | Донат", "Введите количество доната для промокода.\nЕсли указать 0, этот приз не будет выдан.");
                }
                case 3:
                {
                    ShowDevPromoSetAmountDialog(playerid, DIALOG_DEVPROMO_SET_EXP, "{FF6347}"SERVER_NAME"{FFFFFF} | EXP", "Введите количество EXP для промокода.\nЕсли указать 0, этот приз не будет выдан.");
                }
                case 4:
                {
                    new rewardText[256];
                    ...
                    if(strlen(PlayerDevPromoDraft[playerid][DevPromoCode]) == 0)
                    {
                        SendClientMessage(playerid, -1, "{ff2400}| {ffffff}Сначала задайте название промокода.");
                        return ShowDevPromoBuilderDialog(playerid);
                    }
                    ...
                    {
                        SendClientMessage(playerid, -1, "{ff2400}| {ffffff}Добавьте хотя бы одну награду в промокод.");
                        return ShowDevPromoBuilderDialog(playerid);
                    }
                    ...
                    format(confirmText, sizeof(confirmText), "{ffffff}Название промокода: {ffff00}%s\n\n{ffffff}Награды:\n%s", PlayerDevPromoDraft[playerid][DevPromoCode], rewardText);
                    ShowPlayerDialog(playerid, DIALOG_DEVPROMO_CONFIRM_CREATE, DIALOG_STYLE_MSGBOX, "{FF6347}"SERVER_NAME"{ffffff} | Создание промокода", confirmText, "Создать", "Назад");
                }
            }
            return 1;
        }
        case DIALOG_DEVPROMO_SET_NAME:
        {
            if(!response)
            {
                return ShowDevPromoBuilderDialog(playerid);
            ...
            if(strlen(inputtext) == 0)
            {
                ...
                return ShowPlayerDialog(playerid, DIALOG_DEVPROMO_SET_NAME, DIALOG_STYLE_INPUT, "{FF6347}"SERVER_NAME"{FFFFFF} | Название промокода", "Введите название нового промокода:", "Сохранить", "Назад");
            }

            format(PlayerDevPromoDraft[playerid][DevPromoCode], MAX_PROMO_CODE_LENGTH, "%s", inputtext);
            return ShowDevPromoBuilderDialog(playerid);
        }
        case DIALOG_DEVPROMO_SET_MONEY:
        {
            if(!response)
            {
                return ShowDevPromoBuilderDialog(playerid);
            ...
            if(!IsUnsignedNumber(inputtext))
            {
                SendClientMessage(playerid, -1, "{ff2400}| {ffffff}Введите корректное число от 0 и выше.");
                return ShowDevPromoSetAmountDialog(playerid, DIALOG_DEVPROMO_SET_MONEY, "{FF6347}"SERVER_NAME"{FFFFFF} | Вирты", "Введите количество виртов для промокода.\nЕсли указать 0, этот приз не будет выдан.");
            }

            PlayerDevPromoDraft[playerid][DevPromoMoney] = strval(inputtext);
            return ShowDevPromoBuilderDialog(playerid);
        }
        case DIALOG_DEVPROMO_SET_DONATE:
        {
            if(!response)
            {
                return ShowDevPromoBuilderDialog(playerid);
            ...
            {
                SendClientMessage(playerid, -1, "{ff2400}| {ffffff}Введите корректное число от 0 и выше.");
                return ShowDevPromoSetAmountDialog(playerid, DIALOG_DEVPROMO_SET_DONATE, "{FF6347}"SERVER_NAME"{FFFFFF} | Донат", "Введите количество доната для промокода.\nЕсли указать 0, этот приз не будет выдан.");
            }

            PlayerDevPromoDraft[playerid][DevPromoDonate] = strval(inputtext);
            return ShowDevPromoBuilderDialog(playerid);
        }
        case DIALOG_DEVPROMO_SET_EXP:
        {
            if(!response)
            {
                return ShowDevPromoBuilderDialog(playerid);
            ...
            {
                SendClientMessage(playerid, -1, "{ff2400}| {ffffff}Введите корректное число от 0 и выше.");
                return ShowDevPromoSetAmountDialog(playerid, DIALOG_DEVPROMO_SET_EXP, "{FF6347}"SERVER_NAME"{FFFFFF} | EXP", "Введите количество EXP для промокода.\nЕсли указать 0, этот приз не будет выдан.");
            }

            PlayerDevPromoDraft[playerid][DevPromoExp] = strval(inputtext);
            return ShowDevPromoBuilderDialog(playerid);
        }
        case DIALOG_DEVPROMO_CONFIRM_CREATE:
        {
            if(!response)
            {
                return ShowDevPromoBuilderDialog(playerid);
            ...
            format(promoCode, sizeof(promoCode), "%s", PlayerDevPromoDraft[playerid][DevPromoCode]);
            mysql_format(mysql, query, sizeof(query), "SELECT 1 FROM `dev_promocodes` WHERE `code` = '%e' LIMIT 1", promoCode);
            result = mysql_query(mysql, query, true);

            if(mysql_errno() != 0)
            {
                SendClientMessage(playerid, -1, "{ff2400}| {ffffff}Ошибка базы данных при создании промокода.");
                cache_delete(result);
                return 1;
            }
            ...
            (
                ...
                query,
                sizeof(query),
                "INSERT INTO `dev_promocodes` (`code`, `money`, `donate`, `exp`, `created_by`, `created_at`) VALUES ('%e', %d, %d, %d, %d, NOW())",
                promoCode,
                PlayerDevPromoDraft[playerid][DevPromoMoney],
                PlayerDevPromoDraft[playerid][DevPromoDonate],
                PlayerDevPromoDraft[playerid][DevPromoExp],
            ...
            if(mysql_errno() != 0)
            {
                SendClientMessage(playerid, -1, "{ff2400}| {ffffff}Не удалось создать промокод. Попробуйте позже.");
                cache_delete(result);
                return 1;
            }
            cache_delete(result);

            SendClientMessage(playerid, -1, "{66cc00}| {ffffff}Промокод успешно создан.");
            ResetPlayerDevPromoDraft(playerid);
            return 1;
        }
        case DIALOG_DEVPROMO_ACTIVATE_INPUT:
        {
            if(!response)
            {
                return 1;
            ...
            if(strlen(inputtext) == 0)
            {
                SendClientMessage(playerid, -1, "{ff2400}| {ffffff}Введите название промокода.");
                return callcmd::dpromo(playerid, "");
            }

            return ShowDevPromoActivationDialog(playerid, inputtext);
        }
        case DIALOG_DEVPROMO_ACTIVATE_CONFIRM:
        {
            if(!response)
            {
                ResetPlayerDevPromoPending(playerid);
            ...
            return ActivatePendingDevPromo(playerid);
        }
...
CMD:devpromo(playerid, params[])
{
    new action[16];

    if(GetPlayerAdminEx(playerid) < 8)
    {
        return SendClientMessage(playerid, -1, "{ff2400}| {ffffff}У вас недостаточно прав для использования этой команды.");
    }

    if(sscanf(params, "s[16]", action))
    {
        return SendClientMessage(playerid, -1, "{ff2400}| {ffffff}Используйте: /devpromo add");
    }
...
CMD:dpromo(playerid)
{
    ShowPlayerDialog(playerid, DIALOG_DEVPROMO_ACTIVATE_INPUT, DIALOG_STYLE_INPUT, "{FF6347}"SERVER_NAME"{FFFFFF} | Активация промокода", "Введите название промокода для активации:", "Далее", "Отмена");
    return 1;
}
