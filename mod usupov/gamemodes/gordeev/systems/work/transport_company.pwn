...
public OnGameModeInit()
{
    ...
    {
        CreateDynamicPickup(1318, 23, coord_tcompany[i][0], coord_tcompany[i][1], coord_tcompany[i][2], 0, 0);
        enter_tcompany[i] = CreateDynamicSphere(coord_tcompany[i][0], coord_tcompany[i][1], coord_tcompany[i][2], 2.0, 0, 0);
        Create3DTextLabel("{0099FF}Транспортная компания", -1, coord_tcompany[i][0], coord_tcompany[i][1], coord_tcompany[i][2], 10.0, 0);

        CreateDynamicPickup(1318, 23, coord_tcompany_exit[0], coord_tcompany_exit[1], coord_tcompany_exit[2], i+1, 1);
        exit_tcompany = CreateDynamicSphere(coord_tcompany_exit[0], coord_tcompany_exit[1], coord_tcompany_exit[2], 1.0, -1, 1);
        Create3DTextLabel("{0099FF}Выход", -1, coord_tcompany_exit[0], coord_tcompany_exit[1], coord_tcompany_exit[2], 10.0, i+1);

        CreateDynamicPickup(1314, 23, coord_tcompany_order[0], coord_tcompany_order[1], coord_tcompany_order[2], i, 1);
    ...
    CreateActorEx("Илюша Вертон", "{828282}очень хочет арбузы..", 228, -2639.756835,2879.549072,37.632812,177.616, 0, 0);
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == 2903)
    {
        if(!response)
        {
            ...
            if(GetPVarInt(playerid, "dostup_order")) return SendClientMessage(playerid, -1, ""tc_msg"Ваш уровень дальнобойщика не позволяет взять этот заказ.");
...
CMD:tcstats(playerid)
{
    ...
    if(player_tc[playerid][p_TC_Level] > 10)
    {
        ...
        Например: за заказ оплатой 40.000 руб. вы получите {FFAE00}%d {FFFFFF}руб.", GetPlayerIncreaseTC(playerid), salary);
