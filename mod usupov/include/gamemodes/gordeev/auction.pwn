        ...
        case TYPE_BUSINESS:
        {
            for(new businessid; businessid < MAX_BUSINESS; businessid++)
            {
                ...
                if(!strlen(auction_slot[slot][SLOT_NAME]) || !strcmp(auction_slot[slot][SLOT_NAME], "-", true))
                    format(auction_slot[slot][SLOT_NAME], 24, GetBusinessData(businessid, B_NAME));
...
stock AuctionBusiness(sql_id, slot)//Автор данной системы https://t.me/welsistudio (Welsi Studio)
{
        ...
        if(!mysql_errno())
        {
            if(player != -1) SetPlayerData(player, P_BUSINESS, businessid);
            ...
            if(player != -1) SendClientMessage(player, 0x66CC00FF, "Напишите {0099FF}/business {66CC00}чтобы узнать о возможностях");
