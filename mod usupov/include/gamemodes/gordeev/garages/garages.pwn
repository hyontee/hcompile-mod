// добавляем в начало мода и ставим 0 если у вас не заменка олд
// если олд заменка с gui или нев с gui то ставим 1 
// это для Notification если что
/*#define USE_ZOMENO_OLD_ENGINE 1

//в enum E_PLAYER_STRUCT // структура игрока
   P_IN_GARAGE,			// в какой гараж вошел
   P_GARAGE, 
   
// в stock SendPlayerOffer ко всем кейсам
            case OFFER_TYPE_SELL_GARAGE:
			{
				format(fmt_str, sizeof fmt_str, "%s предлагает Вам купить гараж №%d по цене %d руб", GetPlayerNameEx(playerid), value_1, value_2);
				SendClientMessage(to_player, 0x3399FFFF, fmt_str);

				SendClientMessage(to_player, 0xFFFFFFFF, "Введите {00CC00}/yes {FFFFFF}чтобы заключить сделку или {FF6600}/no {FFFFFF}для отказа");
				
			if(USE_ZOMENO_OLD_ENGINE == 0)
            {
				ShowNotification(to_player, 4, "Предложение от игрока", 7, "/yes", ">>");
            }
            else ShowClientNotification(to_player, 4, 5, 1, 10, "Предложение от игрока!", "");
            
				format(fmt_str, sizeof fmt_str, "Вы предложили %s купить Ваш гараж за %d руб", GetPlayerNameEx(to_player), value_2);
				SendClientMessage(playerid, 0x3399FFFF, fmt_str);
			}

//в CMD:yes(playerid, params[]) ко всем кейсам
                    case OFFER_TYPE_SELL_GARAGE:
					{
						if(GetPlayerGarage(offer_id) == value_1 && GetPlayerGarage(playerid) == -1)
						{
							if(GetPlayerMoneyEx(playerid) >= value_2)
							{
								new Float: g_pos_x = GetGarageData(value_1, GARAGE_X);
								new Float: g_pos_y = GetGarageData(value_1, GARAGE_Y);
								new Float: g_pos_z = GetGarageData(value_1, GARAGE_Z);
								...
								if(IsPlayerInRangeOfPoint(playerid, 7.0, g_pos_x, g_pos_y, g_pos_z) && IsPlayerInRangeOfPoint(offer_id, 7.0, g_pos_x, g_pos_y, g_pos_z))
								{
									...
									format(fmt_str, sizeof fmt_str, "%s продал Вам гараж за %d руб", GetPlayerNameEx(offer_id), value_2);
								...
								else SendClientMessage(playerid, 0xCECECEFF, "Вы и продавец должны находиться рядом с гаражем который хотите купить");
							}
...
// ShowOwnableCarLoadDialog заменяем на это
...
new garageid = GetPlayerData(playerid, P_GARAGE);
	...
	if(rent_car)
	{
		...
		format(w_str, sizeof w_str, "1. Отметить транспорт на GPS {FF6600}(300 руб)\n""{FFFFFF}2. Загрузить транспорт на этой парковке(%d)", price_auto);
	...
	if(LoadCarGarage[playerid] == 0)
		...
		"2. Загрузить транспорт на месте {FF6600}выгрузки\n"\
		"3. Загрузить транспорт на месте {FF6600}парковки\n"\
		"4. Сбросить парковку автомобиля",
		"Выбрать", "Закрыть"
	   	);
		}
		else {
		            Dialog
	              	(
		             	playerid, DIALOG_OWNABLE_CAR_LOAD_GARAGE, DIALOG_STYLE_LIST,
		           	"{FFCD00}Система управления транспортом",
		           	"1. Отметить место загрузки транспорта на GPS {FF6600}(300 руб)\n"\
...
// в LoadOwnableCar добавляем в начало

new garageid = GetPlayerGarage(playerid);
new virtual_world = garageid + 2000;
// теперь в LoadOwnableCar заменяем это
...
if(LoadCarGarage[playerid] == 1)
		{
		vehicleid = CreateVehicle
		(
			GetOwnableCarData(idx, OC_MODEL_ID),
			-0.292650,2000.242065,1553.358764,355.240234,
			GetOwnableCarData(idx, OC_COLOR_1),
			GetOwnableCarData(idx, OC_COLOR_2),
			garageid,
			1,
		...
		else if(LoadCarGarage[playerid] == 2)
		{
		vehicleid = CreateVehicle
		(
			GetOwnableCarData(idx, OC_MODEL_ID),
			504.964385,2007.766113,1547.719726,142.482574,
			GetOwnableCarData(idx, OC_COLOR_1),
			GetOwnableCarData(idx, OC_COLOR_2),
			garageid,
			1,
		...
		else if(LoadCarGarage[playerid] == 0)
		{
	vehicleid = CreateVehicle
	(
		GetOwnableCarData(idx, OC_MODEL_ID),
...
stock LoadOwnableCarRent(playerid, idx)
{
    return LoadOwnableCar(idx);
}

#define ShowClientNotification(%0,%1,%2,%3,%4,%5,%6) ShowNotificationNew(%0,%1,%2,%3,%4,%5,%6)
   ...
   if(dialogid == DIALOG_GARAGE_IMPROVEMENT)
	...
	new garageid = GetPlayerGarage(playerid);
	   ...
	   if(response)
	   {
	    ...
	    {
        	ShowClientNotification(playerid, 3, 5, 1, 10, "Улучшение было куплено!", "");
            GivePlayerMoneyEx(playerid, -5000000);
        }
	    }
 	   else 
