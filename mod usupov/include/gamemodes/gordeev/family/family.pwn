...
public: welsi_LoadFamily()
{
    ...
    for(new i; i < MAX_FAM;i++)
    {
        ...
        "(SELECT COUNT(*) FROM family_auto WHERE owner_id = %d) AS cars_count", GetFamily(i, family_database), GetFamily(i, family_database)); //спасибо Grok
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == 2220)
    {
        if(response)
        {
            switch(listitem)
            {
                ...
                case 5:
                {
                    mysql_format(mysql, f_string64, sizeof f_string64, "SELECT * FROM family_auto WHERE owner_id = %d", GetPlayerFamily(playerid, ID_SQL_FAMILY));
                    new Cache:cache = mysql_query(mysql, f_string64);
    ...
    if(dialogid == 2222)
    {
        if(response)
        {
            ...
            if(listitem == up || listitem == down)
            {
                ...
                mysql_format(mysql, fmt_text, sizeof fmt_text, "SELECT * FROM family_auto WHERE family_owner='%d'", GetFamily(f, family_database));
    ...
    if(dialogid == 2224)
    {
        if(response)
        {
            ...
            mysql_format(mysql, f_string64, sizeof f_string64, "SELECT * FROM family_auto WHERE id=%d", id);
            new Cache:cache = mysql_query(mysql, f_string64);
    ...
    if(dialogid == 2235)
    {
        if(response)
        {
            ...
            else
            {
                ...
                mysql_format(mysql, f_string144,sizeof f_string144, "DELETE FROM family_auto WHERE family_owner = %d", f_sql);
    ...
    if(dialogid == 2239)
    {
        if(response)
        {
            ...
            mysql_format(mysql, f_string144, sizeof f_string144, "UPDATE family_auto SET rang=%d WHERE id=%d LIMIT 1", rang, id);
            mysql_query(mysql, f_string144, false);
...
{
    new fam_id = fam;
    new fam_sql_id = GetPlayerFamily(playerid, ID_SQL_FAMILY);
    
    if(GetFamilyData(fam_id, F_FAMILY_CARS) >= 5)
        return SendClientMessage(playerid, -1, ""c_f_r"В семье не хватает слотов для авто.");
    
    new query[512];
    mysql_format(mysql, query, sizeof query, "SELECT * FROM ownable_cars WHERE id=%d", database_car);
    new Cache:cache_sql = mysql_query(mysql, query, true);
    
    if(mysql_errno() || !cache_num_rows())
    {
        cache_delete(cache_sql);
        return SendClientMessage(playerid, 0x999999FF, "Транспорт не найден в БД.");
    }
    
    new model_id = cache_get_field_content_int(0, "model_id");
    new color_1 = cache_get_field_content_int(0, "color_1");
    new color_2 = cache_get_field_content_int(0, "color_2");
    new Float:pos_x = cache_get_field_content_float(0, "pos_x");
    new Float:pos_y = cache_get_field_content_float(0, "pos_y");
    new Float:pos_z = cache_get_field_content_float(0, "pos_z");
    new Float:angle = cache_get_field_content_float(0, "angle");
    new mileage = cache_get_field_content_float(0, "mileage");
    new vinilcar = cache_get_field_content_int(0, "vinilcar");
    new pt_engine = cache_get_field_content_int(0, "pt_engine");
    new pt_brake = cache_get_field_content_int(0, "pt_brake");
    new pt_stability = cache_get_field_content_int(0, "pt_stability");
    new nitro = cache_get_field_content_int(0, "nitro");
    new launch = cache_get_field_content_int(0, "launch");
    new fars = cache_get_field_content_int(0, "fars");
    new diski = cache_get_field_content_int(0, "diski");
    cache_delete(cache_sql);
    
    // INSERT в family_auto со всеми полями
    mysql_format(mysql, query, sizeof query,
        "INSERT INTO family_auto (owner_id,model_id,color_1,color_2,pos_x,pos_y,pos_z,angle,create_time,status,alarm,key_in,mileage,vinilcar,pt_engine,pt_brake,pt_stability,nitro,launch,fars,diski,rang) \
        VALUES (%d,%d,%d,%d,%f,%f,%f,%f,%d,%d,%d,%d,%f,%d,%d,%d,%d,%d,%d,%d,%d,%d)",
        fam_sql_id, model_id, color_1, color_2, pos_x, pos_y, pos_z, angle, gettime(),
        0, 0, 0, mileage, vinilcar, pt_engine, pt_brake, pt_stability, nitro, launch, fars, diski, 1);
    
    mysql_query(mysql, query, true);
    if(mysql_errno())
    {
        printf("[MoveOwnableCar] SQL ERROR: %s", query);
        SendClientMessage(playerid, 0x999999FF, "Ошибка SQL при добавлении транспорта в семью.");
        return 0;
    }
    
    // DELETE из ownable_cars
    mysql_format(mysql, query, sizeof query, "DELETE FROM ownable_cars WHERE id=%d", database_car);
    mysql_query(mysql, query, false);
    
    UnloadPlayerOwnableCar(playerid);
    SendClientMessage(playerid, -1, ""c_f_g" Вы успешно добавили транспорт в семью.");
    
    format(query, sizeof query, "Владелец %s добавил в семью автомобиль %s", GetPlayerNameEx(playerid), GetVehicleInfo(model_id-400, VI_NAME));
    for(new i = 0; i < MAX_PLAYERS; i++)
    {
        if(!IsPlayerConnected(i) || !IsPlayerLogged(i)) continue;
        if(GetPlayerIdFamily(i) != fam_id) continue;
        SendClientMessage(i, -1, query);
    }
    
    AddFamilyData(fam_id, F_FAMILY_CARS, +, 1);
    mysql_format(mysql, query, sizeof query, "UPDATE family SET family_auto=%d WHERE id=%d LIMIT 1", GetFamilyData(fam_id, F_FAMILY_CARS), fam_sql_id);
    mysql_query(mysql, query, false);
    return 1;
}
stock MoveFamilyCar(database_car, playerid)
{
    ...
    if(fam == -1) return SendClientMessage(playerid, -1, ""c_f_r"Вы не состоите в семье.");

    new query[512];
    mysql_format(mysql, query, sizeof query, "SELECT * FROM family_auto WHERE id=%d", database_car);
    new Cache:cache_sql = mysql_query(mysql, query, true);
    ...
    // Используем ТОЧНО ТЕ ЖЕ колонки, что и в оригинальном рабочем коде
    mysql_format(mysql, query, sizeof query, 
        "INSERT INTO ownable_cars (owner_id, model_id, color_1, color_2, pos_x, pos_y, pos_z, angle, create_time, number) VALUES (%d, %d, %d, %d, %f, %f, %f, %f, %d, '%s')", 
        GetPlayerAccountID(playerid), model_id, color_1, color_2, pos_x, pos_y, pos_z, angle, gettime(), number);
        
    mysql_query(mysql, query, true);
    if(mysql_errno())
    {
        printf("[MoveFamilyCar] SQL ERROR: %s", query);
        return SendClientMessage(playerid, -1, ""c_f_r"Ошибка SQL при возврате транспорта.");
    }

    mysql_format(mysql, query, sizeof query, "DELETE FROM family_auto WHERE id=%d", database_car);
    mysql_query(mysql, query, false);
    ...
    if(!mysql_errno())
    {
        ...
        new fmt_msg[144];
        format(fmt_msg, sizeof fmt_msg, "Владелец %s забрал из семьи автомобиль %s", GetPlayerNameEx(playerid), GetVehicleInfo(model_id-400, VI_NAME));
        SendFamilyMessage(fam, fmt_msg);
...
stock LoadFamilyCar(database)
{
    ...
    mysql_format(mysql, f_string64, sizeof f_string64, "SELECT * FROM family_auto WHERE id=%d", database);
