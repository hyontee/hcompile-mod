...
stock AirDrop()
{
    ...
    SendClientMessageToAll(-1, ""SC" На сервере произошло событие {FFFF00}AIRDROP {686868}(длится 40 минут)");
    ...
    SendClientMessageToAll(-1, ""SC" Чтобы сломать {FFFF00}ящик{FFFFFF} используйте {FFFF00}/crashbox");
    ...
    {
        if(GetPlayerAdminEx(playerid) > 0 && GetPlayerAdminEx(playerid) < 8) // Если игрок админ
        {
        ...
        GivePlayerDonateRub(playerid, 50);
            ...
            // Уведомление
            SendClientMessage(playerid, 0xFF0000AA, "Вы Администратор, для вас Air-Drop запрещён!");
            SendClientMessage(playerid, 0x32a44bFF, "Вам выдана компенсация: 200.000 игровой валюты и 50 BC");
            }
        }
    ...
    if(air_sphere[0] <= areaid <= air_sphere[sizeof air_sphere - 1])
    {
       ...
       {    
            if(airdrop[id_air][AIR_STATE] == AIR_STATE_CRASH)
            {
                ...
                Dialog(playerid, 2330, DSM, "Информация о призах в ящике", text, "Забрать", "Назад");
            }
       }
       else
       {
               //Автор: Welsi Тг канал:t.me/welsistudio
            if(airdrop[id_air][AIR_STATE] == AIR_STATE_LOAD) ShowNewNotification(playerid, 5, 5, 1, 10, "Введите /crashbox", "Ок");
            else{
...
CMD:airdrop(playerid)
{
    ...
    DialogTAB45(playerid, 2331, DIALOG_STYLE_LIST, "Информация о AirDrop'е", list, "Отметить", "Закрыть");
    return 1;
}
...
stock UnAirDrop()
{
    ...
    SendClientMessageToAll(-1, ""SC" Все {FFFF00}ящики {FFFFFF}с призами на карте были удалены");
