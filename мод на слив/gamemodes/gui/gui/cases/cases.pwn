...
stock Cases_AddToPlayer(playerid, caseIdx, amount)
{
    ...
    if(amount <= 0) return 0;
    SetPlayerCaseCountByType(playerid, casesIdx, amount);
    return 1;
}
...
stock Cases_Init()
{
    // ÎÁÍÓËßÅÌ ÂÑÅ ÊÅÉÑÛ ÏÅÐÅÄ ÈÍÈÖÈÀËÈÇÀÖÈÅÉ
    ...
    // ÊÅÉÑÛ 7-19: Èíèöèàëèçèðóåì âñå ïîäðÿä
    ...
    // ÂÛÂÎÄÈÌ ÑÏÈÑÎÊ ÈÍÈÖÈÀËÈÇÈÐÎÂÀÍÍÛÕ ÊÅÉÑÎÂ
...
CMD:cases(playerid, params[])
{
    ...
    {
        SendClientMessage(playerid, -1, "{FFCC00}Ó Âàñ íåò äîñòóïà");
        return 1;
    } 
...
CMD:givecases(playerid, params[])
{
    ...
    if(sscanf(params, "udd", to_player, type_case, count))
        return SendClientMessage(playerid, COLOR_GREY, "Èñïîëüçóéòå: /givecases [id] [type_case:1-6] [count]");

    if(!IsPlayerConnected(to_player)) return SendClientMessage(playerid, COLOR_RED, "Èãðîê íå íàéäåí.");
    if(type_case < 1 || type_case > 6) return SendClientMessage(playerid, COLOR_RED, "type_case äîëæåí áûòü îò 1 äî 6.");
    if(count <= 0) return SendClientMessage(playerid, COLOR_RED, "count äîëæåí áûòü áîëüøå 0.");

    AddPlayerCaseCountByType(to_player, type_case, count);
    SavePlayerAccount(to_player);

    new str[128];
    format(str, sizeof(str), "Âû âûäàëè %d êåéñ(îâ) òèïà %d èãðîêó %s.", count, type_case, GetPlayerNameEx(to_player));
    SendClientMessage(playerid, COLOR_WHITE, str);

    format(str, sizeof(str), "Àäìèíèñòðàòîð âûäàë âàì %d êåéñ(îâ) òèïà %d.", count, type_case);
