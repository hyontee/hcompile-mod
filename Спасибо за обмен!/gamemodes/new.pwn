...
public OnGameModeInit()
{
    ...
    for(new i = 0; i < TotalTransportSpots; i++)
    {
        ...
        new label[128];
        format(label, sizeof(label), "{DD5757}MIDSH ZBT | {808080}Транспорт\n{FFFFFF}Подойдите чтобы открыть меню");
...
public OnPlayerConnect(playerid)
{
    ...
    SendClientMessage(playerid, -1, "{66CC66}| {ffffff}Добро пожаловать на {00CCFF}MIDSH ZBT!");
...
public OnPlayerSpawn(playerid)
{
	...
	SendClientMessage(playerid, -1, "{66CC66}| {00CCFF}[DEBUG] Client « MIDSH ZBT » started!");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
        ...
        switch(listitem)
        {
            ...
            case 1:
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_AF_COMMANDS, DIALOG_STYLE_LIST,
                    "{E0584B}MIDSH ZBT {FFFFFF}| Выберите команду",
    ...
    if(dialogid == DIALOG_AF_COMMANDS)
    {
        ...
        ShowPlayerDialog(playerid, DIALOG_AF_INPUT_PARAMS, DIALOG_STYLE_INPUT,
            "{E0584B}MIDSH ZBT {FFFFFF}| Параметры команды",
            info, "Далее", "Назад");
        return 1;
    }
    ...
    if(dialogid == DIALOG_AF_INPUT_PARAMS)
    {
        ...
        ShowPlayerDialog(playerid, DIALOG_AF_CONFIRM, DIALOG_STYLE_MSGBOX,
            "{E0584B}MIDSH ZBT {FFFFFF}| Подтверждение", preview, "Далее", "Назад");
        return 1;
    }
    ...
    if(dialogid == DIALOG_AF_LIST)
    {
        ...
        ShowPlayerDialog(playerid, DIALOG_AF_ACCEPT, DIALOG_STYLE_MSGBOX,
            "{E0584B}MIDSH ZBT {FFFFFF}| Принятие формы", preview, "Да", "Нет");
        return 1;
    }
    ...
    if(dialogid == ENTER_PROMOCODE)
    {
        ...
        format(msg, sizeof(msg),
            ...
            Приятной игры на {E0584B}MIDSH ZBT!", inputtext);

        ShowPlayerDialog(playerid, PROMO_PRISE, DIALOG_STYLE_MSGBOX, "{E0584B}MIDSH ZBT{FFFFFF} | Активация промокода", msg, "Закрыть", "");
        return 1;
    }
...
public OnPlayerPickUpDynamicPickup(playerid, pickupid)
{
    for(new i = 0; i < TotalTransportSpots; i++)
    {
        if(pickupid == TransportSpots[i][tPickup])
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_TRANSPORT_MENU, DIALOG_STYLE_TABLIST_HEADERS, "{DD5757}MIDSH ZBT {808080}| Транспорт", "{DD5757}номер\t{DD5757}доступное действие\t{DD5757}стоимость\n#1\tАрендовать мотоцикл\tбесплатно\n#2\tВызвать личный транспорт\tбесплатно", "Выбрать", "Отмена");
            return 1;
        }
    }
    ...
    for(new i = 0; i < TotalHouses; i++)
    {
        if(pickupid == HouseInfo[i][hPickup])
        {
            ...
            if(HouseInfo[i][hOwnerID] == 0)
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_BUYHOUSE, DIALOG_STYLE_TABLIST_HEADERS, "{E0584B}MIDSH ZBT {FFFFFF}| Покупка", str, "Купить", "Отмена");
            }
            else
            {
                ...
                ShowPlayerDialog(playerid, DIALOG_ENTER_HOUSE, DIALOG_STYLE_MSGBOX,
                    "{E0584B}MIDSH ZBT {FFFFFF}| Вход в дом",
                    str, "Войти", "Отмена");
            }
            return 1;
        }
    ...
    if(rows > 0)
    {
        ...
        if(Player[playerid][pBanTime] > 0)
        {
            ...
            format(string, sizeof(string),
                "{FFFFFF}Ваш аккаунт заблокирован на сервере {E0584B}MIDSH ZBT{FFFFFF}\n\n"\
                "Причина: {E0584B}%s\n"\
            ...
            ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{E0584B}MIDSH ZBT | Блокировка", string, "Понятно", "");
...
public OnPlayerClickPlayerTextDraw(playerid, PlayerText:playertextid)
{
    if(playertextid == avtsln_PTD[playerid][0])
    {
        ...
        ShowPlayerDialog(playerid, DIALOG_BUY_CAR_CONFIRM, DIALOG_STYLE_TABLIST_HEADERS, "{E0584B}MIDSH ZBT {FFFFFF}| Автосалон", string, "Купить", "Отмена");
        return 1;
    }
...
stock ShowPlayerCars(playerid, page)
{
    ...
    ShowPlayerDialog(playerid, DIALOG_MY_CARS, DIALOG_STYLE_TABLIST_HEADERS, "{E0584B}MIDSH ZBT {FFFFFF}| Транспорты", string, "Выбрать", "Отмена");
    return 1;
}
...
stock ShowCarActionMenu(playerid, car_slot)
{
    ...
    ShowPlayerDialog(playerid, DIALOG_CAR_ACTION, DIALOG_STYLE_TABLIST_HEADERS, "{E0584B}MIDSH ZBT {FFFFFF}| Управление", menu_str, "Далее", "Назад");
    return 1;
}
...
CMD:time(playerid)
{
    ...
    ShowPlayerDialog(playerid, 0, DIALOG_STYLE_MSGBOX, "{E0584B}MIDSH ZBT | Точное время", str, "Закрыть", "");
    return 1;
}
...
CMD:house(playerid)
{
    ...
    ShowPlayerDialog(playerid, DIALOG_MENU_HOUSE, DIALOG_STYLE_TABLIST_HEADERS,
        "{E0584B}MIDSH ZBT {FFFFFF}| Управление",
        str, "Выбрать", "Закрыть"
    );
    return 1;
}
...
CMD:deltex(playerid, params[])
{
    ...
    if(Admin3DText[playerid] != INVALID_3DTEXT_ID)
    {
        DestroyDynamic3DTextLabel(Admin3DText[playerid]);
        Admin3DText[playerid] = INVALID_3DTEXT_ID;
        SendClientMessage(playerid, -1, "{E0584B}MIDSH ZBT {FFFFFF}| Метка удалена.");
    }
    else SendClientMessage(playerid, -1, "{E0584B}MIDSH ZBT {FFFFFF}| У вас нет активной метки.");
    return 1;
}
...
stock AF_ShowList(playerid)
{
    ...
    ShowPlayerDialog(playerid, DIALOG_AF_LIST, DIALOG_STYLE_TABLIST_HEADERS,
        "{E0584B}MIDSH ZBT {FFFFFF}| Список форм", dialog, "Принять", "Назад");
    return 1;
}
...
CMD:af(playerid, params[])
{
    ...
    ShowPlayerDialog(playerid, DIALOG_AF_MAIN, DIALOG_STYLE_LIST,
        "{E0584B}MIDSH ZBT {FFFFFF}| Админ-формы",
...
stock ShowAHelpPage(playerid, mode, page)
{
    ...
    {
        format(title, sizeof(title), "{DD5757}MIDSH ZBT | {FFFFFF}Сокращенные команды [%d ур.]", page);
    }
    else
    {
        format(title, sizeof(title), "{DD5757}MIDSH ZBT | {FFFFFF}Все команды [%d ур.]", page);
    }
...
stock ShowAHelpMain(playerid)
{
    ...
    ShowPlayerDialog(playerid, DIALOG_AHELP_MAIN, DIALOG_STYLE_TABLIST_HEADERS,
        "{DD5757}MIDSH ZBT {FFFFFF}| Команды администратора",
        string,
        "Выбрать",
        "Закрыть"
    );
