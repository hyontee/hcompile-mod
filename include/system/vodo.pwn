...
new kapitanp3;
new kapitanp4;
new yg_areas[6]; // для test 1 yg
new bat_areas1[7]; // для test 1 bat
new bat_areas2[6]; // для test 1 bat
new lit_areas1[6]; // для test 1 lit
new lit_areas2[6]; // для test 1 lit
...
enum E_PLAYER_VODO_ITEMS {
    yg_collected[6],      // test 1 yg (6 предметов)
    bat1_collected[7],    // test 1 bat (7 предметов)
    bat2_collected[6],    // test 1 bat (6 предметов)
    lit1_collected[6],    // test 1 lit (6 предметов)
    lit2_collected[6],    // test 1 lit (6 предметов)
    yzka_collected[7]     // yzka (7 предметов)
}
new PlayerVodoItems[1][1];
new VodolazItemNames[1][64] = {
    "Стиральная машина",
    "Колесо внедорожника", 
    "Старинная ваза",
    "Якорь корабля",
...
// ============================================================================
// Интеграция найденных предметов с основным инвентарём VIGSA
// Предметы используют существующие ID клиента, чтобы корректно отображались.
// Метка VODO:* в item_plate отделяет находки водолаза от обычных предметов.
// ============================================================================
...
#define VODO_ITEM_GOLD_CHAIN        (990)
...
{
    if(slot < 1 || slot >= MAX_INVENTORY_SLOTS) return -1;
    if(g_PlayerInventory[playerid][slot][inv_itemId] <= 1) return -1;

    new tag[16];
    for(new i = 1; i < sizeof(VodolazInventoryItemIds); i++)
    {
        format(tag, sizeof(tag), "VODO:%d", i);
        if(g_PlayerInventory[playerid][slot][inv_itemId] == VodolazInventoryItemIds[i]
            && !strcmp(g_PlayerInventory[playerid][slot][inv_itemPlate], tag, true))
        {
            return i;
        }
    }
...
stock Vodo_ApplyInventoryName(playerid, slot, item_index)
{
    if(slot < 1 || slot >= MAX_INVENTORY_SLOTS) return 0;
    if(item_index < 1 || item_index >= sizeof(VodolazItemNames)) return 1;

    format(g_PlayerInventory[playerid][slot][inv_itemName], 64, "%s", VodolazItemNames[item_index]);
    return 1;
}
...
{
    new total = 1;
    for(new slot = 1; slot < MAX_INVENTORY_SLOTS; slot++)
    {
        if(Vodo_GetSlotItemIndex(playerid, slot) == -1) continue;
...
stock bool:Vodo_AddCollectedItem(playerid, collected_index, bool:show_message = true)
{
    ...
    if(free_slot == -1)
    {
        Send(playerid, -1, "В инвентаре нет свободного места. Освободите слот и поднимите предмет снова.");
        return false;
    }

    new item_id = VodolazInventoryItemIds[item_index];
    new plate_tag[16];
    format(plate_tag, sizeof(plate_tag), "VODO:%d", item_index);

    if(!Inventory_AddItem(playerid, item_id, free_slot, 1, plate_tag))
    {
        Send(playerid, -1, "Не удалось положить находку в инвентарь. Проверьте свободное место и допустимый вес.");
        return false;
    }

    Vodo_ApplyInventoryName(playerid, free_slot, item_index);
    ...
    {
        new msg[160];
        format(msg, sizeof(msg), "Вы подняли {FF9900}%s{FFFFFF}. Предмет помещён в ваш инвентарь.", VodolazItemNames[item_index]);
        Send(playerid, -1, msg);
    }
    return true;
}
...
stock bool:Vodo_RemoveInventoryItem(playerid, slot)
{
    ...
    if(!Inventory_RemoveOneItem(playerid, slot)) return false;
...
stock Vodo_MigrateLegacyItems(playerid)
{
    ...
    // Переносим старые находки только если в новом инвентаре ещё нет предметов VODO.
    if(inventory_count == 0 && legacy_count > 0)
    {
        ...
        for(new i = 0; i < legacy_count; i++)
        {
            ...
            if(!Inventory_AddItem(playerid, VodolazInventoryItemIds[item_index], free_slot, 1, plate_tag)) break;
            Vodo_ApplyInventoryName(playerid, free_slot, item_index);
            migrated++;
        }
...
if(dialogid == 58909) // Продажа предметов непосредственно из инвентаря
{
    ...
    new msg[160];
    format(msg, sizeof(msg), "Вы продали {FF9900}%s {FFFFFF}за {00FF00}%d рублей{FFFFFF}!", item_name, item_price);
    Send(playerid, -1, msg);
...
CMD:vodopredmpokaz(playerid)
{
    ...
    for(new slot = 0; slot < MAX_INVENTORY_SLOTS; slot++)
    {
        ...
        for(new part = 0; part < count && listed < MAX_INVENTORY_SLOTS; part++)
        {
            format(row, sizeof(row), "%d\t%s\t%d рублей\n",
                listed + 1,
                VodolazItemNames[item_index],
                VodolazItemPrices[item_index]
            );
            strcat(dialog_text, row);
