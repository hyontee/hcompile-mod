/*
    ...
    - Клик по машине -> выбор цвета -> спавнит
    - Клик по "Следующая/Предыдущая" внизу -> листает
    - Лог в админ-чат:
      [A] Администратор Nick заспавнил авто Name (цвет: X, Y).
...
#define VVEH_DLG_COLOR  (61341)
#define VVEH_PER_PAGE   (25)
#define VVEH_MAX        (482)

// ЦВЕТА ДЛЯ ВЫБОРА
static const gVehicleColors[][2] = {
    {0, 0},     // Черный
    {1, 1},     // Белый
    {3, 3},     // Красный
    {4, 4},     // Синий
...
static const gColorNames[][] = {
    "Черный", "Белый", "Красный", "Синий", "Желтый",
    "Оранжевый", "Зеленый", "Серебро", "Золото", "Фиолетовый",
    "Розовый", "Темно-синий", "Бордовый", "Коричневый", "Серый",
    "Бирюзовый", "Мятный", "Лаванда", "Шампань", "Хаки"
};
...
static gVvehModel[MAX_PLAYERS];
static gVvehName[MAX_PLAYERS][64];
...
stock Vveh_AdminLogSpawn(playerid, vehname[], color1, color2)
{
    #if defined SendMessageToAdmins && defined GetPlayerNameEx
        new fmt_msg[160];
        format(fmt_msg, sizeof fmt_msg, "[A] Администратор %s заспавнил авто %s (цвет: %d, %d).", GetPlayerNameEx(playerid), vehname, color1, color2);
        SendMessageToAdmins(fmt_msg, 0xFF5533FF);
    #else
        #pragma unused playerid, vehname, color1, color2
    #endif
    return 1;
}

stock Vveh_SpawnVehicleForPlayer(playerid, modelid, vehname[], color1, color2)
{
    ...
    new veh = CreateVehicle(modelid, fx, fy, z, a, color1, color2, 0);
    ...
    new msg[160];
    format(msg, sizeof msg, ""SC"Транспорт %s (%d) создан (цвет: %d, %d).", vehname, modelid, color1, color2);
    SCM(playerid, 0x00FF99FF, msg);

    Vveh_AdminLogSpawn(playerid, vehname, color1, color2);
    return 1;
}

stock Vveh_ShowColorList(playerid)
{
    new body[2048];
    strcat(body, "Выберите цвет автомобиля:\n");
    strcat(body, "______________________________\n");
    
    for (new i = 0; i < sizeof(gColorNames); i++)
    {
        new row[64];
    ...
    new caption[128];
    format(caption, sizeof caption, "Выбор цвета для %s", gVvehName[playerid]);
    
    ShowPlayerDialog(playerid, VVEH_DLG_COLOR, DIALOG_STYLE_LIST, caption, body, "Выбрать", "Назад");
    return 1;
}
...
stock Vveh_ShowList(playerid)
{
    ...
    if (pages > 1)
    {
        if (gVvehPage[playerid] > 0)
            strcat(body, "Предыдущая страница\n");
        if (gVvehPage[playerid] < pages - 1)
            strcat(body, "Следующая страница\n");
    }

    new caption[96];
    format(caption, sizeof caption, "Создание автомобиля (стр. %d/%d)", gVvehPage[playerid] + 1, pages);

    ShowPlayerDialog(playerid, VVEH_DLG_LIST, DIALOG_STYLE_LIST, caption, body, "Выбрать", "Отмена");
    return 1;
}
...
stock bool:Vveh_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if (dialogid == VVEH_DLG_LIST)
    {
        ...
        if (pages > 1)
        {
            if (gVvehPage[playerid] > 0 && listitem == shown)
            {
                gVvehPage[playerid]--;
                Vveh_ShowList(playerid);
                return true;
            }
            else if (gVvehPage[playerid] < pages - 1)
            {
                new nextIndex = shown;
                if (gVvehPage[playerid] > 0) nextIndex++;
                
                if (listitem == nextIndex)
                {
                    gVvehPage[playerid]++;
                    Vveh_ShowList(playerid);
                    return true;
        ...
        gVvehModel[playerid] = gVvehIds[idx];
        format(gVvehName[playerid], 64, gVvehNames[idx]);
        
        Vveh_ShowColorList(playerid);
        return true;
    }
    
    if (dialogid == VVEH_DLG_COLOR)
    {
        if (!response)
        {
            Vveh_ShowList(playerid);
        ...
        if (listitem >= 0 && listitem < sizeof(gVehicleColors))
        {
            Vveh_SpawnVehicleForPlayer(
                playerid, 
                gVvehModel[playerid], 
                gVvehName[playerid],
                gVehicleColors[listitem][0],
                gVehicleColors[listitem][1]
            );
        }
        return true;
    }
