...
// BLACK BONUS fuel station GUI integration
// Connected to the existing fuel_stations system.

#if !defined BrDialogFuelFill
    #define BrDialogFuelFill GUIFuelFill
#endif
...
stock AZC_ShowFuelStation(playerid, vehicleid, stationid)
{
    ...
    SetPVarInt(playerid, "azc_gui_open", 1);

    new price = GetFuelStationData(stationid, FS_FUEL_PRICE);
    if(price < 1) price = 5;
...
stock AZC_ResetPlayerVars(playerid)
{
    ...
    DeletePVar(playerid, "azc_gui_open");
    DeletePVar(playerid, "azc_area_station");
    DeletePVar(playerid, "azc_processing");
    return 1;
}
...
stock AZC_DoRefuel(playerid, liters, fuel_type)
{
    ...
    if(station_price > 0) price_per_liter = station_price;
    if(price_per_liter < 1) price_per_liter = 5;
...
stock AZC_HandleFuelGui(playerid, Node:json)
{
    ...
    JSON_GetInt(json, "c", close_action);
    ...
    if(liters <= 0 && close_action > 1) liters = close_action;
    ...
    if(fuel_type < 0) fuel_type = 1;
    if(GetPVarInt(playerid, "azc_processing")) return 1;

    SetPVarInt(playerid, "azc_processing", 1);
    new result = AZC_DoRefuel(playerid, liters, fuel_type);
    DeletePVar(playerid, "azc_processing");
    return result;
}

stock AZC_TryOpenAtNearestStation(playerid, Float:distance = 15.0)
{
    if(GetPlayerState(playerid) != PLAYER_STATE_DRIVER || !IsPlayerDriver(playerid))
        return 0;

    new stationid = GetNearestFuelStation(playerid, distance);
    if(stationid == -1) return 0;

    return AZC_ShowFuelStation(playerid, GetPlayerVehicleID(playerid), stationid);
}

stock AZC_CloseFuelStation(playerid)
{
    if(GetPVarInt(playerid, "azc_gui_open"))
        HidePlayerGUI(playerid, BrDialogFuelFill);
...
{
    #pragma unused params
    SendClientMessage(playerid, 0xFFCD00FF, "BLACK BONUS AZS: /azc ili /azs - otkryt GUI zapravki");
...
CMD:addazc(playerid, params[])
{
    ...
    mysql_format(mysql, query, sizeof query,
        "INSERT INTO `fuel_stations` (`owner_id`,`name`,`improvements`,`fuels`,`fuel_price`,`buy_fuel_price`,`balance`,`rent_time`,`price`,`rent_price`,`lock`,`x`,`y`,`z`,`eviction`) VALUES (0,'BLACK BONUS AZS',0,5000,5,2,0,0,%d,%d,0,%f,%f,%f,0)",
        price,
        rent_price,
    ...
    format(g_fuel_station[idx][FS_NAME], 20, "BLACK BONUS AZS");
    ...
    new msg[96];
    format(msg, sizeof msg, "BLACK BONUS AZS created. ID: %d", idx);
    SendClientMessage(playerid, 0x66CC00FF, msg);
    return 1;
}
