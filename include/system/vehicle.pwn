...
// FIX: central bounds check for "model - 400" style indexing used across the
// gamemode (g_vehicle_info, MaxPassengers in anticheat.inc, etc). Every place
// that used to do GetVehicleInfo(model-400, ...) without checking the result
// was in range is the root cause of the reported OOB crashes (index far
// outside the real array size of 212 / 259). Use this before indexing.
stock bool: IsValidVehicleModelId(model)
{
    // This function means: "has an entry in g_vehicle_info".
    // It must NOT be used as a generic validator for CRMP custom models.
    if(model < 400)
        return false;

    new idx = model - 400;
    if(idx < 0 || idx >= sizeof(g_vehicle_info))
        return false;

    return true;
}

stock bool: IsKnownCustomVehicleModel(model)
{
    return (model >= 612 && model <= 30000);
}

stock bool: IsKnownVehicleModel(model)
{
    return IsValidVehicleModelId(model) || IsKnownCustomVehicleModel(model);
}
