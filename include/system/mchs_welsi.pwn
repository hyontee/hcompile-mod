...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    ...
    {
        if(!player_mchs_active{playerid}) ShowNotificationLaird(playerid, 2, 6, 0, 0, "Для начала начните смену.", "");
        Dialog(playerid, 3502, DSM, "Инвентарь МЧС", "Вы хотите взять огнетушитель?", "Взять", "Отмена");
    }
    if(areaid == veh_mchs)
    {
        if(!player_mchs_active{playerid}) return ShowNotificationLaird(playerid, 2, 6, 0, 0, "Сначала начните смену в МЧС.", "");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
            ...
            switch(listitem)
            {
                case 0:
                {
                    ...
                    else
                    {
                        ...
                        ShowNotificationLaird(playerid, 1, 6, 0, 0, "Вы начали смену. Возьмите огнетушитель и Т/С.", "");
                    }
                }
                case 1:
                {
    ...
    if(dialogid == 3502)
    {
        if(response)
        {
            ...
            ShowNotificationLaird(playerid, 2, 6, 0, 0, "Вы взяли огнетушитель.", "");
        }
    }
    if(dialogid == 3503)
    {
        if(response)
        {
            ...
            ShowNotificationLaird(playerid, 1, 6, 0, 0, "Вы успешно взяли рабочее Т/С.", "");
            ShowNotificationLaird(playerid, 2, 6, 0, 0, "Введите /meslist для просмотра доступных вызовов.", "");
        }
    }
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(PRESSED(KEY_FIRE))
	{
        if(press_fire{playerid}) return ShowNotificationLaird(playerid, 2, 6, 0, 0, "Продолжайте тушить очаг возгорания.", "");
			...
			if(IsPlayerInRangeOfAnyFire(playerid) >= 1)
			{
                ...
                if(all_fire[IsPlayerInRangeOfAnyFire(playerid)][F_ID_Z] == player_order[playerid])
                {
                    ...
                    { 
                       ...
                       ShowNotificationLaird(playerid, 1, 6, 0, 0, "Вы потушили очаг возгорания.", "");

                        if(Order(id))
                        {
                            ShowNotificationLaird(playerid, 1, 6, 0, 0, "Вы успешно выполнили заказ.", "");
                ...
                else ShowNotificationLaird(playerid, 2, 6, 0, 0, "Данный очаг возгорания не вашего заказа.", "");
			}
			else ShowNotificationLaird(playerid, 2, 6, 0, 0, "Не советуем использовать огнетушитель просто так.", "");
		}
	}
...
{
    if(!player_mchs_active{playerid}) return ShowNotificationLaird(playerid, 2, 6, 0, 0, "Вы не работаете в МЧС.", "");
    new Cache:result;
    ...
    if(!rows) return ShowNotificationLaird(playerid, 2, 6, 0, 0, "На данный момент заказов нет, ожидайте {FFFF00}'PayDay'", "");

    if(player_order[playerid]) return ShowNotificationLaird(playerid, 2, 6, 0, 0, "Вы уже взяли заказ.", "");
