...
#define MAX_BID_HAT 100_000_000 //максимальная ставка в Орел и Решка
#define MAX_BID_HAT_str "100.000.000" //максимальная ставка в Орел и Решка 
...
{
    printf("[W_SYSTEM] Игра \"Орёл и решка\" загружена");
...
cmd:hat(playerid, params[])
{
    ...
    if(sscanf(params, "d", to_player)) return SendClientMessage(playerid, -1, ""HAT_MSG"/hat [ID - игрока]");
...
cmd:accepthat(playerid, params[])
{
   ...
   // if(strlen(params) > 3) return SendClientMessage(playerid, -1, ""HAT_MSG"/accepthat [Yes / No]");
    ...
    else SendClientMessage(playerid, -1, ""HAT_MSG"/accepthat [Yes / No]");
...
stock AcceptHAT(playerid, to_player)
{
    ...
    SendClientMessage(to_player, -1, ""HAT_MSG"{D22323}Чтобы отказаться /accepthat No");
    SendClientMessage(playerid, -1, ""HAT_MSG"{D22323}Чтобы отозвать /accepthat No");
    SendClientMessage(to_player, -1, ""HAT_MSG"{45CF29}Чтобы принять /accepthat Yes");
