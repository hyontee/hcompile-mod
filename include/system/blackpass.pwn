...
stock BlackPass_GrantCaseReward(playerid, case_type, amount)
{
    ...
    switch(case_type)
    {
        case 1:
        {
            AddPlayerData(playerid, P_COUNT_TODAY_CASE, +, amount);
            UpdatePlayerDatabaseInt(playerid, "counttodaycases", GetPlayerData(playerid, P_COUNT_TODAY_CASE));
        }
        case 2:
        {
            AddPlayerData(playerid, P_COUNT_BOMJ_CASE, +, amount);
            UpdatePlayerDatabaseInt(playerid, "countbomjcases", GetPlayerData(playerid, P_COUNT_BOMJ_CASE));
        }
        case 3:
        {
            AddPlayerData(playerid, P_COUNT_STANDART_CASE, +, amount);
            UpdatePlayerDatabaseInt(playerid, "countstancases", GetPlayerData(playerid, P_COUNT_STANDART_CASE));
        }
        case 24:
        {
            AddPlayerData(playerid, P_COUNT_DOP_CASE1, +, amount);
            UpdatePlayerDatabaseInt(playerid, "countdopcases1", GetPlayerData(playerid, P_COUNT_DOP_CASE1));
        }
        default: return 0;
    }
    return 1;
}
stock BlackPass_GrantRewardDirect(playerid, itemid)
{
    new reward_type, internal, count, rarity, price;
    new reward_name[64];
    #pragma unused rarity
    #pragma unused price

    if(!BlackPass_GetRewardInventoryData(itemid, reward_name, sizeof(reward_name), reward_type, internal, count, rarity, price))
        return 0;
    ...
    switch(reward_type)
    {
        ...
        case 2: GivePlayerMoneyEx(playerid, count, "BlackPass reward", true, true);
        case 3: GivePlayerDonateRub(playerid, count, "BlackPass reward");
        case 4: result = BlackPass_GrantCaseReward(playerid, internal, count);
        case 5:
        {
            if(GetFreeOwnableCarID() == -1) result = 0;
            else BuyPlayerCar(playerid, internal, internal, 1);
        }
        case 8: BlackPass_SendX2Reward(playerid, count);
        case 9:
        {
        ...
        case 11:
        {
            ...
            if(free_slot == -1) result = 0;
            else if(count > 1) result = Inventory_AddItem(playerid, 134, free_slot, count);
            else result = Inventory_AddItem(playerid, internal, free_slot, 1);
        }
        case 21, 23: BlackPass_AddDust(playerid, count);
        case 34:
        {
            AddPlayerData(playerid, P_DONATE_CASE, +, count);
    ...
    {
        new notify[144];
        format(notify, sizeof(notify), "Награда BlackPass получена: %s", reward_name);
        ShowNotificationSile(playerid, 3, 5, 0, 0, notify, " ");
        BlackPass_LogEvent(playerid, "reward_grant", itemid, reward_type, internal, count, 0);
    }
    else
    {
        printf("[BLACKPASS][REWARD][ERROR] player=%d item=%d type=%d internal=%d count=%d", playerid, itemid, reward_type, internal, count);
        ShowNotificationSile(playerid, 2, 5, 0, 0, "Награду не удалось выдать: освободите место и повторите", " ");
    }
    return result;
}
...
stock BlackPass_QueueInventoryReward(playerid, itemid)
{
    ...
    // Типы без аналога в системе /reward (X2-бустер, "билеты") - выдаём мгновенно, как раньше
    if(reward_type == 8 || reward_type == 34)
        return BlackPass_GrantRewardDirect(playerid, itemid);

    new result;
    switch(reward_type)
    {
        case 1:  result = BPR_GiveReward(playerid, BPR_TYPE_EXP, 0, reward_name, rarity, count, 30, price, -1);
        case 2:  result = BPR_GiveReward(playerid, BPR_TYPE_MONEY, 0, reward_name, rarity, count, 30, price, -1);
        case 3:  result = BPR_GiveReward(playerid, BPR_TYPE_BC, 0, reward_name, rarity, count, 30, price, -1);
        case 4:  result = BPR_GiveCase(playerid, internal, reward_name, rarity, count);
        case 5:  result = BPR_GiveCar(playerid, internal, reward_name, rarity, 365, price);
        case 9:  result = BPR_GiveReward(playerid, BPR_TYPE_VIP, internal, reward_name, rarity, count, 30, price, -1);
        case 10: result = BPR_GiveReward(playerid, BPR_TYPE_BP_EXP, 0, reward_name, rarity, count, 30, price, -1);
        case 11:
        {
            if(internal == BPR_SKIN_EL)
                result = BPR_GiveSkin(playerid, count, reward_name, rarity, 365, price);
            else
                result = BPR_GiveAccessory(playerid, internal, -1, reward_name, rarity, 365, price);
        }
        case 21: result = BPR_GiveReward(playerid, BPR_TYPE_DUST, 0, reward_name, rarity, count, 30, price, -1);
        case 23: result = BPR_GiveReward(playerid, BPR_TYPE_EVENT_RES, 0, reward_name, rarity, count, 30, price, -1);
        default: return BlackPass_GrantRewardDirect(playerid, itemid);
    }

    if(result)
    {
        new notify[144];
        format(notify, sizeof(notify), "Награда BlackPass отправлена в /reward: %s", reward_name);
        ShowNotificationSile(playerid, 3, 5, 0, 0, notify, " ");
        BlackPass_LogEvent(playerid, "reward_queue", itemid, reward_type, internal, count, 0);
    }
    else
    {
        printf("[BLACKPASS][REWARD][ERROR] queue failed player=%d item=%d type=%d internal=%d count=%d", playerid, itemid, reward_type, internal, count);
        ShowNotificationSile(playerid, 2, 5, 0, 0, "Не удалось поставить награду в очередь /reward", " ");
    }
...
stock BlackPass_QueueDeluxeRewards(playerid, bool:skip_level_reward = true)
{
    for(new i = 1; i <= 4; i++)
    {
        ...
        if(!BlackPass_QueueInventoryReward(playerid, BlackPass_GetDeluxeInventoryItemId(i))) return 0;
    }
    return 1;
}
...
{
    return ShowNotificationSile(playerid, 3, 4, 0, 0, "Награды BlackPass теперь можно забрать в /reward", " ");
}
...
stock BlackPass_ClaimTaskReward(playerid, task_id)
{
    ...
    {
        ShowNotificationSile(playerid, 3, 3, 0, 0, "Вы успешно получили новый уровень SIMPLE PASS.", "qq");
...
stock BlackPass_HandlePurchasePremium(playerid, Node:json)
{
    ...
    if(is_deluxe)
    {
        ...
        {
            if(BlackPass_QueueDeluxeRewards(playerid, true))
            {
    ...
    else
    {
        ...
        BlackPass_SendMainPacket(playerid, json, 0);
        BlackPass_SendStateRefresh(playerid, 0, -1);
        BlackPass_ShowLootNotification(playerid, "Награды Premium выданы");
    }
    return 1;
}
