...
stock BlackPass_QueueInventoryReward(playerid, itemid)
{
    ...
    if(granted)
    {
        ShowNotificationSander(playerid, 3, 4, 0, 0, "Награда успешно отправлена в инвентарь!", "");
    }
...
{
    ShowNotificationSander(playerid, 3, 3, 0, 0, message, "");
    return 1;
}
...
{
    return ShowNotificationSander(playerid, 2, 4, 0, 0, "Добыча BlackPass временно отключена", "");
}
...
stock BlackPass_ClaimTaskReward(playerid, task_id)
{
    ...
    ShowNotificationSander(playerid, 3, 4, 0, 0, notify_str, "");

    if(BlackPass_GetCurrentLevel(playerid) > old_level)
    {
        ShowNotificationSander(playerid, 3, 3, 0, 0, "Вы успешно получили новый уровень Black Pass.", "");
...
stock BlackPass_BeginTrackTask(playerid, task_id)
{
    ...
    ShowNotificationSander(playerid, 3, 3, 0, 0, "Место отмечено у Вас на GPS", "");
...
stock BlackPass_HandleTaskExchange(playerid, Node:json)
{
    ...
    {
        ShowNotificationSander(playerid, 2, 3, 0, 0, "Недостаточно BC.", "");
        return 1;
    }
    ...
    {
        ShowNotificationSander(playerid, 2, 3, 0, 0, "Сейчас это задание нельзя заменить.", "");
        return 1;
    }
    ...
    ShowNotificationSander(playerid, 3, 3, 0, 0, "Вы потратили 10 BC и успешно сменили задание!", "");
...
stock BlackPass_HandleRefreshRating(playerid, Node:json)
{
    ...
    if(g_blackpass_rating_refresh_until[playerid] > current_time)
    {
        ...
        ShowNotificationSander(playerid, 2, 4, 0, 0, "Обновлять рейтинг можно раз в 30 секунд", "");
        return 1;
    }
...
CMD:clearbp(playerid, params[])
{
    ...
    if(GetPlayerAdminEx(playerid) < 12) return ShowNotificationSander(playerid, 2, 3, 0, 0, "У вас нет доступа к использованию данной команды", "");
