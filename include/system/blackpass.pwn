...
stock BlackPass_QueueInventoryReward(playerid, itemid)
{
    ...
    // BlackPass rewards are stored in the same Reward inventory as case rewards.
    // Case 99 is reserved for BlackPass rewards.
    if(!AddPlayerCaseReward(playerid, BLACKPASS_REWARD_CASE_ID, itemid, false))
    {
        printf("[BLACKPASS][REWARD][ERROR] player=%d item=%d: failed to add to Reward", playerid, itemid);
        return 0;
    }

    printf("[BLACKPASS][REWARD] player=%d item=%d added to Reward (case=%d)", playerid, itemid, BLACKPASS_REWARD_CASE_ID);
    return 1;
}
