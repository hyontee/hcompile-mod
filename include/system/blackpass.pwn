...
#define ShowClientNotification(%0,%1,%2,%3) ShowNotificationSile(%0, %1, %2, 0, 0, %3, "")
#define ShowNotificationNew(%0,%1,%2,%3) ShowNotificationSile(%0, %1, %2, 0, 0, %3, "")
...
{
    ShowClientNotification(playerid, 3, 3, message);
    return 1;
}
...
ShowNotificationNew(playerid, 3, 4, notify_str);

    if(BlackPass_GetCurrentLevel(playerid) > old_level)
    {
        ShowNotificationNew(playerid, 3, 3, "Вы успешно получили новый уровень Black Pass.");
...
stock BlackPass_BeginTrackTask(playerid, task_id)
{
    ...
    ShowNotificationNew(playerid, 3, 3, "Место отмечено у Вас на GPS");
...
stock BlackPass_HandleTaskExchange(playerid, Node:json)
{
    ...
    {
        ShowNotificationNew(playerid, 2, 3, "Недостаточно BC.");
        return 1;
    }
    ...
    {
        ShowNotificationNew(playerid, 2, 3, "Сейчас это задание нельзя заменить.");
        return 1;
    }
    ...
    ShowNotificationNew(playerid, 3, 3, "Вы потратили 10 ВС и {FFFF00}успешно\nсменили задание{FFFF00}{FFFFFF}!");
...
stock BlackPass_HandleRefreshRating(playerid, Node:json)
{
    ...
    if(g_blackpass_rating_refresh_until[playerid] > current_time)
    {
        ...
        ShowNotificationNew(playerid, 2, 4, "Обновлять рейтинг можно раз в 30 секунд");
        return 1;
    }
