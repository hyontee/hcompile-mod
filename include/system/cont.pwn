...
enum Cont_Item_Structure
{
	...
	ItemPrice, //Цена содержимого конта
}
...
//public OnPlayerEnterDynamicArea(playerid, areaid)
	...
	for(new cont=1; cont < MAX_CONT+1; cont++)
	{
		if(areaid == ContsZone[cont])
		{
			...
			{
		    	ShowNotification(playerid, 4, "Взаимодействовать", 5, "/cont", ">>");
			}
		}
	}
//----------------------------------------------
...
// public OnPlayerClickTextDraw(playerid, Text:clickedid)
    ...
    if(clickedid == cont_take) return 1; //SCM(playerid, CYELLOW, "| "CW"Данная функция недоступна ;3 "CY"| "CW"by BLACKBONUS");
				    ...
				    if(GetPlayerMoney(playerid) >= strval(inputtext))
				    {
				        if(strval(inputtext)-99999 > cInfo[cont][BetPrice])
				        {
							...
							else
							{
							    ...
							    SPD(playerid, 19996, DSI, ""CLR""SERVER_NAME""CW"| Битва за контейнер", strkbass,  "Готово", "Отмена");
...
stock StartCont()
{
	print("=== Доставка контейнеров в порт | by BLACKBONUS ===");
	LoadContItemList();
    StopCont();
    for(new cont=1; cont < MAX_CONT+1; cont++)
    {
...
stock SpawnCont(contid)
{
	...
	if(size == 0) return print("Нет предметов для региона %s.", regionstr);
    new item, limit;
    do
    {
        item = random(size);
    	...
    	limit++;
    	if(limit == size && cInfo[contid][ItemType] == 0) return printf("В регионе %s отсуствуют предметы!", regionstr);
    }
...
stock LoadTextDraw()
{
	...
	for(new cont = 1; cont < MAX_CONT+1; cont++)
	{
		cont_name[cont] = TextDrawCreate(294.799987, 121.457778, "by BLACKBONUS");
		...
		cont_model[cont] = TextDrawCreate(226.399902, 149.333343, "by BLACKBONUS");
...
{
	print("Загрузка предметов контейнера.");
	mysql_function_query(mysql, !"SET NAMES 'cp1251'", false, "", ""); // Руссификатор пон
	if(mysql_errno() == 0)
	{
	    for(new i=1; i <= 4; i++)
	    {
			...
			mysql_function_query(mysql, qString, true, "LoadContItemListInfo", "d", i);
		}
	}
	else
	{
 		print("ВНИМАНИЕ: Ошибка c подключением к базе данных.");
		//mysql_close(mysql);
	}
}
...
public LoadContItemListInfo(region)
{
    ...
    if(!rows)
    {
 		printf("AIEIAIEA: A aaca aaiiuo iao oiaa?ia aey eiioaeia?ia %d-ai ?aaeiia.", region);
    }
	else
	{
		if(rows <= sizeof cItem_Info[])
		{
		    ...
		    for(new i; i < rows; i++)
		    {
  				//region = cache_get_field_content_int(i, "region", mysql);
		        cItem_Info[region][i][Type] = cache_get_field_content_int(i, "type", mysql);
		        cItem_Info[region][i][ItemId] = cache_get_field_content_int(i, "item", mysql);
		        cache_get_field_content(i, "name", cItem_Info[region][i][Name], mysql, 30);
		        cItem_Info[region][i][ContPrice] = cache_get_field_content_int(i, "contprice", mysql);
		        cItem_Info[region][i][ItemPrice] = cache_get_field_content_int(i, "itemprice", mysql);
