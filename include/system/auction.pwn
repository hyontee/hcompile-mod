...
// Skin name helpers required by auction.
...
stock GetSkinName(skin_id)
{
    return GetSkinNameByGameId(skin_id);
}
    ...
    if(dialogid == 5992) // согласие
    {
        ...
        switch(GetPVarInt(playerid, "type_param"))
        {
            ...
            case TYPE_CLOTHES:
            {
                ...
                format(text, sizeof text, "Вы хотите выставить на аукцион одежду %s", GetSkinNameByGameId(server));

                SetPVarInt(playerid, "tovar_param", id_clothers);
                SetPVarInt(playerid, "tovar_param_server", server);

                format(name, 24, GetSkinNameByGameId(server));
