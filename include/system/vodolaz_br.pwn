// АВТОР СЛИВА СИСТЕМЫ VVEZ STUDIO
// СИСТЕМА РАССЧИТАНА НА МОДЫ ЛАИРДА, НО МБ И К ДРУГИМ ПОДОЙДЕТ
...
/* ИЩЕМ В ОСНОВНОМ МОДЕ P_ADMIN_STATUS, и ниже добавляем следующие строки:
...
*/ // ДАЛЕЕ ВСЕ ИДЕТ ЧЕРЕЗ ПОДКЛЮЧЕНИЕ ИНКЛУДА
...
for(new i = 0; i < sizeof(yg_areas); i++)
    {
        if(areaid == yg_areas[i])
        {
            ...
            {
                ShowNotificationXxesao(playerid, 4, 6, VODOLAZ_OFFER, 0, "Взаимодействовать", ">>");
                return callcmd::vodopremd(playerid);
                return 1;
            }
            else
            {
                ShowNotificationXxesao(playerid, 4, 6, VODOLAZ_OFFER, 0, "Предмет уже собран", " ");
                return 1;
            }
        }
    }
	...
	{
	ShowNotificationXxesao(playerid, 4, 6, VODOLAZKA_OFFER, 0, "Взаимодействовать", ">>");
...
if(GetPlayerLevel(playerid)<18) return ShowNotificationXxesao(playerid, 2, 6, 0, 0, "Для работы требуется 18 уровень!", "");
ShowPlayerDialog(playerid, 58901, DIALOG_STYLE_LIST, "{ed4b00}Капитан пирса", "{ed4b00}1.{fff7fc}Арендовать лодку \t\t\t\t{dbd5d9}Нажмите для аренды\n{ed4b00}2.{fff7fc}Посмотреть список заданий \t\t\t\t{dbd5d9}Нажмите для просмотра\n{ed4b00}3.{fff7fc}Продажа найденных предметов  \t\t\t\t{dbd5d9}Нажмите для продажи","Выбрать","Закрыть");
}
/*
