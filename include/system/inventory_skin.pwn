		//Автор: Welsi
		//Телеграмм канал автора(больше систем там): https://t.me/welsistudio

//в начало мода
#define GetSkinInfo(%0,%1)		g_skin_info[%0][%1]
#define GetSkinName(%0)			GetSkinInfo(S_NAME)
...
//В DIALOG_BIZ_CLOTHING_BUY после if(!mysql_errno())
//вместо SetPlayerData(playerid, P_SKIN, skinid);
GivePlayerOwnableSkin(playerid, skinid);

//к енуму диологов
DIALOG_OWNABLE_SKIN_LIST,
DIALOG_OWNABLE_SKIN_LOAD,
//к диологам
			case DIALOG_OWNABLE_SKIN_LIST:
			{
				if(response)
				{
					new idx = GetPlayerListitemValue(playerid, listitem);

					ShowOwnableSkinLoadDialog(playerid, idx, true);
				}
			}
			case DIALOG_OWNABLE_SKIN_LOAD:
			{
				if(response)
				{
					...
					switch(listitem + 1)
					{
						case 1:
						{
							...
							format(fmt_str, sizeof(fmt_str), "ID SKIN: %d", GetItemsInfo(skin_id, I_ID_OBJ));
...
//в DIALOG_CONTENER2
// вместо SetPlayerData(playerid, P_SKIN, skin_cont[id]);
GivePlayerOwnableSkin(playerid, skin_cont[id]);
...
stock ShowOwnableSkinLoadDialog(playerid, id)
{
	...
	Dialog
	(
		playerid, DIALOG_OWNABLE_SKIN_LOAD, DIALOG_STYLE_LIST,
...
CMD:myskins(playerid)
{
		...
		{	
				...
				Dialog
				(
					playerid, DIALOG_OWNABLE_SKIN_LIST, DIALOG_STYLE_LIST,
...
stock CheckSkinPlayer(playerid)
{
	...
	if(!rows)
	{
		...
		mysql_query(mysql, query, false);
		if(mysql_errno()) return SCM(playerid, -1, "ошибка");
