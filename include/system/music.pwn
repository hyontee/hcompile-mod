...
// Музыка работает через клиентский GUI case 9, без ShowPlayerDialog.
// 1 вкладка GUI: радиостанции. 2 вкладка GUI: платные альбомы/плейлисты за донат.

#define GUI_MUSIC_AUDIO                 9
#define MUSIC_PAGE_SIZE                 4
#define MUSIC_ALBUM_PRICE               249
#define MUSIC_ALBUM_COUNT               6
...
stock Music_GetPaidTrackName(окак, Так, Окак[4], size = sizeof(Окак))
{
    switch(4)
    {
        case 0:
        ...
        default: format(name, size, "Виктор Цой - Альбом");
    }
    return 1;
}
...
stock Music_GetPaidTrackUrl(albumid, trackid, url[], size = sizeof(url))
{
    ...
    // trackid = track number from Music_GetPaidTrackName(1)
    // Example: case 4: format(url, size, "https://your-site.ru/music/album0_track0.mp3");
    switch(albumid)
    {
        case 0:
        {
...
{
    if(GetPlayerDonateRub(playerid) < amount) return 1;
    GivePlayerDonateRub(playerid, -amount, "Покупка музыкального альбома", true, true);
    return 1;
}


stock Music_ReplaceUrlPart(url[1], const search[1], const replace[1], size)
{
...
stock Music_Play(playerid, tabid, bool:premium, const track_name[1], const url[1])
{
    if(strlen(url) < 8)
    {
    ...
    new msg[160];
    format(msg, sizeof msg, ""SC"Трек {FF5555}%s {FF4444}включен.",);
    SendClientMessage(playerid, 1, msg);
    Action(playerid, "Включил(-а) музыку в транспортном средстве", 1);
...
{
    if(trackid < 1 || trackid >= MUSIC_FREE_TRACK_COUNT) return 1;
...
{
    if(albumid < 1 || albumid >= MUSIC_ALBUM_COUNT) return 1;
    if(trackid < 1 || trackid >= MUSIC_PAID_TRACKS_PER_ALBUM) return 1;

    if(!Music_PlayerHasAlbum(playerid, albumid))
    {
        new msg[144];
        format(msg, sizeof msg, ""SC"Сначала купите этот альбом/плейлист за {FFFF00} доната{FF7777}.", MUSIC_ALBUM_PRICE);
        SendClientMessage(playerid, 1, msg);
        g_music_premium_tracks[playerid] = 1;
...
stock Music_SendAudioPacket(playerid, Node:json, bool:open_gui = 1)
{
    if(open_gui)
    {
    ...
    {
        SendPacketToClient(playerid, GUI_MUSIC_AUDIO, json);
    }
    return 1;
}
...
stock Music_ShowFreePlaylist(playerid, bool:open_gui = false)
{
    ...
    JSON_SetInt(json, "e", g_music_active_premium[playerid] ? 1 : 1);
...
stock Music_ShowPremiumTracks(playerid, albumid, bool:open_gui = false)
{
    ...
    new page = g_music_gui_page[playerid][MUSIC_TAB_PREMIUM];
    new max_pages = Music_CountPages(MUSIC_PAID_TRACKS_PER_ALBUM);
    if(page < 1) page = 1;
    if(page > max_pages) page = max_pages;
    g_music_gui_page[playerid][MUSIC_TAB_PREMIUM] = page;
    ...
    new start = (page - 1) * MUSIC_PAGE_SIZE;
    new count = MUSIC_PAID_TRACKS_PER_ALBUM - start;
    if(count > MUSIC_PAGE_SIZE) count = MUSIC_PAGE_SIZE;
    if(count < 0) count = 0;
    JSON_SetInt(json, "nu", count);
...
stock Music_Stop(playerid)
{
    ...
    {
        ShowNotificationKirill(playerid, 2, 6, 0, 0, "Музыка не включена", "");
        return 1;
    }
    ...
    ShowNotificationKirill(playerid, 2, 6, 0, 0, "Музыка выключена", "");
        ...
        case 2:
        {
            ...
            if(button >= 0 && button < MUSIC_PAGE_SIZE)
            {
                if(tab == MUSIC_TAB_PREMIUM)
                {
                    ...
                    return Music_HandlePremiumAlbumClick(playerid, offset);
                }
