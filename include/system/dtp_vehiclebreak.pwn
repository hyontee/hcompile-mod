...
//  • Двигатель НЕ глушим
//  • GUI 13 через ShowNotificationt (у тебя это уже точно работает)
...
//  1) Этот include должен подключаться ПОСЛЕ того, как объявлен ShowNotificationt.
...
// ShowNotificationt(playerid, type, duration, icon, subId, caption, btnCaption)
//  - type 4: без кнопки
//  - type 6: с кнопкой
stock DTP_Notify_NoButton(playerid, duration, caption[])
{
    ShowNotificationt(playerid, 4, duration, DTP_NOTIFY_ICON, 0, caption, "");
    return 1;
}

stock DTP_Notify_Button(playerid, duration, subId, caption[], btnCaption[])
{
    ShowNotificationt(playerid, 6, duration, DTP_NOTIFY_ICON, subId, caption, btnCaption);
    return 1;
}
