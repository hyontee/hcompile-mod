...
/* 
    Полная система ТК (Бат) 
    Собрано: Часть 1 + Часть 2 + GPS
*/
...
new TCOverdue; // Переменная для хранения времени окончания аренды
...
new LastSpawnTime[3]; // По количеству мест (у нас их 3)


new TCOwner[MAX_PLAYER_NAME] = "None"; // Имя владельца
new TCBalance; // Баланс ТК
new TCBuyPickup; // ID пикапа
...
// --- ALS Hook: OnGameModeInit ---
public OnGameModeInit()
{
    // 1. ЗАГРУЗКА ДАННЫХ ТК (строго после подключения!), то есть добавляете в свой мод в конец OnGameModeInit()
    mysql_tquery(mysql, "SELECT * FROM `trucking_company` LIMIT 1", "LoadTCData");

      SetTimer("CheckTCOverdue", 60000, true); // Запуск проверки каждую минуту (60000 мс)
   

    // 2.. СОЗДАНИЕ ПИКАПОВ
    TCBuyPickup = CreatePickup(1274, 23, 1.0810, 2508.8562, 2011.0451, 0); // Пикап покупки/управления
    ...
    // 3. СОЗДАНИЕ 3D ТЕКСТОВ
    Create3DTextLabel("{ffcc00}« {FFFFFF}Вход в офис {ffcc00}»", 0xFFFFFFFF, 2327.743164, 2009.634887, 16.660203, 10.0, 0, 1);	
    Create3DTextLabel("{ffcc00}« {FFFFFF}Выход {ffcc00}»", 0xFFFFFFFF, -0.200575, 2500.248291, 2011.045166, 10.0, 0, 1);
    Create3DTextLabel("{ffcc00}« {FFFFFF}Трудоустройство {ffcc00}»", 0xFFFFFFFF, 2.011036, 2503.443603, 2011.045166, 10.0, 0, 1);
    Create3DTextLabel("{ffcc00}« {FFFFFF}Автопарк {ffcc00}»", 0xFFFFFFFF, 2316.708007, 2012.259277, 16.161876, 10.0, 0, 1);
    Create3DTextLabel("{ffcc00}« {FFFFFF}Покупка транспортной компани {ffcc00}»", 0xFFFFFFFF, 1.0810, 2508.8562, 2011.0451, 10.0, 0, 1);
...
CMD:tcompany(playerid)
{
    ...
    new str[256];
    // Поменяли текст первого пункта
    format(str, sizeof(str), "1. Снять прибыль\n2. Продлить аренду\n3. Продать бизнес государству");
    ShowPlayerDialog(playerid, DLG_TC_MENU, DIALOG_STYLE_LIST, "{FFCC00}Управление ТК", str, "Выбрать", "Закрыть");
    return 1;
}
...
case DLG_TC_MENU: {
    if(response) {
        switch(listitem) {
            case 0: { // Пункт: Снять прибыль
                if(TCBalance <= 0) 
                    return SendClientMessage(playerid, 0xFF0000FF, "[Ошибка] На балансе ТК нет денег!");
                
                new str[128];
                format(str, sizeof(str), "{FFFFFF}На балансе предприятия: {00FF00}%d Руб.\n{FFFFFF}Введите сумму, которую хотите снять:", TCBalance);
                ShowPlayerDialog(playerid, DLG_TC_WITHDRAW, DIALOG_STYLE_INPUT, "{FFCC00}Снятие прибыли", str, "Снять", "Назад");
            }
            case 1: { // Пункт: Продлить аренду
                // Вычисляем сколько дней осталось до конца аренды
                new days = (TCOverdue - gettime()) / 86400; 
                if(days < 0) days = 0;
                
                new str[256];
                format(str, sizeof(str), 
                    "{FFFFFF}Ваша ТК оплачена еще на: {FFCC00}%d дн.\n\
                    {FFFFFF}Стоимость оплаты 1 дня - {FFCC00}1.000.000 Руб.\n\n\
                    {FFFFFF}Введите количество дней для продления (макс. до 7 дней всего):", days);
                
                ShowPlayerDialog(playerid, DLG_TC_PAY, DIALOG_STYLE_INPUT, "{FFCC00}Оплата аренды", str, "Оплатить", "Назад");
            }
            case 2: { // Пункт: Продать в гос
                new str[128];
                format(str, sizeof(str), "{FFFFFF}Вы уверены, что хотите продать ТК в гос. за {00FF00}250.000.000 Руб.{FFFFFF}?");
                ShowPlayerDialog(playerid, DLG_TC_SELL_GS, DIALOG_STYLE_MSGBOX, "{FF0000}Продажа бизнеса", str, "Продать", "Отмена");
            }
        }
    }
    return 1;
...
case DLG_TC_PAY: {
    if(response) {
        ...
        new price = days * 1000000; // Цена за день, например 1кк
        if(GetPlayerMoney(playerid) < price) return SendClientMessage(playerid, -1, "У вас недостаточно денег!");

        GivePlayerMoney(playerid, -price);
        
        // Если аренда уже кончилась, считаем от текущего времени, если нет — прибавляем к остатку
        ...
        new str[128];
        format(str, sizeof(str), "Вы продлили аренду на %d дн. за %dРублей.", days, price);
...
case DLG_TC_SELL_GS: {
    if(response) {
        GivePlayerMoney(playerid, 250000000); // Возвращаем половину стоимости (или сколько захочешь)
        format(TCOwner, MAX_PLAYER_NAME, "None");
        TCBalance = 0;
        
        SendClientMessage(playerid, 0xFFFF00FF, "Вы успешно продали бизнес государству.");

        // Обнуляем владельца в базе данных
        new query[128];
    ...
    return 1;
        ...
        case DLG_TC_BUY: {
            if(response) {
                if(GetPlayerMoney(playerid) < 500000000) 
                    return SendClientMessage(playerid, 0xFF0000FF, "[Ошибка] У вас нет 500.000.000Рублей!");

                GivePlayerMoney(playerid, -500000000);
                GetPlayerName(playerid, TCOwner, MAX_PLAYER_NAME);
                TCBalance = 0;
                
                SendClientMessage(playerid, 0x00FF00FF, "Вы купили транспортную компанию! Открыть меню управления можно через {ff6600}/tcompany");
                
                // Сохранение в БД (пример для MySQL R41+)
                new query[128];
        ...
        // --- НОВЫЙ: Меню управления для владельца ---
        case DLG_TC_OWNER: {
            if(response) {
                if(listitem == 0) { // Снять деньги
                    new str[128];
                    format(str, sizeof(str), "{FFFFFF}На балансе предприятия: {00FF00}%dРублей\n{FFFFFF}Введите сумму, которую хотите снять:", TCBalance);
        ...
        // --- НОВЫЙ: Ввод суммы для снятия ---
case DLG_TC_WITHDRAW: {
if(!response) return OnPlayerCommandText(playerid, "/tcompany");

    new amount = strval(inputtext);
    if(amount <= 0 || amount > TCBalance) {
        new str[150];
        format(str, sizeof(str), "{FF0000}[Ошибка] Неверная сумма! {FFFFFF}Доступно: {00FF00}%d Руб.", TCBalance);
        return ShowPlayerDialog(playerid, DLG_TC_WITHDRAW, DIALOG_STYLE_INPUT, "{FFCC00}Снятие прибыли", str, "Снять", "Назад");
    }
    ...
    // Сохранение нового баланса в базу данных
    new query[128];
...
case DLG_TC_RENT: {
    if(response) {
        ...
        if(spawnIdx == -1) return SendClientMessage(playerid, -1, "{FF0000}[Ошибка] {FFFFFF}Все площадки заняты. Подождите немного.");
    ...
    return 1;
        ...
        case DLG_TC_ZAKAZ: {
            if(response) {
                ...
                switch(listitem) {
                    case 0: { SetPlayerRaceCheckpoint(playerid, 2, 1735.4753, 2258.5187, 15.4875, 0.0, 0.0, 0.0, 5.0); SetGPSMarker(playerid, 1735.4753, 2258.5187, 15.4875); }
                    case 1: { SetPlayerRaceCheckpoint(playerid, 2, 2381.7846, 1728.0380, 13.2791, 0.0, 0.0, 0.0, 5.0); SetGPSMarker(playerid, 2381.7846, 1728.0380, 13.2791); }
                    case 2: { SetPlayerRaceCheckpoint(playerid, 2, 1661.1229, 1330.0316, 12.3625, 0.0, 0.0, 0.0, 5.0); SetGPSMarker(playerid, 1661.1229, 1330.0316, 12.3625); }
                }
...
// --- ALS Hook: OnPlayerEnterRaceCheckpoint ---
public OnPlayerEnterRaceCheckpoint(playerid)
{
    if(IsLoading[playerid]) return 1;

    if(TC_Checkpoint[playerid] == 1) { 
        IsLoading[playerid] = true; TogglePlayerControllable(playerid, 0);
...
public TCLoading(playerid, percent)
{
    ...
    if(percent <= 100) { // Логика ЗАГРУЗКИ
        ...
        if(percent == 100) {
            ...
            if(ZakazID[playerid] == 1) { SetPlayerRaceCheckpoint(playerid, 2, 2276.2939, -739.6619, 12.9967, 0.0, 0.0, 0.0, 5.0); SetGPSMarker(playerid, 2276.2939, -739.6619, 12.9967); }
            else if(ZakazID[playerid] == 2) { SetPlayerRaceCheckpoint(playerid, 2, -992.3968, 2151.2387, 44.1668, 0.0, 0.0, 0.0, 5.0); SetGPSMarker(playerid, -992.3968, 2151.2387, 44.1668); }
            else if(ZakazID[playerid] == 3) { SetPlayerRaceCheckpoint(playerid, 2, -1327.7851, -1545.6258, 60.4284, 0.0, 0.0, 0.0, 5.0); SetGPSMarker(playerid, -1327.7851, -1545.6258, 60.4284); }
        ...
        if(r_percent == 100) {
            ...
            if(strcmp(TCOwner, "None", true) != 0) 
            {
                ...
                // Поиск владельца в сети для уведомления
                for(new i = GetPlayerPoolSize(); i >= 0; i--)
                {
                    ...
                    {
                        new msg[144];
                        format(msg, sizeof(msg), "{FFCC00}[Бизнес] {FFFFFF}Сотрудник завершил заказ. На баланс ТК зачислено: {00FF00}%dРублей", commission);
...
{
    if(strcmp(TCOwner, "None", true) == 0) // Если ТК свободна
    {
        ShowPlayerDialog(playerid, 1004, DIALOG_STYLE_MSGBOX, "Покупка ТК", 
            "{FFFFFF}Вы действительно хотите приобрести ТК за {00FF00}500.000.000Рублей{FFFFFF}?", 
            "Купить", "Отмена");
    }
    else // Если куплена
    {
        new str[128];
        format(str, sizeof(str), "{FF0000}Транспортная компания уже куплена!\n{FFFFFF}Владелец: {FFFF00}%s", TCOwner);
...
// Эту функцию (callback) добавь в любое место мода вне других функций
forward LoadTCData();
public LoadTCData()
{
    new rows, fields;
    ...
    if(rows > 0)
    {
        // 1. Загружаем имя владельца
        cache_get_field_content(0, "owner", TCOwner, mysql, MAX_PLAYER_NAME);
        
        // 2. Загружаем баланс
        new temp_bal[32];
        cache_get_field_content(0, "balance", temp_bal, mysql, sizeof(temp_bal));
        TCBalance = strval(temp_bal);

        // 3. Загружаем время окончания аренды (overdue)
        new temp_ov[32];
        cache_get_field_content(0, "overdue", temp_ov, mysql, sizeof(temp_ov));
        TCOverdue = strval(temp_ov);

        // ПРОВЕРКА НА СЛЕТ:
        // Если владелец есть, но время аренды меньше текущего времени сервера
        if(strcmp(TCOwner, "None", true) != 0 && TCOverdue < gettime())
        {
            printf("[Система ТК] Срок аренды истек (%d). Сброс владельца.", TCOverdue);
            
            format(TCOwner, MAX_PLAYER_NAME, "None");
            TCBalance = 0;
            TCOverdue = 0;

            // Обнуляем данные в базе данных сразу
            mysql_tquery(mysql, "UPDATE trucking_company SET owner = 'None', balance = 0, overdue = 0");
        }
        else 
        {
            printf("[Система ТК] Данные загружены. Владелец: %s, Баланс: %dРублей, Оплачена до (Unix): %d", TCOwner, TCBalance, TCOverdue);
        }
    }
    else
    {
        printf("[Система ТК] Записи в БД не найдены. Создаю пустую запись...");
        // Если в таблице пусто, можно вставить дефолтную строку через mysql_tquery
        format(TCOwner, MAX_PLAYER_NAME, "None");
        TCBalance = 0;
        TCOverdue = 0;
    }
...
forward PutPlayerInRentVehicle(playerid, vehicleid);
public PutPlayerInRentVehicle(playerid, vehicleid)
{
    // Проверяем, в сети ли игрок и существует ли еще машина
...
// Объявляем перехват (hook)
public OnPlayerStateChange(playerid, newstate, oldstate)
{
    if(newstate == PLAYER_STATE_DRIVER) 
    {
        // Проверяем, является ли авто арендованным этим игроком
        if(TC_Vehicle[playerid] != INVALID_VEHICLE_ID && GetPlayerVehicleID(playerid) == TC_Vehicle[playerid])
        {
            ...
            SendClientMessage(playerid, -1, "Приятной поездки! Используйте {ff6600}/zakaz {FFFFFF}для работы.");
        }
    }
