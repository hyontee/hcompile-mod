...
stock FireSystemFinishCall(playerid, bool:success = true)
{
    ...
    new owner = fire_call_owner[callid];
    new partner = fire_call_partner[callid];
    new owner_vehicleid = INVALID_VEHICLE_ID;
    new partner_vehicleid = INVALID_VEHICLE_ID;
    ...
    {
        owner_vehicleid = fire_player_gazel_vehicle[owner];
        if(owner_vehicleid == INVALID_VEHICLE_ID)
        {
            owner_vehicleid = fire_player_driver_vehicle[owner];
        }
    }

    if(partner != INVALID_PLAYER_ID)
    {
        partner_vehicleid = fire_player_gazel_vehicle[partner];
        if(partner_vehicleid == INVALID_VEHICLE_ID)
        {
            partner_vehicleid = fire_player_driver_vehicle[partner];
        }
    }
    ...
    if(success)
    {
        ...
        if(owner != INVALID_PLAYER_ID && IsPlayerConnected(owner))
        {
            ...
            FireSystemCloseTutorialGUI(owner);
        }
        ...
        if(partner != INVALID_PLAYER_ID && IsPlayerConnected(partner))
        {
            ...
            FireSystemCloseTutorialGUI(partner);
        }

        FireSystemBroadcastOrders();
    }
    else
    {
        fire_call_enabled[callid] = true;
    ...
    if(owner != INVALID_PLAYER_ID && IsPlayerConnected(owner))
    {
        FireSystemStartRetTimer(owner, owner_vehicleid);
    }

    if(partner != INVALID_PLAYER_ID && IsPlayerConnected(partner))
    {
        FireSystemStartRetTimer(partner, partner_vehicleid);
    }
    return 1;
}
...
stock FireSystemShowCallsMenu(playerid)
{
    ...
    (
        ...
        "{E0584B}МЧС {FFFFFF}-> Список вызовов",
        body,
        "Принять",
        "Выход"
    );
...
stock FireSystemTryExtinguish(playerid)
{
    ...
    for(new slot; slot < FIRE_CALL_OBJECT_COUNT; slot++)
    {
        ...
        FireSystemConsumeExtinguisherAmmo(playerid, 1);
...
stock FireSystemBroadcastOrders()
{
    new text[144];
    format(text, sizeof text, "{FFFF00}Диспетчер: {FFFFFF}Появились новые заказы. Используйте {FFFF00}/meslist {FFFFFF}чтобы взять вызов.");
...
stock FireSystemSyncExtData(playerid, bool:save_changes = true)
{
    ...
    if(current_weapon != FIRE_WEAPON_EXTINGUISHER)
    {
...
stock FireSystemConsumeExtinguisherAmmo(playerid, amount = 1)
{
    if(amount <= 0)
        return 1;
    ...
    fire_player_extinguisher_count[playerid] -= amount;
    if(fire_player_extinguisher_count[playerid] < 0)
        fire_player_extinguisher_count[playerid] = 0;
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == FIRE_DIALOG_GAZEL_RENT)
    {
        ...
        ShowNotificationNew(playerid, 3, 5, -1, 0, "Вы успешно арендовали автомобиль {FFFF00}Газель.", "");
        return 1;
    }
...
public FireSystemVehicleReturnTimer(playerid)
{
    ...
    {
        FireSystemClearVehicleReturnTimer(playerid);
        SendClientMessage(playerid, -1, "{FF6347}Вы не вернулись в автомобиль. Рабочий день МЧС завершен.");
        FireSystemEndWorkDay(playerid);
        return 1;
    }
...
stock FireSystemStartRetTimer(playerid, vehicleid)
{
    ...
    SendClientMessage(playerid, -1, "{FFFF00}Вернитесь в автомобиль в течение 60 секунд.");
    FireSystemShowVehicleReturnText(playerid);
    ...
    if(newstate == PLAYER_STATE_DRIVER && oldstate != PLAYER_STATE_DRIVER)
    {
        ...
        if(FireSystemIsGazelleVehicle(vehicleid))
        {
            if(!fire_player_on_duty[playerid])
            {
                ...
                if(!fire_player_employed[playerid])
                {
                    ShowNotificationNew(playerid, 2, 5, -1, 0, "Транспорт доступен только для сотрудникам МЧС.", "");
                }
...
public OnPlayerTakeDamage(playerid, issuerid, Float:amount, weaponid, bodypart)
{
    ...
    #if defined firesys_OnPlayerTakeDamage
        return firesys_OnPlayerTakeDamage(playerid, issuerid, amount, weaponid, bodypart);
    #else
        return 1;
    #endif
}
...
#endif
#define OnPlayerTakeDamage firesys_OnPlayerTakeDamage
#if defined firesys_OnPlayerTakeDamage
    forward firesys_OnPlayerTakeDamage(playerid, issuerid, Float:amount, weaponid, bodypart);
#endif
