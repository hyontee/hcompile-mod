...
static F_OldSkin[MAX_PLAYERS];
static F_Timer[MAX_PLAYERS]; // Для таймеров
...
stock F_StartCrafting(playerid) {
    ...
    F_Timer[playerid] = SetTimerEx("OnF_CraftingFinished", 8000, false, "i", playerid);
}
...
public OnGameModeInit() {
    f_enter_pickup = CreatePickup(19130, 1, -978.7820, 2070.2219, 45.3795, -1);
    f_exit_pickup = CreatePickup(19130, 1, -31.8808, -3.9729, 1381.0601, -1);
    f_job_pickup = CreatePickup(1275, 1, -25.1597, -10.9273, 1381.0596, -1);
...
// ИСПОЛЬЗУЕМ СТАНДАРТНЫЙ OnPlayerPickUpPickup (БЕЗ Ex)
    ...
    if(pickupid == f_job_pickup) {
        if(F_IsWorking[playerid]) {
            if(F_Salary[playerid] > 0.0) {
                ...
                GivePlayerMoney(playerid, total_money);
                new str[128];
                format(str, sizeof(str), "{00FF00}Вы закончили смену. {FFFFFF}Заработано: %d руб.", total_money);
                SendClientMessage(playerid, -1, str);
            } else {
                SendClientMessage(playerid, -1, "Вы закончили работу, но ничего не заработали.");
            }

            SetPlayerSkin(playerid, F_OldSkin[playerid]);
            ...
            if(F_Timer[playerid] != 0) {
                KillTimer(F_Timer[playerid]);
                F_Timer[playerid] = 0;
            }
        } else {
            F_OldSkin[playerid] = GetPlayerSkin(playerid);
...
// ОТКЛЮЧАЕМ OnPlayerUpdate (основная причина крашей)
...
#endif
#define OnPlayerPickUpPickup F_OnPlayerPickUpPickup
#if defined F_OnPlayerPickUpPickup
    forward F_OnPlayerPickUpPickup(playerid, pickupid);
#endif
