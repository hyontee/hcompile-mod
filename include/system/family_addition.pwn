...
new CAPTURE_PLAYER_EXP[MAX_PLAYERS][5];
...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    if(areaid == CAPTURE_AREA[0] || areaid == CAPTURE_AREA[1] || areaid == CAPTURE_AREA[2] || areaid == CAPTURE_AREA[3] || areaid == CAPTURE_AREA[4])
    {
        ...
        // Теперь безопасно использовать f и вызывать функции
        if(!show_panel_capt[playerid]) 
            ShowFamilyCaptPanel(playerid, GetPlayerCaptArea(playerid));
    } 
...
stock RecalculateCaptureLeader(type)
{
    ...
    for(new f; f < MAX_FAM; f++)
    {
        if(GetFamily(f, family_database) == -1) continue;
        if(CAPTURE_FAMILY_EXP[f][type] <= CAPTURE_LEADER[type][1]) continue;

        CAPTURE_LEADER[type][0] = f;
        CAPTURE_LEADER[type][1] = CAPTURE_FAMILY_EXP[f][type];
    }
    return 1;
}

stock ResetPlayerCaptureExp(playerid)
{
    for(new type; type < sizeof(CAPTURE_STATUS); type++)
        CAPTURE_PLAYER_EXP[playerid][type] = 0;
...
public OnPlayerLeaveDynamicArea(playerid, areaid)
{
    if(areaid == CAPTURE_AREA[0] || areaid == CAPTURE_AREA[1] || areaid == CAPTURE_AREA[2] || areaid == CAPTURE_AREA[3] || areaid == CAPTURE_AREA[4]) 
    {
        ...
        new lost_exp = CAPTURE_PLAYER_EXP[playerid][type_capt];

        printf("[DEBUG] Игрок %d покидает зону type:%d | Family EXP: %d | Player EXP: %d", 
            playerid, type_capt, CAPTURE_FAMILY_EXP[f][type_capt], lost_exp);

        HideFamilyCaptPanel(playerid, type_capt);

        if(lost_exp > 0)
        {
            if(CAPTURE_FAMILY_EXP[f][type_capt] >= lost_exp)
                CAPTURE_FAMILY_EXP[f][type_capt] -= lost_exp;
            else
                CAPTURE_FAMILY_EXP[f][type_capt] = 0;

            new string[164];
            format(string, sizeof string, "Игрок %s[%d] покинул территорию сражения и потерял %d очков", GetPlayerNameEx(playerid), playerid, lost_exp);
            SendFamilyMessage(f, string);
        }

        CAPTURE_PLAYER_EXP[playerid][type_capt] = 0;
        UpdateExpFamily(f, type_capt);
        RecalculateCaptureLeader(type_capt);
    }
...
stock StartFCapture(type_capt)
{
    ...
    for(new i;i < welsi;i++)
    {
        ...
        foreach(new w : Player) CAPTURE_PLAYER_EXP[w][e[i]] = 0;
...
public OnPlayerDisconnect(playerid, reason)
{
    ...
    if(f != -1 && f >= 0 && f < MAX_FAM)
    {
        for(new type; type < sizeof(CAPTURE_STATUS); type++)
        {
            new lost_exp = CAPTURE_PLAYER_EXP[playerid][type];
            if(lost_exp <= 0) continue;

            if(CAPTURE_FAMILY_EXP[f][type] >= lost_exp)
                CAPTURE_FAMILY_EXP[f][type] -= lost_exp;
            else
                CAPTURE_FAMILY_EXP[f][type] = 0;

            if(CAPTURE_STATUS[type])
            {
                format(f_string144, sizeof f_string144, "Игрок %s[%d] вышел на сражении и потерял %d очков", GetPlayerNameEx(playerid), playerid, lost_exp);
                SendFamilyMessage(f, f_string144);
            }

            CAPTURE_PLAYER_EXP[playerid][type] = 0;
            RecalculateCaptureLeader(type);
        }
    }

    ResetPlayerCaptureExp(playerid);
        ...
        case 0:
        {
            if(!hours)
            {
                ...
                if(GetDayOfWeek(year, month, day) == 4)
                {
                    ...
                    mysql_tquery(mysql, "UPDATE family SET reputation = 0"); /*PERF: sync->async, fire-and-forget write*/
...
{
    new victim_family = GetPlayerIdFamily(playerid);
    new capt = GetPlayerCaptArea(playerid);

    if(victim_family != -1 && victim_family >= 0 && victim_family < MAX_FAM && capt != -1 && CAPTURE_STATUS[capt])
    {
        new lost_exp = CAPTURE_PLAYER_EXP[playerid][capt];

        if(lost_exp > 0)
        {
            new string[164];

            if(killerid != INVALID_PLAYER_ID && killerid != playerid && IsPlayerConnected(killerid) && IsPlayerInDynamicArea(killerid, CAPTURE_AREA[capt]))
            {
                new killer_family = GetPlayerIdFamily(killerid);

                if(killer_family != -1 && killer_family >= 0 && killer_family < MAX_FAM && killer_family != victim_family)
                {
                    format(string, sizeof string, "Игрок %s[%d] убил врага и заработал %d очков", 
                        GetPlayerNameEx(killerid), killerid, lost_exp);
                    SendFamilyMessage(killer_family, string);

                    CAPTURE_FAMILY_EXP[killer_family][capt] += lost_exp;
                    CAPTURE_PLAYER_EXP[killerid][capt] += lost_exp;
                }
            }

            format(string, sizeof string, "Игрок %s[%d] был убит на сражении и потерял %d очков", 
                GetPlayerNameEx(playerid), playerid, lost_exp);
            SendFamilyMessage(victim_family, string);

            if(CAPTURE_FAMILY_EXP[victim_family][capt] >= lost_exp)
                CAPTURE_FAMILY_EXP[victim_family][capt] -= lost_exp;
            else
                CAPTURE_FAMILY_EXP[victim_family][capt] = 0;

            CAPTURE_PLAYER_EXP[playerid][capt] = 0;
            RecalculateCaptureLeader(capt);
        }
    }
...
stock EndFCapture(type)
{
    ...
    foreach(new i : Player) CAPTURE_PLAYER_EXP[i][type] = 0;
