        ...
        if(response)
        {
            ...
            if(!GetPVarInt(playerid, "selection_listitem"))
            {
                ...
                (
                    ...
                    "Гоночные дуэли | Вознограждениe",
                    "Напишите какое вознограждение получит {FFFF00}победитель\n"\
                    "{FFFFFF}Если вы хотите без вознаграждения напишите {FFFF00}0\n"\
                    "{FFFFFF}Число не должно превышать 10.000.000 рублей",
                    "Далее", "Назад"
                );
            }
            else
    ...
    foreach(new x : Player)
    {
        if(player_opponent[x] == p1 || player_opponent[x] == p2)
        {
            if(plant_1 == -1)
            {
                ...
                new engine = (GetVehicleParam(vehicle, V_ENGINE) ^ VEHICLE_PARAM_ON);
