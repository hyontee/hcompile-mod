...
public: welsi_LoadFamily()
{
    ...
    for(new i; i < rows; i ++)
    {
        ...
        SetFamily(i, family_count_veh, 0);
        ...
        if(GetFamily(i, family_slot_veh) < 5)
        {
            SetFamily(i, family_slot_veh, 5);
            mysql_format(mysql, f_string64, sizeof f_string64, "UPDATE family SET slot_veh=5 WHERE id=%d LIMIT 1", GetFamily(i, family_database));
        ...
        SetFamily(i, family_lvl_storage, cache_get_field_content_int(i, "lvl_storage"));
        SetFamily(i, family_lvl_weapon, cache_get_field_content_int(i, "lvl_weapon"));
        SetFamily(i, family_lvl_compound, cache_get_field_content_int(i, "lvl_compound"));
        ...
        for(new l;l < 5;l++)
        {
            sscanf(rang[l], "P<,>s[12]dddddd", name, setting_1, setting_2, setting_3, setting_4, setting_5, setting_6);
    ...
    for(new i; i < MAX_FAM;i++)
    {
        ...
        "(SELECT COUNT(*) FROM family_cars WHERE family_owner = %d) AS cars_count", GetFamily(i, family_database), GetFamily(i, family_database));
        new Cache:ccache = mysql_query(mysql, f_string184);

        if(cache_num_rows(ccache))
        {
...
stock Family_GetPlayerMedkitCount(playerid)
{
	new account_id = GetPlayerAccountID(playerid);
	if(account_id <= 0) return 0;

	new query[196];
	mysql_format(mysql, query, sizeof(query), "SELECT IFNULL(SUM(amount),0) AS total FROM inventory WHERE account_id = %d AND item_id = 22", account_id);
            ...
            case 7:
            {
                if(!take_or_put)
                {
                    if(GetFamily(f, family_heath_kit))
                    {
                        ...
                        new total_meds = Family_GetPlayerMedkitCount(playerid);

                        if(total_meds >= 5)
                            return SendClientMessage(playerid, -1, ""c_f_r"У вас не может быть больше 5 аптечек в инвентаре.");

                        new med_slot = Inventory11_FindFirstItemSlotById(playerid, 22);

                        if(med_slot != -1)
                        {
                            new query[256];
                            mysql_format(mysql, query, sizeof(query),
                                "UPDATE `inventory` SET `amount` = `amount` + 1 WHERE `account_id` = %d AND `slot` = %d AND `item_id` = 22 LIMIT 1",
                                GetPlayerAccountID(playerid), med_slot);
                            mysql_tquery(mysql, query);
                        }
                        else
                        {
                            if(Inventory11_AddItemToDatabase(playerid, 22, 0, 1, 0, 0) == -1)
                ...
                else
                {
                    ...
                    new left_amount;
                    if(!Inventory11_ConsumeMedkitBySlot(playerid, med_slot, left_amount))
                        return SendClientMessage(playerid, -1, ""c_f_r"Не удалось забрать аптечку из инвентаря.");
    ...
    if(dialogid == 2238)
    {
        if(response)
        {
            ...
            if(GetPlayerMoneyEx(playerid) >= 3_000_000)
            {
                ...
                mysql_format(mysql, f_string144, sizeof f_string144, "INSERT INTO family (owner,name,slot_veh,count_veh,count_people) VALUES ('%d','%s',5,0,1)", GetPlayerAccountID(playerid), fam_name);
                mysql_query(mysql, f_string144, false);
                ...
                if(!mysql_errno())
                {
                    ...
                    SetFamily(free_id, family_lvl_storage, cache_get_field_content_int(0, "lvl_storage"));
                    SetFamily(free_id, family_lvl_weapon, cache_get_field_content_int(0, "lvl_weapon"));
                    SetFamily(free_id, family_lvl_compound, cache_get_field_content_int(0, "lvl_compound"));
                    ...
                    mysql_format(mysql, f_string64, sizeof f_string64, "UPDATE family SET count_veh=0,count_people=%d,slot_veh=5 WHERE id=%d LIMIT 1", GetFamily(free_id, family_count_people), GetFamily(free_id, family_database));
                    mysql_query(mysql, f_string64, false);
    ...
    if(dialogid == 2243)
    {
        ...
        if(!response)
        {
            if(IsPlayerConnected(inviter))
                SendClientMessage(inviter, -1, ""c_f"\xc8\xe3\xf0\xee\xea \xee\xf2\xea\xe7\xe0\xeb\xf1\xff \xee\xf2 \xe2\xe0\xf8\xe5\xe3\xee \xef\xf0\xe8\xe4\xeb\xe0\xf8\xe5\xed\xe8\xff \xe2 \xf1\xe5\xec\x8c\x8e.");
            SendClientMessage(playerid, -1, ""c_f"\xc2\xfb \xee\xf2\xea\xe0\xe7\xe0\xeb\xe8\xf1\xfc \xee\xf2 \xef\xf0\xe8\xe3\xeb\xe0\xf8\xe5\xed\xe8\xff.");
        ...
        {
            DeletePVar(playerid, "invite_player");
            return SendClientMessage(playerid, -1, ""c_f_r"\xc8\xe3\xf0\xee\xea, \xf1\xe4\xe5\xeb\xe0\xe2\xf8\xe8\xe9 \xef\xf0\xe8\xe3\xeb\xe0\xf8\xe5\xed\xe8\xe5, \xe2\xfb\xf8\xe5\xeb \xe8\xe7 \xe8\xe3\xf0\xfb.");
        }

        new f = GetPlayerIdFamily(inviter);
        if(f == -1)
            return SendClientMessage(playerid, -1, ""c_f_r"\xd1\xe5\xec\x8c\x8f \xf3\xe6\xe5 \xed\xe5 \xf1\xf3\xf9\xe5\xf1\xf2\xe2\xf3\xe5\xf2.");

        if(GetPlayerIdFamily(playerid) != -1)
            return SendClientMessage(playerid, -1, ""c_f_r"\xd3 \xe2\xe0\xf1 \xf3\xe6\xe5 \xe5\xf1\xf2\xfc \xf1\xe5\xec\x8c\x8f.");

        new compound_level = GetFamily(f, family_lvl_compound);
        if(compound_level < 1) compound_level = 1;
        if(compound_level > 7) compound_level = 7;
        if(GetFamily(f, family_count_people) + 1 > count_compound_level[compound_level-1])
            return SendClientMessage(playerid, -1, ""c_f_r"\xd1\xee\xf1\xf2\xe0\xe2 \xf1\xe5\xec\x8c\xe8 \xf3\xe6\xe5 \xef\xee\xeb\xee\xed.");

        SetPlayerFamily(playerid, ID_FAMILY, f);
        SetPlayerFamily(playerid, ID_SQL_FAMILY, GetFamily(f, family_database));
        ...
        UpdatePlayerDatabaseInt(playerid, "family_id", GetFamily(f, family_database));
        ...
        UpdateColumnFamilyInt(GetFamily(f, family_database), "count_people", GetFamily(f, family_count_people));

        format(f_string144, sizeof f_string144, "%s[%d] \xef\xf0\xe8\xed\xff\xeb \xe2 \xf1\xe5\xec\x8c\x8e %s", GetPlayerNameEx(playerid), playerid, GetFamily(f, family_name));
        SendFamilyMessage(f, f_string144);
        SendFamilyLog(type_log_invite, inviter, GetPlayerAccountID(playerid), f_string144);

        LoadPlayerFamily(playerid);
        ShowNotificationNew(playerid, 2, 5, -1, -1, "\xc2\xfb \xef\xf0\xe8\xed\xff\xf2\xfb \xe2 \xf1\xe5\xec\x8c\x8e", "\xc2\xfb \xe0\xe2\xf2\xee\xec\xe0\xf2\xe8\xf7\xe5\xf1\xea\xe8 \xef\xee\xeb\xf3\xf7\xe8\xeb\xe8 1 \xf0\xe0\xed\xe3.");
        if(IsPlayerConnected(inviter))
            ShowNotificationNew(inviter, 2, 5, -1, -1, "\xc8\xe3\xe3\xf0\xee\xea \xef\xf0\xe8\xed\xff\xeb \xe2\xe0\xf8\xf3 \xef\xf0\xe8\xe3\xeb\xe0\xf8\xe5\xed\xe8\xe5", "\xd2\xe5\xef\xe5\xf0\xfc \xe2 \xf1\xe5\xec\x8c\xe5 \xe5\xf1\xf2\xfc \xed\xee\xe2\xfb\xe9 \xf3\xf7\xe0\xf1\xf2\xed\xe8\xea.");
...
stock MoveOwnableCar(playerid, database_car, fam)
{
    ...
    if(fam < 0 || fam >= MAX_FAM)
        fam = fam_id;

    if(GetFamily(fam_id, family_database) <= 0)
        return 0;

    if(GetFamily(fam_id, family_count_veh) >= GetFamily(fam_id, family_slot_veh))
        return SendClientMessage(playerid, -1, "No free family vehicle slots");
    ...
    new pt_engine, pt_brake, pt_stability;
    new comfort, sport, sport_plus, drift;
    new comfort1, sport1, drift1, sport_plus1;
    new comfort2, sport2, drift2, sport_plus2;
    new comfort3, sport3, drift3, sport_plus3;
    new comfort4, sport4, drift4, sport_plus4;
    new mc_vinil_current, mc_exhsuast_current, mc_strab_current;
    new mc_enable_neon, mc_horn_current, mc_horn_id;
    new highlight_type, stroboscope_type, exhaust_type;
    new is_hydro_active, is_hydro_bought;
    new is_farlight_active, is_farlight_bought;
    new is_pnevmo_bought, pnevmo_selected;
    new mc_pdvradar, launch_v, nitro_v, nitro_level;
    new mc_wheel_offset_ft, mc_wheel_offset_rr, mc_align_ft, mc_align_rr;
    new mc_width_ft, mc_width_rr, mc_wheel_radius, mc_wheel_colors, mc_wheel_comp;
    new mc_lights_color, mc_underlights_color, mc_underlights_color_lf, mc_underlights_color_rt;
    new mc_body_colors, mc_tonir_ft, mc_tonir_rr;
    new mc_vynil_name, mc_vinyl_id;
    new mc_supercharger_v, mc_pt_turbo, mc_color_body_r, mc_color_body_g, mc_color_wheels;
    new mc_toner_front, mc_toner_rear, mc_light_color;
    new mc_neon_center, mc_neon_left, mc_neon_right;

    pt_engine        = cache_get_field_content_int(0, "pt_engine");
    pt_brake         = cache_get_field_content_int(0, "pt_brake");
    pt_stability     = cache_get_field_content_int(0, "pt_stability");
    comfort          = cache_get_field_content_int(0, "comfort");
    sport            = cache_get_field_content_int(0, "sport");
    sport_plus       = cache_get_field_content_int(0, "sport_plus");
    drift            = cache_get_field_content_int(0, "drift");
    comfort1         = cache_get_field_content_int(0, "comfort1");
    sport1           = cache_get_field_content_int(0, "sport1");
    drift1           = cache_get_field_content_int(0, "drift1");
    sport_plus1      = cache_get_field_content_int(0, "sport_plus1");
    comfort2         = cache_get_field_content_int(0, "comfort2");
    sport2           = cache_get_field_content_int(0, "sport2");
    drift2           = cache_get_field_content_int(0, "drift2");
    sport_plus2      = cache_get_field_content_int(0, "sport_plus2");
    comfort3         = cache_get_field_content_int(0, "comfort3");
    sport3           = cache_get_field_content_int(0, "sport3");
    drift3           = cache_get_field_content_int(0, "drift3");
    sport_plus3      = cache_get_field_content_int(0, "sport_plus3");
    comfort4         = cache_get_field_content_int(0, "comfort4");
    sport4           = cache_get_field_content_int(0, "sport4");
    drift4           = cache_get_field_content_int(0, "drift4");
    sport_plus4      = cache_get_field_content_int(0, "sport_plus4");
    mc_vinil_current    = cache_get_field_content_int(0, "vinil_current");
    mc_exhsuast_current = cache_get_field_content_int(0, "exhsuast_current");
    mc_strab_current    = cache_get_field_content_int(0, "strab_current");
    mc_enable_neon      = cache_get_field_content_int(0, "EnableNeon");
    mc_horn_current     = cache_get_field_content_int(0, "horn_current");
    mc_horn_id          = cache_get_field_content_int(0, "horn_id");
    highlight_type   = cache_get_field_content_int(0, "highlight_type");
    stroboscope_type = cache_get_field_content_int(0, "stroboscope_type");
    exhaust_type     = cache_get_field_content_int(0, "exhaust_type");
    is_hydro_active  = cache_get_field_content_int(0, "is_hydro_active");
    is_hydro_bought  = cache_get_field_content_int(0, "is_hydro_bought");
    is_farlight_active = cache_get_field_content_int(0, "is_FarLight_active");
    is_farlight_bought  = cache_get_field_content_int(0, "is_FarLight_bought");
    is_pnevmo_bought = cache_get_field_content_int(0, "is_pnevmo_bought");
    pnevmo_selected  = cache_get_field_content_int(0, "pnevmo_selected");
    mc_pdvradar         = cache_get_field_content_int(0, "pdvradar");
    launch_v         = cache_get_field_content_int(0, "launch");
    nitro_v          = cache_get_field_content_int(0, "nitro");
    nitro_level      = cache_get_field_content_int(0, "nitro_level");
    mc_wheel_offset_ft  = cache_get_field_content_int(0, "wheel_offset_ft");
    mc_wheel_offset_rr  = cache_get_field_content_int(0, "wheel_offset_rr");
    mc_align_ft         = cache_get_field_content_int(0, "align_ft");
    mc_align_rr         = cache_get_field_content_int(0, "align_rr");
    mc_width_ft         = cache_get_field_content_int(0, "width_ft");
    mc_width_rr         = cache_get_field_content_int(0, "width_rr");
    mc_wheel_radius     = cache_get_field_content_int(0, "wheel_radius");
    mc_wheel_colors     = cache_get_field_content_int(0, "wheel_colors");
    mc_wheel_comp       = cache_get_field_content_int(0, "wheel_comp");
    mc_lights_color     = cache_get_field_content_int(0, "lights_color");
    mc_underlights_color    = cache_get_field_content_int(0, "underlights_color");
    mc_underlights_color_lf = cache_get_field_content_int(0, "underlights_color_lf");
    mc_underlights_color_rt = cache_get_field_content_int(0, "underlights_color_rt");
    mc_body_colors      = cache_get_field_content_int(0, "body_colors");
    mc_tonir_ft         = cache_get_field_content_int(0, "tonir_ft");
    mc_tonir_rr         = cache_get_field_content_int(0, "tonir_rr");
    mc_vynil_name       = cache_get_field_content_int(0, "vynil_name");
    mc_vinyl_id         = cache_get_field_content_int(0, "diski");
    mc_supercharger_v   = cache_get_field_content_int(0, "supercharger");
    mc_pt_turbo         = cache_get_field_content_int(0, "pt_turbo");
    mc_color_body_r     = cache_get_field_content_int(0, "color_body_r");
    mc_color_body_g     = cache_get_field_content_int(0, "color_body_g");
    mc_color_wheels     = cache_get_field_content_int(0, "color_wheels");
    mc_toner_front      = cache_get_field_content_int(0, "toner_front");
    mc_toner_rear       = cache_get_field_content_int(0, "toner_rear");
    mc_light_color      = cache_get_field_content_int(0, "light_color");
    mc_neon_center      = cache_get_field_content_int(0, "neon_center");
    mc_neon_left        = cache_get_field_content_int(0, "neon_left");
    mc_neon_right       = cache_get_field_content_int(0, "neon_right");
...
(
    ...
    pt_engine, pt_brake, pt_stability, comfort, sport, sport_plus, drift, \
    comfort1, sport1, drift1, sport_plus1, comfort2, sport2, drift2, sport_plus2, \
    comfort3, sport3, drift3, sport_plus3, comfort4, sport4, drift4, sport_plus4, \
    vinil_current, exhsuast_current, strab_current, EnableNeon, horn_current, horn_id, \
    highlight_type, stroboscope_type, exhaust_type, is_hydro_active, is_hydro_bought, \
    is_FarLight_active, is_FarLight_bought, is_pnevmo_bought, pnevmo_selected, pdvradar, \
    launch, nitro, nitro_level, wheel_offset_ft, wheel_offset_rr, align_ft, align_rr, \
    width_ft, width_rr, wheel_radius, wheel_colors, wheel_comp, lights_color, \
    underlights_color, underlights_color_lf, underlights_color_rt, body_colors, tonir_ft, tonir_rr, \
    vynil_name, diski, supercharger, pt_turbo, color_body_r, color_body_g, color_wheels, \
    toner_front, toner_rear, light_color, neon_center, neon_left, neon_right) \
    VALUES \
    ...
    comfort1, sport1, drift1, sport_plus1, comfort2, sport2, drift2, sport_plus2,
    comfort3, sport3, drift3, sport_plus3, comfort4, sport4, drift4, sport_plus4,
    mc_vinil_current, mc_exhsuast_current, mc_strab_current, mc_enable_neon, mc_horn_current, mc_horn_id,
    highlight_type, stroboscope_type, exhaust_type, is_hydro_active, is_hydro_bought,
    is_farlight_active, is_farlight_bought, is_pnevmo_bought, pnevmo_selected, mc_pdvradar,
    launch_v, nitro_v, nitro_level, mc_wheel_offset_ft, mc_wheel_offset_rr, mc_align_ft, mc_align_rr,
    mc_width_ft, mc_width_rr, mc_wheel_radius, mc_wheel_colors, mc_wheel_comp, mc_lights_color,
    mc_underlights_color, mc_underlights_color_lf, mc_underlights_color_rt, mc_body_colors, mc_tonir_ft, mc_tonir_rr,
    mc_vynil_name, mc_vinyl_id, mc_supercharger_v, mc_pt_turbo, mc_color_body_r, mc_color_body_g, mc_color_wheels,
    mc_toner_front, mc_toner_rear, mc_light_color, mc_neon_center, mc_neon_left, mc_neon_right
);
    mysql_query(mysql, query, false);
    if(mysql_errno())
    {
        new sql_error = mysql_errno();
        if(sql_error == 1054 || sql_error == 1146 || sql_error == 1064)
        {
            // Some servers already have an old/minimal family_cars table.
            // Fall back to the guaranteed base columns so the vehicle is still moved.
            mysql_format(mysql, query, sizeof query,
                "INSERT INTO family_cars (family_owner, model_id, color_1, color_2, pos_x, pos_y, pos_z, angle, create_time, number, region, number_type, rang) "\
                "VALUES (%d, %d, %d, %d, %f, %f, %f, %f, %d, '%s', '%s', %d, 1)",
                fam, model_id, color_1, color_2, pos_x, pos_y, pos_z, angle, gettime(), number, region, number_type);
            mysql_query(mysql, query, false);
            if(mysql_errno())
            {
                cache_delete(cache_sql);
                SendClientMessage(playerid, -1, "Failed to add vehicle to family");
                return 0;
            }
        }
        else
    ...
    mysql_format(mysql, f_string64, sizeof f_string64, "DELETE FROM ownable_cars WHERE id=%d LIMIT 1", database_car);
    mysql_query(mysql, f_string64, false);

    if(mysql_errno())
    {
        cache_delete(cache_sql);
        SendClientMessage(playerid, -1, "Failed to remove personal vehicle record");
        return 0;
    }

    {
        ...
        UpdateColumnFamilyInt(GetFamily(fam_id, family_database), "count_veh", GetFamily(fam_id, family_count_veh));
    }
    cache_delete(cache_sql);
    return 1;
}
...
stock LoadFamilyCar(database)
{
    ...
    if(vehicleid != INVALID_VEHICLE_ID)
    {
        ...
        LoadVehicleTuningFromCache(vehicleid, free_id, result);
...
stock LoadVehicleTuningFromCache(vehicleid, familycarid, Cache:result)
{
    #pragma unused familycarid
    ...
    new Float:brake_power;
    switch(cache_get_field_content_int(0, "pt_brake"))
    {
        case 1: brake_power = 1.5;
        case 2: brake_power = 2.0;
        case 3: brake_power = 2.5;
        case 4: brake_power = 3.0;
    }
    if(brake_power != 0.0)
    {
        vInfo[i][vBrakePower] = brake_power;
        ApplyBrakePower(vehicleid, brake_power);
    }

    if(cache_get_field_content_int(0, "highlight_type"))
    {
        vInfo[i][vHighLights] = 299;
        SetVehicleHighLight(vehicleid, 299, 1);
    }

    vInfo[i][vStrob] = 0;
    SetVehicleStroboscope(vehicleid, 5, 0);
    new strob_type = cache_get_field_content_int(0, "stroboscope_type");
    if(strob_type)
    {
        vInfo[i][vStrob] = strob_type;
        SetVehicleStroboscope(vehicleid, strob_type, 1);
    }

    vInfo[i][vExhaustSound] = cache_get_field_content_int(0, "exhaust_type");
    if(vInfo[i][vExhaustSound] != 0)
        SetVehicleExhaust(vehicleid, vInfo[i][vExhaustSound], 1);

    vInfo[i][vHornSound] = cache_get_field_content_int(0, "horn_id");
    SetVehicleHornSound(vehicleid, 0, 0);

    new vinyl_id = cache_get_field_content_int(0, "diski");
    if(vinyl_id != 0)
        SetVehicleVinyl(vehicleid, vinyl_id, 1);

    if(cache_get_field_content_int(0, "pt_turbo"))
    {
        new Float:speed_bonus = float(cache_get_field_content_int(0, "pt_turbo")) * 0.08;
        vInfo[i][vMaxSpeed] = vInfo[i][vMaxSpeed] * (1.0 + speed_bonus);
        SetVehicleMaxSpeed(vehicleid, vInfo[i][vMaxSpeed]);
    }

    if(cache_get_field_content_int(0, "pt_engine"))
    {
        new Float:speed_bonus = float(cache_get_field_content_int(0, "pt_engine")) * 0.07;
        vInfo[i][vMaxSpeed] = vInfo[i][vMaxSpeed] * (1.0 + speed_bonus);
        SetVehicleMaxSpeed(vehicleid, vInfo[i][vMaxSpeed]);
    }

    if(cache_get_field_content_int(0, "supercharger"))
    {
        new Float:speed_bonus = float(cache_get_field_content_int(0, "supercharger")) * 0.09;
        vInfo[i][vMaxSpeed] = vInfo[i][vMaxSpeed] * (1.0 + speed_bonus);
        SetVehicleMaxSpeed(vehicleid, vInfo[i][vMaxSpeed]);
    }

    vInfo[i][vLaunchControl]   = cache_get_field_content_int(0, "launch");
    vInfo[i][vNitro]           = cache_get_field_content_int(0, "nitro");
    vInfo[i][IsHydroActive]    = cache_get_field_content_int(0, "is_hydro_active");
    vInfo[i][IsHydroBought]    = cache_get_field_content_int(0, "is_hydro_bought");
    vInfo[i][IsFarLightActive] = cache_get_field_content_int(0, "is_FarLight_active");
    vInfo[i][IsFarLightBought] = cache_get_field_content_int(0, "is_FarLight_bought");
    vInfo[i][SelectedPnevmo]   = cache_get_field_content_int(0, "pnevmo_selected");
    vInfo[i][IsPnevmoBought]   = cache_get_field_content_int(0, "is_pnevmo_bought");
    vInfo[i][pdvradar]         = cache_get_field_content_int(0, "pdvradar");

    vInfo[i][wheel_offset_ft] = cache_get_field_content_int(0, "wheel_offset_ft");
    if(vInfo[i][wheel_offset_ft] == 0) vInfo[i][wheel_offset_ft] = 50;
    vInfo[i][wheel_offset_rr] = cache_get_field_content_int(0, "wheel_offset_rr");
    if(vInfo[i][wheel_offset_rr] == 0) vInfo[i][wheel_offset_rr] = 50;
    vInfo[i][align_ft] = cache_get_field_content_int(0, "align_ft");
    vInfo[i][align_rr] = cache_get_field_content_int(0, "align_rr");
    vInfo[i][width_ft] = cache_get_field_content_int(0, "width_ft");
    if(vInfo[i][width_ft] == 0) vInfo[i][width_ft] = 40;
    vInfo[i][width_rr] = cache_get_field_content_int(0, "width_rr");
    if(vInfo[i][width_rr] == 0) vInfo[i][width_rr] = 40;
    vInfo[i][wheel_radius] = cache_get_field_content_int(0, "wheel_radius");
    vInfo[i][wheel_colors] = cache_get_field_content_int(0, "wheel_colors");
    vInfo[i][wheel_comp]   = cache_get_field_content_int(0, "wheel_comp");

    vInfo[i][lights_color]           = cache_get_field_content_int(0, "lights_color");
    vInfo[i][underlights_color]      = cache_get_field_content_int(0, "underlights_color");
    vInfo[i][underlights_color_lf]   = cache_get_field_content_int(0, "underlights_color_lf");
    vInfo[i][underlights_color_rt]   = cache_get_field_content_int(0, "underlights_color_rt");
    vInfo[i][body_colors]            = cache_get_field_content_int(0, "body_colors");
    vInfo[i][tonir_ft]               = cache_get_field_content_int(0, "tonir_ft");
    vInfo[i][tonir_rr]               = cache_get_field_content_int(0, "tonir_rr");

    vInfo[i][vComfort]     = cache_get_field_content_int(0, "comfort");
    vInfo[i][vSport]       = cache_get_field_content_int(0, "sport");
    vInfo[i][vDrift]       = cache_get_field_content_int(0, "drift");
    vInfo[i][vSportPlus]   = cache_get_field_content_int(0, "sport_plus");
    vInfo[i][vEnableNeon]  = cache_get_field_content_int(0, "EnableNeon");
    vInfo[i][exhsuast_current] = cache_get_field_content_int(0, "exhsuast_current");
    vInfo[i][strab_current]    = cache_get_field_content_int(0, "strab_current");
    vInfo[i][horn_current]     = cache_get_field_content_int(0, "horn_current");
    vInfo[i][vinil_current]    = cache_get_field_content_int(0, "vinil_current");

    vInfo[i][vComfort1]    = cache_get_field_content_int(0, "comfort1");
    vInfo[i][vSport1]      = cache_get_field_content_int(0, "sport1");
    vInfo[i][vDrift1]      = cache_get_field_content_int(0, "drift1");
    vInfo[i][vSportPlus1]  = cache_get_field_content_int(0, "sport_plus1");

    vInfo[i][vComfort2]    = cache_get_field_content_int(0, "comfort2");
    vInfo[i][vSport2]      = cache_get_field_content_int(0, "sport2");
    vInfo[i][vDrift2]      = cache_get_field_content_int(0, "drift2");
    vInfo[i][vSportPlus2]  = cache_get_field_content_int(0, "sport_plus2");

    vInfo[i][vComfort3]    = cache_get_field_content_int(0, "comfort3");
    vInfo[i][vSport3]      = cache_get_field_content_int(0, "sport3");
    vInfo[i][vDrift3]      = cache_get_field_content_int(0, "drift3");
    vInfo[i][vSportPlus3]  = cache_get_field_content_int(0, "sport_plus3");

    vInfo[i][vComfort4]    = cache_get_field_content_int(0, "comfort4");
    vInfo[i][vSport4]      = cache_get_field_content_int(0, "sport4");
    vInfo[i][vDrift4]      = cache_get_field_content_int(0, "drift4");
    vInfo[i][vSportPlus4]  = cache_get_field_content_int(0, "sport_plus4");

    new hex = cache_get_field_content_int(0, "light_color");
    if(hex != 0) SetVehicleLightColor(vehicleid, hex, 1);

    new toner_front = cache_get_field_content_int(0, "toner_front");
    if(toner_front)
    {
        vInfo[i][vTonerFront] = toner_front;
        SetVehicleTint(vehicleid, toner_front, 1);
    }

    new toner_rear = cache_get_field_content_int(0, "toner_rear");
    if(toner_rear)
    {
        vInfo[i][vTonerRear] = toner_rear;
        SetVehicleTint(vehicleid, toner_rear, 1);
    }

    new neon_center = cache_get_field_content_int(0, "neon_center");
    new neon_left   = cache_get_field_content_int(0, "neon_left");
    new neon_right  = cache_get_field_content_int(0, "neon_right");
    if(neon_center != 0 || neon_left != 0 || neon_right != 0)
        SetVehicleLightingColor(vehicleid, neon_center, neon_right, neon_left, 1);


    return 1;
}

stock SyncVehicleTuningForPlayer(playerid, vehicleid)
{
    if(!IsPlayerConnected(playerid) || !IsValidVehicle(vehicleid)) return 0;


    UpdateVehicleDamageStatus(vehicleid, playerid);
    return 1;
}
        ...
        case 3:
{
    ...
    if(item_id == 0)
    {
        ...
        if(Inventory11_AddItemToDatabase(playerid, reward_server_id, 0, 1, 0, 0) == -1)
        {
            ShowNotificationNew(playerid, 2, 7, -1, -1, "Нет свободного места в инвентаре!", "");
            return 1;
        }
    ...
    else
    {
        ...
        if(Inventory11_AddItemToDatabase(playerid, item_server_id, 0, 1, 0, 0) == -1)
        {
            ShowNotificationNew(playerid, 2, 7, -1, -1, "Нет свободного места в инвентаре!", "");
            return 1;
        }
            ...
            if(s_value == 1)
            {
                 switch(id_value)
                 {
              			...
              			if(GetFamily(f, family_money) >= price_up_sklad[GetFamily(f, family_lvl_storage)])
                        {
                            ...
                            UpdateColumnFamilyInt(GetFamily(f, family_database), "lvl_storage", GetFamily(f, family_lvl_storage));
                            UpdateColumnFamilyInt(GetFamily(f, family_database), "money", GetFamily(f, family_money));
           			  ...
           			  if(GetFamily(f, family_money) >= price_up_weapon[GetFamily(f, family_lvl_weapon)])
                        {
                            ...
                            UpdateColumnFamilyInt(GetFamily(f, family_database), "lvl_weapon", GetFamily(f, family_lvl_weapon));
                            UpdateColumnFamilyInt(GetFamily(f, family_database), "money", GetFamily(f, family_money));
           			  ...
           			  if(GetFamily(f, family_money) >= price_up_compound[GetFamily(f, family_lvl_compound)])
                        {
                            ...
                            UpdateColumnFamilyInt(GetFamily(f, family_database), "lvl_compound", GetFamily(f, family_lvl_compound));
                            UpdateColumnFamilyInt(GetFamily(f, family_database), "money", GetFamily(f, family_money));
        ...
        case 5:
{
    ...
    new fid = GetPlayerIdFamily(playerid);
    if(fid == -1)
    {
    ...
    if(id_value == 0)
    {
        ...
        new lvl_storage = GetFamily(fid, family_lvl_storage);
        new lvl_weapon = GetFamily(fid, family_lvl_weapon);
        new lvl_compound = GetFamily(fid, family_lvl_compound);

        new idx_storage = (lvl_storage > 0) ? (lvl_storage - 1) : 0;
        new idx_weapon = (lvl_weapon > 0) ? (lvl_weapon - 1) : 0;

        JSON_SetInt(response, "t", 5);
        JSON_SetInt(response, "r", GetFamily(fid, family_reputation));
        JSON_SetInt(response, "am", GetFamily(fid, family_count_people));
        JSON_SetInt(response, "s", GetPlayerSkinEx(playerid));

        new Node:myNode = JSON_Array(JSON_Int(GetFamily(fid, family_money)), JSON_Int(count_money_level[idx_storage]));
        new Node:mkNode = JSON_Array(JSON_Int(GetFamily(fid, family_material)), JSON_Int(count_material_level[idx_storage]));
        new Node:msNode = JSON_Array(JSON_Int(GetFamily(fid, family_mask)), JSON_Int(count_mask_level[idx_storage]));
        new Node:kbNode = JSON_Array(JSON_Int(0), JSON_Int(0));
        new Node:pnNode = JSON_Array(JSON_Int(GetFamily(fid, family_patron)), JSON_Int(count_patron_level[idx_weapon]));
        new Node:btNode = JSON_Array(JSON_Int(GetFamily(fid, family_armour)), JSON_Int(count_armour_level[idx_storage]));
        new Node:ugNode = JSON_Array(
            JSON_Int(lvl_storage), JSON_Int(7),
            JSON_Int(lvl_weapon), JSON_Int(7),
            JSON_Int(lvl_compound), JSON_Int(7)
        );
    ...
    else if(id_value == 3)
{
    ...
    if(r_value == 1)
    {
   	JSON_SetInt(response, "s", 1);
        if(s_value == 1)
        {
            ShowNotificationNew(playerid, 2, 5, -1, -1, "Склад временно недоступен.", "");
            JSON_SetInt(response, "s", 1);
        }
    }
    else if(r_value == 0)
        ...
        case 9:
{
    ...
    else if(s_value == 2)
    {
        ...
        if(current_donate >= price)
        {
            ...
            JSON_SetInt(response, "s", 2);
            JSON_SetInt(response, "r", 1);
            JSON_SetInt(response, "m", GetPlayerData(playerid, P_FAMILY_TOKEN));
        ...
        else
        {
            ...
            JSON_SetInt(response, "need", price - current_donate);
...
alias:famcreate("createfam", "fcreate")
CMD:famcreate(playerid, params[])
{
    if(GetPlayerIdFamily(playerid) != -1)
        return SendClientMessage(playerid, -1, ""c_f_r"Вы уже состоите в семье.");

    if(GetPlayerMoneyEx(playerid) < 3_000_000)
        return SendClientMessage(playerid, -1, ""c_f_r"Для создания семьи нужно 3 000 000 рублей.");
    ...
    ShowPlayerDialog(playerid, 2238, DIALOG_STYLE_INPUT, "{FF0000}Создание семьи",
        "Введите название семьи (от 6 до 32 символов).\nСоздание стоит {FFB700}3 000 000{FFFFFF} рублей.",
        "Создать", "Отмена");
    return 1;
}
...
CMD:family(playerid, params[])
{
    new f = GetPlayerIdFamily(playerid);
    if(f == -1)
        return ShowNotificationNew(playerid, 2, 6, -1, -1, "Вы не состоите в семье.", "");
...
CMD:faminvite(playerid, params[])
{
    ...
    if(f == -1)
        return SendClientMessage(playerid, -1, ""c_f"\xd3 \xe2\xe0\xf1 \xed\xe5\xf2\xf3 \xf1\xe5\xec\x8c\xe8.");

    new rank = GetPlayerRangFamily(playerid);
    if(rank < 1 || rank > 5)
        return SendClientMessage(playerid, -1, ""c_f_r"\xc2\xe0\xf8\xe5\xec\xf3 \xf0\xe0\xed\xe3\xf3 \xed\xe5\xe4\xee\xf1\xf2\xf3\xef\xed\xee \xef\xf0\xe8\xed\xe8\xec\xe0\xf2\xfc \xe8\xe3\xf0\xee\xea\xee\xe2.");

    if(family_rang_dostup[f][rank-1][0])
        return SendClientMessage(playerid, -1, ""c_f_r"\xc2\xe0\xf8\xe5\xec\xf3 \xf0\xe0\xed\xe3\xf3 \xed\xe5\xe4\xee\xf1\xf2\xf3\xef\xed\xee \xef\xf0\xe8\xed\xe8\xec\xe0\xf2\xfc \xe8\xe3\xf0\xee\xea\xee\xe2.");

    new to_player;
    if(sscanf(params, "d", to_player))
        return SendClientMessage(playerid, -1, ""c_f_r"/faminvite [ID \xe8\xe3\xf0\xee\xea\xe0]");

    if(!IsPlayerConnected(to_player) || to_player == playerid)
        return SendClientMessage(playerid, -1, ""c_f_r"\xc8\xe3\xf0\xee\xea \xed\xe5\xf2 \xe2 \xf1\xe5\xf2\xe8.");

    if(GetPlayerIdFamily(to_player) != -1)
        return SendClientMessage(playerid, -1, ""c_f_r"\xd3 \xe8\xe3\xf0\xee\xea\xe0 \xf3\xe6\xe5 \xe5\xf1\xf2\xfc \xf1\xe5\xec\x8c\x8f.");
    ...
    if(GetFamily(f, family_count_people) + 1 > count_compound_level[compound_level-1])
        return SendClientMessage(playerid, -1, ""c_f_r"\xd1\xee\xf1\xf2\xe0\xe2 \xf1\xe5\xec\x8c\xe8 \xef\xee\xeb\xee\xed.");

    if(!IsPlayerInRangeOfPlayer(playerid, to_player, 5.0))
        return SendClientMessage(playerid, 0xCECECEFF, "\xc8\xe3\xf0\xee\xea \xf1\xeb\xe8\xf8\xea\xee\xec \xe4\xe0\xeb\xe5\xea\xee");

    format(f_string144, sizeof f_string144, "{FFFF00}%s{FFFFFF} \xef\xf0\xe8\xe3\xeb\xe0\xf8\xe0\xe5\xf2 \xe2\xe0\xf1 \xe2 \xf1\xe5\xec\x8c\x8e %s",
        GetPlayerNameEx(playerid), GetFamily(f, family_name));

    DeletePVar(to_player, "invite_player");
    ShowPlayerDialog(to_player, 2243, DIALOG_STYLE_MSGBOX,
        "{FF0000}\xcf\xf0\xe8\xe3\xeb\xe0\xf8\xe5\xed\xe8\xe5 \xe2 \xf1\xe5\xec\x8c\x8e",
        f_string144, "\xcf\xf0\xe8\xed\x8f\xf2\xfc", "\xce\xf2\xea\xe0\xe7\xe0\xf2\xfc");

    SetPVarInt(to_player, "invite_player", playerid);

    ShowNotificationNew(to_player, 2, 5, -1, -1, "\xcf\xf0\xe8\xe3\xeb\xe0\xf8\xe5\xed\xe8\xe5 \xe2 \xf1\xe5\xec\x8c\x8e", "\xc2\xe0\xec \xef\xf0\xe8\xf8\xeb\xee \xef\xf0\xe8\xe3\xeb\xe0\xf8\xe5\xed\xe8\xe5 \xe2 \xf1\xe5\xec\x8c\x8e.");
    ShowNotificationNew(playerid, 2, 5, -1, -1, "\xcf\xf0\xe8\xe3\xeb\xe0\xf8\xe5\xed\xe8\xe5 \xee\xf2\xef\xf0\xe0\xe2\xeb\xe5\xed\xee", "\xc8\xe3\xf0\xee\xea \xef\xee\xeb\xf3\xf7\xe8\xeb \xef\xf0\xe8\xe3\xeb\xe0\xf8\xe5\xed\xe8\xe5.");
    return 1;
}
...
stock Family_OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
