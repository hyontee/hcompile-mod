...
{
    {"Наличные до 200.000 рублей"},
    {"Наличные до 500.000 рублей"},
    {"Наличные до 1.000.000 рублей"},
    {"Пакет акций до 200.000 рублей"},
    {"Пакет акций до 500.000 рублей"},
    {"Пакет акций до 1.000.000 рублей"}
};
        ...
        switch(orders_collectors[order][progress_order])
        {
            ...
            case 1:// игрок взял деньги/акции
            {
                ...
                if(orders_collectors[order][place_start] != Place_Col_1)
                {
                    ...
                    ""SC" Отправляйтесь в {FFFF00}отделение банка{FFFFFF} и возмите денежные средства из {FFFF00}хранилища.");
                } 
                else
                {
                    SendClientMessage(orders_collectors[order][collector_id_collector], -1, ""SC" Отправляйтесь к {FFFF00}банкомату{FFFFFF} и отнесите ценный груз.");
                }
            }
            case 2:// игрок подъежает к месту завершения заказа
            {
                ...
                {
                    SendClientMessage(orders_collectors[order][collector_id_collector], -1,
                    ""SC" Отправляйтесь в {FFFF00}отделение банка{FFFFFF} и отнесите денежные средства в {FFFF00}хранилище.");
                } 
                else
                {
                    SendClientMessage(orders_collectors[order][collector_id_collector], -1, ""SC" Отправляйтесь к {FFFF00}банкомату{FFFFFF} и отнесите ценный груз.");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
            ...
            switch(listitem)
            {
                case 0:
                {
                    ...
                    (
                        ...
                        "{FFFF00}Инкассатор{FFFFFF} - сотрудник банка, занимающийся транспортировкой денежных средств и других драгоценностей\n"\
                        "по нуждам банка или его клиентов.\n\n"\
                        "Данная работа - {FFFF00}командная{FFFFFF}, работать на ней можно только в паре с напарником.\n"\
                        "Для поиска напарника можно воспользоваться командой "BR"/collist\n\n"\
                        "{FFFFFF}Пара состоит из двух человек:\n"\
                        ""BR"1. {FFFF00}Водитель - член команды, арендовавший автомобиль. Он никогда не покидает автомобиль,\n"\
                        "следит за обстановкой.\n"\
                        ""BR"2. {FFFF00}Инкассатор - член команды, выполняющий роли пассажира в автомобиле\n"\
                        ...
                        ""BR"1. {FFFF00}Инкассатор 1 класса{FFFFFF} - самый младший класс класс инкассатора, которыйможет взаимодействовать заказами\n"\
                        "с меньшим числом средств и арендовать {FFFF00}Газель Инкассатора.\n"\
                        ""BR"2. {FFFF00}Инкассатор 2 класса{FFFFFF} - средний класс инкассатора, который может взаимодействовать\n"\
                        "с заказами с минимальным и средним количеством средств, а также арендовать {FFFF00}Газель Инкассатора{FFFFFF} или {FFFF00}Volkswagen Transporter.\n"\
                        ""BR"3. {FFFF00}Инкассатор 3 класса{FFFFFF} - самый высокий класс инкассатора, который может взаимодействовать со всеми заказами,\n"\
                        "а также арендовать {FFFF00}Газель Инкассатора, {FFFF00}Volkswagen Transporter{FFFFFF} или {FFFF00}Mercedes-Benz V-klasse.\n\n"\
                        "Для открытия нового уровня необходимо зарабатывать {FFFF00}навык{FFFFFF}, который\n"\
                        "выдается за каждый {FFFF00}успешно выполненный заказ.{FFFFFF}.\n\n"\
                ...
                case 1:
                {
                    if(!player_collector[playerid][job_active])
                    {
                        ...
                        (
                            ...
                            ""BR"1. {FFFFFF}Иметь {FFFF00}15 {FFFFFF}игровой уровень.\n"\
                            ""BR"2. {FFFFFF}Иметь {FFFF00}лицензию на оружие.\n"\
                            ""BR"3. {FFFFFF}Иметь {FFFF00}водительские права.\n"\
                            "{FFFFFF}Если Вы готовы трудоустроиться, нажмите {FFFF00}Далее{FFFFFF}.",
                            "Далее", "Закрыть"
                ...
                {
                    if(!player_collector[playerid][job_active])
                     return SendClientMessage(playerid, -1, ""USC"Сначало начните {FFFF00}рабочий день.");
                    ...
                    switch(player_collector[playerid][c_quest_progress_1])
                    {
                        ...
                        case 1:format(quest_1, sizeof quest_1, "{FFFF00}Выполняется");
                    ...
                    switch(player_collector[playerid][c_quest_progress_2])
                    {
                        ...
                        case 1:format(quest_2, sizeof quest_2, "{FFFF00}Выполняется");
                    ...
                    switch(player_collector[playerid][c_quest_progress_3])
                    {
                        ...
                        case 1:format(quest_3, sizeof quest_3, "{FFFF00}Выполняется");
                ...
                case 4:
                {
                    ...
                    switch(player_collector[playerid][rang_collector])
                    {
                        case 1:
                        {
                            format(exp, sizeof exp, "{FFFF00}3. {FFFFFF}Необходимое количество навыков для открытия следующего класса: {FFFF00}%d/%d{FFFFFF}\n",
                        ...
                        case 3:
                        {
                            format(exp, sizeof exp, "{FFFF00}3. {FFFFFF}Достигнут максимальный уровень инкассатора\n");
                    ...
                    (
                        text, sizeof text, 
                        "{FFFF00}1. {FFFFFF}Занимаемая должность: {FFFF00}Инкассатор %d класса{FFFFFF}\n"\
                        "{FFFF00}2. {FFFFFF}Текущий навык инкассатора: {FFFF00}%d{FFFFFF}\n"\
                        "%s\n"\
                        "{FFFF00}4. {FFFFFF}Доступные заказы:\n"\
                        "%s",
    ...
    if(dialogid == 1810)
    {
        if(response)
        {
            ...
            format(text, sizeof text, ""SC"Вы успешно взяли заказ {FFFF00}№%d{FFFFFF}, отправляйтесь на отмеченнную метку на GPS.", id + 1);
    ...
    if(dialogid == 1812)
    {
        if(response)
        {
            ...
            {
                SendClientMessage(playerid, -1, ""SC"Вы начали работу {FFFF00}инкассатора{FFFFFF}. Найдите напарника при помощи {FFFF00}/colinv [ID]");
            ...
            (
                ...
                "{FFFF00}Инкассатор{FFFFFF} - сотрудник банка, занимающийся транспортировкой денежных средств\n"\
                "и других драгоценностей по нуждам банка или его клиентов\n\n"\
                "Для трудоустройства инкассатором Вам необходимо иметь {FFFF00}15 игровой уровень{FFFFFF}\n"\
                "и {FFFF00}лицензию на оружие{FFFFFF}. Помните, что инкассация это командная работа:\n"\
                "перед началом работы Вам необходимо найти напарников и принять заказ вместе с ними. \n"\
                "Более подробно с работой Вы сможете ознакомиться в любом из {FFFF00}Банков{FFFFFF} или\n"\
                "в {FFFF00}списке команд {FFFFFF}(/mn >> Список команд).\n\n"\
                "Если Вы готовы трудоустроиться нажмите - {FFFF00}далее.",
                "Далее",
                "Отмена"
            );
        ...
        if(response)
        {
            ...
            switch(listitem)
            {
                ...
                case 1:
                {
                    ...
                    (
                        text, sizeof text,
                        "{FFFF00}1. {FFFFFF}Название заказа: {FFFF00}%s{FFFFFF}\n"\
                        "{FFFF00}2. {FFFFFF}Начальная точка заказа: {FFFF00}%s{FFFFFF}\n"\
                        "{FFFF00}3. {FFFFFF}Конечная точка заказа: {FFFF00}%s{FFFFFF}\n"\
                        "{FFFF00}4. {FFFFFF}Расстояние между точками: {FFFF00}%d\n"\
                        "{FFFF00}4. {FFFFFF}Напарник: {FFFF00}%s",
    ...
    if(dialogid == 1815)
    {
        if(response)
        {
            if(GetPVarInt(playerid, "c_q_d_1"))
            {
                ...
                SendClientMessage(playerid, -1, ""SC"Вы начали выполнять задание {FFFF00}\"Вместе веселеe\"");
            ...
            else if(GetPVarInt(playerid, "c_q_d_2"))
            {
                ...
                SendClientMessage(playerid, -1, ""SC"Вы начали выполнять задание {FFFF00}\"Достойный инкассатор\"");
            ...
            else if(GetPVarInt(playerid, "c_q_d_3"))
            {
                ...
                SendClientMessage(playerid, -1, ""SC"Вы начали выполнять задание {FFFF00}\"Это высокая позиция\"");
            ...
            switch(listitem)
            {
                case 0:
                {
                    ...
                    (
                        ...
                        "Ваша задача: {FFFF00}выполнить разные заказы не менее чем с двумя напарниками.\n"\
                        "{FFFFFF}Награда: {FFFF00}5.000 рублей.",
                        "Далее", "Назад"
                    );
                ...
                case 1:
                {
                    ...
                    (
                        ...
                        "Ваша задача: {FFFF00}выполнить не менее 5 заказов за день.\n"\
                        "{FFFFFF}Награда: {FFFF00}5.000 рублей.",
                        "Далее", "Назад"
                    );
                ...
                case 2:
                {
                    ...
                    (
                        ...
                        "Ваша задача: {FFFF00}заработать 10 очков рейтинга за 24 часа\n"\
                        "Напарники могут быть разными.{FFFFFF}Награда: {FFFF00}10.000 рублей.",
                        "Далее", "Назад"
                    );
...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    ...
    if(sphere_arenda_collector[0] <= areaid <= sphere_arenda_collector[3])
    {
        ...
        (
            ...
            ""BR"#1.\t{FFFFFF}Газель \"Инкассация\"\t{FFFFFF}5000 рублей\t{979595}1\n"\
            ""BR"#3.\t{FFFFFF}Volkwagen Multivan T6\t{FFFFFF}7500 рублей\t{979595}2\n"\
            ""BR"#2.\t{FFFFFF}Mersedes-Benz V-Class\t{FFFFFF}10000 рублей\t{979595}3",
            "Выбрать", "Отмена"
        );
    ...
    if(player_collector[playerid][order_collector] != -1)
    {
        ...
        if(areaid == orders_collectors[order][order_sphere_start])
        {
            ...
            if(orders_collectors[order][collector_id_collector] != playerid)
                return SendClientMessage(playerid, -1, ""USC"Водитель {FFFFFF}не может брать {FFFF00}денежные средства.");

            if(IsPlayerInAnyVehicle(playerid))
                return SendClientMessage(playerid, -1, ""USC"Выйдите из транспорта чтобы взять{FFFF00} денежные средства.");

            SendClientMessage(playerid, -1, ""SC" Вы успешно взяли денежные средства. Отправляйтесь в {FFFF00}инкассационный автомобиль.");
        ...
        if(areaid == orders_collectors[order][order_sphere_end])
        {
            ...
            SendClientMessage(playerid, -1, ""SC" Заказ успешно выполнен, отправляйтесь к {FFFF00}Банку {FFFFFF}для просмотра новых заказов.");
            SendClientMessage(partner, -1, ""SC" Заказ успешно выполнен, отправляйтесь к {FFFF00}Банку {FFFFFF}для просмотра новых заказов.");

            new salary;

            switch(orders_collectors[order][orders_type])
            {
                case 1,2:salary = 15000 + random(1000); 
                case 3,4:salary = 18000 + random(1000);
                case 5,6:salary = 20000 + random(1000);
            }
            ...
            ShowNotification(playerid, 3, "Вы успешно доставили груз", 3, "", "");
            ShowNotification(partner, 3, "Вы успешно доставили груз", 3, "", "");
            ...
            if(player_collector[playerid][c_quest_progress_1] ||player_collector[partner][c_quest_progress_1])
            {
                if(player_collector[playerid][c_quest_progress_1])
                {
                    ...
                    format(text, sizeof text, "Вы выполнили {FFFF00}%d{FFFFFF} из {FFFF00}2{FFFFFF} для выполнения задания {FFFF00}Вместе веселее",
                    player_collector[playerid][c_quest_count_1]);
                    SendClientMessage(playerid, -1, text);
                } 
                ...
                if(player_collector[playerid][c_quest_count_1] >= 2 ||player_collector[partner][c_quest_count_1] >= 2)
                {
                    if(player_collector[playerid][c_quest_count_1] >= 2)
                    {
                        ...
                        format(text, sizeof text, "Вы успешно выполнили ежедневное задание {FFFF00}Вместе веселее",
                        player_collector[playerid][c_quest_count_1]);
                        SendClientMessage(playerid, -1, text);
                        SendClientMessage(playerid, -1, ""SC" Вам начислено {FFFF00}5000{FFFFFF} рублей за выполнение задания.");
                    }
                    if(player_collector[partner][c_quest_count_1] >= 2)
                    {
                        ...
                        player_collector[partner][c_quest_count_1]);
                        SendClientMessage(partner, -1, text);
                        SendClientMessage(partner, -1, ""SC" Вам начислено {FFFF00}5000{FFFFFF} рублей за выполнение задания.");  
                    } 
                }
            }
            ...
            if(player_collector[playerid][c_quest_progress_2] ||player_collector[partner][c_quest_progress_2])
            {
                if(player_collector[playerid][c_quest_progress_2])
                {
                    ...
                    format(text, sizeof text, "Вы выполнили {FFFF00}%d{FFFFFF} из {FFFF00}5{FFFFFF} для выполнения задания {FFFF00}Достойный инкассатор",
                    player_collector[playerid][c_quest_count_2]);
                    SendClientMessage(playerid, -1, text);  
                } 
                ...
                if(player_collector[playerid][c_quest_count_2] >= 5 ||player_collector[partner][c_quest_count_2] >= 5)
                {
                    if(player_collector[playerid][c_quest_count_2] >= 5)
                    {
                        ...
                        format(text, sizeof text, "Вы успешно выполнили ежедневное задание {FFFF00}Достойный инкассатор",
                        player_collector[playerid][c_quest_count_1]);
                        SendClientMessage(playerid, -1, text);
                        SendClientMessage(playerid, -1, ""SC" Вам начислено {FFFF00}5000{FFFFFF} рублей за выполнение задания.");
                    }
            ...
            SendClientMessage(playerid, -1, ""SC"Вам был начислен {FFFF00}1 {FFFFFF}опыт инкассатора.");
            SendClientMessage(partner, -1, ""SC"Вам был начислен {FFFF00}1 {FFFFFF}опыт инкассатора.");
            ...
            if(player_collector[playerid][c_quest_progress_3] || player_collector[partner][c_quest_progress_3])
            {
                if(player_collector[playerid][c_quest_progress_3])
                {
                    ...
                    format(text, sizeof text, "Вы выполнили {FFFF00}%d{FFFFFF} из {FFFF00}10{FFFFFF} для выполнения задания {FFFF00}Это высокая позиция",
                    player_collector[playerid][c_quest_count_3]);
                    SendClientMessage(playerid, -1, text);  
                } 
                ...
                if(player_collector[playerid][c_quest_count_3] >= 5 ||player_collector[partner][c_quest_count_3] >= 5)
                {
                    if(player_collector[playerid][c_quest_count_3] >= 5)
                    {
                        ...
                        format(text, sizeof text, "Вы успешно выполнили ежедневное задание {FFFF00}Это высокая позиция",
                        player_collector[playerid][c_quest_count_1]);
                        SendClientMessage(playerid, -1, text);
                        SendClientMessage(playerid, -1, ""SC" Вам начислено {FFFF00}10000{FFFFFF} рублей за выполнение задания.");
                    }
                    if(player_collector[partner][c_quest_count_3] >= 5)
                    {
                        ...
                        player_collector[partner][c_quest_count_1]);
                        SendClientMessage(partner, -1, text);
                        SendClientMessage(partner, -1, ""SC" Вам начислено {FFFF00}10000{FFFFFF} рублей за выполнение задания.");
                    } 
                }
            }            
        }
...
public OnPlayerExitVehicle(playerid, vehicleid)
{
    if(player_collector[playerid][order_collector] != -1)
    {
        ...
        {  
            ...
            {
                SendClientMessage(playerid, -1, ""SC" У вас есть {FFFF00}180 {FFFFFF}секунд чтобы сесть обратно в транспорт.");
            ...
            {
                SendClientMessage(playerid, -1, ""SC" У вас есть {FFFF00}60 {FFFFFF}секунд чтобы сесть обратно в транспорт.");
...
{
    for(new i;i < 4;i++) CreateDynamicActor(123, place_bank[0], place_bank[1], place_bank[2], place_bank[3], _, _, i+1, 1);

    CreateDynamic3DTextLabel("{FFFF00}Павел - Инкассатор\n{FFFFFF}Подойдите для {FFFF00}взаимодействия", -1, place_bank[0], place_bank[1], place_bank[2], 10.0);
    ...
    for(new i;i < 4;i++)
    {
        ...
        CreateDynamic3DTextLabel("{FFFF00}Начальник службы автопарка\n{FFFFFF}Подойдите для {FFFF00}взаимодействия", -1, coord_arenda_c[i][0], coord_arenda_c[i][1], coord_arenda_c[i][2], 10.0);
...
CMD:colinv(playerid, params[])
{
    ...
    if(player_collector[playerid][order_collector] != -1) 
        return SendClientMessage(playerid, -1, ""USC" Сначало выполните заказ.");

    if(player_collector[playerid][player_partner] != -1)
    {
        player_collector[player_collector[playerid][player_partner]][player_partner] = -1;
        SendClientMessage(player_collector[playerid][player_partner], -1, ""USC" Игрок отказался от напарника. Вы больше не напарники");

        player_collector[playerid][player_partner] = -1;
        SendClientMessage(playerid, -1, ""USC" Вы отказались от напарника. Вы больше не напарники");
        
    }

    if(sscanf(params, "u", params[0]))
    return SendClientMessage(playerid, 0xFFFFFFFF, "{FFFF00}| {FFFFFF}Используйте {FFFF00}/colinv{FFFFFF} [ID игрока]");

    if(!IsPlayerLogged(partner) || !IsPlayerConnected(partner)) 
        return SendClientMessage(playerid, -1, ""USC" Данный игрок не в сети.");

    if(!player_collector[partner][job_active])
        return SendClientMessage(playerid, -1, ""USC" Игрок не работает в инкассации");

    if(!player_collector[playerid][job_active])
        return SendClientMessage(playerid, -1, ""USC" Вы не работаетe в инкассации");

    new Float:x, Float:y, Float:z;
    GetPlayerPos(partner, x, y, z);
    new Float:distance = GetPlayerDistanceFromPoint(playerid, x, y, z);

    if(distance >= 7.0) return SendClientMessage(playerid, -1, ""USC" Игрок находиться слишком далеко.");

    if(player_collector[partner][player_partner] != -1)
        return SendClientMessage(playerid,-1, ""USC" У игрока уже есть напарник");

    new text[52];
    format(text, sizeof text, "%s приглашает вас в напарники", GetPlayerNameEx(playerid));

    ShowNotification(partner, 4, text, 6, "/_yes_colinv", ">>");
...
public:UpdateOrderListCollector()
{
    ...
    else
    {
        ...
        foreach(new i : Player)
        {
            ...
            SendClientMessage(i, -1, "{FFFF00}| [Диспетчер]: В список заказов добавлен новый заказ: {FFFF00}/blist");
        }
    }
...
stock load_order_colllector(id)
{
    ...
    else
    {
        ...
        orders_collectors[id][order_e_3dtext] = CreateDynamic3DTextLabel("{FFFF00}Ценный груз\n{FFFFFF}Подойдите для взаимодействия", -1, place_safe_bank[0], place_safe_bank[1], place_safe_bank[2], 
        6.0, _, _, _, world_e, 1);
    } 
