...
stock EnterPlayerToGarage(playerid, garageid)
{
	...
	if(GetGarageData(garageid, GARAGE_IMPROVEMENTS) == 1)
	{
		...
		{
			SetVehiclePos(vehicleid, -0.2940, 2006.1188, 1554.2031);
	...
	else if(GetGarageData(garageid, GARAGE_IMPROVEMENTS) == 2)
	{
		...
		if(vehicleid != INVALID_VEHICLE_ID)
		{
			SetVehiclePos(vehicleid, 492.663848, 1988.835205, 1548.586645);
...
CMD:addgarage(playerid, params[])
{
	...
	if(sscanf(params, "d", price))
		return SendClientMessage(playerid, 0xCECECEFF, "Используйте: /addgarage [стоимость]");

	if(price < 1)
		return SendClientMessage(playerid, 0x999999FF, "Стоимость гаража не может быть меньше 1");

	if(g_garage_loaded >= MAX_GARAGES)
		return SendClientMessage(playerid, 0xFF0000FF, "Достигнут лимит гаражей.");
	...
	if(mysql_errno(mysql) != 0)
	{
		format(fmt_text, sizeof fmt_text, "Ошибка MySQL: %d", mysql_errno(mysql));
		return SendClientMessage(playerid, 0xFF0000FF, fmt_text);
	}
	...
	format(labelText, sizeof labelText, "Нажмите [гудок] чтобы въехать");
	SetGarageData(idx, G_LABEL, CreateDynamic3DTextLabel(labelText, 0x3399FFFF, pos_x, pos_y, pos_z + 1.0, 15.0));
	...
	SendClientMessage(playerid, 0x66CC33FF, "Гараж успешно создан.");
