...
enum E_GARAGE_STRUCT
{
	...
	Float: GARAGE_EXIT_X,      // позиция после выхода из дома  
	Float: GARAGE_EXIT_Y,      // позиция после выхода из дома  
	Float: GARAGE_EXIT_Z,      // позиция после выхода из дома  
...
if(newkeys == KEY_CROUCH)
   {
       ...
       if(IsPlayerInRangeOfPoint(playerid,  5.0, -0.2940,2006.1188,1554.2031))
       {
               ...
               if(!IsPlayerInVehicle(playerid, GetPlayerOwnableCar(playerid))) return SendClientMessage(playerid, -1, "{FFFFFF}Вы должны быть за рулем своего автомобиля");
   ...
   if(dialogid == DIALOG_GARAGE_IMPROVEMENT)
	{
	    if(response)
		{
			switch(listitem)
			{
				case 0:
				{
				SendClientMessage(playerid, 0x999999FF, "{FFFFFF}У вас уже приобретено это улучшение");
				}
				case 1:
				{
				SendClientMessage(playerid, 0x999999FF, "{FFFFFF}У вас уже приобретено это улучшение");
			...
			if(dialogid == DIALOG_OWNABLE_CAR_LOAD_GARAGE)
			{
              ...
              if(LoadCarGarage[playerid] == 0)
	      	{
					switch(listitem + 1)
					{
						...
						case 2:
						{
						SendClientMessage(playerid, -1, "{FFFF00}| {FFFFFF}Временно не доступно.");
						} 
