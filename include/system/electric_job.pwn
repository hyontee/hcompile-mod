    ...
    if(GetPlayerAdminEx(playerid) > 0 && GetPlayerAdminEx(playerid) != 13) 
{
    return SendClientMessage(playerid, 0xCECECEFF, "Администраторам запрещено работать!");
        ...
        else SendClientMessage(playerid, -1, ""USC" Работа электрика с 1 уровня");
    }
...
stock CreateElectric()
{
    ...
    for(new e;e < sizeof orders_electric;e++)
    {
        ...
        else CreateDynamic3DTextLabel("{FFFF00}Электрический щиток", -1, orders_electric[e][E_X], orders_electric[e][E_Y], orders_electric[e][E_Z], 10.0);
    }
