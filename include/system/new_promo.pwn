...
stock ShowPromoCreationConfirmation(playerid)
{
    ...
    format(szMessage, sizeof(szMessage), "{FF6347}"FAN4IIK_NAME" {FFFFFF}| Подтверждение создания промокода:\n\n");
    ...
    ShowPlayerDialog(playerid, DIALOG_CONFIRM_PROMO_CREATION, DIALOG_STYLE_MSGBOX,
        "{FF6347}"FAN4IIK_NAME" {FFFFFF}| Подтверждение создания промокода", szMessage, "Создать", "Отмена"
    );
}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    switch(dialogid)
    {
        ...
        case DIALOG_ADDPROMO_CODE:
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_ADDPROMO_PRIZE_COUNT, DIALOG_STYLE_INPUT, "{FF6347}"FAN4IIK_NAME" {FFFFFF}| Создание промокода", "Введите количество призов (1-5):", "Далее", "Отмена");
            return 1;
        }
        case DIALOG_ADDPROMO_PRIZE_COUNT:
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_ADDPROMO_USES_LIMIT, DIALOG_STYLE_INPUT, "{FF6347}"FAN4IIK_NAME" {FFFFFF}| Создание промокода", "Введите лимит использований промокода (0 - безлимитно):", "Далее", "Отмена");
            return 1;
        }
        ...
        case DIALOG_ADDPROMO_USES_LIMIT:
        {
            ...
            format(szDialogTitle, sizeof(szDialogTitle), "{FF6347}"FAN4IIK_NAME" {FFFFFF}| Настройка приза %d/%d (Тип)",
        ...
        case DIALOG_ADDPROMO_SINGLE_PRIZE_TYPE:
        {
            ...
            new szDialogTitle[128]; 
            format(szDialogTitle, sizeof(szDialogTitle), "{FF6347}"FAN4IIK_NAME" {FFFFFF}| Настройка приза %d/%d",
                currentPrizeIndex + 1,
                PlayerPromoCreationData[playerid][NewPromoPrizeCount]);
        ...
        case DIALOG_ADDPROMO_SINGLE_PRIZE_VALUE:
        {
            ...
            new szDialogTitle[128];
            format(szDialogTitle, sizeof(szDialogTitle), "{FF6347}"FAN4IIK_NAME" {FFFFFF}| Настройка приза %d/%d (Тип длительности)",
                currentPrizeIndex + 1,
                PlayerPromoCreationData[playerid][NewPromoPrizeCount]);
        ...
        case DIALOG_ADDPROMO_SINGLE_PRIZE_DURATION:
        {
            ...
            if(durationType == 0) 
            {
                format(szDialogTitle, sizeof(szDialogTitle), "{FF6347}"FAN4IIK_NAME" {FFFFFF}| Настройка приза %d/%d (Длительность)",
                    currentPrizeIndex + 1,
                    PlayerPromoCreationData[playerid][NewPromoPrizeCount]);
...
case DIALOG_APPLY_PROMO_CODE:
{
    ...
    if(GetPromoCodeDataByString(inputtext, promoData))
    {
        ...
        if(creator_playerid != -1)
        {
            ...
            ShowPlayerDialog(playerid, DIALOG_PROMO_PRIZE_INFO, DIALOG_STYLE_MSGBOX, "{FF6347}"FAN4IIK_NAME"{ffffff} | Активация промокода", szActivationMessage, "Ок", "");
...
CMD:addcpromo(playerid)
{
    ...
    ShowPlayerDialog(playerid, DIALOG_ADDPROMO_CODE, DIALOG_STYLE_INPUT, "{FF6347}"FAN4IIK_NAME" {FFFFFF}| Создание промокода", "Введите название промокода (без пробелов):", "Далее", "Отмена");
    return 1;
}
