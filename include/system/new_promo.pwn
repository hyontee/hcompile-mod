...
stock GetPromoCodeDataByID(promoId, String:promoData[]);
stock GetPromoCodeDataByString(const promoCode[], String:promoData[]);
...
case DIALOG_APPLY_PROMO_CODE:
{
    ...
    promoIndex = GetPromoCodeIndexByString(inputtext);
    if(promoIndex != -1)
    {
        if(PromoCodes[promoIndex][UsesLeft] > 0 || PromoCodes[promoIndex][PromoUses] == 0) 
        {
