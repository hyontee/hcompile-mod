    ...
    if(playertextid == welsi_tun_PTD[playerid][9])
    {
        ...
        switch(player_wheel_panel[playerid][tun_sct_panel])
        {
            case 1:
            {
                ...
                SetVehicleClearance(vehicleid, player_wheel_panel[playerid][tun_count_set]);
            }
            case 2:
            {
                player_wheel_panel[playerid][tun_count_set] += 0.07;
                SetVehicleSeparateClearance(vehicleid, player_wheel_panel[playerid][tun_count_set]);
            }
            case 3:
            {
                player_wheel_panel[playerid][tun_count_set] += 0.05;
                SetVehicleWheelRadius(vehicleid, player_wheel_panel[playerid][tun_count_set]);
            }
            case 4:
            {
                player_wheel_panel[playerid][tun_count_set] += 2.0;
                SetVehicleWheelAlignment(vehicleid, 0, floatround(player_wheel_panel[playerid][tun_count_set]));
                SetVehicleWheelAlignment(vehicleid, 1, floatround(player_wheel_panel[playerid][tun_count_set]));
            }
        }
...
{
    print("[RAYDEN]   Система шиномонтажа загружена.");
