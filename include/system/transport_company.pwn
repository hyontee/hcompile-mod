...
public:LoadPlayerProgressTC(playerid)
{
    ...
    printf("[SANDER_SYSTEM] Player:%d | Level_TC: %d | Exp_TC: %d", playerid, player_tc[playerid][p_TC_Level],player_tc[playerid][p_TC_Exp]);
...
{
    print("[SANDER_SYSTEM] Система работы дальнобойщика загружена.");
...
public:UpdateOrderTC()
{
    ...
    if(order[0] || order[1])
    {
        print("[SANDER_SYSTEM] В список заказов дальнобойщиков добавлено:");
        printf("[SANDER_SYSTEM] %d в бусаево и %d в батырево", order[0], order[1]);
    }
