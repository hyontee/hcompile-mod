       ...
       if(items_order[IsPlayerInItemsDiver(playerid)][I_DV_OBJECT] == -1) return 1;
        ...
        if(GetPlayerDiver(playerid, DIVER_JOOB))
        {
            if(IsPlayerInItemsDiver(playerid) != -1)
            {
                ...
                {
                    // ShowNotificationSander(playerid, 1, 6, 0, 0, "Вы подобрали предмет.", "");
                    return 1;
                }
                else ShowNotificationSander(playerid, 2, 6, 0, 0, "Этот предмет не с вашего заказа.", "");
                return 0;
            }
        }
        else ShowNotificationSander(playerid, 2, 6, 0, 0, "Устройтесь на работу водолаза.", "");
        return 0;
    }
    ...
    if(dialogid == 2770)
    {
        if(response)
        {
            switch(listitem+1)
            {
                case 1:
                {
                    if(!GetPlayerDiver(playerid, DIVER_JOOB)) return ShowNotificationSander(playerid, 2, 6, 0, 0, "Сначала возьмите смену.", "");

                    if(GetPlayerDiver(playerid, DIVER_ORDER)) return ShowNotificationSander(playerid, 2, 6, 0, 0, "Сначала возьмите смену.", "");

                    new id, orders[MAX_DIVER];
                    ...
                    if(id == 0) return ShowNotificationSander(playerid, 1, 6, 0, 0, "Немного подождите...", "");
                    ...
                    ShowPlayerGPS(playerid, order_diver[GetPlayerDiver(playerid, DV_ID)][DV_X], order_diver[GetPlayerDiver(playerid, DV_ID)][DV_Y]);
                ...
                case 2:
                {
                    if(GetPlayerDiver(playerid, DIVER_JOOB))
                    {
                        ...
                        if(GetPlayerDiver(playerid, DV_ID_VEH) != -1)
                        {
                            ...
                            SetPlayerDiver(playerid, DV_ID_VEH, -1);
                            ShowNotificationSander(playerid, 2, 6, 0, 0, "Вы уволились с работы 'Водолаз'.", "");
                        }
                    }
                    else
                    {
                ...
                case 3:
                {
                    if(!GetPlayerDiver(playerid, DIVER_JOOB)) return ShowNotificationSander(playerid, 2, 6, 0, 0, "Сначала начните смену.", "");
                    ...
                    if(GetPlayerDiver(playerid, DV_ID_VEH) != -1)
                    {
                        DestroyVehicle(GetPlayerDiver(playerid, DV_ID_VEH));
                        SetPlayerDiver(playerid, DV_ID_VEH, -1);
                        ShowNotificationSander(playerid, 1, 6, 0, 0, "Вы вернули лодку.", "");
                    }
                    
                    else
                    {
                        if(!(GetPlayerMoneyEx(playerid) >= 200000)) return ShowNotificationSander(playerid, 1, 6, 0, 0, "Вам необходимо 200.000 рублей.", "");
                        new vehicle;
...
CMD:takeitems(playerid)
{
    ...
    switch(items_order[i_id][I_DV_TYPE])
    {
        case I_DIVER_BOX:
        {
            AddPlayerDiver(playerid, DV_BOX, +, 1);
            UpdatePlayerDatabaseInt(playerid, "box", GetPlayerDiver(playerid, DV_BOX));
            ShowNotificationSander(playerid, 1, 6, 0, 0, "Вы подобрали 'Шкатулка'.", "");
        }
        case I_DIVER_TIRE:
        {   
            AddPlayerDiver(playerid, DV_TIRE, +, 1);
            UpdatePlayerDatabaseInt(playerid, "tire", GetPlayerDiver(playerid, DV_TIRE));
            ShowNotificationSander(playerid, 1, 6, 0, 0, "Вы подобрали 'Шина'.", "");
        }
        case I_DIVER_GOLD:
        {
            AddPlayerDiver(playerid, DV_GOLD, +, 1);
            UpdatePlayerDatabaseInt(playerid, "gold", GetPlayerDiver(playerid, DV_GOLD));
            ShowNotificationSander(playerid, 1, 6, 0, 0, "Вы подобрали 'Слиток золота'.", "");
        }
        case I_DIVER_TV:
        {
            AddPlayerDiver(playerid, DV_TV, +, 1);
            UpdatePlayerDatabaseInt(playerid, "television", GetPlayerDiver(playerid, DV_TV));
            ShowNotificationSander(playerid, 1, 6, 0, 0, "Вы подобрали 'Телевизор'.", "");
        }
        case I_DIVER_BOOT:
        {
            AddPlayerDiver(playerid, DV_BOOT, +, 1);
            UpdatePlayerDatabaseInt(playerid, "boot", GetPlayerDiver(playerid, DV_BOOT));
            ShowNotificationSander(playerid, 1, 6, 0, 0, "Вы подобрали 'Сапог с кладом'.", "");
        }
    }
    ...
    if(items_count >= 4)
    {
        ...
        SetPlayerDiver(playerid, DV_ID, -1);
        SetPlayerDiver(playerid, DIVER_ORDER, false);
        ShowNotificationSander(playerid, 1, 6, 0, 0, "Вы собрали все предметы.", "");
