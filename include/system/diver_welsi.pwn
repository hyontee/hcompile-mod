...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    ...
    for(new i = 0; i < MAX_I_DIVER; i++)
    {
        if(areaid == items_order[i][I_DV_AREA])
        {
            ...
            {
                ShowNotificationXxesao(playerid, 2, 6, 0, 0, "Устройтесь на работу водолаза.", "");
                return 0;
            }

            if(!GetPlayerDiver(playerid, DIVER_ORDER))
            {
                ShowNotificationXxesao(playerid, 2, 6, 0, 0, "У вас нет активного заказа. Возьмите заказ у капитана.", "");
                return 0;
            }

            if(items_order[i][I_DV_ORDER] != GetPlayerDiver(playerid, DV_ID))
            {
                ShowNotificationXxesao(playerid, 2, 6, 0, 0, "Этот предмет не из вашего заказа.", "");
                return 0;
            }
	        	...
	        	SetPVarInt(playerid, "shpere_diver", areaid);
          		{
		            ...
		            "{FF5252}"SERVER_NAME"{ffffff} | Водолаз",
    ...
    if(dialogid == 2770)
    {
        if(response)
        {
            switch(listitem+1)
            {
                case 1:
                {
                    if(!GetPlayerDiver(playerid, DIVER_JOOB)) return ShowNotificationXxesao(playerid, 2, 6, 0, 0, "Сначала возьмите смену.", "");

                    if(GetPlayerDiver(playerid, DIVER_ORDER)) return ShowNotificationXxesao(playerid, 2, 6, 0, 0, "Сначала возьмите смену.", "");
                    ...
                    if(r_orders == -1) return ShowNotificationXxesao(playerid, 1, 6, 0, 0, "Немного подождите...", "");
                    ...
                    ShowNotificationXxesao(playerid, 1, 6, 0, 0, "Вы взяли заказ", "");
                ...
                case 2:
                {
                    if(GetPlayerDiver(playerid, DIVER_JOOB))
                    {
                        ...
                        if(GetPlayerDiver(playerid, DV_ID_VEH) != -1)
                        {
                            ...
                            SetPlayerDiver(playerid, DV_ID_VEH, -1);
                            ShowNotificationXxesao(playerid, 2, 6, 0, 0, "Вы уволились с работы 'Водолаз'.", "");
                            SendDestroyWayPoint(playerid, 5);
                        }
                    }
                    else
                    {
                        ShowNotificationXxesao(playerid, 1, 6, 0, 0, "Вы устроились на работу 'Водолаз' ", "");
               ...
               case 3:
                {
                    if(!GetPlayerDiver(playerid, DIVER_JOOB)) return ShowNotificationXxesao(playerid, 2, 6, 0, 0, "Сначала начните смену.", "");
                    ...
                    if(GetPlayerDiver(playerid, DV_ID_VEH) != -1)
                    {
                        DestroyVehicle(GetPlayerDiver(playerid, DV_ID_VEH));
                        SetPlayerDiver(playerid, DV_ID_VEH, -1);
                        ShowNotificationXxesao(playerid, 1, 6, 0, 0, "Вы вернули лодку", "");
                    }

                    else
                    {
                        if(!(GetPlayerMoneyEx(playerid) >= 20000)) return ShowNotificationXxesao(playerid, 1, 6, 0, 0, "Вам необходимо 20.000 рублей.", "");
                        new vehicle;

                        if(GetPVarInt(playerid, "shpere_diver") == sphere_actor[0])
                        {
                            new r = random(3);
                        	ShowNotificationXxesao(playerid, 1, 6, 0, 0, "Вы арендовали лодку", "");
    ...
    if(response)
    {
        switch(listitem + 1)
        {
            case 1:  // Колесо
            {
                ...
                {
                    ShowNotificationXxesao(playerid, 2, 6, 0, 0, "У вас нет колёс для продажи.", "");
                    return 1;
                }
                ...
                ShowNotificationXxesao(playerid, 1, 6, 0, 0, str, "");
            ...
            case 2:  // Аккумулятор
            {
                ...
                {
                    ShowNotificationXxesao(playerid, 2, 6, 0, 0, "У вас нет Аккумулятора для продажи.", "");
                    return 1;
                }
            ...
            case 3:  // Стиральная машина
            {
                ...
                {
                    ShowNotificationXxesao(playerid, 2, 6, 0, 0, "У вас нет Стиральной машины для продажи.", "");
                    return 1;
                }
            ...
            case 4:  // Ваза
            {
                ...
                {
                    ShowNotificationXxesao(playerid, 2, 6, 0, 0, "У вас нет Ваз для продажи.", "");
                    return 1;
                }
            ...
            case 5:  // Бензопил
            {
                ...
                {
                    ShowNotificationXxesao(playerid, 2, 6, 0, 0, "У вас нет Бензопилы для продажи.", "");
                    return 1;
                }
...
CMD:takeitems(playerid)
{
    ...
    if(item_id == -1 || item_id >= MAX_I_DIVER)
        return ShowNotificationXxesao(playerid, 2, 6, 0, 0, "Вы не находитесь рядом с предметом.", "");

    if(items_order[item_id][I_DV_OBJECT] == -1)
        return ShowNotificationXxesao(playerid, 2, 6, 0, 0, "Этот предмет уже подобран.", "");

    if(!GetPlayerDiver(playerid, DIVER_JOOB))
        return ShowNotificationXxesao(playerid, 2, 6, 0, 0, "Устройтесь на работу водолаза.", "");

    if(!GetPlayerDiver(playerid, DIVER_ORDER))
        return ShowNotificationXxesao(playerid, 2, 6, 0, 0, "У вас нет активного заказа.", "");

    if(items_order[item_id][I_DV_ORDER] != GetPlayerDiver(playerid, DV_ID))
        return ShowNotificationXxesao(playerid, 2, 6, 0, 0, "Этот предмет не из вашего заказа.", "");
    ...
    switch(items_order[item_id][I_DV_TYPE])
    {
        case I_DIVER_BOX:
        {
            AddPlayerDiver(playerid, DV_BOX, +, 1);
            UpdatePlayerDatabaseInt(playerid, "box", GetPlayerDiver(playerid, DV_BOX));
            ShowNotificationXxesao(playerid, 1, 6, 0, 0, "Вы подобрали 'Шкатулку'", "");
        }
        case I_DIVER_TIRE:
        {
            AddPlayerDiver(playerid, DV_TIRE, +, 1);
            UpdatePlayerDatabaseInt(playerid, "tire", GetPlayerDiver(playerid, DV_TIRE));
            ShowNotificationXxesao(playerid, 1, 6, 0, 0, "Вы подобрали 'Шину'", "");
        }
        case I_DIVER_GOLD:
        {
            AddPlayerDiver(playerid, DV_GOLD, +, 1);
            UpdatePlayerDatabaseInt(playerid, "gold", GetPlayerDiver(playerid, DV_GOLD));
            ShowNotificationXxesao(playerid, 1, 6, 0, 0, "Вы подобрали 'Слиток золота'", "");
        }
        case I_DIVER_TV:
        {
            AddPlayerDiver(playerid, DV_TV, +, 1);
            UpdatePlayerDatabaseInt(playerid, "television", GetPlayerDiver(playerid, DV_TV));
            ShowNotificationXxesao(playerid, 1, 6, 0, 0, "Вы подобрали 'Телевизор'", "");
        }
        case I_DIVER_BOOT:
        {
            AddPlayerDiver(playerid, DV_BOOT, +, 1);
            UpdatePlayerDatabaseInt(playerid, "boot", GetPlayerDiver(playerid, DV_BOOT));
            ShowNotificationXxesao(playerid, 1, 6, 0, 0, "Вы подобрали 'Сапог с кладом'", "");
        }
    }
    ...
    if(remaining_items == 0)
    {
        ...
        ShowNotificationXxesao(playerid, 1, 6, 0, 0, "Вы собрали все предметы! Возвращайтесь на пирс.", "");
    }
    else
    {
        new str[128];
...
stock ShowSellDialog(playerid)
{
    ...
    (
        playerid, 2771, DIALOG_STYLE_TABLIST_HEADERS,
        "{FF5252}"SERVER_NAME"{ffffff} | Продажа предметов",
        dialog_text,
        "Продать", "Назад"
    );
}
