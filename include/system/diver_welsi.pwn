...
// В начале файла добавьте эти функции, если их нет
...
{
    // Проверка нахождения в зоне предметов
    for(new i = 0; i < MAX_I_DIVER; i++)
    {
        if(areaid == items_order[i][I_DV_AREA])
        {
            ...
            if(GetPlayerDiver(playerid, DIVER_JOOB))
            {
                ...
                {
                    // Разрешаем подобрать предмет
                    return 1;
                }
                else 
                {
                    ShowNotificationNew(playerid, 2, 6, 0, 0, "Этот предмет не с вашего заказа.", "");
                    return 0;
                }
            }
            else 
            {
                ShowNotificationNew(playerid, 2, 6, 0, 0, "Устройтесь на работу водолаза.", "");
                return 0;
            }
        }
    }
    
    // Проверка нахождения в зоне актеров
...
{
    // Создание актеров
    CreateActorEx("{D4544B}Капитан пирса", "{FFFFFF}Подойдите для взаимодействия", 5344, 676.336730, 187.356491, 2.778203, 175.494964);
    CreateActorEx("{D4544B}Капитан пирса", "{FFFFFF}Подойдите для взаимодействия", 111, -2526.549072, 357.963623, 2.060437, 124.978698);
    ...
    if(dialogid == 2770)
    {
        if(response)
        {
            switch(listitem + 1)
            {
                ...
                {
                    if(!GetPlayerDiver(playerid, DIVER_JOOB)) 
                        return ShowNotificationNew(playerid, 2, 6, 0, 0, "Сначала устройтесь на работу.", "");

                    if(GetPlayerDiver(playerid, DIVER_ORDER)) 
                        return ShowNotificationNew(playerid, 2, 6, 0, 0, "У вас уже есть активный заказ.", "");

                    // Собираем доступные заказы
                    ...
                    if(availableCount == 0) 
                        return ShowNotificationNew(playerid, 1, 6, 0, 0, "Нет свободных заказов, подождите...", "");
                ...
                case 2: // Устроиться/уволиться
                {
                    if(GetPlayerDiver(playerid, DIVER_JOOB))
                    {
                        ...
                        ShowNotificationNew(playerid, 2, 6, 0, 0, "Вы уволились с работы 'Водолаз'.", "");
                    }
                    else
                    {
                        SetPlayerSkin(playerid, 5344);
                        SetPlayerDiver(playerid, DIVER_JOOB, true);
                        ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы устроились на работу 'Водолаз'.", "");
                    }
                }
                
                case 3: // Арендовать лодку
                {
                    if(!GetPlayerDiver(playerid, DIVER_JOOB)) 
                        return ShowNotificationNew(playerid, 2, 6, 0, 0, "Сначала начните смену.", "");

                    if(GetPlayerDiver(playerid, DV_ID_VEH) != -1)
                    {
                        DestroyVehicle(GetPlayerDiver(playerid, DV_ID_VEH));
                        SetPlayerDiver(playerid, DV_ID_VEH, -1);
                        ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы вернули лодку.", "");
                    }
                    else
                    {
                        if(GetPlayerMoneyEx(playerid) < 200000) 
                            return ShowNotificationNew(playerid, 1, 6, 0, 0, "Вам необходимо 200.000 рублей.", "");
        ...
        if(response)
        {
            switch(listitem)
            {
                case 0: // Шкатулка
                {
                    ...
                    {
                        ShowNotificationNew(playerid, 2, 6, 0, 0, "У вас нет шкатулок!", "");
                        return 1;
                    }
                    ...
                    ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы продали шкатулку.", "");
                ...
                case 1: // Шина
                {
                    ...
                    {
                        ShowNotificationNew(playerid, 2, 6, 0, 0, "У вас нет шин!", "");
                        return 1;
                    }
                    ...
                    ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы продали шину.", "");
                    
                    OnDialogResponse(playerid, 2770, 1, 3, "");
                }
                
                case 2: // Золотой слиток
                {
                    if(GetPlayerDiver(playerid, DV_GOLD) <= 0) 
                    {
                        ShowNotificationNew(playerid, 2, 6, 0, 0, "У вас нет золотых слитков!", "");
                        return 1;
                    }
                    ...
                    ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы продали золотой слиток.", "");
                ...
                case 3: // Телевизор
                {
                    ...
                    {
                        ShowNotificationNew(playerid, 2, 6, 0, 0, "У вас нет телевизоров!", "");
                        return 1;
                    }
                    ...
                    ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы продали телевизор.", "");
                    
                    OnDialogResponse(playerid, 2770, 1, 3, "");
                }
                
                case 4: // Сапог с кладом
                {
                    if(GetPlayerDiver(playerid, DV_BOOT) <= 0) 
                    {
                        ShowNotificationNew(playerid, 2, 6, 0, 0, "У вас нет сапог с кладом!", "");
                        return 1;
                    }
                    ...
                    ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы продали сапог с кладом.", "");
...
CMD:takeitems(playerid)
{
    ...
    if(i_id == -1) 
        return SCM(playerid, -1, "Вы должны находиться у предмета");
    
    if(items_order[i_id][I_DV_OBJECT] == -1)
        return SCM(playerid, -1, "Этот предмет уже подобрали");
    
    if(!GetPlayerDiver(playerid, DIVER_JOOB))
        return ShowNotificationNew(playerid, 2, 6, 0, 0, "Устройтесь на работу водолаза.", "");
    
    if(items_order[i_id][I_DV_ORDER] != GetPlayerDiver(playerid, DV_ID))
        return ShowNotificationNew(playerid, 2, 6, 0, 0, "Этот предмет не с вашего заказа.", "");
    ...
    switch(items_order[i_id][I_DV_TYPE])
    {
        case I_DIVER_BOX:
        {
            AddPlayerDiver(playerid, DV_BOX, +, 1);
            UpdatePlayerDatabaseInt(playerid, "box", GetPlayerDiver(playerid, DV_BOX));
            ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы подобрали 'Шкатулка'.", "");
        }
        case I_DIVER_TIRE:
        {   
            AddPlayerDiver(playerid, DV_TIRE, +, 1);
            UpdatePlayerDatabaseInt(playerid, "tire", GetPlayerDiver(playerid, DV_TIRE));
            ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы подобрали 'Шина'.", "");
        }
        case I_DIVER_GOLD:
        {
            AddPlayerDiver(playerid, DV_GOLD, +, 1);
            UpdatePlayerDatabaseInt(playerid, "gold", GetPlayerDiver(playerid, DV_GOLD));
            ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы подобрали 'Слиток золота'.", "");
        }
        case I_DIVER_TV:
        {
            AddPlayerDiver(playerid, DV_TV, +, 1);
            UpdatePlayerDatabaseInt(playerid, "television", GetPlayerDiver(playerid, DV_TV));
            ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы подобрали 'Телевизор'.", "");
        }
        case I_DIVER_BOOT:
        {
            AddPlayerDiver(playerid, DV_BOOT, +, 1);
            UpdatePlayerDatabaseInt(playerid, "boot", GetPlayerDiver(playerid, DV_BOOT));
            ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы подобрали 'Сапог с кладом'.", "");
        }
    }

    DestroyDiverItems(i_id);

    // Проверяем, все ли предметы собраны
    for(new i = 0; i < MAX_I_DIVER; i++)
    {
    ...
    // Если собраны все 4 предмета (по 4 на заказ)
    if(items_count >= 4)
    {
        RemoveDiverOrder(items_order[i_id][I_DV_ORDER]);
        SetPlayerDiver(playerid, DV_ID, -1);
        SetPlayerDiver(playerid, DIVER_ORDER, false);
        ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы собрали все предметы.", "");

        // Отправляем к ближайшему пирсу
        ...
        SendClientMessage(playerid, -1, "Ближайший {D4544B}'Капитан Пирса' {FFFFFF}отмечен на карте.");
    }
...
// Вспомогательные функции (если их нет в вашем моде)
