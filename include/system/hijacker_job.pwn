    ...
    if(dialogid == 2645)
    {
        if(response)
        {
            switch(listitem)
            {
                ...
                case 1:
                {
                    if(GetPlayerLevel(playerid) < 3) return 	ShowNotificationLonexs(playerid, 2, 5, 0, 0, "Для работы автоугонщика необходим 3 уровень!", " ");
...
//Автор данной системы https://t.me/welsistudio (Welsi Studio)
                    ...
                    if(player_order_hijacker[playerid] == -1) return 	ShowNotificationLonexs(playerid, 2, 5, 0, 0, "Сейчас заданий нет.", " ");
...
{
    CreateHijacker();
    print("Работа автоугонщика загружена.");
