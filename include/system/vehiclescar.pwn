new small_veh_to_market, medium_veh_to_market, high_veh_to_market, bike_veh_to_market, pickup_used_market;

new sphere_veh_enter_used_market[8], shpr_exit_veh_us_mrkt_to_int[8];

new truck_veh_to_market; // Счетчик грузовых авто
new pickup_exit_used_market[3]; // Расширяем массив при необходимости
...
new car_market_type[76][STRUCT_VEH_MARKET] = //СЮДА ДОБАВЛЯТЬ ТРАНСПОРТ И ИХ КЛАСС
{
    {462, VEH_BIKE},
    {468, VEH_BIKE},
    {523, VEH_BIKE},
    ...
    {503, VEH_HIGH},//dodge
    {505, VEH_HIGH},//cadilac
    {400, VEH_HIGH},//bmw x5
    {495, VEH_HIGH},//ford raptor
    {502, VEH_HIGH},//nissan gtr
    {405, VEH_HIGH},//audi rs6
    {402, VEH_HIGH},//akula
    {410, VEH_HIGH},//mers cls
    {475, VEH_HIGH},//audi q8
    {506, VEH_HIGH},//porshe ?
    {604, VEH_HIGH},//porshe ?
    {490, VEH_HIGH},//range rover
    {533, VEH_HIGH},//audi r8
    {429, VEH_HIGH},//mers gtr
    {494, VEH_HIGH},//bmw i8
    {466, VEH_HIGH},//bmw f90
    {451, VEH_HIGH},//maclaren
    {579, VEH_HIGH},//gelic
    {411, VEH_HIGH},//aston martin
    {541, VEH_HIGH},//ferrari
    {415, VEH_HIGH},//lamba
    {413, VEH_TRUCK}, // Gazelle 3221
    {418, VEH_TRUCK}, // Volkswagen Multivan T6
    {483, VEH_TRUCK}, // Mercedes-Benz Sprinter 319CDI
    {440, VEH_TRUCK}, // Mercedes-Benz V-Class
    {562, VEH_MEDIUM},//skyline
    {507, VEH_MEDIUM},//a4
    {546, VEH_MEDIUM},//hundai
    {426, VEH_MEDIUM},//e39
    {516, VEH_MEDIUM},//volksfagen polo
    {467, VEH_MEDIUM},
    {527, VEH_MEDIUM},//bmw m3
    {445, VEH_MEDIUM},//acura
    {477, VEH_MEDIUM},//mazda rx7
    {589, VEH_MEDIUM},//golf
    {587, VEH_MEDIUM},//focus ford
    {436, VEH_MEDIUM},//mitsubisi
    {560, VEH_MEDIUM},//subaru
    {565, VEH_MEDIUM},//mers a45
    {550, VEH_MEDIUM},//camry
    {603, VEH_MEDIUM},//mustang
    {526, VEH_MEDIUM},//infiniti
    {551, VEH_MEDIUM},//alfa romeo
    {442, VEH_MEDIUM}//volvo 60
};
...
{
    print("[KM_SYSTEM] Авторынок успешно загружен.");
    print("[KM_SYSTEM] Авторынок успешно загружен.");
    print("[KM_SYSTEM] Авторынок успешно загружен.");
    CreateUsedMarket();
...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    if(areaid == pickup_used_market)
    {
        ...
        (
            ...
            "{FF6347}4. {FFFFFF}Грузовой класс\t{FF6347}%d\t{7A7A7A}Нажмите для входа\n"\
            "{FF6347}5. {FFFFFF}Мотоциклы\t{FF6347}%d\t{7A7A7A}Нажмите для входа",
            small_veh_to_market,
        ...
        if(is_player_in_area_used_market(playerid, true) != -1)
        {
            ...
            if(IsPlayerInAnyVehicle(playerid))
            {
                ...
                switch(vehicle_market)
                {
		            ...
		            case 4: { // Новый case для грузового класса
		                player_to_market[playerid][MARKET_TRUCK] = true;
		                truck_veh_to_market++;
		            }
                }
...
stock player_exit_from_used_market(playerid)
{
    ...
    if(player_to_market[playerid][MARKET_TRUCK])
    {
        truck_veh_to_market--;
        player_to_market[playerid][MARKET_TRUCK] = false;
    }
...
stock player_to_used_market(playerid)
{
    ...
    if(player_to_market[playerid][MARKET_TRUCK]) used_market = 5;
...
stock CreateUsedMarket()
{
    for(new c;c < 8;c++)
    {
        ...
        Create3DTextLabel("{FFFF00}Авторынок\n\n{FFFFFF}Нажмите {FF0000}'гудок'{FFFFFF} находясь в машине", -1, coord_veh_enter_used_market[c][0], coord_veh_enter_used_market[c][1], coord_veh_enter_used_market[c][2] + 0.3, 20.0, 0);
    }
    ...
    for(new c;c < 8;c++)
    {
        ...
        CreateDynamic3DTextLabel("{FFFF00}Выезд\n\n{FFFFFF}Нажмите {FF0000}'гудок'{FFFFFF} находясь в машине", -1, pos_exit_veh_us_mrkt_to_int[c][0], pos_exit_veh_us_mrkt_to_int[c][1], pos_exit_veh_us_mrkt_to_int[c][2] + 0.3, 20.0);
    }
    for(new c;c < 2;c++)
    {
