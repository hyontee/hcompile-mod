...
public:CREATE_ACCOUNTS_TABLE_WP()
{
    ...
    if(mysql_errno())
    {
        mysql_tquery(mysql, "ALTER TABLE `accounts` ADD `weekly_prizes` TEXT NOT NULL DEFAULT '0,0,0,0,0,0,0' AFTER `level`, ADD `weekly_day` INT NOT NULL DEFAULT '0' AFTER `weekly_prizes`"); /*PERF: sync->async, fire-and-forget schema write*/
        
        if(!mysql_errno()) print("[LAIRD_SYSTEM] Столбцы созданы для еженедельных наград");
    }
...
public OnGameModeInit()
{
    ...
    print("[LAIRD_SYSTEM] Система еженедельных наград загружена.");
...
public:CheckWeeklyPrizes()
{
    ...
    if(!day_weekly && !hour && !minute) mysql_tquery(mysql, "UPDATE accounts SET weekly_prizes = '0,0,0,0,0,0,0'"); /*PERF: sync->async, fire-and-forget write*/
    
    if(!hour && !minute) mysql_tquery(mysql, "UPDATE accounts SET weekly_day = 0"); /*PERF: sync->async, fire-and-forget write*/
