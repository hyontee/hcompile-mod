...
stock PacketIncomingUse(playerid, Node:JSONObject)
{
    ...
    switch(type)
    {
        case 0: // Из инвентаря в шкаф
        {
            ...
            if(house_sql_id == 0)
            {
                ShowNotificationSander(playerid, 2, 6, -1, -1, "Ошибка: ID дома не найден", "");
        ...
        case 1: // Из шкафа в инвентарь
        {
            ...
            if(storage_index == -1)
            {
                ShowNotificationSander(playerid, 2, 6, -1, -1, "Ошибка: Шкаф не найден", "");
                JSON_SetInt(response, "t", 1);
                JSON_SetInt(response, "s", 0);
                SendPacketToClient(playerid, 34, response);
                JSON_Cleanup(response);
            ...
            if(item_id == 134) // Скин
            {
                ...
                if(skin_count >= 3)
                {
                    ShowNotificationSander(playerid, 2, 6, -1, -1, "Нельзя иметь больше 3 скинов в инвентаре", "");
                    JSON_SetInt(response, "t", 1);
                    JSON_SetInt(response, "s", 0);
                    SendPacketToClient(playerid, 34, response);
                    JSON_Cleanup(response);
        ...
        case 8: // Закрыть
        {
            ...
            ShowNotificationSander(playerid, 2, 4, -1, -1, "Шкаф закрыт", "");
