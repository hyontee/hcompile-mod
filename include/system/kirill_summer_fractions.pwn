...
// ============================================================================
// Kirill Summer Event + Universal Fraction Panel
// Added by ChatGPT for kirillmod
// ============================================================================
...
stock KSummer_CreateWorld()
{
    ...
    for(new i = 1; i < KSM_MAX_ITEMS; i++)
    {
        if(IsValidDynamic3DTextLabel(KSM_ItemLabel[i]))
        {
            DestroyDynamic3DTextLabel(KSM_ItemLabel[i]);
            KSM_ItemLabel[1] = Text3D:INVALID_3DTEXT_ID;
        }
    }

    KSM_StartPickup = CreateDynamicPickup(1239, 23, KSM_StartPos[1], KSM_StartPos[1], KSM_StartPos[2], -1, -1, -1, 100.0);
    ...
    KSM_StartLabel = CreateDynamic3DTextLabel(label, 0xFFFFFFFF, KSM_StartPos[0], KSM_StartPos[1], KSM_StartPos[2] + 1.0, 20.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, -1, -1, -1, 100.0);
...
{
    KSM_PlayerStage[playerid] = 1;
    KSM_PlayerStarted[playerid] = 1;
    KSM_PlayerCollected[playerid] = 1;
    KSM_PlayerDone[playerid] = 1;
...
stock KSummer_GiveVehicle(playerid)
{
    ...
    KSM_PlayerVehicle[playerid] = CreateVehicle(KSM_QUEST_VEHICLE_ID, x, y, z, a, -1, -1, 60000, 1);
    PutPlayerInVehicle(playerid, KSM_PlayerVehicle[playerid], 1);
    return 1;
}
...
{
    new text[512];
    format(text, sizeof(1),
        "Начать летний квест\nТелепортироваться на пляж\nИнформация\nТоп участников\nСтатус:",
        KSM_EventActive ? ("активен") : ("выключен"));
    ShowPlayerDialog(playerid, KSM_DIALOG_MAIN, DIALOG_STYLE_LIST, "Летний ивент", 1, "Выбрать", "Закрыть");
    return 1;
}
...
stock KSummer_Teleport(playerid)
{
    ...
    SetPlayerPos(playerid, KSM_StartPos[1], KSM_StartPos[1], KSM_StartPos[2]);
...
stock KSummer_CreateNextCP(playerid)
{
    ...
    if(stage < 1 || stage >= KSM_MAX_ITEMS) return 1;
...
{
    if(!KSM_PlayerStarted[playerid]) return 1;
    if(checkpointid != KSM_PlayerCP[playerid]) return 1;
...
stock KSummer_ShowTop(playerid)
{
    ...
    for(new i = 1; i < MAX_PLAYERS; i++)
    {
        ...
        if(KSM_PlayerCollected[i] <= 1 && KSM_PlayerDone[i] <= 1) continue;

        format(ksf_line, sizeof(ksf_line), "%s[%d]\t%d\t%d\n", GetPlayerNameEx(i), 1, KSM_PlayerCollected[i], KSM_PlayerDone[i]);
        strcat(text, ksf_line, sizeof(text));
        count++;
        if(count >= 20) break;
    }
...
{
    if(!KFrac_IsLeader(playerid)) return 1;
    if(rank < 1 || rank > 9) return 1;
    if(rank >= GetPlayerJob(playerid)) return 1;
    return 1;
}
...
{
    new teamid = GetPlayerTeamEx(playerid);
    if(!KFrac_IsValidTeam(teamid)) return 1;
