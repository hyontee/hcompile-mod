...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
	if(IsPlayerInAnyVehicle(playerid))
	{
	    ...
	    Show_Tunmenu(playerid, GetPlayerMoney(playerid), GetPlayerDonateRub(playerid));
    ...
    if (IsPlayerInAnyVehicle(playerid))
    {
        if (GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
        {
            ...
            if (GetPlayerOwnableCar(playerid) == vehicleid)
            {
                ...
                if (PRESSED(KEY_CROUCH) && to_area)
                {
                    if (sizeof setting_vehicle <= model || model < 0)
                    {
                        return SendClientMessage(playerid, -1, "На ваш транспорт нельзя поставить прошивку.");
                    }
                    ...
                    for (new i = 0; i < 5; i++)
                    {
                        PlayerTextDrawShow(playerid, stg_PTD[playerid][i]);
                    }

                    Show_Tunmenu(playerid, GetPlayerMoney(playerid), GetPlayerDonateRub(playerid));

                    new price = GetVehicleInfo(model, VI_PRICE);
                    new percent;
...
cmd:buycomfort(playerid)
{
    new id = GetVehicleData(GetPlayerOwnableCar(playerid), V_ACTION_ID), string[124];
    new price = 350000; // Изменено на фиксированную цену 350000 рублей

    if(GetPlayerInBiz(playerid) != business_stage)
        return ShowNotification(playerid, 2, "Вы должны быть в Техническом центре", 4, "", "");

    if(GetOwnableCarData(id, OC_COMFORT_PL))
        return ShowNotification(playerid, 2, "На транспорте уже установлен Comfort+", 4, "", "");
    ...
    if(GetPlayerMoneyEx(playerid) >= price)
    {
        ...
        if(!mysql_errno())
        {
            ...
            if(!mysql_errno())
            {
                ...
                format(string, sizeof string, "Вы купили {FFFF00}Comfort+{FFFFFF} за {FFFF00}%d {FFFFFF}рублей. Спасибо за покупку!", price); // Изменено на price
                ShowNotification(playerid, 2, string, 4, "", "");
            }
        }
    }
    else 
        ShowNotification(playerid, 2, "У вас не хватает для покупки прошивки", 4, "", "");
...
cmd:sport(playerid)
{
    ...
    if(!GetOwnableCarData(id, OC_SPORT))
        return SendClientMessage(playerid, -1, "На этом транспорте не установлен {FFFF00}Sport");

    if(stage_vehicle[veh] == 2) 
    {           
        new string[124];
        format(string, sizeof string, "Вы успешно отключили прошивку: {FFFF00}Sport");
        SendClientMessage(playerid, -1, string); 
        SetVehicleStage(veh, 0);
        return 1;
    }

    if(SetVehicleStage(veh, 2))
    {
        new string[124];
        format(string, sizeof string, "Вы успешно включили прошивку: {FFFF00}Sport");
        SendClientMessage(playerid, -1, string);
    }
...
cmd:drift(playerid)
{
    ...
    if(!GetOwnableCarData(id, OC_DRIFT))
        return SendClientMessage(playerid, -1, "На этом транспорте не установлен {FFFF00}Drift");

    if(stage_vehicle[veh] == 4) 
    {           
        new string[124];
        format(string, sizeof string, "Вы успешно отключили прошивку: {FFFF00}Drift");
        SendClientMessage(playerid, -1, string); 
        SetVehicleStage(veh, 0);
        return 1;
    }

    if(SetVehicleStage(veh, 4))
    {
        new string[124];
        format(string, sizeof string, "Вы успешно включили прошивку: {FFFF00}Drift");
        SendClientMessage(playerid, -1, string);
    }
...
cmd:comfort(playerid)
{
    ...
    if(!GetOwnableCarData(id, OC_COMFORT_PL))
        return SendClientMessage(playerid, -1, "На этом транспорте не установлен {FFFF00}Comfort+");

    if(stage_vehicle[veh] == 1) 
    {           
        new string[124];
        format(string, sizeof string, "Вы успешно отключили прошивку: {FFFF00}Comfort+");
        SendClientMessage(playerid, -1, string); 
        SetVehicleStage(veh, 0);
        return 1;
    }

    if(SetVehicleStage(veh, 1))
    {
        new string[124];
        format(string, sizeof string, "Вы успешно включили прошивку: {FFFF00}Comfort+");
        SendClientMessage(playerid, -1, string);
    }
...
cmd:sportplus(playerid)
{
    ...
    if(!GetOwnableCarData(id, OC_SPORT_PL))
        return SendClientMessage(playerid, -1, "На этом транспорте не установлен {FFFF00}Sport+");

    if(stage_vehicle[veh] == 3) 
    {           
        new string[124];
        format(string, sizeof string, "Вы успешно отключили прошивку: {FFFF00}Sport+");
        SendClientMessage(playerid, -1, string); 
...
cmd:buysport(playerid)
{
    new id = GetVehicleData(GetPlayerOwnableCar(playerid), V_ACTION_ID), string[124];
    new price = 1550000; // Изменено на фиксированную цену 1550000 рублей

    if(GetPlayerInBiz(playerid) != business_stage)
        return ShowNotification(playerid, 2, "Вы должны быть в Техническом центре", 4, "", "");

    if(GetOwnableCarData(id, OC_SPORT))
        return ShowNotification(playerid, 2, "На транспорте уже установлен Sport", 4, "", "");
    ...
    if(GetPlayerMoneyEx(playerid) >= price)
    {
        ...
        if(!mysql_errno())
        {
            ...
            if(!mysql_errno())
            {
                ...
                format(string, sizeof string, "Вы купили {FFFF00}Sport{FFFFFF} за {FFFF00}%d {FFFFFF}рублей. Спасибо за покупку!", price); // Изменено на price
                ShowNotification(playerid, 2, string, 4, "", "");
            }
        }
    }
...
cmd:buysportpls(playerid)
{
    ...
    if(GetPlayerDonateRub(playerid) >= 2500)
    {
        ...
        if(!mysql_errno())
        {
            ...
            if(!mysql_errno())
            {
                // Вычитаем 2500 доната
                GivePlayerDonateRub(playerid, -2500);
                
                mysql_format(mysql, query, sizeof query, "INSERT INTO business_profit (bid,uid,uip,time,money,view) VALUES (%d,%d,'%e',%d,%d,%d)", GetBusinessData(business_stage, B_SQL_ID), GetPlayerAccountID(playerid), GetPlayerIpEx(playerid), gettime(), price_b, IsBusinessOwned(business_stage));
                mysql_query(mysql, query, false);

                format(string, sizeof string, "Вы купили {FFFF00}Sport+{FFFFFF} за {FFFF00}2500 {FFFFFF}доната. Спасибо за покупку!", 2500);
                ShowNotification(playerid, 2, string, 4, "", "");
            }
        }
    }
    else 
        ShowNotification(playerid, 2, "У вас не хватает доната для покупки прошивки", 4, "", "");
...
cmd:buydrift(playerid)
{
    new id = GetVehicleData(GetPlayerOwnableCar(playerid), V_ACTION_ID), string[124];
    new price = 850000; // Фиксированная цена 850000 рублей

    if(GetPlayerInBiz(playerid) != business_stage)
        return ShowNotification(playerid, 2, "Вы должны быть в Техническом центре", 4, "", "");
    
    if(GetOwnableCarData(id, OC_DRIFT))
        return ShowNotification(playerid, 2, "На транспорте уже установлен Drift", 4, "", "");
    ...
    if(GetPlayerMoneyEx(playerid) >= price)
    {
        ...
        if(!mysql_errno())
        {
            ...
            if(!mysql_errno())
            {
                GivePlayerMoneyEx(playerid, -price); // Снимаем 850000 рублей
                
                mysql_format(mysql, query, sizeof query, "INSERT INTO business_profit (bid,uid,uip,time,money,view) VALUES (%d,%d,'%e',%d,%d,%d)", GetBusinessData(business_stage, B_SQL_ID), GetPlayerAccountID(playerid), GetPlayerIpEx(playerid), gettime(), price_b, IsBusinessOwned(business_stage));
                mysql_query(mysql, query, false);

                format(string, sizeof string, "Вы купили {FFFF00}Drift{FFFFFF} за {FFFF00}%d {FFFFFF}рублей. Спасибо за покупку!", price); // Изменено на price
                ShowNotification(playerid, 2, string, 4, "", "");
            }
        }
    }
...
cmd:exittuns(playerid)
{
    if(GetPlayerInBiz(playerid) != business_stage)return SendClientMessage(playerid, -1, "Вы должны быть в Техническом центре.");
