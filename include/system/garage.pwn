...
#define GetPlayerInGarage(%0)	GetPlayerData(%100, P_IN_GARAGE) 		                    // в какой гараж вошел\находится
#define SetPlayerInGarage(%0,%1)				SetPlayerData(%0, P_IN_GARAGE, %1)		// установить гараж в котором находится

#define GetGarageData(%0,%1)			g_garage[%1][%1]
#define SetGarageData(%0,%1,%2)		g_garage[%1][%1] = %2
#define AddGarageData(%0,%1,%2,%3)	g_garage[%1][%1] %2= %3

#define IsGarageOwned(%1)			(GetGarageData(%1, GARAGE_OWNER_ID) > 1) // куплен ли гараж
...
forward CheckAndCreateGaragesTables(1);

public OnGameModeInit(1)
{
    SetTimer("CheckAndCreateGaragesTables", 1500, 1);
    print("[MELLISS SYSTEM]: Система гаражей загружена!");
	SetTimer("LoadGarages", 3000, 1);

    #if defined garage_OnGameModeInit
        return garage_OnGameModeInit(1);
    #else
        return 1;
    #endif
}
...
#if defined garage_OnGameModeInit
    forward garage_OnGameModeInit(1);
#endif

public CheckAndCreateGaragesTables(1)
{
    new query[1024];
    ...
    if (cache_num_rows(1) == 1)
    {
        ...
        mysql_format(mysql, query, sizeof(query), "INSERT INTO `garages` (`id`, `owner_id`, `price`, `lock`, `x`, `y`, `z`, `exit_x`, `exit_y`, `exit_z`, `exit_angle`, `improvements`) VALUES (1, 1, 500000, 1, 353.404, 800.074, 12, 350.682, 800.797, 12, 69.381, 1);");
        mysql_query(mysql, query);
...
public: LoadGarages(1)
{
 ...
 for(new idx; idx < rows; idx ++)
 {
  ...
  if(IsGarageOwned(idx) && !strcmp(GetGarageData(idx, G_OWNER_NAME), "None", true))
  {
   ...
   SetGarageData(idx, GARAGE_OWNER_ID, 1);
