...
public CheckPlayerGPUsAfterConnect(playerid)
{
    ...
    if(GetPlayerGPUCount(playerid) > 0 && GetPlayerMiningRig(playerid))
    {
        ...
        else ShowNewNotification(playerid, 1, 6, 0, 0, "Для авоматического запуска майнинга активируйте его в доме!", "");
    }
}
...
public OnMiningTimer(playerid) 
{
    ...
    if(houseid == -1) 
    {
        ...
        {
            StopPlayerMining(playerid);
            ShowNewNotification(playerid, 2, 6, 0, 0, "Майнинг остановлен - у вас нет дома!", "");
        }
        return;
    }
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[]) 
{
    ...
    else if(dialogid == DIALOG_GPU_BUY) 
    {
        ...
        if(houseid == -1) 
        {
            ShowNewNotification(playerid, 2, 6, 0, 0, "Для покупки видеокарты необходим собственный дом!", "");
            return 1;
        }
        ...
        switch(listitem)
        {
            ...
            case 1: // Включить/Выключить майнинг
            {
                ...
                else
                {
                    ...
                    if(houseid == -1) 
                    {
                        ShowNewNotification(playerid, 2, 6, 0, 0, "Для майнинга необходим собственный дом!", "");
                        ShowMiningManagementDialog(playerid);
                        return 1;
                    }
...
{
    new houseid = GetPlayerHouse(playerid, HOUSE_TYPE_HOME);
    if(houseid == -1) return ShowNewNotification(playerid, 2, 6, 0, 0, "Для майнинга необходим собственный дом!", "");

    new type = GetHouseData(GetPlayerInHouse(playerid), H_TYPE);
    if(!IsPlayerInRangeOfPoint(playerid, 50.0, GetHouseTypeInfo(type, HT_ENTER_POS_X), GetHouseTypeInfo(type, HT_ENTER_POS_Y), GetHouseTypeInfo(type, HT_ENTER_POS_Z))) return ShowNewNotification(playerid, 2, 6, 0, 0, "Вы должны находиться у себя дома для просмотра статистики. Используйте /btc чтобы проверить свой баланс Bitcoin", "");
...
{
    new houseid = GetPlayerHouse(playerid, HOUSE_TYPE_HOME);
    if(houseid == -1) return ShowNewNotification(playerid, 2, 6, 0, 0, "Для установки рига необходим собственный дом!", "");
    ...
    new type = GetHouseData(GetPlayerInHouse(playerid), H_TYPE);
    if(!IsPlayerInRangeOfPoint(playerid, 50.0, GetHouseTypeInfo(type, HT_ENTER_POS_X), GetHouseTypeInfo(type, HT_ENTER_POS_Y), GetHouseTypeInfo(type, HT_ENTER_POS_Z))) return ShowNewNotification(playerid, 2, 6, 0, 0, "Вы должны находиться у себя дома для покупки", "");
...
{
    new houseid = GetPlayerHouse(playerid, HOUSE_TYPE_HOME);
    if(houseid == -1) return ShowNewNotification(playerid, 2, 6, 0, 0, "Для доступа к инвентарю необходим собственный дом!", "");
    if(!GetPlayerMiningRig(playerid)) return ShowNewNotification(playerid, 2, 6, 0, 0, "Сначала купите майнинг-риг!", "");
    
    new type = GetHouseData(GetPlayerInHouse(playerid), H_TYPE);
    if(!IsPlayerInRangeOfPoint(playerid, 50.0, GetHouseTypeInfo(type, HT_ENTER_POS_X), GetHouseTypeInfo(type, HT_ENTER_POS_Y), GetHouseTypeInfo(type, HT_ENTER_POS_Z))) return ShowNewNotification(playerid, 2, 6, 0, 0, "Вы должны находиться у себя дома для просмотра видеокарт", "");
...
{
    new houseid = GetPlayerHouse(playerid, HOUSE_TYPE_HOME);
    if(houseid == -1) return ShowNewNotification(playerid, 2, 6, 0, 0, "Для продажи видеокарт необходим собственный дом!", "");
    if(!GetPlayerMiningRig(playerid)) return ShowNewNotification(playerid, 2, 6, 0, 0, "Сначала купите майнинг-риг!", "");
    
    new type = GetHouseData(GetPlayerInHouse(playerid), H_TYPE);
    if(!IsPlayerInRangeOfPoint(playerid, 50.0, GetHouseTypeInfo(type, HT_ENTER_POS_X), GetHouseTypeInfo(type, HT_ENTER_POS_Y), GetHouseTypeInfo(type, HT_ENTER_POS_Z))) return ShowNewNotification(playerid, 2, 6, 0, 0, "Вы должны находиться у себя дома для продажи", "");
