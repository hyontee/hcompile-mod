...
stock FamilyAdd_OnGameModeInit()
{
    TextDrawFamilyWar();
...
stock FamilyAdd_OnPlayerEnterDynamicArea(playerid, areaid)
{
...
stock FamilyAdd_OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
...
stock FamilyAdd_OnPlayerLeaveDynamicArea(playerid, areaid)
{
    if(areaid == CAPTURE_AREA[0] || areaid == CAPTURE_AREA[1] || areaid == CAPTURE_AREA[2] || areaid == CAPTURE_AREA[3] || areaid == CAPTURE_AREA[4]) 
    {
...
stock FamilyAdd_OnPlayerDisconnect(playerid, reason)
{
...
stock FamilyAdd_OnPlayerSpawn(playerid)
{
    if(GetPlayerIdFamily(playerid) != -1)
    {
        for(new i;i < sizeof(CAPTURE_STATUS);i++)
...
stock FamilyAdd_OnPlayerDeath(playerid, killerid, reason)
{
...
stock FamilyAdd_OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
