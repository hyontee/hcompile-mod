...
stock thief_OnDialogResponse ( playerid, dialogid, response, listitem, inputtext [ ] )
{
	...
	switch ( dialogid )
	{
		case d_thief_quest:
		{
			...
			new scm_string [ 144 ] ;
			format ( scm_string, sizeof scm_string, "{"#cGInfo"}* {"#cWH"}Тебе необходимо снять детали с %s. Примерное нахождение отмечено на карте чёрным квадратом.", GetVehicleNameEx ( -1, GetVehicleModel ( vehicle_id ) ) ) ;
			...
			veh_info [ vehicle_id - 1 ] [ v_label ] = CreateDynamic3DTextLabel ( scm_string, col_blue, 0.0, 0.0, 0.2, 5.0, INVALID_PLAYER_ID, vehicle_id ) ;
...
callback: calllback_thief_timer ( playerid )
{
	...
	new comp, extra_comp;
	extra_comp = RandomEx ( EXTRA_COMPONENT_BOOT, EXTRA_COMPONENT_BUMP_FRONT ) ;
	comp = ccExtra ;
	
	sc_car_component ( p_t_info [ playerid ] [ c_vehicle ], comp, extra_comp, false ) ;
	...
	if ( random ( _chance ) == 1 )
	{
		...
		global_string [ 0 ] = EOS ;
		format ( global_string, 128, "* Вам был добавлен предмет '%s'. Откройте инвентарь, используйте /mm или радиальное меню.", item_name ( _det_id ) ) ;
