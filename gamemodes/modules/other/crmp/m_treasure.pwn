...
static garage_car [ 34 ] = { 405, 426, 435, 445, 466, 470, 475, 480, 502, 507, 516, 540, 547, 550, 555, 559, 566, 567, 600, 602, 604, 3258, 3259, 3263, 3276, 3281, 3283, 3285, 3290, 3292, 3303, 3314, 3315, 3351 } ;
...
stock tr_EnterDynamicArea ( playerid, areaid )
{
	switch ( area_info [ areaid ] [ a_type ] )
	{
		case area_type_treasure:
		{
			...
			if ( player_device { playerid } == 2 ) send_check_cinfo ( playerid, "Клад. Нажмите для взаимодействия", 1, -1, CINFO_TREASURE_ID, CINFO_TYPE_AREA, PICTURE_INFO_FINGER, "Раскопать", "" ) ;
		...
		case area_type_treasure_card:
		{
			...
			if ( player_device { playerid } == 2 ) send_check_cinfo ( playerid, "Вы находитесь не далеко от клада!", 0, 500, CINFO_TREASURE_CARD, CINFO_TYPE_AREA, PICTURE_INFO_FINGER, "", "" ) ;
...
stock treasure_spawn ( )
{
	...
	treasure_place ( _random ) ;
	return 1 ;
}

stock treasure_unplace ( i )
{
	if ( IsValidDynamicObject ( te_info [ i ] [ te_object ] [ 0 ] ) ) DestroyDynamicObject ( te_info [ i ] [ te_object ] [ 0 ] ) ;
	if ( IsValidDynamicObject ( te_info [ i ] [ te_object ] [ 1 ] ) ) DestroyDynamicObject ( te_info [ i ] [ te_object ] [ 1 ] ) ;
	if ( IsValidDynamicArea ( te_info [ i ] [ te_area ] ) ) DestroyDynamicArea ( te_info [ i ] [ te_area ] ) ;
...
stock open_treasure ( playerid, i )
{
	...
	switch ( random_item )
	{
		...
		case 1: give_player_item_prise ( playerid, 0, 1 ), random_random = 0 ;
		case 2: give_player_item_prise ( playerid, 1, 1 ), random_random = 1 ;
		case 3: give_player_item_prise ( playerid, 2, 1 ), random_random = 2 ;
		case 4: give_player_item_prise ( playerid, 3, 1 ), random_random = 3 ;
		case 5: give_player_item_prise ( playerid, 35, 1 ), random_random = 35 ;
		case 6: give_player_item_prise ( playerid, 36, 1 ), random_random = 36 ;
		case 7: give_player_item_prise ( playerid, 37, 1 ), random_random = 37 ;
		case 8: give_player_item_prise ( playerid, 905, random ( 10 ) + 1 ), random_random = 905 ;
		case 9: give_player_item_prise ( playerid, 19941, random ( 3 ) + 1 ), random_random = 19941 ;
		case 10: give_player_item_prise ( playerid, 1463, random ( 10 ) + 1 ), random_random = 1463 ;
		case 11: give_player_item_prise ( playerid, 2684, random ( 3 ) + 1 ), random_random = 2684 ;
		case 12: give_player_item_prise ( playerid, 1080, 1 ), random_random = 1080 ;
		case 13: give_player_item_prise ( playerid, 1018, 1 ), random_random = 1018 ;
		case 14: give_player_item_prise ( playerid, 1038, 1 ), random_random = 1038 ;
		case 15: give_player_item_prise ( playerid, 1140, 1 ), random_random = 1140 ;
		case 16: give_player_item_prise ( playerid, 1165, 1 ), random_random = 1165 ;
		case 17: give_player_item_prise ( playerid, 19773, 1 ), random_random = 19773 ;
		case 18: give_player_item_prise ( playerid, 45, 1 ), random_random = 45 ;
		case 19: give_player_item_prise ( playerid, 55, 1 ), random_random = 55 ;
		case 20:
		{
			new _random = random ( sizeof garage_skin ) ;
			random_random = garage_skin [ _random ] [ 0 ] + skin_cross ;
			give_player_item_prise ( playerid, random_random, 1 ) ;
		}
		case 21:
		{
...
callback: treasure_timer ( playerid, _te_id )
{
	...
	if ( player_device { playerid } == 2 ) send_check_cinfo ( playerid, "Клад. Нажмите для взаимодействия", 1, -1, CINFO_TREASURE_ID, CINFO_TYPE_AREA, PICTURE_INFO_FINGER, "Забрать", "" ) ;
	else GameTextForPlayer ( playerid, "~n~~n~~n~~n~~n~~n~~n~~n~~g~ PRESS: ~r~Y", 3000, 3 ) ;
	return 1 ;
}
	...
	switch ( dialogid )
	{
		case d_treasure:
		{
			...
			if ( listitem == 0 )
			{
				...
				if ( player_device { playerid } == 2 ) mobile_object_edit ( playerid, true, 11713 ) ;
				else EditDynamicObject ( playerid, _treasure_object ) ;
				return 1 ;
			}
		...
		case d_treasure_select:
		{
			...
			switch ( listitem )
			{
				...
				case 1:
				{
					...
					new select_id = GetPVarInt ( playerid, "a_id_select" ) ;
					new _treasure_object = CreateDynamicObject ( 11713, te_info [ select_id ] [ te_pos ] [ 0 ], te_info [ select_id ] [ te_pos ] [ 1 ], te_info [ select_id ] [ te_pos ] [ 2 ], 0.0, -90.0, 0.0, 0, 0, -1, 300.0, 300.0 ) ;

					create_type { playerid } = 9 ;
					create_object_id [ playerid ] = _treasure_object ;
					if ( player_device { playerid } == 2 ) mobile_object_edit_at ( playerid, 11713, te_info [ select_id ] [ te_pos ] [ 0 ], te_info [ select_id ] [ te_pos ] [ 1 ], te_info [ select_id ] [ te_pos ] [ 2 ], 0.0, -90.0, 0.0 ) ;
					else EditDynamicObject ( playerid, _treasure_object ) ;
				}
			}
			return 1 ;
...
stock tr_OnPlayerEditDynamicObject ( playerid, response, Float:x, Float:y, Float:z )
{
	if ( player_device { playerid } != 2 && response == EDIT_RESPONSE_FINAL || player_device { playerid } == 2 && response == 1 )
	{
		...
		if ( create_type { playerid } == 9 )
		{
			...
			if ( te_info [ select_id ] [ te_status ] )
			{
				treasure_unplace ( select_id ) ;
				treasure_place ( select_id ) ;
			}
			SendClientMessage ( playerid, col_white, !"{"#cGInfo"}* {"#cWH"}Позиция клада сохранена." ) ;
