...
new NewBiePass [ MAX_BATTLE_PASS_ITEMS ] [ ENUM_BATTLE_PASS_ITEMS ] =
{
	...
	{"Аптечка", BP_INVENTORY, 145, BP_ITEM_COMMON, BATTLEPASS_NO_RENDER, 							"Деньги: 1 000 000"valute_title_"", 1, 1_000_000, BP_ITEM_COMMON, BATTLEPASS_NO_RENDER}, // 5
	...
	{"Аптечка", BP_INVENTORY, 145, BP_ITEM_EPIC, BATTLEPASS_NO_RENDER,								"Рулетка аксессуаров", 10, 1, BP_ITEM_RARE, BATTLEPASS_NO_RENDER}, // 23
		...
		if ( player_device { playerid } != 2 )
		{
			...
			switch ( NewBiePass [ BPLevel ] [ bpiType ] )
			{
				...
				case BP_SKIN:
				{
					...
					give_player_item_prise ( playerid, NewBiePass [ BPLevel ] [ bpiAmount ] + skin_cross, 1 ) ;
	...
	else if ( _param1 == 4 )
	{
		if ( _param2 == 1 )
		{
			if ( gNewBieMonitorBonus [ playerid ] == _param3 )
			{
				...
				global_string [ 0 ] = EOS ;
				format ( global_string, 144, "* Вам был добавлен предмет '%s'. Откройте инвентарь, используйте /mm или радиальное меню.", item_name ( _item ) ) ;
				SendClientMessage ( playerid, col_yellow, global_string ) ;
