...
stock checking_winter_progress ( playerid, quest_id, amount_plus )
{
	...
	if ( year_quest_progress [ playerid ] == _quest_progress [ quest_id ] )
	{
	    if ( quest_id == sizeof year_quest_name - 1 )
	    {
			...
			if ( random ( 10 ) == 1 ) give_player_item_prise ( playerid, RandomEx ( skin_cross + 4718, skin_cross + 4719 ), 1, 20 ) ;
			else 
			{
				new _acc_id ;
...
stock winter_EnterDynamicArea ( playerid, areaid )
{
	if ( GetPlayerState ( playerid ) == PLAYER_STATE_ONFOOT )
	{
		switch ( area_info [ areaid ] [ a_type ] )
		{
			...
			{
				send_check_cinfo ( playerid, "Задания от Деда Мороза", 1, -1, CINFO_SANTA_ID, CINFO_TYPE_AREA, PICTURE_INFO_SUCESS, "Принять", "" ) ;
				return 1 ;
			}
			case area_type_snegurochka:
			{
				...
				send_check_cinfo ( playerid, "Задания от Снегурочки", 1, -1, CINFO_SNEGURKA_ID, CINFO_TYPE_AREA, PICTURE_INFO_SUCESS, "Принять", "" ) ;
				return 1 ;
			}
			case area_type_lottery_year:
			{
				send_check_cinfo ( playerid, "Новогодняя лотерея", 1, -1, CINFO_WINTER_LOTTERY_ID, CINFO_TYPE_AREA, PICTURE_INFO_SUCESS, "Принять", "" ) ;
				return 1 ;
			}
		}
	}
...
{
	CreateActor ( 121, santa_position [ 0 ], santa_position [ 1 ], santa_position [ 2 ], santa_position [ 3 ] ) ;
	...
	CreateActor ( 40, snegurochka_position [ 0 ], snegurochka_position [ 1 ], snegurochka_position [ 2 ], snegurochka_position [ 3 ] ) ;
	...
	for ( new i = 0 ; i < sizeof actor_lottery_year ; i ++ )
	{
		...
		#endif
		/*CreateActor ( 4571, actor_lottery_year [ i ] [ 0 ], actor_lottery_year [ i ] [ 1 ], actor_lottery_year [ i ] [ 2 ], actor_lottery_year [ i ] [ 3 ] ) ;
...
callback: winter_winner_callback ( playerid )
{
	...
	switch ( _u_lottery )
	{
		case 0:
		{
			give_player_item_prise ( playerid, 4645 + skin_cross, 1, -1 ) ;
		...
		case 1:
		{
			give_player_item_prise ( playerid, skin_cross + 4622, 1, -1 ) ;
			give_player_donate ( playerid, 1000, 2 ) ;
			insert_donate_log ( playerid, INVALID_PLAYER_ID, 1000, p_info [ playerid ] [ donate ], "(donate) выйгрыш лотерея" ) ;
		...
		case 2:
		{
			give_player_item_prise ( playerid, 3370, 1, -1 ) ;
			give_player_donate ( playerid, 1000, 2 ) ;
			insert_donate_log ( playerid, INVALID_PLAYER_ID, 1000, p_info [ playerid ] [ donate ], "(donate) выйгрыш лотерея" ) ;
		...
		case 3:
		{
			give_player_item_prise ( playerid, 4707, 1, -1 ) ;
			give_player_item_prise ( playerid, 3377, 1, -1 ) ;
			give_player_donate ( playerid, 1000, 2 ) ;
			insert_donate_log ( playerid, INVALID_PLAYER_ID, 1000, p_info [ playerid ] [ donate ], "(donate) выйгрыш лотерея" ) ;
		...
		case 4:
		{
			give_player_item_prise ( playerid, 8253, 1, -1 ) ;
			give_player_item_prise ( playerid, 3382, 1, -1 ) ;
