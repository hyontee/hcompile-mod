...
{
	{ 163.6877, -1159.4562, 1206.9222 },
	{ 156.6397, -1152.6892, 1206.9222 },
	{ 171.0277, -1152.5960, 1206.9222 }
} ;
...
stock coin_businesses_loading ( )
{
	...
	{
		CreateDynamic3DTextLabel ( "** Орёл и решка **\n{"#cGR3D"}Подойдите для взаимодействия", col_blue, coin_position [ i ] [ 0 ], coin_position [ i ] [ 1 ], coin_position [ i ] [ 2 ], 10.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, -1, -1 ) ;
	...
	switch ( area_info [ areaid ] [ a_type ] )
	{
		case area_type_coin:
		{
			if ( player_device { playerid } == 2 )
				send_check_cinfo ( playerid, "Орёл и режка. Нажмите для взаимодействия", 1, -1, CINFO_COIN_ID, CINFO_TYPE_AREA, PICTURE_INFO_SUCESS, "Играть", "" ) ;
...
stock coin_OnDialogResponse ( playerid, dialogid, response, listitem, inputtext [ ] )
{
	switch ( dialogid )
	{
		...
		case d_coin_bet:
		{
			...
			if ( p_info [ targetid ] [ hour_played ] < 3 )
			{
