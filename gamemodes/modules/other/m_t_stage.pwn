...
stock stage_OnDialogResponse ( playerid, dialogid, response, listitem, inputtext [ ] )
{
	...
	switch ( dialogid )
	{
		case d_stage:
		{
			...
			new _v_id = p_t_info [ playerid ] [ tuning_vehicle ], _s_lvl = stage_select { playerid } ;
...
stock update_stage_progress ( playerid, _stage_id )
{
	...
	new _v_model = getReplacableVehicleModel ( _v_id ) ;
...
{
	if ( player_checking_hash [ playerid ] < 17 ) return SendClientMessage ( playerid, col_gray, !"{"#cRInfo"}* {"#cGRInfo"}Сперва обновите игру." ) ;
