	        ...
	        switch ( listitem )
	        {
	            ...
	            case 1:
	            {
					...
					foreach(new i: streamed_players[playerid])
					{
						...
						format( global_string, sizeof global_string, "%s{"#cWH"}%s[%d] - {"#cBL"}%s\n", global_string, p_info [ i ] [ name ], i, GetVehicleNameEx ( veh_info [ player_vehicle [ i ] - 1 ] [ v_vehicle ] ) ) ;
					}
					if(TotalPla == 0)
					{
	            ...
	            case 2:
	            {
					...
					foreach(new i: player_race[playerid])
					{
						...
						format( global_string, sizeof global_string, "%s{"#cWH"}%s[%d] - {"#cBL"}%s\n", global_string, p_info [ i ] [ name ], i, GetVehicleNameEx ( veh_info [ GetPlayerVehicleID ( i ) - 1 ] [ v_vehicle ] ) ) ;
					}
					if(TotalPla == 0)
					{
