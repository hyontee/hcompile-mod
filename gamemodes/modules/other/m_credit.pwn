	...
	switch ( dialogid )
	{
		...
		case d_credit_property_house:
		{
			...
			else
			{
				...
				{
				    if ( houses_info [ i ] [ houses_number ] == h_info [ s_house_id - 1 ] [ h_id ] ) return SendClientMessage ( playerid, col_gray, !"{"#cRInfo"}* {"#cGRInfo"}Дом выставлен на продажу." ), show_give_credit ( playerid ) ;
				}
		...
		case d_credit_property_bizz:
		{
			...
			else
			{
				...
				{
				    if ( business_info [ i ] [ business_number ] == b_info [ business_id ] [ b_id ] ) return SendClientMessage ( playerid, col_gray, !"{"#cRInfo"}* {"#cGRInfo"}Бизнес выставлен на продажу." ), show_give_credit ( playerid ) ;
				}
		...
		case d_businesses_credit:
		{
			if ( ! response ) 
			{
				...
				insert_debtor_message ( "Банк", "{"#cRInfo"}* {"#cGRInfo"}Вам было отказано в кредите!", p_t_info [ playerid ] [ creditor_id ] ) ;
			...
			{
				format ( dialog_string, sizeof dialog_string, "{"#cGInfo"}* {"#cWH"}Вам был одобрен кредит!\n{"#cGInfo"}* {"#cWH"}Перевод можно получить в любом Банке." ) ;
				insert_debtor_message ( "Банк", dialog_string, p_t_info [ playerid ] [ creditor_id ] ) ;
			}
	    ...
	    for ( new i = 0 ; i < rows ; i ++ )
	    {
	        ...
	        if ( db_credit_day == 1 )
	        {
				...
				if ( db_pledge_type == 0 )
				{
					...
					format ( scm_string, sizeof scm_string, "{"#cRInfo"}* {"#cGRInfo"}Вы просрочили кредит. Банк взыщит часть суммы в виде {"#cGN"}%d${"#cGRInfo"}!\n{"#cRInfo"}* {"#cGRInfo"}У Вас будет образована задолженность перед государством!", _collector_money ) ;
					insert_debtor_message ( "Банк", scm_string, db_owner ) ;
					
					format ( scm_string, sizeof scm_string, "{"#cGInfo"}* {"#cWH"}Игрок просрочил кредит!\n\n{"#cGInfo"}* {"#cWH"}Вы автоматически взыскали часть суммы в виде {"#cGN"}%d${"#cWH"}.", _collector_money ) ;
	            ...
	            else if ( db_pledge_type == 1 )
	            {
					...
					format ( scm_string, sizeof scm_string, "{"#cRInfo"}* {"#cGRInfo"}Вы просрочили кредит. Имущество продано!" ) ;
					insert_debtor_message ( "Банк", scm_string, db_owner ) ;
					
					format ( scm_string, sizeof scm_string, "{"#cGInfo"}* {"#cWH"}Игрок просрочил кредит! Имущество продано, деньги переданы в Банк." ) ;
					insert_debtor_message ( "Банк", scm_string, b_info [ db_creditor - 1 ] [ b_owner_inc ] ) ;
	            }
	            ...
	            else if ( db_pledge_type == 3 )
	            {
	                ...
	                format ( scm_string, sizeof scm_string, "UPDATE `cellars` SET `cl_owner` = '-1', `cl_ownername` = 'Собственность штата' WHERE `cl_id` = '%d' LIMIT 1", db_creditor, get_player_account_name ( db_creditor ), db_pledge ) ;
					mysql_tquery ( sql_connection, scm_string ) ;
			...
			format ( scm_string, sizeof scm_string, "{"#cBInfo"}* {"#cWH"}До конца кредита по карте №%d осталось %d дней.", db_id, db_credit_day ) ;
			insert_debtor_message ( "Банк", scm_string, db_owner ) ;
...
callback: callback_delite_credit ( )
{
	...
	for ( new i = 0 ; i < rows ; i ++ )
	{
		...
		insert_debtor_message ( "Кредит", "{"#cRD"}* {"#cGRInfo"}Срок действия Вашей заявки истёк!\n{"#cRD"}* {"#cGRInfo"}Заявка на кредит отклонена.", account_id ) ;
	}
