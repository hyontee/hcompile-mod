...
stock carsmenu_country ( _plate_type, _dest [ ], _size )
{
    switch ( _plate_type )
    {
        case 2: format ( _dest, _size, "RU" ) ;
        case 3: format ( _dest, _size, "UA" ) ;
        case 4: format ( _dest, _size, "BY" ) ;
        case 5: format ( _dest, _size, "KZ" ) ;
        case 6: format ( _dest, _size, "RU POLICE" ) ;
        default: format ( _dest, _size, "UNAVAILABLE" ) ;
    }
    return 1 ;
}
...
stock asdaweaw ( playerid )
{
    ...
    format ( global_string, 512, "\
        SELECT \
            ...
            uv.v_plate_type, uv.v_plate, uv.v_region, \
...
callback: LoadOwnableCarsList ( playerid, bool: showList )
{
    ...
    if ( showList )
    {
        new Node: node = JSON_Array ( ), 
            ...
            numberPlate [ 24 ], country [ 12 ], region [ 12 ], 
        ...
        for ( new i = 0, Node: carNode, carsLoaded ; i < rows ; i ++ )
        {
            ...
            else if ( cache_get_field_content_int ( i, "v_plate_type" ) > 1 )
            {
                cache_get_field_content ( i, "v_plate", numberPlate ) ;
                cache_get_field_content ( i, "v_region", region ) ;
                carsmenu_country ( cache_get_field_content_int ( i, "v_plate_type" ), country, sizeof country ) ;
            }
...
stock show_packet_carsmenu ( playerid, actionId, data [ ] )
{
	...
	switch ( actionId )
    {
        ...
        {
            new _number [ 24 ], country [ 12 ], region [ 12 ],
			...
			cache_get_field_content ( vehicleIdx, "licence_plate_country", country, sql_connection, sizeof country ) ;
			...
			if ( plateType <= 1 && cache_get_field_content_int ( vehicleIdx, "v_plate_type", sql_connection ) > 1 )
			{
				plateType = cache_get_field_content_int ( vehicleIdx, "v_plate_type", sql_connection ) ;
				cache_get_field_content ( vehicleIdx, "v_plate", _number, sql_connection, sizeof _number ) ;
				cache_get_field_content ( vehicleIdx, "v_region", region, sql_connection, sizeof region ) ;
				carsmenu_country ( plateType, country, sizeof country ) ;
			}
