...
// Новая лёгкая PlayerTextDraw-панель капта.
// Показывает объект, очки игрока, очки семьи, лидера и таймер.
new PlayerText:family_capture_td[MAX_PLAYERS][5];
new bool:family_capture_td_created[MAX_PLAYERS];
new bool:family_capture_td_showed[MAX_PLAYERS];
new family_capture_td_type[MAX_PLAYERS];

// Безопасный режим панели захвата.
// На мобильном CRMP клиент может вылетать при показе старых TextDraw возле вышек/завода.
// Поэтому панель захвата больше не показывается текстдравами при входе в зону.
#define FAMILY_CAPTURE_SAFE_TD_MODE 2
...
{
    new capture_type_enter = GetCaptureTypeByAreaId(areaid);
    if(capture_type_enter != -1)
    {
        ...
        if(f < 0 || f >= MAX_FAM)
        {
            printf("[FAMILY_CAPTURE_SAFE] Invalid family index on enter | player:%d family:%d max:%d", playerid, f, MAX_FAM);
            #if defined capt_OnPlayerEnterDynamicArea
                return capt_OnPlayerEnterDynamicArea(playerid, STREAMER_TAG_AREA:areaid);
            #else
                return 0;
        ...
        if(!show_panel_capt[playerid])
            ShowFamilyCaptPanel(playerid, capture_type_enter);
    } 
    
    // Остальной код без изменений
    if(areaid == FAM_CONT_AREA[0] || areaid == FAM_CONT_AREA[1]) 
    {
        if(GetPlayerIdFamily(playerid) != -1) 
            return SendClientMessage(playerid, -1, ""c_f"Вы вошли в зону боев за контейнер, в этой зоне Ваша законопослушность не будет уменьшаться.");
    }
    ...
    if(areaid == FAM_CONT_AREA_P[0] || areaid == FAM_CONT_AREA_P[1])
    {
        ...
        if(FAM_CONT_BOX[number])
        {
            if(GetPVarInt(playerid, "box_in_hard")) 
                return SendClientMessage(playerid, -1, ""c_f_r"У вас уже есть на руках ящик.");
    ...
    if(IsPlayerInDynamicArea(playerid, FAM_CONT_AREA[0]) || IsPlayerInDynamicArea(playerid, FAM_CONT_AREA[1]) && GetPlayerIdFamily(playerid) != -1)
    {
        ...
        foreach(new i : Vehicle)
        {
            ...
            {
                if(!GetPVarInt(playerid, "box_in_hard")) 
                    return SendClientMessage(playerid, -1, ""c_f_r"У вас нет на руках ящика.");
                ...
                if(GetCarFamily(idx, V_F_COUNT_BOX) != 50) 
                    SendClientMessage(playerid, -1, ""c_f"Вы положили ящик в автомобиль, отправляйтесь обратно в контейнер за новым.");
                else 
                    SendClientMessage(playerid, -1, ""c_f"Вы положили ящик в автомобиль, отправляйтесь в место сдачи ящиков");
            }
...
stock GetCaptureTypeByAreaId(areaid)
{
    for(new i; i < sizeof(CAPTURE_AREA); i++)
    {
        if(CAPTURE_AREA[i] == -1) continue;
        if(areaid == CAPTURE_AREA[i]) return i;
    }
    return -1;
}
...
stock GetPlayerCaptArea(playerid)
{
    ...
    {
        if(CAPTURE_AREA[i] == -1) continue;
        if(IsPlayerInDynamicArea(playerid, CAPTURE_AREA[i])) return i;
    }
    return -1;
}
...
{
    new type_capt = GetCaptureTypeByAreaId(areaid);
    if(type_capt != -1)
    {
        new f = GetPlayerIdFamily(playerid);

        // КРИТИЧЕСКАЯ ПРОВЕРКА
        if(f == -1 || type_capt == -1 || !CAPTURE_STATUS[type_capt]) 
        {
            printf("[DEBUG] Игрок %d покинул зону захвата, но условия не выполнены (f:%d, type:%d, status:%d)", 
                playerid, f, type_capt, CAPTURE_STATUS[type_capt]);
        ...
        // Проверка диапазона массивов
        if(f < 0 || f >= MAX_FAM || type_capt < 0 || type_capt >= 5)
        {
            printf("[ERROR] Выход за границы массива! f:%d (MAX:%d), type_capt:%d", f, MAX_FAM, type_capt);
        ...
        printf("[DEBUG] Игрок %d покидает зону type:%d | Family EXP: %d | Player EXP: %d", 
            playerid, type_capt, CAPTURE_FAMILY_EXP[f][type_capt], CAPTURE_PLAYER_EXP[playerid]);
...
stock StartFCapture(type_capt)
{
    ...
    for(new i;i < welsi;i++)
    {
        ...
        format(string, sizeof string, "{fad201}|{FFFFFF} Началось сражение за {fad201}%s{FFFFFF}, найдите место проведение с помощью — карты", name_object[e[i]]);
...
public OnPlayerDisconnect(playerid, reason)
{
    ...
    CAPTURE_PLAYER_EXP[playerid] = 0;
    HideFamilyCapturePlayerTD(playerid);
    DestroyFamilyCapturePlayerTD(playerid);
    ...
    if(GetPlayerIdFamily(playerid) != -1 && CAPTURE_STATUS[0] || CAPTURE_STATUS[1] || CAPTURE_STATUS[2])
    {
        if(CAPTURE_PLAYER_EXP[playerid] != 0)
        {
            ...
            if(killerid != INVALID_PLAYER_ID)
            {
                ...
                // ? КРИТИЧЕСКАЯ ПРОВЕРКА:
                if(f != -1 && f >= 0 && f < MAX_FAM && capt != -1)
                {
                    format(string, sizeof string, "Игрок %s[%d] убил врага и заработал %d очков", 
                        GetPlayerNameEx(killerid), killerid, CAPTURE_PLAYER_EXP[playerid]);
                    SendFamilyMessage(f, string);
            ...
            format(string, sizeof string, "Игрок %s[%d] был убит на сражении и потерял %d очков", 
                GetPlayerNameEx(playerid), playerid, CAPTURE_PLAYER_EXP[playerid]);
            SendFamilyMessage(GetPlayerIdFamily(playerid), string);
            
            if(capt != -1 && CAPTURE_FAMILY_EXP[GetPlayerIdFamily(playerid)][capt]-CAPTURE_PLAYER_EXP[playerid] >= 0)
            {
                CAPTURE_FAMILY_EXP[GetPlayerIdFamily(playerid)][capt] -= CAPTURE_PLAYER_EXP[playerid];
                if(CAPTURE_LEADER[capt][0] == GetPlayerIdFamily(playerid))   
                    CAPTURE_LEADER[capt][1] -= CAPTURE_PLAYER_EXP[playerid];
            } 
            CAPTURE_PLAYER_EXP[playerid] = 0;
        }
    }
...
stock CreateFamilyCapturePlayerTD(playerid)
{
    if(family_capture_td_created[playerid]) return 1;

    family_capture_td[playerid][0] = CreatePlayerTextDraw(playerid, 7.0000, 247.0000, "LD_SPAC:white");
    PlayerTextDrawTextSize(playerid, family_capture_td[playerid][0], 162.0000, 78.0000);
    PlayerTextDrawAlignment(playerid, family_capture_td[playerid][0], 1);
    PlayerTextDrawColor(playerid, family_capture_td[playerid][0], 0x000000AA);
    PlayerTextDrawBackgroundColor(playerid, family_capture_td[playerid][0], 255);
    PlayerTextDrawFont(playerid, family_capture_td[playerid][0], 4);
    PlayerTextDrawSetProportional(playerid, family_capture_td[playerid][0], 0);
    PlayerTextDrawSetShadow(playerid, family_capture_td[playerid][0], 0);

    family_capture_td[playerid][1] = CreatePlayerTextDraw(playerid, 12.0000, 249.0000, "СЕМЕЙНЫЙ КАПТ");
    PlayerTextDrawLetterSize(playerid, family_capture_td[playerid][1], 0.2300, 1.1600);
    PlayerTextDrawAlignment(playerid, family_capture_td[playerid][1], 1);
    PlayerTextDrawColor(playerid, family_capture_td[playerid][1], 0xFAD201FF);
    PlayerTextDrawBackgroundColor(playerid, family_capture_td[playerid][1], 255);
    PlayerTextDrawFont(playerid, family_capture_td[playerid][1], 2);
    PlayerTextDrawSetProportional(playerid, family_capture_td[playerid][1], 1);
    PlayerTextDrawSetShadow(playerid, family_capture_td[playerid][1], 0);

    family_capture_td[playerid][2] = CreatePlayerTextDraw(playerid, 12.0000, 263.0000, "Объект: -");
    PlayerTextDrawLetterSize(playerid, family_capture_td[playerid][2], 0.1800, 1.0800);
    PlayerTextDrawAlignment(playerid, family_capture_td[playerid][2], 1);
    PlayerTextDrawColor(playerid, family_capture_td[playerid][2], -1);
    PlayerTextDrawBackgroundColor(playerid, family_capture_td[playerid][2], 255);
    PlayerTextDrawFont(playerid, family_capture_td[playerid][2], 1);
    PlayerTextDrawSetProportional(playerid, family_capture_td[playerid][2], 1);
    PlayerTextDrawSetShadow(playerid, family_capture_td[playerid][2], 0);

    family_capture_td[playerid][3] = CreatePlayerTextDraw(playerid, 12.0000, 277.0000, "Ваши очки: 0 | Семья: 0");
    PlayerTextDrawLetterSize(playerid, family_capture_td[playerid][3], 0.1800, 1.0800);
    PlayerTextDrawAlignment(playerid, family_capture_td[playerid][3], 1);
    PlayerTextDrawColor(playerid, family_capture_td[playerid][3], -1);
    PlayerTextDrawBackgroundColor(playerid, family_capture_td[playerid][3], 255);
    PlayerTextDrawFont(playerid, family_capture_td[playerid][3], 1);
    PlayerTextDrawSetProportional(playerid, family_capture_td[playerid][3], 1);
    PlayerTextDrawSetShadow(playerid, family_capture_td[playerid][3], 0);

    family_capture_td[playerid][4] = CreatePlayerTextDraw(playerid, 12.0000, 291.0000, "Лидер: нет | Время: 00:00");
    PlayerTextDrawLetterSize(playerid, family_capture_td[playerid][4], 0.1800, 1.0800);
    PlayerTextDrawAlignment(playerid, family_capture_td[playerid][4], 1);
    PlayerTextDrawColor(playerid, family_capture_td[playerid][4], -1);
    PlayerTextDrawBackgroundColor(playerid, family_capture_td[playerid][4], 255);
    PlayerTextDrawFont(playerid, family_capture_td[playerid][4], 1);
    PlayerTextDrawSetProportional(playerid, family_capture_td[playerid][4], 1);
    PlayerTextDrawSetShadow(playerid, family_capture_td[playerid][4], 0);

    family_capture_td_created[playerid] = true;
    family_capture_td_showed[playerid] = false;
    family_capture_td_type[playerid] = -1;
    return 1;
}

stock UpdateFamilyCapturePlayerTD(playerid, t)
{
    if(t < 0 || t >= sizeof(CAPTURE_AREA)) return 0;

    new f = GetPlayerIdFamily(playerid);
    if(f < 0 || f >= MAX_FAM) return 0;

    CreateFamilyCapturePlayerTD(playerid);

    new td_text[180], leader_name[48];
    if(CAPTURE_LEADER[t][0] >= 0 && CAPTURE_LEADER[t][0] < MAX_FAM)
        format(leader_name, sizeof leader_name, "%s", GetFamily(CAPTURE_LEADER[t][0], family_name));
    else
        format(leader_name, sizeof leader_name, "нет");

    format(td_text, sizeof td_text, "Объект: %s", name_object[t]);
    PlayerTextDrawSetString(playerid, family_capture_td[playerid][2], td_text);

    format(td_text, sizeof td_text, "Ваши очки: %d | Семья: %d", CAPTURE_PLAYER_EXP[playerid], CAPTURE_FAMILY_EXP[f][t]);
    PlayerTextDrawSetString(playerid, family_capture_td[playerid][3], td_text);

    format(td_text, sizeof td_text, "Лидер: %s (%d) | Время: %d:%02d", leader_name, CAPTURE_LEADER[t][1], ConvertUnixTime(CAPTURE_SECOND[t], CONVERT_TIME_TO_MINUTES), ConvertUnixTime(CAPTURE_SECOND[t], CONVERT_TIME_TO_SECONDS));
    PlayerTextDrawSetString(playerid, family_capture_td[playerid][4], td_text);

    family_capture_td_type[playerid] = t;
    return 1;
}

stock ShowFamilyCapturePlayerTD(playerid, t)
{
    if(t < 0 || t >= sizeof(CAPTURE_AREA)) return 0;

    UpdateFamilyCapturePlayerTD(playerid, t);

    if(!family_capture_td_showed[playerid])
    {
        for(new i; i < 5; i++) PlayerTextDrawShow(playerid, family_capture_td[playerid][i]);
        family_capture_td_showed[playerid] = true;
    }
    return 1;
}

stock HideFamilyCapturePlayerTD(playerid)
{
    if(!family_capture_td_created[playerid]) return 1;
    if(family_capture_td_showed[playerid])
    {
        for(new i; i < 5; i++) PlayerTextDrawHide(playerid, family_capture_td[playerid][i]);
    }
    family_capture_td_showed[playerid] = false;
    family_capture_td_type[playerid] = -1;
    return 1;
...
stock DestroyFamilyCapturePlayerTD(playerid)
{
    if(!family_capture_td_created[playerid]) return 1;
    for(new i; i < 5; i++) PlayerTextDrawDestroy(playerid, family_capture_td[playerid][i]);
    family_capture_td_created[playerid] = false;
    family_capture_td_showed[playerid] = false;
    family_capture_td_type[playerid] = -1;
    return 1;
}
...
stock ShowFamilyCaptPanel(playerid, t)
{
    ...
    #if FAMILY_CAPTURE_SAFE_TD_MODE == 0
        for(new i; i < 9; i++) TextDrawShowForPlayer(playerid, fam_TD[t][i]);
        TextDrawShowForPlayer(playerid, fam_PTD[f][t]);
    #else
        ShowFamilyCapturePlayerTD(playerid, t);
    #endif
...
stock HideFamilyCaptPanel(playerid, t)
{
    ...
    show_panel_capt[playerid] = false;
        ...
        if(f >= 0 && f < MAX_FAM)
        {
            ...
            TextDrawHideForPlayer(playerid, fam_PTD[f][t]);
        }
    #else
        HideFamilyCapturePlayerTD(playerid);
    #endif
...
public:CaptureUpd(type)
{
    ...
    for(new i; i < MAX_FAM;i++)
    {
        ...
        foreach(new i : Player)
        {
            ...
            if(family_capture_td_type[i] != type) continue;
            if(GetPlayerIdFamily(i) == -1) { HideFamilyCaptPanel(i, type); continue; }
            if(!IsPlayerInDynamicArea(i, CAPTURE_AREA[type])) { HideFamilyCaptPanel(i, type); continue; }
            UpdateFamilyCapturePlayerTD(i, type);
        }
    #endif
...
{   
    for(new i;i < sizeof(fam_TD);i++)
    {
        ...
        fam_TD[i][2] = TextDrawCreate(10.3332, 252.2221, "Ваши очки:"); // пусто
        ...
        fam_TD[i][3] = TextDrawCreate(10.3332, 266.7406, "Очки лидера:"); // пусто
        ...
        fam_TD[i][7] = TextDrawCreate(40.6666, 284.9925, "Время:");
...
stock StartFCont(type_capt)
{
    ...
    for(new i;i < welsi;i++)
    {
        ...
        format(string, sizeof string, "{fad201}|{FFFFFF} Началось сражение за {fad201}Семейные контейнеры{FFFFFF}, найдите место проведение с помощью — карты");
...
// Stock для обработки KEY_CROUCH
stock HandleFamilyCrouch(playerid)
{
    ...
    format(f_string144, sizeof f_string144, "%s[%d] пригнал машину на черный рынок, семья получила %d реп., — %d рублей", 
