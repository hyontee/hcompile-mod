...
stock MoveFamilyCar(database_car, playerid)
{
    ...
    (
        ...
        (owner_id, model_id, comfort, sport, sport_plus, drift, wheels_kl, wheels_size, wheels_raz, wheels_otkl, \
        color_1, color_2, pos_x, pos_y, pos_z, angle, number, region, number_type, \
        status, alarm, key_in, mileage, create_time, health, \
        vinilcar, pt_engine, pt_brake, pt_stability, nitro, launch, fars, diski, fuel) \
        VALUES \
        (%d, %d, 0, 0, 0, 0, 0, 0, 0, 0, \
        %d, %d, %f, %f, %f, %f, '%s', '%s', %d, \
        0, 0, 0, 0, %d, 1000, \
        0, 0, 0, 0, 0, 0, 0, 0, 50.0)",
        GetPlayerAccountID(playerid), model_id,
        color_1, color_2, pos_x, pos_y, pos_z, angle, number, region, number_type,
        gettime()
    );
    mysql_query(mysql, query, false);
...
{
        if(GetPlayerData(playerid, P_TOTALHOUR) < 4)
        return SendClientMessage(playerid, 0x999999FF, "Нужно отыграть 4 часа, чтобы пользоваться складом семьи");
    switch(type)
    {
        case 1:
        {
