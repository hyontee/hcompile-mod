...
CMD:showtools(playerid, params[])
{
    ...
    switch(screenType)
    {
        ...
        default: ShowNotificationSile(playerid, 0, 3, 0, 0, "Хуйню высрал", "");
    }
	...
	switch(screenType)
	{
		case 1:
		{
			...
			switch(buttonId)
			{
				...
				case 2:
				{
					...
					else ShowNotificationSile(playerid, 0, 3, 0, 0, "Игрок не найден", "");
				}
				case 3:
				{
		...
		case 2:
		{
			...
			switch(tableButtonId)
			{
				case 1, 2: 
				{
						if(GetPlayerAdminEx(playerid) < 1) return ShowNotificationSile(playerid, 2, 5, -1, -1, "Вам не доступно использование данной функции",  "");
						
						new fmt_msg[128], bn[64];
						JSON_GetString(json, "bn", bn, sizeof(bn));
						format(fmt_msg, sizeof fmt_msg, "Администратор %s кикнул игрока %s. Причина: %s", GetPlayerNameEx(playerid), GetPlayerNameEx(to_player), bn);
						...
						new fmt_text[512];
						format(fmt_text, sizeof fmt_text, "[A] Администратор %s[%d] кикнул %s[%d]. Причина: %s", GetPlayerNameEx(playerid), playerid, GetPlayerNameEx(to_player), to_player, bn);
				...
				case 50..60: 
				{
					...
					new fmt_msg[128], bn[128], br;
					JSON_GetString(json, "bn", bn, sizeof(bn));
					JSON_GetInt(json, "br", br);
					format(fmt_msg, sizeof fmt_msg, "Администратор %s выдал блокировку чата игроку %s на %d мин. Причина: %s", GetPlayerNameEx(playerid), GetPlayerNameEx(to_player), br, bn);

					 SendClientMessageToAll(0xFF5533FF, fmt_msg);

					SetPlayerData(to_player, P_MUTE, br * 60);
					UpdatePlayerDatabaseInt(to_player, "mute", br * 60);
					...
					GetPlayerNameEx(to_player), GetPlayerAccountID(to_player), br, bn);
				...
				case 80..89: 
				{
					...
					JSON_GetInt(json, "br", br);
					JSON_GetString(json, "bn", bn, sizeof(bn));
					format(fmt_msg, sizeof fmt_msg, "Администратор %s посадил в тюрьму игрока %s на %d мин. Причина: %s", GetPlayerNameEx(playerid), GetPlayerNameEx(to_player), br, bn);

					SendClientMessageToAll(0xFF5533FF, fmt_msg);

					SendClientMessage(to_player, 0xCECECEFF, "Время до окончания заключения: {CCCC00}/time");

					JailPlayer(to_player, br * 60);
				...
				case 100..107: 
				{
					if(GetPlayerAdminEx(playerid) < 2) return ShowNotificationSile(playerid, 2, 5, -1, -1, "Вам не доступно использование данной функции",  "");
					...
					new fmt_msg[128], bn[128];
					JSON_GetString(json, "bn", bn, sizeof(bn));
					format(fmt_msg, sizeof fmt_msg, "Администратор %s выдал предупреждение игроку %s [%d|3]. Причина: %s", GetPlayerNameEx(playerid), GetPlayerNameEx(to_player), GetPlayerData(to_player, P_WARN), bn);
					...
					format(fmt_msg, sizeof fmt_msg, "Выдал варн %s[acc:%d] (%d/3). Причина: %s", GetPlayerNameEx(to_player), uid, warns, bn);
					SendLog(playerid, LOG_TYPE_ADMIN_ACTION, fmt_msg);
				...
				case 18..30:
				{
					...
					JSON_GetInt(json, "br", br);
   					...
   					if(GetPlayerAdminEx(playerid) < 3)
       					return ShowNotificationSile(playerid, 2, 3, 0, 0, "У вас нет доступа к использованию данной функции", " ");

   					if(!IsPlayerConnected(to_player) || !IsPlayerLogged(to_player))
       					return ShowNotificationSile(playerid, 2, 4, 0, 0, "Такого игрока нет", " ");
					   ...
					   {
							new announce[256];
   					 	format(announce, sizeof announce, "Администратор %s навсегда заблокировал игрока %s. Причина: %s", GetPlayerNameEx(playerid), GetPlayerNameEx(to_player), bn);
   					 	SendClientMessageToAll(0xFF5533FF, announce);
					   } 
					   else 
					   {
   						new announce[256];
   						format(announce, sizeof announce, "Администратор %s заблокировал игрока %s на %d дней. Причина: %s", GetPlayerNameEx(playerid), GetPlayerNameEx(to_player), br, bn);
   						SendClientMessageToAll(0xFF5533FF, announce);
  				 	} 
   
  					new fmt_text[512];
  					format(fmt_text, sizeof fmt_text, "[A] %s[%d] забанил игрока игрока %s[%d]. Причина: %s", GetPlayerNameEx(playerid), playerid, GetPlayerNameEx(to_player), to_player, bn);
					  SendMessageToAdmins(fmt_text, 0x999999FF);

   					InfoAstats[1] += 1;

   					new log_msg[256];
   					format(log_msg, sizeof log_msg, "Забанил %s[acc:%d] на %d дней. Причина: %s", GetPlayerNameEx(to_player), GetPlayerAccountID(to_player), br, bn);
   					SendLog(playerid, LOG_TYPE_SUPERADMIN_ACTION, log_msg);

   					AddBan(GetPlayerAccountID(to_player), gettime(), br, GetPlayerIpEx(to_player), bn, GetPlayerNameEx(playerid));
					BanEx(to_player, bn);
   					Kick(to_player);
		...
		case 3:
		{
			...
			ShowNotificationSile(playerid, 2, 5, -1, -1, f_string144, "");
				switch(tableButtonId)
				{
					case 1:
					{
