...
{
    // Выдаём через BPR систему
    Cases_GiveRewardViaBPR(playerid, rewardType, rewardValue, rewardCount, rewardRarity);
    
    // Для отладки
    printf("[Cases] Reward given: type=%d, value=%d, count=%d, rarity=%d", 
        rewardType, rewardValue, rewardCount, rewardRarity);
    
    return 1;
}

// Добавь эту функцию в cases.pwn для выдачи наград через систему BPR
stock Cases_GiveRewardViaBPR(playerid, rewardType, rewardValue, rewardCount, rewardRarity)
{
    switch(rewardType)
    {
        case REWARD_TYPE_MONEY:  // Деньги
        {
            BPR_GiveCurrency(playerid, 2, "Деньги", rewardCount, 0);
            printf("[Cases] Player %d received $%d via BPR", playerid, rewardCount);
            return 1;
        }
        
        case REWARD_TYPE_BC:  // Black Coins
        {
            BPR_GiveCurrency(playerid, 3, "Black Coins", rewardCount, 0);
            printf("[Cases] Player %d received %d BC via BPR", playerid, rewardCount);
            return 1;
        }
        
        case REWARD_TYPE_CASE:  // Кейс
        {
            BPR_GiveCase(playerid, rewardValue, "Кейс из награды", rewardRarity, rewardCount);
            printf("[Cases] Player %d received case ID %d via BPR", playerid, rewardValue);
            return 1;
        }
        
        case REWARD_TYPE_VEHICLE:  // Машина
        {
            BPR_GiveCar(playerid, rewardValue, "Автомобиль из кейса", rewardRarity, 365, 500);
            printf("[Cases] Player %d received car model %d via BPR", playerid, rewardValue);
            return 1;
        }
        
        case REWARD_TYPE_VIP:  // VIP
        {
            BPR_GiveCurrency(playerid, 9, "VIP", rewardValue, rewardCount);
            printf("[Cases] Player %d received VIP type %d for %d days via BPR", playerid, rewardValue, rewardCount);
            return 1;
        }
        
        case REWARD_TYPE_ITEM:  // Предмет в инвентарь
        {
            BPR_GiveAccessory(playerid, rewardValue, -1, "Предмет из кейса", rewardRarity, 30, 0);
            printf("[Cases] Player %d received item ID %d via BPR", playerid, rewardValue);
            return 1;
        }
        ...
        case REWARD_TYPE_DUST:  // Пыль
        {
            ...
            printf("[Cases] Player %d received %d dust via Cases_GiveDust", playerid, rewardCount);
            return 1;
        }
        
        default:
        {
            // Для неизвестных типов - просто даём через старую систему
            BPR_GiveReward(playerid, rewardType, rewardValue, "Reward", rewardRarity, rewardCount, 30, 0, -1);
            printf("[Cases] Player %d received unknown type %d via BPR", playerid, rewardType);
            return 1;
        }
    }
}
