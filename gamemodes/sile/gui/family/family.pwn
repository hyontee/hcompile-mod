...
// FIX: "fam" removed from here - it is its own command (family chat, see CMD:fam below).
// Having it both as an alias of /family and as a standalone CMD:fam registered "fam" twice
// with Pawn.CMD, which is undefined behaviour (one silently shadows the other at load time).
