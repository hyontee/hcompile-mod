//>> Файл: bykranin_src/systems/blackpass.pwn  (2138 строк)
//>> Тема: самостоятельные модули (были отдельными файлами в исходном проекте)
//>> Внутри: BlackPass_LogEvent, BlackPass_ResetPlayer, BlackPass_GetLevelFromExperience, BlackPass_GetMaxExperience, BlackPass_BuildClaimFlagsString, BlackPass_ParseClaimFlagsString, BlackPass_GetCurrentLevel
//>> Файл подключается из gamemodes/bykranin.pwn; порядок include менять нельзя (см. bykranin_src/GUIDE.md).
// BEGIN INLINED: include/system/blackpass.pwn
...
stock BlackPass_QueueInventoryReward(playerid, itemid)
{
    ...
    if(g_PlayerRewardsCount[playerid] >= MAX_CASE_REWARDS)
    {
        printf("[BLACKPASS][REWARD][QUEUE][ERROR] player=%d item=%d storage full", playerid, itemid);
        return 0;
    }

    printf("[BLACKPASS][REWARD][QUEUE] player=%d item=%d", playerid, itemid);

    new idx = g_PlayerRewardsCount[playerid];
    g_PlayerRewards[playerid][idx][rCase] = BLACKPASS_REWARD_CASE_ID;
    g_PlayerRewards[playerid][idx][rItem] = itemid;
    g_PlayerRewards[playerid][idx][rBonus] = 0;
    g_PlayerRewardsCount[playerid]++;

    SavePlayerCaseRewardDB(playerid, BLACKPASS_REWARD_CASE_ID, itemid, false);
    return 1;
}
...
// END INLINED: include/system/blackpass.pwn
