//>> Файл: bykranin_src/systems/weekly_prizes.pwn  (296 строк)
//>> Тема: самостоятельные модули (были отдельными файлами в исходном проекте)
//>> Внутри: CREATE_ACCOUNTS_TABLE_WP, LoadWeeklyPrizes, UnLoadWeeklyPrizes, OnPlayerConnect, OnPlayerDisconnect, OnGameModeInit, CheckWeeklyPrizes
//>> Файл подключается из gamemodes/bykranin.pwn; порядок include менять нельзя (см. bykranin_src/GUIDE.md).
// BEGIN INLINED: include/system/weekly_prizes.pwn
enum Weekly_Prizes
{
    WP_Name[24],
    WP_Type,
...
public:CREATE_ACCOUNTS_TABLE_WP()
{
    ...
    if(mysql_errno())
    {
        ...
        if(!mysql_errno()) print("[KRANIN_SYSTEM] Столбцы созданы для еженедельных наград");
    }
...
public OnGameModeInit()
{
    ...
    print("[KRANIN_SYSTEM] Система еженедельных наград загружена.");
...
#endif
// END INLINED: include/system/weekly_prizes.pwn
