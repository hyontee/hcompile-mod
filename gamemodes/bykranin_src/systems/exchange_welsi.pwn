//>> Файл: bykranin_src/systems/exchange_welsi.pwn  (813 строк)
//>> Тема: самостоятельные модули (были отдельными файлами в исходном проекте)
//>> Внутри: OnGameModeInit, OnPlayerDisconnect, OnPlayerConnect, OnDialogResponse, /changeprop, /yeschange, Exchange
//>> Файл подключается из gamemodes/bykranin.pwn; порядок include менять нельзя (см. bykranin_src/GUIDE.md).
// BEGIN INLINED: include/system/exchange_welsi.pwn
#define E_HOUSE 1
#define E_VEHICLE   2
#define E_BUSINESS  3
...
{
    print("[KRANIN_SYSTEM] Команда для обмена загружена.");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
        if(dialogid == 1610)
    {
        if(response)
        {
            ...
            if(is_select_other)
            {
                ...
                ShowNotificationNew(playerid, 1, 3, 0, 0, "Выберите имущество игрока", "");
            }
            else
            {
        ...
        if(dialogid == 1612)
    {
        if(response)
        {
            ...
            {
                ShowNotificationNew(playerid, 1, 3, 0, 0, "Выберите тип имущества игрока", "");
...
// END INLINED: include/system/exchange_welsi.pwn
