//>> Файл: bykranin_src/systems/core/vehicle/auto-race/callbacks.pwn  (43 строк)
//>> Тема: самостоятельные модули (были отдельными файлами в исходном проекте)
//>> Внутри: (данные/макросы)
//>> Файл подключается из gamemodes/bykranin.pwn; порядок include менять нельзя (см. bykranin_src/GUIDE.md).
// BEGIN INLINED: modules/core/vehicle/auto-race/callbacks.pwn
/*
	@__OnServerTimerAutoRace
*/

forward @__OnServerTimerAutoRace();
@__OnServerTimerAutoRace()
{
	new
		ar_playerid,
	...
	if (--s_autorace_Server_Countdown[_:ar_type] == 0) {
		for (new i = s_autorace_Server_Count[_:ar_type] - 1; i >= 0; i--) {
...
// END INLINED: modules/core/vehicle/auto-race/callbacks.pwn
