//>> Файл: bykranin_src/systems/inventory_skin.pwn  (593 строк)
//>> Тема: самостоятельные модули (были отдельными файлами в исходном проекте)
//>> Внутри: /myskinsz, CheckSkinPlayer, GivePlayerOwnableSkin, ShowOwnableSkinLoadDialog, OnDialogResponse
//>> Файл подключается из gamemodes/bykranin.pwn; порядок include менять нельзя (см. bykranin_src/GUIDE.md).
// BEGIN INLINED: include/system/inventory_skin.pwn
/*
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == 1790)
    {
        if(response)
        {
            if(listitem == 0 && GetPVarInt(playerid, "Form"))
            {
                SendClientMessage(playerid, 0xCECECEFF, "Сначала закончите рабочий день и снимите форму в раздевалке");
                return 1;
            }
...
#endif
// END INLINED: include/system/inventory_skin.pwn
