...
{
    new stored = GetPlayerData(playerid, P_GARAGE_TYPE);
    ...
    // Backward compatibility: older builds stored MySQL garages.id in P_GARAGE_TYPE.
    new idx = GetGarageIndexByDbId(stored);
    if(idx != -1 && GetGarageData(idx, GARAGE_OWNER_ID) == account_id)
    {
        SetPlayerData(playerid, P_GARAGE_TYPE, idx);
        return idx;
    }
...
stock SyncPlayerGarageData(playerid)
{
 ...
 if (cache_num_rows() > 0)
 {
     ...
     if (garage_id > 0)
     {
         ...
         SetPlayerData(playerid, P_GARAGE_TYPE, runtime_idx);
...
stock HandleGarageHorn(playerid, newkeys, oldkeys)
{
    // React only to a new horn press, not to a held key.
    ...
    // Inside garage: horn at the gate means vehicle exit.
    new in_garage = GetPlayerInGarage(playerid);
    if(in_garage != -1 && in_garage < g_garage_loaded)
    {
        new bool:is_gate = false;
        if(GetGarageData(in_garage, GARAGE_IMPROVEMENTS) == 2)
        {
            is_gate = IsPlayerInRangeOfPoint(playerid, 8.0, 492.663848, 1988.835205, 1548.586645);
        }
        else
        {
            is_gate = IsPlayerInRangeOfPoint(playerid, 8.0, -0.2940, 2006.1188, 1554.2031);
        }

        if(!is_gate) return 1;

        new Float:exit_x = GetGarageData(in_garage, GARAGE_EXIT_X);
        new Float:exit_y = GetGarageData(in_garage, GARAGE_EXIT_Y);
        new Float:exit_z = GetGarageData(in_garage, GARAGE_EXIT_Z);
        new Float:exit_a = GetGarageData(in_garage, GARAGE_EXIT_ANGLE);
        ...
        for(new i = 0; i < MAX_PLAYERS; i++)
        {
            ...
            SetPlayerData(i, P_IN_GARAGE, -1);
            SetCameraBehindPlayer(i);
        }

        FreeGarageVehicleSlot(in_garage, vehicleid);
    ...
    // Outside: horn near the player's own garage means vehicle entry.
    ...
    if(!IsPlayerInRangeOfPoint(playerid, 7.0, GetGarageData(garageid, GARAGE_X), GetGarageData(garageid, GARAGE_Y), GetGarageData(garageid, GARAGE_Z))) return 0;

    if(GetGarageData(garageid, GARAGE_IMPROVEMENTS) == 2 && FindFreeGarageSlot(garageid) == -1)
        return SendClientMessage(playerid, 0xCECECEFF, "\xc2 \xe3\xe0\xf0\xe0\xe6\xe5 \xed\xe5\xf2 \xf1\xe2\xee\xe1\xee\xe4\xed\xfb\xf5 \xec\xe5\xf1\xf2.");

    if(!PutVehicleDirectlyInGarage(vehicleid, garageid)) return 0;

    SetPlayerData(playerid, P_IN_GARAGE, garageid);
...
{
    if(HandleGarageHorn(playerid, newkeys, oldkeys)) return 1;
...
stock SavePlayerGarageOwnership(playerid)
{
    ...
    // Recover the link from garages.owner_id if P_GARAGE_TYPE was lost/reset.
    ...
    if(!mysql_errno(mysql) && cache_num_rows() > 0)
    {
        ...
        if(runtime_idx != -1) SetPlayerData(playerid, P_GARAGE_TYPE, runtime_idx);
        return 1;
    }
    cache_delete(result);
    return 0;
