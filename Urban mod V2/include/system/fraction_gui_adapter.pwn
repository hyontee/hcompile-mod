...
// guiid for the fraction/organization menu screen (page: FRACTION_GUI_PAGE_*, see below).
// Kept as a distinct id so it does not collide with existing case labels in IPacket:252 (ipacket.inc).
#if !defined GUIFractionSystem
    #define GUIFractionSystem (46)
#endif

// guiid of the client's Family System screen. The org vehicle garage (OrgTransportGui*)
// deliberately reuses this exact screen/layout, so server-side dispatch (case 45 in
// IPacket:252) must check OrgTransportGuiIsOpen() to route packets between the real
// family system and the org transport garage while they share the same guiid.
#if !defined GUIFamilySystem
    #define GUIFamilySystem (45)
#endif

// ShowPlayerGUI/HidePlayerGUI already exist elsewhere in the project (HidePlayerGUI is a
// thin wrapper around SendPacketToClient). UpdatePlayerGUI and OnPacketIncoming did not
// exist yet anywhere in the project (confirmed by compiler error 017), so they are added
// here as the same kind of thin wrapper: both just push the given JSON to the client on
// the given guiid channel - "update" for pages of an already-open GUI, and
// OnPacketIncoming for pushing an out-of-band response (e.g. action result) to the client.
stock UpdatePlayerGUI(playerid, guiid, Node:json)
{
    return SendPacketToClient(playerid, guiid, json);
}
