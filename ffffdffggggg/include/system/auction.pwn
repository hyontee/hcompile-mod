...
{
    new query[3072];
    gAuctionDatabaseReady = false;

    // errno 2006/2013/2055 means the old connection is no longer usable.
    if(!MySQL_Reconnect())
    {
        printf("[AUCTION] MySQL is unavailable. Retry in 5 seconds.");
    ...
    if(err)
    {
        printf("[AUCTION] auction_lots query failed, errno=%d", err);

        if(err == 2006 || err == 2013 || err == 2055)
        {
            MySQL_Reconnect();
        }
    ...
    if(err)
    {
        printf("[AUCTION] auction_notifications query failed, errno=%d", err);
