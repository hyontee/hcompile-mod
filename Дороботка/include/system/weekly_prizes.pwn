...
{
    {"100 доната-руб.", WP_RUB, 100},
    {"500000 руб.", WP_MONEY, 500000},
    {"150 доната-руб.", WP_RUB, 150},
    {"VIP подписка (3 дня.)", WP_VIP, 3},
    {"30000000 руб.", WP_MONEY, 30000000},
    {"500 доната-руб.", WP_RUB, 500},
    {"3 слот на машину", WP_SLOT, 3}
};
...
new player_wp[MAX_PLAYERS][8]; // [0..6] - статусы дней, [7] - флаг получения награды сегодня (0 - не получал, 1 - получал)
...
public:CREATE_ACCOUNTS_TABLE_WP()
{
    ...
    if(mysql_errno())
    {
        ...
        if(!mysql_errno()) print("[Lonexs_SYSTEM] Столбцы созданы для еженедельных наград");
    }
...
public:LoadWeeklyPrizes(playerid)
{
    ...
    if(!mysql_errno())
    {
        ...
        sscanf(weekly, "P<,>a<i>[7]", player_wp[playerid]); // Читаем ровно 7 элементов статусов дней
...
public OnGameModeInit()
{
    ...
    print("[Lonexs_SYSTEM] Система еженедельных наград загружена.");
...
public:CheckWeeklyPrizes()
{
    ...
    {
        if(player_wp[i][7]) continue; // Если уже получал награду за отыгрыш сегодня — пропускаем

        // Ищем первый доступный день со статусом WAITING (0)
        new target_day = -1;
        for(new q = 0; q < 7; q++)
        {
            if(player_wp[i][q] == STATUS_WP_WAITING)
            {
                // Проверяем, что предыдущие дни получены (или это самый первый день)
                if(q == 0 || player_wp[i][q - 1] == STATUS_WP_TOOK_IT)
                {
                    target_day = q;
                }
                break;
        ...
        // Если отыграл 2 часа (7200 секунд)
        if(GetPlayerData(i, P_GAME_FOR_DAY) >= 7200)
        {
            SendClientMessage(i, -1, "Поздравляем, вы отыграли 2 часа и можете забрать свою награду! ({FFFF00}/everyprize{FFFFFF})");
            player_wp[i][target_day] = STATUS_WP_MAYBE; // Меняем статус на "Можно получить"
            player_wp[i][7] = 1;                        // Ставим флаг, что за сегодня награда разблокирована
...
cmd:everyprize(playerid)
{
    ...
    for(new i = 0, prize[28], list[84]; i < 7; i++)
    {
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == 3882)
    {
        if(response)
        {
            ...
            // Проверка на последовательность получения наград
            if(day > 0 && player_wp[playerid][day - 1] != STATUS_WP_TOOK_IT && player_wp[playerid][day] != STATUS_WP_TOOK_IT)
            {
            ...
            {
                case STATUS_WP_WAITING: 
                {
                    SendClientMessage(playerid, -1, "{E6E32C}[Еженедельные награды] {FFFFFF}Чтобы получить эту награду, отыграйте 2 часа.");
                }
                case STATUS_WP_MAYBE:
                {
                    switch(weekly_prizes[day][WP_Type])
                    {
                        case WP_RUB: 
                        {
                            GivePlayerDonateRub(playerid, weekly_prizes[day][WP_Index]);
                        }
                        case WP_MONEY: 
                        {
                            GivePlayerMoneyEx(playerid, weekly_prizes[day][WP_Index]);
                        }
                        case WP_VIP: 
                        {
                    ...
                    player_wp[playerid][day] = STATUS_WP_TOOK_IT; // Переводим в статус "Получен"
                    UnLoadWeeklyPrizes(playerid);
                }
                case STATUS_WP_TOOK_IT: 
                {
                    SendClientMessage(playerid, -1, "{E6E32C}[Еженедельные награды] {FFFFFF}Вы уже получили данный приз!");
                }
            }
        }
        return 1;
