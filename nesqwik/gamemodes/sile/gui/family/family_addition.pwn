...
public OnGameModeInit()
{
    ...
    // CRASH FIX: MAX_FAM (5000) is just the array-size cap, not the real number of
    // families. Looping TextDrawExpFamily() up to MAX_FAM tried to create 25000 global
    // TextDraws, far past SA-MP's hard 2048 TextDraw limit - so fam_PTD ended up holding
    // invalid/uncreated TextDraw IDs for most families, and showing those to a player
    // (ShowFamilyCaptPanel, called the instant someone enters a capture zone) is what
    // crashed the client. Only create TextDraws for families that can actually be
    // loaded (see MAX_ACTIVE_FAMILIES / LoadFamily's rows>300 clamp in family.pwn).
    for(new i;i < MAX_ACTIVE_FAMILIES;i++) TextDrawExpFamily(i);
...
stock GetPlayerCaptArea(playerid)
{
    ...
    {
        if(CAPTURE_AREA[i] == -1) continue; // Р·РѕРЅР° РЅРµ Р°РєС‚РёРІР° - РЅРµ РїСЂРѕРІРµСЂСЏРµРј, С‡С‚РѕР±С‹ РЅРµ РєСЂР°С€РЅСѓС‚СЊ СЃС‚СЂРёРјРµСЂ
...
{
    if(t < 0 || t >= sizeof(CAPTURE_AREA)) return 0; // Р·Р°С‰РёС‚Р° РѕС‚ РІС‹С…РѕРґР° Р·Р° РіСЂР°РЅРёС†С‹ РјР°СЃСЃРёРІР° (t=-1) вЂ” РєСЂР°С€ РєР»РёРµРЅС‚Р°
    show_panel_capt[playerid] = true;

    for(new i; i < 9;i++) TextDrawShowForPlayer(playerid, fam_TD[t][i]);

    // BUGFIX: GetPlayerIdFamily() returns -1 for players with no family, and that was used
    // directly as an array index into fam_PTD without any check - a negative index passed to a
    // native (TextDrawShowForPlayer) reading garbage/out-of-bounds memory, crashing the client.
    new f = GetPlayerIdFamily(playerid);
    if(f >= 0 && f < MAX_FAM) TextDrawShowForPlayer(playerid, fam_PTD[f][t]);
...
stock HideFamilyCaptPanel(playerid, t)
{
    ...
    // BUGFIX: same negative-index issue as ShowFamilyCaptPanel above - this is the one actually
    // hit in practice, since CaptureUpd's timer calls HideFamilyCaptPanel specifically for
    // players who have NO family (f == -1), crashing every tick while a capture is active.
    new f = GetPlayerIdFamily(playerid);
    if(f >= 0 && f < MAX_FAM) TextDrawHideForPlayer(playerid, fam_PTD[f][t]);
    return 1;
}
