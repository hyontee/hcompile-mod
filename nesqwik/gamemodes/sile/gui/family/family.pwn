...
#define MAX_FAM  5000
// MAX_FAM is only an array-size cap. The real number of families that can ever be
// loaded at once is capped at 300 by LoadFamily() below ("if(rows > 300) rows = 300").
// Anything that creates real per-family resources (TextDraws, etc.) at startup must
// loop up to MAX_ACTIVE_FAMILIES, NOT MAX_FAM - see family_addition.pwn.
#define MAX_ACTIVE_FAMILIES 300
...
public: welsi_LoadFamily()
{
    ...
    if(rows > MAX_ACTIVE_FAMILIES)
    {
        rows = MAX_ACTIVE_FAMILIES;
        ...
        if(s_value == 1) // Переименование ранга
{
    ...
    utf8_to_cp1251(newRankName); // FIX: konvertirovat nujno POSLE polucheniya stroki iz JSON, a ne do
...
else // Получение списка рангов
{
    ...
    // Названия рангов (FIX: family_rang_name hranitsya v cp1251, klientu nujen UTF-8)
    new rn0_buf[128], rn1_buf[128], rn2_buf[128], rn3_buf[128], rn4_buf[128];
    format(rn0_buf, sizeof rn0_buf, "%s", family_rang_name[f][0]); cp1251_to_utf8(rn0_buf);
    format(rn1_buf, sizeof rn1_buf, "%s", family_rang_name[f][1]); cp1251_to_utf8(rn1_buf);
    format(rn2_buf, sizeof rn2_buf, "%s", family_rang_name[f][2]); cp1251_to_utf8(rn2_buf);
    format(rn3_buf, sizeof rn3_buf, "%s", family_rang_name[f][3]); cp1251_to_utf8(rn3_buf);
    format(rn4_buf, sizeof rn4_buf, "%s", family_rang_name[f][4]); cp1251_to_utf8(rn4_buf);
    ...
    else if(id_value == 2)
    {
        ...
        if(s_value == 1)
        {
            ...
            cp1251_to_utf8(rank_name); // FIX: klientu (JSON "rn") nujen UTF-8, a rank_name byl v cp1251
