...
// Подключать после gamemodes/sile/gui/cases/cases.pwn и до gamemodes/sile/ipacket.inc.
...
{
    // BUGFIX: BP (BlackPass) rewards are stored with case_id 90/91 (BP_REWARD_CASE_STANDARD /
    // BP_REWARD_CASE_PREMIUM). Those ids are virtual and never exist in the real case table
    // (MAX_CASES cases only go up to id 29), so Cases_GetIndex(90/91) always returned -1 and every
    // BP reward silently failed to resolve here - meaning it was stored in `rewards` but could never
    // be shown or claimed from /reward. Handle them directly from BP_GetRewardData instead.
    if(caseId == BP_REWARD_CASE_STANDARD || caseId == BP_REWARD_CASE_PREMIUM)
    {
        new bpType, bpInternal, bpCount, bpSpray, bpName[32];
        new row = (caseId == BP_REWARD_CASE_PREMIUM) ? BP_REWARD_ROW_PREMIUM : BP_REWARD_ROW_STANDARD;

        if(!BP_GetRewardData(row, awardId, bpType, bpInternal, bpCount, bpSpray, bpName, sizeof(bpName)))
            return 0;

        rewardRarity = 0;
        sprayPrice = bpSpray;
        rewardValue = bpInternal;
        rewardCount = bpCount;

        // BP_GetRewardData uses its own small type numbering that only partially overlaps with
        // REWARD_TYPE_* (e.g. BP's "10" means BC, but REWARD_TYPE_BC is 3 - REWARD_TYPE_BP_EXP is 10).
        // Translate it here so Reward_GiveToPlayer grants the correct thing instead of silently
        // hitting its `default: return 0` case.
        switch(bpType)
        {
            case 10: rewardType = REWARD_TYPE_BC;      // BC currency
            case 12: rewardType = REWARD_TYPE_CASE;    // grants a real case (internal = real case id 1/2/3)
            case 15: rewardType = REWARD_TYPE_DUST;    // "dust"
            default: rewardType = bpType;              // 1=EXP, 2=MONEY, 9=VIP, 11=ITEM, 134=skin(ITEM) already match
        }

        format(name, name_size, "%s", bpName);
        return 1;
    }
...
stock Reward_GiveToPlayer(playerid, rewardType, rewardValue, rewardCount, rewardRarity, isBonus)
{
    ...
    switch(rewardType)
    {
        case REWARD_TYPE_EXP:
        {
            ...
            ShowNotificationSile(playerid, 1, 5, 1, 0, msg, "");
        }
        ...
        case REWARD_TYPE_VIP:
        {
            ...
            ShowNotificationSile(playerid, 1, 5, 1, 0, "VIP награда активирована", "");
        }
        ...
        case REWARD_TYPE_ITEM:
        {
            if(!Reward_AddInventoryItem(playerid, rewardValue, rewardCount))
                return ShowNotificationSile(playerid, 2, 5, 1, 1, "Нет свободного места в инвентаре", ""), 0;
...
CMD:reward(playerid, params[])
{
    ...
    // BUGFIX: mode 0's query explicitly excludes case_id 90/91 (BP rewards) at the SQL level,
    // so BlackPass items never even reached Reward_FindAwardData from the normal /reward command -
    // only /rewardfull (mode 2) included them, and nothing told players that command existed.
    // Show everything by default instead.
...
public OnPlayerRewardsLoad(playerid)
{
    ...
    if(rows == 0)
    {
        if(GetPVarInt(playerid, "reward_offset") == 0)
            ShowNotificationSile(playerid, 2, 4, 1, 1, "У вас нет доступных наград", "");
...
public OnRewardDebug(playerid, dbId, rewardAction)
{
    ...
    if(rewardAction == 3)
    {
        ...
        {
            ShowNotificationSile(playerid, 2, 4, 1, 1, "Эту награду нельзя распылить", "");
            deleteReward = false;
        }
    }
    else
