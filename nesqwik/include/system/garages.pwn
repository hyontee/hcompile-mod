...
public LoadGarages()
{
    ...
    if(rows > 0)
    {
        ...
        g_GarageCount = (rows < MAX_GARAGES) ? (rows) : (MAX_GARAGES);
...
{
    // BUGFIX: this used to run a separate "SELECT id ORDER BY id DESC LIMIT 1" query to guess
    // which row was just inserted. If /addgarage was used more than once before earlier callbacks
    // finished (or under any concurrent insert), that SELECT could race and return the wrong id -
    // multiple callbacks could grab the same "latest" id, clobbering the same g_GarageData slot /
    // pickups while g_GarageCount still incremented once per callback. cache_insert_id() reads the
    // id generated by THIS callback's own INSERT, with no race.
    new garageid = cache_insert_id();
