...
#if defined _SILE_HUNTER_JOB_INCLUDED
    #endinput
#endif
#define _SILE_HUNTER_JOB_INCLUDED
...
new STREAMER_TAG_ACTOR:HunterAnimalActor[MAX_PLAYERS][HUNTER_ANIMAL_COUNT];
...
stock Hunter_RemoveAnimalObject(playerid, animalid)
{
    ...
    if(HunterAnimalActor[playerid][animalid] != STREAMER_TAG_ACTOR:INVALID_STREAMER_ID)
    {
        if(IsValidDynamicActor(HunterAnimalActor[playerid][animalid]))
            DestroyDynamicActor(HunterAnimalActor[playerid][animalid]);

        HunterAnimalActor[playerid][animalid] = STREAMER_TAG_ACTOR:INVALID_STREAMER_ID;
    }
    return 1;
}
...
stock Hunter_CreateAnimalObject(playerid, animalid)
{
    ...
    HunterAnimalActor[playerid][animalid] = CreateDynamicActor(
        HunterAnimalModel[animalid],
        HunterAnimalPos[animalid][0],
        HunterAnimalPos[animalid][1],
        HunterAnimalPos[animalid][2],
    ...
    if(IsValidDynamicActor(HunterAnimalActor[playerid][animalid]))
    {
        ApplyDynamicActorAnimation(HunterAnimalActor[playerid][animalid], "PED", "IDLE_STANCE", 4.1, 1, 0, 0, 1, 0);
    }
    return 1;
}
...
stock Hunter_GetAnimalByActor(playerid, STREAMER_TAG_ACTOR:actorid)
{
    for(new i = 0; i < HUNTER_ANIMAL_COUNT; i++)
    {
        if(HunterAnimalActor[playerid][i] == actorid)
            return i;
    }
    return -1;
}
...
{
    ShowNotificationSile(playerid, 5, 5, HUNTER_NOTIFY_ACTION, subid, text, "E");
    return 1;
}
...
{FF4500}#7 {FFFFFF}Получить АК-47 и метку на животных {7D7D7D}работает после покупки тарифа"
    ...
    ShowPlayerDialog(playerid, HUNTER_DIALOG_MAIN, DIALOG_STYLE_LIST, "{FF0000}SLAY RUSSIA | {FFFFFF}Охотник", text, "Выбрать", "Закрыть");
    return 1;
}
...
{FFFFFF}Ваш скилл охотника: {FFFF00}%d",
    ...
    ShowPlayerDialog(playerid, HUNTER_DIALOG_STATS, DIALOG_STYLE_MSGBOX, "{FF0000}SLAY RUSSIA | {FFFFFF}Статистика охотника", text, "Назад", "Закрыть");
    return 1;
}
...
stock Hunter_ShowTariffs(playerid)
{
    ShowPlayerDialog(playerid, HUNTER_DIALOG_TARIFFS, DIALOG_STYLE_TABLIST_HEADERS,
        "{FF0000}SLAY RUSSIA | {FFFFFF}Тарифы охоты",
        "#\tТариф\tЦена\tОписание\n\
...
stock Hunter_ShowBaits(playerid)
{
    ShowPlayerDialog(playerid, HUNTER_DIALOG_BAITS, DIALOG_STYLE_TABLIST_HEADERS,
        "{FF0000}SLAY RUSSIA | {FFFFFF}Приманки",
...
stock Hunter_ShowPrices(playerid)
{
    ShowPlayerDialog(playerid, HUNTER_DIALOG_PRICES, DIALOG_STYLE_TABLIST_HEADERS,
        "{FF0000}SLAY RUSSIA | {FFFFFF}Стоимость добычи",
        "Животное\tНаграда\n\
...
stock Hunter_ShowTrophies(playerid)
{
    ...
    ShowPlayerDialog(playerid, HUNTER_DIALOG_TROPHIES, DIALOG_STYLE_MSGBOX, "{FF0000}SLAY RUSSIA | {FFFFFF}Трофеи", text, "Назад", "Закрыть");
    return 1;
}
...
public OnPlayerGiveDamageDynamicActor(playerid, STREAMER_TAG_ACTOR:actorid, Float:amount, weaponid, bodypart)
{
    ...
    new animalid = Hunter_GetAnimalByActor(playerid, actorid);
    if(animalid != -1)
    {
        if(!HunterJobActive[playerid])
            return SendClientMessage(playerid, COLOR_WHITE, "{FF0000}| {FFFFFF}Сначала начните работу охотника у Николыча.");
    ...
    #if defined Hunter_OnPlayerGiveDamageDynamicActor
        return Hunter_OnPlayerGiveDamageDynamicActor(playerid, actorid, amount, weaponid, bodypart);
    #else
        return 1;
    #endif
}
#if defined _ALS_OnPlayerGiveDamageDynamicActor
    #undef OnPlayerGiveDamageDynamicActor
#else
    #define _ALS_OnPlayerGiveDamageDynamicActor
#endif
#define OnPlayerGiveDamageDynamicActor Hunter_OnPlayerGiveDamageDynamicActor
#if defined Hunter_OnPlayerGiveDamageDynamicActor
    forward Hunter_OnPlayerGiveDamageDynamicActor(playerid, STREAMER_TAG_ACTOR:actorid, Float:amount, weaponid, bodypart);
#endif
...
public OnPlayerConnect(playerid)
{
    ...
    for(new i = 0; i < HUNTER_ANIMAL_COUNT; i++)
    {
        ...
        HunterAnimalActor[playerid][i] = STREAMER_TAG_ACTOR:INVALID_STREAMER_ID;
    }
    Hunter_SpawnPlayerAnimals(playerid);
