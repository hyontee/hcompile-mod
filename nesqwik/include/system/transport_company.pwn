...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == 2903)
    {
        if(!response)
        {
            ...
            {
                // FIX: sbrasyvaem "zavisshiy" zakaz, inache p_TC_Order ostanetsya != -1
                // i ShowDialogOrders navsegda budet blokirovat igroka soobscheniem "Snachala vypolnite proshliy zakaz"
                player_tc[playerid][p_TC_Order] = -1;
            ...
            // FIX: klientskiy GPS ranshe voobsche ne stavilsya dlya zakazov dalnoboyschika -
            // stavim metku GPS na tochku zagruzki (rabotaet i dlya priczepa, i bez nego)
            NavGUI_SetPoint(playerid, "Точка загрузки", 51, x, y, z);
...
stock DefaultPlayerOrder(playerid)
{
    ...
    DisablePlayerGPS(playerid); // FIX: СЃРЅРёРјР°РµРј РІРѕР·РјРѕР¶РЅСѓСЋ GPS-РјРµС‚РєСѓ Р·Р°РєР°Р·Р° РїСЂРё СЃР±СЂРѕСЃРµ СЃРѕСЃС‚РѕСЏРЅРёСЏ (РѕС‚РјРµРЅР°/СЃРјРµСЂС‚СЊ/РЅРѕРІС‹Р№ Р·Р°РєР°Р·)
...
public:LoadTrailerTC(playerid)
{
    ...
    switch(player_tc[playerid][p_TC_Progress_Order])
    {
        case 2:
        {
            ...
            // FIX: ranshe GPS-metka na tochku razgruzki voobsche ne stavilas,
            // hotya soobschenie nizhe obeschalo "otmecheno na karte"
            NavGUI_SetPoint(playerid, "Точка разгрузки", 51, x, y, z);
        ...
        case 3:
        {
            ...
            if(IsValidDynamic3DTextLabel(player_tc[playerid][p_TC_Text])) DestroyDynamic3DTextLabel(player_tc[playerid][p_TC_Text]);
            if(IsValidDynamicArea(player_tc[playerid][p_TC_Sphere_Finish])) DestroyDynamicArea(player_tc[playerid][p_TC_Sphere_Finish]);
            DisablePlayerGPS(playerid); // FIX: СЃРЅРёРјР°РµРј GPS-РјРµС‚РєСѓ РїРѕСЃР»Рµ Р·Р°РІРµСЂС€РµРЅРёСЏ Р·Р°РєР°Р·Р°
        }
    }
