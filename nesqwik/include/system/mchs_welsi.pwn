// MCHS Welsi system ported under laird.pwn structure

#define		MAX_FIRE	81
#define     MCHS_FIRE_PROGRESS_MAX 15
...
#define SC              "{FFD700}Информация: {ffffff}"
#define USC             "{FAD201}Подсказка: {ffffff}"
...
new mchs_pickup_exit, mchs_pickup_enter, mchs_pickup_npc, mchs_pickup_inv, mchs_pickup_veh;
new mchs_dynamic_pickup_exit, mchs_dynamic_pickup_enter, mchs_dynamic_pickup_npc, mchs_dynamic_pickup_inv, mchs_dynamic_pickup_veh;
...
{
	{0, 0.0, 0.0, 0.0, 0, MCHS_FIRE_PROGRESS_MAX},
...
{
    mysql_tquery(mysql, "CREATE TABLE IF NOT EXISTS `order_mchs` (`id` INT NOT NULL, `id_player` INT NOT NULL DEFAULT 0, `state` INT NOT NULL DEFAULT 0, PRIMARY KEY (`id`)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4", "", "");

    // print("[W_SYSTEM] Система огня и работа МЧС загружена\nАвтор: Welsi Studio");
    exit_mchs = CreateDynamicSphere(-2600.141113,-283.324401,1246.680419, 1.5);
    enter_mchs = CreateDynamicSphere(-2558.123535,-284.654937,27.354429, 2.0);

    // Пикапы МЧС: создаём и dynamic, и обычные, чтобы было видно на любой сборке.
    mchs_dynamic_pickup_enter = CreateDynamicPickup(1239, 1, -2558.123535,-284.654937,27.354429, -1, -1, -1, 80.0);
    mchs_dynamic_pickup_exit  = CreateDynamicPickup(1239, 1, -2600.141113,-283.324401,1246.680419, -1, -1, -1, 80.0);
    mchs_dynamic_pickup_npc   = CreateDynamicPickup(1239, 1, -2615.917724,-282.265045,1252.043823, -1, -1, -1, 80.0);
    mchs_dynamic_pickup_inv   = CreateDynamicPickup(1239, 1, -2608.042480,-282.297698,1252.043823, -1, -1, -1, 80.0);
    mchs_dynamic_pickup_veh   = CreateDynamicPickup(1239, 1, -2555.758544,-277.883331,27.260526, -1, -1, -1, 80.0);

    mchs_pickup_enter = CreatePickup(1239, 23, -2558.123535,-284.654937,27.354429, -1);
    mchs_pickup_exit  = CreatePickup(1239, 23, -2600.141113,-283.324401,1246.680419, -1);
    mchs_pickup_npc   = CreatePickup(1239, 23, -2615.917724,-282.265045,1252.043823, -1);
    mchs_pickup_inv   = CreatePickup(1239, 23, -2608.042480,-282.297698,1252.043823, -1);
    mchs_pickup_veh   = CreatePickup(1239, 23, -2555.758544,-277.883331,27.260526, -1);

    CreateActorEx("{FAD201}Работа МЧС", "{FFFFFF}Подойдите для взаимодействия", 267, -2615.917724,-282.265045,1252.043823,265.929382, 1, 1);
    ...
    CreateDynamic3DTextLabel("Взять экипировку\n'Огнетушитель'", 0xFFFF00FF, -2608.042480,-282.297698,1252.043823, 15.0);
    CreateDynamic3DTextLabel("РАБОЧИЙ ТРАНСПОРТ МЧС", 0xFFFF00FF, -2555.758544,-277.883331,27.260526, 15.0);
    CreateDynamic3DTextLabel("Вход в МЧС", 0xFAD201FF, -2558.123535,-284.654937,27.354429, 5.0);
    CreateDynamic3DTextLabel("Выход из МЧС", 0xFAD201FF, -2600.141113,-283.324401,1246.680419, 5.0);
...
stock Mchs_OpenNearPoint(playerid)
{
    if(IsPlayerInRangeOfPoint(playerid, 3.0, -2558.123535,-284.654937,27.354429))
    {
        SetPlayerPosEx(playerid, -2600.153564,-281.323333,1246.680419,5.701898, 1, 1);
        return 1;
    }
    if(IsPlayerInRangeOfPoint(playerid, 3.0, -2600.141113,-283.324401,1246.680419))
    {
        SetPlayerPosEx(playerid, -2556.406005,-284.757751,27.354429,275.161071, 0, 0);
        return 1;
    }
    if(IsPlayerInRangeOfPoint(playerid, 3.5, -2615.917724,-282.265045,1252.043823))
    {
        Dialog(playerid, 3501, DSL, "{FFD700}\xcc\xe8\xed\xe8\xf1\xf2\xe5\xf0\xf1\xf2\xe2\xee \xd7\xe5\xf0\xe5\xe7\xe2\xfb\xf7\xe0\xe9\xed\xfb\xf5 \xd1\xe8\xf2\xf3\xe0\xf6\xe8\xe9", "{00CC00}\xcd\xe0\xf7\xe0\xf2\xfc{FFFFFF}/{FAD201}\xe7\xe0\xea\xee\xed\xf7\xe8\xf2\xfc \xf1\xec\xe5\xed\xf3\n\xc8\xed\xf4\xee\xf0\xec\xe0\xf6\xe8\xff \xef\xee \xf0\xe0\xe1\xee\xf2\xe5 \xcc\xd7\xd1", "\xcf\xf0\xee\xe4\xee\xeb\xe6\xe8\xf2\xfc", "\xce\xf2\xec\xe5\xed\xe0");
        return 1;
    }
    if(IsPlayerInRangeOfPoint(playerid, 3.5, -2608.042480,-282.297698,1252.043823))
    {
        if(!player_mchs_active{playerid}) return ShowNotificationSile(playerid, 2, 6, 0, 0, "\xc4\xeb\xff \xed\xe0\xf7\xe0\xeb\xe0 \xed\xe0\xf7\xed\xe8\xf2\xe5 \xf1\xec\xe5\xed\xf3.", ""), 1;
        Dialog(playerid, 3502, DSM, "\xc8\xed\xe2\xe5\xed\xf2\xe0\xf0\xfc \xcc\xd7\xd1", "\xc2\xfb \xf5\xee\xf2\xe8\xf2\xe5 \xe2\xe7\xff\xf2\xfc \xee\xe3\xed\xe5\xf2\xf3\xf8\xe8\xf2\xe5\xeb\xfc?", "\xc2\xe7\xff\xf2\xfc", "\xce\xf2\xec\xe5\xed\xe0");
        return 1;
    }
    if(IsPlayerInRangeOfPoint(playerid, 3.5, -2555.758544,-277.883331,27.260526))
    {
        if(!player_mchs_active{playerid}) return ShowNotificationSile(playerid, 2, 6, 0, 0, "\xd1\xed\xe0\xf7\xe0\xeb\xe0 \xed\xe0\xf7\xed\xe8\xf2\xe5 \xf1\xec\xe5\xed\xf3 \xe2 \xcc\xd7\xd1.", ""), 1;
        Dialog(playerid, 3503, DSM, "{FFD700}\xd0\xe0\xe1\xee\xf7\xe8\xe9 \xf2\xf0\xe0\xed\xf1\xef\xee\xf0\xf2 \xcc\xd7\xd1", "\xc2\xfb \xf5\xee\xf2\xe8\xf2\xe5 \xe2\xe7\xff\xf2\xfc \xf0\xe0\xe1\xee\xf7\xe8\xe9 \xf2\xf0\xe0\xed\xf1\xef\xee\xf0\xf2?", "\xc2\xe7\xff\xf2\xfc", "\xce\xf2\xec\xe5\xed\xe0");
        return 1;
    }
    ShowNotificationSile(playerid, 2, 5, -1, -1, "\xd0\xff\xe4\xee\xec \xed\xe5\xf2 \xf2\xee\xf7\xea\xe8 \xcc\xd7\xd1.", "");
    return 1;
}

CMD:mchsuse(playerid, params[])
{
    #pragma unused params
    return Mchs_OpenNearPoint(playerid);
}
...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    ...
    if(areaid == npc_mchs_1)
    {
        ...
        (
            playerid, 3501, DSL, 
            "{FFD700}Министерство Черезвычайных Ситуаций",
            "{00CC00}Начать{FFFFFF}/{FAD201}закончить смену\n"\
            "Информация по работе МЧС", 
            "Продолжить", "Отмена"
        );
    }
    if(areaid == invent_mchs)
    {
        if(!player_mchs_active{playerid}) return ShowNotificationSile(playerid, 2, 6, 0, 0, "Для начала начните смену.", "");
        Dialog(playerid, 3502, DSM, "Инвентарь МЧС", "Вы хотите взять огнетушитель?", "Взять", "Отмена");
    }
    if(areaid == veh_mchs)
    {
        if(!player_mchs_active{playerid}) return ShowNotificationSile(playerid, 2, 6, 0, 0, "Сначала начните смену в МЧС.", "");
        Dialog(playerid, 3503, DSM, "{FFD700}Рабочий транспорт МЧС", "Вы хотите взять рабочий транспорт?", "Взять", "Отмена");
    }
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == 35500)
    {
        if(response)
        {
            ...
            if(!(0 <= id < sizeof(order_mchs))) return 1;
            if(order_mchs[id][MS_State] != STATE_ORDER_NO_TAKE)
            {
                ShowNotificationSile(playerid, 2, 6, 0, 0, "Этот вызов уже занят.", "");
                return Mchs_ShowOrders(playerid);
            }
            if(player_order[playerid])
            {
                return ShowNotificationSile(playerid, 2, 6, 0, 0, "Вы уже взяли вызов МЧС.", "");
            }

            EnablePlayerGPS(playerid, 55, coord_order[id][0], coord_order[id][1], coord_order[id][2], "Место вызова МЧС отмечено на GPS");
            ...
            mysql_format(mysql, String128m, sizeof String128m,
                "INSERT INTO `order_mchs` (`id`, `id_player`, `state`) VALUES (%d, %d, %d) ON DUPLICATE KEY UPDATE `id_player` = %d, `state` = %d",
                id,
            ...
            mysql_tquery(mysql, String128m);

            LoadFireOrder(id + 1);
            new mchs_progress_text[48];
            format(mchs_progress_text, sizeof mchs_progress_text, "~y~Прогресс: ~w~0/%d", MCHS_FIRE_PROGRESS_MAX);
            GameTextForPlayer(playerid, mchs_progress_text, 3000, 5);
            ShowNotificationSile(playerid, 1, 6, 0, 0, "Вы приняли вызов МЧС. Езжайте на точку.", "");
        }
        return 1;
    }
    if(dialogid == 3501)
        ...
        if(response)
        {
            switch(listitem)
            {
                case 0:
                {
                    ...
                    {
                        SetPlayerSkin(playerid, 19276);
                        player_mchs_active{playerid} = true;
                        ShowNotificationSile(playerid, 1, 6, 0, 0, "Вы начали смену. Возьмите огнетушитель и Т/С.", "");
                    }
                }
                case 1:
                {
    ...
    if(dialogid == 3502)
    {
        if(response)
        {
            ...
            ShowNotificationSile(playerid, 2, 6, 0, 0, "Вы взяли огнетушитель.", "");
        }
    }
    if(dialogid == 3503)
    {
        if(response)
        {
            ...
            veh_id = CreateVehicle(407, -2554.483398, -301.090240, 28.850000, 270.0, 3, 3, -1);
            ...
            SetTimerEx("MchsPutPlayerVehicle", 350, false, "ii", playerid, veh_id);

            ShowNotificationSile(playerid, 1, 6, 0, 0, "Вы успешно взяли рабочее Т/С.", "");
            ShowNotificationSile(playerid, 2, 6, 0, 0, "Введите /meslist для просмотра доступных вызовов.", "");
        }
    }
...
{
    if(PRESSED(KEY_WALK) || PRESSED(KEY_YES) || PRESSED(KEY_SECONDARY_ATTACK))
    {
        if(IsPlayerInRangeOfPoint(playerid, 3.5, -2558.123535,-284.654937,27.354429)
        || IsPlayerInRangeOfPoint(playerid, 3.5, -2600.141113,-283.324401,1246.680419)
        || IsPlayerInRangeOfPoint(playerid, 3.5, -2615.917724,-282.265045,1252.043823)
        || IsPlayerInRangeOfPoint(playerid, 3.5, -2608.042480,-282.297698,1252.043823)
        || IsPlayerInRangeOfPoint(playerid, 3.5, -2555.758544,-277.883331,27.260526))
        {
            Mchs_OpenNearPoint(playerid);
            return 1;
        }
    }
    if(PRESSED(KEY_FIRE))
	{
        if(press_fire{playerid}) return ShowNotificationSile(playerid, 2, 6, 0, 0, "Продолжайте тушить очаг возгорания.", "");
		...
		if(GetPlayerWeapon(playerid) == 42)
		{
			if(IsPlayerInRangeOfAnyFire(playerid) >= 1)
			{
                ...
                if(all_fire[IsPlayerInRangeOfAnyFire(playerid)][F_ID_Z] == player_order[playerid])
                {
                    ...
                    if(all_fire[id_fire][F_SCORE] < MCHS_FIRE_PROGRESS_MAX)
                    {
                        all_fire[id_fire][F_SCORE] ++;
                    }

                    format(text, sizeof text, "~y~Прогресс: ~w~%d/%d", all_fire[id_fire][F_SCORE], MCHS_FIRE_PROGRESS_MAX);
                    ...
                    { 
                       ...
                       ShowNotificationSile(playerid, 1, 6, 0, 0, "Вы потушили очаг возгорания.", "");

                        if(Order(id))
                        {
                            ShowNotificationSile(playerid, 1, 6, 0, 0, "Вы успешно выполнили заказ.", "");
                            order_mchs[id - 1][MS_State] = STATE_ORDER_COMPLETED;
                            GivePlayerMoneyEx(playerid, 1650000 + random(230000)); //ВМЕСТО 10000 ПОСТАВЬ ЦИФРЫ ЗП
                            endOrders(id);
                            player_order[playerid] = 0;
                ...
                else ShowNotificationSile(playerid, 2, 6, 0, 0, "Данный очаг возгорания не вашего заказа.", "");
			}
			else ShowNotificationSile(playerid, 2, 6, 0, 0, "Не советуем использовать огнетушитель просто так.", "");
		}
	}
...
{
    if(1 <= id <= sizeof(order_mchs)) order_mchs[id - 1][MS_State] = STATE_ORDER_TAKE;
...
CMD:mchsinv(playerid)
{
    if(!player_mchs_active{playerid}) return ShowNotificationSile(playerid, 2, 6, 0, 0, "Сначала начните смену в МЧС.", "");
    return Dialog(playerid, 3502, DSM, "Инвентарь МЧС", "Вы хотите взять огнетушитель?", "Взять", "Отмена");
}

CMD:mchscar(playerid)
{
    if(!player_mchs_active{playerid}) return ShowNotificationSile(playerid, 2, 6, 0, 0, "Сначала начните смену в МЧС.", "");
    return Dialog(playerid, 3503, DSM, "{FFD700}Рабочий транспорт МЧС", "Вы хотите взять рабочий транспорт?", "Взять", "Отмена");
}


stock Mchs_EnsureOrders()
{
    new active;
    for(new i; i < sizeof(order_mchs); i++)
    {
        if(order_mchs[i][MS_State] == STATE_ORDER_NO_TAKE || order_mchs[i][MS_State] == STATE_ORDER_TAKE)
        {
            active++;
        }
    }
    ...
    for(new i; i < sizeof(order_mchs); i++)
    {
        order_mchs[i][MS_IDp] = INVALID_PLAYER_ID;
        order_mchs[i][MS_State] = STATE_ORDER_NO_TAKE;

        mysql_format(mysql, String128m, sizeof String128m,
            "INSERT INTO `order_mchs` (`id`, `id_player`, `state`) VALUES (%d, 0, %d) ON DUPLICATE KEY UPDATE `id_player` = 0, `state` = %d",
            i,
            STATE_ORDER_NO_TAKE,
            STATE_ORDER_NO_TAKE
        );
...
stock Mchs_ShowOrders(playerid)
{
    if(!player_mchs_active{playerid})
    {
        return ShowNotificationSile(playerid, 2, 6, 0, 0, "Вы не работаете в МЧС.", "");
    }
    if(player_order[playerid])
    {
        return ShowNotificationSile(playerid, 2, 6, 0, 0, "Вы уже взяли вызов МЧС.", "");
    ...
    strcat(list, "{FFFFFF}№\tРасстояние\n");

    for(new id; id < sizeof(order_mchs); id++)
    {
        ...
        format(mchs_line, sizeof(mchs_line), "{FFFFFF}Вызов №%d\t{B3B3B3}%.2f м\n", id + 1, dista);
        strcat(list, mchs_line);
    ...
    if(!count)
    {
        return ShowNotificationSile(playerid, 2, 6, 0, 0, "Сейчас нет свободных вызовов МЧС.", "");
    }

    Dialog(playerid, 35500, DIALOG_STYLE_TABLIST_HEADERS, "Доступные вызовы МЧС", list, "Взять", "Закрыть");
    return 1;
}
...
CMD:mchslist(playerid)
{
    return Mchs_ShowOrders(playerid);
}

CMD:morders(playerid)
{
    return Mchs_ShowOrders(playerid);
}

public:MchsPutPlayerVehicle(playerid, veh_id)
{
    if(IsPlayerConnected(playerid) && IsValidVehicle(veh_id))
    {
        SetVehicleVirtualWorld(veh_id, GetPlayerVirtualWorld(playerid));
        LinkVehicleToInterior(veh_id, GetPlayerInterior(playerid));
        SetVehicleVelocity(veh_id, 0.0, 0.0, 0.0);
