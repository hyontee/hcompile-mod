...
stock FamilyGuiSendFamilySettings(playerid)
{
    ...
    for(new i; i < 5; i++)
    {
        new rn_list_utf8_buf[128];
        format(rn_list_utf8_buf, sizeof rn_list_utf8_buf, "%s", family_rang_name[family_id][i]);
        cp1251_to_utf8(rn_list_utf8_buf); // FIX: konvertiruem v UTF-8 dlya klienta
        rank_name_list = JSON_Append(rank_name_list, JSON_Array(JSON_String(rn_list_utf8_buf)));
...
stock FamilyGuiSendPlayerInfo(playerid, const selected_name[])
{
    ...
    if(family_id != -1 && rank >= 1 && rank <= 5)
        format(rn_utf8_buf, sizeof rn_utf8_buf, "%s", family_rang_name[family_id][rank - 1]);
    else
        format(rn_utf8_buf, sizeof rn_utf8_buf, "Участник");
    cp1251_to_utf8(rn_utf8_buf); // FIX: klientu nujen UTF-8, a family_rang_name hranitsya v cp1251
    JSON_SetString(response, "rn", rn_utf8_buf);
