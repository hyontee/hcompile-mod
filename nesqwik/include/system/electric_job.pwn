...
stock Electric_EnterOffice(playerid)
{
    ...
    ShowNotificationSile(playerid, 3, 4, 1, 1, "Вы вошли в помещение электриков.", " ");
    return 1;
}
...
stock Electric_ExitOffice(playerid)
{
    ...
    SetCameraBehindPlayer(playerid);
    ShowNotificationSile(playerid, 3, 4, 1, 1, "Вы вышли из помещения электриков.", " ");
    return 1;
}
...
stock Electric_ShowUseHint(playerid)
{
    ...
    ShowNotificationSile(playerid, 3, 4, 1, 1, "Нажмите кнопку взаимодействия или используйте /euse.", " ");
    return 1;
}
...
stock Electric_FinishOrder(playerid)
{
    ...
    SendClientMessage(playerid, -1, text);
    ShowNotificationSile(playerid, 3, 5, 0, 0, "Заказ электрика выполнен. Деньги начислены.", " ");
...
stock Electric_FailOrder(playerid, const reason[])
{
    ...
    SendClientMessage(playerid, -1, text);
    ShowNotificationSile(playerid, 2, 5, 0, 0, "Заказ электрика провален.", " ");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == ELECTRIC_DIALOG_FAULT)
    {
        ...
        else
        {
            ...
            ShowNotificationSile(playerid, 2, 5, 0, 0, "Неправильный узел. Попробуйте ещё раз.", " ");
            Electric_ShowFaultDialog(playerid);
        }
        return 1;
    }
...
CMD:eorders(playerid)
{
    ...
    SendClientMessage(playerid, -1, text);
    ShowNotificationSile(playerid, 3, 5, 0, 0, "Новый заказ электрика принят. Езжайте на точку.", " ");
    if(p >= 0 && p <= 3)
    {
