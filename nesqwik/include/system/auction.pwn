...
stock AuctionSkinName(skinid, dest[], len = 24)
{
    format(dest, len, "Skin_%d", skinid);
    return 1;
}

stock AuctionCalcEndTime()
{
    ...
    new m = 59 - minutes;
    new s = 59 - seconds;
    new left = (h * 3600) + (m * 60) + s - 10;

    if(left <= 3600) left = 10800;
    return gettime() + left;
}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == 5992) // согласие
    {
        ...
        switch(GetPVarInt(playerid, "type_param"))
        {
            ...
            case TYPE_CLOTHES:
            {
                ...
                new server = cache_get_field_content_int(0, "game_id");
                new skin_name[24];
                AuctionSkinName(server, skin_name);
    ...
    if(dialogid == 5993)
    {
        if(response)
        {
            ...
            else if(player_info_panel[playerid][auc_filter_type] == 2)
            {
                ...
                if(sscanf(inputtext, "P<,>dd", min_price, max_price))
                {
                    ...
                    player_info_panel[playerid][auc_filter_type] = 0;
                ...
                else
                {
                    ...
                    {
                        player_info_panel[playerid][auc_filter_type] = 2;
            ...
            else if(player_info_panel[playerid][auc_filter_type] == 1)
            {
            ...
            {       
                switch(listitem)
                {
                    case 0:
                    {
                        ...
                        player_info_panel[playerid][auc_filter_type] = 1;
                        return 1;
                    }
                    case 1:
                    {   
...
{
    // Auto-renaming all businesses from auction source disabled for this mod.
    ...
    if(playertextid == auction_PTD[playerid][30])
    {
        ...
        switch(GetPVarInt(playerid, "type_param"))
        {
            ...
            case TYPE_CLOTHES:
            {
                ...
                new rows = cache_num_rows(), query[54], id, model_id, skin_name[24];
				...
				{
					id = cache_get_field_content_int(i, "id");
					model_id = cache_get_field_content_int(i, "game_id");

                    AuctionSkinName(model_id, skin_name);
...
CMD:auc_reload(playerid)
{
    if(GetPlayerAdminEx(playerid) < 5) return 1;
    LoadBusinessToAuction();
    SendClientMessage(playerid, -1, "Auction: gov businesses reloaded.");
    return 1;
}
...
CMD:auction(playerid)
{
   ...
   // Auction is available 24/7 in this integration.
...
stock create_auction_slot(playerid, type, opisanie[124], start_rate, data_base, server_id, number_vehicle[7] = "", number_SUM = 0)
{
    ...
    switch(type)
    {
        ...
        case TYPE_CLOTHES:
        {
            AuctionSkinName(server_id, auction_slot[id][SLOT_NAME], 24);
        }
    }
...
stock finish_auction_slot(slot)
{
    if(auction_slot[slot][OWNER_RATE] == -1)
    {
        ...
        {
            auction_slot[slot][TIMER_END] = AuctionCalcEndTime();
            return 1;
        }
    ...
    switch(auction_slot[slot][SLOT_TYPE])
    {
        case TYPE_BUSINESS:
        {
            ...
            // FIX: во всех других case ниже слот сбрасывается через reset_slot_auction, а здесь этого не было.
            // Из-за этого SLOT_TYPE оставался TYPE_BUSINESS, TIMER_END оставался в прошлом,
            // и AuctionUpdate() (таймер каждую секунду) снова и снова вызывал finish_auction_slot()
            // для этого же слота: продавцу каждый тик дублировались деньги за лот, а слот навсегда оставался занят.
            reset_slot_auction(slot);
        }
        case TYPE_HOUSE:
        {
        ...
        case TYPE_CLOTHES:
        {
            ...
            mysql_format(mysql, query, sizeof query, "INSERT INTO inventory_skins (owner_skin,game_id,use_skin) VALUES (%d,%d,0)", auction_slot[slot][OWNER_RATE], auction_slot[slot][SERVER_ID]);
            mysql_query(mysql, query, false);
...
public:LoadBusinessToAuction()
{
    ...
    mysql_format(mysql, query, sizeof query, "SELECT * FROM business WHERE `owner_id` = 0 ORDER BY id ASC");
    ...
    {
        new business_sql = cache_get_field_content_int(row, "id");
        new price = cache_get_field_content_int(row, "price");
        new slot = -1;

        for(new s; s < MAX_AUCTION_SLOT; s++)
        {
            if(auction_slot[s][SLOT_TYPE] == TYPE_BUSINESS && auction_slot[s][DATABASE_ID] == business_sql)
            {
                slot = s;
                break;
            }
        ...
        auction_slot[slot][DATABASE_ID] = business_sql;
        auction_slot[slot][START_RATE] = price;
        ...
        format(auction_slot[slot][OWNER_SLOT_NAME], 24, "GOS");
        cache_get_field_content(row, "name", auction_slot[slot][SLOT_NAME], mysql, 24);

        if(!auction_slot[slot][SLOT_NAME][0])
        {
            format(auction_slot[slot][SLOT_NAME], 24, "Business_%d", business_sql);
        }

        auction_slot[slot][SERVER_ID] = -1;
        for(new businessid; businessid < MAX_BUSINESS; businessid++)
        {
            if(GetBusinessData(businessid, B_SQL_ID) == business_sql)
            {
                auction_slot[slot][SERVER_ID] = businessid;
                format(auction_slot[slot][OPISANIE_SLOT], 124, "Business (%d)", businessid);
                break;
            }
        }
    ...
    cache_delete(result);
    printf("[Auction]: gov businesses loaded: %d, skipped: %d", loaded, skipped);
    return 1;
}
...
stock LeafAuction(playerid, all_slot)
{
    ...
    new active_slot_id[MAX_AUCTION_SLOT], slot = -1, filter = player_info_panel[playerid][auc_filter_type], 
