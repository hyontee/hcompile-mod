...
stock GetPromoCodeDataByID(promoId, E_PROMO_DATA:promoData[]);
stock GetPromoCodeDataByString(const promoCode[], E_PROMO_DATA:promoData[]);
...
{
    print("[MISTERS_SYSTEM] Система промокодов загружена.");
...
public OnPromoTimerTick(playerPromoSQLID)
{
    ...
    {
        printf("[MISTERS_PROMO] Ошибка: Не удалось найти данные player_promos (SQL ID: %d) для таймера. errno: %d", playerPromoSQLID, mysql_errno());
...
public CheckAndCreatePromoTables()
{
    ...
    if(mysql_errno() != 0)
    {
        printf("[MISTERS_PROMO] Таблица 'promocodes' не найдена. Создаем новую структуру...");
        ...
        if(mysql_errno() != 0)
        {
            printf("[MISTERS_PROMO] Ошибка при создании таблицы 'promocodes': %d", mysql_errno());
        }
        else
        {
            printf("[MISTERS_PROMO] Таблица 'promocodes' успешно создана с новой структурой.");
        }
    }
    cache_delete(cache);
    ...
    if(mysql_errno() != 0)
    {
        printf("[MISTERS_PROMO] Таблица 'activated_promos' не найдена. Создаем новую структуру...");
        ...
        if(mysql_errno() != 0)
        {
            printf("[MISTERS_PROMO] Ошибка при создании таблицы 'activated_promos': %d", mysql_errno());
        }
        else
        {
            printf("[MISTERS_PROMO] Таблица 'activated_promos' успешно создана.");
        }
    }
    cache_delete(cache);
    ...
    if(mysql_errno() != 0)
    {
        printf("[MISTERS_PROMO] Таблица 'promo_prizes' не найдена. Создаем...");
        ...
        if(mysql_errno() != 0)
        {
            printf("[MISTERS_PROMO] Ошибка при создании таблицы 'promo_prizes': %d", mysql_errno());
        }
        else
        {
            printf("[MISTERS_PROMO] Таблица 'promo_prizes' успешно создана.");
        }
    }
    cache_delete(cache);
    ...
    {
        printf("[MISTERS_PROMO] Таблица 'player_promos' не найдена или имеет старую структуру. Создаем новую...");
       
        if(mysql_errno() == 0) {
            mysql_query(mysql, "DROP TABLE IF EXISTS `player_promos`", false);
            printf("[MISTERS_PROMO] Старая таблица 'player_promos' удалена.");
        }
        ...
        if(mysql_errno() != 0)
        {
            printf("[MISTERS_PROMO] Ошибка при создании таблицы 'player_promos': %d", mysql_errno());
        }
        else
        {
            printf("[MISTERS_PROMO] Таблица 'player_promos' успешно создана с новой структурой.");
        }
    }
    cache_delete(cache);
    ...
    if(cache_num_rows() == 0)
    {
        printf("[MISTERS_PROMO] Столбец 'promo_id' в 'ownable_cars' не найден. Добавляем...");
        mysql_format(mysql, query, sizeof(query), "ALTER TABLE `ownable_cars` ADD `promo_id` INT NOT NULL DEFAULT '0' AFTER `create_time`");
        mysql_query(mysql, query, false);
        if(mysql_errno() != 0)
        {
            printf("[MISTERS_PROMO] Ошибка при добавлении столбца 'promo_id': %d", mysql_errno());
        }
        else
        {
            printf("[MISTERS_PROMO] Столбец 'promo_id' успешно добавлен.");
        }
    }
    cache_delete(cache);
    ...
    if(cache_num_rows() == 0)
    {
        printf("[MISTERS_PROMO] Столбец 'promo_status_car' в 'ownable_cars' не найден. Добавляем...");
        ...
        if(mysql_errno() != 0)
        {
            printf("[MISTERS_PROMO] Ошибка при добавлении столбца 'promo_status_car': %d", mysql_errno());
        }
        else
        {
            printf("[MISTERS_PROMO] Столбец 'promo_status_car' успешно добавлен.");
        }
    }
    cache_delete(cache);
}
...
public LoadPromoCodesFromDatabase()
{
    ...
    {
        printf("[MISTERS_PROMO] ВНИМАНИЕ: Количество промокодов (%d) превышает размер массива (%d). Загружено только %d.", g_iLoadedPromoCodes, MAX_PROMO_CODES_ARRAY_SIZE, MAX_PROMO_CODES_ARRAY_SIZE);
        g_iLoadedPromoCodes = MAX_PROMO_CODES_ARRAY_SIZE;
    }
    else if(g_iLoadedPromoCodes == 0) {
        printf("[MISTERS_PROMO] В базе данных нет промокодов для загрузки.");
    }
    ...
    cache_delete(result);
    printf("[MISTERS_PROMO] Все промокоды успешно загружены. Загружено: %d", g_iLoadedPromoCodes);
...
stock SetPlayerPromoTimer(playerid, playerPromoSQLID, durationHours)
{
    ...
    {
        printf("[MISTERS_PROMO] Игрок %s (ID: %d) достиг лимита активных временных призов (%d). Таймер для приза %d не установлен.",
    ...
    printf("[MISTERS_PROMO] Игроку %s (ID: %d) установлен таймер для приза из player_promos.id: %d на %d часов.",
...
stock StopPlayerPromoTimer(playerid)
{
    ...
    for(new i = 0; i < g_iPlayerPromoTimersCount[playerid]; i++)
    {
        ...
        if(mysql_errno() == 0 && cache_num_rows(result) > 0)
        {
            ...
            mysql_query(mysql, query, false);
            printf("[MISTERS_PROMO] Таймер для приза (player_promos.id: %d) игрока %s (ID: %d) остановлен и время сохранено. Осталось: %d секунд.",
...
stock RevokePromoPrize(accountId, prizeType, prizeValue, promoIdForCarRevoke)
{
    ...
    switch(prizeType)
    {
        ...
        case PRIZE_TYPE_CAR:
        {
            ...
            if(rows > 0)
            {
                ...
                printf("[MISTERS_PROMO] Автомобиль (SQL ID: %d, Model: %d) отозван у игрока %s (ID: %d) из-за истечения срока действия приза.",
    ...
    printf("[MISTERS_PROMO] Приз типа %d (value %d) отозван у игрока %s (ID: %d).", prizeType, prizeValue, GetPlayerNameEx(playerid), accountId);
    return 1;
}
...
stock ShowPromoCreationConfirmation(playerid)
{
    ...
    format(szTemp, sizeof(szTemp), "{ffffff}Промокод: {FF6347}%s\n", promoCode);
    ...
    format(szTemp, sizeof(szTemp), "{ffffff}Лимит использований: {FF6347}%s\n\n", szUsesLimitText);
    ...
    for(new i = 0; i < prizeCount; i++)
    {
        ...
        switch (prizeType)
        {
            case PRIZE_TYPE_MONEY:
            {
                format(szPrizeDetails, sizeof(szPrizeDetails), "{ffffff}%d. Деньги: {FF6347}%d руб. {888888}(%s)\n", i + 1, prizeValue, szDurationText);
            }
            case PRIZE_TYPE_DONATE:
            {
                format(szPrizeDetails, sizeof(szPrizeDetails), "{ffffff}%d. Донат: {FF6347}%d доната {888888}(%s)\n", i + 1, prizeValue, szDurationText);
            }
            case PRIZE_TYPE_CAR:
            {   
                format(szPrizeDetails, sizeof(szPrizeDetails), "{ffffff}%d. Автомобиль: {FF6347}%s (модель ID: %d) {888888}(%s)\n", i + 1, GetVehicleInfo(prizeValue - 400, VI_NAME), prizeValue, szDurationText);
            }
        }
...
stock CreatePromoCodeInDatabase(playerid)
{
    ...
    if(mysql_errno() != 0)
    {
        ...
        printf("[MISTERS_PROMO] Ошибка создания промокода (promocodes table): %d", mysql_errno());
        cache_delete(result);
        return 0;
    }
    ...
    if(promoId == 0)
    {
        ...
        printf("[MISTERS_PROMO] Ошибка: cache_insert_id вернул 0 для промокода '%s'.", promoCode);
        return 0;
    }
    ...
    for(new i = 0; i < prizeCount; i++)
    {
        ...
        if(mysql_errno() != 0)
        {
            ...
            SendClientMessage(playerid, -1, szMsg);
            printf("[MISTERS_PROMO] Ошибка добавления приза (promo_prizes table) для промокода ID %d, приз %d: %d", promoId, i, mysql_errno());
        }
    }
    ...
    switch(dialogid)
    {
            ...
            if(response)
            {
                switch(listitem)
                {
                    ...
                    /*case 1:
                    {
                        ...
                        "{EB4C42} #2.\t\t\t\t{FFFFFF}Второй уровень %s\n"\
                    ...
                    case 2:
                    {
                        ShowPlayerDialog(playerid, DIALOG_PROMO_INFO, DIALOG_STYLE_MSGBOX,
                            "{FF0000} Промокод",
                            "{FF6347}Промо-код{ffffff} - уникальный набор символов, позволяющий пользователям использовать его при регистрации и получать бонусы\nПромо-код разделен на 10 уровней, на каждом из которых Вы будете получать {FF6347}увеличенный бонус {ffffff}за регистрацию игрока,\nа также {FF6347}увеличенный проц. доната{ffffff}, за донат игрока. Повысить свой уровень можно за активацию промо-кода или за донат в управлении промокодом.\nДля получения наград используйте {FF6347}/checkpromo{FFFFFF} и {FF6347}/mypromo {FFFFFF} для управления промокодом",
                            "Назад", ""
                        );
                    }
                    case 3:
        ...
        case DIALOG_PROMO_UPGRADE_LEVELS:
        {
            if(response)
            {
                ...
                {
                    format(dialog_text, sizeof(dialog_text), "Вы действительно желаете повысить уровень промокода за {FF6347}%d {ffffff}активаций?\nПри повышении Вы получите новые условия: {FF6347}%d{ffffff} проц. от доната игрока, {FF6347}%d{ffffff} рублей\nЗа приглашенного игрока в виде бонуса", needed_activations, (target_level == 1 ? 0 : PROMO_LEVEL_DATA[target_level][2]), promo_money_bonus);
                }
                else if(promo_donat_balance >= needed_donat)
                {
                    format(dialog_text, sizeof(dialog_text), "Вы действительно желаете повысить уровень промокода за {FF6347}%d {ffffff}доната?\nПри повышении Вы получите новые условия: {FF6347}%d{ffffff} проц. от доната игрока, {FF6347}%d{ffffff} рублей\nЗа приглашенного игрока в виде бонуса", needed_donat, (target_level == 1 ? 0 : PROMO_LEVEL_DATA[target_level][2]), promo_money_bonus);
                }
                else
                {
        ...
        case DIALOG_ACT_PROMO:
        {
            ...
            switch (listitem)
            {
                case 0: // Активировать промокод
                {
                    callcmd::bcode(playerid, "");
                }
                case 1: // Подарок ютуберу
                {
                   Dialog(
                       playerid, DIALOG_GIFT_PROMO_YT, DIALOG_STYLE_LIST,
                       "{FF0000}Подарок ютуберу",
                       "{FF6347}#1 {FFFFFF}Подбросить наверх (слапнуть)\t\t\t\t\t\t{00CC00}25 доната\n"\
                       "{FF6347}#2 {FFFFFF}Забрать оружие\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t{00CC00}100 доната\n"\
                       "{FF6347}#3 {FFFFFF}Установить 1 хп ютуберу\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t{00CC00}100 доната\n"\
                       "{FF6347}#4 {FFFFFF}Телепортировать авто ютубера к себе\t\t\t\t\t\t\t\t{00CC00}100 доната\n"\
                       "{FF6347}#5 {FFFFFF}Телепортироваться к ютуберу\t\t\t\t\t\t{00CC00}250 доната\n"\
                       "{FF6347}#6 {FFFFFF}Телепортировать к себе\t\t\t\t\t\t\t\t\t\t\t{00CC00}250 доната\n"\
                       "{FF6347}#7 {FFFFFF}Выдать мут на 5 минут\t\t\t\t\t\t\t\t\t\t\t\t{00CC00}500 доната\n"\
                       "{FF6347}#8 {FFFFFF}Посадить в тюрьму на 5 минут\t\t\t\t\t\t{00CC00}500 доната\n"\
                       "{FF6347}#9 {FFFFFF}Кикнуть с сервера\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t{00CC00}1000 доната",
                       "Далее", "Назад"
                   );
                }                
                case 2: // Управление/создание промокода
                {
                    ...
                    else
                    {
                        ...
                        (
                            ...
                            "{FF6347}Промокод {FFFFFF}- уникальный набор симлов, позволяющий Вам и человеку, использующему Ваш промокод получить вознограждение\n"\
                            "Вы можете самостоятельно создать {FF6347}свой промокод{FFFFFF}, при условии что он свободен.\n\n"\
                            "\n"\
                            "Однако стоит помнить что количество символов в промокоде определяют его {FF6347}стоимость\n"\
                            "{FF6347}От 3 символов - {FFFFFF}10.000.000 рублей или 2.000 ВС\n"\
                            "{FF6347}От 5 символов до 7 символов - {FFFFFF}5.000.000 рублей.\n"\
                            "{FF6347}От 7 символов - {FFFFFF}3.000.000 рублей.\n"\
                            "Вы можете использовать {FF6347}латиницу и кирилицу, # и цифры.\n\n"\
...
case DIALOG_APPLY_PROMO_CODE:
{
    ...
    new E_PROMO_DATA:promoData[PROMO_DATA_SIZE]; 

    new accountId = GetPlayerAccountID(playerid);
    //new promoSqlId = promoData[PromoID]; перестало работать + сложна Mister Odin
    new query[286];
    new Cache:result;
    ...
    if(GetPromoCodeDataByString(inputtext, promoData))
    {
        if(promoData[UsesLeft] > 0 || promoData[PromoUses] == 0) 
        {
            ...
            if(mysql_errno() != 0 || cache_num_rows(result) == 0)
            {
                ...
                printf("[MISTERS_PROMO] Ошибка загрузки призов для промокода ID %d: %d", promoSqlId, mysql_errno());
                cache_delete(result);
                return 1;
            }
            ...
            if(rows == 1)
            {
                ...
                if(prizeDuration == 0) {
                    ...
                    format(szDurationText, sizeof(szDurationText), " {ffffff}на {FF6347}%d {ffffff}ч.", prizeDuration);
                }
                ...
                switch (prizeType)
                {
                    ...
                    {
                        GivePlayerMoneyEx(playerid, prizeValue);
                        format(currentPrizeMsg, sizeof(currentPrizeMsg), "{ffffff}- {FF6347}%d руб.%s\n", prizeValue, szDurationText);
                    }
                    case PRIZE_TYPE_DONATE:
                    {
                        GivePlayerDonateRub(playerid, prizeValue);
                        format(currentPrizeMsg, sizeof(currentPrizeMsg), "{ffffff}- {FF6347}%d доната%s\n", prizeValue, szDurationText);
                    }
                    case PRIZE_TYPE_CAR:
                    {
                        GivePromoCar(playerid, prizeValue, promoSqlId);
                        format(currentPrizeMsg, sizeof(currentPrizeMsg), "{ffffff}- Автомобиль: {FF6347}%s (модель ID: %d)%s\n", GetVehicleInfo(prizeValue - 400, VI_NAME), prizeValue, szDurationText);
                    }
                    case PRIZE_TYPE_EXP:
                    {
                        AddPlayerData(playerid, P_EXP, +, prizeValue);
                        UpdatePlayerDatabaseInt(playerid, "exp", GetPlayerData(playerid, P_EXP));
                        format(currentPrizeMsg, sizeof(currentPrizeMsg), "{ffffff}- {FF6347}%d EXP\n", prizeValue);
                    }
                }
                ...
                if(mysql_errno() != 0)
                {
                    printf("[MISTERS_PROMO] Ошибка записи активации приза %d для игрока %d: %d", prizeSqlId, accountId, mysql_errno());
                }
                else
                {
            ...
            else if(rows == 2)
            {
                ...
                if(prizeDuration2 == 0) {
                    ...
                    format(szDurationText, sizeof(szDurationText), " {ffffff}на {FF6347}%d {ffffff}ч.", prizeDuration2);
                }
                ...
                if(mysql_errno() != 0)
                {
                    printf("[MISTERS_PROMO] Ошибка записи активации приза %d для игрока %d: %d", prizeSqlId2, accountId, mysql_errno());
                }
                else
                {
                    new playerPromoEntryID = cache_insert_id(insertResult);
            ...
            else if(rows == 3)
            {
                ...
                if(prizeDuration3 == 0) {
                    ...
                    format(szDurationText, sizeof(szDurationText), " {ffffff}на {FF6347}%d {ffffff}ч.", prizeDuration3);
                }
                ...
                if(mysql_errno() != 0)
                {
                    printf("[MISTERS_PROMO] Ошибка записи активации приза %d для игрока %d: %d", prizeSqlId3, accountId, mysql_errno());
                }
                else
                {
                    new playerPromoEntryID = cache_insert_id(insertResult);
            ...
            else if(rows == 4)
            {
                ...
                if(prizeDuration4 == 0) {
                    ...
                    format(szDurationText, sizeof(szDurationText), " {ffffff}на {FF6347}%d {ffffff}ч.", prizeDuration4);
                }
                ...
                if(mysql_errno() != 0)
                {
                    printf("[MISTERS_PROMO] Ошибка записи активации приза %d для игрока %d: %d", prizeSqlId4, accountId, mysql_errno());
                }
                else
                {
                    new playerPromoEntryID = cache_insert_id(insertResult);
            ...
            else if(rows == 5)
            {
                ...
                if(prizeDuration4 == 0) {
                    ...
                    format(szDurationText, sizeof(szDurationText), " {ffffff}на {FF6347}%d {ffffff}ч.", prizeDuration5);
                }
                ...
                if(mysql_errno() != 0)
                {
                    printf("[MISTERS_PROMO] Ошибка записи активации приза %d для игрока %d: %d", prizeSqlId5, accountId, mysql_errno());
                }
                else
                {
                    new playerPromoEntryID = cache_insert_id(insertResult);
...
public OnPromoCodeCheckComplete(playerid, const promoCode[])
{
    ...
    {    
        ...
        else
        {
            ...
            format(message, sizeof(message), "{FF6347}| {FFFFFF}Промокод '{FF6347}%s{FFFFFF}' успешно создан! С Вашего баланса было списано {FF6347}%d доната.", promoCode, vc_cost);
            SendClientMessage(playerid, -1, message);
            callcmd::mypromo(playerid, "");
            LoadPromoCodesFromDatabase();
        }
    ...
    else 
    {
        ...
        format(message, sizeof(message), "{FF6347}| {FFFFFF}Промокод '{FF6347}%s{FFFFFF}' успешно создан! С Вашего баланса было списано {FF6347}%d руб.", promoCode, cost);
        SendClientMessage(playerid, -1, message);
        callcmd::mypromo(playerid, "");
        LoadPromoCodesFromDatabase();
    }
...
{
    new dialog_text[512];
    new list_item_3_text[512];
    
    if(HasPlayerCreatedPromo(playerid))
    {
        format(list_item_3_text, sizeof(list_item_3_text), "{EB4C42} #3. {FFFFFF}Управление промокодом\t\t\t\t\t\t\t {888888} нажмите для взаимодействия");
    }
    else
    {
        format(list_item_3_text, sizeof(list_item_3_text), "{EB4C42} #3. {FFFFFF}Создать промокод\t\t\t\t\t\t\t {888888} нажмите для взаимодействия");
    }

    format(dialog_text, sizeof(dialog_text), "{EB4C42} #1. {FFFFFF}Активировать промокод\t\t\t\t\t {888888} нажмите для взаимодействия\n{EB4C42} #2. {FFFFFF}Подарок ютуберу\t\t\t\t\t\t {888888} нажмите для взаимодействия\n%s\n{EB4C42} #4. {FFFFFF}Забрать призы\t\t\t\t\t\t\t {888888} нажмите для взаимодействия", list_item_3_text);
