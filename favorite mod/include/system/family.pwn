...
{//Автор: https://t.me/welsistudio (Welsi Studio)
    new Cache:cache = mysql_query(mysql, "SELECT * FROM accounts WHERE family_id", true);//Автор: https://t.me/welsistudio (Welsi Studio)

    if(mysql_errno())
    {
        mysql_query(mysql, "ALTER TABLE `accounts` ADD `family_id` INT NOT NULL DEFAULT '-1' AFTER `admin`, ADD `family_rang` INT NOT NULL DEFAULT '1' AFTER `family_id`, ADD `family_mute` INT NOT NULL AFTER `family_rang`, ADD `family_access` VARCHAR(24) NOT NULL DEFAULT '0,0,0,0,0,0,0' AFTER `family_mute`", false);
            ...
            if(GetPVarInt(playerid, "input_ad_family"))
            {
                ...
                else
                {
                    ...
                    else
                    {
                       ...
                       if(!mysql_errno()) FSendClientMessage(f_string144, playerid, -1, ""c_f_g"Вы успешно создали объявление: {FF6347}%s", inputtext);
                    }
                    cache_delete(cache);
                }
