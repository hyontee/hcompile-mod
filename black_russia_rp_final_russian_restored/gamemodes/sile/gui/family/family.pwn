...
stock FamilyCreate(playerid, const source_name[])
{
    ...
    if(!(strlen(fam_name) >= 6 && strlen(fam_name) <= 31))
        ...
        if(response)
        {
            if(GetPVarInt(playerid, "select_type_2236"))
            {
                ...
                switch(GetPVarInt(playerid, "select_type_2236"))
                {
                    ...
                    case 2:
                    {
                        if(listitem >= 0 && listitem <= 1 && GetPVarInt(playerid, "prichina") == 0) return ShowMembersFamily(playerid, listitem);
...
stock ResolveFamilyCarVinylName(family_index, dest[], size, bool:save_if_missing = false)
{
    if(size <= 0) return 0;
    dest[0] = '\0';
    if(!(family_index >= 0 && family_index < MAX_OWNABLE_CARS)) return 0;
...
{
    if(!(fam >= 0 && fam < MAX_FAM) || !(rang >= 0 && rang < 5)) return 0;
