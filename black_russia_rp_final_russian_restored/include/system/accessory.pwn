...
stock ConvertMoneyACS(money, string[], length = sizeof string)
{
    ...
    {
        if(string[i] != '\0' && '0' <= string[i] && string[i] <= '9')
        {
...
{
    return (index >= 0 && index < sizeof(accessory_shop_data));
}
...
stock AccessoryShowInventory(playerid)
{
    ...
    for(new i = 0; i < rows; i++)
    {
        ...
        if(!(acs >= 0 && acs < sizeof accessory)) continue;
...
stock UseAccessory(playerid, database)
{
    ...
    if(!(id_acs >= 0 && id_acs < sizeof accessory))
    {
