...
new taxi_company[3][3]; //sql , server_id
    ...
    for(new i, string[84]; i < 3; i++)
    {
        for(new e; e < g_business_loaded; e++)
        {
            if(GetBusinessData(e, B_SQL_ID) && strfind(GetBusinessData(e, B_NAME), taxi_name[i]) != -1)
            {
                ...
                new world_id = taxi_company[i][1] + 255;

                format(string, sizeof string, "Таксопарк: {FFFF00}\"%s\"{FFFFFF}\nПодойдите для ознакомления", taxi_name[i]);

                Create3DTextLabel(string, -1, taxi_main[0], taxi_main[1], taxi_main[2], 3.5, world_id);
                CreateDynamicActor(25, taxi_main[0], taxi_main[1], taxi_main[2], taxi_main[3], _, _, world_id, 1);
                taxi_main_area[i] = CreateDynamicSphere(taxi_main[0], taxi_main[1], taxi_main[2], 1.5, world_id, 1);
                
                taxi_company[i][2] = world_id;
            }
        }
    }
        ...
        if(response){
            ...
            switch(listitem)
            {
                case 0: {
                    ...
                    UpdatePlayerDatabaseInt(playerid, "taxi_company", select);
...
stock TextDrawTaxi()
{
	taxi_panel_TD[0] = TextDrawCreate(14.5833, 208.9259, "txd:br_taxistik");
...
stock PlayerTextDrawTaxi(playerid){
    ...
    taxi_panel_PTD[playerid][0] = CreatePlayerTextDraw(playerid, 18.7500, 211.7777, "_1234_M."); // пусто
    ...
    taxi_panel_PTD[playerid][1] = CreatePlayerTextDraw(playerid, 18.7500, 223.7037, "_123345_p."); // пусто
    ...
    taxi_panel_PTD[playerid][2] = CreatePlayerTextDraw(playerid, 19.1666, 235.1111, "_KoЇop¦"); // пусто
    ...
    taxi_panel_PTD[playerid][3] = CreatePlayerTextDraw(playerid, 34.5833, 260.5184, "_04:23"); // пусто
