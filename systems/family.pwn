        ...
        if(response)
        {
            ...
            if(GetPlayerDonateRub(playerid) >= 150)
            {
                ...
                if(!mysql_errno())
                {
                    ...
                    GivePlayerDonateRub(playerid, -150);
...
CMD:family(playerid)
{
    ...
    format(headers, sizeof(headers), "№\tНазвание\tОписание");
    format(dialogStr, sizeof(dialogStr),
        "%s\n"\
        "1\tОбъявления семьи\tНажмите для взаимодействия\n"\
        "2\tИнформация\tНажмите для взаимодействия\n"\
        "3\tНастройки семьи\tНажмите для взаимодействия\n"\
        "4\tУправление семьей\tНажмите для взаимодействия\n"\
        "5\tСклад семьи\tНажмите для взаимодействия\n"\
        "6\tАвтопарк семьи\tНажмите для взаимодействия\n"\
        "7\tУлучшения семьи\tНажмите для взаимодействия\n"\
        "8\tРейтинг семей\tНажмите для взаимодействия\n"\
        "9\tСемейные логи\tНажмите для взаимодействия\n"\
        "10\tПокинуть семью\tНажмите для взаимодействия",
        headers
    );

    Dialog(playerid, 2220, DIALOG_STYLE_TABLIST_HEADERS,
        "{FF0000}Панель семьи",
        dialogStr,
        "Выбрать", "Закрыть"
    );
...
stock BuyFamVit4lka(playerid)
{
    Dialog
    (
        playerid, 2238, DIALOG_STYLE_INPUT, 
        "{FFD700}Создание банды {FFFFFF}| Название",
        "Стоимость создания банды — {FFD700}150 BC\n"\
...
stock slotfam(playerid)
{
    if(GetPlayerIdFamily(playerid) == -1)
        return ShowNotificationNew(playerid, 2, 6, 0, 0, "Вы не состоите в семье", "");
...
stock obmenfam(playerid)
{
    if(GetPlayerIdFamily(playerid) == -1 || GetPlayerRangFamily(playerid) != 5)
        return ShowNotificationNew(playerid, 2, 6, 0, 0, "Вы не лидер семьи", "");
...
stock buyfamname(playerid)
{
    ...
    if(GetPlayerDonateRub(playerid) >= 100)
    {
        ...
        (
            ...
            "{FF0000}Изменение названия семьи",
            "Напишите новое название семьи ниже\n"\
