...
// Compatibility with the notification system used by rexen.pwn.
stock ShowNotificationNew(playerid, type, duration, id, subId, caption[], btnCaption[])
{
    return ShowNotificationSile(playerid, type, duration, id, subId, caption, btnCaption);
}
...
stock FireSystemShowCallProgressText(playerid, progress)
{
    if(!fire_player_call_prog_td_created[playerid])
    {
        fire_player_call_prog_td[playerid] = CreatePlayerTextDraw(playerid, 320.0, 214.0, " 0/15");
    ...
    new string[32];
    format(string, sizeof string, " %d/%d", progress, FIRE_CALL_OBJECT_HEALTH);
...
{
    return ShowNotificationNew(playerid, type, 4, -1, 0, text, "");
}
...
stock FireSystemSendCallAcceptedMessages(playerid, callid)
{
    ...
    format(text, sizeof text, "{FFFF00}{FFFFFF}   {FFFF00}%s{FFFFFF}.", fire_call_name[callid]);
    ...
    (
        text,
        sizeof text,
        "{FFFF00}{FFFFFF}: {FFFF00}%d{FFFFFF}. : {FFFF00}%.1f {FFFFFF}.",
...
stock FireSystemSendRewardMessages(playerid, payout, experience)
{
    ...
    format(text, sizeof text, "{FFFF00}{FFFFFF}     {FFFF00}%d {FFFFFF},   .", payout);
    SendClientMessage(playerid, -1, text);

    format(text, sizeof text, "{FFFF00}{FFFFFF}   {FFFF00}%d {FFFFFF} .", experience);
    SendClientMessage(playerid, -1, text);
    return 1;
}
...
public FireSystemCallTimer(playerid)
{
    ...
    if(fire_player_call_time_left[playerid] <= 0)
    {
        ...
        FireSystemShowCallNotify(playerid, 2, "   .");
        return 1;
    }
...
stock FireSystemAttachPartnerToCall(ownerid, partnerid, callid)
{
    ...
    FireSystemShowCallNotify(ownerid, 3, "    .");
    FireSystemShowCallNotify(partnerid, 3, "     .");
    return 1;
}
...
stock FireSystemFinishCall(playerid, bool:success = true)
{
    ...
    if(success)
    {
        ...
        if(owner != INVALID_PLAYER_ID && IsPlayerConnected(owner))
        {
            GivePlayerMoneyEx(owner, payout, ":  ", true, true);
            ...
            FireSystemShowCallNotify(owner, 3, " .   .");
        }

        if(partner != INVALID_PLAYER_ID && IsPlayerConnected(partner))
        {
            GivePlayerMoneyEx(partner, payout, ":  ", true, true);
            ...
            FireSystemShowCallNotify(partner, 3, " .   .");
        }
    }
    else
    {
...
stock FireSystemStartCall(playerid, callid)
{
    ...
    {
        new text[128];
        format(text, sizeof text, "     ,  {FFFF00}%d {FFFFFF}.", need_rank);
    ...
    if(partnerid != INVALID_PLAYER_ID)
    {
        ...
        {
            return FireSystemShowCallNotify(playerid, 2, "     ."), 1;
        }
    ...
    FireSystemShowCallNotify(playerid, 3, "     GPS.");
...
stock FireSystemShowCallsMenu(playerid)
{
    ...
    new body[4096];
    strcat(body, "{FFFF00}##\t{FFFF00} \t{FFFF00}\t{FFFF00}\n");
    ...
    (
        ...
        "{E0584B}Black Russia {555555}RolePlay {FFFFFF}->  |  ",
        body,
        "",
        ""
    );
...
stock FireSystemTryExtinguish(playerid)
{
    ...
    for(new slot; slot < FIRE_CALL_OBJECT_COUNT; slot++)
    {
        ...
        ShowNotificationNew(playerid, 3, 5, -1, 0, "  .", "");
...
stock FireSystemEnsureDBColumn(column_name[], column_definition[])
{
    ...
    if(mysql_errno())
    {
        printf("[FIRESYSTEM]     mhcwork.%s, errno=%d", column_name, mysql_errno());
        cache_delete(result);
        return 0;
    }
...
stock FireSystemCreateDatabase()
{
    ...
    if(mysql_errno())
    {
        printf("[FIRESYSTEM]     mhcwork, errno=%d", mysql_errno());
    }
...
stock FireSystemSavePlayerData(playerid)
{
    ...
    for(new attempt; attempt < 2; attempt++)
    {
        ...
        printf("[FIRESYSTEM]    mhcwork  account_id=%d, errno=%d", account_id, mysql_errno());
        return 0;
    }
...
stock FireSystemInvitePartner(playerid, targetid)
{
    ...
    new text[144];
    format(text, sizeof text, "  %s     .", GetPlayerNameEx(targetid));
    ...
    format(text, sizeof text, "  %s", GetPlayerNameEx(playerid));
    ShowNotificationNew(targetid, 4, 6, playerid, 0, text, ">>");
    return 1;
}
...
CMD:mesinvite(playerid, params[])
{
    ...
    if(sscanf(params, "i", targetid))
        return SendClientMessage(playerid, 0xCECECEFF, ": /mesinvite [id ]");
...
stock FireSystem_HandleNotifyClick(playerid, notify_type, notify_id, notify_sub_id)
{
    ...
    format(text, sizeof text, "   %s  .", GetPlayerNameEx(inviter));
    SendClientMessage(playerid, -1, text);
    ...
    {
        FireSystemShowCallNotify(inviter, 3, "  .");
        FireSystemShowCallNotify(playerid, 3, "   .");
    }
    return 1;
}
...
{
    SendClientMessage(playerid, -1, "{FFFFFF} {FFFF00} {FFFFFF} {FFFF00}   {FFFFFF}  ,   {FFFF00}2- {FFFFFF}.");
    SendClientMessage(playerid, -1, "{FFFFFF}        {FFFF00} {FFFFFF},   {FFFF00}2- {FFFFFF}.");
    return 1;
}
...
stock FireSystemHirePlayer(playerid)
{
    ...
    SendClientMessage(playerid, -1, "{FFFF00}{FFFFFF}    : {FFFF00}\"\"{FFFFFF}.");
    return 1;
}
...
stock FireSystemStartWorkDay(playerid)
{
    ...
    SendClientMessage(playerid, -1, "{FFFF00}  {FFFFFF}     \"{FFFF00} {FFFFFF}\".");
    SendClientMessage(playerid, -1, "{FFFFFF}     {FFFF00}/meslist {FFFFFF}  {FFFF00} {FFFFFF} .");
...
stock FireSystemEndWorkDay(playerid)
{
    ...
    FireSystemSavePlayerData(playerid);
    SendClientMessage(playerid, -1, "{FFFFFF}    {FFFF00}{FFFFFF}.");
    return 1;
}
...
stock FireSystemShowWardrobeMenu(playerid)
{
    ...
    (
        ...
        DIALOG_STYLE_TABLIST_HEADERS,
        "{E0584B}RAGE RUSSIA {FFFFFF}-> ",
        "{E0584B}##\t{E0584B}\t{AFAFAF} \n\
{E0584B}#1.\t{FFFFFF}/  \t{AFAFAF}  \n\
{E0584B}#2.\t{FFFFFF}/  \t{AFAFAF}  \n\
{E0584B}#3.\t{FFFFFF}  \t{AFAFAF}  ",
        "",
        ""
    );
    return 1;
...
{
    return ShowNotificationNew(playerid, type, 4, -1, button, text, "");
}

stock FireSystemSendNotEmployedNotify(playerid)
{
    return ShowNotificationNew(playerid, 2, 5, 0, 0, "    .", "");
}
...
stock FireSystemShowInventoryMenu(playerid, bool:sync_weapons = true)
{
    ...
    (
        ...
        sizeof body,
        "{E0584B}##\t{E0584B}\t{AFAFAF}\n\
{E0584B}#1.\t{FFFFFF}\t{FFFFFF}%d/%d\n\
{E0584B}#2.\t{FFFFFF}\t{FFFFFF}%d/%d\n\
{E0584B}#3.\t{FFFFFF}\t{FFFFFF}%d/%d\n\
{E0584B}-\t{FFFFFF} \t{FFFFFF}%d/100",
        extinguisher,
...
stock FireSystemTakeInventoryItem(playerid, item)
{
    ...
    switch(item)
    {
        case 0:
        {
            ...
            {
                FireSystemSendInventoryNotify(playerid, 2, "    .");
            ...
            FireSystemApplyExtinguisherAmmo(playerid);
            FireSystemSavePlayerData(playerid);
            FireSystemSendInventoryNotify(playerid, 3, "  .");
            FireSystemShowInventoryMenu(playerid, false);
            return 1;
        }
        case 1:
        {
            ...
            GivePlayerWeapon(playerid, FIRE_WEAPON_SHOVEL, 1);
            FireSystemSavePlayerData(playerid);
            FireSystemSendInventoryNotify(playerid, 3, "  .", 1);
            FireSystemShowInventoryMenu(playerid, false);
            return 1;
        }
        case 2:
...
stock FireSystemShowPersonalCard(playerid)
{
    ...
    format(title, sizeof title, "{E0584B}RAGE RUSSIA {FFFFFF}->    %s", GetPlayerNameEx(playerid));
    format
    (
        body,
        sizeof body,
        "{FFFF00}1. {FFFFFF} : {FFFF00} (%d)\n\
{FFFF00}2. {FFFFFF}      : {FFFF00}%d/{FFFFFF}%d",
        rank,
        current_exp,
        next_need
    );

    ShowPlayerDialog(playerid, FIRE_DIALOG_PERSONAL_CARD, DIALOG_STYLE_MSGBOX, title, body, "", "");
    return 1;
}
...
stock FireSystemShowCadetTopic(playerid, topic)
{
    ...
    switch(topic)
    {
        case 0:
        {
            format(title, sizeof title, "{E0584B} {FFFFFF}->  ");
            format
            (
                body,
                sizeof body,
 ...
 {FFFF00} {FFFFFF},   {FFFF00}   ."
        ...
        case 4:
        {
            ...
            (
                body,
                sizeof body,
                "{FFFF00} {FFFFFF}  {FFFF00} {FFFFFF} {FFFF00}.\n\
\n\
{FFFF00}   {FFFFFF}-       .\n\
\n\
{FFFF00}   {FFFFFF}-  . ,     .\n\
...
\n\
    ...
    ShowPlayerDialog(playerid, FIRE_DIALOG_CADET_TOPIC, DIALOG_STYLE_MSGBOX, title, body, "", "");
    return 1;
}
...
stock FireSystem_OnPlayerConnect(playerid)
{
...
stock FireSystem_OnPlayerDisconnect(playerid, reason)
{
    FireSystemAbortActiveCall(playerid, false);
    FireSystemSyncExtData(playerid, true);
    FireSystemSavePlayerData(playerid);
...
stock FireSystem_OnPlayerSpawn(playerid)
{
    FireSystemLoadPlayerData(playerid);
...
stock FireSystem_OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == FIRE_DIALOG_GAZEL_RENT)
    {
        ...
        ShowNotificationNew(playerid, 3, 5, -1, 0, "    {FFFF00}.", "");
        return 1;
    }
    if(dialogid == FIRE_DIALOG_WARDROBE_MENU)
    {
        ...
        switch(listitem)
        {
            ...
            {
                if(!fire_player_employed[playerid])
                    return SendClientMessage(playerid, -1, "{FFFFFF}    {FFFF00}{FFFFFF}."), 1;
...
public FireSystemVehicleReturnTimer(playerid)
{
    ...
    if(fire_player_return_time[playerid] <= 0)
    {
        ...
        ShowNotificationNew(playerid, 2, 5, -1, 0, "     .", "");
        return 1;
    }
...
stock FireSystemStartRetTimer(playerid, vehicleid)
{
    ...
    SendClientMessage(playerid, -1, "{FFFF00}{FFFFFF}   {FFFF00}60{FFFFFF}     .");
    FireSystemShowVehicleReturnText(playerid);
...
stock FireSystem_OnPlayerStateChange(playerid, newstate, oldstate)
{
...
stock FireSystem_OnPlayerExitVehicle(playerid, vehicleid)
{
...
stock FireSystem_OnPlayerUpdate(playerid)
{
    FireSystemSyncExtData(playerid, true);
...
stock FireSystem_OnPlayerEnterDynamicArea(playerid, areaid)
{
...
stock FireSystem_OnGameModeInit()
{
    FireSystemCreateDatabase();
    FireSystemInitCalls();
    ...
    CreateDynamic3DTextLabel("{FFFF00}   \n\n{FFFFFF}  {FFFF00}.", -1, -2557.969238, -284.699520, 27.394428, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 0);
    ...
    CreateDynamic3DTextLabel("{FFFF00}   \n\n{FFFFFF}  {FFFF00}.", -1, -2600.051757, -283.558868, 1246.720458, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 1);
    ...
    CreateDynamic3DTextLabel("{FFFF00} \n\n{FFFFFF}  {FFFF00}.", -1, -2615.686523, -282.474395, 1252.083862, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 1);

    CreateDynamic3DTextLabel("{FFFFFF}  \n{FFFF00}0 {FFFFFF}{FFFFFF} {FFFF00}30", -1, -2614.153564, -275.899963, 1246.723144, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 1);
    ...
    CreateDynamic3DTextLabel("{FFFF00} \n\n{FFFFFF}  {FFFF00}.{FFFF00}", -1, -2605.152832, -281.472106, 1252.083862, 5.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 1, 0, 1);

    fire_instructor_actor = CreateActorEx("{FFFF00} ", "{FFFFFF}  {FFFF00}.", 19275, -2623.872802, -270.104125, 1252.083862, 168.477981, 0, 1);
...
CMD:firerank(playerid, params[])
{
    if(GetPlayerAdminEx(playerid) < 13)
        return SendClientMessage(playerid, -1, "У вас недостаточно прав. Требуется 13 уровень админки.");

    fire_player_experience[playerid] = FIRE_NEXT_CLASS_EXPERIENCE * 2;

    SendClientMessage(playerid, -1, "Вам выдан 3 ранг пожарного.");
    return 1;
}