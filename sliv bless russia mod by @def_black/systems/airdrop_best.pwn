...
stock AirDrop()
{
    ...
    // Очистка чата и анонс
    for(new i = 0; i < 5; i++) SendClientMessageToAll(-1, "");
    
    SendClientMessageToAll(-1, ""SC" Внимание! На сервере началось событие {FFFF00}«AirDrop»");
    SendClientMessageToAll(-1, ""SC" По всей карте разбросаны {FFFF00}коробки с с призами!");
    SendClientMessageToAll(-1, ""SC" Чтобы узнать местоположение {FFFF00}коробок,{FFFFFF} используйте {FFFF00}/airdrop");


    // Создание {FFFF00}AirDropов
    for(new a; a < MAX_AIRDROP; a++)
    {
        new string[512];
        air_pickup[a] = CreatePickup(19054, 23, airdrop[a][AIR_X], airdrop[a][AIR_Y], airdrop[a][AIR_Z] - 0.6, 0.0, 0.0, 0.0, 0);
        format(string, sizeof string, "Коробка {FFFF00}#%d\n{BEBEBE}[ Подойдите для взаимодействия ]", a);
        text_in_obj[a] = CreateDynamic3DTextLabel(string, -1, airdrop[a][AIR_X], airdrop[a][AIR_Y], airdrop[a][AIR_Z], 10.0);
    ...
    // Выдача компенсации администраторам
    foreach(new i : Player)
    {
        if(GetPlayerAdminEx(i) > 0 && GetPlayerAdminEx(playerid) < 8) // Если игрок админ
        {
        GivePlayerMoneyEx(i, 500000, "ADMIN: AirDrop Compensation", true, false);
        
        // Выдача донатной валюты (рубли)
        GivePlayerDonateRub(i, 50, "ADMIN: AirDrop Compensation");
        
            
            // Уведомление
            SendClientMessage(i, 0xFF0000AA, "Вы Администратор, для вас Air-Drop запрещён!");
            SendClientMessage(i, 0x32a44bFF, "Вам выдана компенсация: 500.000 игровой валюты и 50 BC");
        }
    }
...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
   new id_air = IsPlayerInRangeOfAnyAirDrop(playerid);
    ...
    {
        if(airdrop[id_air][AIR_STATE] == AIR_STATE_LOAD) ShowNewNotification(playerid, 5, 5, AIR, 10, "Взаимодействие", ">>");
            else{
    ...
    if(dialogid == 2330)
    {
        if(response)
        {
            ...
            if(GetPlayerExp(playerid) > GetExpToNextLevel(playerid))
		    {
		    	...
		    	SCM(playerid, -1, "{FFFF00}Поздравляем! Ваш уровень повышен");
		    }
            ShowNewNotification(playerid, 3, 6, 0, 0, "Вы забрали все призы", "");
        }
        else 
        {
            // DeleteAirDrop(player_drop[playerid]);
            ShowNewNotification(playerid, 2, 6, 0, 0, "Вы отказались от подарков", "");
        }
    }
    if(dialogid == 2331)
    {
        if(response)
        {
            ...
            EnablePlayerGPS(playerid, 55, airdrop[id][AIR_X], airdrop[id][AIR_Y], airdrop[id][AIR_Z], "Местоположение {FFFF00}AirDropа отмечено на карте!");
        }
    }
...
stock DeleteAirDrop(id_air)
{
 ...
 //   DestroyPickup(air_pickup[id_air]);
...
stock crashbox(playerid)
{
    // Проверка на админа
    if(GetPlayerAdminEx(playerid) > 0 && GetPlayerAdminEx(playerid) < 8)
    {
        SendClientMessage(playerid, 0xFF0000AA, "Вы Администратор, для вас Air-Drop запрещён! Вы получаете компенсацию!");
        return 1;
    }
  ...
  //  ApplyAnimationEx(playerid, "BASEBALL", "Bat_4", 3.1, 1, 1, 1, 0, 0, 0, USE_ANIM_TYPE_NONE - 1);
    ShowNewNotification(playerid, 4, 6, AIR, 0, "Вскрыть коробку", ">>");
    return 1;
}
...
CMD:airdrop(playerid)
{
    ...
    new drop, text[185], list[2048] = "№\tНазвание\tРасстояние\tСтатус\n";
    new Float:dist;
    ...
    for(new d, count; d < MAX_AIRDROP; d++)
    {
        ...
        format(text, sizeof text, "{CA5757}#%d\t{FFFF00}Коробка\t{7E7E7E}%.2f м\t{00FF00}Доступен\n", d+1, dist);
        strcat(list, text);
        drop++;
        SetPlayerListitemValue(playerid, count ++, d);
    }

    if(!drop) return ShowNewNotification(playerid, 2, 6, 0, 0, "Событие еще не началось / все коробки открыты", "");

    DialogTAB45(playerid, 2331, DIALOG_STYLE_TABLIST_HEADERS, "{CA5757}AirDrop {FFFFFF}| Информация о коробках на карте", list, "Показать", "Закрыть");
    return 1;
} 
...
stock UnAirDrop()
{
    ...
    SendClientMessageToAll(-1, ""SC" Внимание! На сервере ЗАВЕРШЕНО событие {FFFF00}«AirDrop»");
    SendClientMessageToAll(-1, ""SC" Коробки с {FFFF00}призами {FFFFFF}были удалены!");
    SendClientMessageToAll(-1, ""SC" Следующий {FFFF00}«AirDrop» {ffffff}произойдёт в {FFFF00}XX:00!");
...
stock OpenAirDrop(playerid)
{
    
    new id = IsPlayerInRangeOfAnyAirDrop(playerid), text[74];
  //  DestroyPickup(id);
    airdrop[id][AIR_STATE] = AIR_STATE_CRASH;
    format(text, sizeof text, "Вскрытая коробка {FFFF00}#%d\n{FFFFFF}Вскрыта игроком {FFFF00}%s", id, GetPlayerData(playerid, P_NAME));
    UpdateDynamic3DTextLabelText(text_in_obj[id], -1, text);
    ...
    new id_air = IsPlayerInRangeOfAnyAirDrop(playerid);
       ...
       {    
                ...
                new text[1028];
                new id = player_drop[playerid], money = random(1000000) + 250000,donate = random(55) + 1,exp = random(5) + 5; 
                format(text, sizeof text,\
                "Вы открыли коробку{FFFF00} #%d!\n\
                 \n\
                 {CA5757}ВАШИ ПРИЗЫ:\n\
                  \n\
                {00FF00}%d {FFFFFF}игровой валюты {00FF00}(руб)\n\
                {FFD700}%d {FFFFFF}донат-рублей {FFD700}(BC)\n\
                {FA8072}%d {FFFFFF}очков опыта {FA8072}(exp)\n\
                 \n\
                {FFFFFF}Чтобы забрать все призы нажмите {00FF00}\"Забрать\" \n\
                {FFFFFF}Если вы нажмете {CA5757}\"Отмена\"{FFFFFF} Вы не получите призы.",
                id, money, donate, exp
                );    
                ...
                Dialog(playerid, 2330, DSM, "{CA5757}Информация о вскрытой коробке", text, "Забрать", "Отмена");
