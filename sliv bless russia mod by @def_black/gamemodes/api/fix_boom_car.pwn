...
public CheckPlayerVehicle(playerid)
{
    if(IsPlayerInAnyVehicle(playerid))
    {
        ...
        if(health < 279.0)
        {
            ...
            {
                //SendClientMessage(playerid, -1, "{868641}Ваш транспорт сломался, вызовите механика ((/call 090))");
                SendClientMessage(playerid, -1, "{868641}Ваш транспорт сломался, почините свой транспорт ((/rem))");
                ShowNewNotification(playerid, 2, 6, 0, 0, "Двигатель неисправен. Используйте /rem для починки", "");
