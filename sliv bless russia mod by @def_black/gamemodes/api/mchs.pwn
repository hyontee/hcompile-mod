...
stock CreateFireScreen(playerid)
{
    ...
    // В коде уже есть это, просто для наглядности
    /*if(!fire_TD)
    {
        fire_TD = TextDrawCreate(314.5835, 204.9333, "ВЫ_ГОРИТЕ!");
        TextDrawLetterSize(fire_TD, 0.9870, 4.0220);
        TextDrawTextSize(fire_TD, 0.0, 120.0);
        TextDrawAlignment(fire_TD, 2);
        TextDrawColor(fire_TD, -16776961);
        TextDrawSetOutline(fire_TD, 1);
        TextDrawBackgroundColor(fire_TD, 255);
        TextDrawFont(fire_TD, 1);
        TextDrawSetProportional(fire_TD, 1);
        TextDrawSetShadow(fire_TD, 0);
    }*/
...
stock ShowFireScreen(playerid)
{
    ...
    //TextDrawShowForPlayer(playerid, fire_TD);
    SendPlayerHudFoneFire(playerid, 1); // фон горения зоменко ип
...
stock HideFireScreen(playerid)
{
    ...
    SendPlayerHudFoneFire(playerid, 0); // фон горения зоменко ип
    //TextDrawHideForPlayer(playerid, fire_TD);
...
public OnPlayerConnect(playerid)
{
    ...
    printf("[CONNECT] player_order[playerid] = %d", player_order[playerid]);
...
public LoadMchs()
{
    ...
    // циклы вызывали ерроры, поэтому сделал так
...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    ...
    if(areaid == invent_mchs)
    {
        ...
        new exting_ammo = GetPlayerAmmo(playerid); // текущие патроны
        new armor;
        GetPlayerArmour(playerid, armor); // текущая броня

        new dialog_str[256];
        format(dialog_str, sizeof(dialog_str),
            "{EB4C42} #1. {FFFFFF}Огнетушитель \t\t %d/15000\n"\
            "{EB4C42} #2. {FFFFFF}Восстановить костюм \t%f/100.0",
            exting_ammo, armor
        );
...
public DamageFromFire(playerid)
{
    ...
    if(player_mchs_active[playerid])
    {
        ...
        if(armour > 0.0) // если есть защитный костюм (типо)
        {
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == 3500)
    {
        if(response)
        {
            ...
            if(id < 0 || id > 9) return 1; // защита
            if(order_mchs[id][MS_State] != STATE_ORDER_NO_TAKE) return 1;
            ...
            switch(listitem)
            {
                case 0:
                {
                    ...
                    else
                    {
                        ...
                        SCM(playerid, -1, ""SC"Для начала работы введите команду {ffff00}/meslist {ffffff}и выберите вызов из списка");
                        SCM(playerid, -1, ""SC"Чтобы{ffff00} арендовать служебный автомобиль{ffffff} вам нужно спуститься на{ffff00} 1-й этаж{ffffff} и сесть в любое авто");
                        player_order[playerid] = -1;
                        StartMchsShift(playerid);
                        if(sex == false) SetPlayerSkin(playerid, 19276); // мужской
                        else            SetPlayerSkin(playerid, 19901); // женский
                    }
                }
                case 1:
                {
                    ...
                    Dialog(
                        ...
                        "Чтобы взять заказ сначала возьмите огнетушитель и арендуйте транспорт \n"\
    ...
    if(dialogid == 3502 && response)
    {
        switch(listitem)
        {
            ...
            case 1: // Восстановить костюм
            {
                ...
                TimerMchsArmour[playerid] = SetTimerEx("ResetMchsArmourCD", 300000, false, "i", playerid); // 5 минут
            }
        }
        return 1;
    }
    if(dialogid == DIALOG_CANCEL_MCHS)
    {
        if(response)
        {
            ...
            if(id != -1)
            {
                ...
                mysql_format(mysql, String128m, sizeof String128m, "UPDATE order_mchs SET id_player = 0, state = %d WHERE id = %d", STATE_ORDER_NO_TAKE, id);
                mysql_query(mysql, String128m, false);
                if(mysql_errno()) print("Ошибка в запросе отмены заказа (3511)");
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    ...
    else if((oldkeys & KEY_FIRE)) // отпущена
    {
        if(TimerFire[playerid])
        {
...
// Тушение (таймер на удержание огнетушителя)
public HandleFirePress(playerid)
{
    ...
    if(all_fire[idx][F_SCORE] >= 10)
    {
        ...
        if(Order(id + 1))
        {
            ...
            endOrders(id); // сброс всех пожаров заказа
            player_order[playerid] = -1;
            order_mchs[id][MS_State] = STATE_ORDER_COMPLETED;

            mysql_format(mysql, String128m, sizeof String128m, "UPDATE order_mchs SET state = %d WHERE id = %d", STATE_ORDER_COMPLETED, id);
            mysql_query(mysql, String128m, false);

            if(mysql_errno()) print("Ошибка в запросе завершения заказа (HandleFirePress)");
            ...
            // ограничения: минимум 15000, максимум 35000
...
public LoadOrdersMchs()
{
    ...
    switch(random(2))
    {
        case 0:
        {
            for(new o = 0; o < 5; o++) LoadOrderInMesList(o);
        }
        case 1:
        {
            for(new o = 5; o < 10; o++) LoadOrderInMesList(o);
        }
    }
    return 1;
}
...
stock LoadOrderInMesList(id)
{
    ...
    mysql_format(mysql, String128m, sizeof String128m,
        "INSERT INTO order_mchs (id,id_player,state) VALUES (%d,0,1)", id);
    mysql_query(mysql, String128m, false);
...
stock endOrders(id)
{
    ...
    for(new i = 0; i < MAX_FIRE; i++)
    {
        ...
        all_fire[i][F_SCORE] = 0; // сброс прогресса
    }
    return 1;
}
...
public TimerMchs()
{
    for(new z = 0; z < sizeof(order_mchs); z++)
    {
        ...
        mysql_format(mysql, String128m, sizeof String128m,
            "INSERT INTO order_mchs (id,id_player,state) VALUES (%d,0,1)", z);
        mysql_query(mysql, String128m, false);
...
{
    if(!player_mchs_active[playerid])
        return SCM(playerid, -1, ""USC" Сначала устройтесь в МЧС");
    ...
    if(mysql_errno())
        return print("Ошибка в запросе 4");

    if(!rows)
        return SCM(playerid, 0xFFFFFFFF3, "На данный момент заказов нет. Ждите пейдей");

    if(player_order[playerid] != -1)
    {
        format(mString512, sizeof mString512, "У вас уже есть действующий заказ #%d.\n\nОтменить его?\n\n{FF6347}ВНИМАНИЕ:{FFFFFF} за невыполненный заказ оплата не начисляется.", player_order[playerid]);
        return Dialog(playerid, DIALOG_CANCEL_MCHS, DIALOG_STYLE_MSGBOX, "{FF6347}"SERVER_NAME" {FFFFFF}| МЧС", mString512, "Отменить", "Назад");
    }

    new id, list[235], text[235];
    new Float:dista;
    list[0] = EOS;
    ...
    if(rows)
    {
        ...
        DialogMhsc(playerid, 3500, DIALOG_STYLE_LIST, "Доступные заказы", list, "Взять", "Назад");
    }
    return 1;
}
