...
{
    print("[W_SYSTEM] Система каршеринга загружена.");
    ...
    if(dialogid == 2311)
    {
        if(response)
        {
            if (listitem == 10 || listitem == 11)
            {
                ...
                new price, text[128], dialog[sizeof text * 10 + 50] = "";
                ...
                for (new i = count - 10, c = -1; i < count; i++)
                {
                    ...
                    format(text, sizeof text, "{FFFFFF}№%d {FFFFFF}%s {FFFFFF}%d р./мин.\n",
                        i + 1,
                        GetVehicleInfo(vehicle_carshare[i] - 400, VI_NAME),
                        price
                    );
                ...
                DialogCarShare(playerid, 2311, DIALOG_STYLE_LIST, "Каршеринг", dialog, "Далее", "Назад");
            }
            else
            {
