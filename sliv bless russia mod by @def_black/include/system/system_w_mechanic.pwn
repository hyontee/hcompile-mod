...
new vihod_mex, vhod_mex, mex_sphere, sphere_item_mex, TaxistArz, TaxistYouzh, Taxist3, Taxist4, Taxist5;
new AvtoPOLICEONE, AvtoBOLKI, AvtoVCH, AvtoALLopg, AvtoSMI; // автоприем во фракцию бай виталька
...
public OnGameModeInit()
{
    ...
    vhod_mex = CreateDynamicSphere(2163.264404,-875.610778,27.947052, 1.5);
    TaxistArz = CreateDynamicSphere(831.484863,799.029357,13.171555, 2.5);
    TaxistYouzh = CreateDynamicSphere(2751.533447,-2442.895263,21.813438, 2.7);
    Taxist3 = CreateDynamicSphere(1792.450927,2529.986328,14.550534, 2.7);
    Taxist4 = CreateDynamicSphere(-2685.532958,1986.828491,9.985312, 2.7);
    Taxist5 = CreateDynamicSphere(-2428.550537,186.391296,26.135101, 2.7);
    
    // - - - - - - - - - - - - - автаприйом бай ветол - - - - - - - - - - - - - 
    
    /*


-1106.722778,1513.601684,1499.985961,44.707550,1,1 // avtopriemGIBDD

-1106.701171,1513.557128,1499.985961,38.534614,1,1 // avtopriemMVD

-1.417386,504.901458,1381.042236,179.572677,1,1 // avtopriemOPGarz

-1.684124,504.495910,1381.042236,179.598541,2,1 // avtopriemOPGbat

-1.727078,504.929199,1381.042236,179.756652,3,1 // avtopriemOPGlytka

1502.980346,2500.476074,2501.040283,267.069854,1,1 // avtopriemBOLKAarz

1502.976562,2500.463378,2501.040283,269.898223,2,1 // avtopriemBOLKAlytka

1985.044555,-2.426989,1381.043579,358.662017,1,1 // avtopriemSMI

498.644439,1511.592407,1501.040039,180.268295,1,1 // avtopriemVOENNAYA
...
*/
    ...
    // - - - - - - - - - - - - - - - - - - - - аолыдцд3дкщащщ3щ3сббчюы - - - - - - - - - //
    

    CreateActorEx("{FFD700}Начальник СТО", "{FFFFFF}Подойдите для взаимодействия", 206, 994.231079,1004.094543,1501.000000,222.366333, 1, 1);
    ...
    CreateDynamic3DTextLabel("Вход в автосервис\n[Работа автомеханика]", 0xFFBB00FF3, 2163.264404,-875.610778,27.947052, 5.0);
    CreateDynamic3DTextLabel("Выход из автосервиса\n[Работа автомеханика]", 0xFFBB00FF3, 993.221069, 1000.334350, 1501.000000, 5.0);
    CreateDynamic3DTextLabel("{FFD700}Таксист\n{FFFFFF}Подойдите для взаимодействия", -1, 1792.450927,2529.986328,14.550534, 5.0);
    CreateActor(71, 1792.450927,2529.986328,14.550534,218.800354);
    CreateDynamic3DTextLabel("{FFD700}Таксист\n{FFFFFF}Подойдите для взаимодействия", -1, -2685.532958,1986.828491,9.985312, 5.0);
    CreateActor(71, -2685.532958,1986.828491,9.985312,355.286895);
    CreateDynamic3DTextLabel("{FFD700}Таксист\n{FFFFFF}Подойдите для взаимодействия", -1, -2428.550537,186.391296,26.135101, 5.0);
    CreateActor(71, -2428.550537,186.391296,26.135101,354.70779);
...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
   ...
   if(areaid == AvtoSMI)
   {
    ...
    if(areaid == player_gps_zone[playerid][1])
    {
            ...
            if(areaid == Taxist5)
    {
        TaxiSystemDlg(playerid);
            ...
            if(areaid == TaxistYouzh)
    {
        TaxiSystemDlg(playerid);
    }
    if(areaid == vihod_mex)
    {
        SetPlayerPosEx(playerid, 2160.697998,-875.649536,27.947052,88.845024, 0, 0);
...
if(areaid == police1)
	...
	new vehicleid = GetPlayerVehicleID(playerid);
        if(vehicleid != 0)
        {
            SetVehicleSpeed(vehicleid, 30.0); // Устанавливаем скорость в 30 м/с

            // Показываем уведомление игроку
            ShowNotification(playerid, 3, "Вы въехали в зону поста", 5, "", "");
        }
	}
	if(areaid == police2)
	{
    ...
    {
        //ShowNewNotification(playerid, 5, 5, 1, 10, "Введите /mechanic", "Ок");
        callcmd::mechanic(playerid);
    }
    if(areaid == sphere_item_mex)
    {
        ...
        //ShowNewNotification(playerid, 5, 5, 1, 10, "Введите /components", "Ок");
        callcmd::components(playerid);
    } 
        ...
        if(response)
        {
            ...
            switch(listitem)
            {
                ...
                case 1:
                {
                   ...
                   if(id >= MAX_MEX_VEH) return SCM(playerid, -1, ""USC"Максимальное количество авто в автосервисе");
                ...
                case 3:
                {
					...
					{
						new money = (random(55000)*2) + 12000 * GetMexAuto(id_veh);

						GivePlayerMoneyEx(playerid, money);
						SCM(playerid, -1, ""SC"Вы успешно обслужили транспорт. Зарплата получена!");
						DeleteVehicleMex(playerid);	
					}
                    player_veh_id{playerid} = -1;
                    p_load_active{playerid} = false;
	...
	if(dialogid == 44559)
        {
            if(response)
                {
				    InvitePlayer(playerid, Fraction[playerid], 1, true);
				
				ShowNotificationNew(playerid, 1, 6, 0, 0, "Поздравляем с трудоустройством!", "");
                    return 1;
                }
                else
                {
...
CMD:mechanic(playerid)
{
    ...
    (
        playerid, 1299, DSL, 
        "{FFFF00}СТО {FFFFFF}| Меню автосервиса",
...
CMD:components(playerid)
{
    ...
    (
        ...
        ""CN"Машинное масло\n"\
        ""CN"Свеча зажигания\n"\
        ""CN"Тормозная колодка\n"\
        ""CN"Ремень безопасности\n"\
        ""CN"Аккумулятор",
        "Взять", "Выйти"
    );
...
stock TaxiSystemDlg(playerid)
    {
        ShowPlayerDialog(playerid, 9664, DIALOG_STYLE_MSGBOX,
            "{d4544b}Таксист{FFFFFF} | Разговор",
            "{FFD700}Привет, дрижище! {FFFFFF}Вижу ты тут новенький?\nКороче, у нас тут большая область, {FFD700}безграничные возможности.\n{FFFFFF}Тебе будет трудно добираться до важных мест на каком-то скутере!\nДавай я тебя подкину? Ты только скажи куда!",
            "Давай", "Позже");
        return 1;
    }

stock ShowTaxiTP(playerid)
{
    ShowPlayerDialog(playerid, 9665, DIALOG_STYLE_LIST, 
                    "{FFFFFF}Таксист | {D4544B}Выберите место", 
                    "Авто-салоны\nМагазины одежды\nРаботы области", 
                    "Выбрать", "Отмена");
    return 1;
}

stock ShowDialogAvto(playerid)
{
      new string[256];
    format(string, sizeof(string),
        "Здравствуйте {FA8072}%s!\n\
        {FFFFFF}Что, хотите к нам устроится?\n\
        {FFFFFF}А у нас как всегда сотрудников не хватает...\n\
        Можем принять тебя к себе! {FA8072}Согласен?", 
        GetPlayerNameEx(playerid));

    ShowPlayerDialog(playerid, 44559, DIALOG_STYLE_MSGBOX,
        "{FA8072}"SERVER_NAME"{FFFFFF} | Устройство на работу",
        string,
        "Вступить", "Отмена");
    return 1;
} 
