...
{
    SetPlayerData(playerid, P_PASSSERV, 0);
...
if(dialogid == 5401) // переделал Werton
	{
	        if(response)
	        {
	                if(listitem == 0)
	...
	if(dialogid == 5402) // переделал Werton
	{
	        if(response)
	        {
	                if(listitem == 0)
	                ...
	                if(listitem == 1)
	                {
	                        ...
	                        {
	                            SCM(playerid, -1, "Вы администратор, поэтому не были кикнуты с сервера.");
	                        }
							else
							{
	...
	if(dialogid == 5403) // переделал Werton
	{
	        if(response)
	        {
    ...
    if(dialogid == 5671) // переделал Werton
    {
        if(isPasswordInputInProgress[playerid])
        {
            ...
            if(response)
            {
                ...
                {
					SetPlayerData(playerid, P_PASSSERV, 1);
	...
	if(dialogid == 5609) // переделал Werton
	{
        if(response)
        {
				new string[156];
...
{
    if(GetPlayerAdminEx(playerid) < 5) return SendClientMessage(playerid, -1, "{ff2400}| {ffffff}Вам не доступна данная команда!");
