...
public: LoadGarages()
{
 ...
 CreatePickup(1318, 23, 1.2011, 1994.7356, 1554.2031, -1, PICKUP_ACTION_TYPE_GARAGE_EXIT, 0);
 CreatePickup(1318, 23, 492.912048,1991.464599,1547.679687, -1, PICKUP_ACTION_TYPE_GARAGE_EXIT, 0);
...
if(newkeys == KEY_CROUCH)
{
    if(IsPlayerInRangeOfPoint(playerid, 5.0, -0.2940, 2006.1188, 1554.2031))
    {
        ...
        if(vehicleid == INVALID_VEHICLE_ID || !IsValidVehicle(vehicleid))
            return SendClientMessage(playerid, -1, "У вас нет машины в гараже");

      
        new Float:x = GetGarageData(garageid, GARAGE_EXIT_X);
        new Float:y = GetGarageData(garageid, GARAGE_EXIT_Y);
        new Float:z = GetGarageData(garageid, GARAGE_EXIT_Z);
        new Float:a = GetGarageData(garageid, GARAGE_EXIT_ANGLE);
        ...
        SendClientMessage(playerid, -1, "Ваша машина выехала из гаража");
        return 1;
    }
}
   if(newkeys == KEY_CROUCH)
...
stock PutVehicleDirectlyInGarage(vehicleid, garageid)
{
    ...
    if(GetGarageData(garageid, GARAGE_IMPROVEMENTS) == 2)
    {
        new slot = FindFreeGarageSlot(garageid);
        if(slot == -1) return 0;
...
{
    SetVehiclePos(vehicleid, -1.121805, 2000.269042, 1554.243164);
    SetVehicleZAngle(vehicleid, 344.905944);
}
...
stock BuyPlayerGarage(playerid, garageid, bool: buy_from_owner = false, price = -1)
{
    ...
    {
        cache_delete(result);
        SendClientMessage(playerid, 0xFF6600FF, "Ошибка БД при сохранении гаража (владелец)");
        return 0;
    }
    cache_delete(result);
    ...
    if(mysql_errno(mysql))
    {
        ...
        mysql_format(mysql, query, sizeof query,
            ...
            db_id, account_id);
        mysql_query(mysql, query, false);
        SendClientMessage(playerid, 0xFF6600FF, "Ошибка БД при сохранении гаража (аккаунт)");
        return 0;
    }
    cache_delete(result);
