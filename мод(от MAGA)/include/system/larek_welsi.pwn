...
public CREATE_LAREK()
{
    new Cache:acc_cache = mysql_query(mysql, "SHOW COLUMNS FROM `accounts` LIKE 'satiety'", true);
    if(mysql_errno(mysql))
    {
        printf("[LAREK] SHOW COLUMNS accounts.satiety error: %d", mysql_errno(mysql));
        if(acc_cache) cache_delete(acc_cache);
        return 1;
    }
    ...
    if(cache_num_rows() == 0)
    {
        ...
        if(mysql_errno(mysql)) printf("[LAREK] ALTER TABLE error: %d", mysql_errno(mysql));
    }
    if(acc_cache) cache_delete(acc_cache);
    ...
    if(mysql_errno(mysql))
    {
        printf("[LAREK] SELECT business error: %d", mysql_errno(mysql));
    ...
    if(rows == 0) 
    {
        ...
        for(new i, s = sizeof larek_default; i < s; i++) 
        {
            ...
            if(mysql_errno(mysql)) 
            {
                printf("[LAREK] ОШИБКА создания ларька #%d: MySQL error %d", i, mysql_errno(mysql));
            }
            else
            {
