...
public: LoadGarages()
{
 ...
 rows = cache_num_rows();
    ...
    {
        printf("[WERTON_GARAGES]: Ошибка при загрузке гаражей из БД: %d", mysql_errno(mysql));
        return 0;
    }
...
#if !USE_ZOMENO_OLD_ENGINE
		    ShowNotificationBR(playerid, 3, "Улучшение было куплено!", 3, "", "");
		    ShowNotificationBR(playerid, 0, "Вы потратили 5000000 рублей", 3, "", "");
...
CMD:garage(playerid, params[])
{
	...
	if(garageid != -1)
	{
		new fmt_str[1024];
		new city[MAX_ZONES_NAME + 1], area[MAX_ZONES_NAME + 1];
		GetCityName(GetGarageData(garageid, GARAGE_X), GetGarageData(garageid, GARAGE_Y), city);
		GetAreaName(GetGarageData(garageid, GARAGE_X), GetGarageData(garageid, GARAGE_Y), area);
		format
		(
