...
public:CREATE_ACCOUNTS_TABLE()
{
    ...
    cache = mysql_query(mysql, "SHOW COLUMNS FROM `accounts` LIKE 'family_id'", true);
    if(cache_num_rows() == 0)
    {
        mysql_query(mysql, "ALTER TABLE `accounts` ADD `family_id` INT NOT NULL DEFAULT '-1' AFTER `admin`", false);
        mysql_query(mysql, "ALTER TABLE `accounts` ADD `family_rang` INT NOT NULL DEFAULT '1' AFTER `family_id`", false);
        mysql_query(mysql, "ALTER TABLE `accounts` ADD `family_mute` INT NOT NULL DEFAULT '0' AFTER `family_rang`", false);
        mysql_query(mysql, "ALTER TABLE `accounts` ADD `family_access` VARCHAR(24) NOT NULL DEFAULT '0,0,0,0,0,0,0' AFTER `family_mute`", false);
    }
    cache_delete(cache);
...
public: welsi_LoadFamily()
{
    ...
    if(mysql_errno(mysql))
        return printf("[FAMILY SQL #1] LoadFamily %d", mysql_errno(mysql));
    ...
    for(new i; i < MAX_FAM;i++)
    {
        ...
        if(cache_num_rows())
        {
            ...
            printf("%d | people = %d car = %d", mysql_errno(mysql), count_people, count_auto);
        }
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
        ...
        if(response)
        {
            switch(listitem)
            {
                ...
                case 1:
                {
                    ...
                    Dialog(playerid, -1, DIALOG_STYLE_MSGBOX, "{FF0000}Информация", string, "Выйти", "");
                }
                case 2:
                {
                ...
                case 8:
                {
                    ...
                    (
                        ...
                        "3. Взаимодействие с деньгами сейфа\n"\
                        "4. Взаимодействие с материалами сейфа\n"\
                        "5. Взаимодействие с масками сейфа\n"\
                        "6. Взаимодействие с аптечкам сейфа\n"\
                        "7. Взаимодействие с оружием сейфа\n"\
                        "8. Взаимодействие с бронежилетами сейфа\n"\
                ...
                case 9:
                {
                    ...
                    (
                        ...
                        "Если вы лидер то семья - удалиться.",
                        "Далее", "Назад"
                    );
                }
            }
    ...
    if(dialogid == DIALOG_FAMILY_CARS)
    {
        if(response)
        {
...
if (dialogid == 2222) {
    if (response) {
        ...
        mysql_format(mysql, sql_query, sizeof(sql_query), "SELECT * FROM family_cars WHERE family_owner='%d'", GetFamily(f, family_database));
        result = mysql_query(mysql, sql_query, false);

        new rows = cache_num_rows();

        format(fmt_text, sizeof(fmt_text), "");

        new query[256], model_id, status;

        for (new i = 0, p = 0; i < rows; i++, p++) {
            ...
            if (strlen(fmt_text) + strlen(query) < sizeof(fmt_text)) {
                strcat(fmt_text, query);
                SetPlayerListitemValue(playerid, p, id);
            } else {
                printf("Error: fmt_text buffer overflow!\n");
                break;
            }
        }
            ...
            switch(listitem)
            {
                case 0:
                {
                    if(GetPlayerFamily(playerid, RANG_FAMILY) >= 4)
                    {
                        ...
                        format(f_string144, sizeof f_string144, "%s %s сейф", GetPlayerNameEx(playerid), status ? "закрыл" : "открыл");
                        SendFamilyMessage(f, f_string144);
                        callcmd::famsklad(playerid);
                    }
                    else SendClientMessage(playerid,-1,""SC"Закрывать/открывать сейф доступно только заместителю или лидеру семьи");
                }
                case 1:
                {
                    if(!GetFamily(f, family_status_storage) && GetPlayerFamily(playerid, RANG_FAMILY) < 5)
                        return ShowNotificationMisters(playerid,  2, 6, 0, 0, "Сейф закрыт", "qq");
                ...
                case 2:
                {
                    ...
                    if(!take_or_put)
                    {
                        ...
                        if(money <= GetFamily(f, family_money))
                        {
                            ...
                            SendClientMessage(playerid, -1, ""c_f_g"Вы взяли из сейфа семьи деньги.");
                            format(f_string144, sizeof f_string144, "%s взял из сейфа %d рублей.", GetPlayerNameEx(playerid), money);
                            SendFamilyMessage(f, f_string144);
                        ...
                        else SendClientMessage(playerid, -1, ""c_f_r"У семьи нет такого количества денег.");
                    }
                    else
                    {
                        ...
                        if(money <= GetPlayerMoneyEx(playerid))
                        {
                            AddFamily(f, family_money, +, money);
                            GivePlayerMoneyEx(playerid, -money);
                            SendClientMessage(playerid, -1, ""c_f_g"Вы положили в сейф семьи деньги.");
                            format(f_string144, sizeof f_string144, "%s положил в сейф %d рублей.", GetPlayerNameEx(playerid), money);
                            SendFamilyMessage(f, f_string144);
                            UpdateColumnFamilyInt(GetFamily(f, family_database), "money", GetFamily(f, family_money));
                            SendFamilyLog(type_log_money, playerid, -1, f_string144);
                        }
                        else SendClientMessage(playerid, -1, ""c_f_r"У вас нет такого количества денег.");
                    }
                }
                case 3:
                {
                    ...
                    if(!take_or_put)
                    {
                        ...
                        { 
                            ...
                            SendClientMessage(playerid, -1, ""c_f_g"Вы взяли из сейфа семьи материалы.");
                            format(f_string144, sizeof f_string144, "%s взял из сейфа %d шт. материалов.", GetPlayerNameEx(playerid), material);
                            SendFamilyMessage(f, f_string144);
                        ...
                        else SendClientMessage(playerid, -1, ""c_f_r"У семьи нет такого количества материалов.");
                    }
                    else
                    {
                        ...
                        if(material <= GetPlayerData(playerid, P_METALL))
                        {
                            ...
                            SendClientMessage(playerid, -1, ""c_f_g"Вы положили в сейф семьи материалы.");
                            format(f_string144, sizeof f_string144, "%s положил в сейф %d шт. материалов.", GetPlayerNameEx(playerid), material);
                            SendFamilyMessage(f, f_string144);
                            UpdateColumnFamilyInt(GetFamily(f, family_database), "material", GetFamily(f, family_material));
                            SendFamilyLog(type_log_material, playerid, -1, f_string144);
                        }
                        else SendClientMessage(playerid, -1, ""c_f_r"У вас нет такого количества материалов.");
                    }
                }
                case 4:
                {
                    if(!take_or_put)
                    {
                        if(GetFamily(f, family_material))
                        {
                            ...
                            SendClientMessage(playerid, -1, ""c_f_g"Вы взяли из сейфа семьи маску.");
                            format(f_string144, sizeof f_string144, "%s взял из сейфа маску.", GetPlayerNameEx(playerid));
                            SendFamilyMessage(f, f_string144);
                        ...
                        else SendClientMessage(playerid, -1, ""c_f_r"У семьи нет маски.");
                    }
                    else
                    {
                        ...
                        if(GetPlayerData(playerid, P_MASK))
                        {
                            ...
                            SendClientMessage(playerid, -1, ""c_f_g"Вы положили в сейф семьи маску.");
                            format(f_string144, sizeof f_string144, "%s положил в сейф маску.", GetPlayerNameEx(playerid));
                            SendFamilyMessage(f, f_string144);
                            UpdateColumnFamilyInt(GetFamily(f, family_database), "mask", GetFamily(f, family_mask));
                            SendFamilyLog(type_log_mask, playerid, -1, f_string144);
                        }
                        else SendClientMessage(playerid, -1, ""c_f_r"У вас нет маски.");
                    }
                }
                case 5:
                {
                        ...
                        switch(listitem)
                        {
                            case 0:
                            {
                                ...
                                (
                                    ...
                                    "{FF0000}Информация",
                                    "На первом уровне улучшения оружия в сейфе доступен {FF6347}Desert Eagle\n"\
                                    "{FFFFFF}На втором уровне {FF6347}AK-47\n"\
                                    "{FFFFFF}На третьем уровне {FF6347}MP5\n"\
                                    "{FFFFFF}На четветром уровне {FF6347}Shotgun\n"\
                                    "{FFFFFF}На пятом уровне {FF6347}Country Rifle\n"\
                                    "{FFFFFF}На шестом уровне {FF6347}Sniper Rifle\n"\
                                    "{FFFFFF}На седьмом уровне {FF6347}M4",
                                    "Выйти", ""
                                );
                            }
                            case 1:
                        ...
                        switch(listitem)
                        {
                            ...
                            case 1:
                            {
                                ...
                                (
                                    ...
                                    "Введите кол-во патронов которые хотите положить в сейф",
                                    "Далее", "Назад"
                                );
                            }
                        }
                ...
                case 6:
                {
                    if(!take_or_put)
                    {
                        if(GetFamily(f, family_armour))
                        {
                            ...
                            SendClientMessage(playerid, -1, ""c_f_g"Вы взяли из сейфа семьи бронежилет.");
                            format(f_string144, sizeof f_string144, "%s взял из сейфа бронежилет.", GetPlayerNameEx(playerid));
                            SendFamilyMessage(f, f_string144);
                    ...
                    else
                    {
                        ...
                        SendClientMessage(playerid, -1, ""c_f_g"Вы положили в сейф семьи бронежилет.");
                        format(f_string144, sizeof f_string144, "%s положил в сейф бронежилет.", GetPlayerNameEx(playerid));
                        SendFamilyMessage(f, f_string144);
                        UpdateColumnFamilyInt(GetFamily(f, family_database), "armour", GetFamily(f, family_armour));
                        SendFamilyLog(type_log_armour, playerid, -1, f_string144);
                    }
                ...
                case 7:
                {
                    ...
                    if(!take_or_put)
                    {
                        if(GetFamily(f, family_heath_kit))
                        {
                            ...
                            SendClientMessage(playerid, -1, ""c_f_g"Вы взяли из сейфа семьи аптечку.");
                            format(f_string144, sizeof f_string144, "%s взял из сейфа аптечку.", GetPlayerNameEx(playerid));
                            SendFamilyMessage(f, f_string144);
                    ...
                    else
                    {
                        ...
                        if(GetPlayerData(playerid, P_MED_CHEST))
                        {
                            ...
                            SendClientMessage(playerid, -1, ""c_f_g"Вы положили в сейф семьи аптечку.");
                            format(f_string144, sizeof f_string144, "%s положил в сейф аптечку.", GetPlayerNameEx(playerid));
                            SendFamilyMessage(f, f_string144);
                            UpdateColumnFamilyInt(GetFamily(f, family_database), "heath_kit", GetFamily(f, family_heath_kit));
                            SendFamilyLog(type_log_heath_kit, playerid, -1, f_string144);
                        }
        ...
        if(response)
        {
            if(GetPVarInt(playerid, "select_weapon"))
            {
                ...
                if(GetFamily(f, family_patron) >= patron)
                {
                    ...
                    format(f_string144, sizeof f_string144, "%s взял из сейфа %s и %d патрон.", GetPlayerNameEx(playerid), name_weapon_family[id], patron);
                    SendFamilyMessage(f, f_string144);
            ...
            switch(weapon)
            {
                ...
                case 24, 30, 29, 25, 33, 34, 31:
                {
                    ...
                    if(ammo >= patron)
                    {
                        ...
                        SendClientMessage(playerid, -1, ""c_f_g"Вы положили в сейф семьи патроны.");
                        format(f_string144, sizeof f_string144, "%s положил в сейф %d патрон.", GetPlayerNameEx(playerid), patron);
                        SendFamilyMessage(f, f_string144);
                        UpdateColumnFamilyInt(GetFamily(f, family_database), "patron", GetFamily(f, family_patron));
                    }
                    else SendClientMessage(playerid, -1, ""c_f_g"У вас нету такого кол-во патрон.");
                }
                default:return SendClientMessage(playerid, -1, ""c_f_r"Ваше оружие не подходит чтобы положить патроны в сейф");
            }
        }
    }
    if(dialogid == 2228)
            ...
            if(GetPVarInt(playerid, "input_ad_family"))
            {
                ...
                else
                {
                    ...
                    else
                    {
                       ...
                       if(!mysql_errno(mysql)) FSendClientMessage(f_string144, playerid, -1, ""c_f_g"Вы успешно создали объявление: {FF6347}%s", inputtext);
                    }
                    cache_delete(cache);
                }
    ...
    if(dialogid == 2229)
    {
        if(response)
        {
            ...
            if(cache_num_rows())
            {
                ...
                "Создатель объявления: {FF6347}%s{FFFFFF}\n\n"\
                "Время: {FF6347}%s{FFFFFF}", ad_text, create_name, time_data
                );
    ...
    if(dialogid == 2230)
    {
        if(GetPVarInt(playerid, "select_get_dostup"))
        {
            ...
            if(!IsPlayerConnected(to_player))
                return ShowNotificationMisters(playerid,  2, 6, 0, 0, "Такого игрока нет", "qq");
        ...
        if(response)
        {
            if(GetPVarInt(playerid, "type_up_family"))
            {
                switch(GetPVarInt(playerid, "type_up_family"))
                {
                    case 1:
                    {
                        ...
                        else FSendClientMessage(f_string144, playerid, -1, ""c_f_r"в сейфе не хватает %d.", price_up_sklad[GetFamily(f, family_lvl_storage)]-GetFamily(f, family_money));
                    }
                    case 2:
                    {
                        ...
                        else FSendClientMessage(f_string144, playerid, -1, ""c_f_r"в сейфе не хватает %d.", price_up_sklad[GetFamily(f, family_lvl_weapon)]-GetFamily(f, family_money));
                    }
                    case 3:
                    {
                        ...
                        else FSendClientMessage(f_string144, playerid, -1, ""c_f_r"в сейфе не хватает %d.", price_up_sklad[GetFamily(f, family_lvl_compound)]-GetFamily(f, family_money));
                    }
                }
            ...
            switch(listitem)
            {
                case 0:
                {
                    ...
                    "На %s уровне сейфа:\n"\
                    "Максимально денег с %d до {FF6347}%d{FFFFFF}\n"\
                    "Максимально материалов с %d до {FF6347}%d{FFFFFF}\n"\
                    "Максимально аптечек с %d до {FF6347}%d{FFFFFF}\n"\
                    "Максимально масок с %d до {FF6347}%d{FFFFFF}\n"\
                    "Максимально бронежилетов с %d до {FF6347}%d{FFFFFF}\n\n"\
                    "Вы уверены что хотите улучшить склад за %d рублей?\n"\
                    "Сумма должна лежать в сейфе семьи.",
                ...
                case 1:
                {
                    ...
                    format(level_up_text, sizeof level_up_text,
                    "На %s уровне оружия:\n"\
                    "Максимально патронов с %d до {FF6347}%d{FFFFFF}\n"\
                    "Новое оружие {FF6347}%s{FFFFFF}\n\n"\
                    "Вы уверены что хотите улучшить склад за %d рублей?\n"\
                    "Сумма должна лежать в сейфе семьи.",
                ...
                case 2:
                {
                    ...
                    format(level_up_text, sizeof level_up_text,
                    "На %s уровне состава:\n"\
                    "Максимально человек с %d до {FF6347}%d{FFFFFF}\n\n"\
                    "Вы уверены что хотите улучшить склад за %d рублей?\n"\
                    "Сумма должна лежать в сейфе семьи.",
    ...
    if(dialogid == 2235)
    {
        if(response)
        {
            ...
            if(GetPlayerRangFamily(playerid) != 5)
            {
                format(f_string144, sizeof f_string144, "%s игрок %s покинул семью. Причина: С/Ж", family_rang_name[GetPlayerIdFamily(playerid)][GetPlayerFamily(playerid, RANG_FAMILY)-1], GetPlayerNameEx(playerid));
                SendFamilyMessage(f, f_string144);
            ...
            else
            {
                foreach(new i : Player)
                {
                    ...
                    {
                        ExitFromFamily(i);
                        SendClientMessage(i, -1, "Лидер покинул семью. Семья удалена.");
                    }                    
                }
                ...
                if(mysql_errno(mysql)) return printf("ERROR SQL (%s)", f_string144);
                ...
                if(mysql_errno(mysql)) return printf("ERROR SQL (%s)", f_string64);
    ...
    if(dialogid == 2237)
    {
        ...
        if(!type)
        {
            ...
            if(!c) return ShowNotificationMisters(playerid,  2, 6, 0, 0, "Логи не найдены", "qq");
        ...
        if(listitem == up || listitem == down)
        {
            ...
            if(!rows) return ShowNotificationMisters(playerid,  2, 6, 0, 0, "Логи не найдены", "qq");
        ...
        if(response)
        {
            ...
            if(GetPlayerMoneyEx(playerid) >= 3_000_000)
            {
                ...
                if(!mysql_errno(mysql))
                {
                    ...
                    format(query, sizeof query, "{FF6347}| {FFFFFF}Вы успешно создали семью: {FF6347}\"%s\" ", fam_name);
                    SendClientMessage(playerid, 0x1E90FFFF, query);
                        
                    format(query, sizeof query, "{FF6347}| {FFFFFF}Для управление семьей используйте: {FF6347}/fm");
                    SendClientMessage(playerid, 0x1E90FFFF, query);
                }
        ...
        if(response)
        {
            if(GetPlayerDonateRub(playerid) >= 100)
            {
                ...
                if(!IsPlayerInRangeOfPlayer(playerid, to_player, 5.0))
                    return ShowNotificationMisters(playerid,  2, 6, 0, 0, "Игрок слишком далеко", "qq");

                format(f_string144, sizeof f_string144, "{FF6347}%s{FFFFFF}хочет передать полномочия семьи %s Вам.",
    ...
    if(dialogid == 2242)
    {
        ...
        FSendClientMessage(f_string144, player_offer, -1, ""c_f_g"Вы передали полномочия вашей семье игроку {FF6347}%s", GetPlayerNameEx(playerid));
        FSendClientMessage(f_string144, playerid, -1, ""c_f_g"Вы получили полномочия семьи игрока {FF6347}%s", GetPlayerNameEx(player_offer));
    }
    if(dialogid == 2243)
    {
        if(response)
            ...
            switch(listitem)
            {
                case 0:
                {
                    ...
                    (
                        ...
                        "Получать репутацию можно в сражениях за {FF6347}разные предприятия {FFFFFF}и\n"\
                        "{FF6347}семейных контейнерах.\n"\
                        "{FFFFFF}Конец сезона семьи происходит в {FF6347}00:00 Четверга.\n\n"\
                        "{FFFFFF}Награда за {FF6347}первое место{FFFFFF} в топе семей:\n"\
                        "90.000 рублей и 40 донат-рублей каждому игроку из семьи\n"\
                        "Два слота для транспорта в семью\n\n"\
                        "{FFFFFF}Награда за {FF6347}второе место{FFFFFF} в топе семей:\n"\
                        "60.000 рублей и 30 донат-рублей каждому игроку из семьи\n"\
                        "Один слот для транспорта в семью\n\n"\
                        "{FFFFFF}Награда за {FF6347}третье место{FFFFFF} в топе семей:\n"\
                ...
                case 1:
                {
                    ...
                    if(mysql_errno(mysql)) return print("[COLLECTOR_SYSTEM] Error Sql Query: №1");
                    ...
                    format(list, sizeof list, "{FF6347}- {FFFFFF}№%d %s %d", mesto, name_pl, rep_pl);
                    strcat(dialog, list);
...
stock MoveOwnableCar(playerid, database_car, fam)
{
    ...
    if(!mysql_errno(mysql)) mysql_query(mysql, f_string64, false);
...
stock LoadFamilyCar(database)
{
    ...
    {
        format(f_string64, sizeof f_string64, "%sСемья: {ffffff}%s", family_type_color[GetFamily(f_server, family_color)], GetFamily(f_server, family_name));
    ...
    switch(type)
    {
        case 1:
        {
            if(!take_or_put)
            {
                ...
                (
                    ...
                    "{FF0000}Взять предмет из сейфа",
                    "{FF6347}|{FFFFFF} Деньги\n"\
                    "{FF6347}|{FFFFFF} Материалы\n"\
                    "{FF6347}|{FFFFFF} Маски\n"\
                    "{FF6347}|{FFFFFF} Оружие\n"\
                    "{FF6347}|{FFFFFF} Бронежилет\n"\
                    "{FF6347}|{FFFFFF} Аптечка",
                    "Далее", "Назад"
                );
                ...
                (
                    playerid, 2225, DIALOG_STYLE_LIST,
                    "{FF0000}Положить предмет в сейф",
                    "{FF6347}|{FFFFFF} Деньги\n"\
                    "{FF6347}|{FFFFFF} Материалы\n"\
                    "{FF6347}|{FFFFFF} Маски\n"\
                    "{FF6347}|{FFFFFF} Оружие\n"\
        ...
        case 3:
        {
            if(!take_or_put)
            {
                ...
                (
                    playerid, 2226, DIALOG_STYLE_INPUT,
                    "{FF0000}Взять материалы из сейфа",
                    f_string144,
                    "Взять", "Назад"
                );
            }
            else
            {
                ...
                (
                    playerid, 2226, DIALOG_STYLE_INPUT,
                    "{FF0000}Положить материалы в сейф",
                    f_string144,
                    "Положить", "Назад"
                );
            }
        ...
        case 4:
        {
            if(!take_or_put)
            {
                ...
                (
                    ...
                    "{FF0000}Взять маску",
                    "Вы хотите взять маску из сейфа?",
                    "Взять", "Назад"
                );
            }
            else
            {
                ...
                (
                    ...
                    "Вы хотите положить маску в сейф?",
                    "Положить", "Назад"
                );
            }
        ...
        case 6:
        {
            if(!take_or_put)
            {
                ...
                (
                    ...
                    "Вы хотите взять бронежилет из сейфа?",
                    "Взять", "Назад"
                );
            }
            else
            {
                ...
                (
                    ...
                    "Вы хотите положить бронежилет в сейф?",
                    "Положить", "Назад"
                );
            }
        ...
        case 7:
        {
            if(!take_or_put)
            {
                ...
                (
                    ...
                    "Вы хотите взять аптечку из сейфа?",
                    "Взять", "Назад"
                );
            }
            else
            {
                ...
                (
                    ...
                    "Вы хотите положить аптечку в сейф?",
                    "Положить", "Назад"
                );
            }
...
CMD:famsklad(playerid)
{
    ...
    if(GetPlayerData(playerid, P_LAST_MP) == 1) return ShowNotificationMisters(playerid,  2, 6, 0, 0, "Вы не можете использовать склад на мероприятии", "qq");

    format(f_string144, sizeof f_string144,
    "| Сейф: %s\n1. Взять с сейфа\n2. Положить в сейф\n3. Доступы", GetFamily(GetPlayerIdFamily(playerid), family_status_storage) ? ("{FF0000}Закрыт") : ("{00FF00}Открыт"));

    Dialog(playerid, 2223, DIALOG_STYLE_LIST, "{FF0000}Сейф", f_string144, "Далее", "Выйти");
    return 1;
}
...
CMD:faminvite(playerid, params[])
{
    ...
    format(f_string144, sizeof f_string144, "{FF6347}%s{FFFFFF} приглашает вас в семью %s",
    GetPlayerNameEx(playerid), GetFamily(f, family_name));
...
stock SendFamilyLog(type, playerid, to_player = -1, text[])
{
    ...
    if(mysql_errno(mysql)) return printf("error insert family_log");
...
cmd:info(playerid)
{
    ...
    ShowPlayerDialog(playerid, -1, DIALOG_STYLE_MSGBOX, "", string, "Закрыть", "");
    return 1;
}
...
stock ShowMembersFamily(playerid, listitem)
{
    ...
    if(mysql_errno(mysql)) return SendClientMessage(playerid, -1, "error");
...
cmd:buyslotfam(playerid)
{
    ...
    (
        ...
        "в вашу семью? Стоимость слота - {FF6347}300 донат-руб.",
        "Далее", "Выйти"
    );
    return 1;
}
...
cmd:exchangefam(playerid)
{
    ...
    (
        ...
        "Стоимость: {FF6347} 100 донат-руб.",
        "Далее", "Выйти"
    );
    return 1;
}
...
cmd:buynamefam(playerid)
{
    ...
    if(GetPlayerDonateRub(playerid) >= 100)
    {
        ...
        (
            ...
            "Напишите название вашей семьи ниже.\n"\
            "Стоимость: {FF6347}100 донат-рублей",
            "Далее", "Выйти"
        );
    }
    else FSendClientMessage(f_string144, playerid, -1, ""c_f_r"У вас не хватает %d донат-рублей", 100-GetPlayerDonateRub(playerid));
...
CMD:famcars(playerid)
{
    ...
    mysql_format(mysql, fmt_text, sizeof fmt_text, "SELECT * FROM family_cars WHERE family_owner = %d", GetPlayerFamily(playerid, ID_SQL_FAMILY));
    new Cache:cache = mysql_query(mysql, fmt_text);

    new rows = cache_num_rows();

    if(!rows)
        ShowNotificationMisters(playerid,  2, 6, 0, 0, "У Вашей семьи нет транспорта", "qq");

    else
    {
        new query[256];

		new model_id, id, status;
        ...
        Dialog
        (
            playerid, DIALOG_FAMILY_CARS, DIALOG_STYLE_LIST,
            "{FF0000}Выберите транспорт",
            fmt_text,
            "Далее", "Закрыть"
        );
...
stock ShowCadr(playerid) {
	if(random(4) == 0) {
        GameTextForPlayer(playerid, "~w~Mod~n~~y~by Mister Odin", 555, 1);
    }
    return 1;
}