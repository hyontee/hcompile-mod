...
{
    new Cache:cache = mysql_query(mysql, "SHOW COLUMNS FROM `accounts` LIKE 'weekly_prizes'", true);

    if(cache_num_rows() == 0)
    {
        mysql_query(mysql, "ALTER TABLE `accounts` ADD `weekly_prizes` TEXT NOT NULL DEFAULT '0,0,0,0,0,0,0' AFTER `level`", false);
        mysql_query(mysql, "ALTER TABLE `accounts` ADD `weekly_day` INT NOT NULL DEFAULT '0' AFTER `weekly_prizes`", false);
        if(!mysql_errno(mysql)) print("[LAIRD_SYSTEM] weekly columns created");
    }
    cache_delete(cache);
...
public:UnLoadWeeklyPrizes(playerid)
{
    ...
    //if(!mysql_errno(mysql)) printf("weekly update");
