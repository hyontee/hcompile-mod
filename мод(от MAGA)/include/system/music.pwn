...
CMD:music(playerid) // уберите /music старый пж
{
    ...
    {
        new title[128];
        format(title, sizeof(title), "{FF5252}%s{ffffff} | Выберите нужное действие", NameServer[DOVARD_NAME]);
...
{
    new title[128];
    format(title, sizeof(title), "{FF5252}%s{ffffff} | Выберите нужную радиостанцию", NameServer[DOVARD_NAME]);
...
{
    new title[128];
    format(title, sizeof(title), "{FF5252}%s{ffffff} | Аудио-система автомобиля", NameServer[DOVARD_NAME]);
