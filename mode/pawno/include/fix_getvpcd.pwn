...
public OnPlayerStateChange(playerid, newstate, oldstate)
{
	...
	if (oldstate == PLAYER_STATE_DRIVER || oldstate == PLAYER_STATE_PASSENGER) {
		...
		if ( seat != -1 )vehicleInfo[vehicleid][e_vi_door][seat] = isclosed ? 0 : 1;
    ...
    { 
 		if ( opev_vehicleid [ playerid ] != GetPlayerVehicleID ( playerid ) || ! used_enter [ playerid ] )return anti_cheat ( playerid, "incar tp", 43 ) ;
...
public OnPlayerEnterVehicle(playerid, vehicleid, ispassenger)
{
	...
	new Float:opev_distance ; 
	opev_distance = GetPlayerDistanceFromPoint ( playerid, veh_x, veh_y, veh_z ) ; 

	if ( opev_distance < 0.35 &&GetVehicleModel ( vehicleid ) != 93 )anti_cheat ( playerid, "tp in car", 95 ) ;
