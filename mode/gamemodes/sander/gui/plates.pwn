    ...
    switch(type)
    {
        ...
        case 2:
        {
            if (GetPlayerMoney(playerid) < 20000)
            {
                new Node:response = JSON_Object();
                JSON_SetInt(response, "t", 2);
                ShowNotificationSander(playerid, 2, 4, 1, 1, "Недостаточно денег", "");
                SendPacketToClient(playerid, 1, response);
                ~response;
                return 0;
            }
            ...
            while (!unique && attempts < maxAttempts)
            {
                ...
                if (!exists)
                {
                    if(country == 0)
                    {
                        ...
                        mysql_format(mysql, query, sizeof query,
                            "SELECT id FROM `player_inventory` WHERE `item_id` = 59 AND `item_plate` = %d AND `is_active` = 0 LIMIT 1",
                            joinpl);
                    }
                    else if(country == 3)
                    {
                        ...
                        JoinPlate(plate, region, joinpl, sizeof(joinpl));
                        mysql_format(mysql, query, sizeof query,
                            "SELECT id FROM `player_inventory` WHERE `item_id` = 83 AND `item_plate` = %d AND `is_active` = 0 LIMIT 1",
                            joinpl);
                    }
                    else if(country == 1)
                    {
                        mysql_format(mysql, query, sizeof query,
                            "SELECT id FROM `player_inventory` WHERE `item_id` = 81 AND `item_plate` = %d AND `is_active` = 0 LIMIT 1",
                            plate);
                    }
                    else if(country == 2)
                    {
                        mysql_format(mysql, query, sizeof query,
                            "SELECT id FROM `player_inventory` WHERE `item_id` = 82 AND `item_plate` = %d AND `is_active` = 0 LIMIT 1",
                            plate);
                    }
                ...
                if (!unique && attempts >= maxAttempts)
                {
                    ...
                    ShowNotificationSander(playerid, 2, 4, 1, 1, "Ошибка генерации номера, попробуйте позже", "");
            ...
            BlackPass_OnPlateRegenerated(playerid);
            return 1;
        }
