...
#define MYSQL_HOST "51.75.44.14"
#define MYSQL_USER "gs110840"
#define MYSQL_PASS "j0694e2d"
#define MYSQL_DB   "gs110840"
...
new RentPlaces[MAX_RENT_PLACES][rent_data];
...
//    FreezePlayer(playerid, false);
	...
	SendClientMessage(playerid, -1, "{66CC66}| {ffffff}Добро пожаловать на {E0584B}BAZE RUSSIA shram!");
    ...
    if(dialogid == ENTER_PROMOCODE)
{
    ...
    format(msg, sizeof(msg),
        ...
        Приятной игры на {E0584B}BAZE RUSSIA!", inputtext);

    ShowPlayerDialog(playerid, PROMO_PRISE, DIALOG_STYLE_MSGBOX, "{E0584B}BAZE RUSSIA{FFFFFF} | Активация промокода", msg, "Закрыть", "");
    return 1;
}
		...
		if(dialogid == DIALOG_RENT_MENU)
{
    ...
    if(listitem == 0)
    {
        new veh = CreateVehicle(462, x + 1.5, y + 1.5, z + 0.5, a, 1, 1, 60000);
        PutPlayerInVehicle(playerid, veh, 0);
        ShowNotification(playerid, 1, 5, 0, 1, "Вы арендовали скутер!", "");
    }
    if(listitem == 1)
    {
        callcmd::car(playerid, "");
        SendClientMessage(playerid, COLOR_WHITE, "Открыто меню управления личным транспортом.");
    }
    return 1;
}
...
stock SavePlayerAccount(playerid)
{
    ...
    new query[256];
    format(query, sizeof(query), "UPDATE `accounts` SET `money` = '%d', `pAdminLvl` = '%d', `level` = '%d', `exp` = '%d' WHERE `id` = '%d'",
        Player[playerid][pMoney],
        Player[playerid][pAdminLvl],
...
CMD:veh(playerid, params[])
{
    ...
    format(str, sizeof(str), "{3399FF}Вы создали транспорт модель %d (ID: %d)", modelid, Player[playerid][pAdminVeh]);
    SendClientMessage(playerid, -1, str);
    return 1;
}
...
CMD:setskin(playerid, params[])
{
    ...
    SavePlayerAccount(targetid);
    format(str, sizeof(str), "{3399FF}Вы установили игроку %s постоянный скин %d.", Player[targetid][pName], skinid);
    SendClientMessage(playerid, -1, str);
...
CMD:skin(playerid, params[])
{
    ...
    if(!IsPlayerConnected(targetid)) return 1;
    SetPlayerSkin(targetid, skinid);
    format(str, sizeof(str), "{3399FF}Вы выдали игроку %s[%d] временный скин %d.", Player[targetid][pName], targetid, skinid);
...
CMD:addarenda(playerid)
{
    if(Player[playerid][pAdminLvl] < 4) return SendClientMessage(playerid, COLOR_RED, "[Ошибка] Вы не уполномочены использовать эту команду.");

    if(TotalRentPlaces >= MAX_RENT_PLACES) return SendClientMessage(playerid, COLOR_RED, "[Ошибка] Достигнут лимит точек аренды.");
    ...
    RentPlaces[id][rPickup] = CreateDynamicPickup(19134, 23, x, y, z, 0, 0, -1, 100.0);
    RentPlaces[id][rText] = CreateDynamic3DTextLabel("{87CEEB}[ Аренда транспорта ]\n{FFFFFF}Встаньте на пикап или нажмите {87CEEB}'ALT'", COLOR_WHITE, x, y, z + 0.5, 15.0, INVALID_PLAYER_ID, INVALID_VEHICLE_ID, 0, 0, -1, -1, 100.0);

    TotalRentPlaces++;

    SendClientMessage(playerid, ADMIN_COLOR, "[Успешно] Вы создали точку аренды транспорта на ваших координатах!");
    return 1;
}
...
CMD:setpackplace(playerid, params[])
{
    if(Player[playerid][pAdminLvl] < 1) return SendClientMessage(playerid, COLOR_RED, "[Ошибка] Вы не уполномочены использовать эту команду.");

    new placeid;
    if(sscanf(params, "d", placeid)) return SendClientMessage(playerid, COLOR_GREY, "Используйте: /setpackplace [1-4]");

    if(placeid < 1 || placeid > 4) return SendClientMessage(playerid, COLOR_RED, "[Ошибка] Неверный ID локации! Доступны только значения от 1 до 4.");
    ...
    new message[128];
    format(message, sizeof(message), "[Телепорт] Вы переместились на спавн-плейс №%d.", placeid);
    SendClientMessage(playerid, ADMIN_COLOR, message);
    return 1;
}
