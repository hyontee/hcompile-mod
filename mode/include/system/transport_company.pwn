...
#define LEVEL_START_TC  1 // Работа транспортной компании доступна с 1 игрового уровня
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    ...
    if(dialogid == 2903)
    {
        if(!response)
        {
            ...
            if(GetPVarInt(playerid, "dostup_order")) { SendClientMessage(playerid, -1, ""tc_msg"Ваш уровень дальнобойщика не позволяет взять этот заказ."); return 1; }


            if(!TC_ReserveFleetSlot(playerid)) return 1;
...
public:LoadTrailerTC(playerid)
{
    ...
    switch(player_tc[playerid][p_TC_Progress_Order])
    {
        ...
        case 3:{
            ...
            salary = TC_CompleteDeliveryReward(playerid, salary);
...
stock ShowDialogOrders(playerid, Locality)
{
    ...
    {
        new string[124];
        format(string, sizeof string, ""tc_msg"Выполнять заказы транспортной компании можно с {FFAE00}%d {FFFFFF}уровня", LEVEL_START_TC);
