...
stock BlackPass_GrantRewardDirect(playerid, itemid)
{
    ...
    if(result)
    {
        ...
        ShowNotificationSander(playerid, 3, 5, 0, 0, notify, " ");
    ...
    else
    {
        ...
        ShowNotificationSander(playerid, 2, 5, 0, 0, "Награду не удалось выдать: освободите место и повторите", " ");
    }
    return result;
}
...
{
    return ShowNotificationSander(playerid, 3, 4, 0, 0, "Награды BlackPass выдаются сразу после получения", " ");
}
...
stock BlackPass_ClaimTaskReward(playerid, task_id)
{
    ...
    ShowNotificationSander(playerid, 3, 4, 0, 0, notify_str, "qq");

    if(BlackPass_GetCurrentLevel(playerid) > old_level)
    {
        ShowNotificationSander(playerid, 3, 3, 0, 0, "Вы успешно получили новый уровень SIMPLE PASS.", "qq");
...
stock BlackPass_BeginTrackTask(playerid, task_id)
{
    ...
    ShowNotificationSander(playerid, 3, 3, 0, 0, "Место отмечено у Вас на GPS", "qq");
...
stock BlackPass_HandleTaskExchange(playerid, Node:json)
{
    ...
    {
        ShowNotificationSander(playerid, 2, 3, 0, 0, "Недостаточно BC.", "qq");
        return 1;
    }
    ...
    {
        ShowNotificationSander(playerid, 2, 3, 0, 0, "Сейчас это задание нельзя заменить.", "qq");
        return 1;
    }
    ...
    ShowNotificationSander(playerid, 3, 3, 0, 0, "Вы потратили 10 ВС и {FFFF00}успешно\nсменили задание{FFFF00}{FFFFFF}!", "qq");
...
stock BlackPass_HandleRefreshRating(playerid, Node:json)
{
    ...
    if(g_blackpass_rating_refresh_until[playerid] > current_time)
    {
        ...
        ShowNotificationSander(playerid, 2, 4, 0, 0, "Обновлять рейтинг можно раз в 30 секунд", "qq");
        return 1;
    }
