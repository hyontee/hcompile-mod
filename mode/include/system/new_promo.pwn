...
stock GetPromoCodeDataByID(promoId, promoData[E_PROMO_DATA]);
stock GetPromoCodeDataByString(const promoCode[], promoData[E_PROMO_DATA]);
    ...
    switch(prizeType)
    {
        case PRIZE_TYPE_MONEY:
        {
            ...
            if(newMoney < 0)
            {
                ...
                UpdatePlayerDatabaseInt(playerid, "money", 0);
                                new fmt_promo_fix[256];
                format(fmt_promo_fix, sizeof fmt_promo_fix, "{ff2400}| {ffffff}У вас было отозвано {ff2400}%d${ffffff} за истечение срока действия приза. Ваш баланс установлен на 0$.", prizeValue);
                SendClientMessage(playerid, -1, fmt_promo_fix);
            }
            else
            {
                GivePlayerMoneyEx(playerid, -prizeValue);
                new fmt_promo_2[256];
                format(fmt_promo_2, sizeof fmt_promo_2, "{ff2400}| {ffffff}У вас было отозвано {ff2400}%d${ffffff} за истечение срока действия приза.", prizeValue);
                SendClientMessage(playerid, -1, fmt_promo_2);
            }
        }
        case PRIZE_TYPE_DONATE:
        {
            ...
            if(newDonate < 0)
            {
                ...
                new fmt_promo_3[256];
                format(fmt_promo_3, sizeof fmt_promo_3, "{ff2400}| {ffffff}У вас было отозвано {ff2400}%d доната{ffffff} за истечение срока действия приза. Ваш баланс доната установлен на 0.", prizeValue);
                SendClientMessage(playerid, -1, fmt_promo_3);
            }
            else
            {
                GivePlayerDonateRub(playerid, -prizeValue);
                new fmt_promo_4[256];
                format(fmt_promo_4, sizeof fmt_promo_4, "{ff2400}| {ffffff}У вас было отозвано {ff2400}%d доната{ffffff} за истечение срока действия приза.", prizeValue);
                SendClientMessage(playerid, -1, fmt_promo_4);
            }
        }
        case PRIZE_TYPE_CAR:
        {
