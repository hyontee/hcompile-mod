...
public Auction_DeliverPendingItems(playerid)
{
    ...
    {
        ShowNotificationSander(playerid, 2, 6, 0, 0, "В хранилище аукциона есть предметы. Освободите инвентарь и введите /auctionitems.", "");
    }
    return delivered;
}
...
public Auction_DeliverPendingMoney(playerid)
{
    ...
    for(new row; row < rows; row++)
    {
        ...
        {
            ShowNotificationSander(playerid, 2, 7, -1, -1, "Освободите место на денежном балансе для выплаты аукциона.", "");
            break;
        }
...
CMD:auction(playerid)
{
   ...
   if(!(8 <= hour <= 21)) return SendClientMessage(playerid, -1, ""USC"Аукцион работает с 8:00 до 22:00");
    ...
    if(GetPlayerVirtualWorld(playerid) != AUCTION_WORLD) { EnablePlayerGPS(playerid, 55, 2090.941650,-2283.999511,23.101566, "Местоположение аукциона отмечено на карте."); return 1; }
