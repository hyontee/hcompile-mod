// Garage player state is kept locally so the system does not modify the main player enum.
new g_player_garage[MAX_PLAYERS];
new g_player_in_garage[MAX_PLAYERS];

#if !defined USE_ZOMENO_OLD_ENGINE
    #define USE_ZOMENO_OLD_ENGINE 0
#endif

#if !defined ShowNewNotification
    #define ShowNewNotification ShowNotificationSile
#endif
// ============================================
...
#define GetPlayerInGarage(%0)	g_player_in_garage[%0]                    // в какой гараж вошел\находится
#define SetPlayerInGarage(%0,%1)				g_player_in_garage[%0] = %1		// установить гараж в котором находится
...
{
	new garageid = g_player_garage[playerid];
	if(garageid != -1)
	{
...
{
	g_player_garage[playerid] = -1;
	g_player_in_garage[playerid] = -1;
	new query[138];
	new Cache: result;
    ...
    if (cache_num_rows() > 0)
    {
        ...
        if (garage_id > 0)
        {
			...
			g_player_garage[playerid] = garage_id;
...
{
    // Р°РІС‚РѕРјР°С‚РёС‡РµСЃРєРёР№ РІС‹С…РѕРґ РёР· РіР°СЂР°Р¶Р° РїРѕ H.
    if((newkeys & KEY_CROUCH) && GetPlayerInGarage(playerid) != -1)
    {
        new in_garage = GetPlayerInGarage(playerid);
        new bool:near_exit = false;

        if(GetGarageData(in_garage, GARAGE_IMPROVEMENTS) >= 2)
            near_exit = IsPlayerInRangeOfPoint(playerid, 4.0, 492.912048, 1991.464599, 1547.679687);
        else
            near_exit = IsPlayerInRangeOfPoint(playerid, 4.0, 1.2011, 1994.7356, 1554.2031);

        if(near_exit)
        {
            new vehicleid = GetPlayerOwnableCar(playerid);
            if(vehicleid != INVALID_VEHICLE_ID && IsPlayerInVehicle(playerid, vehicleid) && GetPlayerState(playerid) == PLAYER_STATE_DRIVER)
            {
                SetVehiclePos(vehicleid, GetGarageData(in_garage, GARAGE_EXIT_X), GetGarageData(in_garage, GARAGE_EXIT_Y), GetGarageData(in_garage, GARAGE_EXIT_Z));
                SetVehicleZAngle(vehicleid, GetGarageData(in_garage, GARAGE_EXIT_ANGLE));
            ...
            {
                SetPlayerPosEx(playerid, GetGarageData(in_garage, GARAGE_EXIT_X), GetGarageData(in_garage, GARAGE_EXIT_Y), GetGarageData(in_garage, GARAGE_EXIT_Z), GetGarageData(in_garage, GARAGE_EXIT_ANGLE), 0, 0);
	...
	if(dialogid == DIALOG_GARAGE_IMPROVEMENT_CONFIRM)
	{
		...
		if(response)
		{
			if(GetGarageData(garageid, GARAGE_IMPROVEMENTS) >= 2)
				return SendClientMessage(playerid, 0x999999FF, "\xd3\xeb\xf3\xf7\xf8\xe5\xed\xe8\xe5 \xf3\xe6\xe5 \xf3\xf1\xf2\xe0\xed\xee\xe2\xeb\xe5\xed\xee.");
			...
			ShowNotification(playerid, 3, "\xd3\xeb\xf3\xf7\xf8\xe5\xed\xe8\xe5 \xe1\xfb\xeb\xee \xea\xf3\xef\xeb\xe5\xed\xee!", 3, "", "");
			ShowNotification(playerid, 0, "\xc2\xfb\xef\xf2\xf0\xe0\xf2\xe8\xeb\xe8 5000000 \xf0\xf3\xe1\xeb\xe5\xe9", 3, "", "");
			GivePlayerMoneyEx(playerid, -5000000);
		}
		else
		{
			Dialog(playerid, DIALOG_GARAGE_IMPROVEMENT, DIALOG_STYLE_LIST,
				"{ff0000}\xd3\xeb\xf3\xf7\xf8\xe5\xed\xe8\xff",
				"{FFFFFF}\xcd\xe0\xe7\xe2\xe0\xed\xe8\xe5\t\t\t\t\t\t\t\t{FFFFFF}\xd6\xe5\xed\xe0\t\t\t\t\t\t\t\t{FFFFFF}\xd1\xf2\xe0\xf2\xf3\xf1\n"\
				"{ffffff}1. \xc2\xee\xe7\xec\xee\xe6\xed\xee\xf1\xf2\xfc \xef\xf0\xee\xe6\xe8\xe2\xe0\xed\xe8\xff \xe2 \xe3\xe0\xf0\xe0\xe6\xe5\t{ffffff}30000 \xf0\xf3\xe1.\t\t{66CC00}\xe8\xec\xe5\xe5\xf2\xf1\xff\n"\
				"{ffffff}2. \xd1\xe5\xe9\xf4 \xe2 \xe3\xe0\xf0\xe0\xe6\xe5{ffffff}\t\t\t\t\t\t\t\t40000 \xf0\xf3\xe1.\t\t\t\t{66CC00}\xe8\xec\xe5\xe5\xf2\xf1\xff\n"\
				"{ffffff}3. \xd3\xeb\xf3\xf7\xf8\xe8\xf2\xfc \xe4\xee \xfd\xeb\xe8\xf2\xed\xee\xe3\xee\t\t\t\t{ffffff}5000000 \xf0\xf3\xe1.\t\t\t\t{ff0000}\xed\xe5 \xe8\xec\xe5\xe5\xf2\xf1\xff",
				"\xc4\xe0\xeb\xe5\xe5", "\xc2\xfb\xe9\xf2\xe8");
		}
	}
					...
					if(response)
					{
							switch(listitem)
							{
                               ...
                               else
						   	{
								...
								case 3:
								{
									if(GetGarageData(garageid, GARAGE_IMPROVEMENTS) < 2) LoadCarGarage[playerid] = 1;					
...
stock EnterPlayerToGarage(playerid, garageid)
{
    ...
    if(improvements >= 2)
    {
        SetPlayerPosEx(playerid, 495.083526, 1990.127807, 1547.679687, 272.186431, 1, garageid + 2000);
    }
    else
    {
        SetPlayerPosEx(playerid, 1.265544, 1996.246215, 1554.203125, 359.354705, 1, garageid + 2000);
    }
...
stock BuyPlayerGarage(playerid, garageid, bool: buy_from_owner = false, price = -1)
{
	if(!IsGarageOwned(garageid) && GetPlayerGarage(playerid) == -1)
	{
		...
		if(GetPlayerMoneyEx(playerid) >= price)
		{
			...
			if(!mysql_errno())
			{
				g_player_garage[playerid] = garageid;
...
CMD:addgarage(playerid, params[])
{
	...
	new Float:garage_x = GetGarageData(idx, GARAGE_X);
	new Float:garage_y = GetGarageData(idx, GARAGE_Y);
	GetCityName(garage_x, garage_y, city);
	GetAreaName(garage_x, garage_y, area);
...
CMD:gsetexitpos(playerid, params[])
{
	...
	SetGarageData(garage_id, GARAGE_EXIT_X, exit_pos_x);
	SetGarageData(garage_id, GARAGE_EXIT_Y, exit_pos_y);
	SetGarageData(garage_id, GARAGE_EXIT_Z, exit_pos_z);
	SetGarageData(garage_id, GARAGE_EXIT_ANGLE, exit_angle);
