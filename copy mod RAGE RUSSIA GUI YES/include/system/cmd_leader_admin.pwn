...
new spisok[45] = 
{
    38, 39, 0, 41, 1, 14, 13, 12, 11, 10, 9, 8, 7, 6, 5, 2, 19, 18, 20, 3, 16, 15,  
    17, 21, 4, 24, 42, 29, 25, 26, 30, 31, 32, 22, 23, 43, 44, 27, 28, 33, 34, 35, 36
};
...
cmd:admins(playerid)
{
    ...
    if(!c) return SendClientMessage(playerid, -1, "{00FA9A}(Ошибка) Администрации нет в сети.");
    ...
    DialogFix(playerid, -1, DIALOG_STYLE_TABLIST_HEADERS, ""BR"Список игроков online (стр 1)", dialog, "Выбрать", "Назад");
    return 1;
}

CMD:leaders(playerid)
{
    new text[174], dialog[sizeof text*10+35] = "{00FA9A}Организация\t{00FA9A}Должность\t{00FA9A}Имя\t{00FA9A}Телефон\n";
    ...
    DialogFix(playerid, -1, DIALOG_STYLE_TABLIST_HEADERS, ""BR"Список игроков online", dialog, "Выбрать", "Назад");
    return 1;
}
