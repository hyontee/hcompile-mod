...
#define DIALOG_CARPASS 2497
#define VIPAD_PRICE        5000
#define VIPAD_COOLDOWN     120
#define DIALOG_WELCOME_MENU 8264

new VipAdCooldown[MAX_PLAYERS];
...
// Compatibility: this build does not contain the SummerEvent implementation.
...
stock GetPlayerDonateRub(playerid)
{
	#pragma unused playerid
	return 0;
}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
	...
	if(dialogid == DIALOG_WELCOME_MENU)
	{
		return 1; // ���� ������ ����������� ��� ������� "�����"
	}
...
#include "../include/system/polia.inc"
#include "../include/system/petrov.pwn"
#include "../include/system/acses.pwn"
...
CMD:playerinf(playerid, params[])
{
	#pragma unused params
	new name[MAX_PLAYER_NAME], dialog[768];
	GetPlayerName(playerid, name, sizeof(name));

	format(dialog, sizeof(dialog),
		"{FFFF00}Информация об игроке\n"\
		"{FFFFFF}----------------------------\n"\
		"{FFFF00}Ник: {FFFFFF}%s\n"\
		"{FFFF00}ID: {FFFFFF}%d\n"\
		"{FFFF00}Уровень администратора: {FFFFFF}%d\n"\
		"{FFFF00}Деньги: {FFFFFF}%d руб.\n"\
		"{FFFF00}Пол: {FFFFFF}%d\n"\
		"{FFFF00}Скин: {FFFFFF}%d\n"\
		"{FFFFFF}----------------------------",
		name, playerid, gPlayerData[playerid][pAdmin],
		gPlayerData[playerid][pMoney], gPlayerData[playerid][pGender],
		gPlayerData[playerid][pSkin]);

	ShowPlayerDialog(playerid, DIALOG_CARPASS, DIALOG_STYLE_MSGBOX,
		"Информация об игроке", dialog, "Закрыть", "");
	return 1;
}
...
CMD:carpass(playerid, params[])
{
	...
	if(sscanf(params, "d", to_player))
		return SendClientMessage(playerid, 0x999999FF, "Используйте: /carpass [id игрока]");

	if(!IsPlayerConnected(to_player))
		return SendClientMessage(playerid, -1, "Такого игрока нет");

	if(!IsPlayerInAnyVehicle(playerid))
		return SendClientMessage(playerid, -1, "Вы не находитесь в транспорте");

	if(!IsPlayerInRangeOfPlayer(playerid, to_player, 6.0))
		return SendClientMessage(playerid, -1, "Игрок находится слишком далеко");
	...
	format(dialog, sizeof(dialog),
		...
		"{FFFFFF}ID транспорта: {FFFF00}%d\n"
		"{FFFFFF}Модель: {FFFF00}%d\n"
		"{FFFFFF}Состояние: {FFFF00}%.0f/1000\n"
		"{FFFFFF}Игрок: {FFFF00}%d",
		GetPlayerNameEx(playerid), vehicleid, model_id, health, playerid);

	ShowPlayerDialog(to_player, DIALOG_CARPASS, DIALOG_STYLE_MSGBOX,
		"Технический паспорт", dialog, "Закрыть", "");

	SendClientMessage(playerid, -1, "Технический паспорт показан игроку.");
	return 1;
}
...
{
	if(gPlayerData[playerid][pAdmin] <= 0)
		return SendClientMessage(playerid, COLOR_RED, "Команда доступна только администраторам.");

	if(isnull(params))
		return SendClientMessage(playerid, -1, "Используйте: /vipad [текст]");

	if(strlen(params) > 50)
		return SendClientMessage(playerid, COLOR_RED, "Нельзя больше 50 символов.");

	new current_time = gettime();
	if(VipAdCooldown[playerid] > current_time)
		return SendClientMessage(playerid, COLOR_RED, "Рекламу можно отправлять раз в 2 минуты.");

	if(gPlayerData[playerid][pMoney] < VIPAD_PRICE)
		return SendClientMessage(playerid, COLOR_RED, "Недостаточно денег.");

	new player_name[MAX_PLAYER_NAME];
	new vip_message[192];
	GetPlayerName(playerid, player_name, sizeof(player_name));

	gPlayerData[playerid][pMoney] -= VIPAD_PRICE;
	ResetPlayerMoney(playerid);
	GivePlayerMoney(playerid, gPlayerData[playerid][pMoney]);
	VipAdCooldown[playerid] = current_time + VIPAD_COOLDOWN;

	format(vip_message, sizeof(vip_message),
		"{FFD700}[VIP] {FFF1A8}%s {FFD700}| Отправил: {FFF1A8}%s[%d]",
		params, player_name, playerid);
	SendClientMessageToAll(-1, vip_message);
	SaveAccount(playerid);
	return 1;
}

CMD:mstart(playerid, params[])
{
	ShowPlayerDialog(playerid, 9001, DIALOG_STYLE_MSGBOX,
		"{00BFFF}MAKVIN STUDIO {FFFFFF}| {00BFFF}Telegram",
		"��������� �� ��� Telegram-�����:\n\n\
		{00BFFF}t.me/makvinstudio11\n\n\
		{FFFFFF}�������, ���������� � ������ ���������� �������.",
		"����������", "");
	return 1;
}
