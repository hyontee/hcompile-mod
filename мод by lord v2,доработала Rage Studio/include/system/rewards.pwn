...
stock Accessory_GetItemIdByModel(modelid)
{
    for(new i = 0; i < sizeof(accessory); i++)
    {
        if(accessory[i][ID_ACCESSORY] == modelid)
            return INVENTORY_ACCESSORY_ITEM_BASE + i;
    }
    return -1;
}
...
#define REWARD_TYPE_VIP 6
#define REWARD_TYPE_BP_EXP 7
#define REWARD_TYPE_ITEM 8
#define REWARD_TYPE_DUST 9
#define REWARD_TYPE_EVENT_RES 10
...
stock Reward_GiveToPlayer(playerid, rewardType, rewardValue, rewardCount, rewardRarity, isBonus)
{
    ...
    switch(rewardType)
    {
        case REWARD_TYPE_EXP:
        {
            ...
            ShowNotificationMisters(playerid, 1, 5, 1, 0, msg, "");
        }
        ...
        case REWARD_TYPE_VIP:
        {
            ...
            ShowNotificationMisters(playerid, 1, 5, 1, 0, "VIP награда активирована", "");
        }
        ...
        case REWARD_TYPE_ITEM:
        {
            if(!Reward_AddInventoryItem(playerid, rewardValue, rewardCount))
                return ShowNotificationMisters(playerid, 2, 5, 1, 1, "Нет свободного места в инвентаре", ""), 0;
...
public OnPlayerRewardsLoad(playerid)
{
    ...
    if(rows == 0)
    {
        if(GetPVarInt(playerid, "reward_offset") == 0)
            ShowNotificationMisters(playerid, 2, 4, 1, 1, "У вас нет доступных наград", "");
...
public OnRewardDebug(playerid, dbId, rewardAction)
{
    ...
    if(rewardAction == 3)
    {
        ...
        {
            ShowNotificationMisters(playerid, 2, 4, 1, 1, "Эту награду нельзя распылить", "");
            deleteReward = false;
        }
    }
    else
