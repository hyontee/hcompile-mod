...
stock StartFCapture(type_capt)
{
    ...
    for(new i;i < br;i++)
    {
        ...
        format(string, sizeof string, "{FF6347}|{FFFFFF} Началось сражение за {FF6347}%s{FFFFFF}, найдите место проведение с помощью карты", name_object[e[i]]);
...
public OnPlayerSpawn(playerid)
{
    if(GetPlayerIdFamily(playerid) != -1)
    {
        for(new i;i < sizeof(CAPTURE_STATUS);i++)
        {
            ...
            GangZoneFlashForPlayer(playerid, CAPTURE_GANG_ZONE[i], 0xFF6347FF);
        }
        ...
        for(new i;i < sizeof(FAM_CONT_STATUS);i++)
        {
            ...
            GangZoneFlashForPlayer(playerid, FAM_CONT_GANG_ZONE[i], 0xFF6347FF);
        }
    }
    CAPTURE_PLAYER_EXP [playerid] = 0;
    show_panel_capt[playerid] = false;
...
stock ShowFamilyCaptPanel(playerid, t)
{
    ...
    ShowNotificationMisters(playerid,  3, 6, 0, 0, "Вы вошли на территорию сражения", "qq");
...
stock HideFamilyCaptPanel(playerid, t)
{
    ...
    ShowNotificationMisters(playerid,  3, 6, 0, 0, "Вы вышли с территории сражения", "qq");
...
stock StartFCont(type_capt)
{
    ...
    for(new i;i < welsi;i++)
    {
        ...
        0, 0);
        FAM_CONT_OBJECT[i] = CreateObject(19321, coord_obj_cont[i][0], coord_obj_cont[i][1], coord_obj_cont[i][2]+0.5, 0.0, 0.0, 0.0);

        FAM_CONT_PICKUP[i] = CreateDynamicPickup(1314, 23, coord_puckup_cont[i][0], coord_puckup_cont[i][1], coord_puckup_cont[i][2], 0, 0);
        ...
        foreach(new w : Player)
        {
            ...
            GangZoneFlashForPlayer(w, FAM_CONT_GANG_ZONE[i], 0xFF6347FF);
        }

        foreach(new w : Player)
        {
            if(GetPlayerIdFamily(w) == -1)
            {
                SendClientMessage(w, -1, "{FF6347}|{FFFFFF} Начался захват упавшего контейнера!\n{FF6347}|{FFFFFF} Вы должны поехать на семейном авто к месту падения и перенести ящики — (/loadfamily)\n{FF6347}|{FFFFFF} Место падения контейнера отмечено на Вашей карте мигающим квадратом.");
            }
       }
