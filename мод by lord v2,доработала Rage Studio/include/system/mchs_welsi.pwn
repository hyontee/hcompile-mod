...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    ...
    {
        if(!player_mchs_active{playerid}) ShowNotificationMisters(playerid, 2, 6, 0, 0, "Для начала начните смену.", "");
        Dialog(playerid, 3502, DSM, "Инвентарь МЧС", "Вы хотите взять огнетушитель?", "Взять", "Отмена");
    }
    if(areaid == veh_mchs)
    {
        if(!player_mchs_active{playerid}) return ShowNotificationMisters(playerid, 2, 6, 0, 0, "Сначала начните смену в МЧС.", "");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == 35500)
    {
        if(response)
        {
            ...
            ShowPlayerGPS(playerid, coord_order[id][0], coord_order[id][1]);
            ...
            if(mysql_errno()) return SCM(playerid, -1, "Ошибка в системе ");
            ...
            switch(listitem)
            {
                case 0:
                {
                    ...
                    else
                    {
                        ...
                        ShowNotificationMisters(playerid, 1, 6, 0, 0, "Вы начали смену. Возьмите огнетушитель и Т/С.", "");
                    }
                }
                case 1:
                {
    ...
    if(dialogid == 3502)
    {
        if(response)
        {
            ...
            ShowNotificationMisters(playerid, 2, 6, 0, 0, "Вы взяли огнетушитель.", "");
        }
    }
    if(dialogid == 3503)
    {
        if(response)
        {
            ...
            ShowNotificationMisters(playerid, 1, 6, 0, 0, "Вы успешно взяли рабочее Т/С.", "");
            ShowNotificationMisters(playerid, 2, 6, 0, 0, "Введите /meslist для просмотра доступных вызовов.", "");
        }
    }
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(PRESSED(KEY_FIRE))
	{
        if(press_fire{playerid}) return ShowNotificationMisters(playerid, 2, 6, 0, 0, "Продолжайте тушить очаг возгорания.", "");
			...
			if(IsPlayerInRangeOfAnyFire(playerid) >= 1)
			{
                ...
                if(all_fire[IsPlayerInRangeOfAnyFire(playerid)][F_ID_Z] == player_order[playerid])
                {
                    ...
                    { 
                       ...
                       ShowNotificationMisters(playerid, 1, 6, 0, 0, "Вы потушили очаг возгорания.", "");

                        if(Order(id))
                        {
                            ShowNotificationMisters(playerid, 1, 6, 0, 0, "Вы успешно выполнили заказ.", "");
                ...
                else ShowNotificationMisters(playerid, 2, 6, 0, 0, "Данный очаг возгорания не вашего заказа.", "");
			}
			else ShowNotificationMisters(playerid, 2, 6, 0, 0, "Не советуем использовать огнетушитель просто так.", "");
		}
	}
...
{
    if(!player_mchs_active{playerid}) return ShowNotificationMisters(playerid, 2, 6, 0, 0, "Вы не работаете в МЧС.", "");
    new Cache:result;
    ...
    if(!rows) return ShowNotificationMisters(playerid, 2, 6, 0, 0, "На данный момент заказов нет, ожидайте {FFFF00}'PayDay'", "");

    if(player_order[playerid]) return ShowNotificationMisters(playerid, 2, 6, 0, 0, "Вы уже взяли заказ.", "");

    new id, text[185], list[sizeof text * 10 + 50] = "{FFFFFF}№\tРасстояние\n";
    new Float:dista;
    ...
    if(rows)
    {
        ...
        DialogMhsc(playerid, 35500, DSTH, "Доступные заказы", list, "Взять", "Назад");
