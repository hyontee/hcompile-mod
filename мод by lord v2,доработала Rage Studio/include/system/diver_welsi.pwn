...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
       ...
       if(items_order[IsPlayerInItemsDiver(playerid)][I_DV_OBJECT] == -1) return 1;
        ...
        if(GetPlayerDiver(playerid, DIVER_JOOB))
        {
            if(IsPlayerInItemsDiver(playerid) != -1)
            {
                ...
                {
                    // ShowNotificationMisters(playerid, 1, 6, 0, 0, "Вы подобрали предмет.", "");
                    return 1;
                }
                else ShowNotificationMisters(playerid, 2, 6, 0, 0, "Этот предмет не с вашего заказа.", "");
                return 0;
            }
        }
        else ShowNotificationMisters(playerid, 2, 6, 0, 0, "Устройтесь на работу водолаза.", "");
        return 0;
    }
    if(sphere_actor[0] <= areaid <= sphere_actor[1])
    {
        ...
        (
            ...
            ""br_с"Водолаз "CW"| Меню работы", 
            ""br_с"1."CW"Взять заказ\n"\
            ""br_с"2."CW"Устроиться/уволиться с работы\n"\
            ""br_с"3."CW"Арендовать лодку\n"\
            ""br_с"4."CW"Продать найденные предметы", 
            "Продолжить", "Выход"
        );
        return 1;
    }
...
public OnGameModeInit()
{
   ...
   CreateActorEx("{FAD201}Капитан пирса", "{FFFFFF}Подойдите для взаимодействия", 5344, 676.336730,187.356491,2.778203,175.494964);
   CreateActorEx("{FAD201}Капитан пирса", "{FFFFFF}Подойдите для взаимодействия", 111,-2526.549072,357.963623,2.060437,124.978698);
...
{
    if(!GetPlayerDiver(playerid, DIVER_JOOB)) 
        return ShowNotificationMisters(playerid, 2, 6, 0, 0, "Сначала возьмите смену.", "");

    if(GetPlayerDiver(playerid, DIVER_ORDER)) 
        return ShowNotificationMisters(playerid, 2, 6, 0, 0, "У вас уже есть заказ.", "");

    new orders[MAX_DIVER];
    new free_orders = 0;
    
    // Собираем свободные заказы
    for(new d = 0; d < MAX_DIVER; d++)
    {
        if(order_diver[d][DV_P_ID] == -1)
        {
            orders[free_orders] = d;
            free_orders++;
        }
    }
    
    if(free_orders == 0) 
        return ShowNotificationMisters(playerid, 1, 6, 0, 0, "Нет свободных заказов, подождите...", "");
    
    // Выбираем случайный свободный заказ
    new r_orders = orders[random(free_orders)];
...
SetPlayerRaceCheckpoint(playerid, 0, 
    order_diver[r_orders][DV_X], 
    order_diver[r_orders][DV_Y], 
    order_diver[r_orders][DV_Z], 
    order_diver[r_orders][DV_X], 
    order_diver[r_orders][DV_Y], 
    order_diver[r_orders][DV_Z], 
    10.0);
    ...
    CreateSmallGangZoneForDiver(playerid, 
        order_diver[r_orders][DV_X], 
        order_diver[r_orders][DV_Y]);
    
    ShowNotificationMisters(playerid, 1, 6, 0, 0, "Заказ получен! Следуйте на точку.", "");
}
   case 2:
{
    if(GetPlayerDiver(playerid, DIVER_JOOB))
    {
        ...
        if(GetPlayerDiver(playerid, DV_ID_VEH) != -1)
        {
            ...
            SetPlayerDiver(playerid, DV_ID_VEH, -1);
            ShowNotificationMisters(playerid, 2, 6, 0, 0, "Вы уволились с работы 'Водолаз'.", "");
        }
    }
    else
    {
        ...
        SetPlayerDiver(playerid, DIVER_JOOB, true);
                ...
                case 3:
                {
                    if(!GetPlayerDiver(playerid, DIVER_JOOB)) return ShowNotificationMisters(playerid, 2, 6, 0, 0, "Сначала начните смену.", "");
                    ...
                    if(GetPlayerDiver(playerid, DV_ID_VEH) != -1)
                    {
                        DestroyVehicle(GetPlayerDiver(playerid, DV_ID_VEH));
                        SetPlayerDiver(playerid, DV_ID_VEH, -1);
                        ShowNotificationMisters(playerid, 1, 6, 0, 0, "Вы вернули лодку.", "");
                    }
                    
                    else
                    {
                        if(!(GetPlayerMoneyEx(playerid) >= 200000)) return ShowNotificationMisters(playerid, 1, 6, 0, 0, "Вам необходимо 200.000 рублей.", "");
                        new vehicle;
...
SetPVarInt(playerid, "DiverBoat", vehicle);
...
{
    printf("[DEBUG] Загружаю предметы для заказа %d", id);  // ДОБАВЬ ЭТУ СТРОКУ
    for(new c;c < MAX_I_DIVER;c++)
    {
        ...
        printf("[DEBUG] Создан предмет на координатах: %f, %f, %f", items_order[c][I_DV_X], items_order[c][I_DV_Y], items_order[c][I_DV_Z]);  // ДОБАВЬ ЭТУ СТРОКУ
    }
    return 1;
}
...
stock RemoveDiverOrder(id)
{
    ...
    // Удаляем гангзону у игрока, если он выполнял этот заказ
    new playerid = order_diver[id][DV_P_ID]; // Это -1, так как мы только что присвоили
    // Нужно получить ID игрока до присвоения -1
    // Немного изменим код:

    // Найдем игрока, выполняющего заказ с id
    for(new p = 0; p < MAX_PLAYERS; p++) // Перебираем всех игроков
    {
        if(IsPlayerConnected(p) && GetPlayerDiver(p, DIVER_ORDER) && GetPlayerDiver(p, DV_ID) == id)
        {
             playerid = p; // Нашли игрока
             break; // Выходим из цикла
        }
    }

    // Теперь удаляем предметы
    ...
    // И удаляем гангзону у найденного игрока
...
CMD:takeitems(playerid)
{
    ...
    switch(items_order[i_id][I_DV_TYPE])
    {
        case I_DIVER_BOX:
        {
            AddPlayerDiver(playerid, DV_BOX, +, 1);
            UpdatePlayerDatabaseInt(playerid, "box", GetPlayerDiver(playerid, DV_BOX));
            ShowNotificationMisters(playerid, 1, 6, 0, 0, "Вы подобрали 'Шкатулка'.", "");
        }
        case I_DIVER_TIRE:
        {   
            AddPlayerDiver(playerid, DV_TIRE, +, 1);
            UpdatePlayerDatabaseInt(playerid, "tire", GetPlayerDiver(playerid, DV_TIRE));
            ShowNotificationMisters(playerid, 1, 6, 0, 0, "Вы подобрали 'Шина'.", "");
        }
        case I_DIVER_GOLD:
        {
            AddPlayerDiver(playerid, DV_GOLD, +, 1);
            UpdatePlayerDatabaseInt(playerid, "gold", GetPlayerDiver(playerid, DV_GOLD));
            ShowNotificationMisters(playerid, 1, 6, 0, 0, "Вы подобрали 'Слиток золота'.", "");
        }
        case I_DIVER_TV:
        {
            AddPlayerDiver(playerid, DV_TV, +, 1);
            UpdatePlayerDatabaseInt(playerid, "television", GetPlayerDiver(playerid, DV_TV));
            ShowNotificationMisters(playerid, 1, 6, 0, 0, "Вы подобрали 'Телевизор'.", "");
        }
        case I_DIVER_BOOT:
        {
            AddPlayerDiver(playerid, DV_BOOT, +, 1);
            UpdatePlayerDatabaseInt(playerid, "boot", GetPlayerDiver(playerid, DV_BOOT));
            ShowNotificationMisters(playerid, 1, 6, 0, 0, "Вы подобрали 'Сапог с кладом'.", "");
        }
    }
    ...
    if(items_count >= 4)
    {
        ...
        SetPlayerDiver(playerid, DV_ID, -1);
        SetPlayerDiver(playerid, DIVER_ORDER, false);
        ShowNotificationMisters(playerid, 1, 6, 0, 0, "Вы собрали все предметы.", "");
        // SendClientMessage(playerid, -1, "Вы завершили заказ, отправляйтесь к ближайшему пирсу {FFD700}[ /gps > Причалы ]{FFFFFF}.");
        ...
        if(distance_actor_1 < distance_actor_2) ShowPlayerGPS(playerid, coord_actor_1[0], coord_actor_1[1]);
        else ShowPlayerGPS(playerid, coord_actor_2[0], coord_actor_2[1]);
        SendClientMessage(playerid, -1, "Ближайший {FAD201}'Капитан Пирса' {FFFFFF}отмечен на карте.");
    }
...
CMD:boatfix(playerid)
{
    new boat = GetPlayerDiver(playerid, DV_ID_VEH);
    if(boat != -1 && IsValidVehicle(boat))
    {
        // Удаляем игрока из любого транспорта
        RemovePlayerFromVehicle(playerid);
        
        // Телепортируем к лодке
        new Float:x, Float:y, Float:z;
        GetVehiclePos(boat, x, y, z);
        SetPlayerPos(playerid, x + 1.0, y + 1.0, z + 1.0);
        
        // Небольшая задержка перед посадкой
        SetTimerEx("ForceEnterBoat", 100, false, "ii", playerid, boat);
        
        SendClientMessage(playerid, -1, "Попытка сесть в лодку...");
    }
    else
    {
        SendClientMessage(playerid, -1, "У вас нет арендованной лодки");
    }
    return 1;
}

// Функция для принудительной посадки
forward ForceEnterBoat(playerid, boatid);
public ForceEnterBoat(playerid, boatid)
{
    if(IsPlayerConnected(playerid) && IsValidVehicle(boatid))
    {
        PutPlayerInVehicle(playerid, boatid, 0);
    }
    return 1;
}