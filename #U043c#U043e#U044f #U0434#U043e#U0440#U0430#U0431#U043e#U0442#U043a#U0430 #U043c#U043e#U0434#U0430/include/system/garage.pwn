...
new garage_percent = (garage_price * 30) / 100;
		...
		{
			if(to_player == playerid || GetPlayerGarage(to_player) != -1 || price <= 0 || GetPlayerMoneyEx(to_player) < price) return 0;

			new seller_money = GetPlayerMoneyEx(playerid) + price;
			new buyer_money = GetPlayerMoneyEx(to_player) - price;

			format(query, sizeof query, "UPDATE accounts seller, accounts buyer, garages g SET seller.money=%d,seller.garage=-1,buyer.money=%d,buyer.garage=%d,g.owner_id=%d,g.lock=0 WHERE seller.id=%d AND buyer.id=%d AND g.id=%d AND g.owner_id=%d", seller_money, buyer_money, garageid, GetPlayerAccountID(to_player), GetPlayerAccountID(playerid), GetPlayerAccountID(to_player), GetGarageData(garageid, GARAGE_ID), GetPlayerAccountID(playerid));
			mysql_query(mysql, query, false);
			...
			SetGarageData(garageid, GARAGE_STATUS, 0);
			format(g_garage[garageid][G_OWNER_NAME], 21, GetPlayerNameEx(to_player));

			GivePlayerMoneyEx(playerid, price, "Продажа гаража игроку", false, false);
			GivePlayerMoneyEx(to_player, -price, "Покупка гаража у игрока", false, false);
			UpdateGarageInfo(garageid);
			EnterPlayerToGarage(to_player, garageid);

			format(query, sizeof query, "Вы продали гараж игроку %s за %d руб", GetPlayerNameEx(to_player), price);
			SendClientMessage(playerid, 0x66CC00FF, query);
			format(query, sizeof query, "%s продал Вам гараж №%d за %d руб", GetPlayerNameEx(playerid), garageid, price);
