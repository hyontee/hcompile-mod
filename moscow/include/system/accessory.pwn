    ...
    #error "Compile gamemodes/new.pwn, not include/system/accessory.pwn directly."
#endif
...
stock GiveAccessory(playerid, acs)
{
    ...
    mysql_format(mysql, string, sizeof string, "INSERT INTO accessory_inventory (player_id, acs_id, use) VALUES (%d, %d, 0)", GetPlayerAccountID(playerid), acs);
    mysql_query(mysql, string, false);
...
//-НАЖАТЫЕ КНОПКИ
	...
	acss_TD[24] = TextDrawCreate(460.0000, 205.0000, "txd:transparent");
	...
	acss_TD[25] = TextDrawCreate(530.0000, 205.0000, "txd:transparent");
...
public: CREATE_TABLIST_ACCESSORY()
{
	...
	if(mysql_errno())
	{
		mysql_query(mysql, 
			...
			`acs_id` INT NOT NULL , \
			`use` INT NOT NULL DEFAULT 0 , PRIMARY KEY (`id`)) ENGINE = InnoDB;", false);
