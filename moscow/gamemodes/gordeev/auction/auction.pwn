...
#define AUCTION_PLAYER_BUSINESS_LOT_TIME 10800
#define AUCTION_GOV_BUSINESS_LOT_TIME 25200
#define AUCTION_TABLE       "auction_lots"
...
new bool:auction_database_loaded;
new bool:auction_business_reload;

stock AuctionNotify(playerid, notify_type, text[], duration = 4)
{
    return ShowNotificationNew(playerid, notify_type, duration, -1, -1, text, "");
}
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
    if(dialogid == 5990)
    {
        if(response)
        {
            ...
            if(!(rate >= price)) return AuctionNotify(playerid, 2, "Ваша ставка должна превышать минимальную ставку на лот.");
            ...
            {   
                if(old_rate != -1)
                {
                    ...
                    foreach(new i142 : Player)
                    {
                        if(GetPlayerAccountID(i142) != old_rate) continue;

                        GivePlayerMoneyEx(i142, auction_slot[id][CURRENT_RATE]);
                        give_money = true;
                        AuctionNotify(i142, 2, "Вашу ставку перебили. Деньги возвращены");
                        break;
                    }
                ...
                new spent_text[48];
                format(spent_text, sizeof spent_text, "Вы потратили %d рублей на ставку", rate);
                AuctionNotify(playerid, 4, spent_text);

                default_select_slot(playerid);

                player_info_panel[playerid][select_slot] = 0;

                for(new i174 = 1; i174 < 6;i174++ ) PlayerTextDrawSetString(playerid, auction_PTD[playerid][i174], "-");
                ...
                AuctionNotify(playerid, 3, "Вы успешно сделали ставку на лот.");
            }
        }
        ...
        switch(GetPVarInt(playerid, "type_param"))
        {
            case TYPE_VEHICLE_NUMBER:
            {
                ...
                if(num[0] == 0)
                {
                    ...
                    for(new i281, sql = GetPlayerAccountID(playerid);i281 < MAX_AUCTION_SLOT;i281++)
                    {
                        if(auction_slot[i281][OWNER_SLOT_SQL] != sql) continue;

                        if(auction_slot[i281][SLOT_NAME] == TYPE_VEHICLE && auction_slot[i281][DATABASE_ID] == id_car) 
...
{
    for(new i979;i979 < MAX_BUSINESS; i979 ++)
    {
        if(GetBusinessData(i979, B_NAME)) continue;
...
public OnPlayerClickPlayerTextDraw(playerid, PlayerText:playertextid)
{
    ...
    if(playertextid == auction_PTD[playerid][6])
    {
        ...
        AuctionFormatTimeLeft(slot, string, sizeof string);
    ...
    if(playertextid == auction_PTD[playerid][30])
    {
        ...
        switch(GetPVarInt(playerid, "type_param"))
        {
            ...
            case TYPE_VEHICLE_NUMBER:
            {
				...
				for(new i1381 = 0; i1381 < rows; i1381 ++)
				{
					id = cache_get_field_content_int(i1381, "id");
					model_id = cache_get_field_content_int(i1381, "model_id") - 400;
					cache_get_field_content(i1381, "number", car_number);

					format(query, sizeof query, "{FFFFFF}%d. %s - %s\n", i1381 + 1, GetVehicleInfo(model_id, VI_NAME), car_number);
					strcat(text, query);
					SetPlayerListitemValue(playerid, i1381, id);
                    printf("lisitem: %d | id_car: %d", i1381, id);
				}
            ...
            case TYPE_CLOTHES:
            {
				...
				for(new i1414 = 0; i1414 < rows; i1414 ++)
				{
					id = cache_get_field_content_int(i1414, "id");
					model_id = cache_get_field_content_int(i1414, "skin_id");

					format(query, sizeof query, "{FFFFFF}%d. %s\n", i1414 + 1, GetSkinInfo(model_id, S_NAME));
					strcat(text, query);
					SetPlayerListitemValue(playerid, i1414, id);
				}
    ...
    {
        new bool:have_sell, list[48], dialog[284], time_str[16];

        for(new c, account = GetPlayerAccountID(playerid);c < MAX_AUCTION_LIST;c++)
        {
            if(auction_slot[c][OWNER_SLOT_SQL] != account) continue;

            AuctionFormatTimeLeft(c, time_str, sizeof time_str);
            format(list, sizeof list, "%d. %s \t %s\n", c + 1, auction_slot[c][SLOT_NAME], time_str);
            strcat(dialog, list);
            have_sell = true;
        }

        if(!have_sell) return AuctionNotify(playerid, 2, "У вас нет ставок на продажу", 3);
    ...
    {
        new bool:have_buy, list[48], dialog[284], time_str[16];
        for(new c, account = GetPlayerAccountID(playerid);c < MAX_AUCTION_LIST;c++)
        {
        ...
        if(!have_buy) return AuctionNotify(playerid, 2, "У вас нет ставок на покупку", 3); 
    ...
    if(playertextid == auction_PTD[playerid][37])
    {
        ...
        if(auction_slot[id_slot][TIMER_END] <= gettime()) return AuctionNotify(playerid, 2, "Время вышло. Лот скоро удалится.");
        ...
        if(GetPlayerMoneyEx(playerid) < auction_slot[id_slot][CURRENT_RATE])
        {
            ...
            AuctionNotify(playerid, 2, dialog, 2);

            return 1;
        }
        else if(auction_slot[id_slot][OWNER_SLOT_SQL] == GetPlayerAccountID(playerid))
        {
            AuctionNotify(playerid, 2, "Вы не можете сделать ставку на свой лот.");
            return 1;
        }
        else if(auction_slot[id_slot][OWNER_RATE] == GetPlayerAccountID(playerid))
        {
            AuctionNotify(playerid, 2, "Вы уже сделали ставку.");
            return 1;
        }
        ...
        switch(auction_slot[id_slot][SLOT_TYPE])
        {
            case TYPE_BUSINESS:
            {
                if(GetPlayerBusiness(playerid) != -1) return AuctionNotify(playerid, 2, "У вас уже имеется бизнес");

                for(new i1542, account = GetPlayerAccountID(playerid); i1542 < MAX_AUCTION_SLOT; i1542 ++)
                {
                    if(auction_slot[i1542][SLOT_TYPE] != TYPE_BUSINESS) continue;
                    else if(auction_slot[i1542][OWNER_RATE] != account) continue;

                    AuctionNotify(playerid, 2, "Вы уже делали ставку на другой бизнес.");
                    return 1;   
                }
            }
            case TYPE_HOUSE:
            {
                ...
                for(new i1555, account = GetPlayerAccountID(playerid); i1555 < MAX_AUCTION_SLOT; i1555 ++)
                {
                    if(auction_slot[i1555][SLOT_TYPE] != TYPE_HOUSE) continue;
                    else if(auction_slot[i1555][OWNER_RATE] != account) continue;
            ...
            case TYPE_SUM_NUMBER:
            {
                if(GetPlayerPhone(playerid)) 
                {
                    for(new i1583; i1583 < 5; i1583++) SendClientMessage(playerid, -1, ""USC" У вас уже есть сим-карта. При выйгрыше она измениться на новую");
                }
            }
        }
    ...
    {
        if(!player_info_panel[playerid][price_tovar]) return AuctionNotify(playerid, 2, "Сначала выберите все параметры");
...
{
    new time = gettime();
    if(player_cd_auction[playerid] > time) return AuctionNotify(playerid, 2, "Подождите немного...");
...
stock HideTabMy(playerid)
{
    ...
    for(new i1922 = 23;i1922 < 27;i1922++) PlayerTextDrawDestroy(playerid, auction_PTD[playerid][i1922]);
...
stock ShowTabMy(playerid)
{
    ...
    PlayerTextDrawDestroy(playerid, auction_PTD[playerid][0]);
    for(new i2004 = 23;i2004 < 27;i2004++) PlayerTextDrawDestroy(playerid, auction_PTD[playerid][i2004]);
...
stock AuctionEnsureSchema()
{
    ...
    if(mysql_errno()) return printf("[ERROR AUCTION MYSQL] CREATE "AUCTION_TABLE);

    auction_database_loaded = true;
    return 1;
}

stock AuctionSaveSlot(slot)
{
    if(mysql <= 0 || !auction_database_loaded || !auction_slot[slot][SLOT_TYPE]) return 0;

    new query[1024];
    mysql_format(mysql, query, sizeof query,
        "REPLACE INTO `"AUCTION_TABLE"` (`slot`,`slot_type`,`owner_slot_sql`,`owner_slot_name`,`slot_name`,`opisanie_slot`,`owner_rate`,`start_rate`,`current_rate`,`timer_end`,`database_id`,`server_id`,`number_vehicle`,`number_sum`) VALUES (%d,%d,%d,'%e','%e','%e',%d,%d,%d,%d,%d,%d,'%e',%d)",
        slot,
    ...
    if(mysql_errno()) return printf("[ERROR AUCTION MYSQL] SAVE SLOT %d", slot);
    return 1;
}

stock AuctionDeleteSlot(slot)
{
    if(mysql <= 0 || !auction_database_loaded) return 0;

    new query[64];
    mysql_format(mysql, query, sizeof query, "DELETE FROM `"AUCTION_TABLE"` WHERE `slot`=%d LIMIT 1", slot);
    mysql_query(mysql, query, false);

    if(mysql_errno()) return printf("[ERROR AUCTION MYSQL] DELETE SLOT %d", slot);
    return 1;
}

stock AuctionSlotExistsByItem(type, database_id)
{
    for(new slot; slot < MAX_AUCTION_SLOT; slot++)
    {
        if(auction_slot[slot][SLOT_TYPE] != type) continue;
        if(auction_slot[slot][DATABASE_ID] != database_id) continue;
...
stock AuctionRefreshSlotServer(slot)
{
    switch(auction_slot[slot][SLOT_TYPE])
    {
        case TYPE_BUSINESS:
        {
            ...
            {
                if(GetBusinessData(businessid, B_SQL_ID) != auction_slot[slot][DATABASE_ID]) continue;

                auction_slot[slot][SERVER_ID] = businessid;
                if(!strlen(auction_slot[slot][SLOT_NAME]) || !strcmp(auction_slot[slot][SLOT_NAME], "-", true))
                    format(auction_slot[slot][SLOT_NAME], 24, "%s (%d)", GetBusinessData(businessid, B_NAME), businessid);
                if(!strlen(auction_slot[slot][OPISANIE_SLOT]) || !strcmp(auction_slot[slot][OPISANIE_SLOT], "-", true))
                    format(auction_slot[slot][OPISANIE_SLOT], 124, "Бизнес (%d)", businessid);
        ...
        case TYPE_HOUSE:
        {
            ...
            {
                if(GetHouseData(houseid, H_SQL_ID) != auction_slot[slot][DATABASE_ID]) continue;

                auction_slot[slot][SERVER_ID] = houseid;
                if(!strlen(auction_slot[slot][SLOT_NAME]) || !strcmp(auction_slot[slot][SLOT_NAME], "-", true))
                    format(auction_slot[slot][SLOT_NAME], 24, "Дом_(%d)", houseid);
        ...
        {
            if(!strlen(auction_slot[slot][SLOT_NAME]) || !strcmp(auction_slot[slot][SLOT_NAME], "-", true))
                valstr(auction_slot[slot][SLOT_NAME], auction_slot[slot][NUMBER_SUM]);
        }
        case TYPE_CLOTHES:
        {
            if(!strlen(auction_slot[slot][SLOT_NAME]) || !strcmp(auction_slot[slot][SLOT_NAME], "-", true))
                format(auction_slot[slot][SLOT_NAME], 24, GetSkinName(auction_slot[slot][SERVER_ID]));
        }
    }
    return 1;
}

stock AuctionLoadSavedSlots()
{
    if(!AuctionEnsureSchema()) return 0;

    new Cache:result = mysql_query(mysql, "SELECT * FROM `"AUCTION_TABLE"` ORDER BY `slot` ASC", true);
    if(mysql_errno()) return print("[ERROR AUCTION MYSQL] LOAD SAVED SLOTS");

    new rows = cache_num_rows(), slot, loaded;
    for(new row; row < rows; row++)
    {
        slot = cache_get_field_content_int(row, "slot");
        if(!(0 <= slot < MAX_AUCTION_SLOT)) continue;

        auction_slot[slot][SLOT_TYPE] = cache_get_field_content_int(row, "slot_type");
        auction_slot[slot][OWNER_SLOT_SQL] = cache_get_field_content_int(row, "owner_slot_sql");
        cache_get_field_content(row, "owner_slot_name", auction_slot[slot][OWNER_SLOT_NAME], mysql, 24);
        cache_get_field_content(row, "slot_name", auction_slot[slot][SLOT_NAME], mysql, 24);
        cache_get_field_content(row, "opisanie_slot", auction_slot[slot][OPISANIE_SLOT], mysql, 124);
        auction_slot[slot][OWNER_RATE] = cache_get_field_content_int(row, "owner_rate");
        ...
        cache_get_field_content(row, "number_vehicle", auction_slot[slot][NUMBER_VEHICLE], mysql, 7);
        auction_slot[slot][NUMBER_SUM] = cache_get_field_content_int(row, "number_sum");

        if(auction_slot[slot][SLOT_TYPE] <= 0 || !AuctionRefreshSlotServer(slot))
        {
    ...
    cache_delete(result);
    printf("[AUCTION] Loaded saved slots: %d", loaded);
    return 1;
}

stock AuctionFormatTimeLeft(slot, string[], size)
{
    new time_left = auction_slot[slot][TIMER_END] - gettime();
    if(time_left < 0) time_left = 0;

    if(ConvertUnixTime(time_left, CONVERT_TIME_TO_HOURS) > 0)
        format(string, size, "%d_ч.", ConvertUnixTime(time_left, CONVERT_TIME_TO_HOURS));
    else if(ConvertUnixTime(time_left, CONVERT_TIME_TO_MINUTES) > 0)
        format(string, size, "%d_мин.", ConvertUnixTime(time_left, CONVERT_TIME_TO_MINUTES));
    else
        format(string, size, "%d_сек.", ConvertUnixTime(time_left, CONVERT_TIME_TO_SECONDS));

    return 1;
}
public:AuctionUpdate()
{
    if(mysql <= 0 || !g_business_loaded)
    {
        SetTimer("AuctionUpdate", 1000, false);
        return 1;
    }

    if(!load_business_action)
    {
        AuctionLoadSavedSlots();
        LoadBusinessToAuction();
        SetTimer("AuctionUpdate", 1000, true);
    }

    if(auction_business_reload)
    {
        auction_business_reload = false;
        LoadBusinessToAuction();
    }
...
stock create_auction_slot(playerid, type, opisanie[124], start_rate, data_base, server_id, number_vehicle[7] = "", number_SUM = 0)
{
    ...
    if(stop_create) return AuctionNotify(playerid, 2, "Вы уже выставляли данный товар.");
    ...
    if(id == -1) return AuctionNotify(playerid, 2, "В данный момент достигнут лимит слотов в аукционе.");
    ...
    auction_slot[id][TIMER_END] = gettime() + AUCTION_PLAYER_BUSINESS_LOT_TIME;
        ...
        case TYPE_BUSINESS:
        {
            for(new s;s < MAX_BUSINESS;s++)
            {
                if(GetBusinessData(s, B_SQL_ID) == data_base)
                {
                    ...
                    format(auction_slot[id][SLOT_NAME], 24, "%s (%d)", GetBusinessData(s, B_NAME), s);
                    break;
                }
            }
        }
    ...
    AuctionNotify(playerid, 3, "Вы выставили слот на аукцион");
...
{
    if(auction_slot[slot][OWNER_RATE] == -1) return reset_slot_auction(slot);

    new bool:money_give;

    foreach(new i2427 : Player)
    {
        if(auction_slot[slot][OWNER_SLOT_SQL] == GetPlayerAccountID(i2427))
        {
            SendClientMessage(i2427, -1, ""SC" Поздравляем! Вы получили деньги за слот с аукциона");
            money_give = true;
            GivePlayerMoneyEx(i2427, auction_slot[slot][CURRENT_RATE] );
        }
    }
    
    new query[128];

    if(!money_give && auction_slot[slot][OWNER_SLOT_SQL] > 0)
    {
    ...
    switch(auction_slot[slot][SLOT_TYPE])
    {
        case TYPE_BUSINESS:
        {
            foreach(new i2451 : Player)
            {
                if(auction_slot[slot][OWNER_RATE] == GetPlayerAccountID(i2451)) SendClientMessage(i2451, -1, ""SC" Поздравляем! Вы получили бизнес с аукциона");
            }
        ...
        case TYPE_HOUSE:
        {
            foreach(new i2460 : Player)
            {
                if(auction_slot[slot][OWNER_RATE] == GetPlayerAccountID(i2460)) SendClientMessage(i2460, -1, ""SC" Поздравляем! Вы получили недвижимость с аукциона");
            }
        ...
        case TYPE_VEHICLE:
        {
            foreach(new i2469 : Player)
            {
                if(auction_slot[slot][OWNER_RATE] == GetPlayerAccountID(i2469)) SendClientMessage(i2469, -1, ""SC" Поздравляем! Вы получили транспорт с аукциона");
            }
        ...
        case TYPE_VEHICLE_NUMBER:
        {
            foreach(new i2485 : Player)
            {
                if(GetPlayerAccountID(i2485) == auction_slot[slot][OWNER_RATE]) SendClientMessage(i2485, -1, ""SC" Поздравляем! Вы приобрели на номерной знак");
                if(GetPlayerAccountID(i2485) == auction_slot[slot][OWNER_SLOT_SQL]) SendClientMessage(i2485, -1, ""SC" Поздравляем! Вы продали на номерной знак");
            }
        ...
        case TYPE_SUM_NUMBER:
        {
            ...
            foreach(new i2505 : Player)
            {
                if(GetPlayerAccountID(i2505) == auction_slot[slot][OWNER_RATE])
                {
                    SetPlayerData(i2505, P_PHONE, auction_slot[slot][NUMBER_SUM]);
                    SendClientMessage(i2505, -1, ""SC" Поздравляем! Вы приобрели на аукционе сим-карту");
                }

                if(GetPlayerAccountID(i2505) == auction_slot[slot][OWNER_SLOT_SQL])
                {
                    SetPlayerData(i2505, P_PHONE, 0);
                    SendClientMessage(i2505, -1, ""SC" Поздравляем! Вы продали на аукционе сим-карту");
                }
            }
        ...
        case TYPE_CLOTHES:
        {
            foreach(new i2532 : Player)
            {
                if(GetPlayerAccountID(i2532) == auction_slot[slot][OWNER_RATE]) SendClientMessage(i2532, -1, ""SC" Поздравляем! Вы приобрели на аукционе одежду");
                if(GetPlayerAccountID(i2532) == auction_slot[slot][OWNER_SLOT_SQL]) SendClientMessage(i2532, -1, ""SC" Поздравляем! Вы продали на аукционе одежду");
            }
...
{
    new slot_type = auction_slot[slot][SLOT_TYPE];

    AuctionDeleteSlot(slot);
    ...
    if(slot_type == TYPE_BUSINESS && load_business_action)
    {
        auction_business_reload = true;
    }
...
stock AuctionBusiness(sql_id, slot)
{
    ...
    foreach(new i2582 : Player){
        if(sql_id == GetPlayerAccountID(i2582)) player = i2582;
    }
    foreach(new i2587 : Player){
        if(auction_slot[slot][OWNER_SLOT_SQL] == GetPlayerAccountID(i2587)) SetPlayerData(i2587, P_BUSINESS, -1);
    }
    
    // Обновляем данные бизнеса в памяти
    SetBusinessData(businessid, B_OWNER_ID, sql_id);
    SetBusinessData(businessid, B_RENT_DATE, gettime() + 259200); // 3 дня
    ...
    // === ИСПРАВЛЕНИЕ: Выдаем бизнес игроку и сохраняем в БД ===
    if(player != -1)
    {
        SetPlayerData(player, P_BUSINESS, businessid);
    }
    
    mysql_format(mysql, query, sizeof query, "UPDATE accounts SET business=%d WHERE id=%d", businessid, sql_id);
    mysql_query(mysql, query, false);
    
    mysql_format(mysql, query, sizeof query, "UPDATE business SET owner_id=%d, rent_time=%d, improvements=0, prod_price=0, balance=0, products=0, `lock`=0 WHERE id=%d", sql_id, gettime() + 259200, GetBusinessData(businessid, B_SQL_ID));
    mysql_query(mysql, query, false);
    // ==========================================================
...
stock AuctionAddBusinessLot(businessid)
{
    if(!(0 <= businessid < MAX_BUSINESS)) return 0;
    if(GetBusinessData(businessid, B_SQL_ID) <= 0) return 0;
    if(GetBusinessData(businessid, B_OWNER_ID) != 0) return 0;
    if(AuctionSlotExistsByItem(TYPE_BUSINESS, GetBusinessData(businessid, B_SQL_ID))) return 1;

    new slot = -1;
    for(new free_slot; free_slot < MAX_AUCTION_SLOT; free_slot++)
    {
        if(auction_slot[free_slot][SLOT_TYPE] != 0) continue;
    ...
    auction_slot[slot][TIMER_END] = gettime() + AUCTION_GOV_BUSINESS_LOT_TIME;
...
public:LoadBusinessToAuction()
{
    ...
    for(new businessid; businessid < g_business_loaded; businessid++)
    {
        ...
        if(AuctionSlotExistsByItem(TYPE_BUSINESS, GetBusinessData(businessid, B_SQL_ID))) continue;

        if(AuctionAddBusinessLot(businessid)) loaded++;
    }

    printf("[AUCTION] Loaded free business lots: %d", loaded);
    return 1;
}
...
stock AuctionHouse(slot, win_slot, houseid)
{
    ...
    foreach(new i2770 : Player)
    {
        if(auction_slot[slot][OWNER_SLOT_SQL] == GetPlayerAccountID(i2770))
        {
            SetPlayerData(i2770, P_HOUSE, -1);
            SetPlayerData(i2770, P_HOUSE_TYPE, -1);
        }
    }
    ...
    if(!mysql_errno())
    {
        foreach(new i2789 : Player)
        {
            if(GetPlayerAccountID(i2789) == win_slot)
            {
                SetPlayerData(i2789, P_HOUSE, 		houseid);
                SetPlayerData(i2789, P_HOUSE_TYPE, 	HOUSE_TYPE_HOME);
            }
        }
...
stock LeafAuction(playerid, all_slot)
{
    ...
    for(new i2857 = 1; i2857 < 6;i2857++ ) PlayerTextDrawSetString(playerid, auction_PTD[playerid][i2857], "-");

    default_select_slot(playerid);

    player_info_panel[playerid][select_slot] = 0;

    for(new i2863;i2863 < 4;i2863++) player_show_slot[playerid][i2863] = -1;
...
{
    new result[256];
    for (new i2928 = 0; i2928 < sizeof(result); i2928++)
    {
        switch(string[i2928])
        {
            case ' ':result[i2928] = '_';
            default: result[i2928] = string[i2928];
        }
    }
    return result;
}
