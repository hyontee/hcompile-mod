        ...
        switch(orders_collectors[order][progress_order])
        {
            ...
            case 1:// игрок взял деньги/акции
            {
                ...
                if(orders_collectors[order][place_start] != Place_Col_1)
                {
                    ...
                    ""SC" Отправляйтесь в {FFD700}отделение банка{FFFFFF} и возмите денежные средства из {FFD700}хранилища.");
                } 
                else
                {
                    SendClientMessage(orders_collectors[order][collector_id_collector], -1, ""SC" Отправляйтесь к {FFD700}банкомату{FFFFFF} и отнесите ценный груз.");
                }
            }
            case 2:// игрок подъежает к месту завершения заказа
            {
                ...
                {
                    SendClientMessage(orders_collectors[order][collector_id_collector], -1,
                    ""SC" Отправляйтесь в {FFD700}отделение банка{FFFFFF} и отнесите денежные средства в {FFD700}хранилище.");
                } 
                else
                {
                    SendClientMessage(orders_collectors[order][collector_id_collector], -1, ""SC" Отправляйтесь к {FFD700}банкомату{FFFFFF} и отнесите ценный груз.");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
            ...
            switch(listitem)
            {
                case 0:
                {
                    ...
                    (
                        ...
                        "{FFD700}Инкассатор{FFFFFF} - сотрудник банка, занимающийся транспортировкой денежных средств и других драгоценностей\n"\
                        "по нуждам банка или его клиентов.\n\n"\
                        "Данная работа - {FFD700}командная{FFFFFF}, работать на ней можно только в паре с напарником.\n"\
                        "Для поиска напарника можно воспользоваться командой "BR"/collist\n\n"\
                        "{FFFFFF}Пара состоит из двух человек:\n"\
                        ""BR"1. {FFD700}Водитель - член команды, арендовавший автомобиль. Он никогда не покидает автомобиль,\n"\
                        "следит за обстановкой.\n"\
                        ""BR"2. {FFD700}Инкассатор - член команды, выполняющий роли пассажира в автомобиле\n"\
                        ...
                        ""BR"1. {FFD700}Инкассатор 1 класса{FFFFFF} - самый младший класс класс инкассатора, которыйможет взаимодействовать заказами\n"\
                        "с меньшим числом средств и арендовать {FFD700}Газель Инкассатора.\n"\
                        ""BR"2. {FFD700}Инкассатор 2 класса{FFFFFF} - средний класс инкассатора, который может взаимодействовать\n"\
                        "с заказами с минимальным и средним количеством средств, а также арендовать {FFD700}Газель Инкассатора{FFFFFF} или {FFD700}Volkswagen Transporter.\n"\
                        ""BR"3. {FFD700}Инкассатор 3 класса{FFFFFF} - самый высокий класс инкассатора, который может взаимодействовать со всеми заказами,\n"\
                        "а также арендовать {FFD700}Газель Инкассатора, {FFD700}Volkswagen Transporter{FFFFFF} или {FFD700}Mercedes-Benz V-klasse.\n\n"\
                        "Для открытия нового уровня необходимо зарабатывать {FFD700}навык{FFFFFF}, который\n"\
                        "выдается за каждый {FFD700}успешно выполненный заказ.{FFFFFF}.\n\n"\
                ...
                case 1:
                {
                    if(!player_collector[playerid][job_active])
                    {
                        ...
                        (
                            ...
                            ""BR"1. {FFFFFF}Иметь {FFD700}15 {FFFFFF}игровой уровень.\n"\
                            ""BR"2. {FFFFFF}Иметь {FFD700}лицензию на оружие.\n"\
                            ""BR"3. {FFFFFF}Иметь {FFD700}водительские права.\n"\
                            "{FFFFFF}Если Вы готовы трудоустроиться, нажмите {FFD700}Далее{FFFFFF}.",
                            "Далее", "Закрыть"
                ...
                {
                    if(!player_collector[playerid][job_active])
                     return SendClientMessage(playerid, -1, ""USC"Сначало начните {FFD700}рабочий день.");
                    ...
                    switch(player_collector[playerid][c_quest_progress_1])
                    {
                        ...
                        case 1:format(quest_1, sizeof quest_1, "{FFD700}Выполняется");
                    ...
                    switch(player_collector[playerid][c_quest_progress_2])
                    {
                        ...
                        case 1:format(quest_2, sizeof quest_2, "{FFD700}Выполняется");
                    ...
                    switch(player_collector[playerid][c_quest_progress_3])
                    {
                        ...
                        case 1:format(quest_3, sizeof quest_3, "{FFD700}Выполняется");
                ...
                case 4:
                {
                    ...
                    switch(player_collector[playerid][rang_collector])
                    {
                        case 1:
                        {
                            format(exp, sizeof exp, "{FFD700}3. {FFFFFF}Необходимое количество навыков для открытия следующего класса: {FFD700}%d/%d{FFFFFF}\n",
                        ...
                        case 3:
                        {
                            format(exp, sizeof exp, "{FFD700}3. {FFFFFF}Достигнут максимальный уровень инкассатора\n");
                    ...
                    (
                        text, sizeof text, 
                        "{FFD700}1. {FFFFFF}Занимаемая должность: {FFD700}Инкассатор %d класса{FFFFFF}\n"\
                        "{FFD700}2. {FFFFFF}Текущий навык инкассатора: {FFD700}%d{FFFFFF}\n"\
                        "%s\n"\
                        "{FFD700}4. {FFFFFF}Доступные заказы:\n"\
                        "%s",
    ...
    if(dialogid == 1810)
    {
        if(response)
        {
            ...
            format(text, sizeof text, ""SC"Вы успешно взяли заказ {FFD700}№%d{FFFFFF}, отправляйтесь на отмеченнную метку на GPS.", id + 1);
    ...
    if(dialogid == 1812)
    {
        if(response)
        {
            ...
            {
                SendClientMessage(playerid, -1, ""SC"Вы начали работу {FFD700}инкассатора{FFFFFF}. Найдите напарника при помощи {FFD700}/colinv [ID]");
            ...
            (
                ...
                "{FFD700}Инкассатор{FFFFFF} - сотрудник банка, занимающийся транспортировкой денежных средств\n"\
                "и других драгоценностей по нуждам банка или его клиентов\n\n"\
                "Для трудоустройства инкассатором Вам необходимо иметь {FFD700}15 игровой уровень{FFFFFF}\n"\
                "и {FFD700}лицензию на оружие{FFFFFF}. Помните, что инкассация это командная работа:\n"\
                "перед началом работы Вам необходимо найти напарников и принять заказ вместе с ними. \n"\
                "Более подробно с работой Вы сможете ознакомиться в любом из {FFD700}Банков{FFFFFF} или\n"\
                "в {FFD700}списке команд {FFFFFF}(/mn >> Список команд).\n\n"\
                "Если Вы готовы трудоустроиться нажмите - {FFD700}далее.",
                "Далее",
                "Отмена"
            );
        ...
        if(response)
        {
            ...
            switch(listitem)
            {
                ...
                case 1:
                {
                    ...
                    (
                        text, sizeof text,
                        "{FFD700}1. {FFFFFF}Название заказа: {FFD700}%s{FFFFFF}\n"\
                        "{FFD700}2. {FFFFFF}Начальная точка заказа: {FFD700}%s{FFFFFF}\n"\
                        "{FFD700}3. {FFFFFF}Конечная точка заказа: {FFD700}%s{FFFFFF}\n"\
                        "{FFD700}4. {FFFFFF}Расстояние между точками: {FFD700}%d\n"\
                        "{FFD700}4. {FFFFFF}Напарник: {FFD700}%s",
    ...
    if(dialogid == 1815)
    {
        if(response)
        {
            if(GetPVarInt(playerid, "c_q_d_1"))
            {
                ...
                SendClientMessage(playerid, -1, ""SC"Вы начали выполнять задание {FFD700}\"Вместе веселеe\"");
            ...
            else if(GetPVarInt(playerid, "c_q_d_2"))
            {
                ...
                SendClientMessage(playerid, -1, ""SC"Вы начали выполнять задание {FFD700}\"Достойный инкассатор\"");
            ...
            else if(GetPVarInt(playerid, "c_q_d_3"))
            {
                ...
                SendClientMessage(playerid, -1, ""SC"Вы начали выполнять задание {FFD700}\"Это высокая позиция\"");
            ...
            switch(listitem)
            {
                case 0:
                {
                    ...
                    (
                        ...
                        "Ваша задача: {FFD700}выполнить разные заказы не менее чем с двумя напарниками.\n"\
                        "{FFFFFF}Награда: {FFD700}5.000 рублей.",
                        "Далее", "Назад"
                    );
                ...
                case 1:
                {
                    ...
                    (
                        ...
                        "Ваша задача: {FFD700}выполнить не менее 5 заказов за день.\n"\
                        "{FFFFFF}Награда: {FFD700}5.000 рублей.",
                        "Далее", "Назад"
                    );
                ...
                case 2:
                {
                    ...
                    (
                        ...
                        "Ваша задача: {FFD700}заработать 10 очков рейтинга за 24 часа\n"\
                        "Напарники могут быть разными.{FFFFFF}Награда: {FFD700}10.000 рублей.",
                        "Далее", "Назад"
                    );
...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    ...
    if(player_collector[playerid][order_collector] != -1)
    {
        ...
        if(areaid == orders_collectors[order][order_sphere_start])
        {
            ...
            if(orders_collectors[order][collector_id_collector] != playerid)
                return SendClientMessage(playerid, -1, ""USC"Водитель {FFFFFF}не может брать {FFD700}денежные средства.");

            if(IsPlayerInAnyVehicle(playerid))
                return SendClientMessage(playerid, -1, ""USC"Выйдите из транспорта чтобы взять{FFD700} денежные средства.");

            SendClientMessage(playerid, -1, ""SC" Вы успешно взяли денежные средства. Отправляйтесь в {FFD700}инкассационный автомобиль.");
        ...
        if(areaid == orders_collectors[order][order_sphere_end])
        {
            ...
            SendClientMessage(playerid, -1, ""SC" Заказ успешно выполнен, отправляйтесь к {FFD700}Банку {FFFFFF}для просмотра новых заказов.");
            SendClientMessage(partner, -1, ""SC" Заказ успешно выполнен, отправляйтесь к {FFD700}Банку {FFFFFF}для просмотра новых заказов.");
  ...
  //          if(g_doubling[DOUBLING_WAGE]) salary = salary * 2;
            ...
            ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы успешно доставили груз", "");
            ShowNotificationNew(partner, 1, 6, 0, 0, "Вы успешно доставили груз", "");
            ...
            if(player_collector[playerid][c_quest_progress_1] ||player_collector[partner][c_quest_progress_1])
            {
                if(player_collector[playerid][c_quest_progress_1])
                {
                    ...
                    format(text, sizeof text, "Вы выполнили {FFD700}%d{FFFFFF} из {FFD700}2{FFFFFF} для выполнения задания {FFD700}Вместе веселее",
                    player_collector[playerid][c_quest_count_1]);
                    SendClientMessage(playerid, -1, text);
                } 
                ...
                if(player_collector[playerid][c_quest_count_1] >= 2 ||player_collector[partner][c_quest_count_1] >= 2)
                {
                    if(player_collector[playerid][c_quest_count_1] >= 2)
                    {
                        ...
                        format(text, sizeof text, "Вы успешно выполнили ежедневное задание {FFD700}Вместе веселее",
                        player_collector[playerid][c_quest_count_1]);
                        SendClientMessage(playerid, -1, text);
                        SendClientMessage(playerid, -1, ""SC" Вам начислено {FFD700}5000{FFFFFF} рублей за выполнение задания.");
                    }
                    if(player_collector[partner][c_quest_count_1] >= 2)
                    {
                        ...
                        player_collector[partner][c_quest_count_1]);
                        SendClientMessage(partner, -1, text);
                        SendClientMessage(partner, -1, ""SC" Вам начислено {FFD700}5000{FFFFFF} рублей за выполнение задания.");  
                    } 
                }
            }
            ...
            if(player_collector[playerid][c_quest_progress_2] ||player_collector[partner][c_quest_progress_2])
            {
                if(player_collector[playerid][c_quest_progress_2])
                {
                    ...
                    format(text, sizeof text, "Вы выполнили {FFD700}%d{FFFFFF} из {FFD700}5{FFFFFF} для выполнения задания {FFD700}Достойный инкассатор",
                    player_collector[playerid][c_quest_count_2]);
                    SendClientMessage(playerid, -1, text);  
                } 
                ...
                if(player_collector[playerid][c_quest_count_2] >= 5 ||player_collector[partner][c_quest_count_2] >= 5)
                {
                    if(player_collector[playerid][c_quest_count_2] >= 5)
                    {
                        ...
                        format(text, sizeof text, "Вы успешно выполнили ежедневное задание {FFD700}Достойный инкассатор",
                        player_collector[playerid][c_quest_count_1]);
                        SendClientMessage(playerid, -1, text);
                        SendClientMessage(playerid, -1, ""SC" Вам начислено {FFD700}5000{FFFFFF} рублей за выполнение задания.");
                    }
            ...
            SendClientMessage(playerid, -1, ""SC"Вам был начислен {FFD700}1 {FFFFFF}опыт инкассатора.");
            SendClientMessage(partner, -1, ""SC"Вам был начислен {FFD700}1 {FFFFFF}опыт инкассатора.");
            ...
            if(player_collector[playerid][c_quest_progress_3] || player_collector[partner][c_quest_progress_3])
            {
                if(player_collector[playerid][c_quest_progress_3])
                {
                    ...
                    format(text, sizeof text, "Вы выполнили {FFD700}%d{FFFFFF} из {FFD700}10{FFFFFF} для выполнения задания {FFD700}Это высокая позиция",
                    player_collector[playerid][c_quest_count_3]);
                    SendClientMessage(playerid, -1, text);  
                } 
                ...
                if(player_collector[playerid][c_quest_count_3] >= 5 ||player_collector[partner][c_quest_count_3] >= 5)
                {
                    if(player_collector[playerid][c_quest_count_3] >= 5)
                    {
                        ...
                        format(text, sizeof text, "Вы успешно выполнили ежедневное задание {FFD700}Это высокая позиция",
                        player_collector[playerid][c_quest_count_1]);
                        SendClientMessage(playerid, -1, text);
                        SendClientMessage(playerid, -1, ""SC" Вам начислено {FFD700}10000{FFFFFF} рублей за выполнение задания.");
                    }
                    if(player_collector[partner][c_quest_count_3] >= 5)
                    {
                        ...
                        player_collector[partner][c_quest_count_1]);
                        SendClientMessage(partner, -1, text);
                        SendClientMessage(partner, -1, ""SC" Вам начислено {FFD700}10000{FFFFFF} рублей за выполнение задания.");
                    } 
                }
            }            
        }
...
public OnPlayerExitVehicle(playerid, vehicleid)
{
    if(player_collector[playerid][order_collector] != -1)
    {
        ...
        {  
            ...
            {
                SendClientMessage(playerid, -1, ""SC" У вас есть {FFD700}180 {FFFFFF}секунд чтобы сесть обратно в транспорт.");
            ...
            {
                SendClientMessage(playerid, -1, ""SC" У вас есть {FFD700}60 {FFFFFF}секунд чтобы сесть обратно в транспорт.");
...
stock LoadCollectorJob()
{
    ...
    CreateDynamic3DTextLabel("{FFD700}Начальник службы инкассации\n{FFFFFF}Подойдите для {FFD700}взаимодействия", -1, place_bank[0], place_bank[1], place_bank[2], 10.0);
    ...
    for(new i;i < 4;i++)
    {
        ...
        CreateDynamic3DTextLabel("{FFD700}Начальник службы автопарка\n{FFFFFF}Подойдите для {FFD700}взаимодействия", -1, coord_arenda_c[i][0], coord_arenda_c[i][1], coord_arenda_c[i][2], 10.0);
...
CMD:colinv(playerid, params[])
{
    ...
    if(player_collector[playerid][order_collector] != -1) 
        return ShowNotificationNew(playerid, 2, 6, 0, 0, "Сначала завершите текущий заказ.", "");
    ...
    if(player_collector[playerid][player_partner] != -1)
    {
        ...
        player_collector[playerid][player_partner] = -1;
        ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы отказались от своего напарника.", "");
        
    }

    if(sscanf(params, "u", params[0]))
    return ShowNotificationNew(playerid, 2, 6, 0, 0, "Используйте '/colinv' [id игрока].", "");

    if(!IsPlayerLogged(partner) || !IsPlayerConnected(partner)) 
        return ShowNotificationNew(playerid, 2, 6, 0, 0, "Не игрока с данным ID.", "");

    if(!player_collector[partner][job_active])
        return ShowNotificationNew(playerid, 2, 6, 0, 0, "Игрок не работает в инкассации.", "");

    if(!player_collector[playerid][job_active])
        return ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы не искассатор.", "");

    new Float:x, Float:y, Float:z;
    GetPlayerPos(partner, x, y, z);
    new Float:distance = GetPlayerDistanceFromPoint(playerid, x, y, z);

    if(distance >= 7.0) return ShowNotificationNew(playerid, 2, 6, 0, 0, "Игрок слишком далеко от вас.", "");

    if(player_collector[partner][player_partner] != -1)
        return ShowNotificationNew(playerid, 2, 6, 0, 0, "У игрока уже есть напарник.", "");

    new text[52];
    format(text, sizeof text, "%s приглашает вас в напарники, введите /joincoll", GetPlayerNameEx(playerid));

    ShowNotificationNew(partner, 2, 6, 0, 0, text, "");
...
public:UpdateOrderListCollector()
{
    ...
    else
    {
        ...
        foreach(new i : Player)
        {
            ...
            SendClientMessage(i, -1, "{FFD700}| [Диспетчер]: В список заказов добавлен новый заказ: {FFD700}/blist");
        }
    }
...
stock load_order_colllector(id)
{
    ...
    else
    {
        ...
        orders_collectors[id][order_e_3dtext] = CreateDynamic3DTextLabel("{FFD700}Ценный груз\n{FFFFFF}Подойдите для взаимодействия", -1, place_safe_bank[0], place_safe_bank[1], place_safe_bank[2], 
        6.0, _, _, _, world_e, 1);
    } 
