...
#define USC             "{D4544B}Подсказка: {ffffff}"
...
public OnGameModeInit()
{
    ...
    CreateActorEx("{D4544B}Работа МЧС", "{FFFFFF}Подойдите для взаимодействия", 267, -2615.917724,-282.265045,1252.043823,265.929382, 1, 1);
    ...
    CreateDynamic3DTextLabel("Вход в МЧС", 0xD4544BFF, -2558.123535,-284.654937,27.354429, 5.0);
    CreateDynamic3DTextLabel("Выход из МЧС", 0xD4544BFF, -2600.141113,-283.324401,1246.680419, 5.0);
...
public OnPlayerEnterDynamicArea(playerid, areaid)
{
    ...
    if(areaid == npc_mchs_1)
    {
        ...
        (
            ...
            "{00CC00}Начать{FFFFFF}/{D4544B}закончить смену\n"\
    ...
    {
        if(!player_mchs_active{playerid}) ShowNotificationNew(playerid, 2, 6, 0, 0, "Для начала начните смену.", "");
        Dialog(playerid, 3502, DSM, "Инвентарь МЧС", "Вы хотите взять огнетушитель?", "Взять", "Отмена");
    }
    if(areaid == veh_mchs)
    {
        if(!player_mchs_active{playerid}) return ShowNotificationNew(playerid, 2, 6, 0, 0, "Сначала начните смену в МЧС.", "");
...
public OnDialogResponse(playerid, dialogid, response, listitem, inputtext[])
{
            ...
            switch(listitem)
            {
                case 0:
                {
                    ...
                    else
                    {
                        ...
                        ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы начали смену. Возьмите огнетушитель и Т/С.", "");
                    }
                }
                case 1:
                {
    ...
    if(dialogid == 3502)
    {
        if(response)
        {
            ...
            ShowNotificationNew(playerid, 2, 6, 0, 0, "Вы взяли огнетушитель.", "");
        }
    }
    if(dialogid == 3503)
    {
        if(response)
        {
            ...
            ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы успешно взяли рабочее Т/С.", "");
            ShowNotificationNew(playerid, 2, 6, 0, 0, "Введите /meslist для просмотра доступных вызовов.", "");
        }
    }
...
public OnPlayerKeyStateChange(playerid, newkeys, oldkeys)
{
    if(PRESSED(KEY_FIRE))
	{
        if(press_fire{playerid}) return ShowNotificationNew(playerid, 2, 6, 0, 0, "Продолжайте тушить очаг возгорания.", "");
			...
			if(IsPlayerInRangeOfAnyFire(playerid) >= 1)
			{
                ...
                if(all_fire[IsPlayerInRangeOfAnyFire(playerid)][F_ID_Z] == player_order[playerid])
                {
                    ...
                    { 
                       ...
                       ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы потушили очаг возгорания.", "");

                        if(Order(id))
                        {
                            ShowNotificationNew(playerid, 1, 6, 0, 0, "Вы успешно выполнили заказ.", "");
                ...
                else ShowNotificationNew(playerid, 2, 6, 0, 0, "Данный очаг возгорания не вашего заказа.", "");
			}
			else ShowNotificationNew(playerid, 2, 6, 0, 0, "Не советуем использовать огнетушитель просто так.", "");
		}
	}
...
{
    if(!player_mchs_active{playerid}) return ShowNotificationNew(playerid, 2, 6, 0, 0, "Вы не работаете в МЧС.", "");
    new Cache:result;
    ...
    if(!rows) return ShowNotificationNew(playerid, 2, 6, 0, 0, "На данный момент заказов нет, ожидайте {FFFF00}'PayDay'", "");

    if(player_order[playerid]) return ShowNotificationNew(playerid, 2, 6, 0, 0, "Вы уже взяли заказ.", "");
