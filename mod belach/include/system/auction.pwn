...
CMD:auction(playerid)
{
    ...
    if(!(8 <= hour <= 21))
        return SendClientMessage(playerid, -1, ""USC"Аукцион работает с 8:00 до 22:00");

    // Доступ к аукциону с 3 уровня
    if(GetPlayerLevel(playerid) < 3)
        return SendClientMessage(playerid, -1, ""USC"Доступ к аукциону доступен с 3 уровня.");
    ...
    if(GetPlayerVirtualWorld(playerid) != AUCTION_WORLD)
    {
        EnablePlayerGPS(
            ...
            -2283.999511,
            23.101566,
            "Местоположение аукциона отмечено на карте."
        );
        return 1;
    }
...
stock Auction_EnsureServiceBusinesses()
{
    ...
    Auction_RegisterServiceBusiness(1, "Шиномонтаж", BUSINESS_TYPE_TIRE_SHOP, 2500000, 1731.149414, 2447.544921, 14.979245, 290.826629);
...
stock AuctionBusiness(sql_id, slot)
{
    ...
    foreach(new i : Player)
    {
        ...
        SendClientMessage(i, 0x66CC00FF, "Напишите {0099FF}/business {66CC00}для управления бизнесом.");
        break;
    }
    return 1;
}
...
public:LoadBusinessToAuction()
{
    ...
    for(new row; row < rows; row++)
    {
        ...
        format(auction_slot[slot][OPISANIE_SLOT], 124, "Государственный бизнес #%d", businessid);
